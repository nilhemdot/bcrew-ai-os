...
- outbound email content should use the branded `buildAgentFeedbackEmail` template and the delegated Gmail MIME sender (`sendGmailMessage`), not the generic Gmail connector body field. V1 email must send real `text/html` with a readable plain-text fallback, short human copy, no technical AIOS/ClickUp writeback explanation, and the signed feedback URL hidden behind a `Start check-in` button in HTML.
...
- v1 surfaces:
  ...
  - per-agent day-30/day-60/day-90 cards once Real Start Date makes a feedback checkpoint due and still inside the 15-day send window
  - reminder cards when a checkpoint is Requested but not Completed
  - one grouped cleanup card when old 30/60/90 checkpoints are outside the send window and should be skipped
