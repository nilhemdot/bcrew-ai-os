...
- Foundation hardening after the Apr 27-28 Strategy detour: surface-freshness sweep, operator-readable build log, and a cleaner tomorrow command order
...
- `FOUNDATION-SWEEP-001` is the next Foundation build.
- `FOUNDATION-CHANGELOG-002` follows so Steve can see what was built, where it lives, and what needs review without reading git logs.

Current command order:

1. Build `FOUNDATION-SWEEP-001`: page/source/job/backlog freshness sweep and verifier guards.
2. Build `FOUNDATION-CHANGELOG-002`: operator-readable Recent Builds with card/proof/status links.
