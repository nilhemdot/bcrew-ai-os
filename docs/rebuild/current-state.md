...
| Owners Admin package | Open now | `SRC-OWNERS-001` is signed off for meaning and now visibly carries the signed-off Owners workbook tabs: Admin, Split Cal, Agent Splits, Listings and Conditional Deals, Sales & Deposit, Goal & KPI Calculator, and CI Report. `SRC-OWNERS-LISTS-001` is signed off for current-reality meaning after the imported `Lists` mirror repair: it owns the governed list/dropdown write target, Admin `N/P/S` validations are repointed, and delegated writes are guarded from the mirror range. First cross-system proof exists: Owners row -> FUB person -> ClickUp roster -> Drive contract package. Row-scoped review runners and `/api/owners/review-queue` exist. | Close `SOURCE-008`, `DATA-005`, `DATA-006`, `DATA-007`, `DATA-008`, `DATA-009`, supported by `FINANCE-002`; keep `npm run sheets:verify` green after sheet-source changes. | Reuse freshness and review patterns on more governed sources. |
