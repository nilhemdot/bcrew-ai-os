...
- Foundation Phase G source lifecycle completion and Recent Work executive UI before broader feature lanes resume
...
20. Remaining Phase G work is still not done: comprehensive changelog, daily executive summary, and source lifecycle expansion. Do not un-pause Scoper, dev intelligence, agent-managed backlog work, Strategy UI, corpus expansion, source lifecycle expansion, or broader agent work by default without the next approved Foundation plan. Next expected card is `CHANGE-LOG-COMPREHENSIVE-001`.
...
| Recent Work | Operator changelog. | Recent Builds executive UI is now live for v1: collapsed closeout cards, review-next queue, proof, known limits, where-it-lives, owning cards, context cards, and same-commit groups that remain individually reviewable. Comprehensive changelog is still a separate future card. |
