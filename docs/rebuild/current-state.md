...
| Shared communications intelligence | Open now | `SRC-GMAIL-001`, `SRC-GCAL-001`, `SRC-MISSIVE-001`, `SRC-SLACK-001`, and `SRC-MEETINGS-001` are now verified readable. The current repo can now read Gmail, Calendar, Missive inbox/thread/comment context, Slack channels and thread history for channels the existing bot is actually in, scan Google Meet artifacts across the enabled delegated BCrew user list with paginated Drive reads, detect standalone transcript docs or transcript sections embedded inside Gemini notes, archive Gmail threads, Missive threads, Slack threads, meeting-note artifacts, and meeting-transcript artifacts into Postgres, mirror organized meeting copies into Crewbert Drive in copy mode, tag meetings `broadcast` vs `discussion`, report transcript gaps, and surface governed candidate lanes for meetings, Gmail, Missive, and Slack with Foundation context plus the first apply paths. Current archive depth is `1,103` Gmail threads, `1,248` Missive threads, `1,371` Slack threads, and `1,498` meeting artifacts. The historical lane includes recovered Zoom chat context back to `2024-10-03`; meeting transcripts currently reach back to `2025-03-12`. A first batch synthesis proof now exists at `docs/handoffs/2026-04-23-shared-comms-synthesis-v1.md`, but it is not yet a persistent operating layer. What remains open is durable cursor-based Gmail/Missive backfill control, cross-source normalization, the last channel-rollout gaps, the full subject-person privacy/query layer, source-backed fact grounding, broader apply/read flows, and the synthesis layer that decides what is still live versus already resolved. | Close `SOURCE-019`, `SOURCE-020`, `COMMS-BACKFILL-001`, `SYNTHESIS-FACTS-001`, and `SYNTHESIS-ENGINE-001` by stabilizing the archive, adding durable backfill state, grounding synthesis in strategy/KPI/finance/source facts, then promoting the first ranked synthesis output into a repeatable strategy/leadership packet. | Keep widening extraction types, apply flows, and channel coverage after the first governed paths stay stable. |
