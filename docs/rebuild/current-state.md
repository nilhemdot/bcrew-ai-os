...
| Shared communications intelligence | Open now | `SRC-GMAIL-001`, `SRC-GCAL-001`, `SRC-MISSIVE-001`, and `SRC-MEETINGS-001` are now verified readable. The current repo can now read Gmail, Calendar, Missive inbox/thread/comment context, scan Google Meet artifacts across the enabled delegated BCrew user list, detect standalone transcript docs or transcript sections embedded inside Gemini notes, archive Gmail threads, Missive threads, meeting-note artifacts, and meeting-transcript artifacts into Postgres, and surface a first governed candidate lane from transcript-first extraction with Foundation context and a DB-backed people directory. What remains open is cross-source normalization, better candidate approval/apply flow, subject-person privacy controls, and widening the governed extraction model beyond this first meeting slice. | Close `SOURCE-019` and `SOURCE-020` by extending the now-live shared archive pattern across more shared-communications surfaces, then add the next governed extraction path on top of that normalized layer. | Then revalidate Slack and widen extraction types after the first governed path is stable. |
