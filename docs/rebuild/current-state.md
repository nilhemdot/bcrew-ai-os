...
| Shared communications intelligence | Open now | `SRC-GMAIL-001`, `SRC-GCAL-001`, `SRC-MISSIVE-001`, and `SRC-MEETINGS-001` are now verified readable. The current repo can now read Gmail, Calendar, Missive inbox/thread/comment context, search Google Gemini notes, list the meeting-note archive root, export note docs to text, archive meeting-note artifacts into Postgres, and surface a first pending task-candidate lane from archived `Next steps` evidence. What remains open is cross-source normalization and widening the governed extraction model beyond this first meeting-note slice. | Close `SOURCE-019` and `SOURCE-020` by extending the now-live meeting-note archive pattern across more shared-communications surfaces, then add the next governed extraction path on top of that normalized layer. | Then revalidate Slack and widen extraction types after the first governed path is stable. |
