...
- memory/retrieval/synthesis spine through governed synthesis: job ledger, old-system salvage contract, atoms/report artifacts, lexical retrieval, pgvector semantic retrieval, hybrid evidence API, source-backed facts, and governed synthesized items with fact/evidence/chunk provenance
...
| Strategy packet | Done for current reality; active hub paused | `SRC-STRATEGY-001` is signed off. Freedom Community, BHAG, Agent Engine, and the strategy-used Owners slice are captured for current-reality strategy use. `FOUNDATION-001` and `SOURCE-014` are closed for this phase. Strategy Evidence Packet and Strategy Advisor v1 proved useful source material and live goal/operating truth, but they also proved the risk of running strategy synthesis before the memory/retrieval/action spine. The active Strategic Execution surface is now a Strategy Hub v2 safety stub; the advisor endpoint returns `strategy_hub_v2_in_progress`, old active 90-day priority generation is disabled, and verifier coverage blocks the old recommendation surface from returning. Live Goal Truth and Live Operating Truth remain available as source-backed APIs for future v2 work. The memory spine now has `INTEL-JOBS-001`, accepted `REPORT-MINING-001`, `INTEL-ATOM-001` v1 report/atom/hit proof, `RETRIEVAL-001` candidate-backed lexical chunks/search, `RETRIEVAL-002` pgvector semantic retrieval, `RETRIEVAL-003` hybrid evidence retrieval with explicit `maxTier`, `SYNTHESIS-FACTS-001` persisted source-backed fact grounding in `intelligence_synthesis_facts`, and `SYNTHESIS-ENGINE-001` governed synthesized items with fact/evidence/chunk provenance. | No source sign-off closeout work remains. Next work is not Strategy Hub review/promote UI or chat polish. Build Action Router. | Later hardening: Freedom drift monitoring, source-backed value hardening, decision provenance, temporal history, privacy/tier filtering, and full Strategy Hub after runtime/router/extraction/synthesis/action-routing are stable. |
