...
13. `GEMINI-WORKSPACE-EYES-ROUTE-PROOF-001` - Gemini Workspace browser eyes route proof is closed under `gemini-workspace-eyes-route-proof-v1`; it proves the logged-in Gemini web app can return structured visual evidence and build candidates for one exact approved public video without the Gemini API key. It is Level 1 experimental with Gemini API fallback and must pass a 3-video pilot before Mark last-50 scale-up.
...
9. `MARK-KASHEF-LAST-50-BASELINE-001` - active next card. Start with a guarded 3-video subscription-eyes pilot using Gemini Workspace browser eyes first and Gemini API fallback; process all 50 only after pilot quality, quota, fallback, and guard checks pass.
