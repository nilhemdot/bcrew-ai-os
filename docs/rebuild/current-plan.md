...
Guardrail: no Strategy UI, Scoper, Agent Factory, corpus expansion, retry/backoff expansion, or new feature lane starts by default. Phase 1 enforcement and the remaining Foundation control layer are now in place; the exact next step is review-only, then Steve chooses the next plan gate: review-queue cleanup or Phase G Track 2.
