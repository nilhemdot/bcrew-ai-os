...
- `shared-comms-intelligence-bite` is a manual composite job for strategy prep: one bounded Gmail candidate bite, one bounded Missive candidate bite, then one synthesis run. Meeting transcript backlog mining is now separated into its own long-window subscription-first job because live proofs showed short API-style timeouts are the wrong runtime shape for large transcript extraction.
...
- Remaining Phase 3 gap: monitor scheduled Missive/Gmail runs, prove partial failure on a real failed item, extend retry semantics to Drive/video lanes, tune subscription-route miners with long windows and pacing, schedule daily synthesis only after monitored router proof, and build review/export gates before broad backfill.
