...
May 25 latest-20 full-watch runner result: `YOUTUBE-LATEST-20-FULL-WATCH-RUNNER-001` is implemented as the guarded runtime bridge after the latest-20 manifest. Default proof is read-only and selects the current 9-video non-Mark batch. Apply mode requires `--apply --live-gemini-api`, uses Gemini API full video/audio/visual watching, captures YouTube page/description evidence, runs the resource-link resolver for every video, blocks unresolved public links before Scoper readiness, persists proposal-only report/atoms/hits, and does not auto-create backlog cards or write externally.
...
11. `YOUTUBE-LATEST-20-FULL-WATCH-RUNNER-001` - V1 runner/proof built. The current selected 9-video non-Mark batch is ready for guarded `--apply --live-gemini-api` full-watch extraction with resource-link resolver packets.
