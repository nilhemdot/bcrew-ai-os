...
- `shared-comms-intelligence-bite` is a manual synthesis-only job for strategy prep. It reads already-mined candidates and records a fresh brief with a long subscription-route timeout. Gmail, Missive, and meeting transcript extraction now run as separate paced subscription miners so slow extraction calls do not block user-facing intelligence.
