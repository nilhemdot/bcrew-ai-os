...
May 28 fallback recovery policy: browser fallback plans now include bounded self-recovery before human escalation. The agent can retry clean isolated source-browser state, use source-specific session first where required, then escalate through the operator assistant/texting lane only after retries fail, auth/2FA is needed, or real source content still cannot be proven. This is policy/readback only; it does not send texts, solve challenges, log in, buy, download, post, or mutate credentials.

May 28 Harlan escalation bridge: stuck source-browser fallback/auth-needed plans now prepare a Harlan operator escalation draft using the existing dry-run auth-escalation contract. Dev/source readback can show `Harlan draft ready`, the packet waits for `DONE`, and proof keeps `sendsMessageNow=false`; real texting, hosted-browser fallback, source-session resume, login, signup, challenge solving, purchases, downloads, posting, messaging, and credential mutation remain approval-bound follow-up work.
