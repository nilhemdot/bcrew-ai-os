...
## Current Sprint: Extract Retire

Live sprint ID: `extract-retire-2026-05-16`.

This sprint is complete under `extract-retire-v1`. `EXTRACT-RETIRE-001` adds a bounded source-crawl retirement policy: historical/backfill/corpus targets that hit a configured clean zero-work threshold become `complete` and `paused`, while current-day lanes stay scheduled. It used synthetic store dogfood only; it did not run extraction, change retry/backoff execution, add sources, change connector auth, build hubs, wire Marketing Video Lab, mutate Canva assets, work Meeting Vault Phase B, or mutate Drive permissions.
...
1. `CRAWL-RUN-LEDGER-001` - done under `crawl-run-ledger-reconcile-v1`. Scope was proof and closeout only: focused `source_crawl_target_runs` / `crawlRunId` dogfood, package-wired focused proof, extraction-runtime verifier coverage, rebuild doc update, and backlog/current-sprint closeout.
2. `RUNTIME-HEALTH-SIMPLIFY-001` - done under `runtime-health-simplify-v1`. Scope was Runtime Health readability only: top command panel, attention-only runtime items, direct jumps to proof sections, collapsed diagnostics by proof area, focused dogfood that rejects the old wall-of-diagnostics shape, runtime reliability verifier coverage, focused proof 16/16, and full Foundation verification 401/401 before closeout.
3. `RUNTIME-FIRST-JOBS-001` - done under `runtime-first-jobs-v1`. Scope was no-auth runtime repair only: restore `leaseSourceCrawlTarget` / `finishSourceCrawlTargetRun` delegates, prove `gmail-current-day` and `missive-current-day` dry-runs do not lease targets or spawn child sync commands, add focused runtime reliability verifier coverage, and close with full Foundation verification at 400/400.
4. `FOUNDATION-IDENTITY-001` - done under `foundation-identity-surface-v1`. V1 exposes metadata-only workspace identity in System Inventory without copying private memory into repo truth.
5. `RUNTIME-SUPERVISOR-001` - done under `runtime-supervisor-v1`. V1 owns `lib/runtime-process-control.js` service-supervisor builders, `server.js` LaunchAgent wiring, `public/foundation-runtime-renderers.js` Runtime Health supervised-service rendering, `lib/foundation-runtime-reliability-verifier.js` coverage, focused proof `scripts/process-runtime-supervisor-check.mjs`, and closeout `docs/handoffs/2026-05-16-runtime-supervisor-closeout.md`.
6. `RUNTIME-WORKER-001` - done under `runtime-worker-reliability-v1`. V1 owns `lib/foundation-worker-reliability.js`, shared worker dry-run parsing, one-shot dry-run runtime-status guard, `workerReliability` job snapshots, Runtime Health worker reliability rendering, runtime reliability verifier coverage, focused proof `scripts/process-runtime-worker-check.mjs`, and closeout `docs/handoffs/2026-05-16-runtime-worker-reliability-closeout.md`.
