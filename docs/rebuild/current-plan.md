...
24. `DEV-BUILD-OPPORTUNITY-SCOPER-001` - convert Director recommendations into proposal-only scoped build candidates only after source baseline/parity state is visible, or mark the scope incomplete because source catch-up is still pending.
25. `BUILD-PORTFOLIO-SCRUM-MASTER-001` - after Scoper produces enough implementation detail, merge/dedupe/enhance related scoped candidates into stronger source-backed build opportunities, assign portfolio scores/ranks, and propose queue order before Steve approval.
