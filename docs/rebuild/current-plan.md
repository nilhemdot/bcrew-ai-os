...
19. Hard-checkpoint backlog reconciliation: `FOUNDATION-PLAN-RECONCILE-001` is done for v1. It promoted the hard-checkpoint sprint plan into backlog truth before Phase G Track 2 starts. Remaining scoped Tier 0 follow-ups are `PERSONAL-WORKSPACE-BOUNDARY-001`, `CEO-DASHBOARD-PATTERN-001`, `DOCTRINE-PROPAGATION-003`, and `DECISION-AUTO-EMIT-002`.
...
21. Gate reliability follow-up: `GATE-RELIABILITY-001` is scoped from the Phase 1 verification run. `process:foundation-ship` has bounded transient retry behavior, but raw `npm run foundation:verify` still hit a `deadlock detected` failure once before passing on rerun. This belongs in backlog truth before Phase G Track 2 so gate failures are reliable signals, not rerun roulette.
22. Foundation Phase G remaining scope: plain-English sweep, comprehensive changelog, Recent Builds executive UI, daily executive summary, source lifecycle expansion, and UI/menu/layout polish remain future scoped work. System Inventory archive/history docs must be hidden from the current-doc default view and exposed in a separate Archive/History section. No Strategy UI, Scoper, Agent Factory, corpus expansion, retry/backoff, or broader feature lane starts by default before Steve explicitly re-plans and approves the next Foundation slice.

Guardrail: no Strategy UI, Scoper, Agent Factory, corpus expansion, retry/backoff, or new feature lane starts by default. Phase 1 enforcement is now in place; continue the Foundation completion plan before Phase G Track 2 by planning the remaining hard-checkpoint cards and `GATE-RELIABILITY-001`.
