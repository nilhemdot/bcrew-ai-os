...
5. `INTEL-JOBS-001` -> `REPORT-MINING-001` -> `INTEL-ATOM-001` -> `RETRIEVAL-001` through `RETRIEVAL-003` — Build the memory/retrieval spine.
   ...
   - `INTEL-JOBS-001`, `REPORT-MINING-001`, and `INTEL-ATOM-001` are now done/hardened enough for the next gate.
   - Use `intelligence_report_artifacts`, `intelligence_atoms`, and `intelligence_atom_hits` as the governed report/atom substrate before scaling video/web/Skool extraction.
