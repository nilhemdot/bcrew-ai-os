...
May 28 Source Browser Agent scope addendum: the evidence-backed call is not to throw away the current browser/runtime work. `AGENT-CAPABILITY-REGISTRY-001` and `AGENT-TEMPLATE-RUNTIME-CONTRACT-001` are already closed, so `SOURCE-BROWSER-AGENTIC-RUNTIME-001` should become the first flagship source agent under that protocol. Existing pieces such as `source:god-mode`, `repo:deep-review`, newsletter intake, `skool:free-god-mode`, source packets, source session broker, and evidence artifacts become tools inside the agent. The missing build layer is the agent harness: exact source packet -> isolated source session -> observe -> plan -> safe action -> extract -> evaluate -> record evidence -> continue, stop, auth-needed, or fail closed. First harness slice now exists in `lib/source-browser-agent-harness.js` with focused proof `npm run process:source-browser-agent-harness-check -- --json`; it does not claim broad live extraction, but it now routes public, repo, newsletter, free Skool, MyICOR, blank-browser, and unsafe-action cases through the right runner/blocker path. The follow-up readback slice serializes agent plans into source-run metadata so Dev readback can show route, auth-needed, failed-closed, blocker, and side-effect truth. The current handoff slice now runs selected YouTube-discovered source rows through Source Browser Agent preflight before `source:god-mode`, `repo:deep-review`, or `skool:free-god-mode`, and persists the agent plan on source handoff crawl items. Scope note: [Source Browser Agent Protocol Scope](../source-notes/source-browser-agent-protocol-scope-2026-05-28.md).
