...
17. `EXTRACTOR-HANDS-BROWSER-RUNTIME-001` - P0. Build governed local Playwright-first browser Hands so approved extractors can open, inspect, click/navigate where explicitly allowed, preserve evidence, and stop at source-packet boundaries.
18. `SOURCE-PACKET-WORKER-RUNNER-001` - done under `source-packet-worker-runner-v1-1`. V1.1 synthetic proof exists for approved source-packet decision -> exact public-page worker -> artifact -> freshness/status -> `source_crawl_items` row -> `intelligence_report_artifacts` proof row, and Dev UI/queue routes show ready/already-run/blocked worker status. Keep broad crawling blocked; browser Hands click/navigation is the next separate card.
19. `BUILD-INTEL-LINK-APPROVAL-SOURCE-PACKETS-001` - P0. Turn approval-required links into exact source packets so approved resources, free communities, paid communities, public sales pages, and purchase candidates are handled deliberately.
20. `YOUTUBE-CREATOR-GOD-MODE-CATCHUP-001` - P0. Catch up all approved public YouTube creators through the highest available God Mode lane before major build promotion; baseline all creators, deepen S/A sources, and keep new releases current.
21. `SOURCE-FAMILY-GOD-MODE-EXTRACTORS-001` - P0. Show and harden God Mode maturity for YouTube, operator-excluded comments, long courses, GitHub, Skool, MyICOR, Drive/Meet training, Gmail/Missive, Slack, meetings, and system signals.
