...
21. `YOUTUBE-GOD-MODE-AUTONOMOUS-WATCH-SCHEDULER-001` - P0 support card. Make catch-up and daily deltas automatic after parity/budget rules are explicit.
22. `DEV-BUILD-OPPORTUNITY-SCOPER-001` - convert Director recommendations into proposal-only scoped build candidates only after source baseline/parity state is visible, or mark the scope incomplete because source catch-up is still pending.
23. `BUILD-PORTFOLIO-SCRUM-MASTER-001` - after Scoper produces enough implementation detail, merge/dedupe/enhance related scoped candidates into stronger source-backed build opportunities, assign portfolio scores/ranks, and propose queue order before Steve approval.
24. `BUILD-OPPORTUNITY-PROMOTION-GATE-001` - after Build Portfolio/Sprint Master produces a reviewed portfolio proposal, approve source-backed build opportunities into backlog cards or existing-card attachments.
25. `BUILD-INTEL-EXTRACTION-IMPLEMENTATION` - close the full Build Intel extraction loop only after the slices above are proven.
