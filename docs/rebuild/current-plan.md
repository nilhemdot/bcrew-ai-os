...
8. `MARK-KASHEF-LAST-50-BASELINE-001` - active next card. Process Mark's approved public last-50 baseline under the Eyes route, overnight guard, and subscription evidence-reasoning boundary.
9. `YOUTUBE-LATEST-20-INTEL-RUN-001` - process approved public last-20 videos for other creators only after the extraction mode is proven.
10. `DEV-TEAM-INTELLIGENCE-DIRECTOR-001` - generate the Director report/view with evidence, scores, suggested card, blockers, duplicate/stale checks, and next action.
11. `BUILD-OPPORTUNITY-PROMOTION-GATE-001` - approve source-backed build opportunities into backlog cards or existing-card attachments.
12. `BUILD-INTEL-EXTRACTION-IMPLEMENTATION` - close the full Build Intel extraction loop only after the slices above are proven.
...
Approval-gated continuation and parked Strategy boundary remain documented for `FOUNDATION-CONTROL-PLANE-TRUTH-CLEANUP-001`, `SKOOL-APPROVED-LESSON-EXTRACT-PROOF-001`, `MYICOR-APPROVED-LESSON-EXTRACT-PROOF-001`, `MARK-KASHEF-LAST-50-BASELINE-001`, `BUILD-INTEL-EXTRACTION-IMPLEMENTATION`, and `STRATEGY-003`.
