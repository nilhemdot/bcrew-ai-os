...
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_CARD_ID = 'SOURCE-MATURITY-FREEDOM-SHEET-ATOM-FLOW-REPAIR-001'
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_CLOSEOUT_KEY = 'source-maturity-freedom-sheet-atom-flow-repair-v1'
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_PLAN_PATH = 'docs/process/source-maturity-freedom-sheet-atom-flow-repair-001-plan.md'
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_APPROVAL_PATH = 'docs/process/approvals/SOURCE-MATURITY-FREEDOM-SHEET-ATOM-FLOW-REPAIR-001.json'
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-29-hot-doc-refresh/2026-05-18-source-maturity-freedom-sheet-atom-flow-repair-closeout.md'
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_SCRIPT_PATH = 'scripts/process-source-maturity-freedom-sheet-atom-flow-repair-check.mjs'
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_SPRINT_ID = 'source-maturity-freedom-sheet-atom-flow-repair-2026-05-18'

export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS = [
  {
    sourceId: 'SRC-FREEDOM-TEAM-001',
    factId: 'fact:a76eb15fe718d1eb8dda2a93',
    label: 'Freedom Team',
    atomTitle: 'Freedom Team current-reality source boundary',
    topicRefs: ['source-maturity', 'freedom-sheet', 'freedom-team', 'team-records', 'source-backed-current-reality'],
  },
  {
    sourceId: 'SRC-FREEDOM-COMMUNITY-REV-001',
    factId: 'fact:280b56f9991a417bc91ef4be',
    label: 'Freedom Community Revenue',
    atomTitle: 'Freedom Community Revenue current-reality source boundary',
    topicRefs: ['source-maturity', 'freedom-sheet', 'freedom-community-revenue', 'revenue-records', 'source-backed-current-reality'],
  },
]

export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID = SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS[0].sourceId
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_FACT_ID = SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS[0].factId

export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES = [
...
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_PROOF_COMMANDS = [
...
export const SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_CHANGED_FILES = [
...
function targetBySourceId(sourceId) {
  return SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS.find(target => target.sourceId === sourceId) || null
}
...
function targetFact(facts = [], factId = SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_FACT_ID) {
  return list(facts).find(fact => (fact.factId || fact.fact_id) === factId) || null
}

export function selectFreedomSheetAtomFlowRepairCandidate({
  beforeRow = {},
  sourceFacts = [],
  target = targetBySourceId(SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID),
} = {}) {
  const failures = []
  const sourceId = target?.sourceId || beforeRow.sourceId || SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  const factId = target?.factId || SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_FACT_ID
...
export function selectFreedomSheetAtomFlowRepairCandidates({
  beforeRows = [],
  sourceFacts = [],
  targets = SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS,
} = {}) {
  const candidates = targets.map(target => selectFreedomSheetAtomFlowRepairCandidate({
...
export function buildFreedomSheetAtomFromFact(candidate = {}, now = new Date().toISOString()) {
  const sourceId = candidate.sourceId || SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  ...
  const naturalKey = `source-maturity-freedom-sheet-atom-flow-repair:${sourceId}:${factId}`
  ...
  const atom = {
    ...
    evidenceExcerpt,
    derivedClaim: claim,
    topicRefs: candidate.target?.topicRefs || ['source-maturity', 'freedom-sheet', 'source-backed-current-reality'],
    ...
    acceptedAt: now,
    acceptedBy: 'codex-source-maturity-freedom-sheet-atom-flow-repair',
    reviewNote: 'Promoted from an existing governed Freedom Sheet source fact only; no live extraction, provider call, Sheets read/write, or external write occurred.',
    tags: ['source-maturity', 'atom-flow-repair', 'freedom-sheet', 'no-live-extraction'],
    metadata: {
...
export function buildFreedomSheetAtomsFromFacts(selection = {}, now = new Date().toISOString()) {
  const atomBuilds = list(selection.candidates).map(candidate => buildFreedomSheetAtomFromFact(candidate, now))
  return {
...
export function evaluateFreedomSheetAtomFlowRepair({
  beforeRow = {},
  afterRow = {},
  candidate = {},
  atomBuild = {},
  ...
  return {
    ...
    failures,
    summary: {
      sourceId: candidate.sourceId || beforeRow.sourceId || afterRow.sourceId || SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
...
export function evaluateFreedomSheetAtomFlowRepairs({
  beforeRows = [],
  afterRows = [],
  selection = {},
  atomBuilds = [],
  ...
  const checks = []
  const summaries = []
  for (const target of SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS) {
    ...
    const evaluation = evaluateFreedomSheetAtomFlowRepair({
      beforeRow: rowBySourceId(beforeRows, target.sourceId),
  ...
  return {
    ...
    summary: {
      targets: summaries,
      sourceIds: SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS.map(target => target.sourceId),
...
export function buildSyntheticFreedomSheetAtomFlowRepairProof() {
  const fact = {
    factId: SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_FACT_ID,
    sourceId: SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
    title: 'Freedom Team current-reality source boundary',
    ...
    detail: 'Evidence comes from the existing governed Freedom Sheet source fact; this proof does not read Sheets or run extraction.',
    metadata: { status: 'ahead', evidenceRefs: ['intelligence_synthesis_facts:fact:a76eb15fe718d1eb8dda2a93'] },
    minTier: 1,
    sensitivity: 'neutral',
    status: 'active',
  }
  ...
  const candidate = selectFreedomSheetAtomFlowRepairCandidate({ beforeRow, sourceFacts: [fact] })
  const atomBuild = buildFreedomSheetAtomFromFact(candidate, '2026-05-18T06:15:00.000Z')
  const evaluation = evaluateFreedomSheetAtomFlowRepair({
    beforeRow,
    afterRow,
    candidate,
  ...
  const missingFact = selectFreedomSheetAtomFlowRepairCandidate({ beforeRow, sourceFacts: [] })
  const tierBlocked = selectFreedomSheetAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, minTier: 4 }] })
  return {
    ok: candidate.ok &&
      atomBuild.ok &&
      evaluation.ok &&
...
export function renderFreedomSheetAtomFlowRepairCloseout(snapshot = {}) {
  const summary = snapshot.summary || {}
  const targets = list(summary.targets)
  const lines = []
  lines.push('# Source Maturity Freedom Sheet Atom Flow Repair Closeout')
  lines.push('')
  lines.push(`Card: \`${SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_CARD_ID}\``)
  lines.push(`Closeout key: \`${SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\``)
  lines.push('')
  lines.push('## What Shipped')
  lines.push('')
  lines.push(`- Promoted ${targets.length || SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS.length} source-backed atoms for the signed-off Freedom Sheet sources from existing source facts.`)
  ...
  if (!targets.length) {
    lines.push(`- Target sources: ${SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_TARGETS.map(target => `\`${target.sourceId}\``).join(', ')}.`)
  }
  ...
  lines.push('')
  for (const boundary of SOURCE_MATURITY_FREEDOM_SHEET_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES) lines.push(`- ${boundary}`)
  lines.push('')
  lines.push('## Next')
  lines.push('')
