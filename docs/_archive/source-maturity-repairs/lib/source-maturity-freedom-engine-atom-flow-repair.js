...
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_CARD_ID = 'SOURCE-MATURITY-FREEDOM-ENGINE-ATOM-FLOW-REPAIR-001'
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_CLOSEOUT_KEY = 'source-maturity-freedom-engine-atom-flow-repair-v1'
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_PLAN_PATH = 'docs/process/source-maturity-freedom-engine-atom-flow-repair-001-plan.md'
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_APPROVAL_PATH = 'docs/process/approvals/SOURCE-MATURITY-FREEDOM-ENGINE-ATOM-FLOW-REPAIR-001.json'
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-29-hot-doc-refresh/2026-05-18-source-maturity-freedom-engine-atom-flow-repair-closeout.md'
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_SCRIPT_PATH = 'scripts/process-source-maturity-freedom-engine-atom-flow-repair-check.mjs'
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_SPRINT_ID = 'source-maturity-freedom-engine-atom-flow-repair-2026-05-18'
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID = 'SRC-FREEDOM-ENGINE-001'
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_FACT_ID = 'fact:c3986feaa16a35be165d95ef'

export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES = [
...
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_PROOF_COMMANDS = [
...
export const SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_CHANGED_FILES = [
...
function targetFact(facts = []) {
  return list(facts).find(fact => (fact.factId || fact.fact_id) === SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_FACT_ID) || null
}

export function selectFreedomEngineAtomFlowRepairCandidate({
  beforeRow = {},
  sourceFacts = [],
  sourceId = SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
} = {}) {
  const failures = []
  ...
  const factId = fact?.factId || fact?.fact_id || SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_FACT_ID
...
export function buildFreedomEngineAtomFromFact(candidate = {}, now = new Date().toISOString()) {
  const sourceId = candidate.sourceId || SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  ...
  const naturalKey = `source-maturity-freedom-engine-atom-flow-repair:${sourceId}:${factId}`
  ...
  const atom = {
    atomId,
    title: `${sourceId} agent engine capacity gap`,
    ...
    evidenceExcerpt,
    derivedClaim: claim,
    topicRefs: ['source-maturity', 'freedom-engine', 'agent-engine-capacity', 'source-backed-goal-truth'],
    ...
    acceptedAt: now,
    acceptedBy: 'codex-source-maturity-freedom-engine-atom-flow-repair',
    reviewNote: 'Promoted from an existing governed Freedom Engine source fact only; no live extraction, provider call, Sheets read/write, or external write occurred.',
    tags: ['source-maturity', 'atom-flow-repair', 'freedom-engine', 'no-live-extraction'],
    metadata: {
...
export function evaluateFreedomEngineAtomFlowRepair({
  beforeRow = {},
  afterRow = {},
  candidate = {},
  atomBuild = {},
  ...
  return {
    ...
    failures,
    summary: {
      sourceId: candidate.sourceId || beforeRow.sourceId || afterRow.sourceId || SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
...
export function buildSyntheticFreedomEngineAtomFlowRepairProof() {
  const fact = {
    factId: SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_FACT_ID,
    sourceId: SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
    title: 'Agent Engine Capacity: Gap This Year',
    claim: 'Agent Engine Capacity Gap This Year is Behind by 15 agents (-28.8%).',
    detail: 'Current recruiting and capacity read from the Freedom KPI Sheet Agent Engine tab. Keep this separate from the 10,000-agent Real Broker community path.',
    metadata: { status: 'behind', evidenceRefs: ['intelligence_synthesis_facts:fact:c3986feaa16a35be165d95ef'] },
    minTier: 1,
    sensitivity: 'neutral',
    status: 'active',
  }
  ...
  const candidate = selectFreedomEngineAtomFlowRepairCandidate({ beforeRow, sourceFacts: [fact] })
  const atomBuild = buildFreedomEngineAtomFromFact(candidate, '2026-05-18T06:15:00.000Z')
  const evaluation = evaluateFreedomEngineAtomFlowRepair({
    beforeRow,
    afterRow,
    candidate,
    atomBuild,
  ...
  const missingFact = selectFreedomEngineAtomFlowRepairCandidate({ beforeRow, sourceFacts: [] })
  const tierBlocked = selectFreedomEngineAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, minTier: 4 }] })
  return {
    ok: candidate.ok &&
      atomBuild.ok &&
      evaluation.ok &&
...
export function renderFreedomEngineAtomFlowRepairCloseout(snapshot = {}) {
  const summary = snapshot.summary || {}
  const lines = []
  lines.push('# Source Maturity Freedom Engine Atom Flow Repair Closeout')
  lines.push('')
  lines.push(`Card: \`${SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_CARD_ID}\``)
  lines.push(`Closeout key: \`${SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\``)
  lines.push('')
  lines.push('## What Shipped')
  lines.push('')
  lines.push(`- Promoted one source-backed atom for \`${summary.sourceId || SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID}\` from an existing Freedom Engine source fact.`)
  lines.push(`- Saved atom: \`${summary.atomId || 'pending atom'}\`.`)
  lines.push(`- Source fact evidence: \`${summary.sourceFactId || SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_TARGET_FACT_ID}\`.`)
  ...
  lines.push('')
  for (const boundary of SOURCE_MATURITY_FREEDOM_ENGINE_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES) lines.push(`- ${boundary}`)
  lines.push('')
  lines.push('## Next')
  lines.push('')
