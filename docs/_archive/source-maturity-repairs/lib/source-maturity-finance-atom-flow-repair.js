...
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_CARD_ID = 'SOURCE-MATURITY-FINANCE-ATOM-FLOW-REPAIR-001'
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_CLOSEOUT_KEY = 'source-maturity-finance-atom-flow-repair-v1'
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_PLAN_PATH = 'docs/process/source-maturity-finance-atom-flow-repair-001-plan.md'
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_APPROVAL_PATH = 'docs/process/approvals/SOURCE-MATURITY-FINANCE-ATOM-FLOW-REPAIR-001.json'
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-29-hot-doc-refresh/2026-05-18-source-maturity-finance-atom-flow-repair-closeout.md'
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_SCRIPT_PATH = 'scripts/process-source-maturity-finance-atom-flow-repair-check.mjs'
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_SPRINT_ID = 'source-maturity-finance-atom-flow-repair-2026-05-18'
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID = 'SRC-FINANCE-001'
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_FACT_ID = 'fact:9d217c40c8fc9dfbdbc590f3'

export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES = [
...
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_PROOF_COMMANDS = [
...
export const SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_CHANGED_FILES = [
...
function targetFact(facts = []) {
  return list(facts).find(fact => (fact.factId || fact.fact_id) === SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_FACT_ID) || null
}

export function selectFinanceAtomFlowRepairCandidate({
  beforeRow = {},
  sourceFacts = [],
  sourceId = SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
} = {}) {
  const failures = []
  ...
  const factId = fact?.factId || fact?.fact_id || SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_FACT_ID
...
export function buildFinanceAtomFromFact(candidate = {}, now = new Date().toISOString()) {
  const sourceId = candidate.sourceId || SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  ...
  const naturalKey = `source-maturity-finance-atom-flow-repair:${sourceId}:${factId}`
  ...
  const atom = {
    atomId,
    title: `${sourceId} finance source truth`,
    ...
    evidenceExcerpt,
    derivedClaim: claim,
    topicRefs: ['source-maturity', 'finance', 'finance-source-truth', 'source-backed-finance-truth'],
    ...
    acceptedAt: now,
    acceptedBy: 'codex-source-maturity-finance-atom-flow-repair',
    reviewNote: 'Promoted from an existing governed Finance source fact only; no live extraction, provider call, Sheets read/write, or external write occurred.',
    tags: ['source-maturity', 'atom-flow-repair', 'finance', 'no-live-extraction'],
    metadata: {
...
export function evaluateFinanceAtomFlowRepair({
  beforeRow = {},
  afterRow = {},
  candidate = {},
  atomBuild = {},
  ...
  return {
    ...
    failures,
    summary: {
      sourceId: candidate.sourceId || beforeRow.sourceId || afterRow.sourceId || SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
...
export function buildSyntheticFinanceAtomFlowRepairProof() {
  const fact = {
    factId: SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_FACT_ID,
    sourceId: SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
    ...
    metadata: { status: 'signed_off_current_reality', evidenceRefs: ['intelligence_synthesis_facts:fact:9d217c40c8fc9dfbdbc590f3'] },
    minTier: 1,
    sensitivity: 'neutral',
    status: 'active',
  }
  ...
  const candidate = selectFinanceAtomFlowRepairCandidate({ beforeRow, sourceFacts: [fact] })
  const atomBuild = buildFinanceAtomFromFact(candidate, '2026-05-18T06:15:00.000Z')
  const evaluation = evaluateFinanceAtomFlowRepair({
    beforeRow,
    afterRow,
    candidate,
    atomBuild,
  ...
  const missingFact = selectFinanceAtomFlowRepairCandidate({ beforeRow, sourceFacts: [] })
  const tierBlocked = selectFinanceAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, minTier: 4 }] })
  return {
    ok: candidate.ok &&
      atomBuild.ok &&
      evaluation.ok &&
...
export function renderFinanceAtomFlowRepairCloseout(snapshot = {}) {
  const summary = snapshot.summary || {}
  const lines = []
  lines.push('# Source Maturity Finance Atom Flow Repair Closeout')
  lines.push('')
  lines.push(`Card: \`${SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_CARD_ID}\``)
  lines.push(`Closeout key: \`${SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\``)
  lines.push('')
  lines.push('## What Shipped')
  lines.push('')
  lines.push(`- Promoted one source-backed atom for \`${summary.sourceId || SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID}\` from an existing Finance source fact.`)
  lines.push(`- Saved atom: \`${summary.atomId || 'pending atom'}\`.`)
  lines.push(`- Source fact evidence: \`${summary.sourceFactId || SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_TARGET_FACT_ID}\`.`)
  ...
  lines.push('')
  for (const boundary of SOURCE_MATURITY_FINANCE_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES) lines.push(`- ${boundary}`)
  lines.push('')
  lines.push('## Next')
  lines.push('')
