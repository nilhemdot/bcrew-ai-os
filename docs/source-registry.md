...
| Build Intel Sources | `SRC-CREATOR-WATCHLIST-001`, `SRC-YOUTUBE-INTEL-001`, `SRC-GITHUB-BUILD-INTEL-001` | Builders, creator content, codebases, and implementation patterns into proposal-only AIOS improvements | [GitHub build intel](source-notes/github-build-intel.md), Mark/Kia notes, and Build Intel handoffs |
...
| `SRC-GMAIL-001` | Gmail | Delegated Google Workspace (`ai@bensoncrew.ca`) + `gmail-current-day`, `gmail-extract-latest`, and `email-attachments-backfill` governed lanes | Mailbox read layer, current-day thread archive, candidate extraction, and PDF/text/calendar attachment V1 | Fallback mailbox thread context, decision-capture inputs, communication context outside Missive, and first Gmail attachment text artifacts | Delegated Google Workspace | V1 Source Boundary Locked, Signed Off For Current Reality | 2026-05-20 |
| `SRC-GCAL-001` | Google Calendar | Delegated Google Workspace (`ai@bensoncrew.ca` primary calendar) + `calendar-current-day` governed target | Calendar read layer and bounded current-window event archive | Meetings, scheduling, governance cadence, organizer/attendee timing context, and source-backed calendar_event artifacts without Calendar writes or event descriptions/raw notes | Delegated Google Workspace | V1 Source Boundary Locked, Signed Off For Current Reality | 2026-05-20 |
...
| `SRC-GMAIL-001` | Gmail | Fallback mailbox thread context, decision-capture inputs, communication context outside Missive, and Gmail PDF/text/calendar attachment V1 artifacts | System | V1 Source Boundary Locked for delegated read-side current reality. Gmail sends, broad team-mailbox exposure, credential mutation, and Missive internal-comment/assignment truth remain out of scope. |
| `SRC-GCAL-001` | Google Calendar | Meeting/cadence event context, scheduling signals, event timing, organizer/attendee metadata, and source-backed calendar_event artifacts | System | V1 Source Boundary Locked for delegated read-side current reality. Calendar writes, invites, RSVP handling, event updates/deletes, credential mutation, broad calendar extraction, and meeting-note/transcript truth remain out of scope. |
| `SRC-GDRIVE-001` | Google Drive | Drive corpus inventory, source document archives, strategy evidence intake, brand/training assets, meeting artifacts, presentation/course asset metadata, and reusable content evidence with provenance | System | V1 Source Boundary Locked for delegated read-side current reality through `drive-corpus-backfill`, `drive-content-extract-backfill`, and local Drive document/PDF/spreadsheet/text artifacts. Drive permission mutation, request-access sends, broad private Drive sweeps, credential/OAuth mutation, raw Drive exposure to broad team/agent surfaces, media/vision extraction, and treating the Strategy Folder as canonical strategy truth remain out of scope. |
