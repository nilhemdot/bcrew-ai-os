...
Status: shipped locally, push pending branch-history safety
...
Local branch status at closeout time: `main` is already one commit ahead of `origin/main` with unrelated Harlan/Fal work (`739c37d Add Harlan voice and Fal image tools`). Foundation changes should be committed with explicit paths only. Pushing `main` directly would also publish that unrelated commit, so push requires either Steve approval to publish the existing local commit or a clean branch/history route for the Foundation commit.
