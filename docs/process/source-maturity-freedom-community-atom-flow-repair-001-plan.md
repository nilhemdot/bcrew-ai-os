# SOURCE-MATURITY-FREEDOM-COMMUNITY-ATOM-FLOW-REPAIR-001 Plan

## Goal

Repair the `SRC-FREEDOM-COMMUNITY-001` source maturity atomized-stage gap by promoting one existing active Freedom Community source fact into a governed `intelligence_atoms` record and supporting atom hit without running extraction, reading/writing Sheets, calling a provider/model, or writing externally.

## What

Build a bounded Freedom Community atom-flow repair. The card uses source fact `fact:18d41cc725fa1afb29009d82`, which records the current community-agent pace rule and says the 10,000-agent goal is ahead by 22 agents. It creates one accepted atom plus one supporting atom hit. It does not claim synthesis, routing, governed apply, or closed-loop completion.

## Why

Live source maturity shows `SRC-FREEDOM-COMMUNITY-001` has active source facts but no fresh promoted atom signal, so it blocks at `atomized`. Foundation should turn existing governed facts into atoms without inventing truth or starting live ingestion. The selected fact intentionally avoids promoting older conflicting behind-pace rows.
...
- Live Foundation data already contains active Tier 1 source fact `fact:18d41cc725fa1afb29009d82` for `SRC-FREEDOM-COMMUNITY-001`; the fact records the current ahead-status rule and warns downstream summaries to use live rows before old packet chatter.

## Scope

- Add `lib/source-maturity-freedom-community-atom-flow-repair.js` as the focused Freedom Community repair contract, constants, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-freedom-community-atom-flow-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-FREEDOM-COMMUNITY-001` and source fact `fact:18d41cc725fa1afb29009d82` in V1.
...
- The focused proof identifies `SRC-FREEDOM-COMMUNITY-001` as a real atomized-stage source maturity gap before mutation.
...
- The atom metadata names `SOURCE-MATURITY-FREEDOM-COMMUNITY-ATOM-FLOW-REPAIR-001` and `source-maturity-freedom-community-atom-flow-repair-v1`.
...
- The source maturity grid shows at least one atom signal for `SRC-FREEDOM-COMMUNITY-001` after repair and no longer reports `nextGap=atomized` for that source.
...
- `lib/source-maturity-freedom-community-atom-flow-repair.js` contains the Freedom Community repair contract, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-freedom-community-atom-flow-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-freedom-community-atom-flow-repair-check`.
- The closeout registry names `source-maturity-freedom-community-atom-flow-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-FREEDOM-COMMUNITY-ATOM-FLOW-REPAIR-001`.
...
- `npm run process:source-maturity-freedom-community-atom-flow-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-freedom-community-atom-flow-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the Freedom Community source moving through the governed memory spine from existing source truth, without opening any live source, paid run, or external write.
