# SOURCE-MATURITY-FINANCE-ROUTING-GAP-REPAIR-001 Plan

## Goal

Repair the `SRC-FINANCE-001` source maturity routed-stage gap by creating one bounded retrieval chunk from the existing source-backed Finance atom, then routing that signal into the internal Action Route Review layer without applying an action, creating a destination record, reading/writing Google Sheets, calling a provider, running extraction, or writing externally.

## What

Build a bounded Finance routing repair. The card creates one active `intelligence_retrieval_chunks` row from the existing governed Finance source fact and accepted atom, then creates one source-backed synthesized item and one approval-required pending `intelligence_action_routes` row. The route is review-only and must remain separate from trusted backlog cards, decisions, open questions, and external actions.

## Why

`SRC-FINANCE-001` already has a governed source fact and accepted atom from the prior Finance atom-flow repair, but the source maturity grid blocks at `routed` because it has no retrieval chunk/route signal. Foundation needs that existing evidence to become reviewable operator work without touching the underlying source, reading/writing Google Sheets, or silently applying anything.
...
- `SOURCE-MATURITY-FINANCE-ATOM-FLOW-REPAIR-001` already created the accepted `SRC-FINANCE-001` atom and atom hit.
...
- Live Foundation data shows `SRC-FINANCE-001` has the target source fact and accepted atom but no route signal.

## Scope

- Add `lib/source-maturity-finance-routing-gap-repair.js` as the focused Finance wrapper, constants, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-finance-routing-gap-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-FINANCE-001`, source fact `fact:9d217c40c8fc9dfbdbc590f3`, atom `atom:d707b8ee4fa72ca29c447e4e`, and retrieval chunk `chunk:source-maturity-finance-current-reality-review`.
...
- No broad Finance mining and no route promotion in this card.

## Acceptance Criteria

- The live backlog card exists and moves through Current Sprint with complete scaffold metadata before `building_now`.
- The focused proof identifies `SRC-FINANCE-001` as a real routed-stage source maturity gap before mutation.
...
- The route metadata names `SOURCE-MATURITY-FINANCE-ROUTING-GAP-REPAIR-001` and `source-maturity-finance-routing-gap-repair-v1`.
- The retrieval chunk metadata has `noLiveExtraction=true`, `noExternalWrite=true`, and `noGoogleSheetsReadWrite=true`.
- The source maturity grid shows at least one route signal for `SRC-FINANCE-001` after repair and no longer reports `nextGap=routed` for that source.
...
- `lib/source-maturity-finance-routing-gap-repair.js` contains the Finance repair contract, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-finance-routing-gap-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-finance-routing-gap-repair-check`.
- The closeout registry names `source-maturity-finance-routing-gap-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-FINANCE-ROUTING-GAP-REPAIR-001`.
...
- Read only atom `atom:d707b8ee4fa72ca29c447e4e` from `intelligence_atoms`.
- Create only retrieval chunk `chunk:source-maturity-finance-current-reality-review` in `intelligence_retrieval_chunks`.
- Build a `source-maturity-finance-routing-gap-repair-v1` synthesized item with verified `synthesisVerification` metadata.
...
- `npm run process:source-maturity-finance-routing-gap-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-finance-routing-gap-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the governed Finance source moved into the same reviewable Action Route path as Slack, Missive, and Owners Lists without a live Sheets read/write, route apply, or external side effect.
