...
Plain English: the current YouTube runner is strong video/audio/visual extraction, but it is not full God Mode yet. Full God Mode means the extractor can read, watch, hear, inspect approved resource links, use browser hands where approved, preserve evidence, and feed the synthesis/router/Director without making Steve chase context manually. YouTube comments are intentionally excluded from the active lane because Steve does not want LLM/runtime spend on low-value comment noise.
...
The system has already watched hundreds of videos through Gemini video/audio/visual analysis. That is valuable, but the name "God Mode" has drifted. The current lane still lacks browser navigation, logged-in/community/course sources, and approved link follow-up. Those gaps must be explicit and fixed before calling extraction complete. YouTube comments are not a blocker; they are excluded by operator decision.
...
- **Reading:** page text, descriptions, docs, posts, resources, and transcripts where allowed
- **Brain:** dedupe, synthesize, score, route, and explain source-backed value
- **Evidence:** every useful claim links back to a source, artifact, timestamp, URL, page hash, or report
- **Boundaries:** paid/private/auth/member/course limits are enforced by source packet, not by memory
...
- A capability matrix exists for each extractor family:
  ...
  - YouTube comments (operator-excluded / no LLM spend)
...
- Public YouTube extraction does not count as complete until approved public resource follow-up and browser hands have an owned route or an explicit blocked reason.
...
- Add a focused proof that fails if a source is labeled full God Mode while hands/links/source-packet status are missing.
