# SOURCE-MATURITY-STRATEGY-ROUTING-GAP-REPAIR-001 Plan

## Goal

Repair the `SRC-STRATEGY-001` source maturity routed-stage gap by creating one bounded retrieval chunk from the existing source-backed Strategy atom, then routing that signal into the internal Action Route Review layer without applying an action, creating a destination record, reading/writing strategy docs, calling a provider, running extraction, or writing externally.

## What

Build a bounded Strategy routing repair. The card creates one active `intelligence_retrieval_chunks` row from the existing governed Strategy source fact and accepted atom, then creates one source-backed synthesized item and one approval-required pending `intelligence_action_routes` row. The route is review-only and must remain separate from trusted backlog cards, decisions, open questions, and external actions.

## Why

`SRC-STRATEGY-001` already has a governed source fact and accepted atom from the prior Strategy atom-flow repair, but the source maturity grid blocks at `routed` because it has no retrieval chunk/route signal. Foundation needs that existing evidence to become reviewable operator work without touching the underlying source, reading/writing strategy docs, or silently applying anything.
...
- `SOURCE-MATURITY-STRATEGY-ATOM-FLOW-REPAIR-001` already created the accepted `SRC-STRATEGY-001` atom and atom hit.
...
- Live Foundation data shows `SRC-STRATEGY-001` has the target source fact and accepted atom but no route signal.

## Scope

- Add `lib/source-maturity-strategy-routing-gap-repair.js` as the focused Strategy wrapper, constants, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-strategy-routing-gap-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-STRATEGY-001`, source fact `fact:0a3bea1f66c3fbdb4b4ee08f`, atom `atom:2a3022e604585b1b83c1a283`, and retrieval chunk `chunk:source-maturity-strategy-source-truth-review`.
...
- Do not create or apply a destination backlog card, decision, question record, external action, connector write, strategy doc write, or provider call.
...
- No broad Strategy mining and no route promotion in this card.

## Acceptance Criteria

- The live backlog card exists and moves through Current Sprint with complete scaffold metadata before `building_now`.
- The focused proof identifies `SRC-STRATEGY-001` as a real routed-stage source maturity gap before mutation.
...
- The route metadata names `SOURCE-MATURITY-STRATEGY-ROUTING-GAP-REPAIR-001` and `source-maturity-strategy-routing-gap-repair-v1`.
- The retrieval chunk metadata has `noLiveExtraction=true`, `noExternalWrite=true`, and `noStrategyDocWrite=true`.
- The source maturity grid shows at least one route signal for `SRC-STRATEGY-001` after repair and no longer reports `nextGap=routed` for that source.
...
- `lib/source-maturity-strategy-routing-gap-repair.js` contains the Strategy repair contract, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-strategy-routing-gap-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-strategy-routing-gap-repair-check`.
- The closeout registry names `source-maturity-strategy-routing-gap-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-STRATEGY-ROUTING-GAP-REPAIR-001`.
...
- Read only atom `atom:2a3022e604585b1b83c1a283` from `intelligence_atoms`.
- Create only retrieval chunk `chunk:source-maturity-strategy-source-truth-review` in `intelligence_retrieval_chunks`.
- Build a `source-maturity-strategy-routing-gap-repair-v1` synthesized item with verified `synthesisVerification` metadata.
...
- If the proof writes strategy docs, this becomes unapproved connector/source work. Repair path: target existing DB fact/atom evidence only and require `noStrategyDocWrite=true`.
...
- `node --check lib/source-maturity-routing-gap-repair.js lib/source-maturity-routing-gap-repair-db.js lib/source-maturity-strategy-routing-gap-repair.js scripts/process-source-maturity-strategy-routing-gap-repair-check.mjs`
- `npm run process:source-maturity-strategy-routing-gap-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-strategy-routing-gap-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the governed Strategy source moved into the same reviewable Action Route path as Slack, Missive, and Owners Lists without a live strategy doc write, route apply, or external side effect.
