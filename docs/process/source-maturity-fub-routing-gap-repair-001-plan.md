# SOURCE-MATURITY-FUB-ROUTING-GAP-REPAIR-001 Plan

## Goal

Repair the `SRC-FUB-001` source maturity routed-stage gap by creating one bounded retrieval chunk from the existing source-backed FUB atom, then routing that signal into the internal Action Route Review layer without applying an action, creating a destination record, calling the FUB API, mutating CRM data, reading/writing Google Sheets, calling a provider, running extraction, or writing externally.

## What

Build a bounded FUB routing repair. The card creates one active `intelligence_retrieval_chunks` row from the existing governed FUB source fact and accepted atom, then creates one source-backed synthesized item and one approval-required pending `intelligence_action_routes` row. The route is review-only and must remain separate from trusted backlog cards, decisions, open questions, and external actions.

## Why

`SRC-FUB-001` already has a governed source fact and accepted atom from the prior FUB atom-flow repair, but the source maturity grid blocks at `routed` because it has no retrieval chunk/route signal. Foundation needs that existing evidence to become reviewable operator work without touching the underlying source, calling FUB, mutating CRM data, reading/writing Google Sheets, or silently applying anything.
...
- `SOURCE-MATURITY-FUB-ATOM-FLOW-REPAIR-001` already created the accepted `SRC-FUB-001` atom and atom hit.
...
- Live Foundation data shows `SRC-FUB-001` has the target source fact and accepted atom but no route signal.

## Scope

- Add `lib/source-maturity-fub-routing-gap-repair.js` as the focused FUB wrapper, constants, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-fub-routing-gap-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-FUB-001`, source fact `fact:af613c177546ec2a7267b669`, atom `atom:a324d8e68f80c29e07daf2c3`, and retrieval chunk `chunk:source-maturity-fub-crm-source-truth-review`.
...
- Do not create or apply a destination backlog card, decision, question record, external action, connector write, FUB API call, CRM mutation, Google Sheets read/write, or provider call.
...
- No broad FUB mining and no route promotion in this card.

## Acceptance Criteria

- The live backlog card exists and moves through Current Sprint with complete scaffold metadata before `building_now`.
- The focused proof identifies `SRC-FUB-001` as a real routed-stage source maturity gap before mutation.
...
- The route metadata names `SOURCE-MATURITY-FUB-ROUTING-GAP-REPAIR-001` and `source-maturity-fub-routing-gap-repair-v1`.
- The retrieval chunk metadata has `noLiveExtraction=true`, `noExternalWrite=true`, `noFubApiCall=true`, `noCrmMutation=true`, and `noGoogleSheetsReadWrite=true`.
- The source maturity grid shows at least one route signal for `SRC-FUB-001` after repair and no longer reports `nextGap=routed` for that source.
...
- `lib/source-maturity-fub-routing-gap-repair.js` contains the FUB repair contract, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-fub-routing-gap-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-fub-routing-gap-repair-check`.
- The closeout registry names `source-maturity-fub-routing-gap-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-FUB-ROUTING-GAP-REPAIR-001`.
...
- Read only atom `atom:a324d8e68f80c29e07daf2c3` from `intelligence_atoms`.
- Create only retrieval chunk `chunk:source-maturity-fub-crm-source-truth-review` in `intelligence_retrieval_chunks`.
- Build a `source-maturity-fub-routing-gap-repair-v1` synthesized item with verified `synthesisVerification` metadata.
...
It does not touch external systems, connectors, FUB live APIs, CRM records, OAuth, provider spend, Harlan/Fal/voice/Canva/OpenHuman features, Drive permissions, Google Sheets, or broad UI layout.
...
- If the proof calls FUB, mutates CRM data, or reads/writes Google Sheets, this becomes unapproved connector/source work. Repair path: target existing DB fact/atom evidence only and require `noFubApiCall=true`, `noCrmMutation=true`, and `noGoogleSheetsReadWrite=true`.
...
- `npm run process:source-maturity-fub-routing-gap-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-fub-routing-gap-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the governed FUB source moved into the same reviewable Action Route path as Slack, Missive, and Owners Lists without a live FUB API call, CRM mutation, Sheets read/write, route apply, or external side effect.
