# SOURCE-MATURITY-FUB-MONITORING-GAP-REPAIR-001 Plan
...
Repair the `SRC-FUB-001` monitored-stage source maturity gap by adding an explicit manual/on-demand refresh boundary to the verified-readable FUB source contract.

This is a narrow V1 source-contract maturity repair. It changes the contract posture and proof only; it does not build FUB automation.

## Why

`SRC-FUB-001` already has source facts and a verified-readable CRM boundary, but live source maturity blocks at `monitored` because the contract does not state a refresh boundary. That creates false noise in the Foundation source queue and hides the next real gap.

Useful operator behavior: Foundation should say FUB is manually/on-demand monitored today, then expose the next actual maturity gap instead of asking Steve to re-prove whether FUB truth exists.

## Existing Work

- `SRC-FUB-001` is verified readable for current FUB CRM context in `lib/source-contracts.js`, `docs/source-registry.md`, and `docs/rebuild/current-state.md`.
- Live source maturity shows `SRC-FUB-001` has existing source fact signals but no monitoring boundary, so its next gap is `monitored`.
...
- Add a truthful manual/on-demand monitoring posture to `SRC-FUB-001`.
- Make clear that no background FUB automation is approved by this repair.
...
- Do not mark atomized, synthesized, routed, FUB automation, or CRM mutation complete.

## Acceptance Criteria

- Synthetic dogfood proves verified-readable FUB facts without a monitoring boundary fail at `monitored`.
- Synthetic dogfood proves the same FUB contract with `updateMethod`, `refreshSchedule`, and `manualRefresh` advances to the next real gap.
- Live proof shows `SRC-FUB-001` has existing source fact signals and a green monitored stage.
- Live proof shows no active extraction target was introduced for `SRC-FUB-001`.
...
- Scope drift risk: FUB data reads or CRM mutation could sneak into a maturity repair. Mitigation: proof and Current Sprint boundaries forbid live FUB API calls, CRM mutation, and active extraction targets.
...
- Synthetic FUB source with signed-off facts but no refresh boundary fails at `monitored`.
- Synthetic repaired FUB source advances from `monitored` to the next real gap.
- Live `SRC-FUB-001` has signed-off source facts and an explicit monitoring boundary.
- Live source maturity no longer blocks FUB at `monitored`.
- No active extraction target is introduced for `SRC-FUB-001`.
...
- `npm run process:source-maturity-fub-monitoring-gap-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-fub-monitoring-gap-repair-check -- --apply --stage=sprint_ready --json`
...
Gate decision tree: static checks (`node --check`) for syntax; focused proof (`process:source-maturity-fub-monitoring-gap-repair-check`) while iterating; full `foundation:verify` once focused proof is green because the blast radius touches source-contract truth and source maturity; full `process:foundation-ship` before push.
...
- `SRC-FUB-001` has explicit `updateMethod`, `refreshSchedule`, and `manualRefresh` posture for manual/on-demand review.
- The source maturity grid clears the monitored-stage gap for FUB.
- The next real gap remains visible.
- No live FUB API call, CRM mutation, live extraction, external write, model/provider call, paid run, auth repair, Drive mutation, or Agent Feedback auto-send occurs.
