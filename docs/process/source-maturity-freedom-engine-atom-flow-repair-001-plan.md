# SOURCE-MATURITY-FREEDOM-ENGINE-ATOM-FLOW-REPAIR-001 Plan

## Goal

Repair the `SRC-FREEDOM-ENGINE-001` source maturity atomized-stage gap by promoting one existing active Freedom Engine source fact into a governed `intelligence_atoms` record and supporting atom hit without running extraction, reading/writing Sheets, calling a provider/model, or writing externally.

## What

Build a bounded Freedom Engine atom-flow repair. The card uses source fact `fact:c3986feaa16a35be165d95ef`, which records the current Agent Engine capacity gap: behind by 15 agents (-28.8%) this year. It creates one accepted atom plus one supporting atom hit. It does not claim synthesis, routing, governed apply, or closed-loop completion.

## Why

Live source maturity shows `SRC-FREEDOM-ENGINE-001` has active source facts but no fresh promoted atom signal, so it blocks at `atomized`. Foundation should turn existing governed facts into atoms without inventing truth or starting live ingestion. The selected fact intentionally promotes the current Agent Engine capacity gap, not older generic report atoms.
...
- Live Foundation data already contains active Tier 1 source fact `fact:c3986feaa16a35be165d95ef` for `SRC-FREEDOM-ENGINE-001`; the fact records the current Agent Engine capacity gap and keeps it separate from the 10,000-agent community path.

## Scope

- Add `lib/source-maturity-freedom-engine-atom-flow-repair.js` as the focused Freedom Engine repair contract, constants, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-freedom-engine-atom-flow-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-FREEDOM-ENGINE-001` and source fact `fact:c3986feaa16a35be165d95ef` in V1.
...
- The focused proof identifies `SRC-FREEDOM-ENGINE-001` as a real atomized-stage source maturity gap before mutation.
...
- The atom metadata names `SOURCE-MATURITY-FREEDOM-ENGINE-ATOM-FLOW-REPAIR-001` and `source-maturity-freedom-engine-atom-flow-repair-v1`.
...
- The source maturity grid shows at least one atom signal for `SRC-FREEDOM-ENGINE-001` after repair and no longer reports `nextGap=atomized` for that source.
...
- `lib/source-maturity-freedom-engine-atom-flow-repair.js` contains the Freedom Engine repair contract, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-freedom-engine-atom-flow-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-freedom-engine-atom-flow-repair-check`.
- The closeout registry names `source-maturity-freedom-engine-atom-flow-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-FREEDOM-ENGINE-ATOM-FLOW-REPAIR-001`.
...
- `npm run process:source-maturity-freedom-engine-atom-flow-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-freedom-engine-atom-flow-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the Freedom Engine source moving through the governed memory spine from existing source truth, without opening any live source, paid run, or external write.
