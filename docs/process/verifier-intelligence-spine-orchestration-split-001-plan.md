# VERIFIER-INTELLIGENCE-SPINE-ORCHESTRATION-SPLIT-001 Plan

Card: `VERIFIER-INTELLIGENCE-SPINE-ORCHESTRATION-SPLIT-001`

## What

Move only the intelligence-spine orchestration layer out of `scripts/foundation-verify.mjs` and into `lib/foundation-intelligence-spine-verifier.js`.

V1 covers the root verifier handoff that calls the existing intelligence-spine verifier, pushes its checks, proves the historical intelligence-spine module split remains closed, and keeps the dogfood proof for job ledger, retrieval tier guard, router approval gate, and synthesis evidence failures in the module. It does not move extraction runtime, surface trust, operator budget, hub safety, process hardening, process trust, connector auth, route budgets, source extraction jobs, frontend assurance, or unrelated verifier domains.

`scripts/foundation-verify.mjs` remains the orchestrator. It gathers the same live payloads and source text, calls `evaluateFoundationIntelligenceSpineVerifierOrchestration({ ... })`, and pushes returned checks into the canonical check list.

## Why

The verifier is still above the under-5K clean target. This sprint removes a real intelligence-spine orchestration domain from the root verifier instead of moving arbitrary tail code.

Operator value: Steve keeps proof that intelligence jobs, atom spine, retrieval, synthesis facts, synthesis engine, and action router checks did not weaken while the root verifier gets smaller. The useful operator behavior is a real workflow where missing ledger proof, missing tier guards, missing approval gates, or missing synthesis evidence fail closed before Steve returns to broader building.

## Acceptance Criteria

- `lib/foundation-intelligence-spine-verifier.js` owns `evaluateFoundationIntelligenceSpineVerifierOrchestration()`.
- `scripts/foundation-verify.mjs` delegates through `evaluateFoundationIntelligenceSpineVerifierOrchestration({ ... })` and no longer owns the intelligence-spine module call, historical split self-check, or dogfood call inline.
- The focused proof calls the real `buildFoundationIntelligenceSpineVerifierDogfoodProof()` path.
- Dogfood proof recreates the failure class and rejects missing job ledger proof, missing retrieval tier guard, missing action-router approval gate, and synthesis evidence gaps.
- `scripts/process-verifier-intelligence-spine-orchestration-split-check.mjs` is read-only and verifies approval, backlog truth, ownership, Plan Critic/closeout truth, package script registration, line-count decrease, module ownership, historical proof compatibility, and root delegation.
- No active sprint overlay replacement.
- No arbitrary tail extraction.
- `scripts/foundation-verify.mjs` line count decreases from the JS-counted baseline of 6,528 and keeps moving toward the under-5K clean target.
...
- Focused proof script passes with `npm run process:verifier-intelligence-spine-orchestration-split-check -- --json`.
- The historical focused proof still passes with `npm run process:verifier-intelligence-spine-split-module-check -- --json`.
...
- Existing intelligence-spine evaluator and dogfood proof in `lib/foundation-intelligence-spine-verifier.js`.
- Existing intelligence-spine checks and historical self-check in `scripts/foundation-verify.mjs`.
- Existing Foundation Hub payloads, source reads, package script map, approval integrity, and build closeout registry.
- Existing focused verifier split proof pattern from the process-hardening and core-governance orchestration splits.
...
- The verifier must prove intelligence spine through actual live backlog cards, current plan/state, approval files, package scripts, source text, build closeouts, snapshots, and dogfood proof.
...
- The module boundary must preserve behavior; it is not a place to loosen job ledger proof, tier guards, retrieval proof, synthesis fact proof, synthesis evidence, action-router approval, owner provenance, or closeout enforcement.

Not next:

- No connector auth, source extraction job, paid call, email send, Canva, Fal, ElevenLabs, voice, or Harlan runtime work.
- No behavior rewrite of intelligence spine, extraction runtime, surface trust, process hardening, process trust, runtime reliability, structural assurance, or Plan Critic scoring.
...
- Focused proof runs first to catch the exact intelligence-spine orchestration failure mode quickly.
...
- Moving this orchestration can accidentally drop an intelligence-spine check, source text, snapshot handoff, or closeout proof; `foundation:verify` and the focused proof catch that by executing the real module path.
