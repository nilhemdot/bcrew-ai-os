# SOURCE-MATURITY-OWNERS-ATOM-FLOW-REPAIR-001 Plan

## Goal

Repair the `SRC-OWNERS-001` source maturity atomized-stage gap by promoting one existing active Owners Dashboard source fact into a governed `intelligence_atoms` record and supporting atom hit without running extraction, reading/writing Sheets, calling a provider/model, or writing externally.

## What

Build a bounded Owners Dashboard atom-flow repair. The card uses source fact `fact:566d0615ac1fcb345e2c6a93`, which records the current Team Goal pace gap: behind by $41.79M (-48.7%). It creates one accepted atom plus one supporting atom hit. It does not claim synthesis, routing, governed apply, or closed-loop completion.

## Why

Live source maturity shows `SRC-OWNERS-001` has active source facts but no fresh promoted atom signal, so it blocks at `atomized`. Foundation should turn existing governed facts into atoms without inventing truth or starting live ingestion. The selected fact intentionally promotes the current Team goal pace gap, not older generic report atoms.
...
- Live Foundation data already contains active Tier 1 source fact `fact:566d0615ac1fcb345e2c6a93` for `SRC-OWNERS-001`; the fact records the current Team Goal pace gap and keeps it separate from community-size or downline-count targets.

## Scope

- Add `lib/source-maturity-owners-atom-flow-repair.js` as the focused Owners Dashboard repair contract, constants, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-owners-atom-flow-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-OWNERS-001` and source fact `fact:566d0615ac1fcb345e2c6a93` in V1.
...
- The focused proof identifies `SRC-OWNERS-001` as a real atomized-stage source maturity gap before mutation.
...
- The atom metadata names `SOURCE-MATURITY-OWNERS-ATOM-FLOW-REPAIR-001` and `source-maturity-owners-atom-flow-repair-v1`.
...
- The source maturity grid shows at least one atom signal for `SRC-OWNERS-001` after repair and no longer reports `nextGap=atomized` for that source.
...
- `lib/source-maturity-owners-atom-flow-repair.js` contains the Owners Dashboard repair contract, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-owners-atom-flow-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-owners-atom-flow-repair-check`.
- The closeout registry names `source-maturity-owners-atom-flow-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-OWNERS-ATOM-FLOW-REPAIR-001`.
...
- `npm run process:source-maturity-owners-atom-flow-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-owners-atom-flow-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the Owners Dashboard source moving through the governed memory spine from existing source truth, without opening any live source, paid run, or external write.
