...
- `SOURCE-MATURITY-OWNERS-ATOM-FLOW-REPAIR-001` already created the accepted `SRC-OWNERS-001` atom and atom hit.
...
- Live Foundation data shows `SRC-OWNERS-001` has the target source fact and accepted atom but no route signal.

## Scope

- Add `lib/source-maturity-owners-routing-gap-repair.js` as the focused Owners Dashboard wrapper, constants, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-owners-routing-gap-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-OWNERS-001`, source fact `fact:566d0615ac1fcb345e2c6a93`, atom `atom:4e4a5b44a75b5bbca64b55f5`, and retrieval chunk `chunk:source-maturity-owners-team-goal-pace-review`.
...
- No broad Owners Dashboard mining and no route promotion in this card.

## Acceptance Criteria

- The live backlog card exists and moves through Current Sprint with complete scaffold metadata before `building_now`.
- The focused proof identifies `SRC-OWNERS-001` as a real routed-stage source maturity gap before mutation.
...
- The route metadata names `SOURCE-MATURITY-OWNERS-ROUTING-GAP-REPAIR-001` and `source-maturity-owners-routing-gap-repair-v1`.
- The retrieval chunk metadata has `noLiveExtraction=true`, `noExternalWrite=true`, and `noGoogleSheetsReadWrite=true`.
- The source maturity grid shows at least one route signal for `SRC-OWNERS-001` after repair and no longer reports `nextGap=routed` for that source.
...
- `lib/source-maturity-owners-routing-gap-repair.js` contains the Owners Dashboard repair contract, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-owners-routing-gap-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-owners-routing-gap-repair-check`.
- The closeout registry names `source-maturity-owners-routing-gap-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-OWNERS-ROUTING-GAP-REPAIR-001`.
...
- Read only atom `atom:4e4a5b44a75b5bbca64b55f5` from `intelligence_atoms`.
- Create only retrieval chunk `chunk:source-maturity-owners-team-goal-pace-review` in `intelligence_retrieval_chunks`.
- Build a `source-maturity-owners-routing-gap-repair-v1` synthesized item with verified `synthesisVerification` metadata.
...
- `npm run process:source-maturity-owners-routing-gap-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-owners-routing-gap-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the governed Owners Dashboard source moved into the same reviewable Action Route path as Slack, Missive, and Owners Lists without a live Sheets read/write, route apply, or external side effect.
