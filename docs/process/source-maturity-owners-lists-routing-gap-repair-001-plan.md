# SOURCE-MATURITY-OWNERS-LISTS-ROUTING-GAP-REPAIR-001 Plan

## Goal

Repair the `SRC-OWNERS-LISTS-001` source maturity routed-stage gap by creating one bounded retrieval chunk from the existing source-backed Owners Lists atom, then routing that signal into the internal Action Route Review layer without applying an action, creating a destination record, reading/writing Google Sheets, calling a provider, running extraction, or writing externally.

## What

Build a bounded Owners Lists routing repair. The card creates one active `intelligence_retrieval_chunks` row from the existing governed Owners Lists source fact and accepted atom, then creates one source-backed synthesized item and one approval-required pending `intelligence_action_routes` row. The route is review-only and must remain separate from trusted backlog cards, decisions, open questions, and external actions.

## Why

`SRC-OWNERS-LISTS-001` already has a governed source fact and accepted atom from the prior source maturity evidence and atom-flow repairs, but the source maturity grid blocks at `routed` because it has no retrieval chunk/route signal. Foundation needs that existing evidence to become reviewable operator work without touching the underlying Sheets source or silently applying anything.
...
- `SOURCE-MATURITY-EVIDENCE-GAP-REPAIR-001` already created the governed `SRC-OWNERS-LISTS-001` source fact.
- `SOURCE-MATURITY-ATOM-FLOW-REPAIR-001` already created the accepted `SRC-OWNERS-LISTS-001` atom and atom hit.
- `SOURCE-MATURITY-ROUTING-GAP-REPAIR-001` and `SOURCE-MATURITY-MISSIVE-ROUTING-GAP-REPAIR-001` already proved the internal review route pattern.
...
- Live Foundation data shows `SRC-OWNERS-LISTS-001` has the target source fact and accepted atom but no route signal.

## Scope

- Add `lib/source-maturity-routing-gap-repair-db.js` as a shared DB writer for source maturity routing repairs so the route insert/update path is not duplicated per source.
- Add `lib/source-maturity-owners-lists-routing-gap-repair.js` as the focused Owners Lists wrapper, constants, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-owners-lists-routing-gap-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-OWNERS-LISTS-001`, source fact `fact:92cc93c4350d6f62e4799937`, atom `atom:ff7840fbb09eb9d87b922a51`, and retrieval chunk `chunk:source-maturity-owners-lists-governed-list-boundary`.
...
- No broad Owners Lists mining and no route promotion in this card.

## Acceptance Criteria

- The live backlog card exists and moves through Current Sprint with complete scaffold metadata before `building_now`.
- The focused proof identifies `SRC-OWNERS-LISTS-001` as a real routed-stage source maturity gap before mutation.
...
- The route metadata names `SOURCE-MATURITY-OWNERS-LISTS-ROUTING-GAP-REPAIR-001` and `source-maturity-owners-lists-routing-gap-repair-v1`.
- The retrieval chunk metadata has `noLiveExtraction=true`, `noExternalWrite=true`, and `noGoogleSheetsReadWrite=true`.
- The source maturity grid shows at least one route signal for `SRC-OWNERS-LISTS-001` after repair and no longer reports `nextGap=routed` for that source.
...
- `lib/source-maturity-routing-gap-repair-db.js` contains the shared source maturity routing repair DB writer.
- `lib/source-maturity-owners-lists-routing-gap-repair.js` contains the Owners Lists repair contract, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-owners-lists-routing-gap-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-owners-lists-routing-gap-repair-check`.
- The closeout registry names `source-maturity-owners-lists-routing-gap-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-OWNERS-LISTS-ROUTING-GAP-REPAIR-001`.
...
- Read only atom `atom:ff7840fbb09eb9d87b922a51` from `intelligence_atoms`.
- Create only retrieval chunk `chunk:source-maturity-owners-lists-governed-list-boundary` in `intelligence_retrieval_chunks`.
- Build a `source-maturity-owners-lists-routing-gap-repair-v1` synthesized item with verified `synthesisVerification` metadata.
...
- `npm run process:source-maturity-owners-lists-routing-gap-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-owners-lists-routing-gap-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the governed Owners Lists source moved into the same reviewable Action Route path as Slack and Missive without a live Sheets read/write, route apply, or external side effect.
