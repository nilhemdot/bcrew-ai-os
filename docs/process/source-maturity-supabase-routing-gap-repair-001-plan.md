# SOURCE-MATURITY-SUPABASE-ROUTING-GAP-REPAIR-001 Plan

## Goal

Repair the `SRC-SUPABASE-001` source maturity routed-stage gap by creating one bounded retrieval chunk from the existing source-backed Supabase atom, then routing that signal into the internal Action Route Review layer without applying an action, creating a destination record, calling the Supabase API, mutating source data, reading/writing external source, calling a provider, running extraction, or writing externally.

## What

Build a bounded Supabase routing repair. The card creates one active `intelligence_retrieval_chunks` row from the existing governed Supabase source fact and accepted atom, then creates one source-backed synthesized item and one approval-required pending `intelligence_action_routes` row. The route is review-only and must remain separate from trusted backlog cards, decisions, open questions, and external actions.

## Why

`SRC-SUPABASE-001` already has a governed source fact and accepted atom from the prior Supabase atom-flow repair, but the source maturity grid blocks at `routed` because it has no retrieval chunk/route signal. Foundation needs that existing evidence to become reviewable operator work without touching the underlying source, calling Supabase, mutating source data, reading/writing external source, or silently applying anything.
...
- `SOURCE-MATURITY-SUPABASE-ATOM-FLOW-REPAIR-001` already created the accepted `SRC-SUPABASE-001` atom and atom hit.
...
- Live Foundation data shows `SRC-SUPABASE-001` has the target source fact and accepted atom but no route signal.

## Scope

- Add `lib/source-maturity-supabase-routing-gap-repair.js` as the focused Supabase wrapper, constants, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- Add `scripts/process-source-maturity-supabase-routing-gap-repair-check.mjs` as the focused proof and live sprint progression script.
- Target only `SRC-SUPABASE-001`, source fact `fact:00bc3e59b79fd30eb77a0bf4`, atom `atom:da1bb7af05f165ee8e7102f6`, and retrieval chunk `chunk:source-maturity-supabase-kpi-source-truth-review`.
...
- Do not create or apply a destination backlog card, decision, question record, external action, connector write, Supabase API call, source mutation, external source read/write, or provider call.
...
- No broad Supabase mining and no route promotion in this card.

## Acceptance Criteria

- The live backlog card exists and moves through Current Sprint with complete scaffold metadata before `building_now`.
- The focused proof identifies `SRC-SUPABASE-001` as a real routed-stage source maturity gap before mutation.
...
- The route metadata names `SOURCE-MATURITY-SUPABASE-ROUTING-GAP-REPAIR-001` and `source-maturity-supabase-routing-gap-repair-v1`.
- The retrieval chunk metadata has `noLiveExtraction=true`, `noExternalWrite=true`, `noSupabaseSourceReadWrite=true`, `noSupabaseSchemaMutation=true`, and `noExternalSourceReadWrite=true`.
- The source maturity grid shows at least one route signal for `SRC-SUPABASE-001` after repair and no longer reports `nextGap=routed` for that source.
...
- `lib/source-maturity-supabase-routing-gap-repair.js` contains the Supabase repair contract, retrieval chunk builder, synthetic dogfood proof, and closeout renderer.
- `scripts/process-source-maturity-supabase-routing-gap-repair-check.mjs` performs live Current Sprint progression, Plan Critic recording, focused proof, optional apply, and closeout writing.
- `package.json` registers `process:source-maturity-supabase-routing-gap-repair-check`.
- The closeout registry names `source-maturity-supabase-routing-gap-repair-v1`.
- The done-card verifier coverage list includes `SOURCE-MATURITY-SUPABASE-ROUTING-GAP-REPAIR-001`.
...
- Read only atom `atom:da1bb7af05f165ee8e7102f6` from `intelligence_atoms`.
- Create only retrieval chunk `chunk:source-maturity-supabase-kpi-source-truth-review` in `intelligence_retrieval_chunks`.
- Build a `source-maturity-supabase-routing-gap-repair-v1` synthesized item with verified `synthesisVerification` metadata.
...
It does not touch external systems, connectors, Supabase live APIs, source records, OAuth, provider spend, Harlan/Fal/voice/Canva/OpenHuman features, Drive permissions, external source, or broad UI layout.
...
- If the proof calls Supabase, mutates source data, or reads/writes external source, this becomes unapproved connector/source work. Repair path: target existing DB fact/atom evidence only and require `noSupabaseSourceReadWrite=true`, `noSupabaseSchemaMutation=true`, and `noExternalSourceReadWrite=true`.
...
- `npm run process:source-maturity-supabase-routing-gap-repair-check -- --apply --stage=scoping --json`
- `npm run process:source-maturity-supabase-routing-gap-repair-check -- --apply --stage=sprint_ready --json`
...
Steve gets the governed Supabase source moved into the same reviewable Action Route path as Slack, Missive, and Owners Lists without a live Supabase API call, source mutation, external source read/write, route apply, or external side effect.
