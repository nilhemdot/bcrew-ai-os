...
1. Run `npm run builder:startup-packet` and read the live builder packet before choosing work, answering project-status questions, or changing code. If it fails, say so and fall back to the files below.
...
- Repeated failures are repair triggers, not report trivia. If a workflow, verifier, ship/fanout, or scheduled job fails repeatedly, normal sprint/value progression stops until Foundation Builder fixes it or attaches it to an active repair card with proof.
- Run DB-heavy Foundation health/recovery gates sequentially. Do not start `foundation:verify`, `process:system-health-nightly-audit-check`, `process:build-lane-repeated-failure-action-gate-check`, `backlog:hygiene`, or governed health jobs in parallel; the shared live Postgres ledger can deadlock and leave orphaned active runs.
- Lessons learned are not done because they are documented. A durable lesson must become a backlog card, verifier rule, Plan Critic rule, Current Sprint gate, operating doctrine, or explicit no-op with proof.
- Keep lane ownership explicit. An Orchestrator/audit lane sequences, inspects, and injects instructions; a Builder/repair lane changes code or live operational state. Do not cross from audit into repair or live job execution without explicit approval and a handoff.
