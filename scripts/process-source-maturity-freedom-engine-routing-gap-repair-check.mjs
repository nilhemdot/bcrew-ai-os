...
import {
  ...
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CHANGED_FILES,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_PATH,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_NOT_NEXT_BOUNDARIES,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PLAN_PATH,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PROOF_COMMANDS,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_SCRIPT_PATH,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_SPRINT_ID,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_ATOM_ID,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_CHUNK_ID,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_FACT_ID,
  SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID,
  buildFreedomEngineRetrievalChunk,
  buildFreedomEngineRoutingRepairRecords,
  buildSyntheticFreedomEngineRoutingGapRepairProof,
  renderFreedomEngineRoutingGapRepairCloseout,
} from '../lib/source-maturity-freedom-engine-routing-gap-repair.js'
import {
...
function buildExistingWorkCheck() {
  return {
    existingCode: [
      ...
      'lib/source-maturity-freedom-engine-atom-flow-repair.js',
    ...
    existingScripts: [
      'scripts/process-source-maturity-freedom-engine-atom-flow-repair-check.mjs',
      'scripts/process-source-maturity-owners-lists-routing-gap-repair-check.mjs',
      'scripts/foundation-verify.mjs',
    ],
    existingPolicy: [
      'Freedom Engine source fact and atom already exist from prior governed repairs.',
    ...
    reused: [
      'Existing SRC-FREEDOM-ENGINE-001 source fact.',
      'Existing SRC-FREEDOM-ENGINE-001 accepted atom.',
    ...
    exactGap: 'SRC-FREEDOM-ENGINE-001 has a source fact and accepted atom but no retrieval chunk or internal action-route signal, so the live source maturity grid blocks at routed.',
    overBroadRisk: 'Freedom Engine routing repair must not fabricate atoms, read Sheets, or silently create trusted backlog/decision/question records.',
    readyBy: 'Steve approved continuous safe overnight Foundation source work; prior Freedom Engine atom-flow repair created the exact evidence needed for this route-ready repair.',
...
function buildCardRow({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  return {
    id: SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID,
    title: 'Repair Freedom Engine source maturity routing gap',
    scope: 'foundation',
    ...
    summary: 'Create a retrieval chunk from the existing Freedom Engine atom, then route that source-backed signal into internal review without applying or writing externally.',
    whyItMatters: 'Foundation should complete the Freedom Engine source maturity loop from existing governed evidence without touching Sheets, running extraction, or inventing source truth.',
    nextAction: closeCard
      ? `Done under \`${SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`. Continue safe source maturity/source-contract work from live truth.`
    ...
    statusNote: closeCard
      ? `Closed under \`${SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`; SRC-FREEDOM-ENGINE-001 now has a retrieval chunk and approval-required pending route signal.`
      : `Scope/proof: \`${SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`; no Sheets read/write, live extraction, auth/provider, or external-write work.`,
...
function buildSprintItem({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  return {
    ...
    planRef: SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PLAN_PATH,
    definitionOfDone: 'SRC-FREEDOM-ENGINE-001 has one active retrieval chunk backed by the existing accepted atom plus an approval-required pending internal action route; source maturity no longer blocks at routed for that source; no Google Sheets read/write, destination record apply, live extraction, or external write occurs; focused proof, backlog hygiene, foundation:verify, and process:foundation-ship pass.',
    proofCommands: SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PROOF_COMMANDS,
    readinessBlockerCleared: 'SOURCE-MATURITY-FREEDOM-ENGINE-ATOM-FLOW-REPAIR-001 created the accepted atom needed for bounded chunk and route repair.',
    notNextBoundaries: SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_NOT_NEXT_BOUNDARIES,
    existingWorkCheck: buildExistingWorkCheck(),
    metadata: {
      approvalRef: SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_APPROVAL_PATH,
...
async function upsertLiveCardAndPlanCritic({ closeCard = false, stage = 'building_now' } = {}) {
  const planSource = await readRepoFile(SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PLAN_PATH)
  const card = buildCardRow({ closeCard, stage })
  const planReview = evaluatePlanCriticPlan({
    planText: planSource,
    card,
    changedFiles: SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CHANGED_FILES,
  ...
  try {
    ...
    await client.query(
      ...
      `,
      [
        `source-maturity-freedom-engine-routing-${stableRunId(SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PLAN_PATH)}`,
        SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID,
        SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PLAN_PATH,
        planReview.status,
        planReview.score,
        ...
        JSON.stringify({ status: planReview.status, score: planReview.score, cardId: SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID }),
        'codex-source-maturity-freedom-engine-routing-gap-repair',
      ],
    )
    await client.query(
      `
        INSERT INTO change_events (event_type, entity_table, entity_id, actor, summary, metadata)
        VALUES ($1,'backlog_items',$2,'codex-source-maturity-freedom-engine-routing-gap-repair',$3,$4::jsonb)
      `,
      [
        closeCard ? 'backlog_status_changed' : 'backlog_updated',
        card.id,
        `${closeCard ? 'Closed' : 'Updated'} ${SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID}.`,
        JSON.stringify({ closeoutKey: SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY, stage }),
      ],
    )
...
async function ensureLiveState({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  await upsertFoundationCurrentSprintOverlay(
    ...
    'codex-source-maturity-freedom-engine-routing-gap-repair',
    {
      ...
      reason: closeCard
        ? 'Close Freedom Engine source maturity routing gap repair sprint item after focused proof.'
        : 'Start Freedom Engine source maturity routing gap repair sprint item with complete scaffold metadata.',
    },
  )
  return upsert
}
    ...
    const [facts, atoms, chunks, routeRows] = await Promise.all([
      pool.query(
        `
          ...
          LIMIT 1
        `,
        [SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_FACT_ID, SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID],
      ),
      pool.query(
        `
          ...
          LIMIT 1
        `,
        [SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID, SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_ATOM_ID],
      ),
      pool.query(
        `
          ...
          LIMIT 1
        `,
        [SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID, SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_CHUNK_ID],
      ),
      pool.query(
        `
...
async function ensureRetrievalChunk({ applyRepair = false, evidence = {} } = {}) {
  ...
  return upsertRetrievalChunk(chunk, 'codex-source-maturity-freedom-engine-routing-gap-repair')
}

async function buildLiveRepairSnapshot({ applyRepair = false } = {}) {
  const beforeGrid = await buildSourceMaturityGrid()
  const beforeRow = beforeGrid.rows.find(row => row.sourceId === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID) || {}
  ...
  const records = buildFreedomEngineRoutingRepairRecords(candidate)
  ...
  if (applyRepair && records.ok) {
    routeRow = await applySourceMaturityRoutingRepairRecords(records, {
      ...
      changeSummary: `Proposed Freedom Engine source maturity routing repair route for ${records.sourceId}.`,
    })
  }
  const afterGrid = await buildSourceMaturityGrid()
  const afterRow = afterGrid.rows.find(row => row.sourceId === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID) || {}
...
async function writeCloseout(snapshot) {
  await fs.writeFile(
    path.join(repoRoot, SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_PATH),
    renderFreedomEngineRoutingGapRepairCloseout(snapshot),
    'utf8',
  )
}
...
async function main() {
  ...
  const synthetic = buildSyntheticFreedomEngineRoutingGapRepairProof()
  const cards = await getBacklogItemsByIds([SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID])
  const activeSprint = await getActiveFoundationCurrentSprint()
  const planCriticRuns = await getPlanCriticRunsByCardIds([SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID])
  ...
  const activeItem = list(activeSprint?.items).find(item => item.cardId === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID) || null
  const card = cards[0] || buildCardRow({ closeCard: args.closeCard, stage: args.stage })
  const cardLane = text(card.lane)

  addCheck(checks, packageJson.scripts?.['process:source-maturity-freedom-engine-routing-gap-repair-check'] === `node --env-file-if-exists=.env ${SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_SCRIPT_PATH}`, 'package script is registered', packageJson.scripts?.['process:source-maturity-freedom-engine-routing-gap-repair-check'] || 'missing')
  ...
  addCheck(checks, Boolean(activeItem), 'Current Sprint includes Freedom Engine repair item', activeItem?.stage || 'missing')
  addCheck(checks, activeItem ? activeItem.planRef === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PLAN_PATH : false, 'Current Sprint item has plan ref', activeItem?.planRef || 'missing')
  addCheck(checks, activeItem ? list(activeItem.proofCommands).includes('npm run process:source-maturity-freedom-engine-routing-gap-repair-check -- --close-card --json') : false, 'Current Sprint proof commands include close-card proof', list(activeItem?.proofCommands).join('; ') || 'missing')
  ...
  addCheck(checks, synthetic.ok, 'synthetic Freedom Engine routing-gap dogfood passes', synthetic.evaluation?.failures?.map(failure => failure.check).join(', ') || 'ok')
  addCheck(checks, live.fact?.fact_id === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_FACT_ID || live.fact?.factId === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_FACT_ID, 'live candidate uses the bounded Freedom Engine fact', live.fact?.fact_id || live.fact?.factId || 'missing')
  addCheck(checks, live.atom?.atom_id === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_ATOM_ID || live.atom?.atomId === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_ATOM_ID, 'live candidate uses the bounded Freedom Engine atom', live.atom?.atom_id || live.atom?.atomId || 'missing')
  addCheck(checks, live.chunk?.chunk_id === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_CHUNK_ID || live.chunk?.chunkId === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_CHUNK_ID, 'bounded Freedom Engine retrieval chunk exists or is buildable', live.chunk?.chunk_id || live.chunk?.chunkId || 'missing')
  ...
  if (repairRequired) {
    ...
    addCheck(checks, live.status === 'healthy', 'live Freedom Engine repair snapshot is healthy', live.failures.map(failure => failure.check).join(', ') || 'healthy')
  } else {
    ...
    addCheck(checks, Boolean(live.fact), 'source-backed Freedom Engine fact exists before mutation', SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_FACT_ID)
    addCheck(checks, Boolean(live.atom), 'source-backed Freedom Engine atom exists before mutation', SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_TARGET_ATOM_ID)
    addCheck(checks, live.records.ok, 'Freedom Engine repair records verify before mutation', list(live.records.failures).join(', ') || live.records.route?.routeId)
  }
  addCheck(checks, planSource.includes('intelligence_retrieval_chunks') && planSource.includes('reject substring-only') && planSource.includes('full `process:foundation-ship`'), 'plan names real behavior proof and full ship gate', SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_PLAN_PATH)
  ...
  addCheck(checks, (await readRepoFile('lib/foundation-build-closeout-source-records.js')).includes(SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY), 'closeout registry includes Freedom Engine repair record', SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY)
  addCheck(checks, (await readRepoFile('lib/foundation-verify-coverage-card-ids.js')).includes(SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID), 'verifier coverage card list includes Freedom Engine repair card', SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CARD_ID)
  addCheck(checks, closeouts.some(record => record.key === SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY), 'build closeout registry exposes Freedom Engine repair closeout', SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY)

  if (args.closeCard) {
    await writeCloseout(live)
    const closeoutSource = await readRepoFile(SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_PATH)
    addCheck(checks, closeoutSource.includes(SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_KEY) && closeoutSource.includes('No live extraction'), 'closeout file is written', SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_GAP_REPAIR_CLOSEOUT_PATH)
  }
  ...
  if (args.json) {
    ...
    console.log(`Source maturity Freedom Engine routing gap repair check: ${result.status}`)
    console.log(`SOURCE_MATURITY_FREEDOM_ENGINE_ROUTING_REPAIR_SUMMARY ${JSON.stringify(result.repair)}`)
