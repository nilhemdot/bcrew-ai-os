...
assert((salesHtml.match(/class="found-nav-item"/g) || []).length === 2, 'Sales Hub sidebar should stay tight: Dashboard and GLS Manager only.')
assert(salesHtml.includes('found-nav-label">GLS System<'), 'Sales Hub sidebar must frame GLS as one system inside Sales Hub.')
assert(salesHtml.includes('href="#gls-system"') && salesHtml.includes('GLS Manager'), 'Sales Hub sidebar must link to the grouped GLS Manager work surface.')
assert(salesJs.includes('/api/sales-hub?refresh=1'), 'Sales Hub must keep a deliberate manual refresh path for live ClickUp data.')
assert(salesJs.includes('AIOS cached view'), 'Sales Hub should explain that leaders are using the cached AIOS view, not opening ClickUp.')
assert(salesJs.includes('Last ClickUp refresh') && salesJs.includes('sales-data-freshness'), 'Sales Hub must show data freshness next to the refresh control.')
assert(salesJs.includes('GLS edits save to AIOS immediately'), 'Sales Hub must explain save behavior versus ClickUp source refresh.')
assert(salesJs.includes('Refresh from ClickUp'), 'Sales Hub refresh control must be specific about pulling source data from ClickUp.')
assert(!salesJs.includes('Open ClickUp View'), 'Sales Hub must not ask sales leaders to open the ClickUp board.')
assert(salesJs.includes('found-nav-open') && !salesJs.includes('found-sidebar-open'), 'Sales Hub mobile menu must toggle the shared mobile nav class.')
assert(salesJs.includes('found-nav-active') && !salesJs.includes("classList.toggle('active'"), 'Sales Hub nav active state must use the shared Foundation nav class.')
assert(salesHtml.includes('aria-label=\"Open or close Sales navigation\"'), 'Sales Hub mobile menu label must follow the shared Foundation nav pattern.')
assert(serverJs.includes('stale_background_refresh'), 'Sales Hub must serve cached data immediately while stale ClickUp refresh runs in the background.')
assert((salesJs.match(/load\(\{ refresh: true \}\)/g) || []).length === 1, 'Only the manual Refresh data button should trigger a live ClickUp refresh.')
assert(salesJs.includes('sales-save-status'), 'Sales Hub must show explicit save/failure status for GLS writes.')
assert(salesJs.includes('Cards stayed open'), 'Sales Hub saves must not refresh and close open GLS cards.')
assert(!salesJs.includes("setSalesStatus('Saved listing case. Refreshing GLS data") && !salesJs.includes("setSalesStatus('Saved project case to ' + formatNumber(response.updatedCount || 0) + ' listings. Refreshing GLS data"), 'Sales Hub save actions must not reload the whole manager after every save.')
...
assert(salesJs.includes('Active GLS cases') && salesJs.includes('Needs owner') && salesJs.includes('Assigned'), 'GLS dashboard must expose active case, needs-owner, and assigned counts in the active pipeline scoreboard.')
assert(!salesJs.includes('Active GLS work'), 'GLS dashboard should not repeat the active work breakdown below the top scoreboard.')
...
assert(stylesCss.includes('sales-case-history') && !stylesCss.includes('sales-case-note'), 'GLS case history UI must be styled without a duplicate current-note card.')
...
assert(report.scoreboard.outcomeSummary.adjustedOrImplemented.caseCount >= report.scoreboard.resolvedResults.adjustedRelisted.caseCount, 'Adjusted/implemented outcome count must include adjusted cases.')
assert(report.scoreboard.outcomeSummary.soldClosed.caseCount === report.scoreboard.resolvedResults.soldClosed.caseCount, 'Sold outcome count must match sold/closed results.')
assert(report.scoreboard.outcomeSummary.failed.caseCount === report.scoreboard.resolvedResults.noActionOrBlocked.caseCount, 'Failed outcome count must match no-action/blocked results.')
...
assert(syntheticScoreboard.outcomeSummary.adjustedOrImplemented.caseCount === 1, 'Synthetic adjusted case must count as adjusted/implemented.')
assert(syntheticScoreboard.outcomeSummary.soldClosed.caseCount === 1, 'Synthetic sold case must count as sold.')
...
console.log(JSON.stringify({
  ...
  adjustedOrImplementedCases: report.scoreboard.outcomeSummary.adjustedOrImplemented.caseCount,
  ...
  leaderPerformance: report.scoreboard.leaderPerformance.map(row => ({
    ...
    adjustedOrImplementedCases: row.adjustedOrImplementedCases,
    soldCases: row.soldCases,
    stuckCases: row.stuckCases,
    failedCases: row.noActionOrBlockedCases,
    winRate: row.winRate,
  })),
