...
import {
  ...
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_CHANGED_FILES,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_PATH,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_PLAN_PATH,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_PROOF_COMMANDS,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_SCRIPT_PATH,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_SPRINT_ID,
  SOURCE_MATURITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
  buildSourceMaturityAtomFromFact,
  buildSyntheticSourceMaturityAtomFlowRepairProof,
  evaluateSourceMaturityAtomFlowRepair,
  renderSourceMaturityAtomFlowRepairCloseout,
  selectSourceMaturityAtomFlowRepairCandidate,
} from '../lib/source-maturity-atom-flow-repair.js'
...
function buildExistingWorkCheck() {
  return {
    ...
    existingPolicy: [
      'SOURCE-MATURITY-GAP-FOLLOWUP-001 routes atomized-stage maturity gaps to SOURCE-MATURITY-ATOM-FLOW-REPAIR-001.',
      'Atom-flow repair may use existing source-backed facts, atoms, candidates, or KB compiler evidence; it must not run extraction or fabricate atoms.',
      'SOURCE-MATURITY-EVIDENCE-GAP-REPAIR-001 created a governed source fact for SRC-OWNERS-LISTS-001 that can feed atom-flow proof.',
    ],
    reused: [
      'Existing SRC-OWNERS-LISTS-001 source fact evidence.',
      'Existing intelligence atom store and source maturity grid.',
      'Existing source maturity gap follow-up child card.',
    ],
    notRebuilt: [
    ...
    exactGap: 'SRC-OWNERS-LISTS-001 is extracted from governed source facts but has no fresh promoted intelligence atom signal, so the source maturity grid blocks at atomized.',
    overBroadRisk: 'Atom-flow repair must not fabricate atoms or imply synthesis/routing is done.',
    readyBy: 'Steve approved continuous safe overnight Foundation source work; SOURCE-MATURITY-GAP-FOLLOWUP-001 scoped atom-flow repair as a child queue.',
    readyAt: '2026-05-18T01:02:00-04:00',
  }
}
...
function buildCardRow({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  return {
    id: SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID,
    ...
    summary: 'Promote an existing source-backed Owners Lists source fact into one governed atom-flow signal without extraction, model calls, Sheets reads/writes, or external writes.',
    whyItMatters: 'Foundation should convert governed source facts into memory-spine atoms so source maturity advances without faking synthesis or routing.',
    nextAction: closeCard
      ? `Done under \`${SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\`. Continue the next safe source-maturity repair from live truth.`
      : 'Persist one source-backed atom for SRC-OWNERS-LISTS-001 from the existing governed source fact only.',
    statusNote: closeCard
      ? `Closed under \`${SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\`; SRC-OWNERS-LISTS-001 now has a fresh atom-flow signal and no longer blocks at atomized.`
      : `Scope/proof: \`${SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\`; no live extraction/auth/provider/external-write work.`,
...
function buildSprintItem({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  return {
    ...
    planRef: SOURCE_MATURITY_ATOM_FLOW_REPAIR_PLAN_PATH,
    definitionOfDone: 'SRC-OWNERS-LISTS-001 has a fresh governed intelligence atom and atom hit backed by an existing active source fact; source maturity no longer blocks at atomized for that source; no live extraction, Sheets read/write, atom fabrication, model call, or external write occurs; focused proof, backlog hygiene, foundation:verify, and process:foundation-ship pass.',
    proofCommands: SOURCE_MATURITY_ATOM_FLOW_REPAIR_PROOF_COMMANDS,
    readinessBlockerCleared: 'SOURCE-MATURITY-GAP-FOLLOWUP-001 scoped atom-flow repair and SRC-OWNERS-LISTS-001 already has an active source fact from evidence-gap repair.',
    notNextBoundaries: SOURCE_MATURITY_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES,
    existingWorkCheck: buildExistingWorkCheck(),
    metadata: {
      approvalRef: SOURCE_MATURITY_ATOM_FLOW_REPAIR_APPROVAL_PATH,
...
async function upsertLiveCardAndPlanCritic({ closeCard = false, stage = 'building_now' } = {}) {
  const planSource = await readRepoFile(SOURCE_MATURITY_ATOM_FLOW_REPAIR_PLAN_PATH)
  const card = buildCardRow({ closeCard, stage })
  const planReview = evaluatePlanCriticPlan({
    planText: planSource,
    card,
    changedFiles: SOURCE_MATURITY_ATOM_FLOW_REPAIR_CHANGED_FILES,
  ...
  try {
    ...
    await client.query(
      ...
      `,
      [
        `source-maturity-atom-flow-${stableRunId(SOURCE_MATURITY_ATOM_FLOW_REPAIR_PLAN_PATH)}`,
        SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID,
        SOURCE_MATURITY_ATOM_FLOW_REPAIR_PLAN_PATH,
        planReview.status,
        planReview.score,
        ...
        JSON.stringify({ status: planReview.status, score: planReview.score, cardId: SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID }),
        'codex-source-maturity-atom-flow-repair',
      ],
    )
    await client.query(
      `
        INSERT INTO change_events (event_type, entity_table, entity_id, actor, summary, metadata)
        VALUES ($1,'backlog_items',$2,'codex-source-maturity-atom-flow-repair',$3,$4::jsonb)
      `,
      [
        closeCard ? 'backlog_status_changed' : 'backlog_updated',
        card.id,
        `${closeCard ? 'Closed' : 'Updated'} ${SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID}.`,
        JSON.stringify({ closeoutKey: SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY, stage }),
      ],
    )
...
async function ensureLiveState({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  assertProcessCheckWriteAllowed({
    ...
    scriptPath: SOURCE_MATURITY_ATOM_FLOW_REPAIR_SCRIPT_PATH,
    operation: 'create/update source maturity atom-flow repair backlog card, Plan Critic row, and Current Sprint overlay',
  ...
  await upsertFoundationCurrentSprintOverlay(
    {
      sprint: {
        sprintId: SOURCE_MATURITY_ATOM_FLOW_REPAIR_SPRINT_ID,
        status: 'active',
        goal: 'Repair a source maturity atomized-stage atom-flow gap with existing governed source fact evidence only.',
        activeBlockerCardId: closeCard ? null : SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID,
        metadata: {
          stage: closeCard ? 'done_this_sprint' : normalizedStage,
          startedBy: 'codex-source-maturity-atom-flow-repair',
          ...
          nextAction: closeCard
            ...
            : 'Persist one source-backed atom from the existing Owners Lists source fact only.',
          priorityOrder: [SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID],
          notNext: SOURCE_MATURITY_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES,
          exitCriteria: [
            'SRC-OWNERS-LISTS-001 receives one active atom backed by an existing source fact.',
            'No live extraction, Sheets read/write, model/provider call, Drive mutation, or external write occurs.',
            'Source maturity no longer blocks at atomized for SRC-OWNERS-LISTS-001.',
    ...
    'codex-source-maturity-atom-flow-repair',
    {
      ...
      reason: closeCard
        ? 'Close source maturity atom-flow repair sprint item after focused proof.'
        : 'Start source maturity atom-flow repair sprint item with complete scaffold metadata.',
    },
  )
  return upsert
}
...
async function loadSourceFacts(sourceId = SOURCE_MATURITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID) {
  const pool = createPool()
  try {
...
async function runRepair({ apply = false } = {}) {
  ...
  const beforeRow = beforeGrid.rows.find(row => row.sourceId === SOURCE_MATURITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID) || {}
  const sourceFacts = await loadSourceFacts()
  const candidate = selectSourceMaturityAtomFlowRepairCandidate({ beforeRow, sourceFacts })
  const atomBuild = buildSourceMaturityAtomFromFact(candidate)
  let savedAtom = null
  let savedHit = null
  if (apply) {
    assertProcessCheckWriteAllowed({
      argv: process.argv.slice(2),
      scriptPath: SOURCE_MATURITY_ATOM_FLOW_REPAIR_SCRIPT_PATH,
      operation: 'persist source maturity atom-flow atom and atom hit',
      allowedFlags: [PROCESS_CHECK_WRITE_FLAGS.apply, PROCESS_CHECK_WRITE_FLAGS.closeCard],
    })
    savedAtom = await upsertIntelligenceAtom(atomBuild.atom, 'codex-source-maturity-atom-flow-repair')
    savedHit = await recordIntelligenceAtomHit(atomBuild.hit, 'codex-source-maturity-atom-flow-repair')
  } else {
  ...
  const afterRow = afterGrid.rows.find(row => row.sourceId === SOURCE_MATURITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID) || {}
...
async function writeCloseout(snapshot) {
  assertProcessCheckWriteAllowed({
    argv: process.argv.slice(2),
    scriptPath: SOURCE_MATURITY_ATOM_FLOW_REPAIR_SCRIPT_PATH,
    operation: 'write source maturity atom-flow repair closeout',
    allowedFlags: [PROCESS_CHECK_WRITE_FLAGS.closeCard],
  })
  await fs.writeFile(
    path.join(repoRoot, SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_PATH),
    renderSourceMaturityAtomFlowRepairCloseout(snapshot),
    'utf8',
  )
}
...
async function buildStatus({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  const cards = await getBacklogItemsByIds([SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID])
  const card = cards[0] || null
  const planCriticRuns = await getPlanCriticRunsByCardIds([SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID])
  const activeSprint = await getActiveFoundationCurrentSprint()
  const activeItem = list(activeSprint?.items).find(item => item.cardId === SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID) || null
  const synthetic = buildSyntheticSourceMaturityAtomFlowRepairProof()
  ...
  addCheck(checks, packageJson.scripts?.['process:source-maturity-atom-flow-repair-check'] === `node --env-file-if-exists=.env ${SOURCE_MATURITY_ATOM_FLOW_REPAIR_SCRIPT_PATH}`, 'package script is registered', packageJson.scripts?.['process:source-maturity-atom-flow-repair-check'] || 'missing')
  ...
  addCheck(checks, activeItem ? activeItem.planRef === SOURCE_MATURITY_ATOM_FLOW_REPAIR_PLAN_PATH : false, 'Current Sprint item has plan ref', activeItem?.planRef || 'missing')
  addCheck(checks, activeItem ? list(activeItem.proofCommands).includes('npm run process:source-maturity-atom-flow-repair-check -- --close-card --json') : false, 'Current Sprint proof commands include close-card proof', list(activeItem?.proofCommands).join('; '))
  addCheck(checks, currentSprintStatus.status === 'healthy', 'Current Sprint status is healthy', currentSprintStatus.status)
  addCheck(checks, synthetic.ok, 'synthetic atom-flow dogfood passes', synthetic.ok ? 'ok' : JSON.stringify(synthetic))
  if (repairRequired) {
  ...
  addCheck(checks, planSource.includes('intelligence_atoms') && planSource.includes('reject substring-only') && planSource.includes('full `process:foundation-ship`'), 'plan names real behavior proof and full ship gate', SOURCE_MATURITY_ATOM_FLOW_REPAIR_PLAN_PATH)
  addCheck(checks, (await readRepoFile('lib/foundation-build-closeout-source-records.js')).includes(SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY), 'closeout registry includes repair record', SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY)
  addCheck(checks, (await readRepoFile('lib/foundation-verify-coverage-card-ids.js')).includes(SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID), 'verifier coverage card list includes repair card', SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID)
  addCheck(checks, closeoutRecords.some(record => record.key === SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY), 'build closeout registry exposes repair closeout', SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY)
  if (closeCard) {
    const closeoutSource = await readRepoFile(SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_PATH)
    addCheck(checks, closeoutSource.includes(SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY) && closeoutSource.includes('No live extraction'), 'closeout file is written', SOURCE_MATURITY_ATOM_FLOW_REPAIR_CLOSEOUT_PATH)
  }
...
async function main() {
  ...
  try {
    ...
    if (args.json) {
      ...
      console.log(`Source maturity atom-flow repair check: ${status.status}`)
      console.log(`  Card: ${SOURCE_MATURITY_ATOM_FLOW_REPAIR_CARD_ID}`)
      console.log(`  Source: ${SOURCE_MATURITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID}`)
