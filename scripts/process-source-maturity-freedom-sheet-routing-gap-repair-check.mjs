...
import {
  ...
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CHANGED_FILES,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_PATH,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_NOT_NEXT_BOUNDARIES,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PLAN_PATH,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PROOF_COMMANDS,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_SCRIPT_PATH,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_SPRINT_ID,
  SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS,
  buildFreedomSheetRoutingRepairRecords,
  buildFreedomSheetRoutingRetrievalChunk,
  buildSyntheticFreedomSheetRoutingGapRepairProof,
  evaluateFreedomSheetRoutingGapRepairs,
  renderFreedomSheetRoutingGapRepairCloseout,
} from '../lib/source-maturity-freedom-sheet-routing-gap-repair.js'
import {
...
function buildExistingWorkCheck() {
  return {
    existingCode: [
      ...
      'lib/source-maturity-freedom-sheet-atom-flow-repair.js',
    ...
    existingScripts: [
      ...
      'scripts/process-source-maturity-freedom-sheet-atom-flow-repair-check.mjs',
      'scripts/process-source-maturity-fub-routing-gap-repair-check.mjs',
      'scripts/foundation-verify.mjs',
    ],
    existingPolicy: [
      'The Freedom Sheet source facts and accepted atoms already exist from prior governed repairs.',
      ...
      'Full backlog detail is owned by dedicated backlog routes; the full diagnostics route should use compact backlog rows.',
    ],
    reused: [
      'Existing SRC-FREEDOM-TEAM-001 and SRC-FREEDOM-COMMUNITY-REV-001 source facts.',
      'Existing accepted atoms for both Freedom Sheet targets.',
      'Existing retrieval chunk upsert store.',
      'Existing Action Route Review Inbox and promotion workflow semantics.',
      'Existing backlog contract/detail/archive routes for full backlog history.',
    ],
    notRebuilt: [
    ...
    exactGap: 'SRC-FREEDOM-TEAM-001 and SRC-FREEDOM-COMMUNITY-REV-001 have source facts and accepted atoms but no route signals, so the live source maturity grid blocks at routed.',
    overBroadRisk: 'Freedom Sheet routing repair must not fabricate atoms, read Sheets, run extraction, call providers, or silently create trusted backlog/decision/question records.',
    readyBy: 'Steve approved continuous safe overnight Foundation source work; the previous Freedom Sheet atom-flow repair created the exact evidence needed for route-ready repair.',
...
function buildCardRow({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  return {
    id: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID,
    title: 'Repair Freedom Sheet source maturity routing gap',
    scope: 'foundation',
    ...
    summary: 'Create bounded retrieval chunks and pending owner-decision routes for the two signed-off Freedom Sheet sources from existing facts and atoms only.',
    whyItMatters: 'Foundation should make Freedom Sheet source-backed facts reviewable as operator work without reading Sheets, running extraction, writing externally, or pretending route apply is approved.',
    nextAction: closeCard
      ? `Done under \`${SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`. Continue the next safe source-maturity/source-contract repair from live truth.`
    ...
    statusNote: closeCard
      ? `Closed under \`${SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`; both Freedom Sheet targets now have retrieval chunks and pending route signals.`
      : `Scope/proof: \`${SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`; no Sheets read/write, live extraction, provider call, route apply, or external-write work.`,
...
function buildSprintItem({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  return {
    ...
    planRef: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PLAN_PATH,
    definitionOfDone: 'SRC-FREEDOM-TEAM-001 and SRC-FREEDOM-COMMUNITY-REV-001 each have one active retrieval chunk backed by existing accepted atoms plus one approval-required pending internal action route; source maturity no longer blocks at routed for either source; no Sheets read/write, live extraction, provider call, destination apply, external write, or Drive mutation occurs; focused proof, backlog hygiene, foundation:verify, and process:foundation-ship pass.',
    proofCommands: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PROOF_COMMANDS,
    readinessBlockerCleared: 'SOURCE-MATURITY-FREEDOM-SHEET-ATOM-FLOW-REPAIR-001 created the accepted atoms needed for bounded chunk and route repair.',
    notNextBoundaries: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_NOT_NEXT_BOUNDARIES,
    existingWorkCheck: buildExistingWorkCheck(),
    metadata: {
      approvalRef: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_APPROVAL_PATH,
...
async function upsertLiveCardAndPlanCritic({ closeCard = false, stage = 'building_now' } = {}) {
  const planSource = await readRepoFile(SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PLAN_PATH)
  const card = buildCardRow({ closeCard, stage })
  const planReview = evaluatePlanCriticPlan({
    planText: planSource,
    card,
    changedFiles: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CHANGED_FILES,
  ...
  try {
    ...
    await client.query(
      ...
      `,
      [
        `source-maturity-freedom-sheet-routing-${stableRunId(SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PLAN_PATH)}`,
        SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID,
        SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PLAN_PATH,
        planReview.status,
        planReview.score,
        ...
        JSON.stringify({ status: planReview.status, score: planReview.score, cardId: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID }),
        'codex-source-maturity-freedom-sheet-routing-gap-repair',
      ],
    )
    await client.query(
      `
        INSERT INTO change_events (event_type, entity_table, entity_id, actor, summary, metadata)
        VALUES ($1,'backlog_items',$2,'codex-source-maturity-freedom-sheet-routing-gap-repair',$3,$4::jsonb)
      `,
      [
        closeCard ? 'backlog_status_changed' : 'backlog_updated',
        card.id,
        `${closeCard ? 'Closed' : 'Updated'} ${SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID}.`,
        JSON.stringify({ closeoutKey: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY, stage }),
      ],
    )
...
async function ensureLiveState({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  assertProcessCheckWriteAllowed({
    ...
    scriptPath: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_SCRIPT_PATH,
    operation: 'create/update Freedom Sheet routing repair backlog card, Plan Critic row, and Current Sprint overlay',
  ...
  await upsertFoundationCurrentSprintOverlay(
    {
      sprint: {
        sprintId: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_SPRINT_ID,
        status: 'active',
        goal: 'Repair the Freedom Sheet routed-stage source maturity gaps with bounded retrieval chunks and approval-required internal routes.',
        activeBlockerCardId: closeCard ? null : SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID,
        metadata: {
          stage: closeCard ? 'done_this_sprint' : normalizedStage,
          startedBy: 'codex-source-maturity-freedom-sheet-routing-gap-repair',
          ...
          nextAction: closeCard
            ...
            : 'Create bounded retrieval chunks and pending approval-required routes from existing Freedom Sheet evidence only.',
          priorityOrder: [SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID],
          notNext: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_NOT_NEXT_BOUNDARIES,
          exitCriteria: [
            'Both Freedom Sheet targets receive active retrieval chunks backed by existing atoms.',
            'Both Freedom Sheet targets receive pending internal action routes backed by fact, atom, and chunk refs.',
            'No route is applied and no destination record is created.',
            'Source maturity no longer blocks at routed for either Freedom Sheet target.',
    ...
    'codex-source-maturity-freedom-sheet-routing-gap-repair',
    {
      ...
      reason: closeCard
        ? 'Close Freedom Sheet routing repair sprint item after focused proof.'
        : 'Start Freedom Sheet routing repair sprint item with complete scaffold metadata.',
    },
  )
  return upsert
}
...
async function loadTargetEvidence() {
  const sourceIds = SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.map(target => target.sourceId)
  const factIds = SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.map(target => target.factId)
  const atomIds = SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.map(target => target.atomId)
  const chunkIds = SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.map(target => target.chunkId)
  const pool = createPool()
  try {
...
async function ensureRetrievalChunks({ applyRepair = false, evidence = {} } = {}) {
  ...
  for (const target of SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS) {
    ...
    if (applyRepair) {
      chunks.push(await upsertRetrievalChunk(chunk, 'codex-source-maturity-freedom-sheet-routing-gap-repair'))
    } else {
      chunks.push(chunk)
    }
  }
...
async function runRepair({ apply = false } = {}) {
  ...
  const targetSourceIds = SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.map(target => target.sourceId)
  ...
  for (const target of SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS) {
    ...
    const record = buildFreedomSheetRoutingRepairRecords(candidate, new Date().toISOString(), target)
    ...
    if (apply && record.ok) {
      routeRow = await applySourceMaturityRoutingRepairRecords(record, {
        ...
        changeSummary: `Proposed Freedom Sheet source maturity routing repair route for ${record.sourceId}.`,
      })
    }
  ...
  const evaluation = evaluateFreedomSheetRoutingGapRepairs({
    beforeRows,
    afterRows,
    candidates,
    records,
...
async function writeCloseout(snapshot) {
  assertProcessCheckWriteAllowed({
    argv: process.argv.slice(2),
    scriptPath: SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_SCRIPT_PATH,
    operation: 'write Freedom Sheet routing repair closeout',
    allowedFlags: [PROCESS_CHECK_WRITE_FLAGS.closeCard],
  })
  await fs.writeFile(
    path.join(repoRoot, SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_PATH),
    renderFreedomSheetRoutingGapRepairCloseout(snapshot),
    'utf8',
  )
}
...
async function buildStatus({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  const cards = await getBacklogItemsByIds([SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID])
  const card = cards[0] || null
  const planCriticRuns = await getPlanCriticRunsByCardIds([SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID])
  const activeSprint = await getActiveFoundationCurrentSprint()
  const activeItem = list(activeSprint?.items).find(item => item.cardId === SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID) || null
  const synthetic = buildSyntheticFreedomSheetRoutingGapRepairProof()
  ...
  const repairRequired = applyRepairNow || list(repair.routeRows).length >= SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.length
  ...
  addCheck(checks, packageJson.scripts?.['process:source-maturity-freedom-sheet-routing-gap-repair-check'] === `node --env-file-if-exists=.env ${SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_SCRIPT_PATH}`, 'package script is registered', packageJson.scripts?.['process:source-maturity-freedom-sheet-routing-gap-repair-check'] || 'missing')
  ...
  addCheck(checks, Boolean(activeItem), 'Current Sprint includes Freedom Sheet routing item', activeItem?.stage || 'missing')
  addCheck(checks, activeItem ? activeItem.planRef === SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PLAN_PATH : false, 'Current Sprint item has plan ref', activeItem?.planRef || 'missing')
  addCheck(checks, activeItem ? list(activeItem.proofCommands).includes('npm run process:source-maturity-freedom-sheet-routing-gap-repair-check -- --close-card --json') : false, 'Current Sprint proof commands include close-card proof', list(activeItem?.proofCommands).join('; '))
  addCheck(
    checks,
  ...
  addCheck(checks, synthetic.ok, 'synthetic Freedom Sheet routing dogfood passes', synthetic.ok ? 'ok' : JSON.stringify(synthetic))
  ...
  addCheck(checks, SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.every(target => factIds.includes(target.factId)), 'live candidates use the bounded Freedom Sheet source facts', factIds.join(', ') || 'missing')
  addCheck(checks, SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.every(target => atomIds.includes(target.atomId)), 'live candidates use the bounded Freedom Sheet atoms', atomIds.join(', ') || 'missing')
  addCheck(checks, SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.every(target => chunkIds.includes(target.chunkId)), 'bounded Freedom Sheet retrieval chunks exist or are buildable', chunkIds.join(', ') || 'missing')
  ...
  addCheck(checks, list(repair.chunks).every(chunk => chunk.metadata?.noGoogleSheetsReadWrite === true), 'bounded retrieval chunks record no Google Sheets read/write', JSON.stringify(list(repair.chunks).map(chunk => chunk.metadata || {})))

  if (repairRequired) {
    for (const check of repair.evaluation.checks) addCheck(checks, check.ok, check.check, check.detail)
    addCheck(checks, repair.evaluation.status === 'healthy', 'live Freedom Sheet routing repair snapshot is healthy', repair.evaluation.failures.map(failure => failure.check).join(', ') || 'healthy')
  } else {
    addCheck(checks, repair.beforeRows.length === SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.length, 'target source maturity rows exist before apply', `${repair.beforeRows.length}/${SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.length}`)
  ...
  addCheck(checks, planSource.includes('intelligence_retrieval_chunks') && planSource.includes('reject substring-only') && planSource.includes('full `process:foundation-ship`'), 'plan names real behavior proof and full ship gate', SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_PLAN_PATH)
  const hubReadRoutesSource = await readRepoFile('lib/hub-read-routes.js')
  addCheck(
    checks,
      hubReadRoutesSource.includes('fullDiagnosticsCompacted') &&
      hubReadRoutesSource.includes('/api/foundation/backlog/:cardId') &&
      hubReadRoutesSource.includes('/api/foundation/backlog/done-archive') &&
      hubReadRoutesSource.includes('buildFoundationHubBacklogContract') &&
      hubReadRoutesSource.includes('compactFullDiagnosticsBacklogItems') &&
      hubReadRoutesSource.includes('fullRowById') &&
      hubReadRoutesSource.includes('return Object.keys(fullRow).length ? fullRow : row') &&
      hubReadRoutesSource.includes('compactFoundationActionRouterForHub') &&
      hubReadRoutesSource.includes('/api/foundation/action-route-review-inbox'),
    'full diagnostics payload is compacted but preserves historical proof fields behind dedicated detail routes',
    'lib/hub-read-routes.js',
  )
  ...
  addCheck(checks, (await readRepoFile('lib/foundation-build-closeout-source-records.js')).includes(SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY), 'closeout registry includes Freedom Sheet routing repair record', SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY)
  addCheck(checks, (await readRepoFile('lib/foundation-verify-coverage-card-ids.js')).includes(SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID), 'verifier coverage card list includes Freedom Sheet routing repair card', SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID)
  addCheck(checks, closeoutRecords.some(record => record.key === SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY), 'build closeout registry exposes Freedom Sheet routing repair closeout', SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY)
  if (closeCard) {
    const closeoutSource = await readRepoFile(SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_PATH)
    addCheck(checks, closeoutSource.includes(SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_KEY) && closeoutSource.includes('No live extraction'), 'closeout file is written', SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CLOSEOUT_PATH)
  }
...
async function main() {
  ...
  try {
    ...
    if (args.json) {
      ...
      console.log(`Source maturity Freedom Sheet routing gap repair check: ${status.status}`)
      console.log(`  Card: ${SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_CARD_ID}`)
      console.log(`  Sources: ${SOURCE_MATURITY_FREEDOM_SHEET_ROUTING_GAP_REPAIR_TARGETS.map(target => target.sourceId).join(', ')}`)
