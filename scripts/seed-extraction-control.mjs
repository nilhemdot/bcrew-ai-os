...
const extractionTargets = [
  ...
  },
  {
    ...
    title: 'Google Calendar current-window sync lane',
    lane: 'current_day',
    targetType: 'delegated_calendar_window',
    status: 'active',
    priority: 'P1',
    runtimeMode: 'scheduled',
    cursorState: { windowHours: 72, lookbackHours: 24, calendarId: 'primary', cursorType: 'calendar_time_window' },
    budget: { maxItemsPerRun: 50, maxRuntimeSeconds: 600, llmBudget: 'none', scheduleEveryMinutes: 1440 },
    dedupePolicy: { key: 'calendar_id:event_id:updated', idempotent: true },
    metadata: {
      foundationJobKey: 'calendar-sync-current',
      sourceScript: 'calendar:sync-events',
    ...
    notes: 'Scheduled read-only Calendar lane: archives a bounded event window into source-backed artifacts and crawl items. No calendar writes, invites, updates, deletes, descriptions, or raw notes.',
  },
  {
  ...
  },
  {
    ...
    sourceId: 'SRC-GMAIL-001',
    title: 'Email attachment content extraction lane',
    lane: 'backfill',
    targetType: 'gmail_attachment_text',
    status: 'active',
    priority: 'P0',
    runtimeMode: 'scheduled',
    cursorState: { cursorType: 'gmail_attachment_query_queue', query: 'has:attachment newer_than:30d' },
    budget: {
      ...
      maxTextChars: 250000,
      retrySkippedReasonPrefixes: ['calendar_invite_not_in_v1'],
      llmBudget: 'none_for_archive',
      requiresFiledOutput: true,
    },
    dedupePolicy: { key: 'mailbox:message_id:part_id:filename:size', idempotent: true },
    metadata: {
      foundationJobKey: 'email-attachment-extract-bite',
      scheduleEveryMinutes: 1440,
      backlogIds: ['EMAIL-ATTACHMENTS-001', 'EXTRACTION-TEAM-001'],
      sourceNote: 'docs/source-notes/shared-communications.md',
      officialApiBasis: ['Gmail users.messages.attachments.get'],
      v1Scope: 'Gmail PDF/text/calendar attachments. Missive attachments, Office conversion, OCR, and media route through explicit follow-on lanes.',
    },
    notes: 'Scheduled daily mission lane: extract a bounded quota of Gmail PDF/text attachments into source-backed artifacts. Unsupported attachments get explicit skip reasons; Missive attachments remain a follow-on after live shape proof.',
  },
  {
  ...
  },
  {
    ...
    metadata: {
      ...
      officialApiBasis: ['Drive files.export for Google Docs', 'Sheets API spreadsheets.values.get for Google Sheets', 'Drive files.get alt=media for PDFs/text blobs', 'Drive files.get alt=media plus local DOCX XML text extraction'],
    },
    notes: 'Scheduled daily mission lane: extract a bounded quota of Google Docs/Sheets/PDF/text content from the Drive inventory queue into source-backed artifacts. Unsupported files get explicit skip reasons.',
  },
  {
  ...
  },
  {
    ...
    sourceId: 'SRC-VIDEO-001',
    title: 'Video content transcript extraction lane',
    lane: 'corpus_mining',
    targetType: 'video_transcript_text',
    status: 'active',
    priority: 'P0',
    runtimeMode: 'scheduled',
    cursorState: { cursorType: 'video_link_manifest_queue', inventoryTargetKey: 'video-link-inventory' },
    budget: {
      missionMode: 'daily_quota',
    ...
    dedupePolicy: { key: 'normalized_url:platform:extractor_version', idempotent: true },
    metadata: {
      foundationJobKey: 'video-content-extract-bite',
      scheduleEveryMinutes: 1440,
      backlogIds: ['MEETING-VIDEO-001', 'MULTIMODAL-EXTRACTOR-001', 'YOUTUBE-SCOUT-001', 'EXTRACTION-TEAM-001'],
      sourceNote: 'docs/source-notes/video-link-inventory.md',
      officialApiBasis: ['DataForSEO YouTube Video Subtitles live advanced'],
      v1Scope: 'YouTube video subtitles from the existing video-link manifest. Channel/profile links route to creator watchlist; Loom/Drive/Skool/Zoom and no-subtitle videos remain explicit follow-on lanes.',
    },
    notes: 'Scheduled daily mission lane: extract a bounded quota of video transcript outputs from the video manifest. V1 handles YouTube subtitle text first; richer watch/vision extraction remains under the multimodal contract.',
  },
  {
