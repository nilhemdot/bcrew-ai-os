...
async function main() {
  ...
  addCheck(checks, !findingById.has('scheduled_job_foundation-verify') || (findingById.get('scheduled_job_foundation-verify').classification?.owner === 'Foundation Builder' && findingById.get('scheduled_job_foundation-verify').classification?.repairCardId === 'FOUNDATION-VERIFY-HEALTH-REPAIR-001'), 'scheduled foundation verifier is green/resolved or routes to verifier health repair', 'scheduled_job_foundation-verify')
  ...
  addCheck(checks, !findingById.has('scheduled_job_conditional-deal-review-readonly') || (findingById.get('scheduled_job_conditional-deal-review-readonly').classification?.owner === 'Steve' && findingById.get('scheduled_job_conditional-deal-review-readonly').classification?.repairCardId === 'ADMIN-DEAL-POLICY-SOURCE-CONTRACT-001'), 'conditional deal forecast sync is green/resolved or Steve approval-bound', 'scheduled_job_conditional-deal-review-readonly')
