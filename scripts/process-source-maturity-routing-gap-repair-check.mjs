...
import {
  ...
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_CHANGED_FILES,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_PATH,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_NOT_NEXT_BOUNDARIES,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_PLAN_PATH,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_PROOF_COMMANDS,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_SCRIPT_PATH,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_SPRINT_ID,
  SOURCE_MATURITY_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID,
  buildSourceMaturityRoutingRepairRecords,
  buildSyntheticSourceMaturityRoutingGapRepairProof,
  evaluateSourceMaturityRoutingGapRepair,
  renderSourceMaturityRoutingGapRepairCloseout,
...
function buildExistingWorkCheck() {
  return {
    ...
    existingPolicy: [
      'SOURCE-MATURITY-GAP-FOLLOWUP-001 routes routed-stage maturity gaps to SOURCE-MATURITY-ROUTING-GAP-REPAIR-001.',
    ...
    reused: [
      'Existing intelligence atoms for source-backed evidence.',
      'Existing intelligence retrieval chunks for evidence chunk references.',
      'Existing source-health facts for extraction/monitoring proof.',
    ...
    exactGap: 'SRC-SLACK-001 has current extracted/atomized/synthesized evidence but no internal action route signal, so the source maturity grid blocks at routed.',
    overBroadRisk: 'Routing a source maturity gap must not silently create trusted backlog/decision/question records or apply any external action.',
    readyBy: 'Steve approved continuous safe overnight Foundation source work, and SOURCE-MATURITY-GAP-FOLLOWUP-001 scoped routing repair as a child queue.',
    readyAt: '2026-05-18T05:45:00-04:00',
  }
}
...
function buildCardRow({ closeCard = false, stage = 'building_now' } = {}) {
  ...
  return {
    id: SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID,
    ...
    summary: 'Route existing source-backed Slack intelligence into the internal action-route review layer without applying or writing externally.',
    whyItMatters: 'Foundation source maturity should not stall after facts and atoms exist; source-backed signals need a safe internal review route before they become action.',
    nextAction: closeCard
      ? `Done under \`${SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`. Continue the next safe source-maturity child repair from live truth.`
      : 'Create an approval-required pending internal action route for an existing source-backed Slack signal, prove no destination apply/external write, and clear the routed-stage maturity gap.',
    statusNote: closeCard
      ? `Closed under \`${SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`; SRC-SLACK-001 now has an approval-required pending route signal.`
      : `Scope/proof: \`${SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY}\`; no live extraction/auth/provider/external-write work.`,
...
function buildSprintItem({ closeCard = false, stage = 'building_now' } = {}) {
  const normalizedStage = normalizeStage(stage)
  return {
    cardId: SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID,
    order: 1,
    stage: closeCard ? 'done_this_sprint' : normalizedStage,
    planRef: SOURCE_MATURITY_ROUTING_GAP_REPAIR_PLAN_PATH,
    definitionOfDone: 'SRC-SLACK-001 has an approval-required pending internal action route backed by an active source-health fact, intelligence atom, and retrieval chunk; source maturity no longer blocks at routed for that source; no destination record is applied; focused proof, backlog hygiene, foundation:verify, and process:foundation-ship pass.',
    proofCommands: SOURCE_MATURITY_ROUTING_GAP_REPAIR_PROOF_COMMANDS,
    readinessBlockerCleared: 'SOURCE-MATURITY-GAP-FOLLOWUP-001 scoped the routing child card and live Slack evidence has active source-health fact, atom, and retrieval chunk refs.',
    notNextBoundaries: SOURCE_MATURITY_ROUTING_GAP_REPAIR_NOT_NEXT_BOUNDARIES,
    existingWorkCheck: buildExistingWorkCheck(),
    metadata: {
      approvalRef: SOURCE_MATURITY_ROUTING_GAP_REPAIR_APPROVAL_PATH,
...
async function upsertLiveCardAndPlanCritic({ closeCard = false, stage = 'building_now' } = {}) {
  const planSource = await readRepoFile(SOURCE_MATURITY_ROUTING_GAP_REPAIR_PLAN_PATH)
  const card = buildCardRow({ closeCard, stage })
  const planReview = evaluatePlanCriticPlan({
    planText: planSource,
    card,
    changedFiles: SOURCE_MATURITY_ROUTING_GAP_REPAIR_CHANGED_FILES,
  ...
  try {
    ...
    await client.query(
      ...
      `,
      [
        `source-maturity-routing-gap-${stableRunId(SOURCE_MATURITY_ROUTING_GAP_REPAIR_PLAN_PATH)}`,
        SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID,
        SOURCE_MATURITY_ROUTING_GAP_REPAIR_PLAN_PATH,
        planReview.status,
        planReview.score,
        ...
        JSON.stringify({ status: planReview.status, score: planReview.score, cardId: SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID }),
    ...
    await client.query(
      `
        ...
        VALUES ($1,'backlog_items',$2,'codex-source-maturity-routing-gap-repair',$3,$4::jsonb)
      `,
      [
        closeCard ? 'backlog_status_changed' : 'backlog_updated',
        card.id,
        `${closeCard ? 'Closed' : 'Updated'} ${SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID}.`,
        JSON.stringify({ closeoutKey: SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY, stage }),
      ],
    )
  ...
  await upsertFoundationCurrentSprintOverlay(
    ...
    'codex-source-maturity-routing-gap-repair',
    {
      ...
      reason: closeCard
        ...
        : 'Start source maturity routing gap repair sprint item with complete scaffold metadata.',
    },
  )
  return upsert
}
...
async function applyRoutingRepair(records) {
  ...
  try {
    ...
    await client.query(
      `
        INSERT INTO change_events (event_type, entity_table, entity_id, actor, summary, metadata)
        VALUES ('intelligence_action_route_proposed','intelligence_action_routes',$1,'codex-source-maturity-routing-gap-repair',$2,$3::jsonb)
      `,
      [
        records.route.routeId,
        `Proposed source maturity routing repair route for ${records.sourceId}.`,
        JSON.stringify({
          cardId: SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID,
          closeoutKey: SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY,
...
async function buildLiveRepairSnapshot({ applyRepair = false } = {}) {
  ...
  const beforeRow = beforeGrid.rows.find(row => row.sourceId === SOURCE_MATURITY_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID) || {}
  ...
  const records = buildSourceMaturityRoutingRepairRecords(candidate)
  ...
  const afterRow = afterGrid.rows.find(row => row.sourceId === SOURCE_MATURITY_ROUTING_GAP_REPAIR_TARGET_SOURCE_ID) || {}
...
async function writeCloseout(snapshot) {
  await fs.writeFile(
    path.join(repoRoot, SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_PATH),
    renderSourceMaturityRoutingGapRepairCloseout(snapshot),
    'utf8',
  )
}
...
async function main() {
  ...
  const synthetic = buildSyntheticSourceMaturityRoutingGapRepairProof()
  const cards = await getBacklogItemsByIds([SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID])
  const activeSprint = await getActiveFoundationCurrentSprint()
  const planCriticRuns = await getPlanCriticRunsByCardIds([SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID])
  ...
  const activeItem = list(activeSprint?.items).find(item => item.cardId === SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID) || null
  const card = cards[0] || buildCardRow({ closeCard: args.closeCard, stage: args.stage })
  const cardLane = text(card.lane)

  addCheck(checks, packageJson.scripts?.['process:source-maturity-routing-gap-repair-check'] === `node --env-file-if-exists=.env ${SOURCE_MATURITY_ROUTING_GAP_REPAIR_SCRIPT_PATH}`, 'package script is registered', packageJson.scripts?.['process:source-maturity-routing-gap-repair-check'] || 'missing')
  ...
  addCheck(checks, activeItem ? activeItem.planRef === SOURCE_MATURITY_ROUTING_GAP_REPAIR_PLAN_PATH : false, 'Current Sprint item has plan ref', activeItem?.planRef || 'missing')
  addCheck(checks, activeItem ? list(activeItem.proofCommands).includes('npm run process:source-maturity-routing-gap-repair-check -- --close-card --json') : false, 'Current Sprint proof commands include close-card proof', list(activeItem?.proofCommands).join('; ') || 'missing')
  ...
  addCheck(checks, synthetic.ok, 'synthetic routing-gap dogfood passes', synthetic.evaluation?.failures?.map(failure => failure.check).join(', ') || 'ok')
  ...
  if (repairRequired) {
    ...
    addCheck(checks, live.status === 'healthy', 'live repair snapshot is healthy', live.failures.map(failure => failure.check).join(', ') || 'healthy')
  } else {
    ...
    addCheck(checks, live.candidate.ok, 'source-backed repair candidate exists before mutation', list(live.candidate.failures).join(', ') || live.candidate.sourceId)
    addCheck(checks, live.records.ok, 'repair records verify before mutation', list(live.records.failures).join(', ') || live.records.route?.routeId)
  }
  addCheck(checks, planSource.includes('intelligence_action_routes') && planSource.includes('reject substring-only') && planSource.includes('full `process:foundation-ship`'), 'plan names real behavior proof and full ship gate', SOURCE_MATURITY_ROUTING_GAP_REPAIR_PLAN_PATH)
  addCheck(checks, (await readRepoFile('lib/intelligence-synthesis.js')).includes("metadata->>'qualityExempt'"), 'synthesis quality snapshot excludes targeted repair runs', 'qualityExempt guard')
  addCheck(checks, (await readRepoFile('lib/foundation-build-closeout-source-records.js')).includes(SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY), 'closeout registry includes repair record', SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY)
  addCheck(checks, (await readRepoFile('lib/foundation-verify-coverage-card-ids.js')).includes(SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID), 'verifier coverage card list includes repair card', SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID)

  if (args.closeCard) {
    await writeCloseout(live)
    const closeoutSource = await readRepoFile(SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_PATH)
    addCheck(checks, closeoutSource.includes(SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_KEY) && closeoutSource.includes('No live extraction'), 'closeout file is written', SOURCE_MATURITY_ROUTING_GAP_REPAIR_CLOSEOUT_PATH)
  }
  ...
  if (args.json) {
    ...
    console.log(`Source maturity routing gap repair check: ${result.status}`)
    console.log(`SOURCE_MATURITY_ROUTING_REPAIR_SUMMARY ${JSON.stringify(result.repair)}`)
