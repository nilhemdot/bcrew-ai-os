...
async function main() {
  ...
  addCheck(checks, dogfood.ok === true, 'dogfood proves active, blocked, partial, and planned source-family handling', JSON.stringify(dogfood.cases))
  ...
  addCheck(checks, creatorStack?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial && /newsletter/i.test(creatorStack?.nextAction || '') && /Skool/i.test(creatorStack?.nextAction || ''), 'creator source stack is visible across source surfaces', creatorStack?.nextAction || 'missing')
  addCheck(checks, newsletters?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial && /AIOS Sources\/Newsletters/.test(newsletters?.nextAction || ''), 'newsletter source lane is visible with mailbox routing', newsletters?.nextAction || 'missing')
  ...
  addCheck(checks, skoolFree?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial && /last 20 days/i.test(skoolFree?.nextAction || '') && /paid\/private/i.test(skoolFree?.nextAction || ''), 'free Skool/community lane is visible separately from paid Skool blockers', skoolFree?.nextAction || 'missing')
  ...
  addCheck(checks, github?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial && Boolean(github?.nextCard), 'GitHub/repos have public/free runtime proof with repo worker still pending', github?.nextCard || 'missing')
