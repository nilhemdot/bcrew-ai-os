...
function extractClaimedNpmScriptsFromText(text) {
  ...
  for (const match of String(text || '').matchAll(scriptPattern)) {
    ...
    if (/^(http|https|localhost|source|status|route|strategy|foundation|runtime|recent|build|proof|process|health|fetch|scout|atom|hit|artifact|research-pool)$/i.test(prefix)) continue
    claims.add(value)
  }
  return Array.from(claims)
}
