...
async function main() {
  ...
  addCheck(
    ...
    fixtureReadiness.ok === true &&
      ...
      fixtureChecks.some(check => check.checkId === 'newsletter-issue-extraction' && check.kind === 'archive_issue_extraction' && check.status === 'readback_available_after_confirmation' && /newsletter:issue-extraction/.test(check.statusCommand || '') && /SRC-GMAIL-001/.test(check.secretRef || '')) &&
  ...
  addCheck(
    ...
    liveReadiness.ok === true &&
      ...
      liveChecks.some(check => check.checkId === 'newsletter-issue-extraction' && check.kind === 'archive_issue_extraction' && /newsletter:issue-extraction/.test(check.statusCommand || '') && /--confirmed/.test(check.statusCommand || '') && /--json/.test(check.statusCommand || '') && check.secretRef === CREATOR_NEWSLETTER_MAILBOX_SOURCE_ID) &&
