...
async function main() {
  ...
  console.log(`  Recipient field hash proof: ${status.summary.recipientSourceFieldHashPresent ? 'yes' : 'no'}`)
  console.log(`  Recipient source present/valid: ${status.summary.recipientSourcePresent ? 'yes' : 'no'}/${status.summary.recipientSourceValid ? 'yes' : 'no'}`)
  console.log(`  Internal oversight mode: ${status.summary.internalOversightMode || 'missing'}`)
  console.log(`  BCC roles applied: ${status.summary.bccRolesApplied.join(', ') || 'none'}`)
  console.log(`  BCC actual send roles: ${status.summary.bccActualSendRoles.join(', ') || 'none'}`)
  console.log(`  Missing BCC/internal oversight config roles: ${status.summary.bccMissingConfiguredRoles.join(', ') || 'none'}`)
  console.log(`  BCC recipient deduped roles: ${status.summary.bccRecipientDedupedRoles.join(', ') || 'none'}`)
