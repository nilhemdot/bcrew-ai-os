...
const CRITICAL_PROCESS_CHECK_SCRIPTS = Object.freeze([
  'process:source-browser-agent-harness-check',
  'process:source-browser-agent-executor-check',
  'process:source-browser-fallback-executor-check',
  'process:source-browser-brain-route-policy-check',
  'process:source-session-broker-check',
  'process:source-session-readiness-check',
])
...
function packageScriptSummary(packageJson = {}) {
  ...
  const processChecks = entries.filter(key => key.startsWith('process:'))
  const prioritizedProcessChecks = [
    ...CRITICAL_PROCESS_CHECK_SCRIPTS.filter(key => processChecks.includes(key)),
    ...processChecks,
  ]
  return {
    count: entries.length,
    sourceScripts: entries.filter(key => key.startsWith('source:')).slice(0, 120),
    processChecks: Array.from(new Set(prioritizedProcessChecks)).slice(0, 360),
...
export async function buildAgenticCodebaseMap({
  ...
  return {
    ...
    readFirst: [
      ...
      'docs/_archive/handoffs/2026-05-28-agentic-codebase-map.md',
    ],
