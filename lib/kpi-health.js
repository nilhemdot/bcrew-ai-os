...
function coerceKpiHealthPeriodYear(value, label = 'KPI health period year') {
  const year = Number(value)
  if (!Number.isInteger(year) || year < 2000 || year > 2100) {
    throw new Error(`${label} must be a four-digit year between 2000 and 2100.`)
  }
  return year
}

function formatKpiHealthPeriodDate(year, monthDay) {
  return `${String(year).padStart(4, '0')}-${monthDay}`
}

export function buildKpiHealthPeriodContract(options = {}) {
  const now = options.now instanceof Date ? options.now : new Date(options.now || Date.now())
  if (Number.isNaN(now.getTime())) throw new Error('KPI health period contract requires a valid runtime date.')
  const configuredYear = options.configuredYear ?? process.env.KPI_HEALTH_PERIOD_YEAR
  const hasConfiguredYear = configuredYear !== undefined && configuredYear !== null && String(configuredYear).trim() !== ''
  const year = hasConfiguredYear
    ? coerceKpiHealthPeriodYear(configuredYear, 'KPI_HEALTH_PERIOD_YEAR')
    : now.getFullYear()
  return {
    contractVersion: KPI_HEALTH_CONTRACT_VERSION,
    sourceId: KPI_HEALTH_PERIOD_CONTRACT_SOURCE_ID,
    mode: hasConfiguredYear ? 'configured_year' : 'runtime_calendar_year',
    year,
    periodStart: formatKpiHealthPeriodDate(year, '01-01'),
    periodEnd: formatKpiHealthPeriodDate(year, '12-31'),
    selectedAt: now.toISOString(),
    timezone: process.env.TZ || 'America/Toronto',
    configuredBy: hasConfiguredYear ? 'KPI_HEALTH_PERIOD_YEAR' : null,
  }
}

function buildKpiHealthRpcParams(shape, periodContract) {
  const period = periodContract || buildKpiHealthPeriodContract()
  const start = period.periodStart
  const end = period.periodEnd
  if (shape === 'company_dashboard_stats') {
    return {
      target_year: period.year,
      leads_start: start,
      leads_end: end,
      consults_set_start: start,
      consults_set_end: end,
      consults_held_start: start,
      consults_held_end: end,
      clients_signed_start: start,
      clients_signed_end: end,
    }
  }
  if (shape === 'company_leads') {
    return {
      leads_start: start,
      leads_end: end,
    }
  }
  if (shape === 'company_appointments') {
    return {
      range_start: start,
      range_end: end,
    }
  }
  if (shape === 'mqy_metrics') {
    return {
      p_start_date: start,
      p_end_date: end,
    }
  }
  throw new Error(`Unsupported KPI health RPC period shape: ${shape}`)
}

export const EXPECTED_KPI_RPC_DEFINITIONS = [
  {
    ...
    periodParamShape: 'company_dashboard_stats',
  ...
  },
  {
    ...
    periodParamShape: 'company_leads',
  ...
  },
  {
    ...
    periodParamShape: 'company_appointments',
...
export function getExpectedKpiRpcs(options = {}) {
  const periodContract = options.periodContract || buildKpiHealthPeriodContract(options)
  return EXPECTED_KPI_RPC_DEFINITIONS.map(definition => ({
    ...definition,
    params: buildKpiHealthRpcParams(definition.periodParamShape, periodContract),
    periodContract,
  }))
}

export const EXPECTED_KPI_RPCS = getExpectedKpiRpcs()
...
function collectKpiHealthPeriodParamYears(params = {}) {
  return Object.values(params || {})
    .map(value => String(value || '').match(/^(\d{4})(?:-\d{2}-\d{2})?$/))
    .filter(Boolean)
    .map(match => Number(match[1]))
    .filter(Number.isFinite)
}

export function evaluateKpiHealthDynamicYearContract({
  rpcDefinitions = [],
  periodContract = buildKpiHealthPeriodContract(),
} = {}) {
  const checks = []
  const expectedRpcs = new Map(getExpectedKpiRpcs({ periodContract }).map(definition => [definition.rpc, definition]))
  for (const definition of rpcDefinitions || []) {
    const expected = expectedRpcs.get(definition.rpc)
    if (!expected) {
      checks.push({
        ok: false,
        rpc: definition.rpc || 'unknown',
        check: 'rpc is part of KPI health contract',
        detail: 'unexpected rpc',
      })
      continue
    }
    const actualParams = definition.params || {}
    const expectedParams = expected.params || {}
    const mismatches = Object.entries(expectedParams)
      .filter(([key, value]) => actualParams[key] !== value)
      .map(([key, value]) => `${key}: expected ${value}, got ${actualParams[key] ?? 'missing'}`)
    const observedYears = collectKpiHealthPeriodParamYears(actualParams)
    checks.push({
      ok: mismatches.length === 0 && observedYears.every(year => year === periodContract.year),
      rpc: definition.rpc,
      check: 'rpc params match selected KPI health period',
      detail: mismatches.length ? mismatches.join('; ') : `year=${periodContract.year}`,
    })
  }
  const missingRpcs = Array.from(expectedRpcs.keys()).filter(rpc =>
    !(rpcDefinitions || []).some(definition => definition.rpc === rpc)
  )
  for (const rpc of missingRpcs) {
    checks.push({
      ok: false,
      rpc,
      check: 'expected KPI rpc is present',
      detail: 'missing rpc definition',
    })
  }
  return {
...
export function buildKpiHealthDynamicYearContractDogfoodProof() {
  const runtime2027 = buildKpiHealthPeriodContract({ now: new Date('2027-04-15T12:00:00-04:00') })
  const dynamic2027 = getExpectedKpiRpcs({ periodContract: runtime2027 })
  const dynamicEvaluation = evaluateKpiHealthDynamicYearContract({
    rpcDefinitions: dynamic2027,
    periodContract: runtime2027,
  })
  const frozen2026 = dynamic2027.map(definition => ({
    ...definition,
    // Dogfood the old failure mode without reintroducing a literal live-year window.
    params: buildKpiHealthRpcParams(definition.periodParamShape, {
      ...runtime2027,
      year: runtime2027.year - 1,
      periodStart: formatKpiHealthPeriodDate(runtime2027.year - 1, '01-01'),
      periodEnd: formatKpiHealthPeriodDate(runtime2027.year - 1, '12-31'),
    }),
  }))
  const frozenEvaluation = evaluateKpiHealthDynamicYearContract({
    rpcDefinitions: frozen2026,
    periodContract: runtime2027,
  })
  const configured2028 = buildKpiHealthPeriodContract({
    now: new Date('2027-04-15T12:00:00-04:00'),
    configuredYear: 2028,
  })
  const configuredEvaluation = evaluateKpiHealthDynamicYearContract({
    rpcDefinitions: getExpectedKpiRpcs({ periodContract: configured2028 }),
    periodContract: configured2028,
  })
  return {
    ok: dynamicEvaluation.ok === true &&
      frozenEvaluation.ok === false &&
      configuredEvaluation.ok === true &&
      runtime2027.year === 2027 &&
      dynamic2027.every(definition => collectKpiHealthPeriodParamYears(definition.params).every(year => year === 2027)),
    runtime2027,
    dynamic2027: {
      ok: dynamicEvaluation.ok,
      rpcCount: dynamic2027.length,
      failures: dynamicEvaluation.failures,
    },
    frozen2026: {
      ok: frozenEvaluation.ok,
      rejected: frozenEvaluation.ok === false,
      failures: frozenEvaluation.failures,
    },
    configured2028: {
      ok: configuredEvaluation.ok,
      mode: configured2028.mode,
      year: configured2028.year,
      failures: configuredEvaluation.failures,
    },
    dogfoodInvariant: 'A synthetic 2027 runtime produces 2027 KPI health params, while the old frozen-2026 shape is rejected by the real period-contract evaluator.',
  }
}
...
export async function getKpiHealthSnapshot(options = {}) {
  ...
  const periodContract = buildKpiHealthPeriodContract({
    now,
    configuredYear: options.periodYear ?? options.configuredYear,
  })
  const expectedKpiRpcs = getExpectedKpiRpcs({ periodContract })
  ...
  const rpcs = []
  for (const definition of expectedKpiRpcs) {
    try {
...
export async function getSafeKpiHealthSnapshot() {
  try {
    ...
    const periodContract = (() => {
      try {
        return buildKpiHealthPeriodContract()
      } catch {
        return null
      }
    })()
...
function isUsableKpiHealthSnapshot(snapshot, nowMs = Date.now()) {
  ...
  const currentPeriod = buildKpiHealthPeriodContract({ now: new Date(nowMs) })
  if (Number(snapshot.periodContract?.year) !== currentPeriod.year) return false
  if (snapshot.periodContract?.periodStart !== currentPeriod.periodStart) return false
  if (snapshot.periodContract?.periodEnd !== currentPeriod.periodEnd) return false
