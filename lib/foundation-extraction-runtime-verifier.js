...
export const VERIFIER_EXTRACTION_RUNTIME_ORCHESTRATION_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-extraction-runtime-orchestration-split-closeout.md'
...
export function evaluateFoundationExtractionRuntimeVerifier(input = {}) {
  ...
  const processFoundationShipSource = input.processFoundationShipSource || ''
  ...
  const karpathyKbVideoPackCard = (input.foundationHub?.backlogItems || []).find(item => item.id === EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CARD_ID) || null
  const karpathyKbVideoPackClosed = karpathyKbVideoPackCard?.lane === 'done'
  const karpathyKbVideoPackCloseout = (input.foundationBuildCloseouts || []).find(item => item.key === EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CLOSEOUT_KEY) || null
  ...
  const karpathyKbVideoPack = buildKarpathyKbVideoPackSnapshot({
    extractionControlSnapshot: input.extractionControlSnapshot || {},
  })
  const karpathyKbVideoPackDogfood = buildKarpathyKbVideoPackDogfoodProof()
  ...
  addCheck(
    ...
    includesAll(llmRuntimeOwnershipSource, ['markStaleFoundationJobRuns', 'Marked failed by stale active-run reaper', 'markStaleLlmCalls', 'Marked failed by stale LLM call reaper', 'markStaleSourceCrawlTargetRuns', 'Marked failed by stale source-crawl run reaper']) &&
      includesAll(foundationWorkerSource, ['markStaleFoundationJobRuns', 'markStaleLlmCalls', 'markStaleSourceCrawlTargetRuns', 'job ' + '${job.key}' + ' failed before completion', 'Foundation worker pass failed', 'readFoundationWorkerShipPause', 'ship gate scheduled-job pause active']) &&
      includesAll(processFoundationShipSource, ['writeFoundationWorkerShipPause', 'clearFoundationWorkerShipPause', 'skipWorkerScheduledPause']),
    'Foundation worker catches job failures and reaps stale active runs/calls',
    'worker pass catches per-job failures, reaps stale active work, and ship gate restarts pause scheduled job selection before due jobs can run',
  )
  addCheck(
    checks,
  ...
  addCheck(
    ...
    includesAll(extractionTargetSource, [
      ...
      includesAll(extractionControlSeedSource, [
        ...
        "retrySkippedReasonPrefixes: ['pdf_too_large_for_v1', 'sheet_text_extraction_not_in_v1', 'office_file_conversion_not_in_v1']",
        'maxSheetRows: 200',
      ]) &&
  ...
  const extractionRuntimeReadinessApiOk =
    input.extractionRuntimeReadinessApi?.status === 'healthy' ||
    input.extractionRuntimeReadinessApi?.ok === true
  const extractionRuntimeReadinessRouteOk =
    !input.extractionRuntimeReadinessApi?.route ||
  ...
  const extractionRuntimeReadinessWiringOk =
    ...
    extractionRuntimeReadinessApiOk &&
    extractionRuntimeReadinessRouteOk &&
    ...
    (input.runtimeReadRoutesSource || input.foundationRuntimeReadRoutesSource)?.includes("app.get('/api/foundation/extraction-runtime-readiness'")
  addCheck(
    checks,
  ...
  const karpathyKbVideoPackCardOk = karpathyKbVideoPackCard &&
    ['executing', 'done'].includes(karpathyKbVideoPackCard.lane) &&
    (!karpathyKbVideoPackClosed || (
      String(karpathyKbVideoPackCard.statusNote || '').includes(EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CLOSEOUT_KEY) &&
      karpathyKbVideoPackCloseout?.operatorCloseout === true &&
      (karpathyKbVideoPackCloseout.backlogIds || []).includes(EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CARD_ID)
    ))
  const karpathyKbVideoPackWiringOk =
    input.packageJson?.scripts?.['process:extractor-queue-karpathy-kb-video-pack-check'] === `node --env-file-if-exists=.env ${EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_SCRIPT_PATH}` &&
    input.karpathyKbVideoPackSource?.includes('buildKarpathyKbVideoPackTarget') &&
    input.karpathyKbVideoPackSource?.includes('pending_approval') &&
    input.karpathyKbVideoPackCheckSource?.includes('dogfood rejects unsafe Karpathy packet variants') &&
    input.karpathyKbVideoPackPlanSource?.includes('No live extraction') &&
    input.karpathyKbVideoPackCloseoutSource?.includes(EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CLOSEOUT_KEY) &&
    input.buildIntelWatchlistSource?.includes('dream-labs-ai') &&
    input.verifierCoverageSource?.includes(EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CARD_ID)
  addCheck(
    checks,
    karpathyKbVideoPack.ok === true &&
      karpathyKbVideoPackDogfood.ok === true &&
      karpathyKbVideoPackCardOk &&
      karpathyKbVideoPackWiringOk,
    'EXTRACTOR-QUEUE-KARPATHY-KB-VIDEO-PACK-001 seeds Karpathy KB source packet without live extraction',
    karpathyKbVideoPackCard
      ? `lane=${karpathyKbVideoPackCard.lane} dogfood=${karpathyKbVideoPackDogfood.ok ? 'pass' : 'blocked'} packet=${karpathyKbVideoPack.ok ? 'pass' : 'blocked'} wiring=${karpathyKbVideoPackWiringOk ? 'pass' : 'blocked'} card=${karpathyKbVideoPackCardOk ? 'pass' : 'blocked'} status=${karpathyKbVideoPack.summary.status}/${karpathyKbVideoPack.summary.runtimeMode}`
      : `missing ${EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CARD_ID}; expected ${EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_PLAN_PATH}, ${EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_APPROVAL_PATH}, ${EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CLOSEOUT_PATH}`,
  )
...
function buildHealthyFixture() {
  return {
    foundationDbSource: [
      ...
      'listDriveContentExtractionQueue retrySkippedReasonPrefixes drive_document drive_spreadsheet drive_pdf drive_text application/vnd.google-apps.spreadsheet application/vnd.openxmlformats-officedocument.wordprocessingml.document text/markdown parentPathIncludes fileIds',
    ...
    foundationWorkerSource: 'markStaleFoundationJobRuns markStaleLlmCalls markStaleSourceCrawlTargetRuns job ${job.key} failed before completion Foundation worker pass failed readFoundationWorkerShipPause ship gate scheduled-job pause active',
    processFoundationShipSource: 'writeFoundationWorkerShipPause clearFoundationWorkerShipPause skipWorkerScheduledPause',
    ...
    extractionTargetSource: [
      ...
      "target.targetKey === 'drive-content-extract-backfill'",
      '--maxPdfBytes= --maxOfficeBytes= --maxSheets= --maxSheetRows= --retrySkippedReasonPrefixes=',
      "target.targetKey === 'email-attachments-backfill'",
    ...
    driveContentExtractionSource: 'GOOGLE_SHEET_MIME DOCX_MIME extractGoogleSheetText drive_google_sheet_values_v1 drive_docx_unzip_document_xml_v1 extractPdfTextWithOcr empty_text_after_ocr_needs_vision_handwriting_extraction drive_pdf_tesseract_ocr_v1',
    ...
    foundationHub: {
      backlogItems: [{
        ...
        lane: 'done',
        statusNote: EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CLOSEOUT_KEY,
      }],
    },
    foundationBuildCloseouts: [{
      ...
      key: EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CLOSEOUT_KEY,
      operatorCloseout: true,
      backlogIds: [EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CARD_ID],
    }],
    packageJson: {
      scripts: {
        'process:extraction-runtime-readiness-check': `node --env-file-if-exists=.env ${EXTRACTION_RUNTIME_READINESS_SCRIPT_PATH}`,
        'process:extractor-queue-karpathy-kb-video-pack-check': `node --env-file-if-exists=.env ${EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_SCRIPT_PATH}`,
      },
    },
    ...
    karpathyKbVideoPackSource: 'buildKarpathyKbVideoPackTarget pending_approval',
    karpathyKbVideoPackCheckSource: 'dogfood rejects unsafe Karpathy packet variants',
    karpathyKbVideoPackPlanSource: 'No live extraction',
    karpathyKbVideoPackCloseoutSource: EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CLOSEOUT_KEY,
    buildIntelWatchlistSource: 'dream-labs-ai Dream Labs AI',
    verifierCoverageSource: EXTRACTOR_QUEUE_KARPATHY_KB_VIDEO_PACK_CARD_ID,
  }
}
