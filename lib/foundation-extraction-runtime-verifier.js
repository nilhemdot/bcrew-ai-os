...
function buildHealthyFixture() {
  return {
    ...
    currentPlan: `Corpus mission lane daily quota missions files the outputs with provenance source_crawl_target_runs crawlRunId ${EXTRACT_RETIRE_CLOSEOUT_KEY}`,
    currentState: `daily quota missions file outputs with provenance source_crawl_target_runs crawlRunId ${EXTRACT_RETIRE_CLOSEOUT_KEY}`,
    ...
    packageSource: '"drive:inventory-links" "process:crawl-run-ledger-check": "node --env-file-if-exists=.env scripts/process-crawl-run-ledger-check.mjs" "process:extract-retire-check": "node --env-file-if-exists=.env scripts/process-extract-retire-check.mjs"',
    crawlRunLedgerScriptSource: 'buildCrawlRunLedgerDogfoodProof focused proof script is read-only',
    extractionRuntimeVerifierSource: 'EXTRACT-RETIRE-001 evaluateExtractRetireSources',
...
export function buildFoundationExtractionRuntimeVerifierDogfoodProof() {
  ...
  return {
    ...
    healthy,
    rejected,
    dogfoodInvariant: 'Healthy extraction/runtime fixture passes; missing worker reapers, corpus quotas, Drive extraction support, LLM provenance, or extract-retire coverage fail closed.',
  }
}
