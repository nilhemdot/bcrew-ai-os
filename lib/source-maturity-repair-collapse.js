...
export const SOURCE_MATURITY_REPAIR_COLLAPSE_NOT_NEXT_BOUNDARIES = [
  'No source-maturity repair/check file deletion, archive, or move in V1.',
...
export function evaluateSourceMaturityRepairCollapse({
  ...
  addCheck(
    ...
    'collapse is implemented as one reusable checker and one shared module',
    list(changedFiles).join(', '),
  )
  addCheck(
    checks,
    SOURCE_MATURITY_REPAIR_COLLAPSE_NOT_NEXT_BOUNDARIES.some(boundary => boundary.includes('No source-maturity repair/check file deletion')) &&
    ...
    'not-next boundaries protect no-delete, no-live-write, and per-hub checkpoint rules',
...
export function renderSourceMaturityRepairCollapseCloseout(snapshot = {}) {
  ...
  lines.push('- Classified the existing source-maturity repair/check surface by family instead of adding another per-card checker pattern.')
  ...
  lines.push('- Kept existing repair wrappers in place; V1 does not delete, archive, or move old files.')
  lines.push('')
  lines.push('## Proof')
  lines.push('')
  ...
  lines.push('')
  lines.push('- This is V1 manifest/control collapse. It does not yet remove the existing source-maturity wrapper/check files.')
  lines.push('- Wrapper migration/archive needs a follow-up card that repoints gates first and proves named check paths do not break.')
