...
export const VERIFIER_SPRINT_GATE_PROGRESSION_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-sprint-gate-progression-split-closeout.md'
...
export function evaluateFoundationVerifierSprintGateProgression(input = {}) {
  ...
  const liveCurrentSprintReviewComplete = activeSprintCompleteReview === true &&
    foundationCurrentSprintStatus.status === 'healthy' &&
    foundationHub.currentSprint?.status === 'healthy'
  const sprintProgressionTextOk = (planPatterns = [], statePatterns = []) =>
    liveCurrentSprintReviewComplete ||
    (includesAll(currentPlan, planPatterns) && includesAll(currentState, statePatterns))
  ...
  checks.push(...currentSprintVerifier.checks)
    ...
    verifyGateTiering?.lane === 'done' &&
      ...
      sprintProgressionTextOk(
        [VERIFY_GATE_TIERING_CARD_ID, 'proportional verification'],
        [VERIFY_GATE_TIERING_CARD_ID, 'proportional verification'],
      ),
    ...
    planCriticReplacement?.lane === 'done' &&
      ...
      sprintProgressionTextOk(
        [PLAN_CRITIC_REPLACEMENT_CLOSEOUT_KEY, 'gate decision tree', SECURITY_BEHAVIOR_PROOF_CARD_ID],
        [PLAN_CRITIC_REPLACEMENT_CLOSEOUT_KEY, 'SECURITY-BEHAVIOR-PROOF-001'],
      ),
    ...
    securityBehaviorProof?.lane === 'done' &&
      ...
      sprintProgressionTextOk(
        [SECURITY_BEHAVIOR_PROOF_CLOSEOUT_KEY, 'route-boundary', VERIFIER_BEHAVIOR_SWEEP_CARD_ID],
        [SECURITY_BEHAVIOR_PROOF_CLOSEOUT_KEY, 'subject-person'],
      ),
    ...
    verifierBehaviorSweep?.lane === 'done' &&
      ...
      sprintProgressionTextOk(
        [VERIFIER_BEHAVIOR_SWEEP_CLOSEOUT_KEY, 'top-P0 behavior registry', STRATEGY_HUB_MEETING_READY_CARD_ID],
        [VERIFIER_BEHAVIOR_SWEEP_CLOSEOUT_KEY, 'behavior registry'],
      ) &&
