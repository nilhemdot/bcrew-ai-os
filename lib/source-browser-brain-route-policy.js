...
export function selectSourceBrowserBrainRoute(input = {}) {
  ...
  if (pageHealthBlocked(pageHealth) || bool(input.browserChallengeDetected || input.browser_challenge_detected)) {
    return route('local_self_recovery_then_harlan', {
      ...
      fallbackChain: ['clean_isolated_local_profile', 'source_session_probe', 'harlan_auth_needed_dry_run'],
      nextAction: 'Retry once with a clean isolated local profile, then prepare Harlan/auth if needed; do not use Steve normal Chrome or hosted Browserbase.',
    })
  }

  if (hostedRequested) {
    return blocked('blocked_hosted_browser_not_approved_for_sprint', 'browserbase_parked_outside_human_web_agent_v1', {
      sourceFamily,
      action,
      selectedBrain: 'none',
      selectedRunner: 'none',
      costPolicy,
      nextAction: 'Do not run Browserbase from this sprint. Use deterministic readers, local isolated browser hands, connectors, Source Session Broker, or Harlan escalation.',
    })
  }
  ...
  if (wantsModel) {
    ...
    return route('stagehand_local_api_brain', {
      sourceFamily,
      action,
      ...
      modelRoute,
      costPolicy,
      nextAction: 'Use local Stagehand only for proof-sized observe/extract/agent work with explicit API model caps.',
    })
  }
...
export function buildSourceBrowserBrainRoutePolicySnapshot() {
  return {
    ...
    guardrails: {
      ...
      hostedBrowserSpendApproved: false,
    ...
    routeMatrix: [
      ...
      { sourceFamily: 'browser_challenge_or_captcha', defaultRoute: 'local_self_recovery_then_harlan; hosted Browserbase is parked outside this sprint', runner: 'source:local-browser-hands/source:agentic-browser' },
    ],
...
export function buildSourceBrowserBrainRoutePolicyDogfood() {
  const cases = [
    ...
    {
      name: 'hosted_browser_blocks_even_when_requested',
      ...
      expect: decision => decision.ok === false && decision.status === 'blocked_hosted_browser_not_approved_for_sprint',
    },
    {
      name: 'stale_browserbase_bakeoff_flag_still_blocks',
      ...
      expect: decision => decision.ok === false && decision.status === 'blocked_hosted_browser_not_approved_for_sprint' && decision.browserbaseUsed === false,
    },
    {
...
export function evaluateSourceBrowserBrainRoutePolicy(snapshot = buildSourceBrowserBrainRoutePolicySnapshot()) {
  ...
  for (const key of [
    'browserbaseParkedOutsideSprint',
  ...
  for (const key of ['browserbaseDefault', 'hostedBrowserSpendApproved', 'normalChromeProfileAllowed', 'externalWritesAllowed', 'harlanLiveSendFromProof', 'rawSecretsVisible']) {
  ...
  for (const routeId of ['deterministic_public_source_worker', 'source_session_broker_harlan_auth', 'stagehand_local_api_brain', 'local_hands_source_specific_runner']) {
