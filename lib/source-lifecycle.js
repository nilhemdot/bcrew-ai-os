...
export function buildSourceLifecycleStatus({
  ...
  const laneSpecs = [
    ...
    },
    {
      ...
      targetKeys: ['video-link-inventory', 'video-content-extract-backfill', 'zoom-audio-recovery-backfill'],
    },
    {
      ...
      targetKeys: ['skool-corpus-backfill'],
    },
  ]
  ...
  return {
    ...
    summary: {
      ...
      allSourceContractsCovered: sourceLifecycles.length >= SOURCE_LIFECYCLE_MIN_SOURCE_CONTRACTS,
...
export async function buildSourceLifecycleExpansionCheck({
  ...
  addFinding(findings, serverSource.includes("app.get('/api/foundation/source-lifecycle'"), 'source lifecycle API route exists', SOURCE_LIFECYCLE_API_PATH)
  ...
  if (sourceLifecycle) {
    const summary = sourceLifecycle.summary || {}
    ...
    addFinding(findings, summary.sourceContractCount >= SOURCE_LIFECYCLE_MIN_SOURCE_CONTRACTS, 'all 35 source contracts covered', String(summary.sourceContractCount || 0))
    addFinding(findings, summary.extractionTargetCount === SOURCE_LIFECYCLE_REQUIRED_TARGET_COUNT, 'all 12 extraction targets covered', String(summary.extractionTargetCount || 0))
  ...
  if (sourceOfTruth) {
    addFinding(findings, Array.isArray(sourceOfTruth.sources) && sourceOfTruth.sources.length >= SOURCE_LIFECYCLE_MIN_SOURCE_CONTRACTS, '/api/source-of-truth remains compatible', String(sourceOfTruth.sources?.length || 0))
  ...
  if (foundationHub) {
    ...
    addFinding(findings, foundationHub.extractionControl?.summary?.targetCount === SOURCE_LIFECYCLE_REQUIRED_TARGET_COUNT, 'foundation hub still reports 12 extraction targets', String(foundationHub.extractionControl?.summary?.targetCount || 0))
  }
