...
export async function buildSalesListingInventory(options = {}) {
  ...
  const actionPlanTracked = activeListings.filter(item => item.actionPlanStatus)
  const groups = groupStaleListings(staleListings)
  ...
  return {
    ...
    summary: {
      ...
      actionPlanTracked: actionPlanTracked.length,
      actionPlanNotTracked: Math.max(0, staleListings.length - staleListings.filter(item => item.actionPlanStatus).length),
    },
    rule: {
