...
function caseIsTakenOn(row) {
  return Boolean(
    ...
    statusRank(row.caseStatus) > statusRank('identified') ||
    row.actionPlanState === 'yes' ||
    ['adjusted', 'conditional', 'firm', 'closed', 'no_action'].includes(row.outcomeStatus || '')
  )
}
...
function caseHasGamePlan(row) {
  return row.actionPlanState === 'yes' ||
    ['action_plan_created', 'action_plan_implemented', 'adjusted', 'closed'].includes(row.caseStatus || '')
}
...
function caseIsSold(row) {
  ...
  return row.outcomeStatus === 'closed' || /\b(closed|sold)\b/.test(statusText)
}
...
function caseIsStillOpen(row) {
  return !caseIsAdjusted(row) && !caseIsMoved(row) && !caseIsSold(row) && !caseIsNoActionOrBlocked(row)
}
