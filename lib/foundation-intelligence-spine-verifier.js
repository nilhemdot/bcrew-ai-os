...
export const VERIFIER_INTELLIGENCE_SPINE_ORCHESTRATION_SPLIT_HANDOFF_PATH = 'docs/handoffs/2026-05-17-verifier-intelligence-spine-orchestration-split-closeout.md'
...
export function evaluateFoundationIntelligenceSpineVerifier(input = {}) {
  ...
  addCheck(
    ...
    includesAll(foundationDbWithBacklogSeedSource, [
      ...
      Number(synthesisEngineSnapshot.activeItems || 0) >= 1 &&
    ...
    `${synthesisEngineSnapshot.activeItems || 0} active items / latestProofQuality=${JSON.stringify(synthesisEngineSnapshot.latestProofQuality || {})} / latestProof=${synthesisEngineSnapshot.latestProofRun?.runId || 'missing'}`,
  )
  addCheck(
    checks,
    includesAll(foundationDbSource, [
...
function healthyInput() {
  ...
  return {
    ...
    intelligenceAtomSpineSnapshot: { totalReports: 1, totalAtoms: 1, totalHits: 1, atomsWithReportArtifact: 1, atomsWithScoperQueryFields: 1, latestScoperQueryProof: [{ reportArtifactId: 'r1', metricRefs: ['INTEL-ATOM-001'], minTier: 1 }] },
