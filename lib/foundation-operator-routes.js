...
import {
  createFoundationBuildLogApiCache,
  normalizeFoundationBuildLogLimit,
  readFoundationBuildLogApiPayload,
...
export function registerFoundationOperatorRoutes(app, deps = {}) {
  ...
  app.get('/api/foundation/build-log', requireAdminToken, async (req, res) => {
    try {
      ...
      const payload = await readFoundationBuildLogApiPayload({
        limit,
        cache: buildLogApiCache,
        getRecentBuildLog,
        getRecentChangeEvents,
        getFoundationBuildCloseouts,
        summarizeFoundationBuildLog,
