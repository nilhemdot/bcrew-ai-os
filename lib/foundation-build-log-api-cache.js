import {
  getPinnedFoundationBuildLogCloseouts,
...
export function evaluateFoundationBuildLogApiCacheAndSlimSource({
  ...
  const checks = [
    ...
    {
      ok: helper.includes('getPinnedFoundationBuildLogCloseouts') &&
        helper.includes('closeouts: pinnedCloseouts'),
      check: 'cache module preserves pinned historical closeouts outside the rolling git window',
    },
    {
...
export function buildFoundationBuildLogApiPayload({
  ...
  const pinnedCloseouts = getPinnedFoundationBuildLogCloseouts({
    builds: keyedBuilds,
    closeouts,
  })
  const visibleBuilds = addFoundationBuildLogKeys([
    ...keyedBuilds,
    ...pinnedCloseouts,
  ])
  const fullGroups = typeof groupFoundationBuildLog === 'function'
    ? groupFoundationBuildLog(visibleBuilds)
    : []
  return {
    generatedAt,
...
export async function readFoundationBuildLogApiPayload({
  ...
  const [builds, recentChanges, closeouts] = await Promise.all([
    getRecentBuildLog(boundedLimit),
    getRecentChangeEvents(5),
    typeof getFoundationBuildCloseouts === 'function'
      ? Promise.resolve(getFoundationBuildCloseouts())
      : Promise.resolve([]),
  ])
...
export async function buildFoundationBuildLogApiCacheDogfoodProof() {
    ...
    getFoundationBuildCloseouts: () => [
      {
        ...
        mentionedBacklogIds: [],
        systemArea: 'Synthetic historical closeout',
        status: 'shipped',
        acceptanceState: 'Verified',
        whatChanged: '31 Foundation nav stale historical closeout.',
        whatItDoes: 'Proves pinned historical closeouts survive outside the rolling git window.',
        whyItMatters: 'Recent Builds verifier-required closeouts cannot age out after enough commits.',
        whereItLives: ['Synthetic closeout registry'],
        proofCommands: ['npm run foundation:verify'],
        proofStatus: 'Synthetic pinned closeout proof.',
  ...
  const pinnedCloseout = second.closeouts?.find(closeout => closeout.closeoutKey === 'foundation-surface-sweep-v1')
  const pinnedBuild = second.builds?.find(build => build.closeoutKey === 'foundation-surface-sweep-v1')
  return {
    ok: Boolean(first.cache?.hit === false &&
      ...
      pinnedCloseout?.backlogIds?.includes('FOUNDATION-SWEEP-001') &&
      pinnedBuild?.backlogIds?.includes('FOUNDATION-SWEEP-001') &&
      pinnedBuild?.source === 'closeout-registry-pinned' &&
    ...
    pinnedCloseout,
    pinnedBuild,
    dogfoodInvariant: 'Repeated reads within TTL reuse cached build-log data, groups contain refs instead of duplicated build objects, and pinned historical closeouts stay visible in the build stream outside the rolling git window.',
  }
}
...
export function buildFoundationBuildLogApiCacheAndSlimDogfoodProof() {
  const healthy = evaluateFoundationBuildLogApiCacheAndSlimSource({
    ...
    `,
    cacheModuleSource: `
      import { getPinnedFoundationBuildLogCloseouts } from './foundation-build-log.js'
      ...
      function buildFoundationBuildLogApiPayload() { const pinnedCloseouts = getPinnedFoundationBuildLogCloseouts(); return { closeouts: pinnedCloseouts } }
