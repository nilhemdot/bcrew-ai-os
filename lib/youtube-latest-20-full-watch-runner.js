...
function withApprovalSource(video = {}) {
  return {
    ...
    approvalSource: video.approvalSource ||
      `${YOUTUBE_LATEST_20_INTEL_RUN_CARD_ID} selected this public/no-auth YouTube video from ${YOUTUBE_CREATOR_DAILY_WATCH_SOURCE_ID}.`,
  }
}
...
export function buildYoutubeLatest20FullWatchSnapshot({
  ...
  addCheck(checks, mergedResults.every(result => result.video?.creatorId), 'all videos have creator identity for source grading', mergedResults.map(result => `${result.video?.videoId}:${result.video?.creatorId}`).join(', '))
  ...
  addCheck(checks, mergedResults.every(result => (
    asNumber(result.resourceLinkSnapshot?.counts?.remainingPublic, 0) === 0 ||
    list(result.resourceLinkSnapshot?.scoperPacket?.resourceLinkDispositions).length >= 1
  )), 'safe public resource links are resolved, blocked, or queued for source-worker follow-up before scoping', mergedResults.map(result => `${result.video?.videoId}:${result.resourceLinkSnapshot?.counts?.remainingPublic || 0}`).join(', '))
  ...
  return {
    ...
    summary: {
      ...
      totalBuildCandidates: topBuildCandidates.length,
	      ...
	      remainingPublicResourceLinkCount: mergedResults.reduce((sum, result) => sum + asNumber(result.resourceLinkSnapshot?.counts?.remainingPublic, 0), 0),
...
export function buildYoutubeLatest20FullWatchDogfoodProof() {
  ...
  const checks = [
    ...
    {
      ok: unresolved.ok === true &&
        unresolved.summary?.remainingPublicResourceLinkCount === 1,
      check: 'snapshot parks unresolved safe public resource links for source-worker follow-up instead of blocking full-watch persistence',
      detail: `${unresolved.status}:${unresolved.summary?.remainingPublicResourceLinkCount}`,
    },
    {
...
export function renderYoutubeLatest20FullWatchReport(snapshot = {}) {
  ...
  lines.push('')
  lines.push(`The system watched ${snapshot.summary?.videoCount || 0} public creator videos through the Gemini API video/audio/visual route, read YouTube page evidence, resolved or queued safe public resource links, blocked unsafe links, and kept every recommendation proposal-only.`)
  lines.push('')
  lines.push('## Videos')
  lines.push('')
