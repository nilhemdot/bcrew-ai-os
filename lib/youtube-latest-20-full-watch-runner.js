...
export const YOUTUBE_LATEST_20_FULL_WATCH_MODEL = process.env.YOUTUBE_LATEST_20_FULL_WATCH_MODEL || 'gemini-3.5-flash'
...
function resourceLinksForResolver(pageEvidence = {}, video = {}) {
  return list(pageEvidence.resourceLinks).map(link => ({
    ...link,
    url: link.normalizedUrl || link.url,
    sourceVideoId: video.videoId,
    sourceUrl: video.url,
    sourceReportArtifactId: YOUTUBE_LATEST_20_FULL_WATCH_REPORT_ARTIFACT_ID,
...
export async function runYoutubeLatest20FullWatchExtraction({
  ...
  for (const [index, video] of approvedVideos.entries()) {
    ...
    const resourceLinkSnapshot = await buildYoutubeResourceLinkResolverSnapshot({
      rawLinks: resourceLinksForResolver(pageEvidence, video),
    ...
    const eyes = await runGeminiEyesForVideo({
      ...
      metadata: {
        ...
        batchReportArtifactId: YOUTUBE_LATEST_20_FULL_WATCH_REPORT_ARTIFACT_ID,
...
export function buildYoutubeLatest20FullWatchWriteSet(snapshot = {}) {
  ...
  const atomInputs = candidates.slice(0, 18).map((candidate, index) => {
    const atomId = `atom:${YOUTUBE_LATEST_20_FULL_WATCH_RUNNER_CARD_ID.toLowerCase()}:api-batch:${slug(candidate.sourceVideoId)}:${slug(candidate.title)}`
    return {
      atomId,
      title: candidate.title,
...
export function verifyYoutubeLatest20FullWatchPersistedProof({ snapshot = {}, report = null, atoms = [], hits = [] } = {}) {
  ...
  addCheck(checks, report?.reportArtifactId === YOUTUBE_LATEST_20_FULL_WATCH_REPORT_ARTIFACT_ID, 'latest-20 full-watch report artifact reads back', report?.reportArtifactId || 'missing')
...
export function renderYoutubeLatest20FullWatchReport(snapshot = {}) {
  ...
  lines.push(`Report artifact: \`${YOUTUBE_LATEST_20_FULL_WATCH_REPORT_ARTIFACT_ID}\``)
