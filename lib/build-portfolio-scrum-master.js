  ...
  const candidates = [
    {
      ...
      portfolioGroupKey: 'extractor-visual-resource-links',
    ...
    {
      id: 'SCOPED-VIDEO-RESOURCE-LINKS-002',
      portfolioGroupKey: 'extractor-visual-resource-links',
      title: 'Capture approved resource links and visual demo evidence during extractor runs',
      director: { missionScore: 76 },
      summary: 'The same extractor lane should connect approved resource-link evidence and visible demo evidence when allowed.',
      scope: {
        what: 'Extend the approved public video source package so approved resource-link evidence and demo evidence are captured as one extraction packet when source rules allow it.',
        why: 'Resource links often contain the actual repo, docs, code, or offer context needed to understand what the creator is demonstrating.',
        details: 'Keep follow-up exact-source-packet only. Store source URL, packet ID, link disposition, visual timestamp references, and approval boundaries.',
        acceptanceCriteria: ['approved resource links are processed only through source packets', 'resource evidence keeps source URL and packet metadata', 'resource evidence can merge with visual demo evidence'],
        definitionOfDone: ['resource links are source-packet linked', 'visual demo evidence is deduped by timestamp'],
        tests: ['fixture proves approved resource-link and visual evidence merge into the same extractor opportunity'],
        risks: ['resource-link follow-up can drift into broad crawling if source packets are not explicit'],
        notNext: ['do not log into Skool or private communities from this card', 'do not treat operator-excluded comments as active evidence work'],
      ...
      sourceRefs: ['youtube-video:resource-links', 'mark-video:visual-extractor'],
    },
    {
  ...
  const checks = [
    {
      ok: extractorGroup?.decision === BUILD_PORTFOLIO_DECISIONS.mergedEnhanced &&
        extractorGroup.candidateIds.includes('SCOPED-VIDEO-RESOURCE-LINKS-002'),
