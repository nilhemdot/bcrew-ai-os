...
const foundationJobDefinitions = [
  ...
  },
  {
    ...
    command: 'npm',
    args: ['run', 'process:system-health-nightly-audit-check', '--', '--json', '--write-report', '--scheduled-report'],
    mutationPosture: 'report_only',
  ...
  {
    key: 'generated-artifact-closeout',
    title: 'Generated Artifact Closeout',
    jobType: 'repo_closeout',
    lane: 'health',
    priority: 'P0',
    cadence: 'daily 08:15 America/Toronto after morning report/source artifact writers',
    enabled: true,
    runtimeMode: 'scheduled',
    scheduleEveryMinutes: 1440,
    scheduleLocalTime: '08:15',
    scheduleTimezone: 'America/Toronto',
    maxRuntimeSeconds: 300,
    budget: 'no_llm_allowlisted_git_push',
    command: 'node',
    args: ['--env-file-if-exists=.env', 'scripts/generated-artifact-closeout.mjs', '--json', '--apply'],
    mutationPosture: 'external_write',
    mutationPostureReason: 'Commits and pushes only allowlisted generated audit/system-health/source-note artifacts after proving main is synced and no other dirty files exist.',
    sourceIds: [],
    description: 'Closes the gap where scheduled report-only jobs write durable repo artifacts but leave the morning worktree dirty.',
    nextAction: 'If blocked, inspect the dirty-file list; non-allowlisted work must be handled by the builder, not this job.',
  },
  {
    ...
    command: 'npm',
    args: ['run', 'process:foundation-lessons-learned-loop-check', '--', '--json', '--scheduled-report'],
    mutationPosture: 'read_only',
  ...
  },
  {
    ...
    lane: 'extract',
    priority: 'P0',
    cadence: 'daily 07:00 America/Toronto live-bounded public YouTube watch',
    enabled: true,
    runtimeMode: 'scheduled',
    scheduleEveryMinutes: 1440,
    ...
    scheduleTimezone: 'America/Toronto',
    maxRuntimeSeconds: 3900,
    budget: 'gemini_api_live_bounded_public_youtube_max_5_run_150_day',
    command: 'npm',
    args: [
      'run',
      'process:youtube-god-mode-autonomous-watch-scheduler-check',
      '--',
      '--json',
      '--mode=live-bounded',
      '--approve-live-budget',
      '--run-live-batch',
      '--batch-size=9',
      '--max-estimated-usd-per-run=5',
      '--max-estimated-usd-per-day=150',
      '--command-timeout-ms=3600000',
    ],
    mutationPosture: 'operational_write',
    mutationPostureReason: 'Steve approved the public YouTube live-bounded watcher operating posture on 2026-05-26. The job spends Gemini only inside explicit run/day caps, processes approved public/no-auth YouTube videos through the guarded full-watch runner, persists governed intelligence artifacts, and refreshes Director/source grades/Dev Hub. It must not use auth, comments, paid/private sources, purchases, downloads, forms, credential mutation, external writes, or automatic backlog promotion.',
    sourceIds: ['SRC-CREATOR-WATCHLIST-001', 'SRC-YOUTUBE-INTEL-001'],
    description: 'Runs the approved public/no-auth YouTube God Mode full-watch batch daily after metadata refresh, then refreshes Director/source ranking/Dev Hub without creating backlog cards or touching private/auth/paid surfaces.',
    nextAction: 'Watch the first scheduled live-bounded runs for provider failure and quality; tune daily cap/batch size from the morning report instead of returning to manual babysitting.',
  },
  {
    key: 'source-god-mode-youtube-handoff',
    title: 'YouTube Source Handoff Runtime',
    jobType: 'source_handoff',
    lane: 'extract',
    priority: 'P0',
    cadence: 'daily 08:15 America/Toronto after public YouTube watch',
    enabled: true,
    runtimeMode: 'scheduled',
    scheduleEveryMinutes: 1440,
    scheduleLocalTime: '08:15',
    scheduleTimezone: 'America/Toronto',
    maxRuntimeSeconds: 3900,
    budget: 'no_llm_bounded_public_source_browser_reads',
    command: 'npm',
    args: ['run', 'source:youtube-handoff', '--', '--apply', '--json', '--max-runs=10'],
    mutationPosture: 'operational_write',
    mutationPostureReason: 'Runs bounded public/free source-browser and repo reads from already-watched YouTube evidence, then writes governed source_crawl readback/report artifacts. It does not submit forms, buy, download, post/comment/message, mutate credentials, use a normal Chrome profile, call LLM providers, or auto-promote backlog/Scoper work; auth/session rows remain parked unless the Source Session Broker marks them ready.',
    sourceIds: ['SRC-YOUTUBE-INTEL-001'],
    description: 'Drains safe public/free follow-up links found by YouTube extraction into source-crawl evidence so newsletters, repos, public resources, and session-gated community rows do not stay trapped in YouTube.',
    nextAction: 'Review source-session readiness for Skool/newsletter/MyICOR rows while this job keeps safe public reads drained automatically.',
  },
  {
    ...
    lane: 'extract',
    priority: 'P0',
    cadence: 'paused after clean zero-work retirement',
    enabled: true,
    runtimeMode: 'paused',
    scheduleEveryMinutes: null,
    maxRuntimeSeconds: 3900,
    ...
    pauseReason: 'The target retired itself complete/paused after clean zero-work. Keep this legacy subtitle lane manual until a new video-content queue is explicitly rebuilt.',
    nextAction: 'Do not schedule this retired legacy bite. Use the YouTube God Mode watcher and the long-course/deep-visual lanes for current public YouTube video extraction.',
  },
  {
