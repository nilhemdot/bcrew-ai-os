const foundationJobDefinitions = [
  ...
  },
  {
    ...
    lane: 'transactions',
    priority: 'P1',
    cadence: 'every 8 hours: marked re-reviews first, then one newest eligible Admin backlog deal',
    enabled: true,
    runtimeMode: 'scheduled',
    ...
    budget: 'connector',
    command: 'npm',
    args: ['run', 'deal-review:admin', '--', '--queued', '--backlog', '--limit=10', '--backlog-limit=1', '--backlog-since=2025-06-01', '--write'],
    sourceIds: ['SRC-OWNERS-001', 'SRC-FUB-001'],
    servesHubs: ['ops'],
    systemSummary: 'Inspects marked Admin deal re-reviews first, then one newest eligible never-inspected Owners deal per run from the June 2025 merger backlog using Date Firm (Executed) and a 10-day maturity gate. At every 8 hours, that is 3 paced backlog deals per day. Scheduled writeback only populates AI review status, action, and findings; it does not auto-fix source fields.',
    systemInputs: ['SRC-OWNERS-001 Admin deal rows', 'SRC-FUB-001 person records', 'Queued review action cells', 'Blank / not-reviewed AI status rows from June 2025 onward'],
    systemOutputs: ['Ops inbox issue cards', 'AI review status/action/findings in the Admin tab', 'Foundation job run status'],
    description: 'Runs explicit re-reviews plus a paced first-pass backlog inspection lane for closed/firm Admin deals.',
  ...
  },
  {
    ...
    title: 'Conditional Deal Review Inspection',
    jobType: 'review',
    lane: 'transactions',
    priority: 'P1',
    cadence: 'every 8 hours: marked re-reviews first, then one newest eligible conditional backlog row',
    enabled: true,
    runtimeMode: 'scheduled',
    scheduleEveryMinutes: 480,
    maxRuntimeSeconds: 600,
    budget: 'connector',
    command: 'npm',
    args: ['run', 'deal-review:conditional', '--', '--queued', '--backlog', '--limit=10', '--backlog-limit=1', '--backlog-since=2025-06-01', '--write'],
    sourceIds: ['SRC-OWNERS-001', 'SRC-FUB-001'],
    servesHubs: ['ops'],
    systemSummary: 'Inspects marked conditional/listing re-reviews first, then one newest eligible never-inspected conditional row when the row has enough date/identity signal. Scheduled writeback only populates AI review status, action, and findings; it does not auto-fix source fields.',
    systemInputs: ['Listings and Conditional Deals rows', 'SRC-FUB-001 person records', 'Queued conditional review action cells', 'Blank / not-reviewed AI status rows from June 2025 onward'],
    systemOutputs: ['Ops inbox issue cards', 'AI conditional review status/action/findings', 'Foundation job run status'],
    description: 'Runs explicit re-reviews plus a paced first-pass backlog inspection lane for conditional/listing rows.',
    nextAction: 'Keep source-field corrections human-owned until Ops approves an apply/fix lane.',
  },
  {
