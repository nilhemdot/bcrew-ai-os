...
export function buildScheduledMutationGuardDogfoodProof() {
  ...
  const ok = scheduledMutatingRuntime.scheduleStatus === 'blocked' &&
    ...
    realVerificationRunsRuntime.due === false
...
const foundationJobDefinitions = [
  ...
  },
  {
    ...
    mutationPostureReason: 'process:verification-runs-check closes backlog/current-sprint rows; scheduler must block it until it is split into read-only proof plus explicit apply lane.',
    sourceIds: [],
  ...
  },
  {
    ...
    command: 'npm',
    args: ['run', 'process:nightly-deep-audit-upgrade-check', '--', '--json', '--endpointTimeoutMs=8000'],
    mutationPosture: 'report_only',
    ...
    sourceIds: [],
    description: 'Runs the scheduled report-only hybrid audit Steve expected: deterministic backend/frontend scan plus changed/high-risk senior-engineer review packets or approved-route LLM review posture.',
    nextAction: 'Review the diff-only morning report; promote findings to backlog manually with Steve/Codex approval.',
  },
  {
    ...
    command: 'npm',
    args: ['run', 'process:foundation-operating-reliability-check', '--', '--json', '--no-api'],
    ...
    description: 'Builds the read-only connector uptime, runtime activation, and morning-health status used before hubs or ship gates discover source outages.',
...
export function getFoundationJobRuntime(job, latestRun, now = new Date()) {
  ...
  if (scheduleLocalTime) {
    const nextRun = latestValid
      ...
      : nextDailyLocalRun(now, scheduleLocalTime, scheduleTimezone, { inclusive: true })
    if (!nextRun) {
      return {
      runtimeMode: 'scheduled',
      due: false,
