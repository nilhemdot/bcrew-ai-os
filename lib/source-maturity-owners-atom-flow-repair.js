...
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_CARD_ID = 'SOURCE-MATURITY-OWNERS-ATOM-FLOW-REPAIR-001'
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_CLOSEOUT_KEY = 'source-maturity-owners-atom-flow-repair-v1'
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_PLAN_PATH = 'docs/process/source-maturity-owners-atom-flow-repair-001-plan.md'
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_APPROVAL_PATH = 'docs/process/approvals/SOURCE-MATURITY-OWNERS-ATOM-FLOW-REPAIR-001.json'
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-29-hot-doc-refresh/2026-05-18-source-maturity-owners-atom-flow-repair-closeout.md'
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_SCRIPT_PATH = 'scripts/process-source-maturity-owners-atom-flow-repair-check.mjs'
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_SPRINT_ID = 'source-maturity-owners-atom-flow-repair-2026-05-18'
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID = 'SRC-OWNERS-001'
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_FACT_ID = 'fact:566d0615ac1fcb345e2c6a93'

export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES = [
...
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_PROOF_COMMANDS = [
...
export const SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_CHANGED_FILES = [
...
function targetFact(facts = []) {
  return list(facts).find(fact => (fact.factId || fact.fact_id) === SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_FACT_ID) || null
}

export function selectOwnersAtomFlowRepairCandidate({
  beforeRow = {},
  sourceFacts = [],
  sourceId = SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
} = {}) {
  const failures = []
  ...
  const factId = fact?.factId || fact?.fact_id || SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_FACT_ID
...
export function buildOwnersAtomFromFact(candidate = {}, now = new Date().toISOString()) {
  const sourceId = candidate.sourceId || SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  ...
  const naturalKey = `source-maturity-owners-atom-flow-repair:${sourceId}:${factId}`
  ...
  const atom = {
    ...
    evidenceExcerpt,
    derivedClaim: claim,
    topicRefs: ['source-maturity', 'owners', 'team-goal-pace', 'source-backed-goal-truth'],
    ...
    acceptedAt: now,
    acceptedBy: 'codex-source-maturity-owners-atom-flow-repair',
    reviewNote: 'Promoted from an existing governed Owners Dashboard source fact only; no live extraction, provider call, Sheets read/write, or external write occurred.',
    tags: ['source-maturity', 'atom-flow-repair', 'owners', 'no-live-extraction'],
    metadata: {
...
export function evaluateOwnersAtomFlowRepair({
  beforeRow = {},
  afterRow = {},
  candidate = {},
  atomBuild = {},
  ...
  return {
    ...
    failures,
    summary: {
      sourceId: candidate.sourceId || beforeRow.sourceId || afterRow.sourceId || SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
...
export function buildSyntheticOwnersAtomFlowRepairProof() {
  const fact = {
    factId: SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_FACT_ID,
    sourceId: SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
    title: 'Team Goal: $2B: Pace',
    claim: 'Pace: Behind by $41.79M (-48.7%).',
    detail: 'Executed-date volume from the Owners Dashboard (Volume Credit filtered by Date Firm/Executed in 2026). Keep this separate from community-size or downline-count targets.',
    metadata: { status: 'behind', evidenceRefs: ['intelligence_synthesis_facts:fact:566d0615ac1fcb345e2c6a93'] },
    minTier: 1,
    sensitivity: 'neutral',
    status: 'active',
  }
  ...
  const candidate = selectOwnersAtomFlowRepairCandidate({ beforeRow, sourceFacts: [fact] })
  const atomBuild = buildOwnersAtomFromFact(candidate, '2026-05-18T06:15:00.000Z')
  const evaluation = evaluateOwnersAtomFlowRepair({
    beforeRow,
    afterRow,
    candidate,
    atomBuild,
  ...
  const missingFact = selectOwnersAtomFlowRepairCandidate({ beforeRow, sourceFacts: [] })
  const tierBlocked = selectOwnersAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, minTier: 4 }] })
  return {
    ok: candidate.ok &&
      atomBuild.ok &&
      evaluation.ok &&
...
export function renderOwnersAtomFlowRepairCloseout(snapshot = {}) {
  const summary = snapshot.summary || {}
  const lines = []
  lines.push('# Source Maturity Owners Dashboard Atom Flow Repair Closeout')
  lines.push('')
  lines.push(`Card: \`${SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_CARD_ID}\``)
  lines.push(`Closeout key: \`${SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\``)
  lines.push('')
  lines.push('## What Shipped')
  lines.push('')
  lines.push(`- Promoted one source-backed atom for \`${summary.sourceId || SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID}\` from an existing Owners Dashboard source fact.`)
  lines.push(`- Saved atom: \`${summary.atomId || 'pending atom'}\`.`)
  lines.push(`- Source fact evidence: \`${summary.sourceFactId || SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_TARGET_FACT_ID}\`.`)
  ...
  lines.push('')
  for (const boundary of SOURCE_MATURITY_OWNERS_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES) lines.push(`- ${boundary}`)
  lines.push('')
  lines.push('## Next')
  lines.push('')
