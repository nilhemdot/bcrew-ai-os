...
const REVIEWED_PHASE_G_PROGRESS_CLOSEOUTS = {
  ...
  'UI-MENU-LAYOUT-POLISH-001': 'ui-menu-layout-polish-v1',
  'RECENT-BUILDS-BILLION-DOLLAR-UI-001': 'recent-builds-billion-dollar-ui-v1',
...
function isReviewedPhaseGCardDone(card) {
  if (!card || card.lane !== 'done') return false
  const approvedCloseout = REVIEWED_PHASE_G_PROGRESS_CLOSEOUTS[card.id]
  return Boolean(approvedCloseout && String(card.statusNote || '').includes(approvedCloseout))
}

function buildCurrentPhaseGReadiness(artifact, backlogItems) {
  const baseline = artifact?.baseline?.phaseGReadiness || {}
  const cardById = new Map((backlogItems || []).map(card => [card.id, card]))
  const nextCard = FOUNDATION_REVIEW_SPRINT_PHASE_G_ORDER.find(cardId => !isReviewedPhaseGCardDone(cardById.get(cardId))) || null
  return {
    ...baseline,
    finalOrder: FOUNDATION_REVIEW_SPRINT_PHASE_G_ORDER,
    notStarted: FOUNDATION_REVIEW_SPRINT_PHASE_G_ORDER.every(cardId => !isReviewedPhaseGCardDone(cardById.get(cardId))),
    nextPlanCard: nextCard,
    completedCards: FOUNDATION_REVIEW_SPRINT_PHASE_G_ORDER.filter(cardId => isReviewedPhaseGCardDone(cardById.get(cardId))),
    source: 'dynamic-live-backlog-after-reviewed-phase-g-closeouts',
  }
}
...
export function buildFoundationReviewSprintStatus({
  ...
  const researchDispositions = artifact?.baseline?.researchCuration?.dispositions || []
  const phaseGReadiness = buildCurrentPhaseGReadiness(artifact, backlogItems)
