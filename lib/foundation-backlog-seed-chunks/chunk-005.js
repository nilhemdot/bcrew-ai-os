export const backlogSeedChunk005 = [
  ...
  {
      id: 'MYICOR-SOURCE-SYSTEM-MAP-001',
      ...
      nextAction: 'Run `npm run process:myicor-source-system-map-check -- --json`, then promote `MYICOR-MCP-CATALOG-SNAPSHOT-001` for persisted MCP catalog/delta state. Actual lesson extraction stays in `MYICOR-APPROVED-LESSON-EXTRACT-PROOF-001` with one exact approved lesson at a time.',
      statusNote: 'Scoped as source-system map only. Verified MCP exposes course catalog, lesson metadata, progress, resource search, trend reports, workstreams/tool stack, lifecycle catalog, and trusted sources. Full lesson body/script, video/audio playback, coaching-call recording inventory when not indexed, embedded links, and visual walkthrough evidence remain browser/content gaps. No broad crawl, lesson content extraction, scripts, video/audio capture, screenshots, downloads, atom/vector writes, external writes, Browserbase, or normal Chrome profile use is approved by this card.',
    },
  {
    ...
    },
  {
      ...
      summary: 'Make every source-browser packet choose the correct brain/runtime route before work starts: deterministic worker first, local virtual browser hands when interaction is needed, Source Session Broker/Harlan for auth/MFA, API/Stagehand only with explicit caps, and hosted Browserbase blocked for Human Web Agent V1.',
      whyItMatters: 'The system cannot keep confusing Codex/Claude subscriptions, API model spend, local Playwright, hosted browser spend, and Harlan auth. If the route is implicit, it will either fail late, spend money unexpectedly, or babysit Steve again.',
      nextAction: 'Prove deterministic, local hands, source-session/Harlan, API/Stagehand, hosted Browserbase blocked behavior, and blocked-action routing; expose the selected route in source-browser readback before running workers.',
      statusNote: 'Scoped as a browser brain route policy, not a broad model refactor. It blocks unsupported subscription-style Stagehand labels such as codex/gpt-5.5, blocks hosted Browserbase requests even when old env keys or stale bakeoff flags exist, requires proof-sized API/model caps, keeps Harlan sends dry-run until live-delivery approval, forbids Steve normal Chrome, and preserves no purchase/download/post/comment/message/profile/credential mutation boundaries.',
    },
  {
  ...
  {
      id: 'CODEBASE-MAP-HUB-001',
      title: 'Surface the Understand-Anything codebase map on the hub launcher',
      team: 'foundation',
      lane: 'scoped',
      priority: 'P2',
      rank: 923,
      source: '2026-05-29 chat with Steve: Understand-Anything (Lum1104/Understand-Anything) codebase map tool, from Better Stack video VmIUXVlt7_I',
      summary: 'Read the local .understand-anything/knowledge-graph.json produced by the Understand-Anything Claude Code plugin and render an interactive Codebase Map tile and page on the AIOS hub launcher in BC brand, so the code graph is viewable inside AIOS instead of only the local dashboard the tool ships.',
      whyItMatters: 'Builders (Claude and Codex) understand the codebase faster, and Steve can see the system shape from the hub with no terminal. The map is a local JSON file that stays on the Mac mini, which fits the modular hub model.',
      nextAction: 'After the first /understand map exists, scope a small slice: a hub-launcher tile, a route that reads knowledge-graph.json, and a brand-compliant graph view (embed the dashboard or render the JSON). Require acceptance proof, verifier coverage, and process:foundation-ship before build. Proposal-only until Steve approves.',
      statusNote: 'Scoped 2026-05-29, not active. Depends on Understand-Anything plugin install plus first map build. Build pulled only via this card.',
    }
]
