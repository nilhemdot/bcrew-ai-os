...
export function compactFoundationJob(job = {}) {
  ...
  return {
    ...
    cadence: job.cadence || null,
	    ...
	    ...(job.scheduleLocalTime ? { scheduleLocalTime: job.scheduleLocalTime } : {}),
	    ...(job.scheduleTimezone ? { scheduleTimezone: job.scheduleTimezone } : {}),
    ...
    mutationAllowlist: job.mutationAllowlist
      ? {
	          ...
	          ...(Array.isArray(job.mutationAllowlist.allowedPostures) ? { allowedPostures: job.mutationAllowlist.allowedPostures } : {}),
	          ...(Array.isArray(job.mutationAllowlist.allowedRuntimeModes) ? { allowedRuntimeModes: job.mutationAllowlist.allowedRuntimeModes } : {}),
	          ...(job.mutationAllowlist.reason ? { reason: job.mutationAllowlist.reason } : {}),
...
export function compactFoundationJobRunSnapshot(snapshot = {}) {
  ...
  return {
    ...
    workerReliability: snapshot.workerReliability
      ? {
          generatedAt: snapshot.workerReliability.generatedAt || null,
          cardId: snapshot.workerReliability.cardId || null,
          closeoutKey: snapshot.workerReliability.closeoutKey || null,
          status: snapshot.workerReliability.status || null,
          plainEnglish: truncateText(snapshot.workerReliability.plainEnglish, 240) || null,
          summary: snapshot.workerReliability.summary || {},
          failClosed: snapshot.workerReliability.failClosed || {},
        }
      : null,
