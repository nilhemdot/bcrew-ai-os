...
export async function runSourceSessionProfileProbe({
  ...
  if (keychainMetadataPresent === undefined) {
    keychainMetadataPresent = await keychainItemExists({ source: resolvedSource, account: resolvedAccount }).catch(() => false)
  }
  ...
  const report = {
    ...
    source: resolvedSource,
    account: resolvedAccount,
    secretRef: buildSourceSessionSecretRef({ source: resolvedSource, account: resolvedAccount }),
