...
function validateRequiredApprovalFields(approval, cardId, checks) {
  ...
  checks.push({
    ok: meetsApprovalThreshold(approval?.score),
    check: `approval score is at least ${APPROVAL_MIN_APPROVED_PLAN_SCORE_LABEL}`,
...
export async function validatePlanApprovalFile({
  ...
  if (Number(approval.approvalSchemaVersion) === APPROVAL_SCHEMA_VERSION) {
    ...
    if (legacyEntry) {
      checks.push({
        ok: meetsApprovalThreshold(approval.score),
        check: `legacy approval keeps ${APPROVAL_MIN_APPROVED_PLAN_SCORE_LABEL} score`,
        detail: String(approval.score ?? 'missing'),
      })
    }
  }
...
export async function buildSyntheticApprovalIntegrityStatus() {
  ...
  try {
    ...
    const approval = {
      ...
      score: APPROVAL_MIN_APPROVED_PLAN_SCORE,
      approvedBy: 'Steve',
