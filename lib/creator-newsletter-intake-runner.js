...
function buildSignupPacket({ report = {}, form = null } = {}) {
  return {
    ...
    nextAction: report.submitAllowedNow
      ? 'Operator-approved newsletter signup can run through the source identity lane, then confirmation email must be read back before claiming subscribed status.'
...
export async function runCreatorNewsletterIntake({
  ...
  const report = {
    ...
    ok: false,
    status: 'blocked',
    reason: 'external_newsletter_signup_requires_live_operator_lane',
    plainEnglish: 'External newsletter signup still requires the live operator/signup lane. This runner proves detection and local fixture submit only.',
  }
  return {
    ...report,
    signupPacket: buildSignupPacket({ report, form: selectedForm }),
