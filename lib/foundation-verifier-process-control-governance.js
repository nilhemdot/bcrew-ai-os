...
export const VERIFIER_PROCESS_CONTROL_GOVERNANCE_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-process-control-governance-split-closeout.md'
...
export const VERIFIER_PROCESS_CONTROL_ORCHESTRATION_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-process-control-orchestration-split-closeout.md'
...
export async function evaluateFoundationVerifierProcessControlGovernance(input = {}) {
  ...
  const readinessFollowupVerifier = await evaluateFoundationVerifierReadinessFollowup({
    ...
    salesHubCheckSource,
    salesUiSource,
    serverSource: readinessServerSource,
    sourceContractsSource,
    sourceOfTruth,
    sourceRegistry,
    systemRegistrationSweep,
...
export async function evaluateFoundationVerifierProcessControlGovernanceOrchestration(input = {}) {
  ...
  const foundationVerifyRootSource = normalizedInput.foundationVerifyRootSource || foundationVerifySource
  const rootLineCount = String(foundationVerifyRootSource || '').split('\n').length
  const rootDelegatesThroughWrapper =
    foundationVerifyRootSource.includes('evaluateFoundationVerifierProcessControlGovernanceOrchestration({') &&
    foundationVerifyRootSource.includes('processControlGovernanceVerifier.checks')
  const oldDirectRootCallRemoved = !foundationVerifyRootSource.includes('evaluateFoundationVerifierProcessControlGovernance({')
