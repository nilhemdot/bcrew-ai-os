...
export function evaluateFoundationAgentFeedbackStoreSplit({
  ...
  addCheck(
    ...
    moduleSource.includes('export function createFoundationAgentFeedbackStore') &&
      ...
      moduleSource.includes('async function listAgentFeedbackSendAttemptsForRunId') &&
...
export function createFoundationAgentFeedbackStore({
  ...
  async function listAgentFeedbackSendAttemptsForRunId(runId) {
    const normalizedRunId = String(runId || '').trim()
    if (!normalizedRunId) throw new Error('Agent Feedback production run id is required.')
...
export async function buildSyntheticFoundationAgentFeedbackStoreBehaviorProof() {
  ...
  async function fakeQuery(sql, values = []) {
    ...
    if (text.includes("metadata ->> 'runId' = $1")) {
      return { rows: rows.sendAttempt ? [rows.sendAttempt] : [] }
    }
  ...
  const send = await store.createAgentFeedbackSendAttempt({
    ...
    milestoneDay: 30,
    tokenHash: 'token-hash',
    metadata: { runId: 'synthetic-production-run' },
  })
  ...
  const runSendAttempts = await store.listAgentFeedbackSendAttemptsForRunId('synthetic-production-run')
  ...
  addCheck(checks, runSendAttempts[0]?.id === send.id, 'send attempt run-id reader maps output shape', runSendAttempts[0]?.id || 'missing')
