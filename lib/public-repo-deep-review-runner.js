...
const REPO_CHROME_PATH_RE = /\/(?:pulls?|projects?|issues?|actions|security|pulse|graphs|network|stargazers|watchers|forks|settings|branches|tags|releases|compare|commits?)(?:\/|$)/i
...
const REPO_CHROME_SNIPPET_RE = /\b(Resetting focus|You signed in with another tab|You signed out in another tab|GitHub Spark|GitHub Copilot|GitHub Models|Instant dev environments|Manage code changes|Find and fix vulnerabilities)\b/i
...
function firstMatchGroup(source = '', patterns = []) {
...
function extractBlobCodeHtml(html = '') {
  const cells = [...String(html || '').matchAll(/<td\b[^>]*class=["'][^"']*\bblob-code\b[^"']*["'][^>]*>([\s\S]*?)<\/td>/gi)]
    .map(match => match[1])
    .filter(Boolean)
...
function extractPrimaryRepoHtml(html = '') {
  const source = String(html || '')
  return firstMatchGroup(source, [
    /<article\b[^>]*class=["'][^"']*\bmarkdown-body\b[^"']*["'][^>]*>([\s\S]*?)<\/article>/i,
    /<div\b[^>]*class=["'][^"']*\breadme\b[^"']*["'][^>]*>([\s\S]*?)<\/div>/i,
    /<div\b[^>]*class=["'][^"']*\bfile-content\b[^"']*["'][^>]*>([\s\S]*?)<\/div>/i,
    /<main\b[^>]*>([\s\S]*?)<\/main>/i,
  ]) || extractBlobCodeHtml(source)
}
...
function repoChromePath(url = '', repoIdentity = {}) {
  const parsed = parsedUrl(url)
  if (!parsed || !repoIdentity?.basePath) return false
  const pathname = parsed.pathname.toLowerCase()
  const basePath = text(repoIdentity.basePath).toLowerCase()
  if (pathname.replace(/\/$/, '') === basePath.replace(/\/$/, '')) return false
  if (!pathname.startsWith(basePath)) return false
  return REPO_CHROME_PATH_RE.test(pathname.slice(basePath.length) || '/')
}
...
function classifyRepoLink({ url = '', label = '', repoIdentity = {} } = {}) {
  ...
  if (repoChromePath(url, repoIdentity)) {
    return {
      url,
      label,
      decision: 'blocked_repo_chrome_path',
      shouldFollow: false,
      reason: 'Repo chrome such as pulls, projects, issues, actions, releases, and settings is not implementation evidence.',
    }
  }
...
function implementationPatternsForPage(page = {}) {
  return IMPLEMENTATION_PATTERN_DEFINITIONS
    .map(definition => {
      ...
      if (REPO_CHROME_SNIPPET_RE.test(snippet)) return null
      return {
...
export async function runPublicRepoDeepReview({
  ...
  while (queue.length && pages.length < number(maxPages, 8)) {
    ...
    const primaryHtml = extractPrimaryRepoHtml(html)
    const pageText = stripHtml(primaryHtml || html)
    const page = {
      url: currentUrl,
      title,
      pageKind: pageKindFor(currentUrl, title),
      text: pageText.slice(0, 12000),
      primaryContentUsed: Boolean(primaryHtml),
      chromeTextFiltered: Boolean(primaryHtml),
  ...
  const report = {
    ...
    pages: pages.map(page => ({
      ...
      title: page.title,
      pageKind: page.pageKind,
      primaryContentUsed: page.primaryContentUsed,
      chromeTextFiltered: page.chromeTextFiltered,
