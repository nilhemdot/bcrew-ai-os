...
function compactSourceLayerStatusForRoute(status = {}) {
  const {
...
export async function buildSourceOfTruthPayload({ repoRoot = process.cwd() } = {}) {
  ...
  const scoredSourceLayerStatus = compactSourceLayerStatusForRoute(
    attachSourceTrustScoresToLayerStatus(sourceLayerStatus, sourceTrustScoring),
  )
