...
import {
  BUILD_CLOSEOUT_DATA_SOURCE_CARD_ID,
  evaluateBuildCloseoutDataSourceUsage,
} from './build-closeout-data-source.js'
...
export const CODE_QUALITY_NIGHTLY_AUDIT_MIN_FINDING_COUNT = 7
...
function routeHasVerifiedCloseout(route = {}, closeouts = []) {
  ...
  return findVerifiedCloseoutByKey(closeouts, route.targetCloseoutKey) ||
    findVerifiedCloseoutByKey(closeouts, route.coveredByCloseoutKey)
}

export function reconcileCodeQualityNightlyAuditFindings({
  findings = [],
...
export function detectProcessReportWritePolicyInText({ relativePath = 'synthetic.mjs', text = '' } = {}) {
  const policy = buildProcessCheckReportOutputPolicyFindingInput({ relativePath, text })
  if (!policy.risk) return []
  return [finding({
    id: 'process-check-report-write-policy-risk',
    cardId: 'CODEBASE-HARDCODE-AUDIT-001',
    severity: 'P1',
    type: 'drift_risk',
    title: 'Process/check path writes repo reports without explicit report-output posture',
    whyItMatters: 'Live extraction and synthesis runs should persist operational truth to the Foundation store. Markdown/source-note files are checkpoint artifacts and should require an explicit `--write-report` posture so normal jobs do not dirty Git or turn repo docs into a runtime database.',
    refs: [makeRef(relativePath, policy.firstWriteLine, policy.classification)],
    proposedCard: PROCESS_CHECK_REPORT_OUTPUT_POLICY_CARD_ID,
    proposedOwner: 'Foundation Process',
    detector: 'process check report-output policy detector',
    falsePositiveNote: policy.reason,
  })]
}

export function detectRuntimePolicyHardcodesInText({ relativePath = 'synthetic.mjs', text = '' } = {}) {
  const findings = []
  if (!/^scripts\/process-.*\.mjs$|^lib\//.test(String(relativePath || ''))) return findings
  const policy = buildRuntimeModelLiteralPolicyFindingInput({ relativePath, text })
  if (policy.risk) {
    findings.push(finding({
      id: 'runtime-model-name-hardcode-risk',
      cardId: 'CODEBASE-HARDCODE-AUDIT-001',
      severity: 'P2',
      type: 'drift_risk',
      title: 'Runtime model name appears outside router/config ownership',
      whyItMatters: 'Model choices should be controlled by the Foundation LLM router or provider capability records. Scattering exact model names makes upgrades brittle and can leave critical intelligence lanes on old models.',
      refs: [makeRef(relativePath, policy.firstLine, policy.firstLiteral)],
      proposedCard: 'LLM-RUNTIME-CONFIG-AUDIT-001',
      proposedOwner: 'LLM Router',
...
async function buildMonolithFindings(repoRoot, allFiles, fileTexts) {
  ...
  const buildCloseoutDataSourceUsage = evaluateBuildCloseoutDataSourceUsage({
    dataSourceSource: fileTexts['lib/build-closeout-data-source.js'] || '',
    buildLogSource: fileTexts['lib/foundation-build-log.js'] || '',
    sourceContractsSource: fileTexts['lib/source-contracts.js'] || '',
    codeQualityAuditSource: fileTexts['lib/code-quality-nightly-audit.js'] || '',
    deepAuditClosureSource: fileTexts['lib/deep-audit-findings-closure-gate.js'] || '',
  })
  const knownSurfaces = [
    {
    ...
    },
    {
      ...
      proposedCard: BUILD_CLOSEOUT_DATA_SOURCE_CARD_ID,
    },
  ]
  const findings = knownSurfaces.filter(surface => {
    ...
    if (surface.id === 'build-closeout-code-owned-data' && buildCloseoutDataSourceUsage.ok) {
      return false
    }
    return true
  }).map(surface => {
    const text = fileTexts[surface.path] || ''
    const refs = surface.id === 'build-closeout-code-owned-data' && buildCloseoutDataSourceUsage.refs.length
      ? buildCloseoutDataSourceUsage.refs.map(ref => makeRef(ref.path, ref.line, ref.detail || ref.match))
      : [makeRef(surface.path, firstLineOf(text, surface.pattern))]
    return finding({
      id: surface.id,
...
function buildMutationFindings(fileTexts, packageJson) {
  ...
  for (const [relativePath, text] of Object.entries(fileTexts)) {
    if (/^scripts\/process-.*\.mjs$/.test(relativePath) || relativePath === 'scripts/intelligence-action-router-apply.mjs') {
      ...
      findings.push(...detectProcessReportWritePolicyInText({ relativePath, text }))
    }
    findings.push(...detectRuntimePolicyHardcodesInText({ relativePath, text }))
  }
...
export function buildSyntheticCodeQualityNightlyAuditProof() {
  ...
  const runtimeModelOwner = detectRuntimePolicyHardcodesInText({
    relativePath: 'lib/llm-provider-pricing.js',
    text: `export const GEMINI_STANDARD_PRICING_BY_MODEL = { '${syntheticGeminiModel}': {} }`,
  })
  return {
    ok: hardcoded.length >= 2 &&
      ...
      runtimeModelHardcode.length === 1 &&
      runtimeModelIdFalsePositive.length === 0 &&
      runtimeModelOwner.length === 0,
    ...
    unguardedReportWriterCount: unguardedReportWriter.length,
    guardedReportWriterCount: guardedReportWriter.length,
    runtimeModelHardcodeCount: runtimeModelHardcode.length,
    runtimeModelIdFalsePositiveCount: runtimeModelIdFalsePositive.length,
    runtimeModelOwnerCount: runtimeModelOwner.length,
  }
}
...
export async function buildCodeQualityNightlyAudit({
  ...
  const findingReconciliation = reconcileCodeQualityNightlyAuditFindings({ findings: rawFindings })
  ...
  return {
    ...
    summary: {
      ...
      rawFindingCount: findingReconciliation.summary.rawFindingCount,
      reconciledClosedFindingCount: findingReconciliation.summary.reconciledClosedFindingCount,
    },
  }
}
...
export function renderCodeQualityNightlyAuditReport(audit = {}) {
  ...
  const reconciliation = audit.findingReconciliation || {}
  const reconciledFindings = Array.isArray(reconciliation.reconciledFindings)
    ? reconciliation.reconciledFindings
    : []
...
- Active findings: ${audit.summary?.findingCount || findings.length} total (${audit.summary?.p0 || 0} P0, ${audit.summary?.p1 || 0} P1, ${audit.summary?.p2 || 0} P2, ${audit.summary?.p3 || 0} P3)
- Raw detector signals: ${audit.summary?.rawFindingCount ?? ((audit.rawFindings || []).length || findings.length)}
- Closed detector signals reconciled out: ${audit.summary?.reconciledClosedFindingCount ?? reconciledFindings.length}
- Active proposed backlog fixes: ${proposedCards.length}
...
${reconciledFindings.length
  ? reconciledFindings.map(finding => `- \`${finding.id}\`: ${finding.title} -> ${finding.closeout?.key || finding.reconciliationReason || 'verified closeout'}`).join('\n')
  : '- None'}
