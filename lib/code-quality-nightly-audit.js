...
export function detectProcessReportWritePolicyInText({ relativePath = 'synthetic.mjs', text = '' } = {}) {
  if (!/^scripts\/process-.*-check\.mjs$/.test(String(relativePath || ''))) return []
  if (!/\b(?:fs\.)?writeFile(?:Sync)?\s*\(/.test(text)) return []
  const hasExplicitReportWriteGuard = /isProcessReportWriteRequested\s*\(/.test(text) ||
    (/PROCESS_CHECK_WRITE_FLAGS\.writeReport/.test(text) && /writeReport/.test(text))
  if (hasExplicitReportWriteGuard) return []
  const line = firstLineOf(text, /\b(?:fs\.)?writeFile(?:Sync)?\s*\(/)
  return [finding({
    ...
    refs: [makeRef(relativePath, line)],
    proposedCard: 'PROCESS-CHECK-REPORT-OUTPUT-POLICY-001',
    proposedOwner: 'Foundation Process',
    detector: 'process check report-output policy detector',
    falsePositiveNote: 'Intentional one-off report generators are acceptable when they are gated by `--write-report` or are not part of live/scheduled extraction.',
  })]
}
...
export function detectRuntimePolicyHardcodesInText({ relativePath = 'synthetic.mjs', text = '' } = {}) {
  ...
  if (/gemini-[0-9.]+-[a-z0-9-]+|gpt-[0-9.]+|claude-[a-z0-9.-]+|opus-[0-9.]+/i.test(text) && !/llm_routes|llm-route|DEFAULT_LLM_ROUTES|model routing|provider capability/i.test(text)) {
    findings.push(finding({
      ...
      refs: [makeRef(relativePath, firstLineOf(text, /gemini-[0-9.]+-[a-z0-9-]+|gpt-[0-9.]+|claude-[a-z0-9.-]+|opus-[0-9.]+/i))],
      proposedCard: 'LLM-RUNTIME-CONFIG-AUDIT-001',
      proposedOwner: 'LLM Router',
      detector: 'runtime model literal detector',
      falsePositiveNote: 'Router defaults, provider docs, historical handoffs, and test fixtures may name exact models; active workload scripts should route through config.',
    }))
  }
  return findings
}
