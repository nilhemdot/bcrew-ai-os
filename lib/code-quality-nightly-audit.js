...
export const CODE_QUALITY_NIGHTLY_AUDIT_REPORT_PATH = 'docs/handoffs/2026-05-13-code-quality-nightly-audit-report.md'
...
export function detectHardcodedLiveTruthInText({ relativePath = 'synthetic.js', text = '' } = {}) {
  ...
  if (/EXPECTED_SOURCE_COUNT|MIN_SOURCE_CONTRACTS|sourceContracts\.length\s*===|all\s+\d+\s+source contracts|\b35 source|\b36 source/i.test(text)) {
    detections.push(finding({
      ...
      refs: refsFor(/EXPECTED_SOURCE_COUNT|MIN_SOURCE_CONTRACTS|sourceContracts\.length\s*===|all\s+\d+\s+source contracts|\b35 source|\b36 source/i),
...
export function detectMutationPatternsInText({ relativePath = 'synthetic.mjs', text = '' } = {}) {
  ...
  return [finding({
    ...
    falsePositiveNote: 'Closeout scripts can be expected mutators, but they need explicit apply/confirm boundaries before reuse by nightly audits.',
  })]
}
...
function buildHardcodedTruthFindings(fileTexts) {
  ...
  if (exactSprintRefs.length) {
    findings.push(finding({
      ...
      falsePositiveNote: 'Acceptable only in explicitly historical closeout or migration checks.',
    }))
  }
  const gstackRefs = collectPatternRefs(fileTexts, /GSTACK_BUILD_INTEL_EXPECTED_COMMIT|EXPECTED_COMMIT|sourceCommit\s*===/i, { limit: 6 })
  if (gstackRefs.length) {
    findings.push(finding({
      ...
      refs: gstackRefs.map(ref => makeRef(ref.path, ref.line, ref.match)),
...
function buildApiStaticFindings(fileTexts) {
  ...
  if (server.includes('git') && server.includes('--max-count') && buildLog.includes('closeoutRecords')) {
    findings.push(finding({
...
async function buildFrontendFindings(repoRoot, fileTexts) {
  ...
  const serverText = fileTexts['server.js'] || ''
  const assetPaths = ['public/foundation.js', 'public/styles.css', 'public/foundation.html']
  const assetMetrics = []
  for (const assetPath of assetPaths) {
    const text = fileTexts[assetPath] || await readText(repoRoot, assetPath)
    const stat = await statFile(repoRoot, assetPath)
    assetMetrics.push({
      path: assetPath,
      bytes: stat?.size || Buffer.byteLength(text),
      gzipBytes: zlib.gzipSync(Buffer.from(text)).length,
      lines: text ? text.split(/\r?\n/).length : 0,
    })
  }
  const foundationJs = assetMetrics.find(item => item.path === 'public/foundation.js')
  const styles = assetMetrics.find(item => item.path === 'public/styles.css')
  if ((foundationJs?.bytes || 0) > 500_000 || (styles?.bytes || 0) > 150_000) {
    findings.push(finding({
      id: 'large-uncached-foundation-assets',
      cardId: 'FOUNDATION-FRONTEND-PERF-AUDIT-001',
      severity: 'P1',
      type: 'performance_risk',
      title: 'Foundation ships large frontend assets under no-store cache policy',
      whyItMatters: 'Every refresh can redownload and reparse the full control-panel bundle and stylesheet.',
      refs: [
        makeRef('public/foundation.html', firstLineOf(fileTexts['public/foundation.html'] || '', /foundation\.js|styles\.css/)),
        makeRef('server.js', firstLineOf(serverText, /Cache-Control.*no-store|no-store/)),
      ],
      proposedCard: 'FOUNDATION-FRONTEND-ASSET-BUDGET-001',
      proposedOwner: 'Foundation Frontend',
      detector: `asset budget detector js=${foundationJs?.bytes || 0} css=${styles?.bytes || 0}`,
    }))
  }
  ...
  const createElementCount = (foundationJsText.match(/document\.createElement/g) || []).length
  const innerHtmlCount = (foundationJsText.match(/innerHTML/g) || []).length
  if (createElementCount > 1000 || innerHtmlCount > 50) {
    findings.push(finding({
      ...
      whyItMatters: 'Backlog and source filters can churn large DOM trees on each interaction as card/source counts grow.',
      refs: [makeRef('public/foundation.js', 1, `${createElementCount} createElement, ${innerHtmlCount} innerHTML`)],
      proposedCard: 'FOUNDATION-FRONTEND-DOM-BUDGET-001',
      proposedOwner: 'Foundation Frontend',
      detector: 'DOM churn signal counter',
    }))
  }
  ...
  return { findings, assetMetrics, routeMatrix: [
    '#current-state',
    '#systems',
    '#backlog',
    '#source-lifecycle',
...
async function buildMonolithFindings(repoRoot, allFiles, fileTexts) {
  ...
  const findings = knownSurfaces.map(surface => {
...
function buildAssumptionFindings(fileTexts) {
  const planCriticRefs = collectPatternRefs(fileTexts, /\b9\.8\b/g, { fileFilter: file => /^scripts\/|^lib\//.test(file), limit: 8 })
  const findings = []
  if (planCriticRefs.length) {
    findings.push(finding({
      ...
      whyItMatters: 'Approval policy can drift when some checks import the registry constant and others hardcode the score.',
      refs: planCriticRefs.map(ref => makeRef(ref.path, ref.line, ref.match)),
      proposedCard: 'APPROVAL-THRESHOLD-REGISTRY-001',
      proposedOwner: 'Plan Critic',
      detector: 'raw approval threshold detector',
      falsePositiveNote: 'The canonical constant is valid; duplicated raw literals are the risk.',
    }))
  }
...
function buildMutationFindings(fileTexts, packageJson) {
  ...
  for (const job of getFoundationJobDefinitions()) {
    ...
    if (detectMutationPatternsInText({ relativePath: scriptPath || scriptName, text }).length) {
