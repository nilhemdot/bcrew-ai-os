...
import { filterGeneralDecisionRecords } from './decision-restricted-queue.js';
    ...
    candidates: {
      ...
      items: {
        ...
        properties: {
          ...
          owner_hint: { type: 'string', maxLength: 120 },
          evidence_excerpt: { type: 'string', maxLength: 420 },
...
function buildCandidateOutputSchema({ maxItems = 8 } = {}) {
  const schema = JSON.parse(JSON.stringify(SHARED_CANDIDATE_OUTPUT_SCHEMA));
  schema.properties.candidates.maxItems = maxItems;
  return schema;
}
...
export function buildFoundationExtractionContext(snapshot, strategyMarkdown, sourceContracts) {
  ...
  return {
    ...
    decisions: generalDecisions.slice(0, 40).map(item => ({
      id: item.id,
      title: item.title,
...
export function getLlmProvenance(llmResult, requestedModel = '') {
  return {
    requestedModel: requestedModel || null,
    model: llmResult?.model || requestedModel || null,
    provider: llmResult?.provider || llmResult?.plan?.selectedRoute?.provider || null,
    authPath: llmResult?.authPath || llmResult?.plan?.selectedRoute?.authPath || null,
    routeKey: llmResult?.routeKey || llmResult?.plan?.selectedRoute?.routeKey || null,
    credentialKey: llmResult?.credentialKey || llmResult?.plan?.selectedRoute?.credentialKey || null,
    callId: llmResult?.call?.callId || null,
  };
}

export function getErrorLlmProvenance(error) {
  if (!error || typeof error !== 'object') return null;
  return error.llmProvenance || error.llm || null;
}

export function assertCompleteLlmProvenance(llm, context = 'LLM extraction') {
  const missing = [];
  if (!llm?.model) missing.push('model');
  if (!llm?.provider) missing.push('provider');
  if (!llm?.authPath) missing.push('authPath');
  if (!llm?.routeKey) missing.push('routeKey');
  if (missing.length) {
    throw new Error(`${context} missing routed LLM provenance: ${missing.join(', ')}`);
  }
  return llm;
}

export function sanitizeExtractedCandidates(rawCandidates, artifact, foundationContext, { extractionMethod, model, llm = null }) {
  ...
  const llmProvenance = assertCompleteLlmProvenance(llm, `Candidate sanitizer for ${artifact?.artifactId || 'unknown artifact'}`);
  ...
  return (Array.isArray(rawCandidates) ? rawCandidates : [])
    .map(rawCandidate => {
      ...
      return {
        ...
        metadata: {
          extractionMethod,
          requestedModel: llmProvenance.requestedModel || model || null,
          model: llmProvenance.model || model || null,
          provider: llmProvenance.provider || null,
          authPath: llmProvenance.authPath || null,
          routeKey: llmProvenance.routeKey || null,
          credentialKey: llmProvenance.credentialKey || null,
          llmCallId: llmProvenance.callId || null,
...
export async function extractSharedCandidatesWithOpenAi({
  ...
  timeoutMs = Number(process.env.LLM_EXTRACTION_TIMEOUT_MS || 120000),
  retryMalformedJson = true,
}) {
  const basePromptLines = (Array.isArray(systemPromptLines) ? systemPromptLines : [])
    .map(line => String(line || '').trim())
    .filter(Boolean);

  async function runExtractorAttempt({
    attempt,
    attemptMaxChars,
    candidateMaxItems,
    promptPrefix = [],
    ...
    const normalizedPrompt = [
      ...promptPrefix,
      ...basePromptLines,
      `Return at most ${candidateMaxItems} candidates.`,
      'Return compact valid JSON only. Do not include markdown, commentary, trailing prose, or unterminated strings.',
    ].join(' ');

    const trimmedContent = shorten(contentText || '', attemptMaxChars);
    const llmResult = await callLlm({
      ...
      responseFormat: {
        ...
        name: schemaName,
        strict: true,
        schema: buildCandidateOutputSchema({ maxItems: candidateMaxItems }),
      },
      maxOutputTokens,
      dryRun: false,
      metadata: {
    ...
    const llm = getLlmProvenance(llmResult, model);
    const outputText = llmResult.outputText;
    if (!outputText) {
      const error = new Error('LLM extraction returned no output text.');
      error.llmProvenance = llm;
      throw error;
    }
    ...
    try {
      ...
      if (error && typeof error === 'object') {
        ...
        error.outputPreview = outputText.slice(0, 280);
      }
      throw error;
    }
  }
  ...
  let llm;
  try {
    const firstAttempt = await runExtractorAttempt({
      attempt: 1,
      attemptMaxChars: maxChars,
      candidateMaxItems: 8,
    });
    parsed = firstAttempt.parsed;
    llm = firstAttempt.llm;
  } catch (error) {
    if (!retryMalformedJson || !(error instanceof SyntaxError)) {
      throw error;
    }
    const retryAttempt = await runExtractorAttempt({
      attempt: 2,
      attemptMaxChars: Math.min(maxChars, 12000),
      candidateMaxItems: 3,
      promptPrefix: [
        'The previous extraction response was malformed JSON. Retry with fewer candidates, shorter evidence excerpts, and stricter compact JSON.',
      ],
    });
    parsed = retryAttempt.parsed;
    llm = retryAttempt.llm;
  }
