...
export function buildGodModeExtractorParitySnapshot({ activeExtractionLanes = [], generatedAt = new Date().toISOString() } = {}) {
  ...
  const families = [
    ...
    family({
      ...
      currentLevel: 'decision_ledger_ready_exact_public_runtime_ready_after_packet',
      modelRoute: 'Source packet decision ledger plus local Playwright-first exact public page runtime',
      cadence: 'Operator-reviewed queue',
      capabilities: {
        hands: 'exact_url_runtime_ready',
      ...
      blockers: [
        'Live public page runs still need first approved production packet and Foundation ingestion review.',
        'Decision records do not start the worker; a separate source-packet worker runner still has to consume approved records.',
