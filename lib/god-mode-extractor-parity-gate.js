...
export function buildGodModeExtractorParitySnapshot({ activeExtractionLanes = [], generatedAt = new Date().toISOString() } = {}) {
  ...
  const families = [
    ...
    family({
      ...
      modelRoute: 'Pending comment extractor route',
      cadence: 'Blocked until extractor proof exists',
      capabilities: {
        reading: 'planned',
        brain: 'planned',
        evidence: 'planned',
      ...
      blockers: ['Needs public comment capture, sampling, provenance, and quality scoring.'],
      nextCard: 'YOUTUBE-PUBLIC-COMMENTS-EXTRACTOR-001',
    }),
    family({
