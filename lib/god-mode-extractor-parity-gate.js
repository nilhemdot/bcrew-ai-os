...
export function buildGodModeExtractorParitySnapshot({ activeExtractionLanes = [], generatedAt = new Date().toISOString() } = {}) {
  ...
  const families = [
    ...
    family({
      ...
      accessBoundary: 'Public/free exact URL auto-read; separate packet/SOP before follow-up crawl, forms, downloads, auth, or paid/private areas',
      currentLevel: 'decision_ledger_worker_persistence_and_production_safe_hands_queue_v1',
      modelRoute: 'Source packet decision ledger, exact public page worker, local Playwright-first bounded Hands V1, and production-safe Hands queue/run status',
      cadence: 'Operator-reviewed queue',
      capabilities: {
        hands: 'production_safe_hands_queue_ready',
        reading: 'runtime_ready_after_packet',
        brain: 'source_packet_classifier',
        evidence: 'runtime_artifact_ready',
        boundaries: PROVEN,
        output: 'proposal_only_runtime_artifact',
        resourceLinks: 'approval_decision_ledger',
        sourcePacket: PROVEN,
      },
      blockers: [
        'Live public page Hands runs still need first approved real packet and Foundation ingestion review.',
        'Bounded Hands V1 proved click_navigation_ready for exact public packets with explicit policy.',
        'Approval records do not start the worker; the separate source-packet worker route consumes approved records.',
        'Bounded Hands runs now have a separate queue/run route, but each run still needs explicit selector/action and next URL policy.',
        'Discovered links are classified only; each follow-up still needs its own source packet.',
        'Bounded Hands V1 allows exactly one approved click; broad crawling remains blocked.',
      ],
    ...
    family({
      ...
      currentLevel: 'planned_newsletter_source_lane',
      modelRoute: 'Mailbox label/folder monitor plus newsletter issue extractor pending',
      cadence: 'Signup/onboarding proof first, then daily mailbox monitor',
      capabilities: {
        hands: 'planned_free_signup_flow',
        reading: 'planned_mailbox_monitor',
        brain: 'planned_issue_extractor',
        evidence: 'planned',
        boundaries: PROVEN,
        output: 'planned_creator_source_stack_update',
