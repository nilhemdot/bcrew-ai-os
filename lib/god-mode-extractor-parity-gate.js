...
export function buildGodModeExtractorParitySnapshot({ activeExtractionLanes = [], generatedAt = new Date().toISOString() } = {}) {
  ...
  const families = [
    ...
    family({
      ...
      currentLevel: 'exact_public_runtime_ready_after_packet',
      modelRoute: 'Source packet preview plus local Playwright-first exact public page runtime',
      ...
      blockers: [
        'Approval UI records packet previews but does not yet persist Steve-approved packet decisions.',
