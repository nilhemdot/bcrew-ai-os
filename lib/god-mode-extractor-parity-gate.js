...
export function buildGodModeExtractorParitySnapshot({ activeExtractionLanes = [], generatedAt = new Date().toISOString() } = {}) {
  ...
  const families = [
    ...
    family({
      ...
      currentLevel: 'decision_ledger_worker_persistence_and_bounded_hands_v1',
      modelRoute: 'Source packet decision ledger, exact public page worker, and local Playwright-first bounded Hands V1',
      cadence: 'Operator-reviewed queue',
      capabilities: {
        hands: 'click_navigation_ready',
      ...
      blockers: [
        'Live public page Hands runs still need first approved production packet and Foundation ingestion review.',
      ...
      nextCard: EXTRACTOR_HANDS_BROWSER_RUNTIME_CARD_ID,
    }),
    family({
