...
export async function buildPlainEnglishSweepStatus({ repoRoot = defaultRepoRoot } = {}) {
  ...
  for (const entry of changedProofEntries) {
    ...
    const proofText = normalizeText(entry.proofText || entry.after)
    if (!relativePath || !proofText) continue
    const sourceText = await readOptionalText(path.join(repoRoot, relativePath))
    addFinding(
      findings,
      sourceText.includes(proofText),
      `changed copy is present for ${entry.id}`,
      `${relativePath} :: ${proofText.slice(0, 80)}`,
    )
  }
