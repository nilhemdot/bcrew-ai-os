...
} from './dev-director-daily-source-review-loop.js'
import {
  SOURCE_EXTRACTION_STATE_LEDGER_REPORT_ARTIFACT_ID,
} from './source-extraction-state-ledger.js'
import {
  MYICOR_MCP_CATALOG_REPORT_ARTIFACT_ID,
} from './myicor-mcp-catalog-snapshot.js'
import {
  MYICOR_APPROVED_LESSON_EXTRACT_PROOF_REPORT_ARTIFACT_ID,
} from './myicor-approved-lesson-extract-proof.js'
import {
  SKOOL_SOURCE_SYSTEM_MAP_REPORT_ARTIFACT_ID,
} from './skool-source-system-map.js'
  ...
  app.get('/api/foundation/dev-team-hub', requireAdminToken, async (_req, res) => {
    try {
      const [
        ...
        getFoundationSnapshot(),
        typeof listYoutubeFullWatchReportArtifacts === 'function'
        ...
        getIntelligenceReportBundle(MYICOR_APPROVED_LESSON_EXTRACT_PROOF_REPORT_ARTIFACT_ID, { atomLimit: 10, hitLimit: 10 }),
      ...
      res.json(buildDevTeamHubV0Snapshot({
        ...
        skoolSourceSystemMapBundle,
        geminiVideoReviewCalls,
        youtubeFullWatchReports: youtubeFullWatchReports || [],
