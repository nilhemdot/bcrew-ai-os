...
export function closeoutRecordAsBuildLogEntry(closeout) {
  return closeout
    ? {
        ...
        backlogIds: asArray(closeout.backlogIds),
        mentionedBacklogIds: asArray(closeout.mentionedBacklogIds),
      }
    : null
}
...
export function buildFoundationVerifierBuildLogCloseoutEntries(input = {}) {
  ...
  return {
    buildLogRecentMultiCloseoutBuild: findBuild(builds, ['RECENT-BUILDS-MULTI-CLOSEOUT-001'], 'recent-builds-multi-closeout-ux-v1'),
    buildLogFullSystemReAuditBuild: findBuild(builds, ['FULL-SYSTEM-RE-AUDIT-001'], 'full-system-re-audit-v1'),
    buildLogWaveCleanupABuild: findBuild(builds, ['LOCAL-DOC-LINK-001', 'DOC-AUTHORITY-INDEX-REPAIR-001', 'DOC-OTHER-TRIAGE-001'], 'wave-cleanup-a-local-docs-triage-v1'),
    buildLogWaveCleanupBBuild: findBuild(builds, ['DOC-CATEGORIZATION-001', 'DOCTRINE-PROPAGATION-002', 'PROCESS-HOOKS-002'], 'wave-cleanup-b-doc-categories-doctrine-hooks-v1'),
    buildLogGatePerformanceBuild: findBuild(builds, ['GATE-PERFORMANCE-001'], 'gate-performance-v1'),
    buildLogPlanReconcileBuild: findBuild(builds, ['FOUNDATION-PLAN-RECONCILE-001'], 'foundation-plan-reconcile-backlog-depth-v1'),
    buildLogPhase1EnforcementBuild: findBuild(builds, ['APPROVAL-FILE-INTEGRITY-001', 'BUILD-LOG-BACKLOG-ID-FIX-001', 'CLOSEOUT-BACKFILL-001', 'PRE-COMMIT-HOOK-INSTALL-001'], 'phase-1-enforcement-v1'),
    buildLogFoundationControlBuild: findBuild(builds, ['GATE-RELIABILITY-001', 'PERSONAL-WORKSPACE-BOUNDARY-001', 'DOCTRINE-PROPAGATION-003', 'DECISION-AUTO-EMIT-002', 'CEO-DASHBOARD-PATTERN-001'], 'foundation-control-layer-v1'),
    buildLogFoundation1100ReviewBuild: findBuild(builds, id(ids, 'FOUNDATION_REVIEW_SPRINT_CARD_IDS'), id(ids, 'FOUNDATION_REVIEW_SPRINT_CLOSEOUT_KEY')),
    buildLogPlainEnglishSweepBuild: findBuild(builds, [id(ids, 'PLAIN_ENGLISH_SWEEP_CARD_ID')], id(ids, 'PLAIN_ENGLISH_SWEEP_CLOSEOUT_KEY')),
    ...
    buildLogAgentOnboardingFeedbackSystemBuild: findBuild(builds, [id(ids, 'AGENT_ONBOARDING_FEEDBACK_SYSTEM_CARD_ID')], id(ids, 'AGENT_ONBOARDING_FEEDBACK_SYSTEM_CLOSEOUT_KEY')),
    buildLogAgentFeedbackSendBuild: closeoutEntry(id(ids, 'AGENT_FEEDBACK_SEND_CARD_ID'), id(ids, 'AGENT_FEEDBACK_SEND_CLOSEOUT_KEY')),
    buildLogAgentFeedbackAutoSendBuild: findBuild(builds, [id(ids, 'AGENT_FEEDBACK_AUTO_SEND_CARD_ID')], id(ids, 'AGENT_FEEDBACK_AUTO_SEND_CLOSEOUT_KEY')),
    ...
    buildLogAgentFeedbackCompanyEmailPolicyBuild: findBuild(builds, [id(ids, 'AGENT_FEEDBACK_COMPANY_EMAIL_POLICY_CARD_ID')], id(ids, 'AGENT_FEEDBACK_COMPANY_EMAIL_POLICY_CLOSEOUT_KEY')),
    buildLogAgentFeedbackSteveFullLoopTestBuild: findBuild(builds, [id(ids, 'AGENT_FEEDBACK_STEVE_FULL_LOOP_TEST_CARD_ID')], id(ids, 'AGENT_FEEDBACK_STEVE_FULL_LOOP_TEST_CLOSEOUT_KEY')),
    buildLogAgentFeedbackRealUserSubmitRepairBuild: findBuild(builds, [id(ids, 'AGENT_FEEDBACK_REAL_USER_SUBMIT_REPAIR_CARD_ID')], id(ids, 'AGENT_FEEDBACK_REAL_USER_SUBMIT_REPAIR_CLOSEOUT_KEY')),
    buildLogFoundationVerifyHealthRepairBuild: findBuild(builds, [id(ids, 'FOUNDATION_VERIFY_HEALTH_REPAIR_CARD_ID')], id(ids, 'FOUNDATION_VERIFY_HEALTH_REPAIR_CLOSEOUT_KEY')),
    buildLogAgentFeedbackProductionAutoSendDryRunBuild: findBuild(builds, [id(ids, 'AGENT_FEEDBACK_PRODUCTION_AUTOSEND_ENABLE_CARD_ID')], id(ids, 'AGENT_FEEDBACK_PRODUCTION_AUTOSEND_DRY_RUN_CLOSEOUT_KEY')),
    buildLogAgentFeedbackProductionAutoSendEnableBuild: findBuild(builds, [id(ids, 'AGENT_FEEDBACK_PRODUCTION_AUTOSEND_ENABLE_CARD_ID')], id(ids, 'AGENT_FEEDBACK_PRODUCTION_AUTOSEND_ENABLE_CLOSEOUT_KEY')),
    buildLogSalesGlsScoreboardBuild: findBuild(builds, ['SALES-GLS-SCOREBOARD-V1'], 'sales-gls-scoreboard-v1'),
    ...
    buildLogGateReliabilityRecurringBuild: findBuild(builds, ['GATE-RELIABILITY-002'], 'gate-reliability-recurring-transient-v1'),
    buildLogGateReliabilityDirectVerifierBuild: findBuild(builds, ['GATE-RELIABILITY-003'], 'gate-reliability-direct-verifier-deadlock-v1'),
  }
}
