...
export function sanitizeSalesListingAssignment(record) {
  ...
  return {
    ...
    glsSnooze: metadata.glsSnooze && typeof metadata.glsSnooze === 'object' ? {
      until: metadata.glsSnooze.until || '',
      reason: metadata.glsSnooze.reason || '',
      snoozedAt: metadata.glsSnooze.snoozedAt || '',
      snoozedBy: metadata.glsSnooze.snoozedBy || '',
    } : null,
    caseHistory: sanitizeSalesListingCaseHistory(metadata.caseHistory),
...
export function buildSalesListingAssignmentSummary(staleListings) {
  ...
  for (const listing of staleListings || []) {
    ...
    if (listing.salesLeaderAssignment?.actionPlanState === 'yes' || ['action_plan_created', 'action_plan_implemented'].includes(caseStatus)) {
      actionPlanCreated += 1
    }
...
export function buildSalesListingCaseSummary(cases) {
  ...
  const agentConnected = rows.filter(row => ['contacted_agent', 'action_plan_created', 'action_plan_implemented'].includes(row.caseStatus || '')).length
  ...
  return {
    ...
    agentConnected,
    actionPlanCreated: Math.max(actionPlanYes, rows.filter(row => ['action_plan_created', 'action_plan_implemented'].includes(row.caseStatus || '')).length),
    actionPlanYes,
    actionPlanNo,
    actionPlanUnknown,
