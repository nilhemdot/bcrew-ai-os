...
export const AGENT_ONBOARDING_NPS_LAUNCH_DATE = process.env.AGENT_ONBOARDING_NPS_LAUNCH_DATE || '2026-04-01'
...
const NPS_MILESTONES = [
  {
    ...
    dueDateField: 'Onboarding NPS 30 Due Date',
    lastSentField: 'Onboarding NPS 30 Last Sent',
  },
  {
    day: 60,
    label: '60',
    statusField: 'Onboarding NPS 60 Status',
    dueDateField: 'Onboarding NPS 60 Due Date',
    lastSentField: 'Onboarding NPS 60 Last Sent',
  },
  {
    day: 90,
    label: '90',
    statusField: 'Onboarding NPS 90 Status',
    dueDateField: 'Onboarding NPS 90 Due Date',
    lastSentField: 'Onboarding NPS 90 Last Sent',
  },
]
...
function buildAgentNpsItems(task, fields, today) {
  ...
  const launchDate = parseDate(AGENT_ONBOARDING_NPS_LAUNCH_DATE)
  if (launchDate && realStart < launchDate) return items
  ...
  NPS_MILESTONES.forEach(milestone => {
    ...
    const storedDue = parseDate(fields.get(milestone.dueDateField))
    const expectedDue = addDays(realStart, milestone.day)
    const due = storedDue || expectedDue
    if (today < due) return

    if (!storedDue) {
      items.push(makeRosterItem(
        'agent-roster-nps-due-date-missing:' + task.id + ':' + milestone.label,
        task.name || 'Roster agent',
        'Real Start Date exists, but ' + milestone.dueDateField + ' is blank. Set it to ' + formatDate(expectedDue) + ' so the day-' + milestone.label + ' onboarding feedback checkpoint is auditable.',
        {
    ...
    if (!currentStatus || normalizeStatus(currentStatus) === 'not started') {
      items.push(makeRosterItem(
        ...
        task.name || 'Roster agent',
        'Day-' + milestone.label + ' onboarding feedback is due. Send the private NPS request from someone outside onboarding, then set ' + milestone.statusField + ' to Requested and record the send date.',
        {
          subtitle: 'Day-' + milestone.label + ' onboarding feedback',
          owner: 'Onboarding feedback',
          rosterStatus: task.status && task.status.status ? task.status.status : '',
    ...
    const lastSent = parseDate(fields.get(milestone.lastSentField))
    if (isRequestedStatus(currentStatus) && (!lastSent || daysBetween(lastSent, today) >= REMINDER_DAYS)) {
      items.push(makeRosterItem(
        'agent-roster-nps-reminder:' + task.id + ':' + milestone.label,
        task.name || 'Roster agent',
        'Day-' + milestone.label + ' onboarding feedback is marked Requested but not Completed. Send a reminder if the agent has not responded; reminders should run every ' + REMINDER_DAYS + ' days until Completed, Skipped, or Blocked.',
        {
          subtitle: 'Day-' + milestone.label + ' onboarding feedback',
          owner: 'Onboarding feedback',
          rosterStatus: task.status && task.status.status ? task.status.status : '',
