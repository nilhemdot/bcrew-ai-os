import { getClickUpFieldMap, isClickUpSnapshotDegraded, normalizeClickUpKey } from './clickup.js'
...
const NPS_OVERDUE_GRACE_DAYS = 15
...
function buildAgentNpsItems(task, fields, today, expiredCheckpoints) {
  const items = []
  ...
  NPS_MILESTONES.forEach(milestone => {
    ...
    const overdueDays = daysBetween(expectedDue, today)
    if (overdueDays > NPS_OVERDUE_GRACE_DAYS) {
      expiredCheckpoints.push({
        task,
        milestone,
        dueDate: expectedDue,
        overdueDays,
    ...
    if (!currentStatus || normalizeStatus(currentStatus) === 'not started') {
      items.push(makeRosterItem(
        ...
        'Day-' + milestone.label + ' onboarding feedback is due and still inside the ' + NPS_OVERDUE_GRACE_DAYS + '-day send window. Use the private AIOS feedback link; when submitted, AIOS stores the response and writes the status, score, and feedback back to ClickUp.',
        {
...
export function buildAgentRosterReviewQueue(snapshot) {
  if (isClickUpSnapshotDegraded(snapshot)) {
    const sourceHealth = snapshot?.sourceHealth || null
    const items = [
      makeRosterItem(
        'agent-roster-source-degraded',
        'ClickUp Agent Roster source is degraded',
        'ClickUp is unavailable right now, so AIOS is serving a fail-soft source-health state instead of pretending the roster has zero issues. Retry after ClickUp recovers.',
        {
          subtitle: 'Source health',
          owner: 'Foundation source boundary',
          reviewStatus: 'Source Degraded',
          reviewAction: 'Retry Source',
          needsFixing: false,
          clickUpUrl: CLICKUP_AGENT_ROSTER_VIEW_URL,
        },
      ),
    ]
    return {
      ...
      listName: snapshot?.list?.name || 'Agent Roster',
      totalTrackedRows: 0,
  ...
  accountableTasks.forEach(task => {
    ...
    items.push(...buildAgentNpsItems(task, taskFields, today, expiredNpsCheckpoints))
  })

  if (expiredNpsCheckpoints.length) {
    const sample = expiredNpsCheckpoints
      .slice(0, MISSING_FIELD_SAMPLE_LIMIT)
      .map(checkpoint => (checkpoint.task.name || 'Roster agent') + ' day-' + checkpoint.milestone.label)
      .join(', ')
    items.push(makeRosterItem(
      'agent-roster-nps-window-expired',
      'Onboarding NPS feedback windows need cleanup',
      expiredNpsCheckpoints.length + ' onboarding feedback checkpoint' + (expiredNpsCheckpoints.length === 1 ? ' is' : 's are') + ' more than ' + NPS_OVERDUE_GRACE_DAYS + ' days past the 30/60/90 due date. Do not send the normal request; set the matching status to Skipped unless Steve explicitly asks for a catch-up survey. Sample: ' + sample + '.',
      {
        subtitle: 'Expired onboarding feedback window',
        owner: 'Onboarding feedback',
