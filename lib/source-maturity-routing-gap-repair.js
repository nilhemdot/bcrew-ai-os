...
export function buildSourceMaturityRoutingRepairRecords(candidate = {}, now = new Date().toISOString()) {
  const failures = []
  ...
  const naturalKey = `source-maturity-routing-gap-repair:${sourceId}:owner-review`
  ...
  const item = {
    synthesizedItemId,
    naturalKey,
    synthesisScopeKey: 'source-maturity-routing-gap-repair-v1',
    ...
    attributes: {
      ...
      themeKey: naturalKey,
      sourceMaturityRepair: SOURCE_MATURITY_ROUTING_GAP_REPAIR_CARD_ID,
    },
  ...
  const route = {
    ...
    metadata: {
      sourceMaturityRoutingGapRepair: true,
      sourceMaturityTargetSourceId: sourceId,
      proposedBy: 'codex-source-maturity-routing-gap-repair',
