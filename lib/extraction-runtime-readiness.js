...
export const EXTRACTION_RUNTIME_READINESS_CLOSEOUT_PATH = 'docs/handoffs/2026-05-17-extraction-runtime-readiness-closeout.md'
...
export function validateExtractionRuntimeQueueItem(item = {}, context = {}) {
  ...
  const status = normalizeText(item.queueStatus || item.status || 'queued')
  ...
  return {
    ...
    paid,
    active,
    runnable: findings.every(finding => finding.ok) && !blockedBySource && !paid && item.liveRunApproved !== true,
  }
}
