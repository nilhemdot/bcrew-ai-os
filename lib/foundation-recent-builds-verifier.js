...
const RECENT_BUILD_CLOSEOUT_EXPECTATIONS = [
  ...
  },
  {
    ...
    related: [
      ...
      { id: 'FOUNDATION-SURFACE-UPDATES-001', lanes: ['scoped', 'done'] },
    ],
    exactProof: ['npm run foundation:verify'],
    text: [
  ...
  },
  {
    ...
    related: [
      { id: 'DRIVE-CONTENT-001', lanes: ['scoped', 'done'] },
  ...
  },
  {
    ...
    related: [
      { id: 'RUNTIME-SUPERVISOR-001', lanes: ['scoped', 'done'] },
      { id: 'SYSTEM-010', lanes: ['scoped', 'done'] },
      { id: 'ACTION-REVIEW-APPLY-001', lanes: ['scoped', 'done'] },
    ],
    exactProof: ['npm run foundation:verify'],
    text: [
...
function relatedBacklogForExpectation(expectation = {}, backlogItems = []) {
  const backlogMap = new Map((backlogItems || []).map(item => [item.id, item]))
  return (expectation.related || []).map(related => {
    const item = backlogMap.get(related.id)
    if (item) return item
    return {
      id: related.id,
      title: null,
      scope: null,
      lane: (related.lanes || [])[0] || null,
      priority: null,
      owner: null,
      updatedAt: null,
    }
...
function normalizeCloseoutLikeBuild(build = {}, expectation = {}, backlogItems = []) {
  return {
    ...build,
    operatorCloseout: build.operatorCloseout !== false,
    closeoutKey: build.closeoutKey || build.key || null,
    backlogIds: normalizeList(build.backlogIds),
    mentionedBacklogIds: normalizeList(build.mentionedBacklogIds),
    whereItLives: normalizeList(build.whereItLives),
    proofCommands: normalizeList(build.proofCommands),
    knownLimits: normalizeList(build.knownLimits),
    relatedBacklog: Array.isArray(build.relatedBacklog) && build.relatedBacklog.length
      ? build.relatedBacklog
      : relatedBacklogForExpectation(expectation, backlogItems),
  }
}
...
function findBuild(builds = [], expectation = {}, {
  ...
  const build = entries.find(entry => {
    const normalized = normalizeCloseoutLikeBuild(entry, expectation, backlogItems)
    const hasCard = (normalized.backlogIds || []).includes(expectation.cardId)
    if (!hasCard) return false
    if (expectation.closeoutKey) return normalized.closeoutKey === expectation.closeoutKey
    return true
  }) || null
  return build ? normalizeCloseoutLikeBuild(build, expectation, backlogItems) : null
}
...
export function evaluateFoundationRecentBuildsVerifier(input = {}) {
  ...
  for (const expectation of RECENT_BUILD_CLOSEOUT_EXPECTATIONS) {
    const build = findBuild(foundationBuildLog.builds || [], expectation, {
      closeouts: foundationBuildLog.closeouts || [],
      foundationBuildCloseouts,
      backlogItems: backlogItems.length ? backlogItems : foundationHub.backlogItems || [],
    })
    addCheck(
      checks,
...
export function buildSyntheticFoundationRecentBuildsVerifierProof() {
  ...
  const expectationCloseouts = RECENT_BUILD_CLOSEOUT_EXPECTATIONS.map(expectation => {
    const build = makeSyntheticBuild(expectation)
    return {
      ...build,
      key: build.closeoutKey,
      closeoutKey: build.closeoutKey,
    }
  })
  const expectationBacklogIds = new Set(expectationCloseouts.flatMap(closeout => closeout.backlogIds || []))
  const firstExpectation = RECENT_BUILD_CLOSEOUT_EXPECTATIONS[0]
  const healthyInput = {
    schemaVersion: 2,
    foundationBuildCloseoutValidation: {
      schemaVersion: 2,
    ...
    foundationBuildCloseouts: [
      ...expectationCloseouts,
      ...allRequiredIds
        .filter(id => !expectationBacklogIds.has(id))
        .map(closeoutTemplate),
    ],
    foundationBuildLog: {
      schemaVersion: 2,
      summary: {
      ...
      builds: RECENT_BUILD_CLOSEOUT_EXPECTATIONS
        .slice(1)
        .map(makeSyntheticBuild),
    },
  ...
  const missingProof = evaluateFoundationRecentBuildsVerifier({
    ...healthyInput,
    foundationBuildCloseouts: healthyInput.foundationBuildCloseouts.map(closeout =>
      (closeout.backlogIds || []).includes(firstExpectation.cardId)
