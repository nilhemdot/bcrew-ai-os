...
export function evaluateFoundationDbSchemaSeedSplit({
  ...
  const sessionSource = String(foundationDbSessionSource || '')
  const rootExportsSessionInitDelegates = foundationDbSource.includes("} from './foundation-db-session.js'") &&
    foundationDbSource.includes('initFoundationDb') &&
    foundationDbSource.includes('bootstrapFoundationDb')
  const sessionImportsSchemaSeedStore = sessionSource.includes("import { createFoundationDbSchemaSeedStore } from './foundation-db-schema-seed-store.js'")
  const sessionOwnsInitDelegates = sessionSource.includes('const foundationDbSchemaSeedStore = createFoundationDbSchemaSeedStore({') &&
    sessionSource.includes('export const initFoundationDb = foundationDbSchemaSeedStore.initFoundationDb') &&
    sessionSource.includes('export const bootstrapFoundationDb = foundationDbSchemaSeedStore.bootstrapFoundationDb')

  addCheck(
    checks,
    foundationDbSource.includes("import { createFoundationDbSchemaSeedStore } from './foundation-db-schema-seed-store.js'") ||
      sessionImportsSchemaSeedStore,
    'foundation-db imports dedicated schema/seed initializer module',
    sessionImportsSchemaSeedStore ? 'session module import marker present' : 'root import marker present',
  )
  addCheck(
    checks,
    (
      ...
      (rootExportsSessionInitDelegates && sessionOwnsInitDelegates),
    'foundation-db keeps stable public init/bootstrap delegates',
    sessionOwnsInitDelegates ? 'facade exports session-owned delegates' : 'root delegate exports present',
  )
  addCheck(
    checks,
    !rootInitPattern,
...
export function buildFoundationDbSchemaSeedSplitDogfoodProof(input = {}) {
  ...
  const missingDelegate = evaluateFoundationDbSchemaSeedSplit({
    ...
    foundationDbSessionSource: String(input.foundationDbSessionSource || '').replace(
      'export const initFoundationDb = foundationDbSchemaSeedStore.initFoundationDb',
      'const initFoundationDb = foundationDbSchemaSeedStore.initFoundationDb',
    ),
  })
