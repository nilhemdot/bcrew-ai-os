...
export function createIntelligenceActionRouterStore({
  ...
  async function selectRouteableSynthesizedItems(client, { maxTier, limit }) {
    const result = await client.query(
      `
        SELECT *
        ...
        WHERE item.status != 'archived'
          ...
          AND COALESCE(item.attributes->>'legacySynthesisProtected', item.metadata->>'legacySynthesisProtected') != 'true'
          AND NOT EXISTS (
            SELECT 1
  ...
  async function proposeActionRoutes(input = {}, actor = 'system') {
    ...
    return withFoundationTransaction(async client => {
      const items = await selectRouteableSynthesizedItems(client, { maxTier, limit })
