...
function clampLimit(value, fallback = 20, max = 500) {
...
function formatMessageStatus(metadata = {}) {
  const messageCount = Number(metadata.messageCount || metadata.message_count || 0)
  if (!Number.isFinite(messageCount) || messageCount <= 0) return 'Thread reply status not captured'
  const replyCount = Math.max(0, messageCount - 1)
  return `${messageCount} message${messageCount === 1 ? '' : 's'} captured / ${replyCount} repl${replyCount === 1 ? 'y' : 'ies'} captured`
}
...
function priorityForItem(item = {}) {
  ...
  const evidenceStrength = String(item.attributes?.evidenceStrength || item.metadata?.evidenceStrength || '').trim()
  if (severity === 'high') return 'P0'
  if (severity === 'watch') return 'P1'
  if (evidenceStrength === 'strong' && item.attributes?.actionReadiness === 'route-ready') return 'P1'
  return 'P2'
}

function plainRouteTypeLabel(routeType) {
  if (routeType === 'needs_owner_decision') return 'owner decision'
  if (routeType === 'backlog_task') return 'backlog task'
  if (routeType === 'owner_action') return 'owner action'
  if (routeType === 'open_question') return 'open question'
  return String(routeType || 'route').replace(/_/g, ' ')
}

function routeReasonForItem({ routeType, item = {}, owner = '' } = {}) {
  const routeScope = item.attributes?.routeScope || item.metadata?.routeScope || 'operational'
  const evidenceStrength = item.attributes?.evidenceStrength || item.metadata?.evidenceStrength || 'early'
  const decisionQuestion = item.attributes?.decisionQuestion || ''
  if (routeType === 'needs_owner_decision') {
    return `Needs an owner before work starts: ${decisionQuestion || item.title || 'this signal has no clear accountable owner yet'}.`
  }
  if (routeType === 'contradiction') {
    return `Conflicting evidence needs human review before the system routes this further.`
  }
  if (routeType === 'open_question') {
    return `Needs a clear answer before execution: ${decisionQuestion || item.title || 'open question from source evidence'}.`
  }
  if (routeType === 'decision') {
    return `Decision needed: this ${routeScope} signal has ${evidenceStrength} proof and needs ${owner || 'the owner'} to approve the next move.`
  }
  if (['backlog_task', 'owner_action'].includes(routeType)) {
    return `Ready for ${plainRouteTypeLabel(routeType)} review: ${owner || 'the owner'} should confirm the next action using the attached source proof.`
  }
  if (routeType === 'ignore') return 'Marked as not useful for the current operating loop.'
  if (routeType === 'snooze') return 'Useful signal, but it belongs in a later review window.'
  return `Route selected for ${plainRouteTypeLabel(routeType)} review with source proof attached.`
}
...
function proposedPayloadForRoute({ routeId, routeType, item, owner }) {
  ...
  if (['backlog_task', 'owner_action'].includes(routeType)) {
    return {
      ...
      summary: shorten(item.summary || item.title, 1200),
      whyItMatters: 'A verified intelligence signal is ready for human review. The system is not applying it automatically.',
...
function buildSourceProofForRoute(route, { atomsById, candidatesByKey, artifactsById, factsById }) {
  const atomIds = uniqueText([...(route.evidenceRefs || []), ...(route.atomRefs || [])])
  const candidateKeys = uniqueText(route.candidateKeys || [])
  const artifactIds = uniqueText(route.artifactIds || [])
  const factRefs = uniqueText(route.factRefs || [])
  const facts = factRefs.map(id => factsById.get(id)).filter(Boolean)
  const items = []
  const usedKeys = new Set()

  for (const atomId of atomIds) {
    const atom = atomsById.get(atomId) || {}
    const candidate = atom.candidate_key ? candidatesByKey.get(atom.candidate_key) : null
    const artifactId = atom.artifact_id || candidate?.artifact_id || ''
    const artifact = artifactId ? artifactsById.get(artifactId) : null
    const contentPreview = artifact?.content_preview || ''
    const sourceKey = `${atomId}|${artifactId || candidate?.candidate_key || ''}`
    if (usedKeys.has(sourceKey)) continue
    usedKeys.add(sourceKey)
    const participants = Array.isArray(artifact?.participants) && artifact.participants.length
      ? artifact.participants
      : (
          Array.isArray(candidate?.metadata?.links?.participants)
            ? candidate.metadata.links.participants
            : []
        )
    const relatedFacts = facts.filter(fact =>
      fact.atom_id === atomId ||
      (candidate?.candidate_key && fact.candidate_key === candidate.candidate_key) ||
      (artifactId && fact.artifact_id === artifactId)
    )
    items.push({
      sourceId: atom.source_id || candidate?.source_id || artifact?.source_id || '',
      sourceType: artifact?.artifact_type || atom.metadata?.artifactType || candidate?.metadata?.contentSource || 'source',
      title: artifact?.title || candidate?.title || atom.title || 'Source evidence',
      sourceAccount: artifact?.source_account || '',
      sourceContainer: artifact?.source_container || '',
      sourceUrl: artifact?.source_url || atom.metadata?.sourceUrl || '',
      occurredAt: artifact?.artifact_created_at || artifact?.artifact_updated_at || candidate?.metadata?.artifactUpdatedAt || atom.updated_at || null,
      latestActivityAt: artifact?.artifact_updated_at || artifact?.updated_at || candidate?.updated_at || atom.updated_at || null,
      from: parseHeaderValue(contentPreview, 'From') || artifact?.source_account || '',
      to: parseHeaderValue(contentPreview, 'To') || '',
      participants,
      threadStatus: formatMessageStatus(artifact?.metadata || candidate?.metadata || {}),
      metadata: artifact?.metadata || candidate?.metadata || {},
      quote: atom.evidence_excerpt || candidate?.evidence_excerpt || '',
      context: shorten(candidate?.summary || atom.content || '', 420),
      includedBecause: shorten(atom.title || candidate?.title || route.routingReason || '', 180),
      factSummaries: relatedFacts.slice(0, 3).map(fact => shorten(fact.title || fact.claim || fact.fact_id, 160)),
      atomId,
      candidateKey: atom.candidate_key || candidate?.candidate_key || '',
      artifactId,
    })
  }

  for (const candidateKey of candidateKeys) {
    if (items.some(item => item.candidateKey === candidateKey)) continue
    const candidate = candidatesByKey.get(candidateKey)
    if (!candidate) continue
    const artifact = candidate.artifact_id ? artifactsById.get(candidate.artifact_id) : null
    const contentPreview = artifact?.content_preview || ''
    items.push({
      sourceId: candidate.source_id || artifact?.source_id || '',
      sourceType: artifact?.artifact_type || candidate.metadata?.contentSource || 'source',
      title: artifact?.title || candidate.title || 'Source evidence',
      sourceAccount: artifact?.source_account || '',
      sourceContainer: artifact?.source_container || '',
      sourceUrl: artifact?.source_url || '',
      occurredAt: artifact?.artifact_created_at || artifact?.artifact_updated_at || candidate.metadata?.artifactUpdatedAt || candidate.updated_at || null,
      latestActivityAt: artifact?.artifact_updated_at || artifact?.updated_at || candidate.updated_at || null,
      from: parseHeaderValue(contentPreview, 'From') || artifact?.source_account || '',
      to: parseHeaderValue(contentPreview, 'To') || '',
      participants: Array.isArray(artifact?.participants) ? artifact.participants : [],
      threadStatus: formatMessageStatus(artifact?.metadata || candidate.metadata || {}),
      metadata: artifact?.metadata || candidate.metadata || {},
      quote: candidate.evidence_excerpt || '',
      context: shorten(candidate.summary || '', 420),
      includedBecause: shorten(candidate.title || route.routingReason || '', 180),
      factSummaries: [],
      atomId: '',
      candidateKey,
  ...
  const enriched = enrichSourceProofItemsWithThreadContext(items)
  const threadContextSummary = buildThreadContextSummary(enriched.items)
  return {
    summary: `${items.length} source evidence item${items.length === 1 ? '' : 's'} rendered for meeting review; ${threadContextSummary.weakFlagCount} weak-proof flag${threadContextSummary.weakFlagCount === 1 ? '' : 's'} surfaced`,
    ...
    technicalRefs: {
      ...
      factRefs,
      atomRefs: atomIds,
      chunkRefs: uniqueText(route.evidenceChunkRefs || []),
      candidateKeys,
      artifactIds,
    },
  }
...
function buildRouteForItem(item = {}) {
  ...
  const routingReason = routeReasonForItem({ routeType, item, owner })
  return {
    routeId,
    routeType,
    destinationTable,
...
export function buildActionRouterOperatorDogfood() {
  const item = {
    synthesizedItemId: 'synthesized-item:dogfood-route',
    naturalKey: 'dogfood-route',
    itemType: 'action_candidate',
    status: 'ready_for_action_router',
    title: 'Agent handoff checkpoint needs build review',
    summary: 'Two source-backed signals show that long-running agent work needs a checkpointed handoff layer before scaling.',
    suggestedOwner: 'Foundation',
    ownerConfidence: 'high',
    ownerAction: 'Confirm Foundation owns this and decide whether it becomes the next scoped build card.',
    sourceIds: ['SRC-DOGFOOD-INTEL-001'],
    factRefs: ['dogfood-fact-1', 'dogfood-fact-2'],
    evidenceRefs: ['dogfood-evidence-1', 'dogfood-evidence-2'],
    evidenceChunkRefs: ['dogfood-chunk-1', 'dogfood-chunk-2'],
    atomRefs: ['dogfood-atom-1', 'dogfood-atom-2'],
    candidateKeys: ['dogfood-candidate-1'],
    artifactIds: ['dogfood-artifact-1'],
    sensitivity: 'neutral',
    minTier: 1,
    attributes: {
      routeScope: 'operational',
      evidenceStrength: 'solid',
      actionReadiness: 'route-ready',
      decisionQuestion: 'Should Foundation build the checkpointed handoff layer now?',
    },
    metadata: {
      routeScope: 'operational',
      evidenceStrength: 'solid',
  ...
  const genericReasonPattern = /Route type .+ selected from synthesized item type|Synthesized item did not have a clear owner/i
  return {
    ok: !genericReasonPattern.test(route.routingReason) &&
      /attached source proof|Decision needed|human review|Ready for/i.test(route.routingReason) &&
      route.proposedPayload?.whyItMatters !== 'Routed from source-backed synthesized intelligence with fact, atom, and retrieval-chunk provenance.',
    route,
    checks: [
      {
        ok: !genericReasonPattern.test(route.routingReason),
        check: 'router reason is not generic internal language',
        detail: route.routingReason,
      },
      {
        ok: /attached source proof|Decision needed|human review|Ready for/i.test(route.routingReason),
        check: 'router reason explains the human review path',
        detail: route.routingReason,
      },
      {
        ok: route.proposedPayload?.whyItMatters !== 'Routed from source-backed synthesized intelligence with fact, atom, and retrieval-chunk provenance.',
        check: 'router payload avoids internal provenance jargon',
        detail: route.proposedPayload?.whyItMatters || '',
      },
    ],
  }
}
...
export function createIntelligenceActionRouterStore({
    ...
    const result = await client.query(
      `
        ...
        WHERE item.status != 'archived'
          ...
          AND item.metadata->'synthesisVerification'->>'status' = 'verified'
          AND item.metadata->'synthesisVerification'->>'supportLevel' = 'verified'
          AND item.metadata->'synthesisVerification'->>'verificationVersion' = 'synthesis-verify-v1'
    ...
    const result = await client.query(
      ...
      [
        ...
        JSON.stringify({
          ...
          [SYNTHESIS_VERIFICATION_METADATA_KEY]: item.metadata?.[SYNTHESIS_VERIFICATION_METADATA_KEY] || null,
        }),
      ]
    )
  ...
  async function approveActionRoute(routeId, input = {}, actor = 'system') {
    return withFoundationTransaction(async client => {
      ...
      requireVerifiedSynthesisRecord(mapRouteRow(route))
  ...
  async function rerouteActionRoute(routeId, input = {}, actor = 'system') {
    return withFoundationTransaction(async client => {
      ...
      if (routeType === 'snooze') {
        const snoozeUntil = String(input.snoozeUntil || input.snooze_until || '').trim()
        const snoozeDuration = String(input.snoozeDuration || input.snooze_duration || '').trim()
        if (snoozeUntil) proposedPayload.snoozeUntil = snoozeUntil
        if (snoozeDuration) proposedPayload.snoozeDuration = snoozeDuration
      }
      ...
      const updatedResult = await client.query(
        ...
        [
          ...
          JSON.stringify({
            ...
            snoozeDuration: input.snoozeDuration || input.snooze_duration || '',
            snoozeUntil: input.snoozeUntil || input.snooze_until || '',
        ...
        await client.query(
          ...
          [
            ...
            JSON.stringify({
              ...
              snoozeDuration: payload.snoozeDuration || route.metadata?.snoozeDuration || '',
              snoozeUntil: payload.snoozeUntil || route.metadata?.snoozeUntil || '',
            }),
          ]
        )
        appliedPayload = {
          synthesizedItemId: destinationRecordId,
          routingStatus: payload.routingStatus || 'ignored',
          snoozeDuration: payload.snoozeDuration || route.metadata?.snoozeDuration || '',
          snoozeUntil: payload.snoozeUntil || route.metadata?.snoozeUntil || '',
        }
      }
  ...
  async function enrichRoutesWithSourceProof(routes = []) {
    if (!routes.length) return routes
    const atomIds = uniqueText(routes.flatMap(route => [...(route.evidenceRefs || []), ...(route.atomRefs || [])]))
    const candidateKeysFromRoutes = uniqueText(routes.flatMap(route => route.candidateKeys || []))
    const artifactIdsFromRoutes = uniqueText(routes.flatMap(route => route.artifactIds || []))
    const factIds = uniqueText(routes.flatMap(route => route.factRefs || []))

    const atomsResult = atomIds.length
      ? await pool.query(
          `
            SELECT atom_id, title, content, source_id, artifact_id, candidate_key,
                   evidence_excerpt, metadata, created_at, updated_at
      ...
      : { rows: [] }
    const atomsById = new Map(atomsResult.rows.map(row => [row.atom_id, row]))
    const candidateKeys = uniqueText([
      ...candidateKeysFromRoutes,
      ...atomsResult.rows.map(row => row.candidate_key).filter(Boolean),
    ])
    const candidatesResult = candidateKeys.length
      ? await pool.query(
          `
            SELECT candidate_key, artifact_id, source_id, title, summary,
                   evidence_excerpt, metadata, created_at, updated_at
      ...
      : { rows: [] }
    const candidatesByKey = new Map(candidatesResult.rows.map(row => [row.candidate_key, row]))
    const artifactIds = uniqueText([
      ...artifactIdsFromRoutes,
      ...atomsResult.rows.map(row => row.artifact_id).filter(Boolean),
      ...candidatesResult.rows.map(row => row.artifact_id).filter(Boolean),
    ])
    const artifactsResult = artifactIds.length
      ? await pool.query(
          `
            SELECT artifact_id, source_id, artifact_type, external_id, title,
                   source_account, source_container, source_url, participants,
                   substring(content_text from 1 for 2400) AS content_preview,
                   artifact_created_at, artifact_updated_at, metadata,
      ...
      : { rows: [] }
    const artifactsById = new Map(artifactsResult.rows.map(row => [row.artifact_id, row]))
    const factsResult = factIds.length
      ? await pool.query(
          `
            SELECT fact_id, fact_type, title, claim, source_id, source_ref,
                   evidence_id, artifact_id, atom_id, candidate_key, source_url,
                   as_of, metadata, created_at, updated_at
      ...
      : { rows: [] }
    const factsById = new Map(factsResult.rows.map(row => [row.fact_id, row]))

    return routes.map(route => ({
      ...route,
      sourceProof: buildSourceProofForRoute(route, {
        atomsById,
        candidatesByKey,
        artifactsById,
        factsById,
  ...
  async function getActionRouterSnapshot({ limit = 20 } = {}) {
    const normalizedLimit = clampLimit(limit, 20, 500)
    const [
      summary,
      latestRun,
      latestProofRun,
      recentRoutes,
      strategyRecentRoutes,
      routeTypes,
      destinations,
      referenceIntegrity,
      ...
      pool.query(`
        SELECT
          ...
          COUNT(*) FILTER (
            WHERE metadata->'synthesisVerification'->>'status' = 'verified'
              AND metadata->'synthesisVerification'->>'supportLevel' = 'verified'
              AND metadata->'synthesisVerification'->>'verificationVersion' = 'synthesis-verify-v1'
          )::int AS verified_synthesis_routes,
          COUNT(*) FILTER (
            WHERE destination_table IN ('decisions', 'backlog_items', 'open_questions')
              AND approval_status IN ('pending', 'approved', 'applied')
              AND COALESCE(metadata->'synthesisVerification'->>'status', '') != 'verified'
          )::int AS unverified_decision_grade_routes,
    ...
    const [mappedRecentRoutes, mappedStrategyRecentRoutes] = await Promise.all([
      enrichRoutesWithSourceProof(recentRoutes.rows.map(mapRouteRow)),
      enrichRoutesWithSourceProof(strategyRecentRoutes.rows.map(mapRouteRow)),
    ])
    ...
    return {
      ...
      verifiedSynthesisRoutes: Number(summary.rows[0]?.verified_synthesis_routes || 0),
      unverifiedDecisionGradeRoutes: Number(summary.rows[0]?.unverified_decision_grade_routes || 0),
      ...
      recentRoutes: mappedRecentRoutes,
      strategyRecentRoutes: mappedStrategyRecentRoutes,
