...
export const AGENT_FEEDBACK_RESULT_FIELD_BY_MILESTONE = RESULT_FIELD_BY_MILESTONE
...
export async function buildAgentFeedbackRequestedWritebackPlan(input = {}) {
  ...
  const requestedOption = findDropdownOption(statusField, 'Requested')
  if (!requestedOption) throw new Error(config.status + ' is missing the Requested option.')
  ...
  return {
    ...
    sequence: 'Write Requested only after Gmail send succeeds.',
  }
}

export async function markAgentFeedbackRequestedInClickUp(input = {}) {
  const milestoneDay = Number(input.milestoneDay)
  const config = RESULT_FIELD_BY_MILESTONE[milestoneDay]
  if (!config) throw new Error('Invalid feedback milestone.')
  ...
  const fieldsByName = fieldMap(fields)
  const statusField = findField(fieldsByName, config.status)
  if (!statusField) throw new Error('Missing ClickUp Agent Roster feedback field: ' + config.status)

  const requestedOption = findDropdownOption(statusField, 'Requested')
  if (!requestedOption) throw new Error(config.status + ' is missing the Requested option.')

  await setClickUpTaskCustomFieldValue(taskId, statusField.id, requestedOption.id)
