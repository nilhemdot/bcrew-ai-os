...
const MISSIVE_MIN_REQUEST_GAP_MS = 500;
const MISSIVE_RETRYABLE_STATUS_CODES = new Set([429, 500, 502, 503, 504]);
const MISSIVE_MAX_ATTEMPTS = 7;
...
async function missiveRequest(pathname, params = {}) {
  ...
  try {
    for (let attempt = 1; attempt <= MISSIVE_MAX_ATTEMPTS; attempt += 1) {
      ...
      if (!retryable || attempt === MISSIVE_MAX_ATTEMPTS) {
      ...
      const retryAfterMs = Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0
        ...
        : 0;
      const backoffMs = Math.max(retryAfterMs, Math.min(10_000, 1000 * attempt * attempt));
