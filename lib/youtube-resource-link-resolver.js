...
export function classifyYoutubeResourceLink(rawLink = {}) {
  ...
  if (/calendly|calendar|booking|book-a-call|checkout-form|payment-form|leadmagnet/.test(lower)) {
    return {
  ...
  if (/skool|circle|discord|member|membership|classroom|myicor|login|sign-in|signin/.test(lower)) {
    return {
  ...
  if (hasSafeHost(host) || isHttpPublicUrl(url)) {
    return {
...
export function buildScoperResourceLinkPacket({
  ...
  return {
    sourceLabel,
    unresolvedPublicResourceLinks: list(unresolvedPublicLinks).map(link => link.url).filter(Boolean),
    approvalRequiredResourceLinks: list(blockedLinks).map(link => ({
      url: link.url,
...
export function buildYoutubeResourceLinkResolverDogfoodProof() {
  const rawLinks = [
    ...
    { url: 'https://github.com/mattpocock/course-video-manager', sourceReportArtifactId: 'report:dogfood' },
    { url: 'https://docs.browserbase.com/introduction', sourceReportArtifactId: 'report:dogfood' },
    { url: 'https://www.aihero.dev/newsletter', sourceReportArtifactId: 'report:dogfood' },
    ...
    const cases = [
      {
        ...
        ok: snapshot.resolvedLinks.length === 4 && snapshot.resolvedLinks.every(link => link.resolutionStatus === 'resolved_public_metadata'),
      },
      {
        name: 'github_repo_with_course_in_name_resolves',
        ok: snapshot.resolvedLinks.some(link => link.url === 'https://github.com/mattpocock/course-video-manager' && link.approvalRequired === false),
      },
      {
        name: 'public_aihero_newsletter_page_resolves_without_form_submit',
        ok: snapshot.resolvedLinks.some(link => link.url === 'https://www.aihero.dev/newsletter' && link.approvalRequired === false),
      },
      {
        name: 'duplicate_public_repo_collapses',
        ok: snapshot.counts.totalLinks === 9 && snapshot.counts.duplicates === 1,
      },
      {
