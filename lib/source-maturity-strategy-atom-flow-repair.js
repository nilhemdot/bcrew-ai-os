...
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_CARD_ID = 'SOURCE-MATURITY-STRATEGY-ATOM-FLOW-REPAIR-001'
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY = 'source-maturity-strategy-atom-flow-repair-v1'
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_PLAN_PATH = 'docs/process/source-maturity-strategy-atom-flow-repair-001-plan.md'
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_APPROVAL_PATH = 'docs/process/approvals/SOURCE-MATURITY-STRATEGY-ATOM-FLOW-REPAIR-001.json'
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-29-hot-doc-refresh/2026-05-18-source-maturity-strategy-atom-flow-repair-closeout.md'
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_SCRIPT_PATH = 'scripts/process-source-maturity-strategy-atom-flow-repair-check.mjs'
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_SPRINT_ID = 'source-maturity-strategy-atom-flow-repair-2026-05-18'
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID = 'SRC-STRATEGY-001'
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_FACT_ID = 'fact:0a3bea1f66c3fbdb4b4ee08f'

export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES = [
...
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_PROOF_COMMANDS = [
...
export const SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_CHANGED_FILES = [
...
function targetFact(facts = []) {
  return list(facts).find(fact => (fact.factId || fact.fact_id) === SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_FACT_ID) || null
}

export function selectStrategyAtomFlowRepairCandidate({
  beforeRow = {},
  sourceFacts = [],
  sourceId = SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
} = {}) {
  const failures = []
  ...
  const factId = fact?.factId || fact?.fact_id || SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_FACT_ID
...
export function buildStrategyAtomFromFact(candidate = {}, now = new Date().toISOString()) {
  const sourceId = candidate.sourceId || SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  ...
  const naturalKey = `source-maturity-strategy-atom-flow-repair:${sourceId}:${factId}`
  ...
  const atom = {
    atomId,
    title: `${sourceId} strategy source truth`,
    ...
    evidenceExcerpt,
    derivedClaim: claim,
    topicRefs: ['source-maturity', 'strategy', 'strategy-source-truth', 'source-backed-strategy-truth'],
    ...
    acceptedAt: now,
    acceptedBy: 'codex-source-maturity-strategy-atom-flow-repair',
    reviewNote: 'Promoted from an existing governed Strategy source fact only; no live extraction, provider call, strategy doc write, or external write occurred.',
    tags: ['source-maturity', 'atom-flow-repair', 'strategy', 'no-live-extraction'],
    metadata: {
...
export function evaluateStrategyAtomFlowRepair({
  beforeRow = {},
  afterRow = {},
  candidate = {},
  atomBuild = {},
  ...
  return {
    ...
    failures,
    summary: {
      sourceId: candidate.sourceId || beforeRow.sourceId || afterRow.sourceId || SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
...
export function buildSyntheticStrategyAtomFlowRepairProof() {
  const fact = {
    factId: SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_FACT_ID,
    sourceId: SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
    ...
    metadata: { status: 'signed_off_current_reality', evidenceRefs: ['intelligence_synthesis_facts:fact:0a3bea1f66c3fbdb4b4ee08f'] },
    minTier: 1,
    sensitivity: 'neutral',
    status: 'active',
  }
  ...
  const candidate = selectStrategyAtomFlowRepairCandidate({ beforeRow, sourceFacts: [fact] })
  const atomBuild = buildStrategyAtomFromFact(candidate, '2026-05-18T06:15:00.000Z')
  const evaluation = evaluateStrategyAtomFlowRepair({
    beforeRow,
    afterRow,
    candidate,
    atomBuild,
  ...
  const missingFact = selectStrategyAtomFlowRepairCandidate({ beforeRow, sourceFacts: [] })
  const tierBlocked = selectStrategyAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, minTier: 4 }] })
  return {
    ok: candidate.ok &&
      atomBuild.ok &&
      evaluation.ok &&
...
export function renderStrategyAtomFlowRepairCloseout(snapshot = {}) {
  const summary = snapshot.summary || {}
  const lines = []
  lines.push('# Source Maturity Strategy Atom Flow Repair Closeout')
  lines.push('')
  lines.push(`Card: \`${SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_CARD_ID}\``)
  lines.push(`Closeout key: \`${SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\``)
  lines.push('')
  lines.push('## What Shipped')
  lines.push('')
  lines.push(`- Promoted one source-backed atom for \`${summary.sourceId || SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID}\` from an existing Strategy source fact.`)
  lines.push(`- Saved atom: \`${summary.atomId || 'pending atom'}\`.`)
  lines.push(`- Source fact evidence: \`${summary.sourceFactId || SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_TARGET_FACT_ID}\`.`)
  lines.push('- The repair uses internal source-fact evidence only and does not run extraction, write strategy docs, call providers, generate recommendations, or write externally.')
  ...
  lines.push('')
  for (const boundary of SOURCE_MATURITY_STRATEGY_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES) lines.push(`- ${boundary}`)
  lines.push('')
  lines.push('## Next')
  lines.push('')
