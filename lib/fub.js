...
const FUB_SYSTEM_NAME = process.env.FUB_SYSTEM_NAME || ''
...
function getFubSystemHeaders() {
  if (!FUB_SYSTEM_NAME || !FUB_SYSTEM_KEY) return {}
  return {
...
export async function listFubLeadSources(contextKey, options) {
  const context = resolveFubContext(contextKey)
  const pageLimit = Math.min(100, Math.max(1, Number(options && options.pageLimit) || 100))
  const maxPages = Math.min(1000, Math.max(1, Number(options && options.maxPages) || 250))
  const counts = new Map()
  let nextEndpoint = '/people?limit=' + pageLimit + '&fields=id,source'
  let pagesScanned = 0
  let peopleScanned = 0

  while (nextEndpoint && pagesScanned < maxPages) {
    const payload = await fubJsonFetch(context.key, nextEndpoint)
    const people = Array.isArray(payload.people) ? payload.people : []
    pagesScanned += 1
    peopleScanned += people.length

    for (const person of people) {
      const source = String((person && person.source) || '').trim()
      if (!source) continue
      counts.set(source, (counts.get(source) || 0) + 1)
    }

    const nextLink = payload && payload._metadata && payload._metadata.nextLink ? String(payload._metadata.nextLink).trim() : ''
    const nextToken = payload && payload._metadata && payload._metadata.next ? String(payload._metadata.next).trim() : ''
    ...
    if (nextToken) {
      ...
      nextEndpoint = '/people?limit=' + pageLimit + '&fields=id,source&next=' + encodeURIComponent(nextToken)
      if (separator === '&') {
  ...
  return {
    ...
    stats: {
      ...
      peopleScanned,
      pagesScanned,
      truncated: Boolean(nextEndpoint),
    },
  }
}
