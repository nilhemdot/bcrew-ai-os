...
export function buildMarkGodModeSmallBatchSnapshot({
  ...
  const checks = []
  addFinding(checks, mergedResults.length >= 3 && mergedResults.length <= MARK_KASHEF_GOD_MODE_SMALL_BATCH_MAX_SIZE, 'batch size is guarded at 3-5 videos', `${mergedResults.length}`)
...
export function verifyMarkGodModeSmallBatchPersistedProof({ snapshot = {}, report = null, atoms = [], hits = [] } = {}) {
  ...
  addFinding(checks, list(report?.metadata?.videoIds).length >= 3 && list(report?.metadata?.videoIds).length <= MARK_KASHEF_GOD_MODE_SMALL_BATCH_MAX_SIZE, 'report records guarded video count', `${list(report?.metadata?.videoIds).length}`)
