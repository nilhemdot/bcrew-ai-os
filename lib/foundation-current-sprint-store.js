...
export function evaluateFoundationCurrentSprintStoreSplit({
  ...
  const checks = []
  const wiringSource = [foundationDbSource, foundationBacklogSprintSource].filter(Boolean).join('\n')
  addEvaluationCheck(
    checks,
  ...
  addEvaluationCheck(
    checks,
    wiringSource.includes('./foundation-current-sprint-store.js') &&
      wiringSource.includes('createFoundationCurrentSprintStore') &&
      wiringSource.includes('foundationCurrentSprintStore') &&
      (
        foundationDbSource.includes('./foundation-current-sprint-store.js') ||
        (
          foundationDbSource.includes("} from './foundation-backlog-sprint-db.js'") &&
          foundationDbSource.includes('getActiveFoundationCurrentSprint') &&
          foundationDbSource.includes('upsertFoundationCurrentSprintOverlay')
        )
      ),
    'foundation-db wires through the dedicated store module',
    foundationBacklogSprintSource.includes('./foundation-current-sprint-store.js')
      ? 'facade exports backlog/sprint-owned store delegates'
