...
export function registerStrategySharedCommsRoutes(app, deps = {}) {
  ...
  function buildStrategyHubV2Payload({ goalTruth, operatingTruth, actionRouter, retrieval, planningEvidence = {}, businessAtoms = {}, sourceTruthStatus = 'live', fallback = null }) {
  ...
  app.get('/api/strategic-execution/v2', requireAdminToken, async (req, res) => {
    try {
      ...
      const payload = buildStrategyHubV2Payload({ goalTruth, operatingTruth, actionRouter, retrieval, planningEvidence, businessAtoms })
      try {
