...
const MOVED_ROUTE_MARKERS = [
  ...
  "app.get('/api/strategic-execution/quarter-context'",
  "app.post('/api/strategic-execution/quarter-context'",
  ...
  '/api/foundation/action-route-review-inbox/:routeId/workflow',
...
export function registerStrategySharedCommsRoutes(app, deps = {}) {
  ...
  const getBusinessAtomDashboardSnapshot = requireDependency(deps, 'getBusinessAtomDashboardSnapshot')
  ...
  const recordActionRouteCuration = requireDependency(deps, 'recordActionRouteCuration')
  ...
  function uniqueRoutesById(routes = []) {
    const seen = new Set()
  ...
  function buildStrategyHubV2Payload({ goalTruth, operatingTruth, actionRouter, retrieval, planningEvidence = {}, businessAtoms = {}, quarterContext = {}, sourceTruthStatus = 'live', fallback = null }) {
    const allRoutes = uniqueRoutesById([
      ...(Array.isArray(actionRouter.recentRoutes) ? actionRouter.recentRoutes : []),
      ...(Array.isArray(actionRouter.strategyRecentRoutes) ? actionRouter.strategyRecentRoutes : []),
    ])
    ...
    const governanceAccountability = buildGovernanceAccountabilitySnapshot({
      goalTruth,
      operatingTruth,
      actionRouter: strategyActionRouter,
      businessAtoms,
  ...
  function buildFoundationActionReviewSnapshot(actionRouter) {
    ...
    return {
      ...
      summary: {
        ...
        pendingRoutes: Number.isFinite(Number(actionRouter?.pendingRoutes)) ? Number(actionRouter.pendingRoutes) : pendingRoutes.length,
        approvedRoutes: Number.isFinite(Number(actionRouter?.approvedRoutes)) ? Number(actionRouter.approvedRoutes) : approvedRoutes.length,
        appliedRoutes: Number.isFinite(Number(actionRouter?.appliedRoutes)) ? Number(actionRouter.appliedRoutes) : appliedRoutes.length,
        rejectedRoutes: Number.isFinite(Number(actionRouter?.rejectedRoutes)) ? Number(actionRouter.rejectedRoutes) : rejectedRoutes.length,
  ...
  app.get('/api/strategic-execution/quarter-context', requireAdminToken, async (_req, res) => {
    try {
      const quarterContext = await getStrategyQuarterSnapshot()
      cacheHeadersNoStore(res)
      res.json(quarterContext)
    } catch (error) {
      sendApiError(
        res,
        500,
        'strategy_quarter_context_failed',
        error instanceof Error ? error.message : 'Failed to load Strategy Quarter context.'
      )
    }
  })

  app.post('/api/strategic-execution/quarter-context', requireAdminToken, async (req, res) => {
    try {
      const allowedKeys = ['quarterKey', 'theme', 'criticalNumber', 'planningStatus', 'ownerNote']
      const unknownFields = getAllowedBodyKeys(req.body || {}, allowedKeys)
      if (unknownFields.length) {
        sendApiError(res, 400, 'invalid_strategy_quarter_context_body', 'Unknown Strategy Quarter context fields.', { unknownFields })
        return
      }
      const result = await updateStrategyQuarterContext(req.body || {}, getRequestActor(req))
      cacheHeadersNoStore(res)
      res.json(result)
    } catch (error) {
      sendApiError(
        res,
        500,
        'strategy_quarter_context_update_failed',
        error instanceof Error ? error.message : 'Failed to update Strategy Quarter context.'
      )
    }
  })
  ...
  app.get('/api/strategic-execution/v2', requireAdminToken, async (req, res) => {
    try {
      const [goalTruth, operatingTruth, actionRouter, retrieval, planningEvidence, businessAtoms, quarterContext] = await Promise.all([
      ...
      const payload = buildStrategyHubV2Payload({ goalTruth, operatingTruth, actionRouter, retrieval, planningEvidence, businessAtoms, quarterContext })
      try {
  ...
  app.post(ACTION_ROUTE_PROMOTION_WORKFLOW_API_PATH, requireAdminToken, async (req, res) => {
    try {
      ...
      const validation = validateActionRoutePromotionWorkflowRequest({
        route,
        input: req.body || {},
        backlogItems: foundationSnapshot.backlogItems || [],
      })
      if (!validation.ok) {
        sendApiError(res, 400, 'action_route_workflow_invalid', 'Action Route workflow request is not valid.', { failures: validation.failures })
        return
      }

      const { normalized } = validation
      const reviewedBy = normalized.reviewedBy || actor
      let destinationRecordId = route.destinationRecordId || null
      ...
      if (normalized.action === 'assign_owner') {
        route = await rerouteActionRoute(routeId, {
          ...
          note: normalized.note || `Foundation Review Inbox assigned owner ${normalized.owner}.`,
        }, actor)
      } else if (normalized.action === 'confirm_decision' || normalized.action === 'promote_to_backlog') {
        if (route.approvalStatus === 'pending') {
          route = await approveActionRoute(routeId, { approvedBy: reviewedBy, approvalNote: normalized.note }, actor)
        }
        if (route.approvalStatus === 'approved') {
          route = await applyApprovedActionRoute(routeId, { applyNote: normalized.note }, actor)
          destinationRecordId = route.destinationRecordId || destinationRecordId
        }
      } else if (normalized.action === 'snooze') {
        route = await rerouteActionRoute(routeId, {
          routeType: 'snooze',
          snoozeDuration: normalized.snoozeDuration || '1w',
          snoozeUntil: normalized.snoozeUntil,
          note: normalized.note || 'Foundation Review Inbox snoozed this action route.',
        }, actor)
        route = await approveActionRoute(routeId, { approvedBy: reviewedBy, approvalNote: normalized.note }, actor)
        route = await applyApprovedActionRoute(routeId, { applyNote: normalized.note }, actor)
        destinationRecordId = route.destinationRecordId || destinationRecordId
      } else if (normalized.action === 'answer_question') {
        route = await rejectActionRoute(routeId, {
          rejectedBy: reviewedBy,
          rejectionNote: `Answered in Foundation Review Inbox: ${normalized.note}`,
        }, actor)
      } else if (normalized.action === 'duplicate' || normalized.action === 'link_existing_card') {
        route = await rejectActionRoute(routeId, {
          rejectedBy: reviewedBy,
          rejectionNote: `${normalized.action === 'duplicate' ? 'Duplicate of' : 'Linked to'} ${normalized.targetCardId}. ${normalized.note}`.trim(),
        }, actor)
      } else if (normalized.action === 'reject') {
        route = await rejectActionRoute(routeId, {
          rejectedBy: reviewedBy,
          rejectionNote: normalized.note,
        }, actor)
      }

      const workflowMetadata = buildActionRoutePromotionWorkflowMetadata({
        route,
        validation,
        result: { route, destinationRecordId },
        actor,
      })
      await recordActionRouteCuration(routeId, {
        closeoutKey: ACTION_ROUTE_PROMOTION_WORKFLOW_CLOSEOUT_KEY,
        disposition: normalized.action,
        recommendation: normalized.note || normalized.targetCardId || normalized.owner || normalized.snoozeDuration,
        reviewedOnly: !validation.internalSideEffect,
        foundationHousekeeping: true,
        businessSideEffect: false,
        applyAllowedWithoutSteve: validation.internalSideEffect,
        externalSideEffectAllowed: false,
        reviewedBy,
        promotionWorkflow: workflowMetadata,
      }, actor)

      route = await getActionRoute(routeId)
      const [actionRouter, nextFoundationSnapshot] = await Promise.all([
      ...
      res.json({
        ok: true,
        route,
        actionRouteReviewInbox: buildActionRouteReviewInboxSnapshot({
          actionRouter,
          backlogItems: nextFoundationSnapshot.backlogItems || [],
        }),
      })
    } catch (error) {
      sendApiError(
        ...
        error instanceof Error ? error.message : 'Failed to apply Action Route Review Inbox workflow.'
      )
    }
  })
