...
export function buildMeetingArtifactExternalId(meetingKey, artifactType, fallbackExternalId = '') {
  const normalizedKey = normalizeMeetingArtifactKey(meetingKey);
  const normalizedType = String(artifactType || '')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_');

  if (!normalizedKey) return String(fallbackExternalId || '').trim();
  return `meeting:${normalizedKey}:${normalizedType || 'artifact'}`;
}
