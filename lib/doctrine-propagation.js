...
const TIER_TWO_SURFACES = [
  {
    ...
    reviewPhrases: ['helpful', 'resourceful'],
  },
  {
    path: 'docs/users/steve.md',
    label: 'docs/users/steve.md',
    purpose: 'shared Steve-facing profile',
    reviewPhrases: ['system-facing preferences', 'shared Foundation operating expectations'],
  },
]

const PRIVATE_MEMORY_FILES = [
  'MEMORY.md',
  'memory/feedback_always_next.md',
...
async function buildTierTwoReviewFindings({ repoRoot = process.cwd() } = {}) {
  ...
  for (const surface of TIER_TWO_SURFACES) {
    ...
    const missingPhrases = surface.reviewPhrases.filter(phrase => !source.includes(phrase))
    if (!missingPhrases.length) continue
    findings.push({
      severity: 'warning',
      ...
      recommendedAction: `Review ${surface.label}; v1 does not auto-edit tier-two surfaces.`,
    })
  }
  return findings
}
...
async function buildPrivateMemoryStats({ repoRoot = process.cwd(), generatedAtMs = 0 } = {}) {
  const stats = []
  for (const relativePath of PRIVATE_MEMORY_FILES) {
