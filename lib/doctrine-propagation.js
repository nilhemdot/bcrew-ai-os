...
export const DOCTRINE_PROPAGATION_SOURCES = [
  ...
  {
    id: 'nothing-manual',
    title: 'Nothing manual stays trusted',
    plainEnglish: 'Foundation rules, gates, and closeouts must become runnable checks, tracked backlog, approved docs, or verifier coverage instead of living only as manual reminders.',
    requiredPhrases: ['Nothing manual stays trusted', 'runnable checks', 'manual reminders'],
  },
  {
    id: 'memory-is-not-backlog',
    title: 'Memory is not backlog',
    plainEnglish: 'Memory can preserve context, but live backlog cards own durable work, scope, priority, proof, and closeout.',
    requiredPhrases: ['Memory is not backlog', 'live backlog cards', 'durable work'],
  },
  {
    id: 'ship-gate-required',
    title: 'Pre-commit and ship gates are required',
    plainEnglish: 'Protected Foundation paths use lightweight pre-commit checks, pre-push ship-proof enforcement, and process:foundation-ship as the canonical full gate.',
    requiredPhrases: ['Pre-commit and ship gates are required', 'pre-push ship-proof', 'process:foundation-ship'],
  },
  {
    id: 'private-workspace-metadata-only',
    title: 'Real private files are metadata-only proof',
    plainEnglish: 'Real private workspace files may be checked only by filename, path class, size, timestamp, metadata fingerprint, and allowlist status; leak tests use synthetic sentinel fixtures.',
    requiredPhrases: ['metadata-only proof', 'synthetic sentinel fixtures', 'allowlist status'],
  },
  {
    id: 'gate-failures-reliable',
    title: 'Gate failures must be reliable signals',
    plainEnglish: 'Transient gate errors can retry through bounded deterministic handling, but permanent DB, schema, or verifier failures must still fail closed.',
    requiredPhrases: ['Gate failures must be reliable signals', 'deterministic handling', 'fail closed'],
  },
  {
    id: 'decision-auto-emit-proposed-only',
    title: 'Detected decisions are proposed only',
    plainEnglish: 'Auto-detected decision language may create proposed decision records only; applied or locked decisions need a separate explicit approval path.',
    requiredPhrases: ['Detected decisions are proposed only', 'proposed decision records only', 'explicit approval path'],
  },
  {
    id: 'ceo-dashboard-pattern',
    title: 'CEO dashboard surfaces answer the operator questions',
    plainEnglish: 'Foundation surfaces should make clear what changed, where it lives, what to review, what is blocked, what is next, and what proof supports confidence.',
    requiredPhrases: ['CEO dashboard surfaces', 'what to review', 'what proof supports confidence'],
  },
]
