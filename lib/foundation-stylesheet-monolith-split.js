...
export const STYLESHEET_MODULE_PATHS = [
  ...
  'public/styles-foundation-data-sources-v2.css',
  'public/styles-strategy-docs.css',
  'public/styles-home-foundation-shell.css',
  'public/styles-strategy-sales.css',
]

export const STYLESHEET_APPROVED_STANDALONE_PAGE_STYLES = {
  'public/login.html': '/login.css',
}
...
export function evaluateStylesheetMonolithSplit({
  ...
  const pagesWithoutRootStylesheet = Object.entries(htmlSources || {})
    ...
    .filter(([path, source]) => {
      const text = String(source || '')
      const standaloneStyle = STYLESHEET_APPROVED_STANDALONE_PAGE_STYLES[path]
      return !text.includes('styles.css') && !(standaloneStyle && text.includes(standaloneStyle))
    })
    .map(([path]) => path)
  ...
  addCheck(
    ...
    'HTML pages continue to load the stable root stylesheet entry point or approved standalone CSS',
    pagesWithoutRootStylesheet.join(', ') || 'all checked pages link styles.css or approved standalone CSS',
  )
...
export function buildStylesheetMonolithSplitDogfoodProof() {
  ...
  const missingPageStylesheet = evaluateStylesheetMonolithSplit({
    rootSource: goodRoot,
    moduleSources,
    htmlSources: { 'public/unapproved.html': '<html><main>No stylesheet</main></html>' },
    minimumCombinedLines: 1,
  })
  ...
  return {
    ok: good.ok === true &&
      ...
      missingPageStylesheet.ok === false,
    invariant: 'stylesheet split only passes when the root is small, imports are complete/in order, modules stay below budget, required selectors survive, and HTML pages use root or approved standalone CSS',
    good,
    oldFailures: {
      ...
      missingPageStylesheetRejected: missingPageStylesheet.ok === false,
    },
  }
}
