...
export const VERIFIER_PROCESS_GOVERNANCE_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-process-governance-split-closeout.md'
...
export async function evaluateFoundationVerifierProcessGovernance(input = {}) {
  ...
  const sourceExtractionGapRequiredSourceIds = [
    'SRC-MISSIVE-001',
    'SRC-SLACK-001',
    'SRC-GDRIVE-001',
    'SRC-FUB-001',
  ...
  const sourceExtractionGapRepresentedSourceIds = new Set([
    ...(sourceExtractionGapFollowupSnapshot.triageItems || []).map(item => item.sourceId),
    ...(sourceExtractionGapFollowupSnapshot.connectedOrNoActionRows || []).map(item => item.sourceId),
  ].filter(Boolean))
  const sourceExtractionGapUnrepresentedSourceIds = sourceExtractionGapRequiredSourceIds
    .filter(sourceId => !sourceExtractionGapRepresentedSourceIds.has(sourceId))
  ...
  const llmAuthAuditVerifierDogfoodProof = buildLlmAuthAuditVerifierDogfoodProof()
    ...
    (sourceExtractionGapFollowupIsBuilding || sourceExtractionGapFollowupIsClosed) &&
      ...
      sourceExtractionGapUnrepresentedSourceIds.length === 0 &&
      ...
      sourceExtractionGapFollowupReportSource.includes('Rows needing triage:') &&
      sourceExtractionGapFollowupReportSource.includes('Triage items:') &&
    ...
    `lane=${sourceExtractionGapFollowupCard?.lane || 'missing'} stage=${sourceExtractionGapFollowupCurrentItem?.stage || 'closed'} triage=${sourceExtractionGapFollowupSnapshot.summary?.triageItemCount || 0} missing=${sourceExtractionGapMissingIds.join(',') || 'none'} unrepresented=${sourceExtractionGapUnrepresentedSourceIds.join(',') || 'none'}`,
  )
  ensure(
    checks,
    ...
    (researchLanePurgeIsBuilding || researchLanePurgeIsClosed) &&
      ...
      (
        researchLanePurgePlanSource.includes('docs/handoffs/research-purge-2026-05-13.md') ||
        researchLanePurgePlanSource.includes('docs/_archive/handoffs/research-purge-2026-05-13.md')
      ) &&
