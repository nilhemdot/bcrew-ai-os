...
function linkFamily(url = '') {
  ...
  if (host.includes('calendly') || host.includes('typeform') || /(^|[/?#&=._-])(form|forms|optin|opt-in|subscribe|subscription|waitlist|book|booking|book-a-call)([/?#&=._-]|$)/i.test(url)) return 'form_or_booking'
...
function formSignalsOf(html = '') {
  ...
  const controls = [...source.matchAll(/<(input|button|textarea|select)\b([^>]*)>/gi)]
    .map(match => {
      ...
      const type = attr(match[0], 'type') || (tag === 'button' ? 'button' : '')
      const name = attr(match[0], 'name')
      const textLabel = tag === 'input' ? attr(match[0], 'placeholder') : ''
      return {
        tag,
    ...
    .filter(control => control.tag || control.type || control.name || control.label)
    .slice(0, 20)
  return {
    formCount: forms.length,
    forms: forms.slice(0, 10),
    controls,
    externalActionRequired: forms.length > 0 || controls.some(control => /submit|email|button/.test(`${control.type} ${control.name} ${control.label}`.toLowerCase())),
  }
}
...
export function buildPublicWebRuntimeArtifact({ packet = {}, html = '', mode = 'synthetic_fixture', fetchedAt = new Date().toISOString() } = {}) {
  ...
  const formSignals = formSignalsOf(html)
  return {
    ...
    evidence: {
      ...
      externalActionRequired: formSignals.externalActionRequired,
    },
    outputDestination: packet.outputDestination,
    sideEffects: sideEffects({
      liveBrowserLaunched: mode === 'live_playwright_exact_url',
      networkFetched: mode === 'live_playwright_exact_url',
    }),
  }
}
...
export async function buildSourcePacketPublicWebRuntimeDogfoodProof() {
  ...
  const approvedCourseNamedRepo = approvedFixturePacket({ url: 'https://github.com/mattpocock/course-video-manager', host: 'github.com' })
  const approvedNewsletter = approvedFixturePacket({ url: 'https://www.aihero.dev/newsletter', host: 'aihero.dev' })
  ...
  const runCourseNamedRepo = await runSourcePacketPublicWebRuntime({
    packet: approvedCourseNamedRepo,
    html: '<html><head><title>course-video-manager</title></head><body><h1>Course Video Manager</h1><p>Public repo README metadata.</p></body></html>',
  })
  const runNewsletter = await runSourcePacketPublicWebRuntime({
    packet: approvedNewsletter,
    html: '<html><head><title>AI Hero Newsletter</title></head><body><h1>AI Hero Newsletter</h1><form action="/api/newsletter"><input type="email" name="email" placeholder="Email"><button type="submit">Subscribe</button></form></body></html>',
  })
  const blockedCases = [
    {
  ...
  return {
    ok: runPublic.ok === true &&
      runSales.ok === true &&
      runCourseNamedRepo.ok === true &&
      runNewsletter.ok === true &&
      runNewsletter.artifact.formSignals.externalActionRequired === true &&
    ...
    runs: {
      ...
      courseNamedRepo: {
        ok: runCourseNamedRepo.ok,
        title: runCourseNamedRepo.artifact?.title || '',
      },
      newsletterPage: {
        ok: runNewsletter.ok,
        formCount: runNewsletter.artifact?.formSignals?.formCount || 0,
        externalActionRequired: runNewsletter.artifact?.formSignals?.externalActionRequired === true,
        sideEffects: runNewsletter.sideEffects,
      },
    },
    blockedCases,
  }
