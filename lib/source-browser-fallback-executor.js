...
function sourceFamilyForFallbackRow(row = {}) {
  ...
  const surface = `${bucketId} ${sourceType} ${row.url || ''} ${row.label || ''} ${row.title || ''}`
  if (
    bucketId === 'paid-auth-gates' ||
    /\b(paid|auth|login|log-in|signin|sign-in|signup|sign-up|register|join|course|training|private|member|checkout|payment|billing|purchase|buy|cart|account|profile|settings|download)\b/i.test(surface)
  ) return 'paid_or_auth_gates'
...
function sourceTypeForFallbackRow(row = {}, sourceFamily = sourceFamilyForFallbackRow(row)) {
  ...
  if (explicit) return explicit
  if (sourceFamily === 'paid_or_auth_gates') return 'paid_or_auth_source'
...
function buildSourcePacket(row = {}) {
  ...
  const resolvedUrl = text(row.url)
  const resolvedHost = text(row.host || hostOf(resolvedUrl))
  return {
    sourceId: text(row.rowId || row.sourceId || resolvedHost || 'source-browser-fallback'),
    url: resolvedUrl,
    sourceType,
    sourceFamily,
    title: text(row.label || row.title || resolvedHost || resolvedUrl),
    preview: text(row.summary || row.description || `Clean isolated retry for ${resolvedHost || 'source'} after a parked fallback row.`),
    action: sourceFamily === 'skool_free_community'
      ...
      : sourceFamily === 'creator_newsletters'
        ...
        : sourceFamily === 'github_docs_public_resources'
          ...
          : sourceFamily === 'paid_or_auth_gates'
            ? 'read_approved_auth_source_after_packet'
          : 'read_public_page',
...
function sourceSessionRequiredFor(packet = {}, fallbackPlan = {}) {
  ...
  return ['creator_newsletters', 'skool_free_community', 'skool_paid_or_private', 'paid_course_training_platforms', 'paid_or_auth_gates'].includes(text(packet.sourceFamily))
}
...
export function buildSourceBrowserFallbackRetryPacket({
  ...
  return {
    ...
    fallbackPlan: {
      ...
      route: text(fallbackPlan.route || (sourceSessionRequired ? 'source_specific_session_then_harlan_operator' : 'clean_isolated_retry_then_harlan_operator')),
      sourceSessionRequired,
      normalChromeProfileAllowed: fallbackPlan.normalChromeProfileAllowed === true,
      previousBlocker: text(fallbackPlan.previousBlocker || fallbackPlan.reason || row.reason || row.plainEnglish || row.nextAction || ''),
      ...
      nextAction: text(fallbackPlan.nextAction || 'Run the clean retry first; if the challenge remains, prepare Harlan/operator escalation or source-specific session repair.'),
    },
    cleanRetry: {
      allowedNow: ready,
    ...
    operatorEscalation: {
      status: 'pending_harlan_operator_or_source_session',
      allowedOnlyAfterCleanRetryFails: true,
      route: sourceSessionRequired ? 'source_session_repair_then_harlan_operator' : 'harlan_operator_escalation_after_clean_retry',
      nextAction: 'Use only after clean isolated retry/source-session retry still cannot prove real source content. Do not use any external hosted browser or a normal Chrome profile from this path.',
    },
...
export async function runSourceBrowserFallbackRetry({
  ...
  if (apply !== true) {
    return {
      ...
      persistence: null,
      sideEffects: sideEffects(),
      plainEnglish: 'Dry run only. No browser, network, source-session, operator escalation, Telegram, or persistence action started.',
    }
  }
  ...
  const status = execution.ok === true && unsafe === false
    ...
    : challengeAgain
      ? 'source_browser_fallback_operator_escalation_required'
      : unsafe
  ...
  return {
    ...
    operatorEscalationRequired: challengeAgain,
    ...
    plainEnglish: status === 'source_browser_fallback_clean_retry_completed'
      ...
      : challengeAgain
        ? 'Clean isolated retry still saw a browser challenge. Park for Harlan/operator escalation or source-specific session repair; do not count this as extracted evidence.'
