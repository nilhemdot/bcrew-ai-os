...
export const FOUNDATION_HEALTH_WATCH_TO_GREEN_CLOSEOUT_PATH = 'docs/handoffs/2026-05-19-foundation-health-watch-to-green-closeout.md'
...
const HEALTHY_FAILURE_TELEMETRY = {
  status: 'healthy',
  summary: {
    redFingerprintCount: 0,
    yellowFingerprintCount: 0,
    eventCount7d: 0,
  },
  topFindings: [],
  fingerprints: [],
...
function buildJob({ key, title, priority = 'P0', latestRun, mutationPosture = 'read_only' } = {}) {
  return {
    key,
    title,
    priority,
...
function buildClassifiedFixture({ includeUnclassified = false } = {}) {
  ...
  return buildFoundationSystemHealthSnapshot({
    ...
    buildLaneFailureTelemetry: HEALTHY_FAILURE_TELEMETRY,
...
function classificationComplete(finding = {}) {
  ...
  return Boolean(
    classification &&
      ...
      classification.blocksCurrentSprint === false,
  )
}
...
export function summarizeFoundationHealthWatchToGreen(systemHealth = {}) {
  ...
  return {
    ok: systemHealth.status === 'healthy' &&
      Number(systemHealth.summary?.unclassifiedRiskCount || 0) === 0 &&
...
export function buildFoundationHealthWatchToGreenDogfoodProof() {
  ...
  const checks = [
    {
      ok: classified.status === 'healthy' &&
        classifiedSummary.rawRiskCount > 0 &&
        classifiedSummary.classifiedFindingCount >= 7 &&
        classifiedSummary.unclassifiedRiskCount === 0,
      check: 'classified approval/watch rows allow green without hiding raw findings',
    },
    {
      ok: classified.findings.every(finding => !['risk', 'watch'].includes(finding.rollupLevel) || classificationComplete(finding)),
      check: 'every non-green classified row has owner, reason, threshold, next action, and non-blocking decision',
    },
    {
