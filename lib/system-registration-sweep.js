...
function findCloseout(foundationBuildLog, buildLogSource, closeoutKey, backlogId, mentionedBacklogIds = []) {
  return normalizeList(foundationBuildLog?.builds).find(build =>
    build.closeoutKey === closeoutKey &&
      (!backlogId || normalizeList(build.backlogIds).includes(backlogId))
...
export async function buildSystemRegistrationSweepStatus({
  ...
  ]), 'Foundation Systems UI renders grouped systems and supporting evidence sources', 'public/foundation.js')
