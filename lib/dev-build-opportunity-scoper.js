...
function needsResourceLinkDisposition(candidate = {}, researchContext = {}) {
  return Boolean(
    candidate.resourceLinkCount ||
      ...
      list(researchContext.unresolvedPublicResourceLinks).length ||
      list(researchContext.approvalRequiredResourceLinks).length
  )
}

function hasQueuedPublicResourceLinkWork(researchContext = {}) {
  const queuedLinks = [
    ...list(researchContext.unresolvedResourceLinks),
    ...list(researchContext.unresolvedPublicResourceLinks),
  ]
  const dispositionText = list(researchContext.resourceLinkDispositions).join(' ')
  return queuedLinks.length > 0 ||
    /public metadata still queued|still resolver work|unresolved public resource|queued for resolver/i.test(dispositionText)
}
...
function scoperMissingPieces({ directorCandidate = {}, researchContext = {} } = {}) {
  ...
  if (hasQueuedPublicResourceLinkWork(researchContext)) {
    missing.push('unresolved_public_resource_links')
  }
  return missing
}
...
export function buildDevBuildOpportunityScoperDogfoodProof() {
  ...
  const researched = buildDevBuildOpportunityScope({
    ...
    researchContext: {
      ...
      resourceLinkDispositions: [
        'Resolved public metadata: github.com - Example AIOS skill repo',
  ...
  const researchedWithQueuedPublicLinks = buildDevBuildOpportunityScope({
    directorCandidate,
    researchContext: {
      sourceIds: ['SRC-YOUTUBE-INTEL-001'],
      sourceRefs: [
      ...
      buildApproach: 'Scope the feature from source evidence.',
      resourceLinkDispositions: [
        'Public metadata still queued for resolver: github.com - https://github.com/example/skill-package',
      ],
      unresolvedPublicResourceLinks: ['https://github.com/example/skill-package'],
      acceptanceCriteria: ['candidate has a build target'],
      definitionOfDone: ['candidate has a proof path'],
      risks: ['resource links can hide the actual code or skill package being discussed'],
      notNext: ['do not manually chase links outside source policy'],
  ...
  const checks = [
    ...
    {
      ok: researchedWithQueuedPublicLinks.ok === false &&
        researchedWithQueuedPublicLinks.status === DEV_BUILD_SCOPER_STATUS.needsResearch &&
        researchedWithQueuedPublicLinks.missing.includes('unresolved_public_resource_links'),
      check: 'Scoper rejects queued public resource links instead of making Steve chase them',
      detail: researchedWithQueuedPublicLinks.missing.join(', '),
    },
    {
  ...
  return {
    ...
    researchedWithQueuedPublicLinks,
    unresearched,
    blocked,
    researchedPortfolio,
