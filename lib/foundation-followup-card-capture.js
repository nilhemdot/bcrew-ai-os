...
export async function buildFoundationFollowupCardCaptureStatus({
  ...
  const seedSource = await readOptionalText(repoRoot, 'lib/foundation-db.js')
  ...
  if (foundationHub) {
    ...
    for (const cardId of FOUNDATION_FOLLOWUP_BUILD_ORDER) {
      ...
      addFinding(findings, card?.lane === 'scoped', `${cardId} remains scoped`, card?.lane || 'missing')
  ...
  const closeout = normalizeList(foundationBuildLog?.builds).find(build => build.closeoutKey === FOUNDATION_FOLLOWUP_CARD_CAPTURE_CLOSEOUT_KEY) || null
  ...
  return {
    ...
    summary: {
      ...
      scopedCardsRemainScoped: followupCards.filter(Boolean).every(card => card.lane === 'scoped'),
