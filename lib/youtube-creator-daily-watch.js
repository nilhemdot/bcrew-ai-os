...
export const YOUTUBE_CREATOR_DAILY_WATCH_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-21-youtube-creator-daily-watch-closeout.md'
...
export const YOUTUBE_CREATOR_DAILY_WATCH_ICOR_CREATOR_ID = 'icor-tom-ai-productivity'
...
export const YOUTUBE_CREATOR_DAILY_WATCH_READBACK_LIMIT = 1000
...
export const YOUTUBE_CREATOR_DAILY_WATCH_DEEP_BASELINE_CREATOR_IDS = [
  YOUTUBE_CREATOR_DAILY_WATCH_MARK_CREATOR_ID,
  YOUTUBE_CREATOR_DAILY_WATCH_ICOR_CREATOR_ID,
]
...
export function baselineDepthForCreator(creatorId = '') {
  return YOUTUBE_CREATOR_DAILY_WATCH_DEEP_BASELINE_CREATOR_IDS.includes(text(creatorId))
...
export function buildYoutubeCreatorDailyWatchSnapshot({
  ...
  const deepBaselineCreators = knownPublicCreators.filter(creator => YOUTUBE_CREATOR_DAILY_WATCH_DEEP_BASELINE_CREATOR_IDS.includes(creator.creatorId))
  const defaultBaselineCreators = knownPublicCreators.filter(creator => !YOUTUBE_CREATOR_DAILY_WATCH_DEEP_BASELINE_CREATOR_IDS.includes(creator.creatorId))
  ...
  addFinding(findings, deepBaselineCreators.every(creator => creator.baselineDepth === YOUTUBE_CREATOR_DAILY_WATCH_MARK_BASELINE_DEPTH), 'deep-baseline public creators use last 50', deepBaselineCreators.map(creator => `${creator.creatorId}:${creator.baselineDepth}`).join(', '))
  addFinding(findings, defaultBaselineCreators.every(creator => creator.baselineDepth === YOUTUBE_CREATOR_DAILY_WATCH_DEFAULT_BASELINE_DEPTH), 'default public creators use last 20', `${defaultBaselineCreators.length} creator(s)`)
  ...
  return {
    ...
    summary: {
      ...
      deepBaselineCreatorIds: YOUTUBE_CREATOR_DAILY_WATCH_DEEP_BASELINE_CREATOR_IDS,
      defaultBaselineDepth: YOUTUBE_CREATOR_DAILY_WATCH_DEFAULT_BASELINE_DEPTH,
    },
    findings,
    failures,
  ...
  const rows = list(items)
    ...
    .map(item => {
      ...
      return {
        ...
        fullWatchBlockedReason: metadata.fullWatchBlockedReason || '',
        commentsCrawled: metadata.commentsCrawled === true,
        memberCommunityCrawled: metadata.memberCommunityCrawled === true,
        transcriptsFetched: metadata.transcriptsFetched === true,
        resourceLinksFollowed: metadata.resourceLinksFollowed === true,
        screenshotsCaptured: metadata.screenshotsCaptured === true,
        externalWrites: metadata.externalWrites === true,
        createsBacklogCardsAutomatically: metadata.createsBacklogCardsAutomatically === true,
      }
    })
  return {
...
export function buildYoutubeCreatorDailyWatchDogfoodProof() {
    ...
    ...buildYoutubeCreatorDailyWatchPlan({
      watchlistEntries: [
        ...
        },
        {
          ...
          consumerLane: 'build_intel',
          cadence: 'weekly',
          whySteveCares: 'Synthetic ICOR proof.',
          platforms: [{ type: 'youtube', label: 'YouTube', url: 'https://www.youtube.com/@myicor', lookupStatus: 'known_public_url', accessBoundary: 'public_lookup_required' }],
          sourceRefs: [],
          active: true,
        },
        {
  ...
  const icor = plan.creators.find(creator => creator.creatorId === 'icor-tom-ai-productivity')
  ...
  const icorRaw = Array.from({ length: 55 }, (_, index) => ({
    href: `https://www.youtube.com/watch?v=icorbase${String(index).padStart(4, '0')}`,
    text: `ICOR AI team setup ${index} ${index + 1} views • ${index + 1} days ago`,
  }))
  ...
  const icorVideos = uniqueCreatorVideos(icorRaw, { creator: icor, limit: icor.baselineDepth, discoveredAt: now })
  ...
  const poolItems = buildYoutubeCreatorDailyWatchPoolItems({
    discoveries: [
      ...
      { creatorId: icor.creatorId, creator: icor.displayName, baselineDepth: icor.baselineDepth, videos: icorVideos, discoveredCount: icorVideos.length },
  ...
  const cases = [
    ...
    { name: 'icor_baseline_depth_last_50', ok: icorVideos.length === 50, detail: `${icorVideos.length}/50` },
