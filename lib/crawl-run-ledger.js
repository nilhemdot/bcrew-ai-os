...
export const CRAWL_RUN_LEDGER_HANDOFF_PATH = 'docs/handoffs/2026-05-16-crawl-run-ledger-reconcile-closeout.md'
  ...
  return {
    ...
    async query(sql, params = []) {
      ...
      if (/UPDATE source_crawl_targets/.test(text) && /lease_owner = NULL/.test(text)) {
        if (state.target.target_key !== params[0] || state.target.lease_owner !== params[11]) return { rows: [] }
        state.target = {
          ...state.target,
          lease_owner: null,
          lease_expires_at: null,
