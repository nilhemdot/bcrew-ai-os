...
export const VERIFIER_OPERATOR_LIVE_SURFACE_ASSURANCE_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-operator-live-surface-assurance-split-closeout.md'
...
export async function evaluateFoundationVerifierOperatorLiveSurfaceAssurance(input = {}) {
  ...
  const missiveCurrentDayHasHealthyItemProof = Boolean(
    missiveCurrentDayTarget?.itemSummary &&
      ...
      Number(missiveCurrentDayTarget.itemSummary.staleLeasedItems || 0) === 0 &&
      Number(missiveCurrentDayTarget.itemSummary.retryExhaustedItems || 0) === 0,
  )
  const missiveCurrentDayLatestRunStillActive = ['running', 'leased'].includes(
    missiveCurrentDayTarget?.scheduler?.latestRunStatus,
  )
  const missiveCurrentDayFailedByStaleReaperWithHealthyProof =
    missiveCurrentDayHasHealthyItemProof &&
    missiveCurrentDayTarget?.lastStatus === 'failed' &&
    String(missiveCurrentDayTarget?.lastError || '').includes('stale source-crawl run reaper')
  const missiveCurrentDayStatusAcceptable =
    nonStaleExtractionStatuses.has(missiveCurrentDayTarget?.lastStatus) ||
    (
      missiveCurrentDayLatestRunStillActive &&
      missiveCurrentDayHasHealthyItemProof &&
      String(missiveCurrentDayTarget?.lastError || '').includes('stale source-crawl run reaper')
    ) ||
    missiveCurrentDayFailedByStaleReaperWithHealthyProof
  ensure(
    checks,
    missiveCurrentDayHasHealthyItemProof &&
      Array.isArray(missiveCurrentDayTarget.healthFindings) &&
      missiveCurrentDayStatusAcceptable,
    ...
    'docs/_archive/audits/2026-04-28-extraction-lane-item-shape.md records Missive normalization and remaining Drive failures',
  )
  ...
  const foundationSurfaceUpdatesCloseout = foundationBuildCloseouts.find(closeout =>
    closeout.key === 'foundation-surface-updates-v1' &&
      (closeout.backlogIds || []).includes('FOUNDATION-SURFACE-UPDATES-001')
  )
  const foundationSurfaceUpdatesCloseoutText = [
    foundationSurfaceUpdatesCloseout?.whatChanged,
    foundationSurfaceUpdatesCloseout?.whatItDoes,
    foundationSurfaceUpdatesCloseout?.whyItMatters,
    ...(foundationSurfaceUpdatesCloseout?.whereItLives || []),
  ].filter(Boolean).join('\n')
  const foundationSurfaceUpdatesShipped = foundationSurfaceUpdates?.lane === 'done' &&
    foundationSurfaceUpdatesCloseout?.operatorCloseout === true
  const foundationSurfaceUpdatesLive = foundationSurfaceUpdates?.lane === 'scoped' || foundationSurfaceUpdatesShipped
  const foundationSurfaceUpdatesScopedProof = foundationSurfaceUpdates?.lane === 'scoped' &&
  ...
  const foundationSurfaceUpdatesDoneProof = foundationSurfaceUpdatesShipped &&
    foundationSurfaceUpdatesText.includes('foundation-surface-updates-v1') &&
    foundationSurfaceUpdatesText.includes('Overview -> Systems -> Backlog -> Recent Work') &&
    foundationSurfaceUpdatesText.includes('moved-to-done signal') &&
    foundationSurfaceUpdatesCloseoutText.includes('plain-English Overview path') &&
    foundationSurfaceUpdatesCloseoutText.includes('clickable Recent Work') &&
    foundationSurfaceUpdatesCloseoutText.includes('done-date signal')
  ensure(
    checks,
    foundationSurfaceUpdatesLive &&
      foundationSurfaceUpdates?.priority === 'P1' &&
      (foundationSurfaceUpdatesScopedProof || foundationSurfaceUpdatesDoneProof) &&
      foundationSurfaceMap.some(surface =>
        surface.section === 'build-log' &&
          (surface.backlogIds || []).includes('FOUNDATION-SURFACE-UPDATES-001')
      ),
    'Foundation operator UX and Recent Work follow-up is closed or live',
