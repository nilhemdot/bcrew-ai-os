...
function compactFullDiagnosticsBacklogItems({ compactRows = [], fullRows = [] } = {}) {
  ...
  return (Array.isArray(compactRows) ? compactRows : []).map(row => {
    ...
    if (!Object.keys(fullRow).length) return row
    const {
      createdAt,
      updatedAt,
      whyItMatters,
    ...
    const compactWhy = truncatePayloadText(whyItMatters || whyItMattersSnake, 180)
    ...
    try {
      ...
      const fullPayload = {
        ...
        backlogContract: {
          ...
          fullDiagnosticsBacklogRowTrimmedFields: ['createdAt', 'updatedAt', 'expanded whyItMatters text'],
