...
function compactStructuredOutputForHub(structured = {}) {
  if (!structured || typeof structured !== 'object' || Array.isArray(structured)) return {}
  return {
    keys: Object.keys(structured).slice(0, 20),
    reviewRouteCount: Array.isArray(structured.reviewRoutes) ? structured.reviewRoutes.length : 0,
    opportunityCount: Array.isArray(structured.opportunities) ? structured.opportunities.length : 0,
    researchPoolCount: Array.isArray(structured.researchPool) ? structured.researchPool.length : 0,
    itemCount: Array.isArray(structured.items) ? structured.items.length : 0,
    compacted: true,
  }
}

function compactIntelligenceReportForHub(report = {}) {
  if (!report || typeof report !== 'object') return null
  return {
    reportArtifactId: report.reportArtifactId || null,
    reportType: report.reportType || null,
    scopeKey: report.scopeKey || null,
    department: report.department || null,
    title: truncatePayloadText(report.title, 180),
    status: report.status || null,
    sourceIds: Array.isArray(report.sourceIds) ? report.sourceIds.slice(0, 12) : [],
    inputArtifactIds: Array.isArray(report.inputArtifactIds) ? report.inputArtifactIds.slice(0, 12) : [],
    inputAtomCount: Array.isArray(report.inputAtomIds) ? report.inputAtomIds.length : 0,
    actionRequiredCount: Array.isArray(report.actionRequiredItems) ? report.actionRequiredItems.length : 0,
    topFindings: Array.isArray(report.topFindings)
      ? report.topFindings.slice(0, 5).map(item => truncatePayloadText(typeof item === 'string' ? item : JSON.stringify(item), 220))
      : [],
    openQuestionCount: Array.isArray(report.openQuestions) ? report.openQuestions.length : 0,
    contradictionCount: Array.isArray(report.contradictions) ? report.contradictions.length : 0,
    structuredOutputSummary: compactStructuredOutputForHub(report.structuredOutputJson),
    outputArtifactId: report.outputArtifactId || null,
...
function compactIntelligenceAtomForHub(atom = {}) {
  if (!atom || typeof atom !== 'object') return null
  return {
    atomId: atom.atomId || null,
    sourceId: atom.sourceId || null,
    artifactId: atom.artifactId || null,
    reportArtifactId: atom.reportArtifactId || null,
    title: truncatePayloadText(atom.title, 180),
    status: atom.status || null,
    atomType: atom.atomType || null,
    valueRoute: atom.valueRoute || null,
    qualityScore: atom.qualityScore ?? null,
    relevanceScore: atom.relevanceScore ?? null,
    confidenceScore: atom.confidenceScore ?? null,
    hitCount: atom.hitCount ?? null,
    updatedAt: atom.updatedAt || null,
  }
}

function compactIntelligenceAtomSpineForHub(spine = {}) {
  if (!spine || typeof spine !== 'object') return spine
  return {
    ...spine,
    recentReports: Array.isArray(spine.recentReports)
      ? spine.recentReports.map(compactIntelligenceReportForHub).filter(Boolean)
      : [],
    recentAtoms: Array.isArray(spine.recentAtoms)
      ? spine.recentAtoms.map(compactIntelligenceAtomForHub).filter(Boolean)
      : [],
    latestScoperQueryProof: Array.isArray(spine.latestScoperQueryProof)
      ? spine.latestScoperQueryProof.map(compactIntelligenceAtomForHub).filter(Boolean)
      : [],
    intelAtomScoperQueryProof: Array.isArray(spine.intelAtomScoperQueryProof)
      ? spine.intelAtomScoperQueryProof.map(compactIntelligenceAtomForHub).filter(Boolean)
      : [],
    fullPayloadCompacted: true,
    detailPath: '/api/foundation/dev-team-hub',
  }
}
  ...
  app.get('/api/foundation-hub', deps.requireAdminToken, async (req, res) => {
    ...
    try {
      ...
      const fullPayload = {
        ...
        intelligenceAtomSpine: compactIntelligenceAtomSpineForHub(snapshot.intelligenceAtomSpine),
