...
export const intelligenceRetrievalSchemaSql = `
  ...
  CREATE TABLE IF NOT EXISTS intelligence_retrieval_runs (
    ...
    run_type TEXT NOT NULL
      CHECK (run_type IN ('candidate_promotion', 'lexical_proof', 'semantic_embedding', 'semantic_proof', 'hybrid_proof')),
...
function normalizeRunType(value) {
  ...
  if (!['candidate_promotion', 'lexical_proof', 'semantic_embedding', 'semantic_proof', 'hybrid_proof'].includes(normalized)) {
...
function buildProofQueryFromChunk(chunk) {
  ...
  return words.slice(0, 3).join(' ') || 'foundation'
}
