...
function extractStaticImportSpecifiers(source = '') {
  const specifiers = []
  for (const line of stripJavaScriptComments(source).split('\n')) {
    if (/^\s*\/\//.test(line) || !line.trim()) continue
    const sourceLine = line.replace(/\s+\/\/.*$/, '')
    const staticImport = sourceLine.match(/^import\s+(?:[^'"]+\s+from\s+)?['"]([^'"]+)['"]/)
    const reExport = sourceLine.match(/^export\s+[^'"]+\s+from\s+['"]([^'"]+)['"]/)
    const specifier = staticImport?.[1] || reExport?.[1] || ''
    if (specifier) specifiers.push(specifier)
  }
  return specifiers
}

function resolveRelativeModule(fromPath, specifier, fileSet) {
  ...
  return candidates.find(candidate => fileSet.has(candidate) && /\.(mjs|js)$/.test(candidate)) || null
}
...
function buildStaticImportCycleSnapshot(trackedSources = {}) {
  const files = Object.keys(trackedSources)
    .filter(file => /^(lib|scripts)\/.+\.(mjs|js)$/.test(file))
    .sort()
  const fileSet = new Set(files)
  const graph = new Map()
  for (const file of files) {
    const targets = extractStaticImportSpecifiers(trackedSources[file])
      .map(specifier => resolveRelativeModule(file, specifier, fileSet))
      .filter(Boolean)
      .sort()
    graph.set(file, Array.from(new Set(targets)))
  }
...
export function buildFoundationTuneupRemapDogfoodProof() {
  ...
  const cycleDetected = buildFoundationTuneupRemapSnapshot({
    map: healthy.map,
    packageScripts: healthy.packageScripts,
    trackedSources: {
      ...healthy.trackedSources,
      'lib/static-cycle-a.js': "import './static-cycle-b.js'\n",
      'lib/static-cycle-b.js': "import './static-cycle-a.js'\n",
    },
    liveCards: healthy.roadmap.requiredDoneCards.map(card => ({ id: card.id, lane: 'done', statusNote: 'done' })),
    planSource: healthy.planSource,
    closeoutSource: healthy.closeoutSource,
  ...
  return {
    ok: healthy.ok === true &&
      baselineNotReduced.ok === false &&
      hubStarted.ok === false &&
      cycleDetected.ok === false &&
      cycleDetected.failed.some(check => check.check === 'static lib/script import cycles are absent'),
    healthy,
    baselineNotReduced,
    hubStarted,
    cycleDetected,
...
export function buildFoundationTuneupRemapSnapshot({
  ...
  const importCycles = buildStaticImportCycleSnapshot(trackedSources)
  ...
  addCheck(
    checks,
    importCycles.cycleCount === 0,
    'static lib/script import cycles are absent',
    importCycles.cycles.map(cycle => cycle.join(' -> ')).join('; ') || `${importCycles.checkedFileCount} files checked`,
  )
  addCheck(
    checks,
  ...
  return {
    ...
    summary: {
      ...
      importCycleCount: importCycles.cycleCount,
      importCycleCheckedFiles: importCycles.checkedFileCount,
      publicDevCssLines,
