...
const RECIPIENT_SOURCE_PERSONAL = 'personal_email'
...
function selectRecipientSource(candidate = {}) {
  ...
  return {
    ...
    recipientSource: internalTeam ? RECIPIENT_SOURCE_COMPANY : RECIPIENT_SOURCE_PERSONAL,
    recipientFieldName: internalTeam ? COMPANY_EMAIL_FIELD : PERSONAL_EMAIL_FIELD,
    recipientRule: internalTeam ? 'clickup-company-email' : 'clickup-personal-email',
    toRole: internalTeam ? 'internal-team-recipient' : 'external-agent',
  }
}
...
async function loadInternalOversightEmailMap(candidate = {}) {
  ...
  for (const role of INTERNAL_OVERSIGHT_BCC_ROLE_KEYS) {
    const candidateEmail = role === 'georgia' ? normalizeText(candidate.recipientEmail) : ''
...
function buildCandidateFromTask({ task, fields, targetName, milestoneDay, now = new Date() }) {
  ...
  const personalEmail = normalizeText(taskFields.get(PERSONAL_EMAIL_FIELD))
  ...
  const recipientEmail = recipientSource.recipientSource === RECIPIENT_SOURCE_COMPANY ? companyEmail : personalEmail
...
function validateRouteSpecificSendApproval(approval = {}, proof = null) {
  ...
  const expectedRecipientRule = proof?.recipientPlan?.recipientRule || 'clickup-personal-email'
  ...
  if (!ok) {
    throw new Error('Real send requires Steve SEND APPROVED naming Georgia, milestone/day, recipient source/rule, BCC/internal oversight roles, and one-send limit.')
  }
  return true
}
...
export async function buildAgentFeedbackSendStatus({
  ...
  addFinding(
    ...
    internalRecipientSource.recipientSource === RECIPIENT_SOURCE_COMPANY &&
      ...
      externalRecipientSource.recipientSource === RECIPIENT_SOURCE_PERSONAL &&
      externalRecipientSource.recipientFieldName === PERSONAL_EMAIL_FIELD,
    'recipient source selection keeps internal/team on Company Email and external agents on Personal Email',
  ...
  addFinding(
    findings,
    !(proof.eligibility?.blockers || []).includes('missing_personal_email') &&
      !(proof.eligibility?.blockers || []).includes('invalid_personal_email'),
    'Georgia is not blocked by Personal Email when Company Email is the recipient source',
  ...
  addFinding(
    findings,
    proof.eligibility?.eligible === true &&
      (proof.eligibility?.blockers || []).length === 0 &&
    ...
    'Contract Link is warning metadata and does not block send eligibility',
