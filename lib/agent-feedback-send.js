...
export const AGENT_FEEDBACK_SEND_BASELINE_PATH = 'docs/_archive/audits/2026-04-30-agent-feedback-send-baseline.md'
export const AGENT_FEEDBACK_SEND_DRY_RUN_PROOF_PATH = 'docs/_archive/audits/2026-04-30-agent-feedback-send-dry-run-proof.md'
...
export async function buildAgentFeedbackSendStatus({
  ...
  const foundationDbSource = await readFoundationDbVerifierSource(repoRoot)
  ...
  const closeout = findFoundationBuildCloseoutEntry(
    ...
    { backlogId: AGENT_FEEDBACK_SEND_CARD_ID },
  )
  ...
  addFinding(findings, verifierCoverageSource.includes(AGENT_FEEDBACK_SEND_CARD_ID), 'foundation verifier covers send card', AGENT_FEEDBACK_SEND_CARD_ID)
  ...
  addFinding(findings, !hasPrivateProofLeak(closeoutProofText), 'send closeout has no raw email addresses, token URLs, or feedback content', 'metadata-only')
  addFinding(findings, sendCard?.lane === 'done' && Boolean(closeout), 'send card is closed as Stage 1 only and closeout exists', sendCard?.lane || 'missing')
