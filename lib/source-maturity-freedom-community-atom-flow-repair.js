...
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_CARD_ID = 'SOURCE-MATURITY-FREEDOM-COMMUNITY-ATOM-FLOW-REPAIR-001'
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY = 'source-maturity-freedom-community-atom-flow-repair-v1'
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_PLAN_PATH = 'docs/process/source-maturity-freedom-community-atom-flow-repair-001-plan.md'
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_APPROVAL_PATH = 'docs/process/approvals/SOURCE-MATURITY-FREEDOM-COMMUNITY-ATOM-FLOW-REPAIR-001.json'
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-29-hot-doc-refresh/2026-05-18-source-maturity-freedom-community-atom-flow-repair-closeout.md'
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_SCRIPT_PATH = 'scripts/process-source-maturity-freedom-community-atom-flow-repair-check.mjs'
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_SPRINT_ID = 'source-maturity-freedom-community-atom-flow-repair-2026-05-18'
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID = 'SRC-FREEDOM-COMMUNITY-001'
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_FACT_ID = 'fact:18d41cc725fa1afb29009d82'

export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES = [
...
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_PROOF_COMMANDS = [
...
export const SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_CHANGED_FILES = [
...
function targetFact(facts = []) {
  return list(facts).find(fact => (fact.factId || fact.fact_id) === SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_FACT_ID) || null
}

export function selectFreedomCommunityAtomFlowRepairCandidate({
  beforeRow = {},
  sourceFacts = [],
  sourceId = SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
} = {}) {
  const failures = []
  ...
  const factId = fact?.factId || fact?.fact_id || SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_FACT_ID
...
export function buildFreedomCommunityAtomFromFact(candidate = {}, now = new Date().toISOString()) {
  const sourceId = candidate.sourceId || SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  ...
  const naturalKey = `source-maturity-freedom-community-atom-flow-repair:${sourceId}:${factId}`
  ...
  const atom = {
    atomId,
    title: `${sourceId} community agent goal path`,
    ...
    evidenceExcerpt,
    derivedClaim: claim,
    topicRefs: ['source-maturity', 'freedom-community', 'community-goal', 'source-backed-goal-truth'],
    ...
    acceptedAt: now,
    acceptedBy: 'codex-source-maturity-freedom-community-atom-flow-repair',
    reviewNote: 'Promoted from an existing governed Freedom Community source fact only; no live extraction, provider call, Sheets read/write, or external write occurred.',
    tags: ['source-maturity', 'atom-flow-repair', 'freedom-community', 'no-live-extraction'],
    metadata: {
...
export function evaluateFreedomCommunityAtomFlowRepair({
  beforeRow = {},
  afterRow = {},
  candidate = {},
  atomBuild = {},
  ...
  return {
    ...
    failures,
    summary: {
      sourceId: candidate.sourceId || beforeRow.sourceId || afterRow.sourceId || SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
...
export function buildSyntheticFreedomCommunityAtomFlowRepairProof() {
  const fact = {
    factId: SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_FACT_ID,
    sourceId: SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
    title: 'Community Goal: 10,000 Agents',
    claim: 'Community Goal: 10,000 Agents is Pace: Ahead by 22 agents (+3.3%).',
    detail: 'Current combined ownership-group downline pace from the Freedom Sheet community tracker. Keep this separate from active productive Benson Crew agent capacity.',
    metadata: { status: 'ahead', evidenceRefs: ['intelligence_synthesis_facts:fact:18d41cc725fa1afb29009d82'] },
    minTier: 1,
    sensitivity: 'neutral',
    status: 'active',
  }
  ...
  const candidate = selectFreedomCommunityAtomFlowRepairCandidate({ beforeRow, sourceFacts: [fact] })
  const atomBuild = buildFreedomCommunityAtomFromFact(candidate, '2026-05-18T06:15:00.000Z')
  const evaluation = evaluateFreedomCommunityAtomFlowRepair({
    beforeRow,
    afterRow,
    candidate,
    atomBuild,
  ...
  const missingFact = selectFreedomCommunityAtomFlowRepairCandidate({ beforeRow, sourceFacts: [] })
  const tierBlocked = selectFreedomCommunityAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, minTier: 4 }] })
  return {
    ok: candidate.ok &&
      atomBuild.ok &&
      evaluation.ok &&
...
export function renderFreedomCommunityAtomFlowRepairCloseout(snapshot = {}) {
  const summary = snapshot.summary || {}
  const lines = []
  lines.push('# Source Maturity Freedom Community Atom Flow Repair Closeout')
  lines.push('')
  lines.push(`Card: \`${SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_CARD_ID}\``)
  lines.push(`Closeout key: \`${SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\``)
  lines.push('')
  lines.push('## What Shipped')
  lines.push('')
  lines.push(`- Promoted one source-backed atom for \`${summary.sourceId || SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID}\` from an existing Freedom Community source fact.`)
  lines.push(`- Saved atom: \`${summary.atomId || 'pending atom'}\`.`)
  lines.push(`- Source fact evidence: \`${summary.sourceFactId || SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_TARGET_FACT_ID}\`.`)
  ...
  lines.push('')
  for (const boundary of SOURCE_MATURITY_FREEDOM_COMMUNITY_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES) lines.push(`- ${boundary}`)
  lines.push('')
  lines.push('## Next')
  lines.push('')
