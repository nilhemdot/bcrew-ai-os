...
export async function buildAgentFeedbackCompanyEmailPolicyStatus({
  ...
  addFinding(findings, steveProof.eligibility?.eligible === true, 'Steve Zahnd dry-run is eligible', steveProof.eligibility?.blockers?.join(', ') || 'eligible')
  ...
  addFinding(findings, georgiaProof.eligibility?.eligible === true && georgiaProof.recipientPlan?.recipientSource === 'company_email', 'Georgia dry-run remains eligible through Company Email but is not test target', georgiaProof.eligibility?.blockers?.join(', ') || 'eligible')
  ...
  addFinding(findings, steveLoopCard?.lane === 'scoped', 'Steve full-loop test card is scoped next', steveLoopCard?.lane || 'missing')
  addFinding(findings, productionCard?.lane === 'scoped', 'production auto-send enable card is scoped later', productionCard?.lane || 'missing')
