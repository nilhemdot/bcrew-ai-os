...
export const AGENT_ONBOARDING_FEEDBACK_SYSTEM_BASELINE_PATH = 'docs/audits/2026-04-30-agent-onboarding-feedback-system-baseline.md'
export const AGENT_ONBOARDING_FEEDBACK_SYSTEM_MANUAL_REVIEW_PATH = 'docs/audits/2026-04-30-agent-onboarding-feedback-system-manual-review.md'
...
function buildKnownCaseProof(ownersReviewQueue) {
  ...
  return {
    ...
    chrisNotDue: chrisItems.length === 0 || chrisItems.every(item => item.reviewStatus !== 'Feedback Due'),
  }
}
...
export async function buildAgentOnboardingFeedbackSystemStatus({
  ...
  addFinding(findings, foundationVerifySource.includes(AGENT_ONBOARDING_FEEDBACK_SYSTEM_CARD_ID), 'foundation verifier covers agent onboarding feedback system card', AGENT_ONBOARDING_FEEDBACK_SYSTEM_CARD_ID)
  addFinding(findings, systems.length === FOUNDATION_SYSTEMS_APPROVED_GROUPED_SYSTEM_COUNT, 'groupedSystems count moved from 12 to 13', `${systems.length}/${FOUNDATION_SYSTEMS_APPROVED_GROUPED_SYSTEM_COUNT}`)
  ...
  addFinding(findings, agentSystem?.implementationState === 'partial', 'Agent Onboarding Feedback implementation state is partial', agentSystem?.implementationState || 'missing')
  ...
  addFinding(findings, includesAll(agentText, AGENT_ONBOARDING_FEEDBACK_SYSTEM_REQUIRED_CONTEXT), 'system record contains required onboarding feedback context', AGENT_ONBOARDING_FEEDBACK_SYSTEM_REQUIRED_CONTEXT.filter(phrase => !agentText.includes(phrase)).join(', '))
  ...
  addFinding(findings, includesAll(sourceContractsSource, [
    ...
    'implementationState: \'partial\'',
    'agent_onboarding_feedback_responses',
  ]), 'source contract contains partial system and response table reference', 'lib/source-contracts.js')
  addFinding(findings, knownCases.openItems > 0 && knownCases.totalTrackedRows > 0, 'Agent Roster review queue is live', `open=${knownCases.openItems} tracked=${knownCases.totalTrackedRows}`)
  addFinding(findings, knownCases.georgiaDue, 'Georgia metadata-only due proof exists', knownCases.georgiaDue ? `${knownCases.georgiaTaskId}:day-${knownCases.georgiaMilestone}:${knownCases.georgiaDueDate}` : 'missing')
  addFinding(findings, knownCases.chrisNotDue, 'Chris metadata-only non-fire proof exists', 'not due or not present in due queue')
  ...
  if (foundationHub) {
    ...
    ownerCard = backlogItems.find(card => card.id === AGENT_ONBOARDING_FEEDBACK_SYSTEM_CARD_ID) || null
    sendCard = backlogItems.find(card => card.id === AGENT_FEEDBACK_SEND_CARD_ID) || null
    emptyAuditCard = backlogItems.find(card => card.id === AGENT_ONBOARDING_FEEDBACK_SYSTEM_EMPTY_AUDIT_CARD_ID) || null
    ...
    addFinding(findings, sendCard?.lane === 'scoped', 'send-path card remains scoped', sendCard?.lane || 'missing')
  ...
  const closeout = normalizeList(foundationBuildLog?.builds).find(build => build.closeoutKey === AGENT_ONBOARDING_FEEDBACK_SYSTEM_CLOSEOUT_KEY) || null
  ...
  return {
    ...
    summary: {
      ...
      groupedSystemCountAfter: FOUNDATION_SYSTEMS_APPROVED_GROUPED_SYSTEM_COUNT,
