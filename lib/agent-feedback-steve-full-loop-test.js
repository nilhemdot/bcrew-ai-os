...
export async function buildAgentFeedbackSteveFullLoopTestStatus({
  ...
  const repairCloseout = (foundationBuildLog?.builds || []).find(build => build.closeoutKey === AGENT_FEEDBACK_REAL_USER_SUBMIT_REPAIR_CLOSEOUT_KEY) || null
  ...
  addFinding(productionCard?.lane === 'scoped', 'production auto-send card remains scoped after repair gate', productionCard?.lane || 'missing')
