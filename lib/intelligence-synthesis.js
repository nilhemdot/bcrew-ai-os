...
function classifyCluster({ facts = [], sourceSupportFacts = [], evidenceRefs = [], sourceIds = [], topicRefs = [] } = {}) {
  ...
  const strategyHubEligible = !operationalOnly && (
    hasGoalGapFact ||
    multiSourcePattern ||
...
function buildThemeTitle({ classification, topicRefs, facts }) {
  ...
  if (classification.routeScope === 'strategy') return `Review repeated ${themeLabel} signal`
...
export function createIntelligenceSynthesisStore({
        ...
        [
          ...
          JSON.stringify({
            ...
            humanSample: {
              ...
              rows: humanSampleRowsForItems(items, 5),
            },
            qualityThresholds: {
  ...
  async function getSynthesisEngineSnapshot({ limit = 20 } = {}) {
    ...
    const [summary, latestRun, latestProofRun, recentItems, sourceSummary, referenceIntegrity, latestProofQuality] = await Promise.all([
      pool.query(`
        SELECT
      ...
      pool.query(`
        ...
        proof_items AS (
          SELECT item.*
          FROM intelligence_synthesized_items item
          JOIN latest_proof proof ON proof.run_id = item.run_id
          WHERE item.status != 'archived'
        ),
        ...
        SELECT
          COUNT(*)::int AS latest_proof_active_items,
          COUNT(*) FILTER (WHERE COALESCE(attributes->>'synthesisQuality', '') = 'clustered')::int AS latest_proof_clustered_items,
          COUNT(*) FILTER (WHERE COALESCE(attributes->>'themeKey', '') != '')::int AS latest_proof_items_with_theme_metadata,
          COUNT(*) FILTER (WHERE COALESCE(metadata->>'strategyHubEligible', attributes->>'strategyHubEligible') = 'true')::int AS latest_proof_strategy_eligible_items,
          COUNT(*) FILTER (
          ...
          )::int AS latest_proof_strategy_items_with_multi_evidence,
          COUNT(*) FILTER (
            WHERE COALESCE(metadata->>'strategyHubEligible', attributes->>'strategyHubEligible') = 'true'
              AND (cardinality(evidence_refs) < 2 OR cardinality(evidence_chunk_refs) < 2)
          )::int AS latest_proof_strategy_single_evidence_items,
          COUNT(*) FILTER (WHERE cardinality(evidence_refs) >= 2)::int AS latest_proof_items_with_multi_evidence,
          COALESCE((SELECT COUNT(*) FROM duplicate_theme_keys), 0)::int AS latest_proof_duplicate_theme_keys
        FROM proof_items
      `),
    ])
    const latestProofRunRow = latestProofRun.rows[0] ? mapRunRow(latestProofRun.rows[0]) : null
    const latestProofRunMetadata = latestProofRunRow?.metadata || {}
    ...
    return {
      ...
      latestProofQuality: {
        activeItems: Number(latestProofQualityRow.latest_proof_active_items || 0),
        clusteredItems: Number(latestProofQualityRow.latest_proof_clustered_items || 0),
        itemsWithThemeMetadata: Number(latestProofQualityRow.latest_proof_items_with_theme_metadata || 0),
        strategyEligibleItems: Number(latestProofQualityRow.latest_proof_strategy_eligible_items || 0),
        strategyItemsWithMultiEvidence: Number(latestProofQualityRow.latest_proof_strategy_items_with_multi_evidence || 0),
        strategySingleEvidenceItems: Number(latestProofQualityRow.latest_proof_strategy_single_evidence_items || 0),
        itemsWithMultiEvidence: Number(latestProofQualityRow.latest_proof_items_with_multi_evidence || 0),
        duplicateThemeKeys: Number(latestProofQualityRow.latest_proof_duplicate_theme_keys || 0),
        humanSampleRows: Array.isArray(latestProofRunMetadata.humanSample?.rows)
          ? latestProofRunMetadata.humanSample.rows.length
          : 0,
      },
