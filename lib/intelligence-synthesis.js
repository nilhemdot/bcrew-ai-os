...
function sentenceFromValue(value = '', fallback = '') {
  const normalized = String(value || '').replace(/\s+/g, ' ').trim()
  if (!normalized) return fallback
  const [first] = normalized.split(/(?<=[.!?])\s+/)
  return shorten(first || normalized, 240)
}
...
function evidenceStrengthLabel({ evidenceRefs = [], evidenceChunkRefs = [], sourceIds = [] } = {}) {
  if (sourceIds.length >= 2 && evidenceRefs.length >= 3 && evidenceChunkRefs.length >= 3) return 'strong'
  if (evidenceRefs.length >= 2 && evidenceChunkRefs.length >= 2) return 'solid'
  return 'early'
}
...
function buildThemeTitle({ classification, topicRefs, facts }) {
  ...
  if (facts.length > 1) return `Review ${themeLabel} signal`
...
function buildThemeSummary({ facts, sourceSupportFacts, evidenceRefs, evidenceChunkRefs, sourceIds, classification }) {
  ...
  const lead = claims.length
    ? sentenceFromValue(claims[0])
    : `The system found a ${classification.routeScope} signal that needs review.`
  const evidenceLine = `Proof: ${countLabel(sourceIds.length, 'source')}, ${countLabel(evidenceRefs.length, 'evidence item')}, and ${countLabel(evidenceChunkRefs.length, 'retrievable proof chunk')}.`
  return shorten([
    facts.length > 1
      ? `${facts.length} related signals point to this: ${lead}`
      : lead,
    evidenceLine,
    support.length ? `Supporting truth: ${support.map(value => sentenceFromValue(value)).join(' / ')}` : '',
...
function buildSynthesizedItems({ runId, synthesisScopeKey, facts, evidence, maxTier, limit = 8 }) {
  ...
  const items = Array.from(clusters.values())
    .map(cluster => {
      ...
      const evidenceStrength = evidenceStrengthLabel({ evidenceRefs, evidenceChunkRefs, sourceIds })
      ...
      ownerAction: ownerDecision.owner
        ? `Confirm ${ownerDecision.owner} owns this ${classification.routeScope} signal, then approve the route with the attached proof.`
        : `Choose the right owner before this ${classification.routeScope} signal can become work.`,
      sourceIds,
      factRefs,
      evidenceRefs,
      evidenceChunkRefs,
      ...
      attributes: {
        ...
        actionReadiness: ownerDecision.owner ? 'route-ready' : 'needs-owner',
        ...
        decisionQuestion: ownerDecision.owner
          ? `Should ${ownerDecision.owner} take this on now?`
          : 'Who owns this before it becomes work?',
        rankingPolicy: 'operator-readable-evidence-first',
      },
      metadata: {
...
export function buildIntelligenceSynthesisOperatorDogfood() {
  const runId = 'synthesis-operator-dogfood'
  const sourceId = 'SRC-DOGFOOD-INTEL-001'
  const facts = [
    {
      factId: 'dogfood-fact-1',
      factType: 'retrieved_evidence',
      sourceId,
      sourceIds: [sourceId],
      evidenceId: 'dogfood-evidence-1',
      atomId: 'dogfood-atom-1',
      candidateKey: 'dogfood-candidate-1',
      artifactId: 'dogfood-artifact-1',
      title: 'Agent handoff keeps losing state',
      claim: 'The agent workflow needs a stronger handoff and checkpoint layer before scaling.',
      detail: 'A repeated build signal points to state loss during long-running agent work.',
      status: 'active',
      minTier: 1,
      sensitivity: 'neutral',
    ...
    {
      factId: 'dogfood-fact-2',
      factType: 'retrieved_evidence',
      sourceId,
      sourceIds: [sourceId],
      evidenceId: 'dogfood-evidence-2',
      atomId: 'dogfood-atom-2',
      candidateKey: 'dogfood-candidate-2',
      artifactId: 'dogfood-artifact-2',
      title: 'Agent handoff needs checkpoint proof',
      claim: 'Long-running build workflows should save state after each meaningful step.',
      detail: 'The same source signal supports checkpointed state passing.',
      status: 'active',
      minTier: 1,
      sensitivity: 'neutral',
      asOf: new Date().toISOString(),
    ...
    results: [
      {
        ...
        candidateKey: 'dogfood-candidate-1',
        artifactId: 'dogfood-artifact-1',
        chunkId: 'dogfood-chunk-1',
        matchedBy: ['atom', 'lexical'],
        evidenceType: 'dogfood',
      },
      {
        evidenceId: 'dogfood-evidence-2',
        atomId: 'dogfood-atom-2',
  ...
  const items = buildSynthesizedItems({
    runId,
    synthesisScopeKey: 'intelligence-spine-god-mode-dogfood',
    facts,
    evidence,
    maxTier: 1,
    limit: 5,
  ...
  const [item] = items
  const internalJargonPattern = /collapsed evidence|atom\/evidence ref|retrieval chunk|ordered-for-review-without-weighted-score/i
  return {
    ok: Boolean(item) &&
      item.metadata?.operatorReadable === true &&
      item.attributes?.actionReadiness &&
      !internalJargonPattern.test(`${item.title} ${item.summary} ${item.ownerAction}`),
    item,
    checks: [
      {
        ok: Boolean(item),
        check: 'dogfood produced a synthesized item',
      },
      {
        ok: item?.metadata?.operatorReadable === true,
        check: 'synthesis marks operator-readable output',
      },
      {
        ok: Boolean(item?.attributes?.actionReadiness),
        check: 'synthesis emits action readiness',
      },
      {
        ok: item ? !internalJargonPattern.test(`${item.title} ${item.summary} ${item.ownerAction}`) : false,
        check: 'synthesis avoids internal/hash-heavy jargon in operator copy',
        detail: item ? `${item.title} | ${item.summary}` : 'no item',
      },
    ],
  }
}
