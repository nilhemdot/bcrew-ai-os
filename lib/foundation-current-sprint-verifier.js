...
export function evaluateFoundationCurrentSprintVerifier(input = {}) {
  ...
  const foundationSprintSurfaceFollowUpOk =
    ...
    (foundationSprintSurfaceFollowUp?.lane === 'done' &&
      cardText(foundationSprintSurfaceFollowUp).includes('foundation-surface-updates-v1'))
  ...
  addCheck(
    ...
    foundationSprintSystem?.lane === 'done' &&
      ...
      (foundationFrontendSource.includes('hub.currentSprint') || foundationFrontendSource.includes('currentSprintPayload.currentSprint')) &&
      ...
      foundationCurrentSprintStatus.status === 'healthy' &&
      foundationHub.currentSprint?.status === 'healthy' &&
      foundationSprintSurfaceFollowUpOk &&
