...
export function evaluateFoundationCurrentSprintVerifier(input = {}) {
  ...
  addCheck(
    ...
    foundationSprintCadence?.lane === 'done' &&
      ...
      currentSprintReviewNextActionOk(foundationHub, input.activeSprintCompleteReview === true) &&
      (foundationHub.currentSprint?.cadence?.currentBlocker?.cardId || input.activeSprintCompleteReview === true) &&
...
export function buildFoundationCurrentSprintVerifierDogfoodProof() {
  ...
  return {
    ...
    missingBuildLog,
    missingDriveGuard,
    dogfoodInvariant: 'Current Sprint verifier accepts healthy sprint proof and rejects unhealthy API/status, missing doctrine markers, missing build-log ownership, and missing Drive/Meeting Vault stop-lines.',
  }
}
