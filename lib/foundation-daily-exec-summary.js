...
export function buildFoundationDailyExecSummary({
  ...
  const latestFiveBuilds = sortedBuilds.slice(0, DAILY_EXEC_SUMMARY_LATEST_BUILD_MINIMUM)
...
export async function buildDailyExecSummaryStatus({
  ...
  addFinding(findings, serverSource.includes("app.get('/api/foundation/daily-summary'"), 'daily summary API route exists', DAILY_EXEC_SUMMARY_API_PATH)
  ...
  addFinding(findings, foundationUi.includes('fetchFoundationDailySummary'), 'UI fetches daily summary API', 'fetchFoundationDailySummary')
  addFinding(findings, foundationUi.includes('renderDailySummary'), 'daily summary renderer exists', 'renderDailySummary')
  addFinding(findings, foundationUi.includes('data-daily-summary-section'), 'daily summary UI carries section proof selectors', 'data-daily-summary-section')
  ...
  if (dailySummary) {
    ...
    addFinding(findings, summary.stillOpenCount > 0, 'still open bucket is separate and populated', String(summary.stillOpenCount || 0))
    ...
    addFinding(findings, summary.nextBuildCount > 0, 'next build bucket is separate and populated', String(summary.nextBuildCount || 0))
