...
export const DAILY_EXEC_SUMMARY_BASELINE_PATH = 'docs/_archive/audits/2026-04-30-daily-exec-summary-baseline.md'
export const DAILY_EXEC_SUMMARY_MANUAL_REVIEW_PATH = 'docs/_archive/audits/2026-04-30-daily-exec-summary-manual-review.md'
...
function foundationHubCardCompleted(foundationHub = {}, cardId = '', closeoutKey = '') {
  const phaseGReadiness = foundationHub.foundation1100Review?.phaseGReadiness || {}
  if (normalizeList(phaseGReadiness.completedCards).includes(cardId)) return true
  const card = normalizeList(foundationHub.backlogItems).find(item => item.id === cardId)
  if (!card || card.lane !== 'done') return false
  if (!closeoutKey) return true
  return new RegExp(closeoutKey).test(card.statusNote || '')
}
...
export function buildFoundationDailyExecSummary({
  ...
  const latestDailyCloseoutRepresented =
    allBuildItems.some(item => item.closeoutKey === DAILY_EXEC_SUMMARY_CLOSEOUT_KEY) ||
    foundationHubCardCompleted(foundationHub, DAILY_EXEC_SUMMARY_CARD_ID, DAILY_EXEC_SUMMARY_CLOSEOUT_KEY)
...
export async function buildDailyExecSummaryStatus({
  ...
  const foundationSourceRegistry = await readOptionalText(repoRoot, 'public/foundation-source-registry-renderers.js')
  const foundationFubLeadSource = await readOptionalText(repoRoot, 'public/foundation-fub-lead-source-renderers.js')
  const foundationSystemInventory = await readOptionalText(repoRoot, 'public/foundation-system-inventory-renderers.js')
  const foundationCurrentState = await readOptionalText(repoRoot, 'public/foundation-current-state-renderers.js')
  const foundationDecisionQuestion = await readOptionalText(repoRoot, 'public/foundation-decision-question-renderers.js')
  const foundationSourceLifecycle = await readOptionalText(repoRoot, 'public/foundation-source-lifecycle-renderers.js')
  ...
  const foundationFrontend = `${foundationUi}\n${foundationData}\n${foundationSourceRegistry}\n${foundationFubLeadSource}\n${foundationSystemInventory}\n${foundationCurrentState}\n${foundationDecisionQuestion}\n${foundationSourceLifecycle}\n${foundationRuntime}\n${foundationOperations}`
  ...
  const phaseGTrack2Complete = !phaseGReadiness.nextPlanCard &&
    foundationHubCardCompleted(
      foundationHub,
      DAILY_EXEC_SUMMARY_PHASE_G_COMPLETION_CARD_ID,
      DAILY_EXEC_SUMMARY_PHASE_G_COMPLETION_CLOSEOUT_KEY,
    )
