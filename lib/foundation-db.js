...
const backlogSeed = [
  ...
  },
  {
    ...
    statusNote: 'Scoped by AGENT-FEEDBACK-SEND-001. This Stage 2 card owns the first real Georgia test send only. Internal/team recipients like Georgia use ClickUp Company Email as the To source; external agents default to ClickUp Personal Email. Onboarding feedback sends use BCC/internal oversight roles Steve, Carson, Ryan, and Georgia by default; if Georgia is To, her BCC address is deduped so she does not receive two copies. The readiness dry-run must show recipientSource metadata and BCC/internal oversight metadata only: company_email or personal_email, field name/hash, present/valid booleans, bccRolesApplied, bccMissingConfiguredRoles only when an approved internal identity is unavailable, and no raw email addresses. It must not run without explicit SEND APPROVED. Proof must remain metadata-only except Gmail message/thread IDs and ClickUp Requested writeback confirmation after an approved send; no token URLs, raw email addresses, or feedback content in tracked proof.',
  },
  {
