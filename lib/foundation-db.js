...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'Treat Skool as a future Foundation corpus source, not just a one-off research target. The system should eventually crawl courses, trainings, posts, comments, links, docs, and video lessons, then extract reusable operating patterns, training assets, content atoms, and course-building material with evidence links.',
    whyItMatters: 'Skool may contain high-value training and community material, but it will become noise if every chat manually browses it from scratch. It needs the same supervised crawl model as Drive: cursor state, inspected artifacts, transcript/video handling, extraction status, source links, and synthesis handoff.',
    nextAction: 'Validate the access path first: official Skool API if available, export path if available, or supervised browser crawler if not. Start with the blueprint kit and Build-Your-Own ClaudeClaw course, tag findings as reusable now / useful later / platform-specific / rejected, and route good patterns into existing backlog cards instead of spawning a second architecture track.',
  ...
  },
  {
    ...
    whyItMatters: 'The business has too much historical data for manual one-off backfills. The productized AI OS also cannot require a human to sit with Codex while it crawls a customer’s history. The system needs durable crawl jobs that take one safe bite per source per day, record what happened, and hand usable evidence to synthesis/scopers.',
    nextAction: 'Design the run model: source id, job type (`current_sync` / `historical_crawl`), cursor/checkpoint, artifact inventory, archive/mirror action, extraction action, synthesis handoff, cost, errors, retry policy, and human-review queue. Then build the first runner around existing Gmail/Missive/Drive/Zoom scripts.',
    ...
    const result = await client.query(
      `
        ...
        ON CONFLICT (target_key, external_id) DO UPDATE SET
          ...
          lease_owner = EXCLUDED.lease_owner,
          lease_expires_at = EXCLUDED.lease_expires_at,
          attempt_count = GREATEST(source_crawl_items.attempt_count, EXCLUDED.attempt_count),
          last_error = EXCLUDED.last_error,
