...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Confirm Slack access on this machine, identify the exact channels that matter as shared cross-hub context starting with the accountability chat, and define the rollout rule: bot/user presence, required channel invites, archive boundary, and whether Slack is workflow-critical, optional, or future-only.',
    statusNote: 'Slack is still not wired in the rebuild. When it lands, it needs an explicit channel rollout list instead of a generic source contract note so new chats like accountability do not get missed.',
  },
  {
    id: 'SOURCE-006',
  ...
  },
  {
    ...
    nextAction: 'Use `docs/source-notes/freedom-marketing.md` to define, for each brand lane and each pillar, which source system owns the truth, which parts remain manual, which current rebuild source contracts already exist, and which missing source contracts need to be added before a real marketing performance surface is built.',
    statusNote: 'This is not a dashboard-build card yet. It is the foundation source-boundary card that should exist before the future marketing rebuild starts.',
  },
  {
    id: 'SOURCE-017',
  ...
  },
  {
    ...
    nextAction: 'Keep the verified Google meeting-note read path as the source boundary, then deepen the archive contract: meeting identity, participants, transcript body, task candidates, decision candidates, and linkage into governed review workflows.',
    statusNote: 'Delegated meeting-note reads are now live in the rebuild: Gemini-note search on `ai@bensoncrew.ca`, archive-root access on `crewbert@bensoncrew.ca`, and doc export to text. The remaining work is governed archive depth and extraction, not basic access.',
  },
  {
    id: 'SOURCE-019',
    ...
    nextAction: 'Use the now-live Gmail / Missive / meeting archive as the first slice, then normalize those records across more shared-communications surfaces and add the next governed extraction path after the first candidate lane proves out.',
    statusNote: 'First slice is live: Gmail threads, Missive conversations, meeting notes, and meeting transcripts land in the shared communications archive, and transcript-first extraction produces pending governed candidates with Foundation context. Remaining work is broader normalization, approval/apply flow, more source coverage, and additional extraction types.',
  },
  {
    id: 'SOURCE-020',
    ...
    nextAction: 'Keep the now-live Gmail / Calendar / Drive / meeting-note readers and Missive bridge stable, then harden broader Drive scope and revalidate Slack as read-only before widening the normalized shared-communications layer.',
    statusNote: 'The port-and-harden path is working: Gmail, Calendar, Drive search/list/export, meeting-note reads, and Missive thread reads are live in the rebuild. Remaining adapter work is Slack plus wider Drive / normalization hardening, not blank-sheet connector work.',
  },
  {
    id: 'SOURCE-021',
    ...
    nextAction: 'Get access to Lee’s original middleware / database logic, trace the rules for `leaddate`, `leadclaimeddate`, re-entry, and counted-opportunity semantics, then turn those rules into one governed source note and one explicit AI read contract for FUB / KPI work.',
    statusNote: 'This is not a blocker for the current FUB baseline, but it is required before calling the deeper opportunity semantics fully closed.',
  },
  {
    id: 'SOURCE-012',
  ...
  try {
    ...
    await client.query(
      ...
      [
        'SOURCE-018',
        'scoped',
        'Keep the verified Google Meet read boundary tight, then deepen the archive contract around meeting identity, participants, transcript body, candidate extraction, and linkage into governed review workflows.',
        'Delegated meeting-note reads are now live in the rebuild: Gemini-note search on ai@, transcript-path detection from standalone transcript docs or embedded transcript sections, archive-root access on crewbert@, and doc export to text. Remaining work is governed archive depth and approval/apply flow, not basic access.',
      ]
    )
    ...
    await client.query(
      ...
      [
        'SOURCE-019',
        'scoped',
        'Use the now-live Gmail / Missive / meeting archive as the first slice, then normalize those records across more shared-communications surfaces and add the next governed extraction path after the first candidate lane proves out.',
        'First slice is live: Gmail threads, Missive conversations, meeting notes, and meeting transcripts land in the shared communications archive, and transcript-first extraction produces pending governed candidates with Foundation context. Remaining work is broader normalization, approval/apply flow, more source coverage, and additional extraction types.',
      ]
    )
    ...
    await client.query(
      ...
      [
        'SOURCE-020',
        'Keep the now-live Gmail / Calendar / Drive / meeting readers, Gmail archive sync, and Missive bridge/archive stable, then revalidate Slack as read-only before widening the normalized shared-communications layer.',
        'The port-and-harden path is working: Gmail, Calendar, Drive search/list/export, meeting reads, Gmail thread archive, and Missive thread reads/archive are live in the rebuild. Remaining adapter work is Slack plus wider Drive / normalization hardening, not blank-sheet connector work.',
      ]
    )
