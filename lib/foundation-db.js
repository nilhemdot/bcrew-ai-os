...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'After the real-user Steve browser submission repair passes, enable production-all auto-send for eligible 30/60/90 onboarding feedback requests through the governed two-key approval path.',
    ...
    nextAction: 'Stage 1 production dry-run report is complete. Steve reviews the would-send list next. Do not enable production, write a production approval artifact, send Gmail, or write ClickUp Requested until Steve separately approves production.',
    statusNote: 'Stage 1 completed on 2026-05-01 under agent-feedback-production-autosend-dry-run-v1, but this card is not fully done and production is not enabled. Stage 1 scans the full ClickUp Agent Roster for 30/60/90 candidates, uses Company Email only, treats Contract Link as warning-only, checks current ClickUp NPS status, send window, duplicate-send ledger, response state, and reminder state, and classifies candidates as would_send, already_requested, completed, outside_window, blocked, skipped, or warning. Proof is metadata-only and contains no raw emails, token URLs, or feedback content. No Gmail send, ClickUp Requested writeback, env toggle, production approval artifact, Georgia special send, roster send, or production enablement happened.',
  },
  {
  ...
  },
  {
    ...
    nextAction: 'After the GLS dashboard review settles, design a thin Sales Dashboard v1: top GROW KPI card, driver tree/system branches, metadata-only signal counts, and links into GLS Manager and future driver surfaces. Do not build a CRM, agent coach, broad source expansion, or write/apply layer in this card.',
    statusNote: 'Captured as a future Sales Dashboard idea only. This is not part of the current GLS scoreboard/menu cleanup and should not be implemented until Steve explicitly pulls the Sales Dashboard v1 slice forward.',
  },
  {
    id: 'RETAIN-001',
    ...
    return [
      {
        id: `gls-case-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        at,
        title: 'Current state captured',
        source,
        actor: salesHistoryActorLabel(actor),
        note: normalizeSalesHistoryText(existing.actionPlanNoReason || existing.actionPlanText || 'History started from existing GLS case state.', 500),
  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        'After the real-user Steve browser submission repair passes, enable production-all auto-send for eligible 30/60/90 onboarding feedback requests through the governed two-key approval path.',
        ...
        'Stage 1 production dry-run report is complete. Steve reviews the would-send list next. Do not enable production, write a production approval artifact, send Gmail, or write ClickUp Requested until Steve separately approves production.',
        'Stage 1 completed on 2026-05-01 under agent-feedback-production-autosend-dry-run-v1, but this card is not fully done and production is not enabled. Stage 1 scans the full ClickUp Agent Roster for 30/60/90 candidates, uses Company Email only, treats Contract Link as warning-only, checks current ClickUp NPS status, send window, duplicate-send ledger, response state, and reminder state, and classifies candidates as would_send, already_requested, completed, outside_window, blocked, skipped, or warning. Proof is metadata-only and contains no raw emails, token URLs, or feedback content. No Gmail send, ClickUp Requested writeback, env toggle, production approval artifact, Georgia special send, roster send, or production enablement happened.',
      ]
    )
