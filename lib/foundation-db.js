...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Define the appointment-quality rules from the KPI How To page and `appointmentUtils.ts`, then build a read-only audit that flags likely 60-90 day same-person stacking, missing outcomes, and non-standard outcomes by agent. Treat buy+sell pairs, multiple properties, and separate deal paths as legitimate exceptions before coaching a correction.',
    statusNote: 'Promoted as a Sales Hub data-quality gate. Multiple close appointments should trigger a question, not an accusation: if it is one opportunity with repeated meetings, coach the agent to move/update the original appointment and outcome; if it is buy+sell or multiple properties, preserve separate outcome tracks.',
  },
  {
    ...
    summary: 'FUB automatically creates lead-stage records when people are added. If agents add realtors, vendors, support-network contacts, duplicates, or other non-leads and do not move them out quickly, KPI can count fake leads. Live check found 13005 active lead-stage rows, including 4497 source `Import`, 1647 source `<unspecified>`, and 2 null-source rows. Founder clarification: `Import`, `<unspecified>`, generic `Sphere`, `SOI`, and similar placeholders are not validated final lead sources.',
    ...
    nextAction: 'Map the KPI lead-list/source-error views and FUB stage/source rules into an AIOS read-only audit for unvalidated leads, invalid/generic sources, non-lead contacts left in lead stages, and stale temporary cleanup states. Use the governed FUB source doctrine and guided correction flow: ask whether the source was met in person, met social, family, referral, introduction, or another governed source, then collect secondary details and Ground Zero.',
    statusNote: 'Promoted as a Sales Hub data-quality gate. Generic source values should trigger guided correction, not acceptance: for referral/introduction, identify the origin person and connect/add them in FUB when permissions exist; for met in person/social, capture where/platform in secondary lead-source info.',
  },
  {
    id: 'KPI-SHOPPING-001',
