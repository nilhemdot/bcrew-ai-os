...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Treat `SRC-OWNERS-001` and `SRC-FINANCE-001` current-reality meaning as closed. Use `DATA-005`, `DATA-006`, `DATA-007`, `DATA-008`, and `DATA-009` for the remaining Owners parity and quality work.',
  ...
  },
  {
    ...
    nextAction: 'Promote the auth/tier/vault spec into implementation: authenticated user/tier attachment, `assertTier` checks, subject_people + sensitivity tagging, uniform redacted response shape, owner-preserving meeting-note ACL policy, and filtered-summary access-request flow.',
    statusNote: 'P0 Foundation gate. Admin-only proof endpoints may stay private, but no broad hub, assistant, query, or human-facing read surface can expose shared-communications intelligence until this is implemented and verified.',
  },
  {
    id: 'SECURITY-003',
  ...
  },
  {
    ...
    statusNote: '`llm_credentials`, `llm_routes`, `llm_route_probes`, and `llm_calls` exist. Missing piece is the explicit hub-capacity policy and dashboard-facing capacity view.',
  },
  {
    ...
    nextAction: 'Define the capacity registry shape for Foundation, Strategy, Ops, Marketing, Sales, Recruiting, Agent Hub, Steve Zahnd, MarketMasters, and Unchained; include owner account, provider, allowed workloads, monthly budget, pacing, fallback, and stop controls.',
    statusNote: 'Doctrine is locked: Foundation + router + adapters. OpenClaw and Claude Code are plugs, not the OS; subscriptions are internal capacity, not the product backend.',
  },
  {
    id: 'LLM-ROUTER-001',
    ...
    statusNote: 'OpenClaw/Codex adapter and guarded OpenAI fallback are live for shared-comms extraction/synthesis. Claude Code adapter, hub capacity lanes, and broader workload migration remain open.',
  },
  {
    id: 'FOUNDATION-003',
  ...
  },
  {
    ...
    priority: 'P0',
    rank: 21,
    source: 'Owners closeout + governed dropdown repair',
    summary: 'Treat the Admin-tab Lead Source and Ground Zero dropdowns as governed FUB-compatible source surfaces backed by the upstream Lists source, not by local helper lists in the Owners Dashboard mirror.',
    whyItMatters: 'The Owners Dashboard `Lists` tab is an IMPORTRANGE mirror. Writing the governed list into the mirror blocked the imported agent roster/cap/list data and broke downstream deal-data formulas. The dropdown must be governed from the upstream source sheet.',
    nextAction: 'Keep `SRC-OWNERS-LISTS-001` as the writable source for the governed lead-source list, keep Admin N/P validations on `=Lists!$J$3:$J`, and keep the imported-range write guard plus sheet verifier active.',
    statusNote: 'Done after 2026-04-24 repair. The governed FUB list now lives in upstream source `Lists!J3:J`, `No Extra Lead Source` is included in the same list, Owners Admin N/P reuse that list, Admin S uses imported AA active agents, and direct writes to Owners `Lists!A:AI` are blocked.',
  },
  {
    ...
    statusNote: 'Done for the Owners Dashboard Lists mirror. `lib/google-delegated.js` blocks values writes and raw batchUpdate writes that overlap the protected mirror range, and `owners:repair-lists` is the controlled repair path.',
  },
  {
    id: 'DATA-020',
  ...
  },
  {
    ...
    summary: 'Create the live operational view for agent and routine runtime state: scheduled, running, degraded, paused, queued, awaiting approval, recently changed, and recently failed.',
    whyItMatters: 'An agent registry is not enough. Humans also need to know what is happening now. The old dashboard had the right instinct with activity, schedule, and status, but the new system should separate static contract from live operations so supervision stays clear instead of noisy. This now applies to code-owned routines too: FUB health, deal review, shared-comms sync, transcript gaps, synthesis, Drive crawl, and Skool crawl.',
    nextAction: 'Define the smallest useful operations surface: routine/agent name, status, schedule, last successful run, next run, current queue, blocked approvals, degraded reasons, recent outputs, recent change events, and last error. Keep it live and operational, not a vanity telemetry board.',
    statusNote: 'Start simple. This is supervision and trust, not a giant NOC dashboard. The immediate product need is seeing which built routines exist but are not actually running.',
  },
  {
    id: 'AGENT-008',
  ...
  },
  {
    ...
    statusNote: 'First slice is live through Foundation job registry, job runs, runtime controls, LaunchAgents, and source target snapshots. Remaining work is the complete activation-state model and decommission/manual-only coverage across every built routine.',
  },
  {
  ...
  },
  {
    ...
    nextAction: 'Document and surface LaunchAgent status, stable log paths, restart behavior, dead-man checks, stop/decommission controls, and cost/process warnings. Tie the remaining work to `SYSTEM-010` instead of scattering it across chats.',
    statusNote: 'Dashboard and Foundation worker LaunchAgents are live. Remaining work is dead-man visibility, operator runbook, decommission controls, and cost/process alarms before autonomous loops expand.',
  },
  {
    ...
    nextAction: 'Keep the first set small, prove 2-3 clean cycles per scheduled lane, keep meeting/Drive/Skool/video lanes manual until retry/reporting are hardened, and record why each routine is scheduled, manual-only, paused, or blocked.',
    statusNote: 'Foundation verify, worker, deal review, and current-day Gmail/Missive lanes have first runtime proof. The remaining closeout is paced monitoring and explicit manual-only status for higher-variance miners.',
  },
  {
    id: 'SKOOL-001',
    ...
    priority: 'P2',
    rank: 44,
    source: 'Mark Kashef Skool access + founder corpus-crawl discussion',
    summary: 'Treat Skool as a future Foundation corpus source only after the access and content-use boundary is explicit. The system should inventory allowed communities, courses, trainings, posts, comments, links, docs, Loom/YouTube/Vimeo/Wistia lessons, and paid material only through approved paths.',
    whyItMatters: 'Skool may contain high-value training and community material, but blind scraping creates policy and product risk. Foundation needs a source matrix before any crawler runs: owner/content-use class, allowed operations, extraction route, storage target, and value routes.',
    nextAction: 'Run the access-path audit in `docs/source-notes/skool-corpus.md`: identify owned/paid communities, official export/Zapier/admin paths, linked media assets, owner/content-use class, and whether each source is `allowed`, `manual_export_only`, `link_inventory_only`, `blocked`, or `needs_permission`.',
    statusNote: 'Blocked from blind scraping. Public Skool policy and limited API/export paths mean this stays access-audit only until permission and extraction boundaries are explicit.',
  },
  {
    ...
    nextAction: 'Add route/back-link schema, define the v1 rule map from synthesized item type/status to decision/backlog/open-question/contradiction/ignore/snooze, update synthesized item statuses after routing, and add a verifier guard for stale unrouted synthesized items.',
    statusNote: 'This is the missing hop between extraction/synthesis and Strategy Hub. Do not build Strategy Hub UI before this loop exists.',
  },
  {
    id: 'VIDEO-001',
    ...
    summary: 'Move `video-link-inventory-bite` from a raw writer into the extraction target runner with cursor, lease, run accounting, provenance-preserving occurrences, and safe repeated execution semantics.',
    whyItMatters: 'Steve should not manually gather Loom, Skool, Drive, YouTube, or Zoom links. But the system also cannot keep rescanning the newest artifacts and overwriting discovery provenance. Video links need the same governed crawl model as Drive.',
    nextAction: 'Do not reopen this as content extraction. Future work belongs to `MULTIMODAL-EXTRACTOR-001`, `CREATOR-WATCHLIST-001`, `YOUTUBE-SCOUT-001`, `LOOM-001`, or `ZOOM-RECOVERY-001`.',
    statusNote: 'Closed as inventory/control wrapper. It inventories media links and provenance; it does not transcript, watch, crawl, or understand videos.',
  },
  {
    id: 'LOOM-001',
    ...
    summary: 'Prove a small, authorized extraction route for Steve-owned Loom videos before adding Loom transcript/download jobs to Foundation.',
    whyItMatters: 'Old Loom videos may contain major training and course IP, but Loom has no first-party open bulk extraction API. The system needs a legal, reliable, source-linked path instead of blind scraping or manual URL collection.',
    nextAction: 'Test 3 to 5 Steve-owned Loom URLs through the approved path, likely Apify or authorized session export, and record transcript, metadata, MP4 availability, auth requirements, cost, failure modes, content-use boundaries, and atom quality.',
    statusNote: 'Do not bulk-run Loom. This is a proof card only until access and extraction behavior are verified.',
  },
  {
    id: 'SOURCE-014',
  ...
  },
  {
    ...
    nextAction: 'Write the first synthesis contract and output shape. It must cover: cross-artifact linking, resolution detection against later artifacts and trusted systems like ClickUp/decision ledgers, cross-source dedup, staleness scoring, actionability ranking, and how batch review versus continuous per-artifact synthesis should coexist. Use old executive/intelligence report, feedback-scout, and director patterns only as shape references: decisions, action items, bottlenecks, escalation-worthy issues, suggested owner, suggested next action, and ranked findings with evidence. Unify those into one Foundation substrate instead of many disconnected skills.',
  ...
  },
  {
    ...
    summary: 'Turn Gmail, Missive, Slack, Drive/Meet, and Zoom historical pulls into resumable backfill jobs with explicit cursor state, oldest/newest coverage, errors, rate-limit handling, and run status instead of relying on one-off manual terminal runs. Split current-day sync from historical backfill so the system can stay fresh while slowly working backward.',
    whyItMatters: 'The archive is now valuable, but Steve needs to know what time window is actually covered. Bounded pulls can look complete while only reaching a shallow slice. Without durable cursor state, 180-day or merger-era coverage becomes tribal memory and cannot be trusted by Strategy Hub, scopers, or future action surfaces.',
    nextAction: 'Define the first backfill-run record model and wire it into the existing scripts: Missive `until` cursor, Gmail mailbox/date-window state, Slack pagination, Drive page tokens, Zoom folder import status, per-run counts, oldest/newest artifact dates, last error, next safe command, and whether the run is `current_sync` or `historical_crawl`. Keep it read/archive only until `SYSTEM-010` supervision exists.',
    statusNote: 'Current archive depth is useful but not historically complete. This card owns the difference between “we have a lot,” “we know coverage is complete,” and “we are staying current every day.”',
  },
  {
    ...
    summary: 'Create the operating model for a supervised extraction team that can connect to sources, understand the source shape, extract rich evidence, synthesize source-backed intelligence, and route outputs to the right operating ledger or hub.',
    whyItMatters: 'The business has too much historical and current data for one-off chat mining. The new system needs a governed extraction team that can work across Drive, Gmail, Missive, Slack, meetings, Zoom, Skool, YouTube/Loom/video links, public web, and future corpora without rebuilding old scout chaos.',
    nextAction: 'Use the first Foundation job registry and extraction control target ledger as the base, then add the missing memory spine: intelligence job ledger, atom schema, retrieval, multimodal extractor contract, and action routing before scaling YouTube/Skool/video ingestion.',
    statusNote: 'First runtime slice is live: registered jobs, DB-backed run history, CLI runner, source-crawl target ledger, and dashboard visibility. Missing pieces are retrieval spine, atom schema, rich multimodal extractors, and governed action routing.',
  },
  {
    id: 'DRIVE-CORPUS-001',
    ...
    summary: 'Create a governed crawler for old Google Drive and shared-drive folders that inventories folders/files, detects useful docs/media/links/presentations, mirrors or organizes approved artifacts into the Crewbert archive, and extracts candidates/atoms one folder at a time.',
    whyItMatters: 'Benson Crew, Zahnd Team, MarketMasters, Houseable, and Steve-owned projects have years of value buried across shared drives: trainings, buyer/seller presentation systems, strategy docs, sales assets, marketing angles, videos, course material, recruiting proof, product/IP, and founder knowledge. Trying to mine all of it manually blocks product work; ignoring it wastes historical IP and future monetization material.',
    nextAction: 'Run a few more manual `drive-corpus-inventory-bite` proofs, then add review filtering for high-value presentation/training/docs before any export/copy/extraction stage.',
    statusNote: 'First read-only Drive bite is live. `drive-corpus-inventory-bite` inspected the Zahnd TEAM OG root, recorded 60 direct children into `source_crawl_items`, discovered 24 child folders and 36 files, queued 31 next folders/roots, and moved/copied/exported nothing.',
  },
  {
    ...
    nextAction: 'Close the target/item model around explicit current vs backfill lanes, lease ownership, retry semantics, partial-failure surfacing, source budgets, and a dashboard view that shows last success, last failure, next bite, and remaining backlog by source.',
    statusNote: 'Source crawl targets and items are live and already used by current-day sync, Drive inventory, and video-link inventory. Remaining work is the full operator control plane and coverage truth across all extraction lanes.',
  },
  {
    ...
    nextAction: 'Stabilize Gmail and Missive scheduled current-day sync, keep meeting notes manual until retry/reporting is harder, then add source-specific cursors, skipped/failed/review-needed counts, and freshness warnings for each enabled source.',
    statusNote: 'Gmail and Missive current-day sync are scheduled through the worker and target ledger. Meeting notes, Slack, Drive, Skool, YouTube, Loom, Zoom, and web current-day behavior still need explicit rollout gates.',
  },
  {
    ...
    nextAction: 'Define a backfill run contract for each source target: fixed bite size, oldest/newest coverage, cursor state, stop condition, per-item failures, retry state, output artifacts, and next safe command.',
    statusNote: 'Some manual/bounded proofs exist, including Drive and shared-comms archive work. The general backfill lane is not closed until every historical crawl records durable coverage and can resume without chat memory.',
  },
  {
    id: 'DRIVE-WORKER-001',
    ...
    nextAction: 'Extend the current read-only Drive bite into review-filtered worker stages: inventory, classify, extract readable artifacts, queue media for multimodal extraction, and keep organizer/move/copy actions dry-run until policy is approved.',
    statusNote: 'Read-only inventory proof exists through `DRIVE-CORPUS-001`. This card owns the worker implementation, extraction stage, and safe progression beyond inventory.',
  },
  {
    id: 'SKOOL-WORKER-001',
  ...
  },
  {
    ...
    summary: 'Make partial target runs operationally visible before scheduling meeting, Drive, Skool, or other high-variance crawlers. If a child sync exits successfully but item-level crawl records failed, the job must surface a warning/failure state or an explicit alert path.',
    ...
    nextAction: 'Prove the partial alert path against a real failed meeting/Drive item, then add retry limit/backoff and extend retry semantics to Drive corpus items before scheduling high-variance crawlers.',
    statusNote: 'First slice done: partial target runs now exit nonzero, and `meeting-notes-retry-failed` can retry failed meeting crawl items from `source_crawl_items`. No-op proof found 0 failed items.',
  },
  {
    ...
    nextAction: 'Watch scheduled targets through 2-3 worker cycles and keep `foundationJobKey` populated on any new scheduled crawl target.',
    statusNote: 'First slice done: extraction target snapshots now attach a `scheduler` object derived from the Foundation job runtime when `metadata.foundationJobKey` exists. Target `nextRunAt` remains crawl checkpoint metadata, while `effectiveNextRunAt` is job-derived for scheduled targets.',
  },
  {
    id: 'EXTRACT-METRICS-001',
    title: 'Separate written, refreshed, and net-new archive metrics',
    team: 'dev',
    lane: 'scoped',
    priority: 'P3',
    rank: 46,
    source: 'Codex runtime review after Gmail item ledger',
    summary: 'Clean up extraction metric language so sync scripts report `written`, `refreshed`, and `net_new` separately instead of overloading `archived` across updated and new artifacts.',
    whyItMatters: 'The behavior is working, but ambiguous metric labels make it harder to tell whether the archive grew or simply refreshed existing records. This matters for coverage confidence and operator dashboards.',
    nextAction: 'Normalize output labels in Gmail/Missive/meeting runners and target metadata. Keep archive delta as net-new artifacts; separately record refreshed/written counts.',
    statusNote: 'Cleanup only. Do not block meeting retry or Drive crawler work on this.',
  },
  {
    id: 'WEB-CRAWLER-001',
    ...
    summary: 'Define the Foundation crawler strategy for public web, YouTube, Loom, Vimeo, Wistia, Skool-linked assets, and bought-course/community sources without violating platform rules or mixing Steve-owned IP with third-party material.',
    whyItMatters: 'The content engine, Strategy Hub, MarketMasters, Steve personal brand, Unchained monetization, and recruiting proof all need outside and historical content. But paid communities and hosted video platforms have access, copyright, and terms boundaries. A crawler that ignores that creates product and legal risk.',
    nextAction: 'Create the first source matrix: platform, access method, allowed operations, transcript/export path, screenshot/frame policy, owner/content-use class, storage target, crawl cadence, dedupe key, and value routes. Start with YouTube/public web where official or permitted access exists; keep Skool/purchased communities as access-audit only until approved.',
    statusNote: 'Pairs with `SKOOL-001`, `DRIVE-CORPUS-001`, `MULTIMODAL-EXTRACTOR-001`, `PLATFORM-INTEL-001`, and `HUB-INTEL-001`.',
  },
  {
    ...
    statusNote: 'The V1 synthesis contract now states the atom boundary. This card turns that doctrine into implementation so Strategy Hub/action routing gets decision-grade source facts, not another long report.',
  },
  {
    id: 'INTEL-JOBS-001',
    ...
    priority: 'P0',
    rank: 9,
    source: '2026-04-25 memory and current-intel audit',
    ...
    nextAction: 'Add intelligence job/run tables or extend the Foundation job/source target model so every extraction and memory job records source_id, job_type, cursor, budget, model/provider, auth path, cost, item counts, failures, output artifact IDs, and next run state.',
    statusNote: 'Missing live backlog card promoted from deep intelligence audit. This should happen before broad external/video crawling.',
  },
  {
    id: 'INTEL-ATOM-001',
    ...
    rank: 10,
    source: '2026-04-25 memory and current-intel audit',
    summary: 'Define the common output contract for extracted value: observations, claims, patterns, demonstrated workflows, decisions, risks, corrections, content ideas, and action candidates, all linked to source evidence.',
    whyItMatters: 'Reports alone are too coarse and raw transcripts are too noisy. Foundation needs small, rich, source-backed atoms that can be searched, synthesized, routed, accepted, rejected, superseded, and reused by hubs.',
    nextAction: 'Design the v1 atom/candidate contract: source_id, artifact_id, modality, timestamp/page/row anchor, evidence excerpt, derived claim, source confidence, sensitivity, content-use class, value_route, suggested owner/action, expires/supersedes fields, and accept/reject feedback state.',
    statusNote: 'This is the infrastructure answer to “reports or atoms?” Use atoms as durable memory; reports/briefs are rendered views over atoms and evidence.',
  },
  {
    id: 'RETRIEVAL-001',
    ...
    priority: 'P0',
    rank: 11,
    source: '2026-04-25 memory architecture audit',
    summary: 'Add chunk tables and Postgres full-text/trigram search over archived communications, docs, reports, and extracted media notes before relying on vector search.',
    ...
    nextAction: 'Add idempotent chunking for shared artifacts and future media artifacts, search_vector fields, GIN indexes, trigram support where useful, and a search endpoint that returns evidence-backed chunks with source filters and sensitivity handling.',
    statusNote: 'Promoted from audit. This is the first retrieval spine step after current extraction control.',
  },
  {
    id: 'RETRIEVAL-002',
    ...
    rank: 12,
    source: '2026-04-25 memory architecture audit',
    summary: 'Enable pgvector in Postgres and add embeddings for chunks and candidates/atoms with HNSW indexes where volume justifies it.',
    ...
    nextAction: 'Install/enable pgvector for local Postgres, add embedding columns/tables, choose embedding model and dimensions, backfill in bounded batches, add HNSW indexes, and measure recall/cost before increasing scope.',
    statusNote: 'Do after lexical search exists. Keep Graphiti/Zep deferred until Postgres memory proves itself.',
  },
  {
    id: 'RETRIEVAL-003',
    ...
    rank: 13,
    source: '2026-04-25 memory architecture audit',
    summary: 'Combine lexical search, semantic vector search, candidate/atom search, source filters, recency, confidence, and sensitivity rules into one evidence API for synthesis and future chat recall.',
    ...
    nextAction: 'Build RRF-style hybrid retrieval over chunks and atoms, return original source evidence, apply sensitivity/source filters, and use it to answer test questions like SocialPilot, Nick Saraev, Foundation closeout, and deal-review history under 2 seconds.',
    statusNote: 'Gate before YouTube scale-up and before claiming memory is strong.',
  },
  {
    id: 'MULTIMODAL-EXTRACTOR-001',
    title: 'Define rich multimodal extraction worker contract',
    team: 'dev',
    lane: 'scoped',
    priority: 'P1',
    rank: 43,
    source: 'Founder extraction standard and old-system extractor audit',
    ...
    whyItMatters: 'The old extractor was too shallow. The new system needs to understand what is said, shown, clicked, demonstrated, and implied, then turn it into valuable atoms with timestamps, screenshots or visual notes where allowed, and source links.',
    nextAction: 'Define extractor inputs/outputs, modality fields, timestamp anchoring, screenshot/frame policy, transcript policy, model route, cost budget, rights/use boundary, quality score, and failure handling. Then implement the first worker only after retrieval and job ledger are in place.',
    statusNote: 'This is the broad platform-agnostic card. YouTube/Loom/Zoom/Skool workers plug into it.',
  },
  {
    ...
    priority: 'P1',
    rank: 44,
    source: 'Old BCrew-Buddy external scout registry + Steve creator list',
  ...
  },
  {
    ...
    priority: 'P1',
    rank: 45,
    source: 'Founder current-intel requirement + official YouTube/Gemini research',
    ...
    nextAction: 'Start with 5 creators and 3 latest videos each. Store metadata, source URL, thesis, tools shown, workflows, prompts/screens, architecture patterns, claims to verify, BCrew applications, adopt/adapt/ignore recommendation, timestamps, and source confidence. Scale to last 10 videos after quality/cost proof.',
    statusNote: 'Do not start with 200 videos. Do not use Playwright screenshots as the default for public YouTube; use Gemini video understanding first and browser capture only as authorized fallback.',
  },
  {
    ...
    priority: 'P2',
    rank: 46,
    source: 'Zoom archive target audit',
  ...
  },
  {
    ...
    statusNote: 'Initial inventory is complete. Full legacy line-read is still open and should run before broad memory/extraction scale-up claims are made.',
  },
  {
    ...
    summary: 'Keep `docs/README.md`, `docs/INDEX.md`, current-plan, current-state, and doc-cleanup aligned around one active-doc set so old audits and handoffs remain evidence until promoted.',
    ...
    nextAction: 'Keep the index generator authoritative, require a promoted-to target for evidence before build work depends on it, and update verifier coverage so the active-doc set cannot drift silently.',
    statusNote: 'First slice done on 2026-04-25: indexes now include generated timestamps, promoted-to column, and no free-floating active-reference status.',
  },
  {
    id: 'DB-SEED-001',
    ...
    nextAction: 'Inventory seed/live mismatches, decide which fields are mutable live state, create migration-style updates for durable doctrine, and make verifier report drift instead of hiding it.',
    statusNote: 'Audit found 133 seed/live field mismatches across 180 seed rows. Do not treat seed text as automatically equal to live backlog truth until this card closes.',
  },
  {
    ...
    nextAction: 'Design the run table, connect `leaseSourceCrawlTarget`, `finishSourceCrawlTargetRun`, and `source_crawl_items` to run IDs, then migrate dashboard/history views to show target runs separately from target config.',
    statusNote: 'Immediate P1 bug patched on 2026-04-25: finish now requires matching lease owner and item events use the returned row key. Full run-ledger design remains open.',
  },
  {
    ...
    nextAction: 'Add DB checks or verifier checks for decision categories and registered source IDs, tighten doc-update status transitions, and make doc-update-applied decisions run the same supersession semantics as normal decision updates.',
    statusNote: 'Source IDs are still code-defined and stored as free text in several tables. API validation is not enough for the long-term memory spine.',
  },
  {
    id: 'AVATAR-001',
  ...
  {
    id: 'SYSTEM-010',
    title: 'Add visible job/process control, kill switches, and decommission workflow',
    team: 'dev',
    lane: 'scoped',
    priority: 'P0',
    rank: 5,
    source: 'Ghost-agent lesson captured 2026-04-15',
    summary: 'Every running scheduler, routine, agent process, automation worker, and orchestration runtime should be visible, stoppable, and cost-tracked from inside the system so ghost processes cannot quietly burn money or keep acting after the team thinks a system is shut down.',
    whyItMatters: 'The old system kept running silently after people thought it was effectively dead. The rebuild now has useful routines like transaction review, FUB health, Gmail/Missive archive, transcript-gap reports, synthesis, Zoom transcription, and future Drive/Skool crawls, but useful scripts are not enough if nothing schedules, supervises, retries, or reports them.',
    nextAction: 'Close the first job/process-control model: routine registry, command, source ids, job type, safety level, dry-run/write mode, schedule, cursor/checkpoint, visible process list, stop/pause controls, last activity, active task count, cost-to-date, dead-man switch behavior, and explicit decommission workflow that stops processes, disables schedules, and records shutdown.',
    statusNote: 'P0 Foundation gate before autonomous loops and agents. Pause/resume exists, but decommission, dead-man, cost/process visibility, and pre-launch kill-switch checks are not closed.',
  },
  {
    id: 'MANDATE-003',
  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        'Treat `SRC-OWNERS-001` and `SRC-FINANCE-001` current-reality meaning as closed. Use `DATA-005`, `DATA-006`, `DATA-007`, `DATA-008`, and `DATA-009` for the remaining Owners parity and quality work.',
    ...
    await client.query(
      ...
      [
        ...
        'P0',
        7,
        'Use the first Foundation job registry and extraction control target ledger as the base, then add the missing memory spine: intelligence job ledger, atom schema, retrieval, multimodal extractor contract, and action routing before scaling YouTube/Skool/video ingestion.',
        'First runtime slice is live: registered jobs, DB-backed run history, CLI runner, source-crawl target ledger, and dashboard visibility. Missing pieces are retrieval spine, atom schema, rich multimodal extractors, and governed action routing.',
      ]
    )
    ...
    await client.query(
      ...
      [
        ...
        'P0',
        5,
        'Extend the first job ledger into real supervision: schedules, pause/disable controls, cursor state, retry policy, cost tracking, and approval-gated write jobs.',
        'First visible job/process control is live for routines. It records command, run status, output tail, duration, exit code, and recent failures. Kill switches and scheduler/daemon control are still next.',
      ]
    )
    ...
    await client.query(
      ...
      [
        ...
        'P0',
        4,
        'Promote the auth/tier/vault spec into implementation: authenticated user/tier attachment, `assertTier` checks, subject_people + sensitivity tagging, uniform redacted response shape, owner-preserving meeting-note ACL policy, and filtered-summary access-request flow.',
        'P0 Foundation gate. Admin-only proof endpoints may stay private, but no broad hub, assistant, query, or human-facing read surface can expose shared-communications intelligence until this is implemented and verified.',
      ]
    )
...
function attachSourceCrawlTargetSchedule(target, foundationJobSchedules) {
  ...
  const scheduler = jobSchedule
    ? {
        ...
        targetNextRunAt: target.nextRunAt || null,
    ...
    const result = await client.query(
      `
        ...
        ON CONFLICT (target_key, external_id) DO UPDATE SET
          ...
          metadata = COALESCE(source_crawl_items.metadata, '{}'::jsonb) || EXCLUDED.metadata,
...
export async function listSourceCrawlItems({ targetKey, status, limit = 50 } = {}) {
  const normalizedTargetKey = String(targetKey || '').trim()
