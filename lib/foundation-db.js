...
const backlogSeed = [
  ...
  },
  {
    ...
    statusNote: 'This should become a governed review layer plus source fixes. Start in-sheet if needed, but do not leave it as a one-time cleanup sheet. The temporary sheet pattern now covers both Admin deal rows and conditional rows. Conditional review is now structurally hardened with `Client Name` and `FUB Person URL / ID`, but most live rows still need those fields populated. Raw FUB `Cond` counts are not fully trustworthy yet because the live conditional set still contains stale old conditional deals, so governed review must use identity plus stage instead of rough counts or agent-only matching. Mature deal reviews should use a `10`-day gate from `Date Firm (Executed)`. The first mature batch is already showing the repeatable failures: stale FUB lifecycle stage, missing Freedom follow-through, and occasional lineage / ISA mismatches.',
  },
  {
    id: 'DATA-007',
