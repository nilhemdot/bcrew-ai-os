...
const backlogSeed = [
  ...
  {
    id: 'SECURITY-004',
    title: 'Gate broad Foundation and Ops read APIs before any non-local access',
    team: 'dev',
    lane: 'done',
    priority: 'P0',
    rank: 4,
    source: '2026-04-25 Foundation code audit',
    summary: 'Broad Foundation/Ops read APIs now use `requireAdminToken` outside localhost: Foundation hub snapshot, Owners review queue/governance, system inventory, changes, and pending doc updates.',
  ...
  try {
    ...
    await client.query(
      ...
      [
        'SECURITY-004',
        'done',
        'Broad Foundation/Ops read APIs now use `requireAdminToken` outside localhost: Foundation hub snapshot, Owners review queue/governance, system inventory, changes, and pending doc updates.',
...
export async function upsertSourceCrawlItem(input, actor = 'system') {
  ...
  return withFoundationTransaction(async client => {
    await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', [`source_crawl_item:${itemKey}`])
    ...
    await insertChangeEvent(client, {
      ...
      entityId: itemKey,
      actor,
      summary: `${itemKey} crawl item updated`,
      metadata: {
    ...
    return mapSourceCrawlItemRow(result.rows[0])
  })
}
...
export async function finishSourceCrawlTargetRun(targetKey, input = {}, actor = 'system') {
  ...
  return withFoundationTransaction(async client => {
    ...
    if (!result.rows[0]) throw new Error(`Source crawl target not found: ${normalizedTargetKey}`)
    return mapSourceCrawlTargetRow(result.rows[0])
  })
}
