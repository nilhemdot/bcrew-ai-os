...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Plan a metadata-only boundary slice: inventory safe file metadata, document allowed visibility, verify private content cannot appear in tracked docs/API/verifier logs, and route any shared durable content into proper repo docs or backlog cards.',
    statusNote: 'Not built. Scope must preserve private memory and user context. No content copy, no broad sharing, no delete/move of local-only files without a separate approved card.',
  },
  {
    ...
    nextAction: 'Plan the doctrine/schema slice before UI work. Define required page-level fields, empty/error states, review-next language, proof links, and how the pattern will be checked by verifier or screenshot review.',
    statusNote: 'Not built. This should guide Phase G UI work, Recent Work redesign, Daily Exec Summary, and System Inventory polish instead of being treated as a loose memory preference.',
  },
  {
    ...
    nextAction: 'Plan a practical v1: add plan-content hashes or signed/checksummed approval metadata, verify stale/edited approvals fail, and keep an explicit emergency path with follow-up-card requirements.',
    statusNote: 'Not built. Do not overbuild a full approval platform; the first slice should make existing approval files tamper-evident enough for Foundation ship gates.',
  },
  {
    ...
    nextAction: 'Update the doctrine source list, regenerate the marked skill section, keep private-memory checks metadata-only, and add verifier coverage for the missing doctrine signals.',
    statusNote: 'Not built. Builds on DOCTRINE-PROPAGATION-001 and DOCTRINE-PROPAGATION-002; do not copy private memory text into tracked docs or skill output.',
  },
  {
    ...
    summary: 'Upgrade decision auto-emission so explicit pivot, park, adopt, defer, lock, disable, override, or sequence-change language in closeouts and plan updates becomes a proposed or applied decision record with source proof.',
    ...
    nextAction: 'Plan v2 around explicit language only: detect candidate decisions from closeouts/current-plan/current-state, show dry-run output, require approval for apply, and verify decision records link back to commit/card/source text.',
    statusNote: 'Not built. DECISION-AUTO-EMIT-001 is dry-run-first v1; this card owns broader real-commit/chat-checkpoint decision capture.',
  },
  {
    ...
    nextAction: 'Audit same-commit closeouts, fix relatedBacklog/backlogIds precision, add synthetic verifier coverage for multi-closeout grouping, and preserve collapsed same-commit UX.',
    statusNote: 'Not built. This is a precision fix for Recent Work data semantics, not a full UI redesign; UI polish remains under RECENT-BUILDS-BILLION-DOLLAR-UI-001.',
  },
  {
    ...
    nextAction: 'Plan a tiered hook path: lightweight pre-commit or pre-push checks, canonical process:foundation-ship for full ships, clear bypass reason, and mandatory follow-up card for bypass.',
    statusNote: 'Not built. Must avoid making tiny edits run the entire heavy verifier every time; pair with approval integrity and gate-performance known limits.',
  },
  {
    ...
    nextAction: 'Use Backlog Hygiene and verifier exception data to make a bounded closeout-backfill plan. Do not invent proof; preserve historical exceptions with explicit owner/review date.',
    statusNote: 'Not built. This should reduce or resolve the remaining done-without-closeout-proof warnings without rewriting history.',
  },
  {
    ...
    nextAction: 'Plan Phase G before building. Scope the target surfaces, acceptance copy rules, and verifier coverage first.',
    statusNote: 'Scoped by Wave Cleanup B only. Do not build this inside Cleanup B.',
  },
  {
    ...
    rank: 7,
    source: 'Path A Phase G planning opened after Wave Cleanup B',
    summary: 'Tighten Recent Work and change-log coverage so shipped changes, closeouts, and where-it-lives links are complete enough for review without reading git logs.',
    whyItMatters: 'Steve needs Foundation to explain what changed and where it lives without rebuilding context from chat.',
    nextAction: 'Plan Phase G before building. Define what counts as comprehensive and how it differs from existing Recent Builds v2.',
    statusNote: 'Scoped by Wave Cleanup B only. Do not build this inside Cleanup B.',
  },
  {
    ...
    rank: 7,
    source: 'Path A Phase G planning opened after Wave Cleanup B',
    summary: 'Redesign the Recent Work / Recent Builds experience so shipped work is grouped, scannable, linked, and executive-readable.',
    ...
    nextAction: 'Plan Phase G before building. Define UX states, grouping, proof links, and verifier-backed acceptance.',
    statusNote: 'Scoped by Wave Cleanup B only. Do not build this inside Cleanup B.',
  },
  {
    ...
    rank: 7,
    source: 'Path A Phase G planning opened after Wave Cleanup B',
    summary: 'Add a daily summary that tells Steve what changed, what needs attention, what is blocked, and what the next command move is.',
    whyItMatters: 'A clean Foundation should reduce the amount of chat archaeology Steve needs before deciding what to do next.',
    nextAction: 'Plan Phase G before building. Decide source inputs, cadence, privacy boundary, and where it lives.',
    statusNote: 'Scoped by Wave Cleanup B only. Do not build this inside Cleanup B.',
  },
  {
    ...
    rank: 7,
    source: 'Path A Phase G planning opened after Wave Cleanup B',
    summary: 'Continue the connect -> verify -> extract -> synthesize -> route lifecycle for the sources that are not fully connected, verified, or extracting yet.',
    whyItMatters: 'Foundation is not truly done until the important source lanes move through the full lifecycle instead of stopping at partial visibility.',
    nextAction: 'Plan Phase G before building. Identify the source lanes, proof level, and first narrow lifecycle slice.',
    statusNote: 'Scoped by Wave Cleanup B only. Do not build this inside Cleanup B.',
  },
  {
    ...
    rank: 7,
    source: 'Path A Phase G planning opened after Wave Cleanup B',
    summary: 'Improve Foundation navigation, page hierarchy, and layout so the operator surface feels coherent and fast to scan.',
    ...
    nextAction: 'Plan Phase G before building. Define target pages, layout rules, mobile/desktop checks, archive/default-view rules, and verifier or screenshot evidence.',
    statusNote: 'Scoped by Wave Cleanup B only. Required scope: System Inventory hides archived/history docs by default and exposes them through a separate Archive/History section so current truth is not mixed with old evidence. Do not build this inside Cleanup B.',
  },
  {
  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        'P0',
        7,
        'Continue from the coverage-by-target panel: use Runtime Health to pick the next missing corpus lane only after the unhealthy/thin list is visible; add retry/backoff only when failed source_crawl_items reappear.',
        'First runtime slice is live: registered jobs, DB-backed run history, CLI runner, source-crawl target ledger, dashboard visibility, Drive Docs/Sheets/PDF/text content extraction, Gmail PDF/text attachment extraction, YouTube subtitle transcript extraction, 2026-04-28 Slack current-day channel item proof, and 2026-04-28 Missive current-day conversation item proof. Missing pieces are richer multimodal extractors, Missive/richer attachment extraction, retry semantics, and governed action routing.',
      ]
    )
