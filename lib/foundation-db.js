...
export async function getStaleLlmCalls({ olderThanSeconds = 240, limit = 20 } = {}) {
  const normalizedLimit = Math.min(100, Math.max(1, Number(limit) || 20))
  const result = await pool.query(
    `
      SELECT call_id, workload, hub_key, provider, model, auth_path,
             credential_key, route_key, status, created_at, finished_at,
             error_message
      FROM llm_calls
      ...
      LIMIT $2
    `,
    [Number(olderThanSeconds || 240), normalizedLimit]
  )
  ...
  const result = await pool.query(
    `
      ...
      SET title = EXCLUDED.title,
          ...
          metadata = EXCLUDED.metadata,
          status = CASE
            WHEN shared_communication_candidates.status IN ('rejected', 'duplicate') THEN 'pending'
          ...
          updated_at = CASE
            WHEN shared_communication_candidates.title IS DISTINCT FROM EXCLUDED.title
              ...
              OR shared_communication_candidates.status IN ('rejected', 'duplicate')
            THEN NOW()
