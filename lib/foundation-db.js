...
const backlogSeed = [
  ...
  },
  {
    ...
    title: 'Show exactly where shipped changes live in the app',
    team: 'foundation',
    lane: 'scoped',
    priority: 'P1',
    rank: 9,
    source: '2026-04-28 Steve review of Recent Builds visibility after extraction-control builds',
    summary: 'Recent Builds explains what changed, but it still leaves too much work for Steve to figure out where the change lives in the product. Major build closeouts need app-level location, surface visibility, and changed-section context, not just file paths.',
    whyItMatters: 'If Steve cannot tell where to click, what menu owns the change, whether it is visible or backend-only, and what changed on that surface, the system still has invisible work. Foundation should make shipped work inspectable from the user point of view.',
    nextAction: 'Add clickable app breadcrumbs to major closeouts, mark each build as new visible surface, updated visible surface, backend-only visible through metric/status, doc-only, or data/source-contract update, and add New or Recently updated tags plus short what-changed notes on affected Foundation pages/cards.',
    statusNote: 'Backlog follow-up only; do not build during EXTRACT-METRICS-001. Acceptance: Recent Builds links directly to the relevant app surface/doc for at least 3 recent closeouts; Runtime Health extraction-control changes show a recently-updated marker or changed-note; doc-only changes route to a doc link/section summary; backend-only changes say where the effect is visible; `foundation:verify` checks major closeout records include app surface metadata, not just file paths. Not in scope: full design-system rewrite, full doc redline/highlight engine, user/access control panel, extraction retry/backoff, Strategy/Scoper/Agent work.',
  },
  {
    id: 'MEMORY-001',
