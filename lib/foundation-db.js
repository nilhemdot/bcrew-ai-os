...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Ship the operator coverage-by-target view as the final v1 control-plane slice: last success, last failure, next bite, remaining backlog, per-source quota, failed-item retry state, and lane consistency across source_crawl_items.',
    statusNote: 'Source crawl targets/items now support scheduled current-day sync, Drive inventory, video-link inventory, daily shared-comms extraction missions, Slack channel item proof, Missive conversation item proof, Runtime Health item summaries/findings, and Foundation-job schedule truth. V1 should close after coverage-by-target ships under EXTRACT-METRICS-001; retry/backoff stays in EXTRACT-RETRY-001 and later hardening should become child cards.',
  },
  {
  ...
  },
  {
    ...
    nextAction: 'Build the coverage-by-target panel now that Missive current-day emits conversation-level source_crawl_items. Link this card to EXTRACT-CONTROL-001 and keep failed-item retry execution parked in EXTRACT-RETRY-001.',
    statusNote: 'Refreshed 2026-04-28. Lane inspection found Slack, Gmail, meetings, Drive corpus/content, attachments, and video lanes already emit source_crawl_items, but missive-current-day had successful target runs with zero items. Missive now writes missive_conversation items; next slice is the coverage-by-target view.',
  },
  {
    id: 'WEB-CRAWLER-001',
