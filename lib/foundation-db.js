...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'Review the builder/reviewer conversations from the intensive Foundation spine, Strategy Hub repair, and Strategic Intelligence reframing cycle, then promote durable lessons into system strategy, backlog cards, or verifier checks.',
    ...
    nextAction: 'Extract proposed patches for `docs/system-strategy.md` or the current strategy/rebuild docs plus any new backlog cards. Topics to mine: function-vs-form testing, loose atom spam, verifier-becomes-spec drift, memory-versus-repo-truth discipline, build closeout discipline, meeting-participant vision, plain-English titles and proof, Strategic Intelligence as a continuous operating loop, gap-resolving Scoper architecture, model-routing/subscription doctrine, AIOS as Q2 priority, and modular hub architecture for Marketing/Ops.',
    statusNote: 'Scoped from Steve review on 2026-04-27 and expanded on 2026-04-28 after the Strategy route, Scoper, and model-routing conversations. Do not create another loose audit doc unless the output is explicitly promoted into repo truth or backlog.',
  },
  {
    ...
    nextAction: 'Ship the meeting-ready UX pass before deeper Scoper work: source proof must render sender, date, quote, thread participants, reply/reply-count or thread status when available, and source link/context instead of internal IDs; Approve/Apply, Needs Owner, Snooze, Ignore, and Reject need tooltips or a legend explaining what happens and where the record lands; Snooze needs 1 day / 1 week / 1 month / 1 quarter / custom; Needs Owner needs a real owner picker; Strategy-approved items must appear in the Strategy workspace applied/done view; Reject must confirm before closing a proposal.',
    statusNote: 'Executing from 2026-04-28 Steve review. V1 route-review pass now renders human-readable source proof, action guide/tooltips, owner picker, 1 day / 1 week / 1 month / 1 quarter / custom snooze choices, reject confirmation, and Applied / Done grouping. Keep executing until Steve smoke-tests the UI and confirms whether the Strategy-local applied view and proof clarity are meeting-ready.',
  },
  {
    ...
    nextAction: 'Depends on STRATEGY-HUB-MEETING-READY-001 being clear enough to review real issues; blocks INTEL-SCOPER-001 implementation until the issue-ledger/schema decision is approved. Land the spec before code. Pick an explicit data model: either a new `intelligence_strategic_issues` table or a defined view/projection over `intelligence_synthesized_items`, and document the chosen fields. Define the mining cadence for continuous strategic issue detection. Define typed urgency, impact, confidence, and staleness fields plus how each is assigned by rules, retrieval evidence, LLM classification, or human review. Define resolution feedback: when an action or issue resolves, what updates the issue, synthesized item, route, atoms, and suppression/resurface rules. Map old BCrew scout -> director -> scoper -> sprint flow to the new architecture so the missing depth layer is explicit. V1 success targets for the first two-week pilot: >= 5 strategic issues surfaced/week, >= 3 scoped/week, >= 2 resolved-to-applied/week, and median manual investigation time <= 30 minutes per issue.',
    statusNote: 'New P0 from 2026-04-28. This is a spec gate, not a vague rename of Strategy Hub. It must become repo truth before Strategic Intelligence code expands, because memory notes do not bind future builders. The spec must commit measurable 10x targets, not only name candidate dimensions.',
  },
  {
    id: 'INTEL-SCOPER-001',
    ...
    nextAction: 'Depends on STRATEGIC-INTEL-001 approving the strategic issue ledger/schema and scoring fields. Build after STRATEGY-HUB-MEETING-READY-001 and the STRATEGIC-INTEL-001 spec gate. V1 must be on-demand through a "Scope this" action, not auto-run on every item. It must query atoms, retrieval chunks, synthesis facts, source contracts, decisions, open questions, backlog, docs, and relevant operating facts; persist to an `intelligence_scoped_cards` style table or a named equivalent; return structured sections for verified already-answered, partial evidence, actual remaining gaps, risks, suggested owner, next steps, and evidence pointers. The Hub UI must show the scoped result after "Scope this" as a meeting-readable panel with verified / partial / remaining-gaps sections, evidence chips, suggested next steps, scoped-card status, and a link back to the stored scoped card. Every verified claim must cite a file, source ID, fact, atom, chunk, decision, route, or backlog ID. Include a minimal Agent Spec: purpose, tools, inputs, outputs, kill switch, max cost/latency per scope, evidence requirements, and human approval boundary. Produce a 5-row human sample before closeout.',
    statusNote: 'New P0 from 2026-04-28. This is the first real tool-using agent shape, but it is deliberately narrower than Agent Factory. It preserves the old Scoper depth without recreating director/agent sprawl.',
  },
  {
