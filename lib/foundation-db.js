...
function buildEngineMetricRow(label, value, detail, sortOrder) {
  return {
    docPath: AGENT_ENGINE_DOC_PATH,
    sourceId: 'SRC-FREEDOM-ENGINE-001',
    groupTitle: 'Live Engine View',
    label,
    value,
    detail,
    asOf: getTodayTorontoIso(),
...
async function getLiveAgentEngineSourceSnapshot() {
  const engineRes = await getSheetValues('service-account', FREEDOM_SHEET_ID, "'Agent Engine'!A1:K10")
  ...
  const nextYearTarget = parseNumber(values[3]?.[4])

  const avgProduction = parseNumber(values[2]?.[7])
  const targetProduction = parseNumber(values[3]?.[7])
  ...
  const recruitingGap = Number.isFinite(requiredRecruitingPace) && Number.isFinite(recruitingSixMonth)
    ...
    : null
  const capacityGap = Number.isFinite(activeAgents) && Number.isFinite(nextYearTarget)
    ? activeAgents - nextYearTarget
    : parseNumber(values[4]?.[4])
  const productionGap = Number.isFinite(avgProduction) && Number.isFinite(targetProduction)
    ? avgProduction - targetProduction
    : parseNumber(values[4]?.[7])
  const splitGap = Number.isFinite(actualSplit) && Number.isFinite(targetSplit)
    ? actualSplit - targetSplit
    : parseNumber(values[4]?.[10])

  const detail = 'Live operating scoreboard from the Freedom KPI Sheet Agent Engine tab.'

  return [
    buildEngineMetricRow('Required Recruiting Pace', formatMonthlyAgentRate(requiredRecruitingPace), detail, 1),
    buildEngineMetricRow('6-Month Recruiting Pace', formatMonthlyAgentRate(recruitingSixMonth), detail, 2),
    buildEngineMetricRow('Recruiting Gap', formatPaceWithPercent(recruitingGap, requiredRecruitingPace, formatAgentDelta), detail, 3),
    buildEngineMetricRow('Active Agents', formatAgentCountPrecise(activeAgents), detail, 4),
    buildEngineMetricRow('Next-Year Start Target', formatAgentCountPrecise(nextYearTarget), detail, 5),
    buildEngineMetricRow('Capacity Gap', formatPaceWithPercent(capacityGap, nextYearTarget, formatAgentDelta), detail, 6),
    buildEngineMetricRow('Avg Production / Agent', `${formatCompactCurrency(avgProduction)} / month`, detail, 7),
    buildEngineMetricRow('Production Target / Agent', `${formatCompactCurrency(targetProduction)} / month`, detail, 8),
    buildEngineMetricRow('Production Gap', formatPaceWithPercent(productionGap, targetProduction, formatCurrencyDelta), detail, 9),
    buildEngineMetricRow('Attrition Rate', formatPercent(attritionRate), detail, 10),
    buildEngineMetricRow('Avg Additions / Month', formatMonthlyAgentRate(avgAdditions), detail, 11),
    buildEngineMetricRow('Avg Attrition / Month', formatMonthlyAgentRate(avgAttrition), detail, 12),
    buildEngineMetricRow('Actual Split', formatPercent(actualSplit), detail, 13),
    buildEngineMetricRow('Target Split', formatPercent(targetSplit), detail, 14),
    buildEngineMetricRow('Split Gap', `${splitGap >= 0 ? 'Above target' : 'Below target'} by ${formatPointDelta(splitGap)}`, detail, 15),
  ]
}
