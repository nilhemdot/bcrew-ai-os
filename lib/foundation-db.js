...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Wait for Steve exact SEND APPROVED naming Georgia, milestone/day, recipient rule, CC roles, and one-send limit. Then send exactly one Gmail message, capture message/thread ID, and write ClickUp Requested only after Gmail succeeds.',
    statusNote: 'Scoped by AGENT-FEEDBACK-SEND-001. This Stage 2 card owns the first real Georgia test send only. It must not run without explicit SEND APPROVED. If Georgia is To, she must not also be duplicated in CC. Proof must remain metadata-only except Gmail message/thread IDs and ClickUp Requested writeback confirmation; no token URLs, raw email addresses, or feedback content in tracked proof.',
  },
  {
  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        'Wait for Steve exact SEND APPROVED naming Georgia, milestone/day, recipient rule, CC roles, and one-send limit. Then send exactly one Gmail message, capture message/thread ID, and write ClickUp Requested only after Gmail succeeds.',
