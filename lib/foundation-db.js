...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'Write the explicit exit gate for calling Foundation ready so the sprint does not keep expanding forever or move into Strategy before the control, privacy, extraction, and verifier legs are actually green.',
    whyItMatters: 'Without a written done test, Foundation can mean whatever the last chat remembered. Steve needs a concrete pass/fail gate that says whether Strategy can begin or which Foundation leg is still blocking.',
    nextAction: 'Define the three-leg minimum test: source-verifiable answer, tier/redaction safety, and structural verifier coverage for every P0 gate. Add runtime/extraction control health if the current sprint depends on scheduled workers. Each failed leg must name the blocker card and next proof command.',
    statusNote: 'Captured on 2026-05-02. This is the sprint exit gate, not the first implementation card. It should be written before Foundation is declared ready for Strategy Hub work.',
  },
  {
