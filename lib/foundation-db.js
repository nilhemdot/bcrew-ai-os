...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Build the first surface-freshness sweep: map each Foundation page to its backing source/API/doc/backlog owner, add stale-surface checks to `foundation:verify`, and require a sweep checklist after work that changes source contracts, connectors, job schedules, docs, backlog, Systems, Data Sources, System Inventory, or hub links. Use the known stale Slack crawl run as the v1 proof case — the sweep must catch and surface it explicitly so future stale runs produce visible verifier failures, not silent rot.',
