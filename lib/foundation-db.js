...
const backlogSeed = [
  ...
  },
  {
    ...
    statusNote: 'Found during the FUB opportunity-semantics audit. This is provider-side cleanup/proof, not a code refactor.',
  },
  {
    ...
    statusNote: 'Do this before the next deep source audit lane. This complements SECURITY-001/006 rotation work but is about operational readiness, not storing secrets in repo truth.',
  },
  {
    id: 'SECURITY-002',
  ...
  },
  {
    ...
    statusNote: 'V1 source boundary locked. Deal Data Entry has AIOS review fields for internal onboarding/deal-management surveys plus client NPS / Google-review workflow at the back of Full Deal List. Conditional forecast rebuilds from ClickUp. Admin review joins ClickUp by Deal # first, then property/address fallback so missing Trade Numbers become explicit findings. Agent Roster is live in Ops for contract-link, roster baseline backfill, onboarding NPS trigger checks, private AIOS feedback capture, ClickUp status/score/feedback result writeback, and personal-email coverage.',
  },
  {
    id: 'SOURCE-005',
  ...
  },
  {
    ...
    priority: 'P1',
    rank: 42,
    source: 'SOURCE-010 closeout',
    summary: 'Turn the signed-off KPI read model into operational health: table/RPC freshness checks, visible status, schema/code drift review against the Lee `zahnd-team-dashboard` repo, and explicit readiness gates before Sales Hub depends on KPI continuously.',
    whyItMatters: 'KPI is an existing production foundation system with several truth layers. Once AI OS reads it continuously, stale tables, changed RPCs, or repo/database drift need to be visible instead of rediscovered in chat.',
    nextAction: 'Build a small KPI health probe for the locked tables/RPCs, expose last-success/freshness in System Health or Data Sources, and record a repeatable Lee repo + Supabase schema audit checklist.',
    statusNote: 'Follow-up to SOURCE-010. This is health/freshness/drift hardening, not another source-meaning review.',
  },
  {
    ...
    summary: 'Agents commonly stack multiple discovery/consult appointments for the same opportunity, fail to update outcomes, or use wrong outcome labels. Live audit found 3442 active appointment rows, 951 missing outcomes, 53 non-standard outcomes, and 289 same-person/same-type appointment stacks covering 838 rows.',
    ...
    nextAction: 'Define the appointment-quality rules from the KPI How To page and `appointmentUtils.ts`, then build a read-only audit that flags likely same-person/same-type stacking, missing outcomes, and non-standard outcomes by agent, with human-review language for legitimate rebooks.',
    statusNote: 'Promoted as a Sales Hub data-quality gate. This is not a connector issue; it is agent data-entry behavior that must be visible before KPI advice is trusted.',
  },
  {
    ...
    summary: 'FUB automatically creates lead-stage records when people are added. If agents add realtors, vendors, support-network contacts, duplicates, or other non-leads and do not move them out quickly, KPI can count fake leads. Live check found 13005 active lead-stage rows, including 4497 source `Import`, 1647 source `<unspecified>`, and 2 null-source rows; KPI source-validation helpers exist in code but are commented out / disabled.',
    ...
    nextAction: 'Map the KPI lead-list/source-error views and FUB stage/source rules into an AIOS read-only audit for unvalidated leads, suspicious sources, non-lead contacts left in lead stages, and stale temporary cleanup states. Distinguish valid historical/import pond rows from fake leads before scoring agents.',
    statusNote: 'Promoted as a Sales Hub data-quality gate. Source text alone is not enough: fake-lead detection needs stage, source, tags/person type, owner, timing, and the cleanup-window workflow before praising lead generation.',
  },
  {
    id: 'KPI-SHOPPING-001',
    ...
    priority: 'P1',
    rank: 45,
    source: 'KPI How To page + founder clarification + 2026-04-20 Shopping List audit',
    summary: 'Shopping List is the manual working-opportunity layer agents are expected to maintain weekly. Prior audit found 289 active rows, 47 active rows with blank action plans, 16 stale high-score rows older than 60 days, and duplicate active entries. Steve also identified Reilly Mitchell and Sofia Fischman coaching calls as source evidence for the intended weekly behavior.',
    ...
    nextAction: 'Mine the Reilly Mitchell and Sofia Fischman coaching calls from Drive/meeting archives, then convert the Shopping List audit into agent-level weekly discipline signals: stale high-score rows, blank action plans, duplicates, missing updates, and signed/not-signed drift.',
    statusNote: 'Scoped as a Sales Hub coaching input. This should run after appointment and lead-validation quality gates, but before broad agent coach automation.',
  },
  {
    id: 'SOURCE-011',
  ...
  },
  {
    ...
    statusNote: 'Core fields are now backfilled for the locked seed decisions. Remaining work is richer linkage and better provenance quality rules.',
  },
  {
    id: 'DECISION-006',
  ...
  },
  {
    ...
    rank: 38,
    source: 'Foundation Operations review',
    summary: 'Keep  as the short operator feed, but add the fuller audit surface behind it so the team can search, filter, and inspect older change events instead of relying on only the latest page-sized slice.',
  ...
  },
  {
    ...
    rank: 39,
    source: 'Foundation Operations review',
    summary: 'Keep the home page as the closeout summary, but evolve  into the deeper diagnostic layer for trust, memory, verification, connector/runtime state, and future alert conditions.',
  ...
  },
  {
    ...
    statusNote: 'Shared archive is materially real across Gmail, Missive, Slack, meeting notes, and meeting transcripts, but it is not complete source understanding until attachment content, Drive file content, and linked meeting videos are handled. See EMAIL-ATTACHMENTS-001, DRIVE-CONTENT-001, and MEETING-VIDEO-001.',
  },
  {
    id: 'SOURCE-020',
  ...
  },
  {
    ...
    nextAction: 'Use the V1 contract and first shared-comms synthesis proof to build the real substrate: persist synthesized items, link them to candidates/artifacts/source facts, detect resolution/supersession, score staleness, rank actionability, and generate the tighter ownership strategy packet instead of another raw candidate dump.',
    statusNote: 'First proof exists as a batch job and handoff. It successfully grouped multi-source issues like KPI/reporting failure, automation-health degradation, SocialPilot instability, finance reconciliation, and source-trust problems. It is not done until it persists state, grounds against source-backed operating facts, and supports source-backed action routing.',
  },
  {
  ...
  },
  {
    ...
    nextAction: 'Add the operator coverage view: last success, last failure, next bite, remaining backlog, per-source quota, and failed-item retry state. Keep verifier coverage on schedule truth and partial-failure behavior.',
    statusNote: 'Source crawl targets/items now support scheduled current-day sync, Drive inventory, video-link inventory, and daily shared-comms extraction missions. Remaining work is operator-facing coverage truth, retry semantics, and source-specific backlog completion state.',
  },
  {
    ...
    statusNote: 'Gmail and Missive current-day sync are scheduled every 2 hours. Meeting notes and Slack current-day sync are scheduled daily through the worker and target ledger. Drive, Skool, YouTube, Loom, Zoom, and web remain corpus/history lanes rather than current-day source sync.',
  },
  {
    ...
    statusNote: 'Daily quota missions now exist for Gmail, Missive, meeting transcript, Slack extraction, and Drive inventory, but historical completeness is still not proven until each source records coverage windows, remaining backlog, and resumable cursors.',
  },
  {
  ...
  },
  {
    ...
    nextAction: 'Keep daily Drive inventory running, then implement DRIVE-CONTENT-001 before treating Drive as strategy-readable. Do not move/copy/organize files until read-only extraction and review rules are proven.',
    statusNote: 'Read-only inventory proof exists and daily inventory is being scheduled. This umbrella now explicitly depends on DRIVE-CONTENT-001 for Google Docs/PDF text extraction and MEETING-VIDEO-001 for Drive/video media review; inventory alone is not sufficient source understanding.',
  },
  {
    ...
    summary: 'Drive access and inventory are live, but inventory is not content understanding. The system can map folders/files; it does not yet extract general Google Docs, PDFs, Slides, Office files, images, audio, or video into normalized source artifacts. V1 should cover Google Docs and PDFs first.',
    ...
    nextAction: 'Implement a read-only Drive content extractor for Google Docs and PDFs from the existing Drive inventory queue. Store normalized text artifacts with file id, URL, mime type, content hash, owner/source folder, extraction status, skip reason, and source-backed provenance. Unsupported or unauthorized files must be marked skipped/policy-excluded, not silently dropped.',
    statusNote: 'Urgent source gap. Service account access appears available; this is an extractor implementation gap, not a permission problem. Daily Drive lane currently inventories/maps only. V1 must track skip reasons for Sheets/Slides/media/images/Office/unknown files until each extractor is approved.',
  },
  {
    ...
    rank: 7,
    source: '2026-04-26 source automation review',
    summary: 'Gmail and Missive thread bodies are archived and scheduled, but attachment content extraction is not proven as part of the source archive. Attachments need their own manifest, safe download rules, mime-type handling, extraction status, and provenance.',
    ...
    nextAction: 'Add an attachment lane for Gmail and Missive: detect attachments, record manifest rows, download only approved mime types and size ranges, route Google Drive/Docs links to the Drive extractor, parse PDFs/docs/text first, mark unsupported images/media as skipped/policy-excluded, and connect outputs to the same shared-comms/source artifact ledger.',
    statusNote: 'Urgent source gap for strategy readiness. Current daily email sync should continue, but it should not be treated as full email understanding until attachment extraction exists with explicit skip reasons.',
  },
  {
    ...
    nextAction: 'Resolve Google Drive video/Zoom/Loom/YouTube links from meeting notes, classify access and rights, prefer existing transcripts where available, estimate cost/model route, then run a small timestamped proof behind explicit quota, ledger, and stop controls.',
    statusNote: 'Approved direction, not optional: Steve expects a high-end multimodal extractor for meeting-linked videos, YouTube/channels, Loom, Skool, and related media. The guardrail is budgeted/observable/quota-controlled execution, not avoiding video spend.',
  },
  {
    id: 'SKOOL-WORKER-001',
  ...
  },
  {
    ...
    nextAction: 'Add target/job semantics for `partial`: dashboard warning state, retry queue/query for failed `source_crawl_items`, retry limit/backoff, and a proof run where one item failure is visible without killing unrelated work.',
    statusNote: 'Current meeting proof records item-level failures and target partial state, but Foundation job success still follows child process exit. Harden before scheduling meetings/Drive.',
  },
  {
    ...
    nextAction: 'Decide the single source of schedule truth. Prefer deriving target schedule display from the Foundation job where `metadata.foundationJobKey` exists, while keeping target next-run for crawl-specific checkpoint planning.',
    statusNote: 'Not blocking current scheduled Gmail/Missive lanes. Fix before broad backfill panels become operator-facing truth.',
  },
  {
  ...
  },
  {
    ...
    priority: 'P0',
    rank: 10,
    source: 'Steve Connect/Understand/Extract/Synthesize correction',
  ...
  },
  {
    ...
    summary: 'Enable pgvector in Postgres and add 1536-dimension embeddings for chunks and candidates/atoms with HNSW indexes where volume justifies it.',
  ...
  },
  {
    ...
    nextAction: 'Define the GOD-mode extractor contract: inputs/outputs, channel/video metadata, transcript acquisition, timestamp anchoring, screenshot/frame policy, visual/tool/workflow detection, model route, cost budget, rights/use boundary, quality score, skip reasons, and failure handling. Then implement the first small worker proof behind quotas and ledgering.',
    statusNote: 'Approved Foundation direction from Steve. YouTube videos/channels, Loom, Skool, Zoom/meeting recordings, Drive videos, slides, demos, and screenshots need one governed multimodal extraction contract instead of scattered one-off scrapers.',
  },
  {
  ...
  },
  {
    ...
    summary: 'Recover and verify the Lee `FUBZahnd` middleware/database logic that sits between raw FUB activity and KPI opportunity semantics. First pass confirms the PersonID vs PID model, non-lead to lead-stage re-entry record creation, `LeadDate`, `LeadClaimedDate`, `LeadDeletedDate`, first-contact, appointment, and deal-stage timestamp semantics. Live proof now confirms 53/53 FUB-linked Owners deal groups match KPI persons and KPI deal_data, and live Supabase has fresh `leadclaimeddate` evidence.',
    ...
    nextAction: 'Use `docs/source-notes/fub-zahnd-middleware.md` and `docs/source-notes/fub-kpi-deal-connection-map.md` as the working contract, then prove the current writer path for `leaddate`/`leadclaimeddate` and decide the Sales Hub coaching language for `Active Client` now that the live stage table marks it `leadstage = true`.',
    statusNote: 'Live connection proof is complete for FUB -> KPI persons -> KPI deal_data -> Owners linked rows: 53/53 matched KPI persons and deal_data, 8/53 showed multiple KPI person rows, 6/53 had leadclaimeddate, and user ID 22 is active Benson Crew Assistant. Second-pass proof found 2451 active non-deleted rows with leadclaimeddate and latest samples updated on 2026-04-26. Remaining work is current writer ownership and coaching-language closeout.',
  },
  {
    id: 'SOURCE-012',
  ...
  },
  {
    ...
    nextAction: 'Extend the first job ledger into real supervision: schedules, pause/disable controls, cursor state, retry policy, cost tracking, and approval-gated write jobs.',
    statusNote: 'First visible job/process control is live for routines. It records command, run status, output tail, duration, exit code, and recent failures. Kill switches and scheduler/daemon control are still next.',
  },
  {
    id: 'MANDATE-003',
  ...
  },
  {
    ...
    nextAction: 'Design the first apply contract: agent identity, connected accounts/API keys, allowed write surfaces, suggested-fix preview, approval step, audit log, rollback/escalation rules, and which fixes are safe to automate versus which require manager or Ops review.',
    statusNote: 'Do not build writes before the read/audit gates are reliable. This card preserves the destination: help agents act and update their systems, not just receive criticism.',
  },
  {
    id: 'RETAIN-001',
