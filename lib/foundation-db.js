...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Document and surface LaunchAgent status, stable log paths, restart behavior, served-HEAD/deploy freshness, dead-man checks, stop/decommission controls, and cost/process warnings. Tie the remaining work to `SYSTEM-010` instead of scattering it across chats.',
    statusNote: 'Dashboard and Foundation worker LaunchAgents are live. Remaining work is dead-man visibility, operator runbook, decommission controls, cost/process alarms, and an explicit dashboard served-code-equals-HEAD or auto-restart-on-push check before autonomous loops expand.',
  },
  {
  ...
  },
  {
    ...
    nextAction: 'Extend the first job ledger into real supervision: schedules, pause/disable controls, cursor state, retry policy, cost tracking, served-code/deploy freshness, and approval-gated write jobs.',
    statusNote: 'First visible job/process control is live for routines. It records command, run status, output tail, duration, exit code, and recent failures. Kill switches, scheduler/daemon control, and dashboard served-code-equals-HEAD or auto-restart-on-push checks are still next.',
  },
  {
    id: 'MANDATE-003',
