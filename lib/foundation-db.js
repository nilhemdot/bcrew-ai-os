...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'Treat Skool as a future Foundation corpus source, not just a one-off research target. The system should eventually crawl courses, trainings, posts, comments, links, docs, and video lessons, then extract reusable operating patterns, training assets, content atoms, recruiting proof, and course-building material with evidence links.',
    whyItMatters: 'Skool may contain high-value training and community material, but it will become noise if every chat manually browses it from scratch. It needs the same supervised crawl model as Drive: current-day deltas, historical bites, cursor state, inspected artifacts, transcript/video handling, extraction status, source links, and synthesis or hub handoff.',
    nextAction: 'Validate the access path first: official Skool API if available, export path if available, or supervised browser crawler if not. Start with a small read-only target, tag findings by value route (`reusable_now`, `useful_later`, `course_material`, `recruiting_proof`, `platform_specific`, `rejected`), and route good patterns into existing backlog cards instead of spawning a second architecture track.',
    statusNote: 'Source contract seed is `SRC-SKOOL-001`. Research only until access, crawl boundaries, and copyright/content-use rules are explicit.',
  },
  {
    id: 'SOURCE-014',
  ...
  },
  {
    ...
    summary: 'Create the operating model for a supervised extraction team that runs current-day sync plus historical corpus crawls across Drive, Gmail, Missive, Slack, meetings, Zoom, Skool, and future source corpora.',
    ...
    nextAction: 'Design the run model: source id, job type (`current_sync` / `historical_crawl`), cursor/checkpoint, artifact inventory, archive/mirror action, extraction action, value route, synthesis or hub handoff, cost, errors, retry policy, and human-review queue. Then build the first runner around existing Gmail/Missive/Drive/Zoom scripts.',
  ...
  },
  {
    ...
    whyItMatters: 'Benson Crew and Zahnd Team have years of value buried across old shared drives: trainings, strategy docs, sales assets, marketing angles, videos, course material, recruiting proof, and founder IP. Trying to mine all of it manually blocks product work; ignoring it wastes the company’s historical IP and future monetization material.',
    nextAction: 'Define the Drive crawl state model: root folder, folder cursor, file type, owner/account, inspected_at, action taken, archive/mirror destination, extraction status, value route, skip reason, error, and next folder. Start read-only/inventory-first, then allow dry-run organization proposals before any copy/move action.',
