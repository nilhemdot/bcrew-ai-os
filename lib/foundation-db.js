...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Add or update the runtime doctrine, likely in `docs/rebuild/current-runtime-map.md` or a linked `docs/system-strategy/model-routing.md`. Preserve the rule: subscriptions are for humans/live operator use when allowed, probed, logged, and classified; system runtime uses official APIs and governed adapters by default. Define task classes for cheap classification/tagging, embeddings, synthesis, Scoper deep tool use, coding, heartbeats, and high-stakes scopes. Verify current official model availability before hardcoding exact model names. Include cost tracking per feature and privacy/tier boundaries.',
