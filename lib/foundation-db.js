...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Create a 9.8 plan first, then build the smallest cadence layer on top of `FOUNDATION-SPRINT-SYSTEM-001`: active sprint executive summary, sprint goal, planned outcome, exit test, start/review dates, stage counts, current risks, next decision, end-of-sprint review fields, and a non-mutating Scrum Master readout that flags stale stages, missing Sprint Ready checks, unapproved work, and unclear next actions. Proof should include a focused `process:foundation-sprint-cadence-check`, backlog hygiene, foundation:verify, and process:foundation-ship.',
    statusNote: 'Captured on 2026-05-10 after Steve review of the Current Sprint UI. `FOUNDATION-SPRINT-SYSTEM-001` shipped the necessary overlay/stage controls, but this card owns the missing scrum cadence layer and operator summary. Scope is sprint process only: no broad UI polish, Strategy, Sales, Agent Feedback, broad corpus, researcher, autonomous agent actions, or meeting Drive permission mutation. The Scrum Master piece is read-only/reporting in v1; it may propose next moves but cannot mutate backlog, approve work, launch builds, send messages, or change Drive permissions. Acceptance requires a visible executive sprint summary, clear sprint goal/outcome, explicit exit criteria, review/retro/next-sprint handoff, Scrum Master drift readout, proof commands, and verifier coverage before treating Current Sprint as the real sprint machine.',
  },
  {
      ...
      CREATE TABLE IF NOT EXISTS change_events (
        ...
        event_type TEXT NOT NULL
          CHECK (event_type IN (
            ...
            'drive_access_preflight_recorded', 'meeting_vault_acl_audit_recorded',
...
export async function listMeetingRawDriveFileCandidates({ limit = 5000 } = {}) {
  ...
  const artifactResult = await pool.query(
    `
      SELECT artifact_id, source_id, artifact_type, external_id, source_account,
