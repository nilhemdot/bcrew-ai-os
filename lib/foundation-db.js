...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'Recover and verify the Lee `FUBZahnd` middleware/database logic that sits between raw FUB activity and KPI opportunity semantics. First pass confirms the PersonID vs PID model, non-lead to lead-stage re-entry record creation, `LeadDate`, `LeadClaimedDate`, `LeadDeletedDate`, first-contact, appointment, and deal-stage timestamp semantics.',
    ...
    nextAction: 'Use `docs/source-notes/fub-zahnd-middleware.md` as the working contract, then run the live proof: query current `stages` LeadStage/ClientStage rows, identify user ID `22`, compare a small sample against live FUB/KPI rows, prove the current writer path for `leaddate`/`leadclaimeddate`, and close the `Active Client` edge case.',
    statusNote: 'Promoted to P0 executing because this is the Sales Hub source-semantics gate. The repo explains the missing logic, but the live stage table and current writer path still need proof before broad sales answers depend on it.',
  },
  {
    id: 'SOURCE-012',
