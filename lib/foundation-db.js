...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Done for v1. Use `npm run process:fanout-check` after a normal Foundation ship, together with `npm run process:ship-check`, before trusting closeout. `FOUNDATION-SURFACE-UPDATES-001` owns Recent Work UI, `RUNTIME-SUPERVISOR-001`/`SYSTEM-010` own runtime restart/dead-man controls, `BACKLOG-HYGIENE-001` owns stale-card findings, and `AGENT-008` owns the repo-vs-agent identity boundary.',
    statusNote: 'Closed on 2026-04-28. V1 adds `scripts/process-fanout-check.mjs`, `npm run process:fanout-check`, `docs/process/ship-fanout.md`, updates the local `bcrew-foundation` skill with current doctrine, and adds verifier coverage. The check fails if closeout/backlog links are missing, UI/API work lacks where-it-lives metadata, backlog changes do not update current plan/state, Recent Builds or verifier are missing, served dashboard commit differs from repo HEAD, the Foundation skill is stale, or USER/SOUL/MEMORY identity boundaries are not tracked under `AGENT-008`. V1 is a script gate, not an automatic Git hook.',
  },
  {
    id: 'MEMORY-001',
