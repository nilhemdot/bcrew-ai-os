...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Continue the ADMIN ONLY - Deal Data Entry review tab-by-tab and column-by-column, confirm exactly which meanings are signed off versus still pending, then record that signed-off boundary in the source registry and Foundation backlog.',
    statusNote: 'Current tab focus: ADMIN ONLY - Deal Data Entry. Foundation-first blocker. Readable is not the same thing as signed off.',
  },
  {
    id: 'SECURITY-001',
  ...
  },
  {
    ...
    nextAction: 'Create a baseline verification pass covering `/api/foundation-hub`, a backlog/decision read-write smoke, doc-update apply integrity, the Google delegated health script, one readable check for each currently signed-off critical source, and a consistency check that source contracts, source notes, and registry status do not contradict each other.',
    statusNote: 'Do the minimum that catches breakage early. This is not a full test suite yet, but it should catch endpoint failure, source-truth drift, and bad doc-apply behavior.',
  },
  {
    id: 'MEMORY-001',
  ...
  },
  {
    ...
    summary: 'Replace the current `dev` / `marketing` hardcoding in the root backlog with a data-driven scope and queue model so Foundation, Strategic Execution, Marketing, departments, and future hubs can own their own queues without baking those assumptions into frontend code.',
    ...
    nextAction: 'Define the first queue-ownership model: scope keys, display labels, root-vs-hub ownership, API shape, filter UI, migration from current `dev` / `marketing` values, and how records graduate out of the root Foundation queue into hub-local queues without losing traceability.',
    statusNote: 'This is the architectural fix for the current backlog hardcoding, not just a filter polish task.',
  },
  {
    id: 'SYSTEM-010',
...
export async function listFubLeadSourceRules() {
  const result = await pool.query(
    `
      SELECT source_name, marketing_type, ownership_type, source_group, notes, updated_at, updated_by
    ...
    const result = await client.query(
      `
        INSERT INTO fub_lead_source_rules (
          source_name, marketing_type, ownership_type, source_group, notes, updated_at, updated_by
        )
        ...
        RETURNING source_name, marketing_type, ownership_type, source_group, notes, updated_at, updated_by
      `,
      [source, marketingType, ownershipType, sourceGroup, notes, actor]
    )
