...
function getFoundationJobHealth(job, latestRun) {
  ...
  if (latestRun.status === 'succeeded') {
    return {
      status: 'live',
      detail: latestRun.finishedAt ? `Last succeeded ${latestRun.finishedAt}.` : 'Last run succeeded.',
    }
  }
...
export async function getFoundationJobRunSnapshot({ limit = 30, includeOutput = false } = {}) {
  ...
  const latestRuns = result.rows.map(row => mapFoundationJobRunRow(row, { includeOutput }))
  const latestByJob = new Map()
  for (const run of latestRuns) {
    if (!latestByJob.has(run.jobKey)) latestByJob.set(run.jobKey, run)
  }

  const jobs = getFoundationJobDefinitions().map(job => {
