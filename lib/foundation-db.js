...
export async function upsertFoundationCurrentSprintOverlay(input = {}, actor = 'system') {
    ...
    for (const rawItem of items) {
      ...
      await client.query(
        ...
        [
          sprintId,
          cardId,
          Number(rawItem.order ?? rawItem.sprintOrder ?? rawItem.sprint_order),
          String(rawItem.stage || '').trim(),
          rawItem.planRef || rawItem.plan_ref || null,
          rawItem.definitionOfDone || rawItem.definition_of_done || '',
...
export async function getFoundationJobControl(jobKey) {
...
export async function getFoundationJobRunById(runId, { includeOutput = false } = {}) {
...
export async function updateFoundationJobRunMetadata(runId, metadataPatch = {}, actor = 'system') {
  const normalizedRunId = String(runId || '').trim()
  if (!normalizedRunId) throw new Error('runId is required.')
...
export async function markFoundationJobRunStopped(runId, input = {}, actor = 'system') {
  const normalizedRunId = String(runId || '').trim()
...
export async function classifySourceCrawlItemRetries({ targetKey = '', limit = 500 } = {}, actor = 'system') {
  const normalizedTargetKey = String(targetKey || '').trim()
...
export async function getRetryableSourceCrawlItems({ targetKey, limit = 10, now = new Date().toISOString() } = {}) {
  const normalizedTargetKey = String(targetKey || '').trim()
  if (!normalizedTargetKey) throw new Error('targetKey is required.')
...
export async function leaseRetryableSourceCrawlItems({ targetKey, runId, leaseOwner = 'system', limit = 10, leaseSeconds = 900 } = {}, actor = 'system') {
  const normalizedTargetKey = String(targetKey || '').trim()
  const normalizedRunId = String(runId || '').trim()
  if (!normalizedTargetKey) throw new Error('targetKey is required.')
  if (!normalizedRunId) throw new Error('runId is required.')
...
export async function markSourceCrawlItemAttemptStarted(input = {}, actor = 'system') {
  const itemKey = String(input.itemKey || '').trim()
...
export async function markSourceCrawlItemAttemptFinished(input = {}, actor = 'system') {
  const itemKey = String(input.itemKey || '').trim()
  const runId = String(input.sourceCrawlRunId || input.runId || input.crawlRunId || '').trim()
  if (!itemKey) throw new Error('itemKey is required.')
  if (!runId) throw new Error('sourceCrawlRunId is required.')
...
export async function markStaleSourceCrawlItems({ olderThanMinutes = SOURCE_CRAWL_STALE_RUN_MINUTES, limit = 50 } = {}, actor = 'system') {
  const normalizedMinutes = Math.max(5, Math.min(24 * 60, Number(olderThanMinutes) || SOURCE_CRAWL_STALE_RUN_MINUTES))
  const normalizedLimit = Math.min(200, Math.max(1, Number(limit) || 50))
  return withFoundationTransaction(async client => {
    const result = await client.query(
...
export async function getExtractionRunHardeningSnapshot({ limit = 50 } = {}) {
...
export async function listMeetingRawDriveFileCandidates({ limit = 5000 } = {}) {
  ...
  return {
    ...
    crawlItems: crawlItemResult.rows.map(mapSourceCrawlItemRow),
  }
}

export async function recordDriveAccessPreflightRun(input = {}, actor = 'system') {
...
export async function getLatestDriveAccessPreflightRun({ cardId = 'DRIVE-ACCESS-REQUEST-001' } = {}) {
  const result = await pool.query(
    `
      SELECT *
...
export async function recordMeetingVaultAclAudit(input = {}, actor = 'system') {
...
export async function getLatestMeetingVaultAclAudit({ cardId = 'MEETING-VAULT-ACL-001' } = {}) {
  const result = await pool.query(
    `
      SELECT *
...
export async function recordMeetingVaultAutoEnforcementRun(input = {}, actor = 'system') {
...
export async function getLatestMeetingVaultAutoEnforcementRun({ cardId = 'MEETING-VAULT-AUTO-ENFORCEMENT-001' } = {}) {
  const result = await pool.query(
    `
      SELECT *
...
export async function getMeetingVaultLegacyExceptions({ limit = 50, status = 'open' } = {}) {
...
export async function buildSynthesisVerificationDbReport({ stamp = false, limit = 500 } = {}) {
