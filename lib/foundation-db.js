...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'Zoom audio transcription still calls OpenAI audio transcription directly with `OPENAI_API_KEY`, and the verifier only blocks direct OpenAI Responses calls. This leaves API spend and provenance gaps outside the BCrew router.',
    ...
    nextAction: 'Either pause/remove the Zoom audio transcription runner or migrate transcription into the router as an explicit transcription workload. Broaden `foundation:verify` to flag direct OpenAI, Anthropic, and Gemini host calls outside approved router/adapters, with explicit exceptions for sanctioned probes.',
    statusNote: 'Confirmed audit blocker. Do before reopening Zoom audio recovery or scheduling any high-volume media/transcription lane.',
  },
  {
    id: 'SECURITY-004',
    ...
    rank: 4,
    source: '2026-04-25 Foundation code audit',
    summary: 'Several Foundation/Ops read APIs expose operating data without `requireAdminToken`: Foundation hub snapshot, Owners review queue/governance, system inventory, changes, and pending doc updates.',
    ...
    nextAction: 'Add interim `requireAdminToken` or split each endpoint into a redacted public/local surface and an admin surface. Then add verifier coverage so new broad read endpoints declare their auth posture.',
    statusNote: 'Confirmed audit blocker. This is the immediate stop-gap before full `SECURITY-002` tier and subject-person redaction.',
  },
  {
    id: 'FOUNDATION-003',
  ...
  {
    id: 'SKOOL-001',
    title: 'Define Skool as a supervised corpus source',
    team: 'dev',
    lane: 'research',
    priority: 'P2',
    rank: 44,
    source: 'Mark Kashef Skool access + founder corpus-crawl discussion',
    summary: 'Treat Skool as a future Foundation corpus source, not just a one-off research target. The system should eventually inventory allowed courses, trainings, posts, comments, links, docs, Loom/YouTube/Vimeo/Wistia lessons, and paid community material, then extract reusable operating patterns, training assets, content atoms, recruiting proof, and course-building material with evidence links.',
    whyItMatters: 'Skool may contain high-value training and community material, but it will become noise if every chat manually browses it from scratch. It also has policy and content-use risk, so the access route must be explicit before any crawler runs. It needs the same supervised crawl model as Drive: current-day deltas where allowed, historical bites, cursor state, inspected artifacts, transcript/video handling, extraction status, source links, and synthesis or hub handoff.',
    nextAction: 'Run the access-path audit captured in `docs/source-notes/skool-corpus.md`: identify owned/paid communities, official export/Zapier/admin paths, linked Loom/YouTube/Vimeo/Wistia assets, owner/content-use class, and whether each source is `allowed`, `manual_export_only`, `link_inventory_only`, `blocked`, or `needs_permission`. Do not build a blind browser scraper.',
    statusNote: 'Source contract seed is `SRC-SKOOL-001`. Public Skool docs show limited Zapier/API-key workflows and platform-policy scraping risk; keep this blocked until access, crawl boundaries, and content-use rules are explicit.',
  },
  {
  ...
  },
  {
    ...
    nextAction: 'Add a `video-link-inventory` extraction target, make the raw script refuse non-dry-run unless called by the target runner, persist cursor/progress, store all discovery occurrences for each normalized URL, and add verifier coverage before repeated runs.',
    statusNote: 'First proof found 104 deduped media links from existing archives. That proves value, not schedule-readiness.',
  },
  {
    id: 'LOOM-001',
    ...
    nextAction: 'Test 3 to 5 Steve-owned Loom URLs through the approved path, likely Apify or authorized session export, and record transcript, metadata, MP4 availability, auth requirements, cost, failure modes, and content-use boundaries.',
  ...
  },
  {
    ...
    summary: 'Create the operating model for a supervised extraction team that runs current-day sync plus historical corpus crawls across Drive, Gmail, Missive, Slack, meetings, Zoom, Skool, YouTube/Loom/video links, public web, and future source corpora.',
    whyItMatters: 'The business has too much historical data for manual one-off backfills. The productized AI OS also cannot require a human to sit with Codex while it crawls a customer’s history. The system needs durable crawl jobs that take one safe bite per source per day, record what happened, and hand usable evidence to synthesis, scopers, content/course builders, recruiting, strategy, sales, ops, marketing, and agent-coaching hubs.',
    nextAction: 'Design the run model: source id, job type (`current_sync` / `historical_crawl` / `corpus_inventory`), cursor/checkpoint, artifact inventory, archive/mirror action, extraction action, value route, owner-entity/sensitivity, synthesis or hub handoff, cost, errors, retry policy, and human-review queue. Then build the first runner around existing Gmail/Missive/Drive/Zoom scripts.',
    statusNote: 'This is not a return to old scouts. These are supervised routines with shared source contracts, one archive, one candidate schema, and one synthesis layer.',
  },
  {
    id: 'DRIVE-CORPUS-001',
  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        'P0',
        7,
        'Use the first Foundation job registry and job-run ledger to move current sync, coverage, verification, transaction reviews, and synthesis out of builder chat. Next hardening step: add cursor/checkpoint state and scheduler supervision.',
        'First runtime slice is live in code: registered jobs, DB-backed run history, CLI runner, API snapshot, and dashboard visibility. This is not the old scout swarm; it is code-owned routines with visible status and explicit source contracts.',
      ]
    )
