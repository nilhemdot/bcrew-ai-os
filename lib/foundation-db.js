...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Map the current Admin-tab source columns to the FUB model, define valid source values and merge rules, add exception rules for bad values like `Import`/`Unspecified`, and design the compliance check that uses Column `BZ` plus ISA-set logic to flag mismatches.',
...
const openQuestionsSeed = [
  ...
  },
  {
    ...
    summary: 'We still need to confirm exact header names and live usage for legacy or partially-understood fields such as Columns D, E, AE, and the exact label sequence around the secondary source / ground-zero source block so the source contract is precise instead of inferred.',
    owner: 'Operations',
  },
]
