...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Done for Stage 1. Stop for review. Real Georgia send is not executed here; it requires Steve\'s exact SEND APPROVED and is scoped under AGENT-FEEDBACK-GEORGIA-SEND-001.',
    statusNote: 'Closed on 2026-04-30 under agent-feedback-send-v1 for Stage 1 only. Dry-run first. No real send without Steve exact SEND APPROVED. Georgia test target. Internal/team recipients like Georgia use ClickUp Company Email; external agents use ClickUp Personal Email. Onboarding feedback sends use BCC/internal oversight roles Steve, Carson, Ryan, and Georgia by default, with To-recipient dedupe so Georgia is not duplicated when she is To. Capture Gmail message/thread ID only in Stage 2 after approval. Built eligibility, metadata-only dry-run, duplicate-send protection, Gmail send path wiring, Requested writeback sequencing, privacy checks, and verifier coverage. Mark Requested in ClickUp only after Gmail send succeeds. Duplicate send protection. No send if recipient source email is missing. No send if milestone window expired. Contract Link is warning/data-quality metadata only and does not block 30/60/90 onboarding feedback send eligibility. Georgia dry-run proof is metadata-only: day-30 milestone, due status, recipient source, BCC roles, token hash, ClickUp Requested writeback plan, and Contract Link warning status when missing. No Gmail send. No ClickUp Requested writeback. No Georgia survey. Stage 2 real send was not executed and requires Steve exact SEND APPROVED naming Georgia, milestone/day, To: ClickUp Company Email, BCC/internal oversight roles, one-send limit, and Gmail-before-Requested sequencing. Submitted feedback still writes Completed, score, and feedback text back to the correct ClickUp Onboarding NPS 30/60/90 Status, Score, and Feedback fields. Feedback response is stored in the agent_onboarding_feedback_responses table with task ID, agent name, milestone day, token hash, score, feedback, and submitted timestamp. Feedback content is not broadly exposed outside approved owner/review surfaces. Private feedback token URLs, raw email addresses, and feedback content are not copied into tracked proof.',
  },
  {
    ...
    nextAction: 'Done for readiness. Stop for review. Next decision is whether Steve approves test-only Georgia/day-30 live-send mode with AGENT_FEEDBACK_AUTO_SEND_ENABLED=true plus an approved allowlist artifact. Production-all requires a separate approval artifact.',
    statusNote: 'Closed on 2026-05-01 under agent-feedback-auto-send-v1 as auto-send readiness only. V1 adds a daily dry-run/report scanner over the ClickUp Agent Roster for 30/60/90 onboarding feedback candidates, reports would-send/sent/skipped/blocked/warning/repair counts in Runtime Health and Ops, and keeps default behavior dry-run/report-only. Live sends require both AGENT_FEEDBACK_AUTO_SEND_ENABLED=true and an approved mode/allowlist artifact; toggle alone cannot send, allowlist alone cannot send, and production-all requires a separate approval artifact. Georgia Day-30 is discovered as would_send in dry-run using Company Email as internal/team To source and BCC/internal oversight roles Steve, Carson, Ryan, and Georgia with To-recipient dedupe. Contract Link remains warning-only. Duplicate-send protection and Gmail-before-ClickUp Requested sequencing remain enforced; if Gmail succeeds but ClickUp Requested fails, the send attempt stays in sent/repair state and does not become resendable. No Gmail send, no ClickUp Requested writeback, no broad live auto-send, no raw email addresses, token URLs, or feedback content were written to tracked docs, verifier logs, build log, or broad API JSON. Closeout owns only AGENT-FEEDBACK-AUTO-SEND-001; AGENT-FEEDBACK-GEORGIA-SEND-001 remains scoped/context for any later test-only live-send approval.',
  },
  {
  ...
  },
  {
    ...
    summary: 'Approve and execute the Georgia day-30 onboarding feedback email only through the governed test-only live-send path.',
    whyItMatters: 'The readiness scanner can discover Georgia as would_send, but a real email and ClickUp Requested writeback are still external actions. They need explicit test-only approval, one-send proof, duplicate protection, and Gmail-before-Requested sequencing.',
    nextAction: 'If Steve approves test-only Georgia/day-30 live-send mode, require AGENT_FEEDBACK_AUTO_SEND_ENABLED=true plus an approved allowlist artifact naming Georgia/day 30, Company Email as To source, BCC roles Steve/Carson/Ryan/Georgia with Georgia deduped, one-send limit, and ClickUp Requested writeback only after Gmail succeeds.',
    statusNote: 'Scoped by AGENT-FEEDBACK-SEND-001 and superseded in design by AGENT-FEEDBACK-AUTO-SEND-001 readiness controls. This card remains the context/follow-up for the first Georgia day-30 test-only live send if Steve approves it. Internal/team recipients like Georgia use ClickUp Company Email as the To source; external agents default to ClickUp Personal Email. Onboarding feedback sends use BCC/internal oversight roles Steve, Carson, Ryan, and Georgia by default; if Georgia is To, her BCC address is deduped so she does not receive two copies. Contract Link is warning/data-quality metadata only and does not block 30/60/90 onboarding feedback send eligibility. Proof must remain metadata-only except Gmail message/thread IDs and ClickUp Requested writeback confirmation after an approved send; no token URLs, raw email addresses, or feedback content in tracked proof.',
  },
  {
  ...
  },
  {
    ...
    summary: 'The v1 evidence proof is now recorded under `SOURCE-021-PROOF-001`. The remaining active work is narrower: prove the current writer path for `leaddate` / `leadclaimeddate` and decide plain-English Sales Hub coaching language for opportunity re-entry, recycled leads, pond/unclaimed ownership, and `Active Client` semantics.',
    ...
    nextAction: 'Use `docs/source-notes/fub-zahnd-middleware.md` and `docs/source-notes/fub-kpi-deal-connection-map.md` as the working contract. Prove which current process writes `leaddate` and `leadclaimeddate`, then lock coaching copy that does not praise fake lead creation or misread `Active Client` rows.',
    statusNote: 'Backlog hygiene pass on 2026-04-28 split completed v1 evidence into `SOURCE-021-PROOF-001` instead of leaving the whole card half-closed. This card remains executing only for current writer ownership and coaching-language closeout. Proof command remains `npm run foundation:verify`; do not close without writer-path proof.',
  },
  {
  ...
  try {
    ...
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS agent_onboarding_feedback_send_attempts (
        ...
        status TEXT NOT NULL
          CHECK (status IN ('sending', 'sent', 'clickup_requested', 'failed')),
    ...
    await client.query(
      ...
      [
        ...
        'Done for v1. Stop for review. Next expected card is AGENT-FEEDBACK-SEND-001 unless Steve pulls FOUNDATION-SYSTEMS-EMPTY-GROUP-AUDIT-001 forward first. SYS-AGENT-ONBOARDING-FEEDBACK-001 stays implementationState partial until AGENT-FEEDBACK-SEND-001 ships.',
    ...
    await client.query(
      ...
      [
        ...
        'Done for Stage 1. Stop for review. Real Georgia send is not executed here; it requires Steve\'s exact SEND APPROVED and is scoped under AGENT-FEEDBACK-GEORGIA-SEND-001.',
        'Closed on 2026-04-30 under agent-feedback-send-v1 for Stage 1 only. Dry-run first. No real send without Steve exact SEND APPROVED. Georgia test target. Internal/team recipients like Georgia use ClickUp Company Email; external agents use ClickUp Personal Email. Onboarding feedback sends use BCC/internal oversight roles Steve, Carson, Ryan, and Georgia by default, with To-recipient dedupe so Georgia is not duplicated when she is To. Capture Gmail message/thread ID only in Stage 2 after approval. Built eligibility, metadata-only dry-run, duplicate-send protection, Gmail send path wiring, Requested writeback sequencing, privacy checks, and verifier coverage. Mark Requested in ClickUp only after Gmail send succeeds. Duplicate send protection. No send if recipient source email is missing. No send if milestone window expired. Contract Link is warning/data-quality metadata only and does not block 30/60/90 onboarding feedback send eligibility. Georgia dry-run proof is metadata-only: day-30 milestone, due status, recipient source, BCC roles, token hash, ClickUp Requested writeback plan, and Contract Link warning status when missing. No Gmail send. No ClickUp Requested writeback. No Georgia survey. Stage 2 real send was not executed and requires Steve exact SEND APPROVED naming Georgia, milestone/day, To: ClickUp Company Email, BCC/internal oversight roles, one-send limit, and Gmail-before-Requested sequencing. Submitted feedback still writes Completed, score, and feedback text back to the correct ClickUp Onboarding NPS 30/60/90 Status, Score, and Feedback fields. Feedback response is stored in the agent_onboarding_feedback_responses table with task ID, agent name, milestone day, token hash, score, feedback, and submitted timestamp. Feedback content is not broadly exposed outside approved owner/review surfaces. Private feedback token URLs, raw email addresses, and feedback content are not copied into tracked proof.',
      ]
    )
    ...
    await client.query(
      ...
      [
        ...
        'Done for readiness. Stop for review. Next expected step is Steve full-loop test planning; no live reminder, Georgia send, Steve test, production auto-send, or ClickUp Requested writeback happened in this build.',
    ...
    await client.query(
      ...
      [
        ...
        'Done for readiness. Stop for review. Next decision is whether Steve approves test-only Georgia/day-30 live-send mode with AGENT_FEEDBACK_AUTO_SEND_ENABLED=true plus an approved allowlist artifact. Production-all requires a separate approval artifact.',
        'Closed on 2026-05-01 under agent-feedback-auto-send-v1 as auto-send readiness only. V1 adds a daily dry-run/report scanner over the ClickUp Agent Roster for 30/60/90 onboarding feedback candidates, reports would-send/sent/skipped/blocked/warning/repair counts in Runtime Health and Ops, and keeps default behavior dry-run/report-only. Live sends require both AGENT_FEEDBACK_AUTO_SEND_ENABLED=true and an approved mode/allowlist artifact; toggle alone cannot send, allowlist alone cannot send, and production-all requires a separate approval artifact. Georgia Day-30 is discovered as would_send in dry-run using Company Email as internal/team To source and BCC/internal oversight roles Steve, Carson, Ryan, and Georgia with To-recipient dedupe. Contract Link remains warning-only. Duplicate-send protection and Gmail-before-ClickUp Requested sequencing remain enforced; if Gmail succeeds but ClickUp Requested fails, the send attempt stays in sent/repair state and does not become resendable. No Gmail send, no ClickUp Requested writeback, no broad live auto-send, no raw email addresses, token URLs, or feedback content were written to tracked docs, verifier logs, build log, or broad API JSON. Closeout owns only AGENT-FEEDBACK-AUTO-SEND-001; AGENT-FEEDBACK-GEORGIA-SEND-001 remains scoped/context for any later test-only live-send approval.',
      ]
    )
    ...
    await client.query(
      ...
      [
        ...
        'Done for v1. Stop for review. Georgia live-send remains paused until Steve decides whether to approve test-only Georgia/day-30 live-send mode.',
    ...
    await client.query(
      ...
      [
        ...
        'Approve and execute the Georgia day-30 onboarding feedback email only through the governed test-only live-send path.',
        'The readiness scanner can discover Georgia as would_send, but a real email and ClickUp Requested writeback are still external actions. They need explicit test-only approval, one-send proof, duplicate protection, and Gmail-before-Requested sequencing.',
        'If Steve approves test-only Georgia/day-30 live-send mode, require AGENT_FEEDBACK_AUTO_SEND_ENABLED=true plus an approved allowlist artifact naming Georgia/day 30, Company Email as To source, BCC roles Steve/Carson/Ryan/Georgia with Georgia deduped, one-send limit, and ClickUp Requested writeback only after Gmail succeeds.',
        'Scoped by AGENT-FEEDBACK-SEND-001 and superseded in design by AGENT-FEEDBACK-AUTO-SEND-001 readiness controls. This card remains the context/follow-up for the first Georgia day-30 test-only live send if Steve approves it. Internal/team recipients like Georgia use ClickUp Company Email as the To source; external agents default to ClickUp Personal Email. Onboarding feedback sends use BCC/internal oversight roles Steve, Carson, Ryan, and Georgia by default; if Georgia is To, her BCC address is deduped so she does not receive two copies. Contract Link is warning/data-quality metadata only and does not block 30/60/90 onboarding feedback send eligibility. Proof must remain metadata-only except Gmail message/thread IDs and ClickUp Requested writeback confirmation after an approved send; no token URLs, raw email addresses, or feedback content in tracked proof.',
      ]
    )
...
export async function updateAgentFeedbackSendAttemptStatus(id, input = {}) {
  ...
  if (!['sending', 'sent', 'clickup_requested', 'failed'].includes(status)) {
