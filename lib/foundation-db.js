...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Plan a narrow reliability slice before Phase G Track 2: identify the deadlock source, add bounded retry/transaction-order/advisory-lock hardening where appropriate, document the behavior, and add synthetic or live regression proof that transient DB deadlocks are handled without hiding permanent failures.',
    statusNote: 'Not built. GATE-PERFORMANCE-001 added one transient retry inside `process:foundation-ship`, but the Phase 1 review still saw raw `npm run foundation:verify` fail once with `deadlock detected` before passing on rerun. This card owns the durable reliability fix, not UI work or broader performance optimization.',
  },
  {
