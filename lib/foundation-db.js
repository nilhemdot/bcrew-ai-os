...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'Execute the real Georgia day-30 onboarding feedback email only after Steve gives exact route-specific SEND APPROVED.',
    whyItMatters: 'The send infrastructure is dry-run proven, but a real email and ClickUp Requested writeback are external actions. They need one explicit approval and one-send proof.',
    nextAction: 'Rerun Georgia Day-30 readiness dry-run with internal/team recipient source = ClickUp Company Email and BCC/internal oversight roles Steve, Carson, Ryan, and Georgia. If eligible, wait for Steve exact SEND APPROVED naming Georgia, day 30, To: ClickUp Company Email, BCC roles, one-send limit, and ClickUp Requested writeback only after Gmail succeeds.',
    statusNote: 'Scoped by AGENT-FEEDBACK-SEND-001. This Stage 2 card owns the first real Georgia test send only. Internal/team recipients like Georgia use ClickUp Company Email as the To source; external agents default to ClickUp Personal Email. Onboarding feedback sends use BCC/internal oversight roles Steve, Carson, Ryan, and Georgia by default; if Georgia is To, her BCC address is deduped so she does not receive two copies. Contract Link is warning/data-quality metadata only and does not block 30/60/90 onboarding feedback send eligibility. The readiness dry-run must show recipientSource metadata and BCC/internal oversight metadata only: company_email or personal_email, field name/hash, present/valid booleans, bccRolesApplied, bccMissingConfiguredRoles only when an approved internal identity is unavailable, contractLinkStatus missing_warning or present, and no raw email addresses. It must not run without explicit SEND APPROVED. Proof must remain metadata-only except Gmail message/thread IDs and ClickUp Requested writeback confirmation after an approved send; no token URLs, raw email addresses, or feedback content in tracked proof.',
  },
  {
  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        'Execute the real Georgia day-30 onboarding feedback email only after Steve gives exact route-specific SEND APPROVED.',
        'The send infrastructure is dry-run proven, but a real email and ClickUp Requested writeback are external actions. They need one explicit approval and one-send proof.',
        'Rerun Georgia Day-30 readiness dry-run with internal/team recipient source = ClickUp Company Email and BCC/internal oversight roles Steve, Carson, Ryan, and Georgia. If eligible, wait for Steve exact SEND APPROVED naming Georgia, day 30, To: ClickUp Company Email, BCC roles, one-send limit, and ClickUp Requested writeback only after Gmail succeeds.',
        'Scoped by AGENT-FEEDBACK-SEND-001. This Stage 2 card owns the first real Georgia test send only. Internal/team recipients like Georgia use ClickUp Company Email as the To source; external agents default to ClickUp Personal Email. Onboarding feedback sends use BCC/internal oversight roles Steve, Carson, Ryan, and Georgia by default; if Georgia is To, her BCC address is deduped so she does not receive two copies. Contract Link is warning/data-quality metadata only and does not block 30/60/90 onboarding feedback send eligibility. The readiness dry-run must show recipientSource metadata and BCC/internal oversight metadata only: company_email or personal_email, field name/hash, present/valid booleans, bccRolesApplied, bccMissingConfiguredRoles only when an approved internal identity is unavailable, contractLinkStatus missing_warning or present, and no raw email addresses. It must not run without explicit SEND APPROVED. Proof must remain metadata-only except Gmail message/thread IDs and ClickUp Requested writeback confirmation after an approved send; no token URLs, raw email addresses, or feedback content in tracked proof.',
      ]
    )
