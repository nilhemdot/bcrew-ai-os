...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Confirm Slack access on this machine, identify the channels or workflows that matter as shared cross-hub context, and record whether Slack is workflow-critical, optional, or future-only.',
    statusNote: 'Shared-communications candidate. Lower priority than Gmail / Calendar / Drive, but still a Foundation boundary decision.',
  },
  {
    id: 'SOURCE-006',
  ...
  },
  {
    ...
    nextAction: 'Use the now-live meeting-note archive and task-candidate lane as the first slice, then extend the same pattern to normalized records across Gmail / Calendar / Missive and add the next governed extraction path after task candidates prove out.',
    statusNote: 'First slice is live: meeting notes land in the shared communications archive and explicit `Next steps` bullets become pending task candidates. Remaining work is broader normalization, more source coverage, and additional governed extraction types.',
  },
  {
    id: 'SOURCE-020',
  ...
  try {
    ...
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS shared_communication_artifacts (
        ...
        artifact_type TEXT NOT NULL
          CHECK (artifact_type IN ('meeting_note', 'email_thread', 'calendar_event', 'missive_thread', 'slack_thread')),
    ...
    await client.query(
      ...
      [
        'SOURCE-018',
        'scoped',
        'Keep the verified Google meeting-note read path as the source boundary, then deepen the archive contract: meeting identity, participants, transcript body, task candidates, decision candidates, and linkage into governed review workflows.',
        'Delegated meeting-note reads are now live in the rebuild: Gemini-note search on `ai@bensoncrew.ca`, archive-root access on `crewbert@bensoncrew.ca`, and doc export to text. The remaining work is governed archive depth and extraction, not basic access.',
      ]
    )
    ...
    await client.query(
      ...
      [
        'SOURCE-019',
        'scoped',
        'Use the now-live meeting-note archive and task-candidate lane as the first slice, then extend the same pattern to normalized records across Gmail / Calendar / Missive and add the next governed extraction path after task candidates prove out.',
        'First slice is live: meeting notes land in the shared communications archive and explicit `Next steps` bullets become pending task candidates. Remaining work is broader normalization, more source coverage, and additional governed extraction types.',
      ]
    )
    ...
    await client.query(
      ...
      [
        'SOURCE-020',
        'Keep the now-live Gmail / Calendar / Drive / meeting-note readers and Missive bridge stable, then harden broader Drive scope and revalidate Slack as read-only before widening the normalized shared-communications layer.',
        'The port-and-harden path is working: Gmail, Calendar, Drive search/list/export, meeting-note reads, and Missive thread reads are live in the rebuild. Remaining adapter work is Slack plus wider Drive / normalization hardening, not blank-sheet connector work.',
      ]
    )
