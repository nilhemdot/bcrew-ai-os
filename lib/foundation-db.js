...
function formatMonthlyAgentRate(value) {
  ...
  if (!Number.isFinite(numeric)) return '—'
  const rounded = Math.round(numeric * 10) / 10
  return `${(rounded % 1 === 0 ? String(Math.round(rounded)) : rounded.toFixed(1))} / month`
}
...
async function getLiveAgentEngineSourceSnapshot() {
  ...
  const [bhagCalcRes, bhagTargetsRes, engineRes] = await Promise.all([
    getSheetValues('service-account', FREEDOM_SHEET_ID, "'Benson Crew Bhag Builder'!A22:B30"),
  ...
  const splitToAgent = parseNumber(calcMap['Split To Agent'])
  const gciPerAgentAverage = parseNumber(calcMap['GCI / Agent Average'])
  const monthlyGciAverage = parseNumber(calcMap['Average Monthly GCI'])
  const totalNetToTeam = parseNumber(calcMap['Total Net to Team'])
  const totalToAgent = parseNumber(calcMap['Total To Agent'])
  const commissionAverage = parseNumber(calcMap['Commission Average'])
  ...
  const recruitingGap = Number.isFinite(requiredRecruitingPace) && Number.isFinite(recruitingSixMonth)
    ...
    : null
  const capacityGap = Number.isFinite(activeAgents) && Number.isFinite(requiredStartAgents)
    ? activeAgents - requiredStartAgents
  ...
  const snapshot = [
    {
      ...
      label: 'GCI / Agent Average',
      value: formatCompactCurrency(gciPerAgentAverage),
      detail: inputsDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 1,
    },
    {
      ...
      value: `${formatCompactCurrency(monthlyGciAverage)} / month`,
      detail: inputsDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 2,
    },
    {
      ...
      sourceId: 'SRC-FREEDOM-BHAG-001',
      groupTitle: 'Engine Inputs',
      label: 'Annual Volume Average',
      value: formatCompactCurrency(annualVolumeAverage),
      detail: inputsDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 3,
    },
    {
      ...
      sourceId: 'SRC-FREEDOM-BHAG-001',
      groupTitle: 'Engine Inputs',
      label: 'Split to Agent',
      value: formatPercent(splitToAgent),
      detail: inputsDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 4,
    },
    ...
    },
    {
      ...
      sourceId: 'SRC-FREEDOM-BHAG-001',
      groupTitle: 'Engine Inputs',
      label: 'Total to Agent',
      value: formatCompactCurrency(totalToAgent),
      detail: inputsDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 6,
    },
    {
      ...
      sourceId: 'SRC-FREEDOM-BHAG-001',
      groupTitle: 'Engine Inputs',
      label: 'Total Net to Team',
      value: formatCompactCurrency(totalNetToTeam),
      detail: inputsDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 7,
    },
    {
      ...
      sourceId: 'SRC-FREEDOM-BHAG-001',
      groupTitle: 'Engine Inputs',
      label: 'Commission Average',
      value: formatPercent(commissionAverage),
      detail: inputsDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 8,
    },
  ...
  snapshot.push(
    ...
    },
    {
      ...
      value: formatAgentCountCeil(requiredStartAgents),
      detail: requirementBhagDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 2,
    },
    {
      ...
      sourceId: 'SRC-FREEDOM-BHAG-001',
      groupTitle: 'Current Requirement',
      label: 'Current Productivity Assumption',
      value: `${formatCompactCurrency(monthlyGciAverage)} / month`,
      detail: requirementBhagDetail,
      asOf: getTodayTorontoIso(),
      sortOrder: 3,
    },
    buildEngineMetricRow('Current Requirement', 'Current Active Agents', formatAgentCountPrecise(activeAgents), requirementEngineDetail, 4),
    buildEngineMetricRow('Current Requirement', 'Capacity Gap', formatPaceWithPercent(capacityGap, requiredStartAgents, formatAgentDelta), requirementEngineDetail, 5),
    buildEngineMetricRow('Current Requirement', 'Required Recruiting Pace', formatMonthlyAgentRate(requiredRecruitingPace), requirementEngineDetail, 6),
    buildEngineMetricRow('Current Requirement', 'Current Recruiting Pace', formatMonthlyAgentRate(recruitingSixMonth), requirementEngineDetail, 7),
    buildEngineMetricRow('Current Requirement', 'Current Avg Production / Agent', `${formatCompactCurrency(avgProduction)} / month`, requirementEngineDetail, 8),
    buildEngineMetricRow('Current Requirement', 'Production Target / Agent', `${formatCompactCurrency(productionTarget)} / month`, requirementBhagDetail, 9, 'SRC-FREEDOM-BHAG-001'),
    buildEngineMetricRow('Current Requirement', 'Production Gap', formatPaceWithPercent(productionGap, productionTarget, formatCurrencyDelta), requirementEngineDetail, 10),
    buildEngineMetricRow('Current Requirement', 'Planning Attrition Assumption', 'Pending source-backed input', 'This assumption is still buried in the recruiting formula and needs to be exposed cleanly in the source of truth.', 11, 'SRC-FREEDOM-BHAG-001'),
    buildEngineMetricRow('Current Requirement', 'Live Attrition Pressure', formatPercent(attritionRate), requirementEngineDetail, 12),
    buildEngineMetricRow('Current Requirement', 'Avg Additions / Month', formatMonthlyAgentRate(avgAdditions), requirementEngineDetail, 13),
    buildEngineMetricRow('Current Requirement', 'Avg Attrition / Month', formatMonthlyAgentRate(avgAttrition), requirementEngineDetail, 14),
    buildEngineMetricRow('Current Requirement', 'Actual Split', formatPercent(actualSplit), requirementEngineDetail, 15),
    buildEngineMetricRow('Current Requirement', 'Target Split', formatPercent(targetSplit), requirementEngineDetail, 16),
    buildEngineMetricRow('Current Requirement', 'Split Gap', `${splitGap >= 0 ? 'Above target' : 'Below target'} by ${formatPointDelta(splitGap)}`, requirementEngineDetail, 17),
  )
