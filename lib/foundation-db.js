...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Build the operator coverage-by-target view next: last success, last failure, next bite, remaining backlog, per-source quota, failed-item retry state, and lane consistency across source_crawl_items.',
    statusNote: 'Source crawl targets/items now support scheduled current-day sync, Drive inventory, video-link inventory, and daily shared-comms extraction missions. 2026-04-28 hard checkpoint found stale `slack-current-day` run `crawl-slack-current-day-20260427145904292-3f93bebd`; the Slack lane now records channel-level source_crawl_items, Runtime Health shows item summaries/health findings, and scheduled targets now display Foundation job schedule as visible truth while target next_run_at is preserved only as runner checkpoint metadata. Remaining work is operator-facing coverage truth, retry semantics, and source-specific backlog completion state.',
  },
  {
