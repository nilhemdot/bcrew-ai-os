...
const backlogSeed = [
  ...
  },
  {
    ...
    nextAction: 'Implement a read-only Drive content extractor for Google Docs and PDFs from the existing Drive inventory queue. Store normalized text artifacts with file id, URL, mime type, content hash, owner/source folder, extraction status, skip reason, and source-backed provenance. Run as a daily quota mission after inventory and keep write/move/copy actions out of scope.',
    statusNote: 'Urgent source gap. Service account access appears available; this is an extractor implementation gap, not a permission problem. Daily Drive lane currently inventories/maps only.',
  },
  {
    ...
    nextAction: 'Add an attachment lane for Gmail and Missive: detect attachments, record manifest rows, download only approved mime types and size ranges, route Google Drive/Docs links to the Drive extractor, parse PDFs/docs/text first, mark unsupported images/media for later multimodal extraction, and connect outputs to the same shared-comms/source artifact ledger.',
    statusNote: 'Urgent source gap for strategy readiness. Current daily email sync should continue, but it should not be treated as full email understanding until attachment extraction exists.',
  },
  {
    ...
    nextAction: 'Use the video-link inventory output as the input queue. First resolve Google Drive video/Zoom/Loom/YouTube links from meeting notes, classify access and rights, prefer existing transcripts where available, then run a small approved video-review proof with timestamped outputs before scheduling daily video missions.',
    statusNote: 'Urgent but dependent on multimodal/video policy. Current meeting text coverage is real; video/recording review is not built yet.',
  },
  {
    id: 'SKOOL-WORKER-001',
