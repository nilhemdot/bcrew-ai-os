...
const backlogSeed = [
  ...
  },
  {
    ...
    summary: 'Create a Foundation admin surface so Steve can manage AIOS users without editing `.env`: list users, add email/name/role, disable users, and review the access-change audit trail.',
    whyItMatters: 'As AIOS moves beyond local-only founder use, user access cannot live in hidden environment JSON. Steve needs an owner-only control panel that changes access audibly without exposing passwords or broadening the full auth/redaction scope.',
    nextAction: 'Define the owner-only user admin flow: list active/disabled users, add email/name/role, disable user, write audit events, hide password/secret material, and add verifier coverage that non-owners cannot manage access.',
    statusNote: 'Backlog follow-up only; do not build during the EXTRACT-CONTROL-001 schedule reconciliation slice. Tie this to SECURITY-002/auth-tier work, but keep it smaller than full subject-person redaction and hub access policy.',
  },
  {
    id: 'SECURITY-003',
  ...
  },
  {
    ...
    nextAction: 'Continue from the Slack current-day proof: add retry/backoff for failed source_crawl_items, coverage-by-target views, and the next missing corpus lane only after Runtime Health makes the unhealthy/thin list clear.',
    statusNote: 'First runtime slice is live: registered jobs, DB-backed run history, CLI runner, source-crawl target ledger, dashboard visibility, Drive Docs/PDF/text content extraction, Gmail PDF/text attachment extraction, YouTube subtitle transcript extraction, and 2026-04-28 Slack current-day channel item proof. Missing pieces are richer multimodal extractors, Missive/richer attachment extraction, retry semantics, coverage views, and governed action routing.',
  },
  {
    id: 'DRIVE-CORPUS-001',
  ...
  },
  {
    ...
    nextAction: 'Add the operator coverage view: last success, last failure, next bite, remaining backlog, per-source quota, failed-item retry state, and schedule reconciliation between Foundation jobs and target next_run_at.',
    statusNote: 'Source crawl targets/items now support scheduled current-day sync, Drive inventory, video-link inventory, and daily shared-comms extraction missions. 2026-04-28 hard checkpoint found stale `slack-current-day` run `crawl-slack-current-day-20260427145904292-3f93bebd`; the Slack lane now records channel-level source_crawl_items, Runtime Health shows item summaries/health findings, and the controlled proof run succeeded with explicit succeeded/skipped channel counts. Remaining work is operator-facing coverage truth, retry semantics, schedule reconciliation, and source-specific backlog completion state.',
  },
  {
  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        'P0',
        7,
        'Continue from the Slack current-day proof: add retry/backoff for failed source_crawl_items, coverage-by-target views, and the next missing corpus lane only after Runtime Health makes the unhealthy/thin list clear.',
...
function minutesBetween(leftIso, rightIso) {
  ...
  return Math.round(Math.abs(leftMs - rightMs) / 60000)
}
...
function buildSourceCrawlTargetHealthFindings(target) {
  ...
  const scheduleGapMinutes = minutesBetween(scheduler.nextRunAt, scheduler.targetNextRunAt)
  if (
    scheduler.source === 'foundation_job' &&
    scheduler.nextRunAt &&
    scheduler.targetNextRunAt &&
    scheduleGapMinutes != null &&
    scheduleGapMinutes > 5
  ) {
    findings.push({
      severity: 'warning',
      type: 'job_target_schedule_mismatch',
      detail: `Foundation job next run and target next_run_at differ by ${scheduleGapMinutes} minutes.`,
      jobNextRunAt: scheduler.nextRunAt,
      targetNextRunAt: scheduler.targetNextRunAt,
    })
  }
