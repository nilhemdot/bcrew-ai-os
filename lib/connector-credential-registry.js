...
export const CONNECTOR_CREDENTIAL_DEFINITIONS = [
  ...
  },
  {
    ...
    unlockedWorkloads: ['Calendar/event reads', 'meeting cadence context'],
    lastProbeKind: 'metadata_presence',
    blockerReason: 'Calendar is readable but not scheduled as an atom-producing source.',
  },
  {
    key: 'fub-api',
...
function statusForDefinition(definition, requirements) {
  if (normalizeText(definition.blockerReason)) return 'blocked'
...
export function buildConnectorCredentialRegistrySnapshot({
  ...
  const rows = CONNECTOR_CREDENTIAL_DEFINITIONS.map(definition => {
    ...
    const sourceUnlocked = status === 'available' && (sourceState.anyPresent || !definition.sourceIds?.length) && !normalizeText(definition.blockerReason)
