import {
  ...
  buildFoundationDormantCapabilityAuditSummary,
  buildFoundationDoneSemanticsSummary,
  classifyFoundationDormantCapability,
...
function makeDogfoodCards() {
  return [
    ...
    {
      id: 'DOGFOOD-DORMANT-WIRE-NOW-001',
      title: 'Synthetic dormant wire-now',
      scope: 'foundation',
      lane: 'done',
      priority: 'P1',
      summary: 'Capability was built but never wired, not triggered, and not active.',
      nextAction: 'Wire it now or explicitly block, park, or retire it.',
      statusNote: 'Dormant capability audit fixture.',
    },
    {
      id: 'DOGFOOD-DORMANT-BLOCKED-001',
      title: 'Synthetic dormant blocked',
      scope: 'foundation',
      lane: 'done',
      priority: 'P1',
      summary: 'Capability is dormant and not live because approval and credential setup are blocked.',
      nextAction: 'Wait for approval before live execution.',
      statusNote: 'Blocked preflight fixture.',
    },
    {
      id: 'DOGFOOD-DORMANT-PARKED-001',
      title: 'Synthetic dormant parked',
      scope: 'foundation',
      lane: 'done',
      priority: 'P1',
      summary: 'Capability is dormant and parked until after remap.',
      nextAction: 'Keep parked for the explicit checkpoint.',
      statusNote: 'Deferred fixture.',
    },
    {
      id: 'DOGFOOD-DORMANT-RETIRED-001',
      title: 'Synthetic dormant retired',
      scope: 'foundation',
      lane: 'done',
      priority: 'P1',
      summary: 'Capability is dormant and retired because it was replaced by a shared runtime hook.',
      nextAction: 'No active wiring remains.',
      statusNote: 'Superseded fixture.',
    },
  ]
}
...
export function buildFoundationDoneSemanticsRepairDogfoodProof() {
  ...
  const dormantById = Object.fromEntries(cards.map(card => [card.id, classifyFoundationDormantCapability(card)]))
  const summary = buildFoundationDoneSemanticsSummary(cards.map(card => ({
    ...card,
    doneSemantics: byId[card.id],
  })))
  const dormantSummary = buildFoundationDormantCapabilityAuditSummary(cards)
  ...
  return {
    ok: byId['DOGFOOD-BLOCKED-PREFLIGHT-001'].status === 'blocked_preflight' &&
      ...
      summary.nonFeatureCompleteDoneItems === 8 &&
      summary.featureCompleteClaimAllowedItems === 1 &&
      dormantById['DOGFOOD-DORMANT-WIRE-NOW-001'].status === 'wire_now' &&
      dormantById['DOGFOOD-DORMANT-BLOCKED-001'].status === 'blocked_preflight' &&
      dormantById['DOGFOOD-DORMANT-PARKED-001'].status === 'parked' &&
      dormantById['DOGFOOD-DORMANT-RETIRED-001'].status === 'retired' &&
      dormantSummary.reviewCandidateIds.includes('DOGFOOD-DORMANT-WIRE-NOW-001'),
    classifications: byId,
    dormantClassifications: dormantById,
    summary,
    dormantSummary,
  }
}
...
export function buildFoundationDoneSemanticsRepairSnapshot({
  ...
  const dormantAudit = summary.dormantCapabilityAudit || {}
  ...
  addCheck(
    checks,
    dormantAudit.rule === 'Built-but-unwired done rows must resolve to wire-now, blocked-preflight, parked, or retired.' &&
      dogfood.dormantClassifications?.['DOGFOOD-DORMANT-WIRE-NOW-001']?.action === 'wire_now' &&
      dogfood.dormantClassifications?.['DOGFOOD-DORMANT-BLOCKED-001']?.action === 'blocked_preflight' &&
      dogfood.dormantClassifications?.['DOGFOOD-DORMANT-PARKED-001']?.action === 'park' &&
      dogfood.dormantClassifications?.['DOGFOOD-DORMANT-RETIRED-001']?.action === 'retire',
    'dormant-capability audit classifies done rows as wire-now, blocked, parked, or retired',
    dormantAudit.reviewCandidateItems === undefined
      ? 'missing dormant audit'
      : `${dormantAudit.reviewCandidateItems} wire-now candidates`,
  )
  addCheck(
    checks,
