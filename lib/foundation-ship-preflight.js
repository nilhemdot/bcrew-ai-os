...
export function buildFoundationShipPreflightDogfoodProof({ now = new Date() } = {}) {
  ...
  const blockedCases = [stale, missing, missingProbe]
  ...
  return {
    ...
    healthy,
    blockedCases,
    dogfoodInvariant: 'Fresh fixture passes; stale/missing LLM auth fixtures fail through the same preflight function path and print the approved repair command.',
  }
}
