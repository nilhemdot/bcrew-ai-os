...
export function buildYoutubeLatest20IntelRunSnapshot({
  ...
  const requestedCreatorRows = requestedCreatorIds.length
    ...
    : []
  const requestedCreatorExhausted = requestedCreatorIds.length > 0 &&
  ...
  addCheck(checks, selectedVideos.length >= 1 || requestedCreatorExhausted, 'next non-Mark public videos are selected or requested creator is exhausted', requestedCreatorExhausted ? 'exhausted' : `${selectedVideos.length}`)
  ...
  const status = requestedCreatorExhausted
  ...
  return {
    ...
    nextAction: requestedCreatorExhausted
      ? 'No relevant unwatched public videos remain for the requested creator under the current Build Intel guard.'
