...
export const YOUTUBE_LATEST_20_MAX_SELECTED_CREATORS = 3
export const YOUTUBE_LATEST_20_MAX_VIDEOS_PER_CREATOR = 3
...
export function selectYoutubeLatest20IntelRunVideos({
  ...
  const selected = []
  for (const creatorId of Array.from(byCreator.keys()).sort().slice(0, Math.max(1, Number(maxCreators) || YOUTUBE_LATEST_20_MAX_SELECTED_CREATORS))) {
    selected.push(...byCreator.get(creatorId).slice(0, Math.max(1, Number(maxVideosPerCreator) || YOUTUBE_LATEST_20_MAX_VIDEOS_PER_CREATOR)))
    if (selected.length >= maxRunVideos) break
  }
  return selected.slice(0, Math.max(1, Number(maxRunVideos) || YOUTUBE_LATEST_20_MAX_RUN_VIDEOS))
}
...
export function buildYoutubeLatest20IntelRunSnapshot({
  ...
  addCheck(checks, Object.values(selectedByCreator).every(count => count <= YOUTUBE_LATEST_20_MAX_VIDEOS_PER_CREATOR), 'selected run caps videos per creator', JSON.stringify(selectedByCreator))
  ...
  return {
    ...
    nextAction: 'Run this selected small batch through the full God Mode Gemini API path, then rerun Director and Scoper with resource-link dispositions.',
