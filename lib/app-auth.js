...
export function setRuntimeAuthUsers(users = []) {
  runtimeAuthUsers = (Array.isArray(users) ? users : [])
    .map(item => ({
      email: normalizeEmail(item.email),
      name: String(item.name || item.email || '').trim(),
      role: normalizeRole(item.role),
...
export function getAuthUsers() {
  ...
  for (const user of parseAuthUsersJson(raw)) {
    byEmail.set(user.email, user)
  }
  for (const user of runtimeAuthUsers) {
    byEmail.set(user.email, user)
  }
