...
function buildRepoDeepReviewQueue(topRepos = []) {
  const rows = list(topRepos).map(repo => {
    ...
    return {
      ...
      nextAction: 'Run `repo:deep-review` for README/docs/examples/license/provenance and extract implementation patterns only; do not clone, install, download, import, or write backlog from this queue.',
    }
  }).sort((left, right) =>
  ...
  return {
    ...
    nextAction: 'Use the top rows with `repo:deep-review`: README/docs/examples/license/provenance first, implementation patterns second, source citations always.',
  }
}
...
function buildRepoImplementationPackets(topRepos = []) {
  ...
  return {
    ...
    nextAction: 'Use these packets to choose which public repos deserve `repo:deep-review` execution before any human/Codex implementation review.',
  }
}
