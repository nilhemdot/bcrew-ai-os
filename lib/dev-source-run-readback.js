...
function buildSourceBrowserAgentReadback(rows = []) {
  ...
  for (const row of agentRows) {
    ...
    brainRouteCounts[brainRoute] = number(brainRouteCounts[brainRoute], 0) + 1
    ...
    if (operatorEscalation) operatorEscalationPreparedCount += 1
  }
  const topPlans = agentRows
    .map(row => {
      ...
      return {
        ...
        brainRoute: text(agentPlan.brainRoute?.routeId || metadata.brainRoute?.routeId),
        selectedBrain: text(agentPlan.brainRoute?.selectedBrain || metadata.brainRoute?.selectedBrain),
        ...
        operatorEscalation: operatorEscalation ? {
          status: text(operatorEscalation.status),
          cardId: text(operatorEscalation.cardId),
          finalStatus: text(operatorEscalation.finalStatus),
          primaryChannel: text(operatorEscalation.notification?.primaryChannel),
          channels: list(operatorEscalation.notification?.channels).slice(0, 4),
          dryRun: operatorEscalation.notification?.dryRun === true,
          externalSent: operatorEscalation.notification?.externalSent === true,
          sendsMessageNow: operatorEscalation.sendsMessageNow === true,
          waitsForDoneToken: text(operatorEscalation.waitsForDoneToken),
        } : null,
  ...
  return {
    ...
    operatorEscalationPreparedCount,
    unsafeSideEffectRows,
    topPlans,
    rawArtifactPathsReturned: false,
