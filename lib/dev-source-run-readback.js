...
function buildRepoDeepReviewQueue(topRepos = []) {
  const rows = list(topRepos).map(repo => {
    ...
    return {
      ...
      reason: reasons.join(' · '),
      nextAction: 'Deep-review README/docs/examples/license/provenance and extract implementation patterns only; do not clone, install, download, import, or write backlog from this queue.',
    }
  }).sort((left, right) =>
  ...
  return {
    ...
    plainEnglish: 'This ranks public repos already found from watched-video evidence for the future repo deep-review worker. It is a review queue, not a clone/install/import lane.',
    nextAction: 'Use the top rows to run or build the governed repo worker: README/docs/examples/license/provenance first, implementation patterns second, source citations always.',
  }
}
...
function repoEvidenceCompleteness(repo = {}) {
  ...
  const checks = [
    { key: 'readme', label: 'README/root overview', seen: /\breadme\b|\/readme/i.test(corpus) },
    { key: 'docs', label: 'Docs/architecture', seen: /\bdocs?\b|architecture|guide|quickstart|getting started/i.test(corpus) },
    { key: 'examples', label: 'Examples/samples', seen: /example|sample|recipe|template|demo/i.test(corpus) },
    { key: 'license', label: 'License/provenance', seen: /license|mit|apache|bsd|gpl|provenance/i.test(corpus) },
...
function buildRepoImplementationPackets(topRepos = []) {
  ...
  const rows = queueRows.map(repo => {
    ...
    return {
      ...
      plainEnglish: `${repo.label || 'This repo'} is a read-only implementation review candidate from saved YouTube/source-browser evidence. The packet is for deciding whether deeper repo review is worth it, not for importing code.`,
      nextAction: completeness.completeEnoughForReview
        ? 'Review README/docs/examples/license/provenance and summarize reusable implementation patterns with citations.'
  ...
  return {
    ...
    rows,
    plainEnglish: 'These packets turn saved public repo metadata into reviewable implementation candidates. They are not extraction runs and they do not import code.',
    nextAction: 'Use these packets to choose which public repos deserve a human/Codex implementation review after source-session work is stable.',
  }
}
