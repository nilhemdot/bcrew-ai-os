...
function latestJobStatusForGroup(jobs = []) {
  ...
  if (attendedJobs.some(job => job.status === 'risk' || job.latestRun?.status === 'failed')) {
...
export function buildFoundationOperatingReliabilityDogfoodProof() {
  ...
  const checks = [
    ...
    },
    {
      ...
      check: 'runtime activation distinguishes blocked mutating checks and old manual failures from current failed jobs',
