...
export function buildMorningHealthSnapshot({
  ...
  const deepAuditCard = backlogItems.find(item => item.id === 'RECURRING-DEEP-AUDIT-001') || null
  ...
  if (!codeQualityJob || codeQualityJob.state === RUNTIME_ACTIVATION_STATES.manual) {
    findings.push(finding({
      id: 'deterministic_scanner_manual',
      severity: 'P1',
      title: 'Deterministic code-quality scanner is not the same as a recurring deep review',
      detail: `The deterministic scanner job is ${codeQualityJob?.state || 'missing'} and report path is ${CODE_QUALITY_NIGHTLY_AUDIT_REPORT_PATH}.`,
      nextAction: 'Review scanner output manually until recurring report quality is approved.',
    }))
  }
  if (!deepAuditCard || deepAuditCard.lane !== 'done') {
    findings.push(finding({
      id: 'recurring_deep_audit_not_running',
      severity: 'P1',
      title: 'Recurring senior-engineer deep audit is not built yet',
      detail: deepAuditCard
        ? `RECURRING-DEEP-AUDIT-001 is lane ${deepAuditCard.lane}.`
        : 'RECURRING-DEEP-AUDIT-001 is missing from live backlog.',
      nextAction: 'Keep this as the follow-up reviewer card; do not confuse it with the deterministic scanner.',
    }))
  }
  ...
  return {
    ...
    deterministicScanner: {
      ...
      note: 'This deterministic scanner is not the recurring senior-engineer deep code audit.',
    },
    recurringDeepAudit: {
      cardId: 'RECURRING-DEEP-AUDIT-001',
      lane: deepAuditCard?.lane || 'missing',
      built: deepAuditCard?.lane === 'done',
      note: 'Recurring deep audit remains a follow-up card with manual approval per run.',
    },
    summary: {
...
export function buildFoundationOperatingReliabilityDogfoodProof() {
  ...
  const checks = [
    ...
    {
      ok: morningHealth.findings.some(item => item.id === 'deterministic_scanner_manual') &&
      ...
      check: 'morning health distinguishes deterministic scanner from recurring deep audit without mutation',
