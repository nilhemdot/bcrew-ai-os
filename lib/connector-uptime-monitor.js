...
function jobsForConnectorGroup(jobs = [], group = {}) {
  ...
  return jobs.filter(job =>
    job.key !== CONNECTOR_UPTIME_MONITOR_JOB_KEY &&
      normalizeList(job.sourceIds).some(sourceId => sourceIds.has(sourceId)),
  )
}

function isManualConnectorJob(job = {}) {
  return ['manual', 'paused', 'decommissioned'].includes(job.runtimeMode) ||
    ['manual', 'paused', 'decommissioned'].includes(job.scheduleStatus) ||
    job.enabled === false
}
...
function latestJobStatusForGroup(jobs = []) {
  ...
  const attendedJobs = jobs.filter(job => !isManualConnectorJob(job))
  if (!attendedJobs.length) {
    return { status: CONNECTOR_HEALTH_STATUSES.healthy, reason: 'Referenced connector jobs are manual-only; old manual failures do not make current connector health red.' }
  }
  if (attendedJobs.some(job => {
    const reconciliation = getAgentFeedbackAutoSendJobRunReconciliation(job.latestRun)
    return reconciliation.reconciled !== true && (job.status === 'risk' || job.latestRun?.status === 'failed')
  })) {
    return { status: CONNECTOR_HEALTH_STATUSES.degraded, reason: 'At least one connector job reports a failed or risk status.' }
  }
  if (attendedJobs.some(job => job.scheduleStatus === 'blocked')) {
    return { status: CONNECTOR_HEALTH_STATUSES.blocked, reason: 'At least one connector job is blocked by schedule posture.' }
  }
  if (attendedJobs.some(job => job.scheduleStatus === 'due')) {
...
export function buildConnectorUptimeSnapshot({
  ...
  const rows = OPERATING_RELIABILITY_CONNECTOR_GROUPS.map(group => {
    ...
    const readinessNotes = unique(registryRows.map(row => row.readinessNote).filter(Boolean))
    ...
    const reasons = unique([
      credential.reason,
      ...readinessNotes.map(note => `Readiness note: ${note}`),
      jobStatus.reason,
      probe?.reason,
    ])
    ...
    return {
      ...
      credentialReadinessNoteCount: readinessNotes.length,
      credentialReadinessNotes: readinessNotes,
      jobKeys: groupJobs.map(job => job.key),
      jobStatuses: groupJobs.map(job => {
        const reconciliation = getAgentFeedbackAutoSendJobRunReconciliation(job.latestRun)
        return {
          key: job.key,
          status: reconciliation.reconciled ? 'reconciled' : job.status || job.scheduleStatus || 'unknown',
          scheduleStatus: job.scheduleStatus || 'unknown',
          latestRunStatus: job.latestRun?.status || null,
          latestRunHealthStatus: reconciliation.reconciled ? 'reconciled' : job.latestRun?.status || null,
          latestRunReconciled: reconciliation.reconciled === true,
          healthIncluded: !isManualConnectorJob(job),
        }
      }),
...
export function classifyRuntimeJobActivation(job = {}, now = new Date()) {
  ...
  const latestRunReconciliation = getAgentFeedbackAutoSendJobRunReconciliation(job.latestRun)
  ...
  if ((job.latestRun?.status === 'failed' || job.status === 'risk') && latestRunReconciliation.reconciled !== true) return RUNTIME_ACTIVATION_STATES.failed
...
export function buildRuntimeActivationSnapshot({
  ...
  const jobs = jobRows.map(job => {
    ...
    return {
      ...
      latestRunStatus: job.latestRun?.status || null,
      latestRunHealthStatus: latestRunReconciliation.reconciled ? 'reconciled' : job.latestRun?.status || null,
      latestRunReconciled: latestRunReconciliation.reconciled === true,
      latestRunReconciliationStatus: latestRunReconciliation.status || '',
      ...
      mutationAllowlist: job.mutationAllowlist || scheduleGuard.mutationAllowlist || null,
...
export function buildMorningHealthSnapshot({
  ...
  const endpointRiskRows = (endpointBudgets?.rows || []).filter(row => row.status === 'risk')
  const endpointReviewRows = (endpointBudgets?.rows || []).filter(row => row.status === 'review')
  ...
  if (endpointRiskRows.length) {
    findings.push(finding({
      id: 'endpoint_budget_risk',
      severity: 'P1',
      title: 'Operator endpoint budget is at risk',
      detail: endpointRiskRows.map(row => `${row.endpoint}: ${row.reason}`).join('; '),
      nextAction: 'Scope a focused performance card before building more hub work on these routes.',
      source: 'foundation_endpoint_budgets',
    }))
  }
  if (endpointReviewRows.length) {
    findings.push(finding({
      id: 'endpoint_budget_review',
      severity: 'P2',
      title: 'Operator endpoint budgets need review',
      detail: endpointReviewRows.map(row => `${row.endpoint}: ${row.reason}`).join('; '),
      nextAction: 'Run or inspect the latest nightly deep audit endpoint metrics before assuming route health.',
      source: 'foundation_endpoint_budgets',
    }))
  }
  ...
  return {
    ...
    summary: {
      ...
      endpointBudgetRiskCount: endpointRiskRows.length,
      endpointBudgetReviewCount: endpointReviewRows.length,
      endpointBudgetMissingCount: endpointBudgets?.summary?.missingCount || 0,
...
export function buildFoundationOperatingReliabilityDogfoodProof() {
  ...
  const manualFailureOnlyConnectors = buildConnectorUptimeSnapshot({
    now: new Date('2026-05-14T09:00:00.000Z'),
    credentialRegistry: {
      generatedAt: new Date('2026-05-14T09:00:00.000Z').toISOString(),
      rows: CONNECTOR_CREDENTIAL_DEFINITIONS.map(definition => ({
    ...
    foundationJobs: {
      jobs: [
        {
          key: 'manual-shared-comms-intelligence-bite',
          sourceIds: ['SRC-SLACK-001'],
          runtimeMode: 'manual',
          scheduleStatus: 'manual',
          status: 'manual',
          latestRun: { status: 'failed', finishedAt: '2026-04-24T12:00:00.000Z' },
        },
      ],
    },
    probeResults: {
      slack: { ok: true, reason: 'ok' },
    },
  })
  const reconciledConnectorFailure = buildConnectorUptimeSnapshot({
    now: new Date('2026-05-14T09:00:00.000Z'),
    credentialRegistry: {
      generatedAt: new Date('2026-05-14T09:00:00.000Z').toISOString(),
      rows: CONNECTOR_CREDENTIAL_DEFINITIONS.map(definition => ({
      ...
      jobs: [
        {
          ...
          latestRun: {
            status: 'failed',
            finishedAt: '2026-05-17T12:41:25.825Z',
            metadata: {
              agentFeedbackAutoSendReconciliation: {
                key: AGENT_FEEDBACK_AUTO_SEND_RECONCILIATION_KEY,
                status: AGENT_FEEDBACK_AUTO_SEND_RECONCILED_STATUS,
    ...
    probeResults: {
      clickup: { ok: true, reason: 'ok' },
    },
  })
  const selfMonitorFailureDoesNotPoisonConnectors = buildConnectorUptimeSnapshot({
    now: new Date('2026-05-14T09:00:00.000Z'),
    credentialRegistry: {
      generatedAt: new Date('2026-05-14T09:00:00.000Z').toISOString(),
      rows: CONNECTOR_CREDENTIAL_DEFINITIONS.map(definition => ({
    ...
    foundationJobs: {
      jobs: [
        {
          key: CONNECTOR_UPTIME_MONITOR_JOB_KEY,
          sourceIds: OPERATING_RELIABILITY_CONNECTOR_GROUPS.flatMap(group => group.sourceIds),
          runtimeMode: 'scheduled',
          scheduleStatus: 'scheduled',
          status: 'risk',
          latestRun: { status: 'failed', finishedAt: '2026-05-14T08:30:00.000Z' },
        },
      ],
    },
    probeResults: Object.fromEntries(OPERATING_RELIABILITY_CONNECTOR_GROUPS.map(group => [
      group.key,
      { ok: true, reason: 'ok' },
    ])),
  })
  const runtimeActivation = buildRuntimeActivationSnapshot({
    ...
    foundationJobs: {
      jobs: [
        ...
        {
          key: 'manual-old-failure',
          title: 'Manual Old Failure',
          runtimeMode: 'manual',
          scheduleStatus: 'manual',
          sourceIds: [],
          mutationPosture: 'report_only',
        ...
        },
        {
          ...
          scheduleMutationGuard: { ok: true, mutationPosture: 'external_write' },
          status: 'risk',
          latestRun: {
            status: 'failed',
            finishedAt: '2026-05-17T12:41:25.825Z',
  ...
  const checks = [
    ...
    {
      ok: manualFailureOnlyConnectors.rows.some(row =>
        row.key === 'slack' &&
          row.status === CONNECTOR_HEALTH_STATUSES.healthy &&
          row.jobStatuses.some(job => job.healthIncluded === false && job.latestRunStatus === 'failed'),
      ),
      check: 'old manual connector job failures stay visible but do not poison current connector health',
      detail: manualFailureOnlyConnectors.rows.map(row => `${row.key}:${row.status}`).join(', '),
    },
    {
      ok: reconciledConnectorFailure.rows.some(row =>
        row.key === 'clickup' &&
          row.status === CONNECTOR_HEALTH_STATUSES.healthy &&
          row.jobStatuses.some(job => job.key === 'agent-feedback-auto-send-readiness' && job.latestRunReconciled === true),
      ),
      check: 'reconciled external-write job failures stay visible but do not degrade connector health',
      detail: reconciledConnectorFailure.rows.map(row => `${row.key}:${row.status}`).join(', '),
    },
    {
      ok: selfMonitorFailureDoesNotPoisonConnectors.rows.every(row =>
        row.status === CONNECTOR_HEALTH_STATUSES.healthy &&
          !row.jobStatuses.some(job => job.key === CONNECTOR_UPTIME_MONITOR_JOB_KEY),
      ),
      check: 'connector uptime monitor failures do not self-poison connector health',
      detail: selfMonitorFailureDoesNotPoisonConnectors.rows.map(row =>
        `${row.key}:${row.status}:${row.jobKeys.join('|') || 'no-self-job'}`,
      ).join(', '),
    },
    {
      ok: leakCheck.ok,
    ...
    },
    {
      ok: runtimeActivation.jobs.some(job => job.key === 'blocked-mutating-check' && job.state === RUNTIME_ACTIVATION_STATES.blocked) &&
        ...
        runtimeActivation.jobs.some(job => job.key === 'manual-old-failure' && job.state === RUNTIME_ACTIVATION_STATES.manual) &&
        runtimeActivation.jobs.some(job => job.key === 'agent-feedback-auto-send-readiness' && job.state === RUNTIME_ACTIVATION_STATES.scheduled && job.latestRunReconciled === true),
      check: 'runtime activation distinguishes blocked mutating checks, old manual failures, and reconciled external-write failures from current failed jobs',
  ...
  return {
    ...
    checks,
    connectorUptime,
    manualFailureOnlyConnectors,
    reconciledConnectorFailure,
    selfMonitorFailureDoesNotPoisonConnectors,
    runtimeActivation,
    morningHealth,
  }
}
