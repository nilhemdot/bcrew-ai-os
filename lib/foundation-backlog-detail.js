...
export const FOUNDATION_BACKLOG_LIST_ENDPOINT_BUDGET_BYTES = 1_100_000
...
export function buildFoundationBacklogListPayload({
  ...
  return {
    ...
    summary: {
      ...
      doneItems: items.filter(item => item.lane === 'done').length,
...
export function validateFoundationBacklogListPayload(payload = {}) {
  ...
  if (Number(payload?.summary?.totalItems) !== (payload?.backlogItems || []).length) failures.push('summary total does not match backlog rows')
  if (!Array.isArray(payload?.meta?.backlogScopes)) failures.push('backlog scopes metadata missing')
  return {
    ok: failures.length === 0,
    failures,
    totalItems: payload?.backlogItems?.length || 0,
  }
}
