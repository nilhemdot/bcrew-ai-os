...
import { getLatestChangeEventForEntity } from './foundation-strategy-docs-db.js'
...
function isGoogleDelegatedAuthFailure(error) {
  const message = error instanceof Error ? error.message : String(error || '')
  return /invalid_grant|No valid verifier for issuer|delegated/i.test(message)
}

function buildOwnersGoogleSourceHealth({ mode = 'delegated', error = null } = {}) {
  if (mode === 'service-account-direct-fallback') {
    return {
      status: 'degraded',
      reason: 'google_delegated_auth_invalid_grant',
      accessMode: mode,
      delegatedUser: FOUNDATION_GOOGLE_USER,
      fallbackUser: 'service-account',
      safeToUse: false,
      externalOutageBlocksCoreApi: false,
      message: 'Google delegated impersonation failed; served read-only Owners governance from service-account direct access.',
...
async function loadOwnersLeadSourceGovernanceGoogleInputs(userEmail) {
  const [listResponse, fileMeta] = await Promise.all([
    getSheetValues(userEmail, OWNERS_SHEET_ID, OWNERS_LEAD_SOURCE_LIST_RANGE),
    getDriveFileMetadata(userEmail, OWNERS_SHEET_ID),
  ])
  return { listResponse, fileMeta }
}

async function loadOwnersReviewQueueGoogleInputs(userEmail) {
  const [adminResponse, conditionalResponse, fileMeta, ownersListResponse] = await Promise.all([
    getSheetValues(userEmail, OWNERS_SHEET_ID, OWNERS_ADMIN_REVIEW_RANGE),
    getSheetValues(userEmail, OWNERS_SHEET_ID, OWNERS_CONDITIONAL_REVIEW_RANGE),
    getDriveFileMetadata(userEmail, OWNERS_SHEET_ID),
    getSheetValues(userEmail, OWNERS_SHEET_ID, OWNERS_LEAD_SOURCE_LIST_RANGE),
  ])
  return { adminResponse, conditionalResponse, fileMeta, ownersListResponse }
}

async function withOwnersGoogleDelegationFallback(loader) {
  try {
    return {
      ...(await loader(FOUNDATION_GOOGLE_USER)),
      sourceHealth: buildOwnersGoogleSourceHealth({ mode: 'delegated' }),
    }
  } catch (error) {
    if (!isGoogleDelegatedAuthFailure(error)) throw error
    return {
      ...(await loader('service-account')),
      sourceHealth: buildOwnersGoogleSourceHealth({
        mode: 'service-account-direct-fallback',
        error,
      }),
    }
  }
...
export function registerOwnersGovernanceRoutes(app, deps = {}) {
  ...
  app.get('/api/owners/lead-source-governance', requireAdminToken, async (_req, res) => {
    try {
      const [googleInputs, rules] = await Promise.all([
        withOwnersGoogleDelegationFallback(loadOwnersLeadSourceGovernanceGoogleInputs),
        listFubLeadSourceRules(),
      ])
      const { listResponse, fileMeta, sourceHealth } = googleInputs
      ...
      const payload = {
        ...
        ownersList: {
          ...
          accessMode: sourceHealth.accessMode,
  ...
  app.get('/api/owners/review-queue', requireAdminToken, async (_req, res) => {
    try {
      ...
      const [googleInputs, fubSnapshot, rules, agentRosterSnapshot] = await Promise.all([
        withOwnersGoogleDelegationFallback(loadOwnersReviewQueueGoogleInputs),
        getFubLeadSourceSnapshot('owner'),
        listFubLeadSourceRules(),
        getClickUpListSnapshotSafe(CLICKUP_AGENT_ROSTER_LIST_ID, { listName: 'Agent Roster' }),
      ])
      const { adminResponse, conditionalResponse, fileMeta, ownersListResponse, sourceHealth } = googleInputs
