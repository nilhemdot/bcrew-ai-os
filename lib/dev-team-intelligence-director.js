...
function buildSourceValueLookup(sourceValueGrader = {}) {
  const grader = sourceValueGrader || {}
  const sourceGrades = list(grader.sourceGrades || grader.structuredOutputJson?.sourceGrades || grader.structured_output_json?.sourceGrades)
  const byId = new Map()
  const byName = new Map()
  const byReportId = new Map()
  const reportRows = new Map()

  for (const source of sourceGrades) {
    ...
    const row = {
      ...
      devBuildScore: Number(devLane.score || source.devBuildScore || 0),
    ...
    for (const reportId of list(source.reportIds).map(text).filter(Boolean)) {
      if (!reportRows.has(reportId)) reportRows.set(reportId, [])
      reportRows.get(reportId).push(row)
    }
  }

  for (const [reportId, rows] of reportRows.entries()) {
    const uniqueRows = []
    const seen = new Set()
    ...
    if (uniqueRows.length === 1) byReportId.set(reportId, uniqueRows[0])
  }

  return { byId, byName, byReportId, sourceGrades }
}

function resolveSourceValue(candidate = {}, lookup = {}) {
  const byId = lookup.byId || new Map()
  const byName = lookup.byName || new Map()
  const byReportId = lookup.byReportId || new Map()
  return byId.get(text(candidate.creatorId)) ||
    byName.get(normalizeName(candidate.creator)) ||
    byReportId.get(text(candidate.sourceReportArtifactId)) ||
    null
}

function sourceQualityScore(sourceValue = null) {
  const grade = text(sourceValue?.devBuildGrade).toUpperCase()
...
function scoreMissionAlignment(candidate = {}) {
  ...
  const creatorQualityScore = sourceQualityScore(candidate.sourceValue)
  const total = Math.max(0, Math.min(100, Math.round(laneTotal + evidenceScore + qualityScore + confidenceScore + sourceTrustScore + creatorQualityScore)))
  return {
    total,
    laneScores,
    evidenceScore,
    ...
    confidenceScore,
    sourceTrustScore,
    sourceQualityScore: creatorQualityScore,
  }
}
...
function buildScopeReadiness(candidate = {}) {
  ...
  const devSourceGrade = text(candidate.sourceValue?.devBuildGrade).toUpperCase()
  ...
  if (['C', 'D'].includes(devSourceGrade)) missing.push('stronger Dev source grade')
...
function normalizeCandidate(raw = {}, source = {}) {
  ...
  return {
    ...
    creator: text(raw.creator || raw.creatorName || raw.metadata?.creator),
    creatorId: text(raw.creatorId || raw.creator_id || raw.metadata?.creatorId),
    ...
    sourceTitle: text(raw.sourceTitle || raw.source_title || raw.metadata?.sourceTitle || raw.metadata?.source_title),
    sourceVideoTitle: text(raw.sourceVideoTitle || raw.source_video_title || raw.videoTitle || raw.video_title || raw.metadata?.sourceVideoTitle || raw.metadata?.source_video_title),
...
export function buildDevTeamIntelligenceDirectorSnapshot({
  ...
  const sourceValueLookup = buildSourceValueLookup(sourceValueGrader)
  ...
  for (const candidate of candidates.filter(Boolean)) {
    ...
    const scored = {
      ...candidate,
      sourceValue: resolveSourceValue(candidate, sourceValueLookup),
    }
    scored.missionScore = scoreMissionAlignment(scored)
    if (scored.sourceValue && !scored.creatorId) {
      scored.creatorId = scored.sourceValue.creatorId
      scored.creator = scored.creator || scored.sourceValue.creator
    }
  ...
  const rankedCandidates = Array.from(deduped.values())
    .sort((left, right) => right.missionScore.total - left.missionScore.total ||
      right.missionScore.sourceQualityScore - left.missionScore.sourceQualityScore ||
  ...
  const checks = [
    ...
    {
      ok: sourceValueLookup.sourceGrades.length === 0 || recommendedBuildNow.every(candidate => !['C', 'D'].includes(text(candidate.sourceValue?.devBuildGrade).toUpperCase())),
      check: 'Director ranking uses Dev creator source grade and keeps C/D sources out of top build-now recommendations',
      detail: recommendedBuildNow.map(candidate => `${candidate.rank}:${candidate.sourceValue?.devBuildGrade || 'ungraded'}:${candidate.title}`).join(' | '),
    },
  ]
  ...
  return {
    ...
    mission: {
      ...
      creatorSourceGradeInfluence: {
        laneId: 'aios_dev_build',
        source: 'grader:build-intel-source-value-grader-001:v1',
        scoring: 'S +10, A +7, B +3, C -6, D -12. This affects Dev idea ranking only; source evidence remains in Foundation for other hubs.',
      },
    },
    sourceCoverage,
    inputArtifactIds,
...
export function buildDevTeamIntelligenceDirectorWriteSet(snapshot = {}) {
  ...
  const reportArtifact = {
    ...
    topFindings: topCandidates.map(candidate => ({
      ...
      creator: candidate.creator || candidate.sourceValue?.creator,
      creatorId: candidate.creatorId || candidate.sourceValue?.creatorId,
      ...
      sourceDevBuildScore: candidate.sourceValue?.devBuildScore || 0,
...
export function buildDevTeamDirectorOperatorDogfood() {
  ...
  const ambiguousReportId = 'batch:dogfood:ambiguous-source-grade'
  const snapshot = buildDevTeamIntelligenceDirectorSnapshot({
    ...
    sourceValueGrader: {
      sourceGrades: [
        {
          creatorId: 'ambiguous-s-source',
          creator: 'Ambiguous S Source',
          devBuildGrade: 'S',
          reportIds: [ambiguousReportId],
          laneScores: [{ laneId: 'aios_dev_build', grade: 'S', score: 92 }],
        },
        {
          creatorId: 'ambiguous-d-source',
          creator: 'Ambiguous D Source',
          devBuildGrade: 'D',
          reportIds: [ambiguousReportId],
          laneScores: [{ laneId: 'aios_dev_build', grade: 'D', score: 18 }],
        },
      ],
    },
    reportBundles: [
      ...
      {
        report: {
          reportArtifactId: ambiguousReportId,
          title: 'Dogfood ambiguous multi-creator report',
          reportType: 'extractor_report',
          status: 'generated',
          metadata: {
            fullWatchRoute: 'gemini_api_youtube_url_video_understanding',
          ...
          structuredOutputJson: {
            buildCandidates: [
              {
                title: 'Ambiguous Report Source Grade Candidate',
                why: 'AIOS agent workflow evidence is useful, but this candidate has no creator identity and the source report belongs to multiple graded creators.',
                recommendedNextStep: 'Keep source provenance visible but do not inherit a creator grade from the shared report ID.',
                confidence: 'high',
                qualityScore: 8,
                evidenceTimestamps: ['02:10'],
                sourceVideoId: 'ambiguous-video-1',
              },
            ],
          },
          actionRequiredItems: [],
  ...
  const ambiguousCandidate = list(snapshot.rankedCandidates)
    .find(candidate => candidate.title === 'Ambiguous Report Source Grade Candidate')
  ...
  return {
    ok: snapshot.ok === true &&
      ...
      !ambiguousCandidate?.sourceValue &&
      ambiguousCandidate?.missionScore?.sourceQualityScore === 0 &&
    ...
    checks: [
      ...
      {
        ok: !ambiguousCandidate?.sourceValue && ambiguousCandidate?.missionScore?.sourceQualityScore === 0,
        check: 'Director does not inherit creator grade from ambiguous multi-creator report ID',
        detail: ambiguousCandidate
          ? `sourceValue=${ambiguousCandidate.sourceValue?.devBuildGrade || 'none'} score=${ambiguousCandidate.missionScore?.sourceQualityScore}`
...
export function renderDevTeamIntelligenceDirectorReport(snapshot = {}) {
  ...
  for (const candidate of list(snapshot.recommendedBuildNow)) {
    ...
    lines.push(`- Dev source grade: ${candidate.sourceValue?.devBuildGrade || 'ungraded'} (${candidate.missionScore?.sourceQualityScore ?? 0})`)
