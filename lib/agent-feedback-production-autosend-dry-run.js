...
import {
  buildSyntheticAgentFeedbackAutoSendReconciliationProof,
...
export const AGENT_FEEDBACK_PRODUCTION_AUTOSEND_DRY_RUN_PROOF_PATH = 'docs/_archive/audits/2026-05-01-agent-feedback-production-autosend-dry-run.md'
...
export async function buildAgentFeedbackProductionAutoSendDryRunStatus({
  ...
  const [
    ...
    readOptionalText(repoRoot, 'lib/hub-read-routes.js'),
    readOptionalText(repoRoot, 'scripts/foundation-verify.mjs'),
    readOptionalText(repoRoot, 'lib/foundation-agent-feedback-verifier.js'),
  ...
  addFinding(findings, includesAll(routeSource, ['agentFeedbackProductionAutoSendDryRun', '/api/foundation/agent-feedback-production-dry-run', '/api/ops/agent-feedback-production-dry-run']), 'live Foundation/Ops APIs expose Stage 1 proof', 'server.js + lib/hub-read-routes.js')
  addFinding(findings, verifierCoverageSource.includes(AGENT_FEEDBACK_PRODUCTION_AUTOSEND_ENABLE_CARD_ID) && verifierCoverageSource.includes('buildAgentFeedbackProductionAutoSendDryRunStatus'), 'foundation verifier covers Stage 1 dry-run', AGENT_FEEDBACK_PRODUCTION_AUTOSEND_ENABLE_CARD_ID)
...
async function summarizeJobRuntime(foundationJobs = {}) {
  ...
  const compactLatestRun = job?.latestRun || latestRuns.find(run => run.jobKey === AGENT_FEEDBACK_AUTO_SEND_JOB_KEY) || null
  const latestRun = compactLatestRun?.runId && !compactLatestRun?.metadata
    ? await getFoundationJobRunById(compactLatestRun.runId, { includeOutput: false }).catch(() => compactLatestRun)
    : compactLatestRun
  ...
  return {
    ...
    lastRunHealthStatus: reconciliation.reconciled ? 'reconciled' : latestRun?.status || '',
    lastRunReconciled: reconciliation.reconciled === true,
    lastRunReconciliationStatus: reconciliation.status || '',
...
export async function buildAgentFeedbackProductionAutoSendEnableStatus({
  ...
  const [
    ...
    Promise.all([
      ...
      readOptionalText(repoRoot, 'public/foundation-decision-question-renderers.js'),
  ...
  const jobRuntime = await summarizeJobRuntime(foundationJobs || {})
  const summary = autoSend.summary || {}
  const reminder = foundationHub?.agentFeedbackReminders || opsHub?.agentFeedbackReminders || null
  const reconciliationDogfood = buildSyntheticAgentFeedbackAutoSendReconciliationProof()
  const lastRunCompletedOrReconciled = Boolean(jobRuntime.lastRunAt) &&
    (jobRuntime.lastRunStatus === 'succeeded' || jobRuntime.lastRunReconciled === true)
  ...
  addFinding(findings, includesAll(routeSource, ['agentFeedbackAutoSend', 'foundationJobs']), 'Foundation/Ops APIs expose auto-send state with job runtime', 'server.js + lib/hub-read-routes.js')
  ...
  addFinding(findings, verifierCoverageSource.includes('buildAgentFeedbackProductionAutoSendEnableStatus'), 'foundation verifier covers production enable status', 'scripts/foundation-verify.mjs')
  ...
  addFinding(findings, reconciliationDogfood.ok === true, 'dogfood accepts only no-open-side-effect stale-reaper reconciliation', reconciliationDogfood.ok ? 'synthetic stale-reaper repair proof' : JSON.stringify(reconciliationDogfood.rejected || {}))
  addFinding(findings, lastRunCompletedOrReconciled, 'last production run succeeded or is reconciled without external-write rerun', `${jobRuntime.lastRunHealthStatus || jobRuntime.lastRunStatus || 'missing'} ${jobRuntime.lastRunAt || ''}`.trim())
  ...
  return {
    ...
    summary: {
      ...
      lastRunHealthStatus: jobRuntime.lastRunHealthStatus,
      lastRunReconciled: jobRuntime.lastRunReconciled,
      lastRunReconciliationStatus: jobRuntime.lastRunReconciliationStatus,
      reconciliationDogfoodOk: reconciliationDogfood.ok === true,
