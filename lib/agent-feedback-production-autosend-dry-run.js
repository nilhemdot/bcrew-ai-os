...
export async function buildAgentFeedbackProductionAutoSendEnableStatus({
  ...
  addFinding(findings, foundationUiSource.includes('Production auto-send enabled') && foundationUiSource.includes('Last run:') && foundationUiSource.includes('Send window:'), 'Runtime Health shows enabled state, last run, next run, send window, and counts', 'public/foundation.js')
  ...
  addFinding(findings, reminder?.runtimeMode === 'dry-run-report-only' && reminder?.summary?.dryRunOnly === true, 'live reminders are explicitly report-only', reminder?.runtimeMode || 'missing')
  addFinding(findings, reminderSource.includes('No reminder runs before a successful initial request') || foundationUiSource.includes('Live reminders stay report-only'), 'reminder report-only reason is visible', 'separate approval prevents surprise follow-up sends')
