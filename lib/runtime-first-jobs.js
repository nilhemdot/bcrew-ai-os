...
function dbExportsDelegate(source = '', delegateName) {
  ...
  return text.includes(`export const ${delegateName} = foundationSourceCrawlStore.${delegateName}`) ||
    new RegExp(`export\\s*{[\\s\\S]*\\b${delegateName}\\b[\\s\\S]*}\\s*from './foundation-source-crawl-db\\.js'`, 'm').test(text)
}

function removeDbDelegateExport(source = '', delegateName) {
  return String(source || '')
    .replace(new RegExp(`export const ${delegateName} = foundationSourceCrawlStore\\.${delegateName}\\n?`, 'g'), '')
    .replace(new RegExp(`\\n\\s*${delegateName},`, 'g'), '')
}
...
export function buildRuntimeFirstJobsDogfoodProof(input = {}) {
  ...
  const oldMissingDbExport = evaluateRuntimeFirstJobsContract({
    ...input,
    foundationDbSource: removeDbDelegateExport(
      removeDbDelegateExport(input.foundationDbSource, 'leaseSourceCrawlTarget'),
      'finishSourceCrawlTargetRun',
    ),
  })
