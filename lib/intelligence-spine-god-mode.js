...
import { buildDirectOpenAiSpendPolicyDogfood } from './llm-spend-policy.js'
...
export function buildIntelligenceSpineGodModeDogfood({ runtimeRoutes = null } = {}) {
  ...
  const directOpenAiSpendPolicy = buildDirectOpenAiSpendPolicyDogfood()
  const checks = [
    ...
    {
      ok: directOpenAiSpendPolicy.ok,
      area: 'spend-policy',
      check: 'direct OpenAI API calls stay blocked unless selected by an operator-configured router route',
      detail: JSON.stringify(directOpenAiSpendPolicy.cases),
    },
  ]
  return {
    ok: synthesis.ok &&
    ...
    proof: {
      ...
      spendPolicy: directOpenAiSpendPolicy,
    },
    checks,
