...
export const FOUNDATION_READINESS_GATE_CARDS = [
  ...
  },
  {
    ...
    requiredForStrategy: true,
    closeoutKey: 'meeting-vault-auto-enforcement-v1',
    proofCommands: ['npm run process:meeting-vault-auto-enforcement-check', 'npm run process:meeting-vault-acl-check'],
  },
  {
    cardId: 'MEETING-VAULT-AUTO-ENFORCEMENT-001',
    label: 'Automatic Meeting Vault forward-flow enforcement',
    requiredForStrategy: true,
    closeoutKey: 'meeting-vault-auto-enforcement-v1',
    proofCommands: ['npm run process:meeting-vault-auto-enforcement-check'],
  },
  {
    cardId: 'DRIVE-ACCESS-REQUEST-001',
    label: 'Delegated Drive access-request and ACL repair preflight',
    requiredForStrategy: true,
    closeoutKey: 'drive-access-request-v1',
...
export const FOUNDATION_READINESS_LEGS = [
  ...
  },
  {
    ...
    required: true,
    blockerCards: ['MEETING-VAULT-ACL-001', 'MEETING-VAULT-AUTO-ENFORCEMENT-001', 'DRIVE-ACCESS-REQUEST-001'],
    conditionalCards: ['MEETING-FORWARD-TRANSCRIPT-ENFORCEMENT-001'],
    proofCommands: ['npm run process:meeting-vault-auto-enforcement-check', 'npm run process:meeting-vault-acl-check'],
    passDescription: 'Original Google Drive meeting notes have automatic forward-flow ACL/vault proof, duplicate-doc prevention, and bounded legacy exceptions, not only AIOS-side redaction.',
    failClosedBehavior: 'Return not_ready when raw file owners, permissions, forward-flow enforcement, dry-run proof, legacy exception bounds, or approval boundaries are unproven.',
  },
  {
...
function isDoneWithCloseout(foundationHub, closeouts, cardId, closeoutKey) {
  const closeoutMatches = closeoutExists(closeouts, cardId, closeoutKey)
  return cardLane(foundationHub, cardId) === 'done' &&
    (!closeoutKey || includesText(cardStatusNote(foundationHub, cardId), closeoutKey) || closeoutMatches) &&
    closeoutMatches
}
...
export function buildFoundationReadinessStatus({
  ...
  const meetingBlockers = openRequiredCards(foundationHub, closeouts, [
  ...
  legs.push(meetingBlockers.length
    ? failedLeg(
      ...
      meetingBlockers,
      'SECURITY-002 protects AIOS responses, but raw Google Drive meeting-note ACL/vault forward-flow proof is still open.',
      'Keep DRIVE-ACCESS-REQUEST-001 preflight closed, then close MEETING-VAULT-AUTO-ENFORCEMENT-001 and MEETING-VAULT-ACL-001 only when automatic source-truth forward-flow proof is green and legacy files are bounded.',
    )
