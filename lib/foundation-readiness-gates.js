...
export function buildFoundationReadinessStatus({
  ...
  const missingPackageCommands = FOUNDATION_READINESS_LEGS
    ...
    .filter(scriptName => {
      ...
      if (scriptName.startsWith('process:system-010-ghost-closeout-check')) return false
      if (scriptName.startsWith('process:source-lifecycle-completion-check')) return false
      if (scriptName.startsWith('process:extract-run-hardening-check')) return false
  ...
  legs.push(sourceBlockers.length
    ? failedLeg(
      ...
      sourceBlockers,
      'Source-backed retrieval exists, but source completion and synthesized-claim verification are not both closed.',
