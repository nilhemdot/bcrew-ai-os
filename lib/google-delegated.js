...
export async function createDrivePermission(
  userEmail,
  fileId,
  { emailAddress, domain, role = 'reader', type = 'user', sendNotificationEmail = false } = {},
) {
  const normalizedEmail = String(emailAddress || '').trim();
  const normalizedType = String(type || 'user').trim().toLowerCase();
  const normalizedDomain = String(domain || '').trim().toLowerCase();
  if (!normalizedEmail && ['user', 'group'].includes(normalizedType)) {
    throw new Error('createDrivePermission requires an email address.');
  }
  if (!normalizedDomain && normalizedType === 'domain') {
    throw new Error('createDrivePermission requires a domain for domain permissions.');
  }
...
export async function driveDownloadFile(userEmail, fileId) {
  const url =
    `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}` +
    '?supportsAllDrives=true&alt=media';
  return googleBufferFetch(userEmail, url, { scopes: [GOOGLE_SCOPES.drive] });
}
...
export async function getSheetValues(userEmail, spreadsheetId, range) {
  ...
  const cacheKey = buildGoogleSheetsCacheKey({
    userEmail,
  ...
  return readGoogleSheetsCachedJson({
    cacheKey,
    operation: 'sheets.values.get',
    userEmail,
    spreadsheetId,
    ranges: [range],
    fetcher: () => googleJsonFetch(userEmail, url, { scopes: [GOOGLE_SCOPES.sheets] }),
  });
}

export async function getSheetValuesBatch(userEmail, spreadsheetId, ranges, options = {}) {
  ...
  const majorDimension = options.majorDimension || '';
  const params = new URLSearchParams();
  ...
  const cacheKey = buildGoogleSheetsCacheKey({
    ...
    params: { valueRenderOption, majorDimension },
  });
...
export async function getSpreadsheetMetadata(userEmail, spreadsheetId) {
  const fields = [
    'properties(title)',
  ...
  const cacheKey = buildGoogleSheetsCacheKey({
    userEmail,
    operation: 'sheets.spreadsheets.get.metadata',
    spreadsheetId,
    fields,
  });
  return readGoogleSheetsCachedJson({
...
export async function getSheetGridData(userEmail, spreadsheetId, ranges, fields) {
  ...
  const cacheKey = buildGoogleSheetsCacheKey({
    userEmail,
    operation: 'sheets.spreadsheets.get.gridData',
    spreadsheetId,
    ranges: normalizedRanges,
    fields,
...
function extractGmailAttachments(payload, path = []) {
  if (!payload) return [];
  ...
  parts.forEach((part, index) => {
    attachments.push(...extractGmailAttachments(part, [...path, part?.partId || String(index)]));
  });
...
export async function getGmailMessage(userEmail, messageId) {
  ...
  return {
    ...
    from: getGmailHeader(detail.payload?.headers, 'From') || 'unknown',
    to: getGmailHeader(detail.payload?.headers, 'To') || '',
    cc: getGmailHeader(detail.payload?.headers, 'Cc') || '',
    bcc: getGmailHeader(detail.payload?.headers, 'Bcc') || '',
    snippet: detail.snippet || '',
...
export async function downloadGmailAttachment(userEmail, messageId, attachmentId) {
  const detail = await googleJsonFetch(
    userEmail,
  ...
  return decodeBase64UrlBuffer(detail.data || '');
}
...
function buildHtmlEmailRaw({ from, to, cc = '', bcc = '', replyTo = '', subject, text, html, fromName = '' }) {
  const cleanFrom = sanitizeEmailHeader(from);
  const cleanTo = sanitizeEmailHeader(to);
  const cleanCc = sanitizeEmailHeader(cc);
  const cleanBcc = sanitizeEmailHeader(bcc);
  const cleanReplyTo = sanitizeEmailHeader(replyTo);
  ...
  if (cleanReplyTo) headers.push(`Reply-To: ${cleanReplyTo}`);
  headers.push(
...
export async function sendGmailMessage(userEmail, { to, cc = '', bcc = '', replyTo = '', subject, text, html, fromName = 'Benson Crew' } = {}) {
