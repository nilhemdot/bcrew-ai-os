...
export function createIntelligenceAtomStore({ pool, withFoundationTransaction, insertChangeEvent, getRegisteredSourceIds }) {
  ...
  async function queryIntelligenceAtomsForScoper(filters = {}) {
    ...
    addTextAny('atom_id', filters.atomIds || filters.atom_ids)
  ...
  async function getIntelligenceAtomSpineSnapshot({ limit = 20 } = {}) {
    ...
    const intelAtomScoperQueryProof = await queryIntelligenceAtomsForScoper({
    ...
    return {
      ...
      intelAtomScoperQueryProof,
    }
  }

  async function getIntelligenceReportBundle(reportArtifactId, { atomLimit = 50, hitLimit = 100 } = {}) {
    const normalizedReportArtifactId = String(reportArtifactId || '').trim()
    if (!normalizedReportArtifactId) throw new Error('reportArtifactId is required for intelligence report bundle reads.')
    const normalizedAtomLimit = Math.min(100, Math.max(1, Number(atomLimit) || 50))
    const normalizedHitLimit = Math.min(200, Math.max(1, Number(hitLimit) || 100))
    ...
    const reportResult = await pool.query(
      `
        ...
        LIMIT 1
      `,
      [normalizedReportArtifactId]
    )
    const report = reportResult.rows[0] ? mapIntelligenceReportArtifactRow(reportResult.rows[0]) : null
    const inputAtomIds = report?.inputAtomIds || []
    ...
    const atomsResult = await pool.query(
      `
        ...
        FROM intelligence_atoms
        WHERE report_artifact_id = $1
           OR atom_id = ANY($2::text[])
        ORDER BY relevance_score DESC, quality_score DESC, updated_at DESC
        LIMIT $3
      `,
      [normalizedReportArtifactId, inputAtomIds, normalizedAtomLimit]
    )
    const atoms = atomsResult.rows.map(mapIntelligenceAtomRow)
    const atomIds = atoms.map(atom => atom.atomId).filter(Boolean)
    ...
    const hitsResult = await pool.query(
      `
        ...
        WHERE report_artifact_id = $1
           OR atom_id = ANY($2::text[])
        ORDER BY confidence DESC NULLS LAST, occurred_at DESC, created_at DESC
        LIMIT $3
      `,
      [normalizedReportArtifactId, atomIds, normalizedHitLimit]
    )

    return {
      reportArtifactId: normalizedReportArtifactId,
      report,
      atoms,
      hits: hitsResult.rows.map(mapIntelligenceAtomHitRow),
    }
  }
