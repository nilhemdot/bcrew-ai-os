...
export async function buildAgentFeedbackReminderDryRunReport({
  ...
  const { source, plans } = await buildReminderCandidatePlans({ now })
...
export async function buildAgentFeedbackReminderStatus({
  ...
  const buildEntries = foundationBuildLog?.builds || foundationBuildLog?.closeouts || []
  const closeout = buildEntries.find(build =>
    build.closeoutKey === AGENT_FEEDBACK_LIVE_REMINDERS_CLOSEOUT_KEY ||
      build.key === AGENT_FEEDBACK_LIVE_REMINDERS_CLOSEOUT_KEY
  ) || (buildLogSource.includes(AGENT_FEEDBACK_LIVE_REMINDERS_CLOSEOUT_KEY)
    ? {
        closeoutKey: AGENT_FEEDBACK_LIVE_REMINDERS_CLOSEOUT_KEY,
    ...
    : null
  ) || buildEntries.find(build =>
    build.closeoutKey === AGENT_FEEDBACK_REMINDER_CLOSEOUT_KEY ||
      build.key === AGENT_FEEDBACK_REMINDER_CLOSEOUT_KEY
  ) || null
  ...
  addFinding(findings, currentPlan.includes('AGENT-FEEDBACK-LIVE-REMINDERS-001` is done') && currentPlan.includes('systems visibility pass'), 'current plan records live reminders and next systems visibility pass', 'current-plan')
