...
export const AGENT_FEEDBACK_REMINDER_PROOF_PATH = 'docs/_archive/audits/2026-05-01-agent-feedback-reminder-cadence-proof.md'
...
export const AGENT_FEEDBACK_LIVE_REMINDERS_PROOF_PATH = 'docs/_archive/audits/2026-05-02-agent-feedback-live-reminders-proof.md'
...
async function readFoundationDbVerifierSource(repoRoot) {
  const parts = await Promise.all([
    readOptionalText(repoRoot, 'lib/foundation-db.js'),
    readOptionalText(repoRoot, 'lib/foundation-db-schema-seed-store.js'),
    readOptionalText(repoRoot, 'lib/foundation-agent-feedback-store.js'),
  ])
...
export async function buildAgentFeedbackReminderStatus({
  ...
  const [
    ...
    readFoundationDbVerifierSource(repoRoot),
  ...
  addFinding(findings, includesAll(routeSource, ['buildAgentFeedbackReminderReadiness', 'agentFeedbackReminders']), 'Foundation and Ops APIs expose reminder readiness status', 'server.js + lib/hub-read-routes.js')
  ...
  addFinding(findings, verifierCoverageSource.includes(AGENT_FEEDBACK_LIVE_REMINDERS_CARD_ID), 'foundation verifier covers live reminder card', AGENT_FEEDBACK_LIVE_REMINDERS_CARD_ID)
