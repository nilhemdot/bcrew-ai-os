...
export function buildHarlanAuthEscalationLoop(overrides = {}) {
  return {
    ...
    notification: {
      ...
      recipient: 'Steve',
      channels: ['harlan', 'telegram', 'email'],
...
export function evaluateHarlanAuthEscalationLoop(loop = buildHarlanAuthEscalationLoop()) {
  ...
  for (const channel of ['harlan', 'telegram', 'email']) {
    if (!asList(notification.channels).includes(channel)) addViolation(violations, 'notification', 'channel_required', channel)
  }
