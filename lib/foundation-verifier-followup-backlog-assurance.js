...
const STRATEGIC_INTEL_CLOSEOUT_KEY = 'strategic-intel-loop-v1'
const INTEL_SCOPER_CLOSEOUT_KEY = 'intel-scoper-v1'
const INTEL_THREAD_CONTEXT_CLOSEOUT_KEY = 'intel-thread-context-v1'
const STRATEGY_QUARTER_CLOSEOUT_KEY = 'strategy-quarter-input-layer-v1'
const MODEL_ROUTING_CLOSEOUT_KEY = 'model-routing-v1'
const FOUNDATION_USERS_CLOSEOUT_KEY = 'foundation-users-v1'
...
export async function evaluateFoundationVerifierFollowupBacklogAssurance(input = {}) {
  ...
  const foundationUsersCloseout = (foundationBuildCloseouts || [])
    .find(closeout => closeout.key === FOUNDATION_USERS_CLOSEOUT_KEY) || null
  const foundationUsersAdminClosed =
    foundationUsersAdmin?.lane === 'done' &&
    ...
    String(foundationUsersAdmin.statusNote || '').includes(FOUNDATION_USERS_CLOSEOUT_KEY) &&
    foundationUsersCloseout?.operatorCloseout === true &&
    (foundationUsersCloseout.backlogIds || []).includes('FOUNDATION-USERS-001')
  const foundationUsersAdminTracked =
    foundationUsersAdmin &&
    ['scoped', 'executing'].includes(foundationUsersAdmin.lane) &&
    foundationUsersAdmin.priority === 'P1' &&
    foundationUsersAdminText.includes('owner-only') &&
    foundationUsersAdminText.includes('without editing `.env`') &&
    foundationUsersAdminText.includes('disable') &&
    ...
    checks,
    foundationUsersAdminTracked || foundationUsersAdminClosed,
    'Foundation user/access control panel is tracked or closed',
    foundationUsersAdmin
  ...
  const runtimeSupervisorText = [
    ...
    checks,
    ['scoped', 'done'].includes(systemProcessControl?.lane) &&
      ...
      (
        systemProcessControl?.lane === 'done' ||
        currentPlan.includes('Served-code-equals-HEAD check is live') ||
        currentPlan.includes('Runtime Supervisor service supervision')
      ) &&
      (
        systemProcessControl?.lane === 'done' ||
        currentPlan.includes('Add auto-restart-on-push next') ||
        currentPlan.includes('runtime-supervisor-v1')
      ),
  ...
  const extractMetricsText = backlogItemText(extractMetrics)
    ...
    extractMetrics?.lane === 'done' &&
      ...
      currentPlan.includes('docs/_archive/audits/2026-04-28-extraction-lane-item-shape.md'),
  ...
  const strategicIntelCloseout = (foundationBuildCloseouts || [])
    .find(closeout => closeout.key === STRATEGIC_INTEL_CLOSEOUT_KEY) || null
  const intelScoperCloseout = (foundationBuildCloseouts || [])
    .find(closeout => closeout.key === INTEL_SCOPER_CLOSEOUT_KEY) || null
  const intelThreadContextCloseout = (foundationBuildCloseouts || [])
    .find(closeout => closeout.key === INTEL_THREAD_CONTEXT_CLOSEOUT_KEY) || null
  const strategyQuarterCloseout = (foundationBuildCloseouts || [])
    .find(closeout => closeout.key === STRATEGY_QUARTER_CLOSEOUT_KEY) || null
  const modelRoutingCloseout = (foundationBuildCloseouts || [])
    .find(closeout => closeout.key === MODEL_ROUTING_CLOSEOUT_KEY) || null
  const strategicIntelScopedWithSpec =
  ...
  const strategicIntelClosedWithLedger =
    strategicIntel?.lane === 'done' &&
    String(strategicIntel?.statusNote || '').includes(STRATEGIC_INTEL_CLOSEOUT_KEY) &&
    strategicIntelCloseout?.operatorCloseout === true &&
    (strategicIntelCloseout.backlogIds || []).includes('STRATEGIC-INTEL-001') &&
    strategicIntelText.includes('intelligence_strategic_issues') &&
    strategicIntelText.includes('issue events are live') &&
    strategicIntelText.includes('weekly targets are encoded') &&
    strategicIntelText.includes('continue DECISION-008')
  const intelScoperScopedWithSpec =
  ...
  const intelScoperClosedWithLedger =
    intelScoper?.lane === 'done' &&
    intelScoper?.priority === 'P0' &&
    String(intelScoper?.statusNote || '').includes(INTEL_SCOPER_CLOSEOUT_KEY) &&
    intelScoperCloseout?.operatorCloseout === true &&
    (intelScoperCloseout.backlogIds || []).includes('INTEL-SCOPER-001') &&
    intelScoperText.includes('scoper outputs') &&
    intelScoperText.includes('proposal-only') &&
    intelScoperText.includes('continue DATA-003')
  const intelThreadContextScoped =
  ...
  const intelThreadContextClosedWithLedger =
    intelThreadContext?.lane === 'done' &&
    ['P0', 'P1'].includes(intelThreadContext?.priority) &&
    String(intelThreadContext?.statusNote || '').includes(INTEL_THREAD_CONTEXT_CLOSEOUT_KEY) &&
    intelThreadContextCloseout?.operatorCloseout === true &&
    (intelThreadContextCloseout.backlogIds || []).includes('INTEL-THREAD-CONTEXT-001') &&
    intelThreadContextText.includes('thread context') &&
    intelThreadContextText.includes('weak-proof') &&
    intelThreadContextText.includes('continue SCOPER-UI-001')
  const strategyQuarterScoped =
  ...
  const strategyQuarterClosedWithLedger =
    strategyQuarter?.lane === 'done' &&
    strategyQuarter?.priority === 'P1' &&
    String(strategyQuarter?.statusNote || '').includes(STRATEGY_QUARTER_CLOSEOUT_KEY) &&
    strategyQuarterCloseout?.operatorCloseout === true &&
    (strategyQuarterCloseout.backlogIds || []).includes('STRATEGY-QUARTER-001') &&
    strategyQuarterText.includes('SRC-STRATEGY-QUARTER-001') &&
    strategyQuarterText.includes('Strategy Hub') &&
    strategyQuarterText.includes('source-backed quarter facts') &&
    strategyQuarterText.includes('continue STRATEGY-003')
  const modelRoutingScoped =
  ...
  const modelRoutingClosedWithLedger =
    modelRouting?.lane === 'done' &&
    modelRouting?.priority === 'P1' &&
    modelRoutingText.includes(MODEL_ROUTING_CLOSEOUT_KEY) &&
    modelRoutingText.includes('docs/rebuild/current-runtime-map.md') &&
    !modelRoutingText.includes('likely in') &&
    modelRoutingText.includes('subscription/system-runtime boundaries') &&
    modelRoutingText.includes('cost/privacy/probe/fallback/stop-control') &&
    modelRoutingCloseout?.operatorCloseout === true &&
    (modelRoutingCloseout.backlogIds || []).includes('MODEL-ROUTING-001')
  ensure(
    checks,
    systemStrategyReview?.lane === 'done' &&
      ...
      (strategicIntelScopedWithSpec || strategicIntelClosedWithLedger) &&
      strategicIntel?.priority === 'P0' &&
      strategicIntelText.includes('intelligence_strategic_issues') &&
      (strategicIntelScopedWithSpec || strategicIntelText.includes(STRATEGIC_INTEL_CLOSEOUT_KEY)) &&
      (strategicIntelScopedWithSpec || strategicIntelText.includes('source-backed strategy items')) &&
      (strategicIntelScopedWithSpec || strategicIntelText.includes('weekly targets are encoded')) &&
      (intelScoperScopedWithSpec || intelScoperClosedWithLedger) &&
      (intelThreadContextScoped || intelThreadContextClosedWithLedger) &&
      (strategyQuarterScoped || strategyQuarterClosedWithLedger) &&
      (modelRoutingScoped || modelRoutingClosedWithLedger) &&
