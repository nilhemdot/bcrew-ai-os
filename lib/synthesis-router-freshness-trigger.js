...
function buildFamilyWatermark(family = {}, runs = [], latestSynthesisSuccess = null) {
  ...
  const latestSuccessfulUpstream = newestRun([archiveSuccess, extractorSuccess].filter(Boolean))
  ...
  const extractorDirty = Boolean(extractorSuccess && (!synthesisAt || isRunAfter(extractorSuccess, latestSynthesisSuccess)))
  ...
  const waitingForExtractor = Boolean(
    archiveDirty &&
    extractorRequired &&
    (!extractorSuccess || isRunAfter(archiveSuccess, extractorSuccess))
  )
  const blockedByExtractor = extractorFailures.length > 0
  ...
  return {
    ...
    failedExtractorJobKeys: extractorFailures.map(run => run.jobKey),
    failedExtractorRuns: extractorFailures.map(run => ({
      runId: run.runId,
      jobKey: run.jobKey,
      status: run.status,
      at: run.atIso,
...
export function buildSynthesisFreshnessMetadataForJobRun(run = {}, snapshot = null) {
  ...
  if (classification.kind === 'source_family') {
    ...
    metadata.marksSynthesisDirty = isSuccess
    metadata.blockedByExtractor = classification.role === 'extractor' && status === 'failed'
  }
...
export async function handleSynthesisFreshnessAfterFoundationJobRun(run = {}, {
  ...
  const snapshotInput = typeof getJobRunSnapshot === 'function'
    ? await getJobRunSnapshot({ limit: 100 })
    : {}
