...
export async function runGeminiEyesForVideo({
  ...
  if (!credential.available) {
    const error = new Error('Gemini API key is missing; set GEMINI_API_KEY or GOOGLE_API_KEY before Eyes proof.')
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        result = await callGeminiApi({
          method: 'POST',
          path: `models/${GEMINI_VIDEO_PRIMARY_MODEL}:generateContent`,
