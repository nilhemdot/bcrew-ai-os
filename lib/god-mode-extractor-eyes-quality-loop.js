...
export const GOD_MODE_EXTRACTOR_EYES_QUALITY_LOOP_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-23-god-mode-extractor-eyes-quality-loop-closeout.md'
...
function geminiRetryDelayMs(error = {}) {
  ...
  if (status === 0 || status === 408 || /fetch failed|network|socket|timed out/i.test(message)) {
    return 30000
  }
...
function buildStandardEyesPrompt({ video, baseline, pageEvidence }) {
...
function formatVideoSegmentForPrompt(segment = null) {
  if (!segment) return 'Full video'
  const label = text(segment.label) || `Segment ${segment.index || ''}`.trim() || 'Segment'
  const start = Number(segment.startSeconds)
  const end = Number(segment.endSeconds)
  const secondsText = Number.isFinite(start) && Number.isFinite(end)
    ? `${start}s to ${end}s`
    : 'bounded clip'
  return `${label} (${secondsText})`
}

function buildLongCourseEyesPrompt({ video, baseline, pageEvidence, videoSegment = null }) {
  const transcriptExcerpt = baseline.transcript?.excerpt || ''
  const descriptionExcerpt = text(pageEvidence?.descriptionText).slice(0, 3000)
  const resourceLinks = list(pageEvidence?.resourceLinks)
    .filter(link => !['public_youtube_internal'].includes(link.classification))
    .slice(0, 18)
    .map(link => `${link.host || 'unknown'} ${link.classification}: ${link.normalizedUrl}`)
    .join('\n')
  return `You are BCrew AI OS Deep Course Extractor Eyes V1.

Analyze this exact public YouTube course or long training for build-intelligence value. Use the video/audio/visual stream, not only transcript text. Treat this like implementation research: map what is taught, what resources are needed, what should be built, what requires approval, and what should be scoped next.

Current video scope: ${formatVideoSegmentForPrompt(videoSegment)}
...
{
  "videoId": "${video.videoId}",
  "summary": "high-signal summary under 500 chars",
  "courseMap": [
    {
      "timestampStart": "MM:SS or HH:MM:SS",
      "timestampEnd": "MM:SS or HH:MM:SS",
      "module": "short module name",
      "whatIsTaught": "plain English lesson under 220 chars",
      "implementationValue": "how BCrew AI OS could use this under 220 chars"
    }
  ],
  "visualEvidence": [
    {
      "timestamp": "MM:SS or HH:MM:SS",
      "visibleTextOrCode": "text/code/UI seen on screen under 220 chars",
      "toolOrSurface": "tool/site/app visible",
      "workflowObservation": "what the creator is doing under 220 chars",
      "buildIntelValue": "why this matters for BCrew AI OS under 220 chars"
    }
  ],
  "workflowMoments": [
    {"timestamp": "MM:SS", "moment": "specific workflow", "transferToAios": "specific transfer"}
  ...
  "resourceNeeds": [
    {"resource": "repo/template/doc/community/site", "whereSeen": "timestamp or description", "approvalNeeded": true, "whyItMatters": "plain English reason"}
  ],
  "buildCandidates": [
    {
      "title": "build candidate title",
      "whyItMatters": "why this could improve AIOS under 280 chars",
      "recommendedNextStep": "specific next implementation/research/scoping step under 280 chars",
      "evidenceTimestamps": ["MM:SS"],
      "confidence": "low|medium|high"
    }
  ],
  "implementationPlan": [
    {"step": "plain English step", "owner": "Extractor|Director|Scoper|Builder|Steve", "dependency": "what must exist first"}
  ],
  "scopingQuestions": ["question the Scoper must answer before build"],
  "missedByTranscriptOnly": ["specific visual/audio/screen detail the transcript-only mode would miss"],
  "riskBoundaries": ["approval/copyright/private/auth/cost boundary"],
  "qualityVerdict": "better_than_baseline|same_as_baseline|worse_than_baseline",
  "confidence": "low|medium|high"
...
- Title: ${baseline.title}
- URL: ${video.url}
- Video scope: ${formatVideoSegmentForPrompt(videoSegment)}
- Baseline score: ${baseline.score}
- Transcript present: ${baseline.transcript?.present ? 'yes' : 'no'}
- Description excerpt: ${descriptionExcerpt || 'none'}
- Resource links observed but not followed:
...
Rules:
- Do not invent links, code, screen details, resources, or timestamps.
- If this request is clipped to a segment, focus only on that segment and make the module names specific to that segment.
- If something is inferred from transcript/description instead of seen on screen, say that explicitly.
- Return ${videoSegment ? '2 to 4' : '5 to 10'} courseMap items, ${videoSegment ? '3 to 5' : '6 to 10'} visualEvidence items, ${videoSegment ? '1 to 3' : '3 to 6'} workflowMoments, ${videoSegment ? '1 to 3' : '2 to 6'} resourceNeeds, ${videoSegment ? 'exactly 2' : '4 to 6'} buildCandidates, and ${videoSegment ? '2 to 4' : '3 to 7'} implementationPlan steps.
- Prioritize practical implementation value over generic summaries.
- Keep every string concise so the JSON completes.
- No private/auth/member/comment extraction is approved. Public YouTube course analysis is allowed; logged-in course/community access is not.`
}

function buildDeepVisualEyesPrompt({ video, baseline, pageEvidence, videoSegment = null }) {
  const transcriptExcerpt = baseline.transcript?.excerpt || ''
  const descriptionExcerpt = text(pageEvidence?.descriptionText).slice(0, 2600)
  const resourceLinks = list(pageEvidence?.resourceLinks)
    .filter(link => !['public_youtube_internal'].includes(link.classification))
    .slice(0, 16)
    .map(link => `${link.host || 'unknown'} ${link.classification}: ${link.normalizedUrl}`)
    .join('\n')
  return `You are BCrew AI OS Deep Visual Review Eyes V1.

Analyze this exact public YouTube video or clipped segment for the important screen-visible implementation details that a normal 1 FPS-ish video watch can miss. Focus hard on code, terminal commands, dashboards, UI layouts, browser actions, forms, settings panels, prompts, diagrams, workflow steps, and any "copy this" or "look at this" screen moment.
...
{
  "videoId": "${video.videoId}",
  "summary": "high-signal summary under 450 chars",
  "visualEvidence": [
    {
      "timestamp": "MM:SS or HH:MM:SS",
      "visibleTextOrCode": "exact text/code/UI/terminal visible on screen under 260 chars",
      "toolOrSurface": "tool/site/app/dashboard/editor visible",
      "workflowObservation": "what the creator is doing under 240 chars",
      "buildIntelValue": "why this matters for BCrew AI OS under 240 chars"
    }
  ],
  "workflowMoments": [
    {"timestamp": "MM:SS", "moment": "specific screen workflow", "transferToAios": "specific transfer"}
  ],
  "resourceNeeds": [
    {"resource": "repo/template/doc/tool/community/site", "whereSeen": "timestamp or description", "approvalNeeded": true, "whyItMatters": "plain English reason"}
  ],
  "buildCandidates": [
    {
      "title": "build candidate title",
      "whyItMatters": "why this could improve AIOS under 260 chars",
      "recommendedNextStep": "specific next implementation/research/scoping step under 260 chars",
      "evidenceTimestamps": ["MM:SS"],
      "confidence": "low|medium|high"
    }
  ],
  ...
  "scopingQuestions": ["question the Scoper must answer before build"],
  "missedByTranscriptOnly": ["specific visual/screen detail the standard/transcript mode would miss"],
  "riskBoundaries": ["approval/copyright/private/auth/cost boundary"],
  "qualityVerdict": "better_than_baseline|same_as_baseline|worse_than_baseline",
  "confidence": "low|medium|high"
}
...
Rules:
- Do not invent code, UI text, links, resources, or timestamps.
- If exact text/code is too small to read, say "unreadable" and describe the visible structure instead of guessing.
- If something is inferred from transcript/description instead of seen on screen, say that explicitly.
- Prioritize exact screen detail over generic summary.
- Return ${videoSegment ? '3 to 5' : '6 to 10'} visualEvidence items, ${videoSegment ? '1 to 3' : '3 to 6'} workflowMoments, ${videoSegment ? '1 to 3' : '2 to 5'} resourceNeeds, ${videoSegment ? 'exactly 2' : '3 to 5'} buildCandidates, and ${videoSegment ? '1 to 3' : '2 to 5'} implementationPlan steps.
- Keep every string concise so the JSON completes.
- No private/auth/member/comment extraction is approved. Public YouTube visual review is allowed; logged-in course/community access is not.`
}

function buildEyesPrompt({ video, baseline, pageEvidence, promptProfile = 'standard', videoSegment = null }) {
  if (promptProfile === 'long_course') return buildLongCourseEyesPrompt({ video, baseline, pageEvidence, videoSegment })
  if (promptProfile === 'deep_visual') return buildDeepVisualEyesPrompt({ video, baseline, pageEvidence, videoSegment })
  return buildStandardEyesPrompt({ video, baseline, pageEvidence })
}
...
function parseJsonObject(raw = '') {
  ...
  try {
    return normalizeParsedJsonShape(JSON.parse(value))
  } catch {}
  const fence = value.match(/```(?:json)?\s*([\s\S]*?)```/i)
  if (fence) {
    try {
      return normalizeParsedJsonShape(JSON.parse(fence[1]))
    } catch {}
  }
  ...
  if (start >= 0 && end > start) {
    try {
      return normalizeParsedJsonShape(JSON.parse(value.slice(start, end + 1)))
    } catch {}
  }
  ...
  if (arrayStart >= 0 && arrayEnd > arrayStart) {
    try {
      return normalizeParsedJsonShape(JSON.parse(value.slice(arrayStart, arrayEnd + 1)))
    } catch {}
  }
  return null
}

function normalizeParsedJsonShape(parsed) {
...
function buildEyesAttemptPrompt({ prompt = '', attempt = 1 } = {}) {
  if (attempt <= 1) return prompt
  return `${prompt}

STRICT JSON RETRY ${attempt}:
- Your previous response was not valid JSON.
- Return one complete JSON object only, not an array and not markdown.
- Escape all line breaks inside string values as \\n.
- Close every quote, object, and array.
- Keep string fields short. If a visible code block is long, summarize it in one short escaped string.
- Do not add commentary before or after the JSON object.`
}

function buildGeminiEyesParseError(outputText = '') {
  const error = new Error(`Gemini Eyes output was not parseable JSON: ${text(outputText).slice(0, 500)}`)
  error.isGeminiEyesJsonParseFailure = true
  return error
}

function retryDelayForGeminiEyesError(error = {}, attempt = 1) {
  if (error?.isGeminiEyesJsonParseFailure) return Math.min(3000, 500 * Math.max(1, attempt))
  return geminiRetryDelayMs(error)
}
...
export async function runGeminiEyesForVideo({
  ...
  const prompt = buildEyesPrompt({ video, baseline, pageEvidence, promptProfile, videoSegment })
  const videoPart = {
    file_data: {
      file_uri: video.url,
    },
  }
  if (videoMetadata && typeof videoMetadata === 'object' && Object.keys(videoMetadata).length > 0) {
    videoPart.file_data.mime_type = 'video/*'
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        result = await callGeminiApi({
          ...
          body: {
            contents: [{
              ...
              parts: [
                videoPart,
                { text: buildEyesAttemptPrompt({ prompt, attempt }) },
              ],
            }],
            generationConfig: {
        ...
        outputText = extractGeminiOutputText(result.json || {})
        parsed = parseJsonObject(outputText)
        if (!parsed) throw buildGeminiEyesParseError(outputText)
        break
      } catch (error) {
        const retryDelayMs = retryDelayForGeminiEyesError(error, attempt)
...
function normalizeEyesOutput(output = {}, video = {}) {
  const courseMap = list(output.courseMap).map((item, index) => ({
    rank: index + 1,
    timestampStart: text(item.timestampStart || item.start || item.timestamp).slice(0, 40),
    timestampEnd: text(item.timestampEnd || item.end).slice(0, 40),
    module: text(item.module || item.title).slice(0, 180),
    whatIsTaught: text(item.whatIsTaught || item.lesson).slice(0, 900),
    implementationValue: text(item.implementationValue || item.transferToAios).slice(0, 900),
  })).filter(item => item.module || item.whatIsTaught || item.implementationValue)
  ...
  const resourceNeeds = list(output.resourceNeeds).map((item, index) => ({
    rank: index + 1,
    resource: text(item.resource).slice(0, 220),
    whereSeen: text(item.whereSeen).slice(0, 220),
    approvalNeeded: item.approvalNeeded !== false,
    whyItMatters: text(item.whyItMatters).slice(0, 900),
  })).filter(item => item.resource || item.whyItMatters)
  const implementationPlan = list(output.implementationPlan).map((item, index) => ({
    rank: index + 1,
    step: text(item.step || item.action).slice(0, 900),
    owner: text(item.owner).slice(0, 120),
    dependency: text(item.dependency).slice(0, 400),
  ...
  return {
    ...
    buildCandidates,
    implementationPlan,
    scopingQuestions: list(output.scopingQuestions).map(item => text(item).slice(0, 400)).filter(Boolean).slice(0, 12),
...
export function buildGodModeEyesDogfoodProof() {
  const arrayWrappedJson = parseJsonObject('[{"videoId":"array-video","summary":"ok"}]')
  const retryPrompt = buildEyesAttemptPrompt({ prompt: 'Return JSON.', attempt: 2 })
  const retryPromptStrict = retryPrompt.includes('STRICT JSON RETRY') &&
    retryPrompt.includes('one complete JSON object only') &&
    retryPrompt.includes('Escape all line breaks inside string values')
  ...
  return {
    ok: transcriptOnly.ok === false &&
      ...
      arrayWrappedJson?.videoId === 'array-video' &&
      retryPromptStrict,
    cases: [
      ...
      { name: 'array_wrapped_json_unwraps', ok: arrayWrappedJson?.videoId === 'array-video' },
      { name: 'malformed_json_retry_prompt_is_strict', ok: retryPromptStrict },
    ],
  }
}
