...
export const VERIFIER_AGENT_FEEDBACK_ORCHESTRATION_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-agent-feedback-orchestration-split-closeout.md'
...
function getSourceHealthReason(value = {}) {
  return value?.sourceHealth?.reason ||
    value?.autoSend?.sourceHealth?.reason ||
    value?.report?.sourceHealth?.reason ||
    ''
}

function hasSourceUnavailable(value = {}) {
  return value?.sourceHealth?.status === 'degraded' ||
    value?.summary?.sourceUnavailable === true ||
    value?.autoSend?.sourceHealth?.status === 'degraded' ||
    value?.autoSend?.summary?.sourceUnavailable === true ||
    value?.report?.sourceHealth?.status === 'degraded'
}

function hasNoDiagnosticExternalSideEffects(value = {}) {
  const sideEffects = value?.report?.sideEffects || value?.autoSend?.report?.sideEffects || {}
  return sideEffects.gmailSent !== true &&
    sideEffects.clickUpRequestedWritten !== true &&
    sideEffects.clickUpReminderWritten !== true
}

function isAcceptedAgentFeedbackSourceOutage({
  foundationHub = {},
  opsHub = {},
  statuses = [],
} = {}) {
  const boundaries = [foundationHub.sourceOutageBoundary, opsHub.sourceOutageBoundary].filter(Boolean)
  const boundaryAccepted = boundaries.some(boundary =>
    boundary.status === 'degraded' &&
    (boundary.summary?.externalOutageBlocksCoreApi === false || boundary.summary?.opsApiFailSoft === true)
  )
  const degradedStatuses = statuses.filter(hasSourceUnavailable)
  const acceptedReasons = new Set(['external_api_error', 'runtime_diagnostic_timeout'])
  const reasons = degradedStatuses.map(getSourceHealthReason).filter(Boolean)
  return boundaryAccepted &&
    degradedStatuses.length > 0 &&
    degradedStatuses.every(hasNoDiagnosticExternalSideEffects) &&
    (reasons.length === 0 || reasons.every(reason => acceptedReasons.has(reason)))
}
...
export function evaluateFoundationAgentFeedbackVerifier(input = {}) {
  ...
  const {
    ...
    agentFeedbackSourceOutageAccepted = false,
  ...
  const agentFeedbackAutoSendBuildLogExact = buildLogAgentFeedbackAutoSendBuild?.backlogIds?.length === 1 &&
    ...
    agentFeedbackAutoSend?.lane === 'done' &&
      ...
      (agentFeedbackAutoSendStatus.status === 'healthy' || agentFeedbackProductionVerifierAccepted || agentFeedbackSourceOutageAccepted) &&
      agentFeedbackAutoSendHasGovernedAction &&
      (agentFeedbackAutoSendStatus.summary?.georgiaDay30RecipientSource === 'company_email' ||
        agentFeedbackSourceOutageAccepted ||
        agentFeedbackProductionVerifierAccepted) &&
  ...
  const agentFeedbackLiveRemindersBuildLogExact =
    ...
    agentFeedbackLiveReminders?.lane === 'done' &&
      ...
      (agentFeedbackReminderStatus.status === 'healthy' || agentFeedbackSourceOutageAccepted) &&
...
export async function evaluateFoundationAgentFeedbackVerifierOrchestration(input = {}) {
  ...
  const agentFeedbackProductionVerifierAccepted = agentFeedbackProductionAutoSendEnable?.lane === 'done' &&
    ...
    agentFeedbackProductionAutoSendEnableStatus.summary?.productionAutoSendEnabled === true &&
    agentFeedbackProductionAutoSendEnableStatus.summary?.liveGuardDecision === 'live_send_allowed'
  const agentFeedbackSourceOutageAccepted = isAcceptedAgentFeedbackSourceOutage({
    foundationHub,
    opsHub,
    statuses: [
      agentFeedbackAutoSendStatus,
