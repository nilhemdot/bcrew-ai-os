...
function buildSourceMap({ files = [], commit = '', repoAvailable = false } = {}) {
  ...
  return {
    ...
    latestMonitoringBoundary: snapshotBaseline.latestMonitoringBoundary,
    latestMonitoringCardId: snapshotBaseline.latestMonitoringCardId,
    snapshotBaseline,
    repoAvailable,
...
Snapshot posture: inspected snapshot evidence, not latest upstream monitoring truth. Latest monitoring remains governed by \`${snapshot.latestMonitoringCardId || 'BUILD-INTEL-GITHUB-MONITOR-001'}\`.
...
- Source posture: ${sourceMap.sourcePosture || snapshot.sourcePosture || 'inspected_snapshot'}
- Latest monitoring boundary: ${sourceMap.latestMonitoringBoundary || snapshot.latestMonitoringBoundary || 'manual_public_github_build_intel_review'}
