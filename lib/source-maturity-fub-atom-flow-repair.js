...
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_CARD_ID = 'SOURCE-MATURITY-FUB-ATOM-FLOW-REPAIR-001'
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_CLOSEOUT_KEY = 'source-maturity-fub-atom-flow-repair-v1'
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_PLAN_PATH = 'docs/process/source-maturity-fub-atom-flow-repair-001-plan.md'
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_APPROVAL_PATH = 'docs/process/approvals/SOURCE-MATURITY-FUB-ATOM-FLOW-REPAIR-001.json'
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-29-hot-doc-refresh/2026-05-18-source-maturity-fub-atom-flow-repair-closeout.md'
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_SCRIPT_PATH = 'scripts/process-source-maturity-fub-atom-flow-repair-check.mjs'
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_SPRINT_ID = 'source-maturity-fub-atom-flow-repair-2026-05-18'
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID = 'SRC-FUB-001'
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_FACT_ID = 'fact:af613c177546ec2a7267b669'

export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES = [
...
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_PROOF_COMMANDS = [
...
export const SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_CHANGED_FILES = [
...
function targetFact(facts = []) {
  return list(facts).find(fact => (fact.factId || fact.fact_id) === SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_FACT_ID) || null
}

export function selectFubAtomFlowRepairCandidate({
  beforeRow = {},
  sourceFacts = [],
  sourceId = SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
} = {}) {
  const failures = []
  ...
  const factId = fact?.factId || fact?.fact_id || SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_FACT_ID
  ...
  return {
    ...
    evidenceRefs: Array.from(new Set([
      sourceRef,
      `intelligence_synthesis_facts:${factId}`,
      ...evidenceRefs,
...
export function buildFubAtomFromFact(candidate = {}, now = new Date().toISOString()) {
  const sourceId = candidate.sourceId || SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  ...
  const naturalKey = `source-maturity-fub-atom-flow-repair:${sourceId}:${factId}`
  ...
  const atom = {
    atomId,
    title: `${sourceId} FUB CRM source truth`,
    ...
    evidenceExcerpt,
    derivedClaim: claim,
    topicRefs: ['source-maturity', 'fub', 'crm-source-truth', 'lead-source-attribution'],
    ...
    acceptedAt: now,
    acceptedBy: 'codex-source-maturity-fub-atom-flow-repair',
    reviewNote: 'Promoted from an existing governed FUB source fact only; no live extraction, FUB API call, CRM mutation, provider call, or external write occurred.',
    tags: ['source-maturity', 'atom-flow-repair', 'fub', 'no-live-extraction'],
    metadata: {
...
export function evaluateFubAtomFlowRepair({
  beforeRow = {},
  afterRow = {},
  candidate = {},
  atomBuild = {},
  ...
  add(savedAtom ? savedAtom.metadata?.noFubApiCall === true || atomBuild.atom?.metadata?.noFubApiCall === true : false, 'atom records no FUB API call', JSON.stringify(savedAtom?.metadata || atomBuild.atom?.metadata || {}))
  add(savedAtom ? savedAtom.metadata?.noCrmMutation === true || atomBuild.atom?.metadata?.noCrmMutation === true : false, 'atom records no CRM mutation', JSON.stringify(savedAtom?.metadata || atomBuild.atom?.metadata || {}))
  ...
  return {
    ...
    failures,
    summary: {
      sourceId: candidate.sourceId || beforeRow.sourceId || afterRow.sourceId || SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
...
export function buildSyntheticFubAtomFlowRepairProof() {
  const fact = {
    factId: SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_FACT_ID,
    sourceId: SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
    ...
    detail: 'Read access is revalidated in the rebuild with both owner/support and Steve account API contexts. This atom-flow repair uses the existing source fact only and does not call FUB live.',
    metadata: { status: 'verified_readable', evidenceRefs: ['intelligence_synthesis_facts:fact:af613c177546ec2a7267b669'] },
    minTier: 1,
    sensitivity: 'neutral',
    status: 'active',
  }
  ...
  const candidate = selectFubAtomFlowRepairCandidate({ beforeRow, sourceFacts: [fact] })
  const atomBuild = buildFubAtomFromFact(candidate, '2026-05-18T06:15:00.000Z')
  const evaluation = evaluateFubAtomFlowRepair({
    beforeRow,
    afterRow,
    candidate,
    atomBuild,
  ...
  const missingFact = selectFubAtomFlowRepairCandidate({ beforeRow, sourceFacts: [] })
  const tierBlocked = selectFubAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, minTier: 4 }] })
  return {
    ok: candidate.ok &&
      atomBuild.ok &&
      evaluation.ok &&
      ...
      atomBuild.atom.metadata.noFubApiCall === true &&
      atomBuild.atom.metadata.noCrmMutation === true &&
...
export function renderFubAtomFlowRepairCloseout(snapshot = {}) {
  const summary = snapshot.summary || {}
  const lines = []
  lines.push('# Source Maturity FUB Atom Flow Repair Closeout')
  lines.push('')
  lines.push(`Card: \`${SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_CARD_ID}\``)
  lines.push(`Closeout key: \`${SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\``)
  lines.push('')
  lines.push('## What Shipped')
  lines.push('')
  lines.push(`- Promoted one source-backed atom for \`${summary.sourceId || SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID}\` from an existing FUB source fact.`)
  lines.push(`- Saved atom: \`${summary.atomId || 'pending atom'}\`.`)
  lines.push(`- Source fact evidence: \`${summary.sourceFactId || SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_TARGET_FACT_ID}\`.`)
  lines.push('- The repair uses internal source-fact evidence only and does not run extraction, call the FUB API, mutate CRM data, call providers, or write externally.')
  ...
  lines.push('- Building-now focused proof moved `SRC-FUB-001` from `atomized` to `routed`; close-card proof is idempotent and may show `routed` before and after.')
  ...
  lines.push('')
  for (const boundary of SOURCE_MATURITY_FUB_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES) lines.push(`- ${boundary}`)
  lines.push('')
  lines.push('## Next')
  lines.push('')
