...
function normalizeMarkApiFullWatchReport(report = null) {
  if (!report) return null
  const structured = report.structuredOutputJson || report.structured_output_json || {}
  const snapshot = structured.snapshot || {}
  return {
    ...
    buildCandidates: list(structured.buildCandidates || snapshot.topBuildCandidates).map((candidate, index) => ({
    ...
    videoResults: list(snapshot.videoResults || structured.videoResults).map(item => ({
      video: item.video || {},
      baseline: item.baseline || {},
      eyes: item.eyes || {},
      pageEvidence: item.pageEvidence || {},
...
function buildCounts({
  ...
  return {
    ...
    apiFullWatchVideos: apiVideos.length,
    apiFullWatchBuildCandidates: list(markApiFullWatch?.buildCandidates).length,
    apiFullWatchTimestampedVisualEvidence: apiVideos
...
export function buildDevTeamHubV0Snapshot({
  ...
  const markApiFullWatch = normalizeMarkApiFullWatchReport(markApiFullWatchBundle.report)
