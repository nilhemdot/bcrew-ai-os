...
function buildCreatorSourceStacks({
  ...
  return list(creatorLeaderboard).map(creator => {
...
export function buildDevTeamHubV0Snapshot({
  ...
  const devOpportunityVisionLens = buildDevOpportunityVisionLensReview({
    generatedAt,
    rankedCandidates: directorReport?.rankedCandidates || [],
    sourceRows: youtubeSourceIntelligence?.sourceGodModeHandoffQueue?.rows || [],
