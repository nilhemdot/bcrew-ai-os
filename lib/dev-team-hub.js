...
function didBatchPersistAfterJobStart(latestBatch = null, job = {}) {
  const hasPersistedBatchEvidence = Number(latestBatch?.videoCount || 0) > 0 ||
    Number(latestBatch?.buildCandidateCount || 0) > 0 ||
    list(latestBatch?.inputArtifactIds).length > 0
  if (!latestBatch?.updatedAt || !job?.latestRunStartedAt || !hasPersistedBatchEvidence) return false
  const batchUpdatedAt = Date.parse(latestBatch.updatedAt)
  const runStartedAt = Date.parse(job.latestRunStartedAt)
  if (!Number.isFinite(batchUpdatedAt) || !Number.isFinite(runStartedAt)) return false
  return batchUpdatedAt >= runStartedAt
}
...
function collectYoutubeReportHandoffLinks(report = {}) {
  ...
  const addRef = (ref, disposition = '', sourceVideoId = '') => {
  ...
  for (const result of youtubeReportVideoResults(report)) {
    const sourceVideoId = text(result.video?.videoId || result.videoId)
    const packet = result.resourceLinkSnapshot?.scoperPacket || result.resourceLinkPacket || {}
    for (const ref of list(packet.resolvedResourceRefs)) addRef(ref, 'resolved_public_resource', sourceVideoId)
    for (const ref of list(packet.unresolvedPublicResourceLinks)) addRef(ref, 'unresolved_public_resource', sourceVideoId)
    for (const ref of list(packet.approvalRequiredResourceLinks)) addRef(ref, 'approval_required_resource', sourceVideoId)
    for (const ref of list(packet.blockedResourceRefs)) addRef(ref, 'blocked_resource', sourceVideoId)
    for (const ref of list(packet.resourceLinkDispositions)) addRef(ref, 'resource_disposition', sourceVideoId)
    for (const ref of list(result.safeResourceFollowResults)) addRef(ref, 'safe_resource_follow_result', sourceVideoId)
  }
...
function addYoutubeHandoffEvidence(bucketMaps, bucketId, link = {}) {
  ...
  if (bucketMaps[bucketId].has(key)) return
  bucketMaps[bucketId].set(key, {
    url: normalizedUrl,
    host,
...
function routeYoutubeHandoffLink(bucketMaps, link = {}) {
  ...
  if (classification.approvalRequired === true || isPaidAuthGateUrl(normalizedUrl)) {
...
function buildYoutubeHandoffEvidenceFromReports(reports = []) {
  const bucketMaps = {
...
function buildYoutubeSourceIntelligenceSnapshot({
  ...
  const latestBatchPersistedAfterRun = didBatchPersistAfterJobStart(latestBatch, watchJob)
  const watchRunNeedsLedgerRepair = watchJob.latestRunStatus === 'failed' && latestBatchPersistedAfterRun
  const watchStageStatus = watchRunNeedsLedgerRepair
    ? 'post_run_review_needed'
    : watchJob.latestRunStatus === 'failed'
      ? 'failed'
      : latestBatch?.status || watchJob.status
  const catchupSummary = catchupSnapshot?.summary || {}
  const approvalSummary = approvalReviewTriage?.summary || {}
  const topCreators = (list(sourceValueGrader?.topDevBuildSources).length
    ? list(sourceValueGrader?.topDevBuildSources)
    : list(sourceValueGrader?.sourceGrades))
    .slice(0, 12)
    .map(row => ({
      creatorId: row.creatorId || '',
      creator: row.creator || row.displayName || row.creatorId || '',
      grade: row.devBuildGrade || row.grade || row.overallGrade || 'ungraded',
      buildCandidates: Number(row.buildCandidates || 0),
      watchedVideos: Number(row.watchedVideos || 0),
      bestDirectorRank: Number(row.bestDirectorRank || 0),
  ...
  const handoffBuckets = [
    ...
    buildHandoffBucket({
      ...
      status: freeCommunityCount ? 'ready_for_free_community_lane' : 'none_found',
      description: 'Free Skool/community links should hand off to the community SOP: free areas only, last 20 days, resources/courses, no posting or paid/private access.',
      route: 'skool-free-community-runner',
  ...
  const stageCards = [
    ...
    },
    {
      ...
      summary: `${Number(catchupSummary.videoAudioVisualWatchedCount || 0)} watched · ${Number(catchupSummary.pendingStandardVideoCount || 0)} pending`,
    ...
    },
    {
      stageId: 'handoff',
      label: 'Handoff',
      status: handoffBuckets.some(bucket => bucket.count > 0 && bucket.status !== 'empty_or_auto_filtered') ? 'waiting' : 'clear',
      title: 'Downstream source handoffs',
      summary: `${list(approvalReviewQueue).length} review rows · ${Number(sourcePacketWorkerQueue?.counts?.ready || 0)} public workers ready · ${Number(sourcePacketHandsQueue?.counts?.ready || 0)} Hands ready`,
      latestRunAt: latestBatch?.updatedAt || null,
      detail: 'Newsletter, Skool/community, paid/auth, public repo/resource, and Scoper routes are separated here.',
    },
    {
      stageId: 'grade',
      label: 'Grade',
      ...
      summary: `${list(sourceValueGrader?.sourceGrades).length} graded creators · ${Number(catchupSummary.ungradedCount || 0)} ungraded`,
  ...
  const executiveSummary = {
    ...
    currentState: [
      'Working now: public YouTube discovery, exact-video full-watch batches, page/description/resource evidence, safe public resource metadata, creator grading, and Director synthesis.',
      'Not yet automatic: YouTube long-course/full-training extraction, downstream newsletter signup, free-community crawling, repo deep review, paid/auth course extraction, and Director-to-Scoper promotion.',
    ],
    workingSteps: [
      {
        label: '1. Discover',
      ...
      },
      {
        ...
        detail: `Safe public links are resolved as read-only metadata; paid/auth/community/login links are parked with source-packet reasons. Latest batch found ${Number(latestBatch?.resolvedPublicResourceLinkCount || 0)} public resources and ${Number(latestBatch?.approvalRequiredLinkCount || 0)} approval-required links.`,
      },
      {
      ...
      },
      {
        ...
        detail: `${Number(catchupSummary.longCoursePendingCount || 0)} public long-course/full-training videos are routed out of the standard short-video lane. They need the bounded long-course extractor so multi-hour trainings produce course maps, implementation plans, resources, and build candidates.`,
      },
    ],
    openGaps: [
      ...
      'YouTube-discovered links do not yet trigger newsletter signup, Skool/free-community extraction, GitHub repo review, or paid-course extraction automatically.',
      'Handoff bucket counts need to read from the full evidence spine, not only the current approval queue, so public/free discoveries are not hidden.',
    ...
    nextBuilds: [
      ...
      'Repair the handoff read model so discovered public resources, repos, newsletters, and free communities show as captured/ready instead of disappearing from the queue.',
      'Build the downstream workers: public resource/repo reader, newsletter intake, free-community SOP runner, and paid/auth source-session runner.',
...
export function buildDevTeamHubV0Snapshot({
  ...
  const youtubeSourceIntelligence = buildYoutubeSourceIntelligenceSnapshot({
    foundationSnapshot,
    dailyWatch,
    catchupSnapshot: youtubeCreatorGodModeCatchup,
