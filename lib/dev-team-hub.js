...
function buildExtractionEconomics({
  ...
  for (const call of calls) {
    ...
    const cost = estimateGeminiStandardTokenCostUsd({ model, inputTokens: input, outputTokens: output })
...
function buildCounts({
  ...
  const acceptedApiWatchedCount = Number(markCatchupRow?.videoAudioVisualWatchedCount || 0)
...
export function buildDevTeamHubV0Snapshot({
  ...
  const youtubeFullWatchReports = uniqueBy([
