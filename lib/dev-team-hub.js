...
function compactDirectorPayload(directorReport = null, rankedLimit = 120) {
  if (!directorReport) {
    return {
      report: null,
      status: 'Needs source',
...
function compactPriorityLensRouter(router = {}, opportunityLimit = 8) {
  return {
    ...
    lenses: list(router.lenses).map(lens => {
      ...
      return {
        ...lens,
        opportunities: opportunities.slice(0, opportunityLimit).map(compactVisionOpportunity),
...
function compactDevOpportunityVisionLensReview(review = {}) {
  ...
  return {
    ...
    priorityLensRouter: compactPriorityLensRouter(review.priorityLensRouter || {}),
  }
}
...
function buildSourceBrowserRunSummary(runItems = []) {
  ...
  const topRuns = rows
    .filter(row => row.metadata.ok === true || text(row.item.status) === 'succeeded')
    .map(row => {
      const metadata = row.metadata
      ...
      return {
        ...
        usefulSignals: list(metadata.usefulSignals).slice(0, 3).map(signal => text(signal.text || signal.title || signal)).filter(Boolean),
      }
    })
    .sort((left, right) =>
...
function buildCreatorSourceStacks({
  ...
  return [...orderedCreatorIds].map(creatorId => creatorsById.get(creatorId)).filter(Boolean).map(creator => {
    ...
    return {
      ...
      parkedSurfaceCount,
      surfaces,
      plainEnglish: `${creator.creator || creator.creatorId} has ${discoveredSurfaceCount} discovered source surface(s). YouTube is proven separately; public pages/repos/newsletters/free communities/paid gates are read from watched-video handoff evidence.`,
    }
  })
}
...
function buildYoutubeSourceIntelligenceSnapshot({
  ...
  sourceGodModeHandoffQueue.sourceRunSummary = buildSourceBrowserRunSummary(sourceGodModeHandoffRunItems)
  ...
  const executiveSummary = {
    ...
    currentState: [
      ...
      `Current blocker: ${buildPromotionStatus}. ${readySourceHandoffRows} runnable source handoff rows remain; ${parkedSourceHandoffRows} rows are parked behind source-specific lanes, auth/session boundaries, social/profile policy, or approval.`,
    ],
    workingSteps: [
      {
        label: '1. Discover',
