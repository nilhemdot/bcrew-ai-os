...
function buildYoutubeSourceIntelligenceSnapshot({
  ...
  const rawPendingStandardVideoCount = Number(catchupSummary.pendingStandardVideoCount || 0)
  ...
  const stageCards = [
    ...
    },
    {
      ...
      summary: autopilotSelectedVideoCount
        ? `${Number(catchupSummary.videoAudioVisualWatchedCount || 0)} watched · ${autopilotSelectedVideoCount} eligible now · ${rawPendingStandardVideoCount} raw pending`
        : `${Number(catchupSummary.videoAudioVisualWatchedCount || 0)} watched · 0 eligible standard videos · ${rawPendingStandardVideoCount} raw pending`,
    ...
    },
    {
      ...
      summary: `${readySourceHandoffRows} public/free rows ready · ${parkedSourceHandoffRows} parked · ${list(approvalReviewQueue).length} legacy review rows`,
      latestRunAt: latestBatch?.updatedAt || null,
      detail: 'Public resources, repos, newsletters, and free communities now route to bounded source-browser workers; paid/auth/product and Scoper paths stay separated.',
    },
    {
      stageId: 'grade',
      label: 'Grade',
  ...
  const executiveSummary = {
    ...
    currentState: [
      ...
      'Not yet automatic: scheduled downstream newsletter signup, scheduled free-community crawling, repo deep review ingestion, paid/auth course extraction, YouTube long-course/full-training extraction, and Director-to-Scoper promotion.',
    ],
    workingSteps: [
      {
        label: '1. Discover',
    ...
    openGaps: [
      ...
      'YouTube-discovered public/free links now have a source-browser handoff queue and proof, but production scheduling, persistence into source stacks, and downstream ingestion still need hardening.',
  ...
  return {
    ...
    summary: {
      ...
      filteredStandardCandidateCount: autopilotRejectedVideoCount,
