...
function buildCounts({ dailyWatch = {}, scoutReport = null, atoms = [], hits = [], approvalLinks = [], reviewRoutes = [], opportunities = [] } = {}) {
  const researchPool = normalizeResearchPool(dailyWatch.researchPool)
  return {
...
export function buildDevTeamHubV0Snapshot({
  ...
  const approvalRequiredLinks = normalizeApprovalLinks(scoutReport)
