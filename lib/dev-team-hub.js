...
function buildSynthesisLane({ synthesisJobs = [], upstreamJobs = [], directorReport = null } = {}) {
  ...
  const stale = Boolean(latestSynthesisRunAt && latestUpstreamRunAt && latestUpstreamRunAt > latestSynthesisRunAt)
  const status = upstreamFailures.length || stale ? 'risk' : groupStatus(synthesisJobs)
  const summary = upstreamFailures.length
  ...
  const detail = upstreamFailures.length
    ? `Blocked by ${jobTitleList(upstreamFailures)}.`
    : stale
  ...
  return {
    ...
    label: stale || upstreamFailures.length ? 'Needs refresh' : 'Synthesis',
    ...
    freshness: {
      ...
      latestUpstreamRunAt,
      stale,
      failedUpstreamJobKeys: upstreamFailures.map(job => job.key),
    },
    jobs: synthesisJobs,
  }
}
...
function buildExtractionLanes({ foundationSnapshot = {}, dailyWatch = {}, counts = {}, markYoutube = {}, scoutReport = null, eyesQualityLoop = null, latestApiFullWatch = null, extractionEconomics = null, directorReport = null } = {}) {
  ...
  const synthesisLane = buildSynthesisLane({ synthesisJobs, upstreamJobs, directorReport })
  return [
    {
