...
const PUBLIC_WEB_FORM_OR_AUTH_RE = /(?:^|[/?#&=._-])(login|signin|sign-in|signup|sign-up|join|checkout|payment|billing|subscription|purchase|buy|cart|booking|book-a-call|calendar|calendly|work-with-me|apply|application|download|upload|account|settings|profile|password|mfa|2fa|form|forms|fan_mail)(?:[/?#&=._-]|$)/i
...
function sourceSessionPrepPhase(row = {}) {
  if (row.bucketId === 'creator-newsletters') {
    return {
      ...
      status: 'blocked_until_newsletter_identity_worker',
      plainEnglish: 'Newsletter page evidence is read; signup still needs the source-identity newsletter lane before any form submit.',
...
function buildSourceSessionPrepClusters(rows = [], limit = 24) {
  const byKey = new Map()
  for (const row of list(rows)) {
...
export function buildSourceSessionPrepQueue({ rows = [] } = {}) {
  ...
  const previewClusters = buildSourceSessionPrepClusters(prepRows, 24)
  counts.clusterCount = new Set(prepRows.map(sourceSessionClusterKey).filter(Boolean)).size
...
function buildExistingRunLookup(runItems = []) {
  ...
  const byUrl = new Map()
  for (const item of list(runItems)) {
    const metadata = metadataOf(item)
    const rowId = text(metadata.rowId)
    const url = text(metadata.url || metadata.exactUrl)
    if (rowId && !byRowId.has(rowId)) byRowId.set(rowId, item)
    if (url && !byUrl.has(url)) byUrl.set(url, item)
  }
  return { byRowId, byUrl }
}

function attachExistingRun(row = {}, existingRunLookup = {}) {
  const existingRun = existingRunLookup.byRowId?.get(row.rowId) || existingRunLookup.byUrl?.get(row.url) || null
...
function queueUrlBoundary(row = {}) {
  ...
  if (row.bucketId === 'public-web-resources') {
    ...
    if (shallowPath && AFFILIATE_OR_TRACKING_PARAM_RE.test(row.url)) {
      return {
        runnable: false,
        parked: true,
  ...
  if (row.bucketId === 'free-communities' && !isSkoolOrLocalFixtureHost(host)) {
    return {
      ...
      reason: 'The free-community runner is Skool-specific. Non-Skool community/signup bridge pages need a source-specific community runner or public-web read first.',
    }
  }
...
export function buildSourceGodModeYoutubeHandoffQueue({
  ...
  for (const [bucketId, config] of Object.entries(BUCKET_CONFIG)) {
    ...
    const allBucketRows = rowsForBucket(bucket)
    ...
    bucketCounts[bucketId] = {
      ...
      hasMore: number(bucket.count, allBucketRows.length) > sourceRows.length,
