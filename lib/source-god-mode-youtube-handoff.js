...
function buildBrowserChallengeFallbackReview({ rows = [] } = {}) {
  ...
  const reviewRows = [...challengeRows]
    .sort((left, right) =>
      number(left.devLanePriority?.priorityRank, 99) - number(right.devLanePriority?.priorityRank, 99) ||
    ...
    .map(row => {
      ...
      return {
        ...
        nextAction: blocker.nextAction || 'retry with a clean source-browser session or route to the source-specific browser fallback',
