...
const BUCKET_CONFIG = {
  ...
  'creator-newsletters': {
    ...
    status: 'ready_for_newsletter_intake_dry_run',
    runnable: true,
    maxPages: 3,
    maxDepth: 1,
...
function sourceSessionPrepPhase(row = {}) {
  if (row.bucketId === 'creator-newsletters') {
    const newsletterPacket = row.existingRunEvidence?.newsletterIntakePacket || {}
    const packetStatus = text(newsletterPacket.status || row.existingRunEvidence?.runtimeStatus || row.existingRunRuntimeStatus)
    if (row.status === 'already_run_source_evidence_saved' && packetStatus === 'newsletter_intake_ready_dry_run') {
      return {
        phase: 'newsletter_live_signup_approval_needed',
        status: 'dry_run_packet_ready_waiting_for_live_signup_approval',
        plainEnglish: 'Newsletter dry-run packet is ready. Live external signup still needs Steve approval, explicit live-submit flags, Gmail current sync, and confirmation readback before subscribed status or issue extraction is claimed.',
        account: newsletterPacket.account || SOURCE_SESSION_BROKER_DEFAULT_FREE_ACCOUNT,
        runner: 'newsletter:intake',
        runAfterSessionCommand: `npm run newsletter:intake -- --url=${row.url} --account=${newsletterPacket.account || SOURCE_SESSION_BROKER_DEFAULT_FREE_ACCOUNT} --apply --allow-external-signup --standing-policy-approved --confirmation-readback-required --json`,
      }
    }
    if (row.status === 'previous_source_run_failed_needs_review' && packetStatus) {
      return {
        phase: 'newsletter_intake_review_needed',
        status: `blocked_${safeKey(packetStatus)}`,
        plainEnglish: 'Newsletter intake already ran but did not produce a ready dry-run signup packet. Review the saved blocker/not-newsletter packet instead of rerunning it as fresh or approving live signup.',
        account: newsletterPacket.account || SOURCE_SESSION_BROKER_DEFAULT_FREE_ACCOUNT,
        runner: 'newsletter:intake',
    ...
    return {
      ...
      plainEnglish: 'Newsletter page evidence is read; the dry-run newsletter intake worker can verify the signup form and source-inbox packet. After any approved live submit, confirmation readback must prove mailbox evidence before subscribed status or recurring issue extraction is claimed.',
...
function sourceSessionPrepRank(row = {}) {
  const phaseRank = {
    ...
    newsletter_live_signup_approval_needed: 2,
    newsletter_intake_review_needed: 3,
    newsletter_signup_lane_needed: 4,
    community_start_url_needed: 5,
    community_runner_needed: 6,
    paid_or_auth_packet_needed: 7,
...
function sourceSessionActionGroupLabel(phase = '') {
  return {
    ...
    newsletter_live_signup_approval_needed: 'Newsletter live signup approval',
    newsletter_intake_review_needed: 'Newsletter intake review',
...
function sourceSessionActionGroupNextAction(phase = '') {
  return {
    ...
    newsletter_live_signup_approval_needed: 'Review the ready dry-run newsletter packets. Run one exact live signup only after Steve approves the explicit submit flags, then require Gmail confirmation readback.',
    newsletter_intake_review_needed: 'Review saved newsletter blockers/not-newsletter packets. Do not approve live signup or rerun as fresh until the row is corrected or parked.',
    newsletter_signup_lane_needed: 'Run newsletter intake dry-run checks first. Any approved live signup must be followed by Gmail archive confirmation readback before subscribed status or issue extraction is claimed.',
...
export function buildSourceSessionPrepQueue({ rows = [] } = {}) {
  ...
  const counts = {
    ...
    newsletterLiveSignupApprovalRows: countWhere(row => row.phase === 'newsletter_live_signup_approval_needed'),
    newsletterIntakeReviewRows: countWhere(row => row.phase === 'newsletter_intake_review_needed'),
    newsletterDryRunRows: countWhere(row => row.phase === 'newsletter_signup_lane_needed'),
...
function existingRunBrowserChallenge(metadata = {}) {
  for (const blocker of list(metadata.blockers)) {
    ...
    if (/browser_challenge_not_source_content|cloudflare|checking your browser|just a moment|security verification/i.test(surface)) {
      return {
        ...
        recovery: text(blocker.nextAction || 'retry clean source-browser session, then prepare Harlan/operator escalation or source-specific session if the challenge remains'),
      }
    }
  }
  ...
  if (text(fallbackPlan.trigger) === 'browser_challenge_not_source_content' || /browser_challenge/i.test(text(fallbackPlan.status))) {
    return {
      ...
      recovery: text(fallbackPlan.nextAction || 'retry clean source-browser session, then prepare Harlan/operator escalation or source-specific session if the challenge remains'),
    }
  }
...
function existingRunEvidenceReadback(existingRun = {}, metadata = {}) {
  ...
  const repoReviewPacket = metadata.repoReviewPacket || null
  ...
  return {
    ...
    newsletterIntakePacket: metadata.newsletterIntakePacket ? {
      packetId: text(metadata.newsletterIntakePacket.packetId),
      status: text(metadata.newsletterIntakePacket.status),
      sourceUrl: text(metadata.newsletterIntakePacket.sourceUrl),
      account: text(metadata.newsletterIntakePacket.account),
      inboxLabel: text(metadata.newsletterIntakePacket.inboxLabel),
      subscribedStatus: text(metadata.newsletterIntakePacket.subscribedStatus),
      requiresConfirmationReadback: metadata.newsletterIntakePacket.requiresConfirmationReadback === true,
      nextAction: text(metadata.newsletterIntakePacket.nextAction),
    } : null,
    valueScore: metadata.valueScore || null,
    sopCompletion: metadata.sopCompletion || null,
    repoReviewPacket: repoReviewPacket ? {
      packetId: text(repoReviewPacket.packetId),
      status: text(repoReviewPacket.status),
      repoKey: text(repoReviewPacket.repoKey),
      rootUrl: text(repoReviewPacket.rootUrl),
      pagesRead: number(repoReviewPacket.pagesRead, 0),
      implementationPatternCount: number(repoReviewPacket.implementationPatternCount, implementationPatterns.length),
      sourceCoverage: repoReviewPacket.sourceCoverage || {},
      blockedLinkCount: number(repoReviewPacket.blockedLinkCount, 0),
    } : null,
    implementationPatterns: implementationPatterns.map(pattern => ({
      patternId: text(pattern.patternId),
      title: text(pattern.title),
      sourceUrl: text(pattern.sourceUrl),
      aiosUse: text(pattern.aiosUse),
    })).filter(pattern => pattern.title).slice(0, 8),
    sourceStackUpdate,
...
function existingRunEvidencePlainEnglish(row = {}, existingRunEvidence = {}) {
  if (row.bucketId === 'public-code-repos') {
    const packet = existingRunEvidence.repoReviewPacket || {}
    const patternCount = number(packet.implementationPatternCount, list(existingRunEvidence.implementationPatterns).length)
    const pagesRead = number(existingRunEvidence.pagesRead || packet.pagesRead, 0)
    const runner = existingRunEvidence.runner || row.runner || 'repo:deep-review'
    if (patternCount || pagesRead) {
      return `${row.label} already has ${runner} evidence saved for this exact URL: ${pagesRead} page(s) read and ${patternCount} implementation pattern(s) captured.`
    }
  }
  return `${row.label} already has source-browser evidence saved for this exact URL.`
}
...
function attachExistingRun(row = {}, existingRunLookup = {}) {
  ...
  const newsletterIntakeUpgradeNeeded = existingStatus === 'succeeded' &&
    row.bucketId === 'creator-newsletters' &&
    row.runner === 'newsletter:intake' &&
    !unsafeSideEffectDetected &&
    (
      existingRunner !== row.runner ||
      !metadata.newsletterIntakePacket
    )
  const operatorEscalationRequired = existingStatus === 'failed' &&
    browserChallenge &&
    text(metadata.sourceBrowserAgentExecutionVersion) &&
    (
      text(metadata.sourceBrowserAgentPlan?.terminalState) === 'failed_closed' ||
      text(metadata.status) === 'source_god_mode_runtime_needs_repair' ||
      text(metadata.status) === 'failed_closed_browser_state_blocked'
    ) &&
    !unsafeSideEffectDetected
  ...
  if (newsletterIntakeUpgradeNeeded) {
    return {
      ...row,
      status: 'newsletter_intake_upgrade_needed',
      runnable: row.runnable === true,
      parked: false,
      runCommand: row.runnable === true ? row.runCommand : '',
      existingRunItemKey: existingRun.itemKey || existingRun.item_key || '',
      ...
      existingRunProcessedAt: existingRun.processedAt || existingRun.processed_at || '',
      previousRunPagesRead: number(metadata.pagesRead, 0),
      plainEnglish: `${row.label} has older generic source-browser evidence, but it has not been upgraded through newsletter:intake for source-inbox packet, confirmation-readback, and recurring issue-extraction readiness.`,
    }
  }
  ...
  if (operatorEscalationRequired) {
    return {
      ...row,
      status: 'previous_clean_retry_operator_escalation_required',
      runnable: false,
      parked: true,
      runCommand: '',
      existingRunItemKey: existingRun.itemKey || existingRun.item_key || '',
      ...
      existingRunEvidence,
      existingRunProcessedAt: existingRun.processedAt || existingRun.processed_at || '',
      existingRunLastError: 'operator_escalation_required_after_clean_retry',
      previousRunPagesRead: number(metadata.pagesRead, 0),
      previousRunBlockers: [
        {
          type: 'browser_challenge_not_source_content',
          reason: browserChallenge.detail || 'clean isolated retry still saw a browser challenge/interstitial',
          nextAction: browserChallenge.recovery || 'prepare Harlan/operator escalation or source-specific session repair; do not retry the same clean local path again',
        },
      ],
      plainEnglish: `${row.label} already had the clean isolated fallback retry and still could not prove real source content. Keep it parked for Harlan/operator escalation or source-specific session repair instead of reselecting the same retry.`,
    }
  }
  ...
  return {
    ...
    existingRunEvidence,
    existingRunProcessedAt: existingRun.processedAt || existingRun.processed_at || '',
    plainEnglish: existingRunEvidencePlainEnglish(row, existingRunEvidence),
  }
}
...
function buildRunCommand(row = {}) {
  ...
  if (row.runner === 'newsletter:intake') {
    return `npm run newsletter:intake -- --url=${row.url} --account=${SOURCE_SESSION_BROKER_DEFAULT_FREE_ACCOUNT} --json`
  }
...
function buildBrowserChallengeFallbackRetryBatch(rows = [], { maxRuns = 10, hostLimit = 1 } = {}) {
  ...
  return {
    status: cleanRetryRows.length ? 'ready_for_bounded_clean_retry' : preparedRows.length ? 'waiting_for_source_session_or_operator_escalation' : 'clear',
    ...
    plainEnglish: cleanRetryRows.length
      ...
      : preparedRows.length
        ? 'No browser-challenge rows can run a clean retry yet; source-session repair or Harlan/operator escalation is still needed.'
...
function buildBrowserChallengeFallbackPlanForRow(row = {}, blocker = {}) {
  ...
  return {
    ...
    route: text(fallbackPlan.route || 'clean_isolated_retry_then_harlan_operator'),
...
function normalizeNewsletterRunResult(row = {}, report = {}) {
  const packet = buildCreatorNewsletterIntakePacket(report)
  const safeFormDetected = Boolean(report.selectedForm)
  return {
    rowId: row.rowId,
    bucketId: row.bucketId,
    runner: row.runner,
    ...
    handsEvents: 0,
    newsletterCandidates: safeFormDetected ? 1 : 0,
    paidGateEvaluations: 0,
    sourceStackUpdate: {
      sourceId: packet.packetId || row.rowId,
      creatorId: row.creatorId || row.host,
      creatorName: row.creator || row.host,
      sourceFamily: 'creator_newsletters',
      sourceType: 'creator_newsletter',
      url: row.url,
      account: packet.account || SOURCE_SESSION_BROKER_DEFAULT_FREE_ACCOUNT,
      status: report.status || 'unknown',
      terminalState: report.ok === true ? 'completed' : 'blocked',
      toolRoute: 'newsletter:intake',
      nextAction: packet.nextAction,
      surfaces: {
        newsletter: safeFormDetected ? 'dry_run_packet_ready' : 'read_no_safe_form',
        submit: report.submitAllowedNow === true ? 'allowed_after_policy' : 'not_submitted',
      },
    },
    sopCompletion: {
    ...
    pageSummaries: [{
      ...
      title: report.title || row.label || row.host,
      textChars: number(report.textChars, 0),
      headings: report.title ? [report.title].slice(0, 1) : [],
      blockers: list(report.blockers).slice(0, 8),
    }],
    freeResourceCaptures: [],
    blockers: list(report.blockers).slice(0, 40),
    valueScore: null,
    usefulSignals: [
      report.plainEnglish,
      packet.nextAction,
    ].map(text).filter(Boolean).map(signal => ({ text: signal })).slice(0, 20),
...
function sourceBrowserAgentFamilyForHandoffRow(row = {}) {
  ...
  if (row.bucketId === 'creator-newsletters') return 'creator_newsletters'
  ...
  for (const row of selectedRows) {
    try {
      ...
      if (row.runner === 'newsletter:intake') {
        ...
        results.push(attachSourceBrowserAgentPlan(normalizeNewsletterRunResult(row, report), row, sourceBrowserAgentPlan))
        continue
      }
...
export function buildSourceGodModeYoutubeHandoffCrawlItemInput(result = {}, {
  ...
  return {
    ...
    metadata: {
      ...
      creatorId: result.creatorId || row.creatorId || '',
      creator: result.creator || row.creator || '',
      sourceCreatorIds: uniqueStrings([result.creatorId, row.creatorId, ...list(result.sourceCreatorIds), ...list(row.sourceCreatorIds)]),
      sourceCreators: uniqueStrings([result.creator, row.creator, ...list(result.sourceCreators), ...list(row.sourceCreators)]),
      sourceVideoId: result.sourceVideoId || row.sourceVideoId || '',
      sourceVideoIds: uniqueStrings([result.sourceVideoId, row.sourceVideoId, ...list(result.sourceVideoIds), ...list(row.sourceVideoIds)]),
      reportArtifactId: result.reportArtifactId || row.reportArtifactId || '',
      reportArtifactIds: uniqueStrings([result.reportArtifactId, row.reportArtifactId, ...list(result.reportArtifactIds), ...list(row.reportArtifactIds)]),
      status: result.status || '',
      ok: result.ok === true,
      ...
      newsletterIntakePacket: result.newsletterIntakePacket || null,
