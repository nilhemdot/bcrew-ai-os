...
export function buildYoutubeCreatorGodModeCatchupSnapshot({
  ...
  const majorBuildPromotionAllowed = baselineIncomplete.length === 0
  const checks = [
    {
      ok: rows.length >= 1,
    ...
    {
      ok: rows.every(row => row.approvedResourceFollowStatus && row.sourcePacketWorkerStatus && row.browserHandsStatus),
      check: 'resource, source-packet worker, and browser Hands statuses are explicit',
    ...
    {
      ok: !majorBuildPromotionAllowed || baselineIncomplete.length === 0,
      check: 'major build promotion gate follows source baseline completion',
      detail: majorBuildPromotionAllowed ? 'ready' : 'blocked_source_baseline_incomplete',
    },
  ]
  ...
  return {
    ...
    buildPromotionReadiness: {
      ...
      status: majorBuildPromotionAllowed ? 'ready_for_scoper_comparison' : 'blocked_source_baseline_incomplete',
      blockerCardId: majorBuildPromotionAllowed ? '' : YOUTUBE_CREATOR_GOD_MODE_CATCHUP_CARD_ID,
      reason: majorBuildPromotionAllowed
        ? 'All approved public creators meet the V1 latest-10 video/audio/visual baseline.'
...
export function buildYoutubeCreatorGodModeCatchupDogfoodProof() {
  ...
  const cases = [
    ...
    { name: 'c_source_throttles_after_baseline', ok: cNoise?.nextWatchAction === 'baseline_met_throttle_deep_watch_unless_lane_override' },
