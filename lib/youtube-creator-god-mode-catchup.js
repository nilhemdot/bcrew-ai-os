...
function videoIdFromLooseUrl(value = '') {
  const direct = youtubeVideoIdFromUrl(value)
  ...
  try {
    ...
    return text(
      url.searchParams.get('sourceVideoId') ||
...
function runMetadataVideoIds(item = {}, knownVideoIds = new Set()) {
  const metadata = item.metadata || {}
  const stack = metadata.sourceStackUpdate || {}
  const explicitIds = [
    metadata.sourceVideoId,
    metadata.videoId,
    ...list(metadata.sourceVideoIds),
    ...list(metadata.videoIds),
    stack.sourceVideoId,
    stack.videoId,
    youtubeVideoIdFromUrl(metadata.sourceUrl || metadata.exactUrl),
  ].map(text).filter(Boolean)
  const inferredIds = [
    videoIdFromLooseUrl(metadata.url),
    videoIdFromLooseUrl(metadata.sourceUrl),
    videoIdFromLooseUrl(metadata.exactUrl),
    stack.creatorId,
    stack.sourceId,
  ].map(text).filter(Boolean)
    .filter(id => knownVideoIds.has(id))
  return Array.from(new Set([...explicitIds, ...inferredIds])).filter(Boolean)
}
...
function sourcePacketIdFromWorkerRun(item = {}) {
  ...
  const stack = metadata.sourceStackUpdate || {}
  return text(
    metadata.sourcePacketId ||
    metadata.source_packet_id ||
    metadata.rowId ||
    stack.sourceId ||
    item.externalId ||
    item.external_id
...
function buildVideoSopEvidenceFromResult({ report = {}, result = {}, workerRunsByVideoId = new Map() } = {}) {
  ...
  return {
    ...
    sourceFollowupRunCount: workerRuns.length,
...
export function buildYoutubeCreatorSourceSopEvidence({
  ...
  for (const report of list(youtubeFullWatchReports)) {
    ...
    if (!results.length) {
      for (const videoId of videoIdsFromReport(report)) knownVideoIds.add(videoId)
    }
  }

  const sourceRuns = [
    ...list(sourcePacketWorkerRuns),
    ...list(sourceFollowupRuns),
  ]
  const workerRunsByVideoId = new Map()
  for (const item of sourceRuns) {
    for (const videoId of runMetadataVideoIds(item, knownVideoIds)) {
      if (!videoId) continue
    ...
    if (!results.length) {
      for (const videoId of videoIdsFromReport(report)) {
        const evidence = {
          ...
          sourceFollowupRunCount: list(workerRunsByVideoId.get(videoId)).length,
  ...
  return {
    ...
    summary: {
      ...
      sourceFollowupRunCount: sourceRuns.length,
      sourceFollowupRunMappedVideoCount: workerRunsByVideoId.size,
...
function buildIntelRelevantForLongCourseLane(item = {}) {
  const relevance = scoreBuildIntelVideo(item)
  return relevance.titlePositiveTerms.length >= 1 &&
    relevance.score >= 16 &&
    relevance.negativeTerms.length === 0
}

function buildIntelRelevantForStandardLane(item = {}) {
  const relevance = scoreBuildIntelVideo(item)
  return relevance.titlePositiveTerms.length >= 1 &&
    relevance.score >= 16 &&
    relevance.negativeTerms.length === 0
...
function standardLaneParkReason(item = {}) {
  const relevance = scoreBuildIntelVideo(item)
  if (relevance.negativeTerms.length) return `not_dev_lane:${relevance.negativeTerms.join('|')}`
  if (!relevance.titlePositiveTerms.length) return 'not_dev_lane:no_dev_terms_in_title'
  if (relevance.score < 16) return `not_dev_lane:low_dev_score_${relevance.score}`
  return 'not_dev_lane:filtered_before_spend'
}
...
function nextWatchActionForRow(row = {}) {
  ...
  if (row.pendingStandardVideoCount > 0) return 'baseline_met_keep_daily_delta_current'
...
function summarizeCreatorSopEvidence({ creatorId = '', rows = [], watchedCount = 0, sourceSopEvidence = null } = {}) {
  ...
  const totals = uniqueEvidence.reduce((acc, item) => {
    ...
    acc.sourceFollowupRunCount += number(item.sourceFollowupRunCount)
    return acc
  }, {
    fullWatchComplete: 0,
    fullPageComplete: 0,
...
function buildCreatorRow({ creator = {}, poolRows = [], fullWatchedVideoIds = new Set(), grade = null, sourceSopEvidence = null } = {}) {
  ...
  const creatorChannelUrl = normalizeYoutubeChannelUrl(creator.channelUrl || creator.channelVideosUrl || '')
  const rows = poolRows
    .filter(item =>
      item.creatorId === creatorId &&
      (!creatorChannelUrl || !item.channelUrl || normalizeYoutubeChannelUrl(item.channelUrl) === creatorChannelUrl)
    )
  ...
  const watchedCount = Math.max(watchedByVideoId, watchedByReportEvidence)
  const longCourseRows = rows.filter(item =>
    item.videoId &&
    !fullWatchedVideoIds.has(item.videoId) &&
    item.publicNoAuth &&
    !item.privateOrPaidAccess &&
    !item.fullWatchBlocked &&
    buildIntelRelevantForLongCourseLane(item) &&
  ...
  const unaccountedStandardRows = rawPendingStandardRows.slice(Math.min(rawPendingStandardRows.length, pendingOffsetFromGrade))
  const pendingStandardRows = unaccountedStandardRows.filter(buildIntelRelevantForStandardLane)
  const parkedStandardRows = unaccountedStandardRows.filter(item => !buildIntelRelevantForStandardLane(item))
  const providerBlockedRows = rows.filter(item =>
    item.videoId &&
    !fullWatchedVideoIds.has(item.videoId) &&
    item.publicNoAuth &&
    !item.privateOrPaidAccess &&
  ...
  const effectiveBaselineTarget = Math.min(baselineTarget, watchedCount + actionableBaselineRows.length)
  const baselineGap = Math.max(0, effectiveBaselineTarget - watchedCount)
  const deepBaselineGap = Math.max(0, deepTarget - watchedCount)
  const baselineAvailabilityStatus = baselineGap > 0
    ? 'needs_more_public_video_watch'
    : watchedCount < baselineTarget
      ? rows.length < baselineTarget
        ? 'available_public_metadata_exhausted_below_target'
        : 'dev_lane_actionable_rows_exhausted_below_target'
      : 'target_met'
  ...
  const sourceFollowupRunCount = number(sopEvidence.sourceFollowupRunCount || sopEvidence.sourcePacketWorkerRunCount)
  const sourcePacketWorkerDone = resourceWorkRequired
    ? sourceFollowupRunCount >= (sopEvidence.approvalRequiredLinkCount + sopEvidence.resolvedPublicResourceLinkCount)
    : sopEvidence.resourceDispositionComplete > 0
  const freeResourceDone = sopEvidence.resolvedPublicResourceLinkCount > 0
    ? sourceFollowupRunCount >= sopEvidence.resolvedPublicResourceLinkCount
    : sopEvidence.resourceDispositionComplete > 0
  const baseRow = {
    creatorId,
    ...
    effectiveBaselineTargetVideos: effectiveBaselineTarget,
    ...
    parkedStandardVideoCount: parkedStandardRows.length,
    providerBlockedVideoCount: providerBlockedRows.length,
    devBuildGrade,
    ...
    nextWatchCandidates: pendingStandardRows.slice(0, Math.max(baselineTarget, deepTarget)).map(item => ({
      videoId: item.videoId,
      title: item.title,
      url: item.url,
      rank: item.rank,
    })),
    parkedStandardCandidates: parkedStandardRows.slice(0, 3).map(item => ({
      videoId: item.videoId,
      title: item.title,
      url: item.url,
      rank: item.rank,
      reason: standardLaneParkReason(item),
    })),
    providerBlockedCandidates: providerBlockedRows.slice(0, 3).map(item => ({
      videoId: item.videoId,
      title: item.title,
      url: item.url,
      rank: item.rank,
      reason: text(item.fullWatchBlockedReason, 'provider_rejected_video'),
      route: buildIntelRelevantForLongCourseLane(item) || buildIntelRelevantForStandardLane(item)
        ? 'retry_with_alternate_video_route'
        : 'parked_provider_blocked_not_dev_relevant',
    })),
...
export function buildYoutubeCreatorGodModeCatchupSnapshot({
  ...
  return {
    ...
    summary: {
      ...
      parkedStandardVideoCount: rows.reduce((sum, row) => sum + row.parkedStandardVideoCount, 0),
      providerBlockedVideoCount: rows.reduce((sum, row) => sum + row.providerBlockedVideoCount, 0),
      ...
      sourceFollowupRunCount: sourceSopEvidence.summary.sourceFollowupRunCount,
      sourceFollowupRunMappedVideoCount: sourceSopEvidence.summary.sourceFollowupRunMappedVideoCount,
...
export function buildYoutubeCreatorGodModeCatchupDogfoodProof() {
  const watchPlan = {
    creators: [
      ...
      {
        creatorId: 'small-channel',
        displayName: 'Small Channel',
        channelUrl: 'https://www.youtube.com/@small-channel',
        channelVideosUrl: 'https://www.youtube.com/@small-channel/videos',
        priority: 'P2',
      },
    ],
  }
  const poolItems = [
    ...
    { external_id: 's-code-course', target_key: YOUTUBE_CREATOR_DAILY_WATCH_TARGET_KEY, source_id: YOUTUBE_CREATOR_DAILY_WATCH_SOURCE_ID, metadata: { creatorId: 's-builder', creator: 'S Builder', videoId: 's-code-course', title: "Let's build GPT: from scratch, in code, spelled out.", duration: '1:56:20', rank: 4, publicNoAuth: true, privateOrPaidAccess: false } },
    ...
    { external_id: 'c-2', target_key: YOUTUBE_CREATOR_DAILY_WATCH_TARGET_KEY, source_id: YOUTUBE_CREATOR_DAILY_WATCH_SOURCE_ID, metadata: { creatorId: 'c-noise', creator: 'C Noise', videoId: 'c-2', title: 'Build Claude Code browser agent workflow', rank: 2, publicNoAuth: true, privateOrPaidAccess: false } },
    { external_id: 'small-1', target_key: YOUTUBE_CREATOR_DAILY_WATCH_TARGET_KEY, source_id: YOUTUBE_CREATOR_DAILY_WATCH_SOURCE_ID, metadata: { creatorId: 'small-channel', creator: 'Small Channel', videoId: 'small-1', title: 'Claude Code memory workflow', rank: 1, publicNoAuth: true, privateOrPaidAccess: false } },
  ]
  const snapshot = buildYoutubeCreatorGodModeCatchupSnapshot({
    generatedAt: '2026-05-26T06:30:00.000Z',
    watchPlan,
    poolItems,
    fullWatchedVideoIds: ['c-1', 'small-1'],
    sourceGrades: [
      ...
      { creatorId: 'small-channel', creator: 'Small Channel', devBuildGrade: 'B', overallGrade: 'B', watchedVideos: 1, buildCandidates: 1, bestDirectorRank: 75, watchRecommendation: 'monitor_daily' },
    ],
  })
  ...
  const small = snapshot.creators.find(row => row.creatorId === 'small-channel')
  const cases = [
    ...
    { name: 'long_course_visible_not_standard_watch', ok: sBuilder?.longCoursePendingCount === 2 && sBuilder?.longCourseCandidates.some(video => video.videoId === 's-code-course') },
    ...
    { name: 'stale_grade_watched_count_does_not_satisfy_current_channel_baseline', ok: cNoise?.videoAudioVisualWatchedCount === 1 && cNoise?.baselineGap > 0 },
    { name: 'available_public_metadata_exhausted_does_not_false_block_baseline', ok: small?.videoAudioVisualWatchedCount === 1 && small?.baselineGap === 0 && small?.baselineAvailabilityStatus === 'available_public_metadata_exhausted_below_target' },
