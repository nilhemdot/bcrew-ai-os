...
export function buildYoutubeCreatorSourceSopEvidence({
  ...
  return {
    ...
    summary: {
      ...
      fullPageEvidenceCount: Array.from(byVideoId.values()).filter(item => item.fullPageComplete).length,
      resourceDispositionEvidenceCount: Array.from(byVideoId.values()).filter(item => item.resourceDispositionComplete).length,
    },
  }
}
