...
export async function buildFoundationRuntimeReliabilityVerifierDogfoodProof() {
  ...
  return {
    ...
    dogfoodInvariant: 'The focused runtime reliability verifier accepts healthy synthetic reliability proof and rejects missing outage boundaries, missing operating status, missing Runtime Supervisor service proof, missing Runtime Health command read, missing Plan Critic dogfood, missing file-size standard dogfood, missing agent status freshness dogfood, missing agent usefulness runtime gates dogfood, missing live-answer preflight dogfood, missing capability registry dogfood, missing agent template dogfood, missing old-system onboarding harvest dogfood, oversized hub payloads, missing ship preflight wiring, and missing ClickUp slow-budget proof.',
  }
}
