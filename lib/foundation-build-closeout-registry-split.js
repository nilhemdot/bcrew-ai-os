...
export const FOUNDATION_BUILD_CLOSEOUT_REGISTRY_SPLIT_HANDOFF_PATH = 'docs/handoffs/2026-05-15-foundation-build-closeout-registry-split-closeout.md'
...
export function evaluateFoundationBuildCloseoutRegistrySplit({
  ...
  const controlPlaneRequiredKeysPresent = FOUNDATION_BUILD_CLOSEOUT_CONTROL_PLANE_REQUIRED_KEYS
    .every(key => String(controlPlaneSource || '').includes(key))
  const checks = [
    {
    ...
    {
      ok: controlPlaneLines >= minimumControlPlaneLines && String(controlPlaneSource || '').includes('export const controlPlaneCloseoutRecords = ['),
    ...
    {
      ok: beforeKeys.length === 0 || sameKeys(beforeKeys, afterKeys),
