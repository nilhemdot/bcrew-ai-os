...
function buildSourceSopReadiness(video = {}) {
  const selectedForWatch = text(video.nextWatchAction) === 'watch_next_standard_public_videos_with_scheduler'
  return [
    buildSopStep({
...
function videoActionAllowsWatch(video = {}) {
  const action = text(video.nextWatchAction)
  return !action || RUNNABLE_WATCH_ACTIONS.has(action)
}
...
export function buildYoutubeGodModeAutonomousWatchPlan({
  ...
  for (const rawVideo of list(candidateVideos).map(normalizeVideo)) {
    ...
    if (!videoActionAllowsWatch(rawVideo)) {
...
export function buildYoutubeGodModeAutonomousWatchSchedulerDogfoodProof() {
  ...
  const candidateVideos = [
    ...
    { videoId: 'sop-blocked-1', creatorId: 'mark-kashef', creator: 'Mark Kashef', title: 'SOP pending before deep watch', relevanceScore: 100, publicNoAuth: true, nextWatchAction: 'complete_youtube_source_sop_before_god_mode_claim' },
  ...
  const cases = [
    ...
    caseResult('blocks video watching when the creator SOP says finish resource/gate work first', base, plan =>
      plan.rejectedVideos.some(video =>
        video.videoId === 'sop-blocked-1' &&
