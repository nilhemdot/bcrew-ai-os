...
function videoActionAllowsWatch(video = {}, config = {}) {
  ...
  return number(video.pendingStandardVideoCount) > 0 && [
    'complete_youtube_source_sop_before_god_mode_claim',
...
export function buildYoutubeGodModeAutonomousWatchSchedulerDogfoodProof() {
  ...
  const candidateVideos = [
    ...
    { videoId: 'sop-blocked-1', creatorId: 'mark-kashef', creator: 'Mark Kashef', title: 'SOP pending before deep watch', relevanceScore: 100, publicNoAuth: true, pendingStandardVideoCount: 1, nextWatchAction: 'complete_youtube_source_sop_before_god_mode_claim' },
  ...
  const cases = [
    ...
    caseResult('catch-up mode can finish safe public standard backlog after baseline without changing paid/private/long-course stops', publicBacklogCatchup, plan =>
      ...
      plan.selectedVideos.some(video => video.videoId === 'sop-blocked-1') &&
