...
function directorRankBonus(rank = null) {
  if (rank === null || rank === undefined || text(rank) === '') return 0
...
function devDirectorRankScore(rank = null) {
  if (rank === null || rank === undefined || text(rank) === '') return 0
  const numeric = asNumber(rank, 9999)
  if (numeric === 1) return 24
  if (numeric <= 5) return 22
  if (numeric <= 15) return 18
  if (numeric <= 35) return 14
  if (numeric <= 75) return 8
  if (numeric <= 150) return 4
  return 0
}

function devCandidateYieldScore(candidateCount = 0) {
  const count = asNumber(candidateCount, 0)
  if (count >= 100) return 20
  if (count >= 70) return 17
...
function devSpecificityScore(foundationSpecificHits = 0, termHits = 0) {
  const specific = asNumber(foundationSpecificHits, 0)
  const broad = asNumber(termHits, 0)
  if (specific >= 18) return 20
  if (specific >= 14) return 16
  if (specific >= 10) return 12
  if (specific >= 6) return 8
  if (specific >= 3) return 5
  return broad >= 8 ? 3 : 0
}

function devResourceScore(source = {}) {
  return Math.min(8, Math.round(
    asNumber(source.resolvedPublicResourceLinkCount, 0) * 0.08 +
    asNumber(source.approvalRequiredLinkCount, 0) * 0.015
  ))
}

function scoreDevBuildLane(source = {}, lane = {}) {
  ...
  const bestDirector = source.directorRanks.length
    ...
    : null
  const evidenceScore = Math.min(14, Math.round(videos.length))
  const candidateYieldScore = devCandidateYieldScore(source.candidates.length)
  const specificityScore = devSpecificityScore(foundationSpecificHits, termHits)
  const directorScore = devDirectorRankScore(bestDirector)
  const resourceScore = devResourceScore(source)
  const confidenceScore = Math.min(6, source.candidates.filter(candidate => candidate.confidence === 'high').length)
  ...
  const offTopicDevCapApplies = offTopicNoiseHits > 0 && foundationSpecificHits < 3
...
function scoreLane(source = {}, lane = {}) {
  if (lane.id === 'aios_dev_build') return scoreDevBuildLane(source, lane)
  const videos = Array.from(source.videos.values())
  const candidateText = evidenceText(source.candidates.flatMap(candidate => [
    candidate.title,
    candidate.whyItMatters,
...
function buildSourceGrade(source = {}) {
  ...
  return {
    ...
    devWatchRecommendation: devLane?.watchRecommendation || 'pause_until_new_signal',
...
export function renderBuildIntelSourceValueGraderReport(snapshot = {}) {
  const lines = [
    ...
    ...snapshot.topDevBuildSources.map((source, index) => [
      ...
      `- Dev watch recommendation: ${source.devWatchRecommendation || source.watchRecommendation}`,
      `- Overall watch recommendation: ${source.watchRecommendation}`,
...
export function buildBuildIntelSourceValueGraderDogfoodProof() {
  const reports = [
    {
      ...
      structured_output_json: {
        videos: Array.from({ length: 5 }, (_, index) => ({
          videoId: `builder-${index + 1}`,
          title: `Claude Code MCP agent workflow ${index + 1}`,
          creatorId: 'builder-source',
          creator: 'Builder Source',
          buildCandidates: [
            { title: `MCP Agent Router ${index + 1}`, whyItMatters: 'AIOS tool registry and agentic workflow for Claude Code browser runtime, source packet routing, Codex hooks, and Foundation extraction.', recommendedNextStep: 'Build MCP routing for the AIOS agent runtime.', confidence: 'high', sourceVideoId: `builder-${index + 1}` },
            { title: `Agent State Hooks ${index + 1}`, whyItMatters: 'Claude Code lifecycle hooks, Codex handoff state, browser source extractor verification, and Foundation provenance.', recommendedNextStep: 'Add hook registry and source-backed state proof.', confidence: 'high', sourceVideoId: `builder-${index + 1}` },
          ],
    ...
    {
      report_artifact_id: 'report:source-title-only',
      structured_output_json: {
        videos: [{
          videoId: 'title-only-1',
          title: 'General updates and notes',
          creatorId: 'source-title-only',
          creator: 'Source Title Only',
          buildCandidates: Array.from({ length: 5 }, (_, index) => ({
            title: `Neutral metadata item ${index + 1}`,
            whyItMatters: 'This is neutral readback with no extracted implementation value.',
            recommendedNextStep: 'Keep the source provenance attached for review.',
            confidence: 'medium',
            sourceVideoId: 'title-only-1',
            sourceTitle: 'Claude Code MCP agent workflow full course',
          })),
          resourceLinkPacket: { resolvedResourceRefs: [], approvalRequiredResourceLinks: [] },
        }],
      },
  ...
  const sourceTitleOnly = snapshot.sourceGrades.find(source => source.creatorId === 'source-title-only')
  const cases = [
    {
      name: 'high_ranked_builder_grades_above_noisy_source',
      ok: gradeRankValue(builder?.devBuildGrade) > gradeRankValue(noisy?.devBuildGrade) &&
        ['sample_only', 'pause_until_new_signal'].includes(noisy?.devWatchRecommendation || noisy?.watchRecommendation),
    },
    {
    ...
    {
      name: 'source_title_provenance_alone_does_not_inflate_dev_grade',
      ok: ['C', 'D'].includes(sourceTitleOnly?.devBuildGrade) &&
        sourceTitleOnly?.laneScores.find(lane => lane.laneId === 'aios_dev_build')?.evidence?.foundationSpecificHits === 0,
    },
  ]
  return {
