...
export function evaluateFoundationVerifierControlLoop(input = {}) {
  ...
  const actionReviewApplyTextLower = actionReviewApplyText.toLowerCase()
    ...
    'Action Route Review Inbox separates proposed findings from normal backlog cards',
    `${actionRouteReviewInboxApi?.summary?.totalReviewItems || 0} review items / ${actionRouteReviewInboxApi?.backlogSeparation?.separatedBacklogItems || 0} separated backlog rows`,
  )
  ensure(
    checks,
