...
export const VERIFIER_CONTROL_LOOP_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-control-loop-split-closeout.md'
...
function compactBacklogText(item = {}, fallbackText = '') {
  return [
    item?.title,
    item?.summary,
    item?.whyItMatters,
...
export function evaluateFoundationVerifierControlLoop(input = {}) {
  ...
  const actionReviewApply =
    (foundationBacklogDoneArchiveApi?.backlogItems || []).find(item => item.id === 'ACTION-REVIEW-APPLY-001') ||
    (foundationHub.backlogItems || []).find(item => item.id === 'ACTION-REVIEW-APPLY-001') ||
    null
  const actionReviewApplyFallbackText = [
    currentPlan,
    currentState,
  ].filter(source => String(source || '').includes('ACTION-REVIEW-APPLY-001')).join('\n')
  const actionReviewApplyText = compactBacklogText(actionReviewApply, actionReviewApplyFallbackText)
  const actionReviewApplyTextLower = actionReviewApplyText.toLowerCase()
    ...
    checks,
    actionRouteReviewInboxApi?.visibleHome === 'Foundation > Review queues > Action Route Review Inbox' &&
      actionRouteReviewInboxApi?.summary?.totalReviewItems >= actionRouterSnapshot.totalRoutes &&
      actionRouteReviewInboxApi?.summary?.sourceCompleteItems === actionRouteReviewInboxApi?.summary?.totalReviewItems &&
      actionRouteReviewInboxApi?.backlogSeparation?.separatedBacklogItems >= 1 &&
      actionRouteReviewInboxApi?.source?.dedupStalenessOwner === 'ACTION-ROUTE-DEDUP-STALENESS-GUARD-001' &&
      actionRouteReviewInboxApi?.dedupStaleness?.contractVersion === 'action-route-dedup-staleness-guard.v1' &&
      Number(actionRouteReviewInboxApi?.dedupStaleness?.summary?.totalReviewItems || 0) === actionRouteReviewInboxApi?.summary?.totalReviewItems &&
      actionRouteReviewInboxApi?.boundaries?.readOnly === true &&
      actionRouteReviewInboxApi?.boundaries?.noExternalWrites === true &&
      Array.isArray(actionRouteReviewInboxApi?.reviewItems) &&
      actionRouteReviewInboxApi.reviewItems.every(item => item.dedupeKey && item.staleness?.status) &&
      actionRouteReviewInboxApi.reviewItems.some(item => item.sourceKind === 'backlog_action_route_candidate') &&
      !(foundationBacklogListApi?.backlogItems || []).some(item => /Action route:\s*action-route:/i.test(String(item.statusNote || ''))),
    'Action Route Review Inbox separates proposed findings from normal backlog cards and carries dedupe/staleness guard',
    `${actionRouteReviewInboxApi?.summary?.totalReviewItems || 0} review items / ${actionRouteReviewInboxApi?.backlogSeparation?.separatedBacklogItems || 0} separated backlog rows / ${actionRouteReviewInboxApi?.summary?.duplicateClusters || 0} duplicate clusters`,
  )
  ensure(
    checks,
    strategySharedCommsRouteSource.includes("app.get('/api/foundation/action-review'") &&
      strategySharedCommsRouteSource.includes("app.get(ACTION_ROUTE_REVIEW_INBOX_API_PATH") &&
      ...
      foundationFrontendSource.includes('function renderActionRouteReviewInbox') &&
      foundationFrontendSource.includes('/api/foundation/action-route-review-inbox') &&
  ...
  const inboxProposal = buildResearchInboxPromotionProposal(validBuildIntelInboxItem)
    ...
    creatorWatchlist?.lane === 'done' &&
      ...
      buildIntelWatchlistSnapshot.summary?.buildIntelCount >= 29 &&
