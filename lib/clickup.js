...
export const DEFAULT_CLICKUP_REQUEST_TIMEOUT_MS = 30000
...
export function getClickUpRequestTimeoutMs(options = {}) {
  return normalizePositiveInteger(
    options.timeoutMs ?? process.env.CLICKUP_REQUEST_TIMEOUT_MS,
    DEFAULT_CLICKUP_REQUEST_TIMEOUT_MS,
  )
}

export async function clickUpGet(path, options = {}) {
  return clickUpRequest('GET', path, undefined, options)
}

export async function clickUpRequest(method, path, body, options = {}) {
  const token = loadClickUpToken()
  if (!token) throw new Error('CLICKUP_PERSONAL_TOKEN is missing')

  const timeoutMs = getClickUpRequestTimeoutMs(options)
  const controller = new AbortController()
  const timeout = setTimeout(() => {
    controller.abort(new Error(`ClickUp ${method} ${path} timed out after ${timeoutMs}ms`))
  }, timeoutMs)
  ...
  try {
    const res = await fetch(`https://api.clickup.com/api/v2${path}`, requestOptions)
    const text = await res.text()
    if (!res.ok) {
      const error = new Error(`ClickUp ${method} ${path} returned ${res.status}: ${text}`)
    ...
    if (error?.name === 'AbortError') {
      const timeoutError = new Error(`ClickUp ${method} ${path} timed out after ${timeoutMs}ms`)
...
export async function listClickUpTasks(listId, options = {}) {
  ...
  const maxPages = normalizePositiveInteger(options.maxPages, 50)
  for (let page = 0; page < maxPages; page += 1) {
    const data = await clickUpGet(
      `/list/${listId}/task?include_closed=${includeClosed}&page=${page}&subtasks=${subtasks}`,
...
export async function getClickUpListSnapshot(listId, options = {}) {
  const [list, fieldsPayload, tasks] = await Promise.all([
    clickUpGet(`/list/${listId}`, { timeoutMs: options.timeoutMs }),
    clickUpGet(`/list/${listId}/field`, { timeoutMs: options.timeoutMs }),
    listClickUpTasks(listId, options),
  ])
...
export async function getClickUpListSnapshotSafe(listId, options = {}) {
  try {
    const snapshot = await getClickUpListSnapshot(listId, options)
    return {
      ...snapshot,
      unavailable: false,
