...
export function buildDevIntelSourceCoverageSnapshot({ generatedAt = new Date().toISOString() } = {}) {
  const rows = [
    ...
    family({
      ...
      feedsDev: false,
      feedPath: 'source:god-mode runtime output -> creator source-stack row -> Dev Director/Scoper evidence',
      currentProof: ['SOURCE-BROWSER-AGENTIC-RUNTIME-001 source:god-mode runtime proof', 'EXTRACTOR-EYES-HANDS-BRAIN-RUNTIME-001 source-stack SOP'],
      nextCard: 'SOURCE-BROWSER-AGENTIC-RUNTIME-001',
      nextAction: 'Wire runtime sourceStackUpdate output into real creator cards: YouTube, blog/site, newsletter, GitHub/docs/resources, free Skool/community, paid Skool/course/training platforms, and other approved sources.',
    ...
    family({
      ...
      currentProof: ['SOURCE-BROWSER-AGENTIC-RUNTIME-001 detects newsletter signup pages/forms without submit', 'EXTRACTOR-EYES-HANDS-BRAIN-RUNTIME-001 newsletter SOP'],
      nextCard: 'SOURCE-BROWSER-AGENTIC-RUNTIME-001',
      nextAction: 'Keep signup detection in source:god-mode, then build the mailbox source lane: sign up approved free creator newsletters with ai@bensoncrew.ca or crewbert@bensoncrew.ca, route issues to AIOS Sources/Newsletters, extract value, grade, and unsubscribe/park low-value sources.',
    ...
    family({
      ...
      feedsDev: false,
      feedPath: 'public repo/resource link -> source:god-mode public/free read -> repo-specific worker/scoper packet',
      currentProof: ['SOURCE-BROWSER-AGENTIC-RUNTIME-001 public/free runtime proof', 'SRC-GITHUB-BUILD-INTEL-001 source boundary locked'],
      nextCard: 'SOURCE-BROWSER-AGENTIC-RUNTIME-001',
      nextAction: 'Use the public/free runtime for public repo links, then add repo-specific README/docs/license/provenance extraction before importing code or treating repo findings as build-ready.',
    ...
    family({
      ...
      currentProof: ['SOURCE-BROWSER-AGENTIC-RUNTIME-001 local free-community runtime proof', 'EXTRACTOR-EYES-HANDS-BRAIN-RUNTIME-001 free Skool/community SOP', 'SKOOL-WORKER-001 preflight'],
      nextCard: 'FREE-SKOOL-COMMUNITY-GOD-MODE-RUNNER-001',
      nextAction: 'Run the first real free-community proof with ai@bensoncrew.ca or crewbert@bensoncrew.ca, inspect allowed free areas and last 20 days of free chat/posts/comments, capture resources, and stop at paid/private/member/action boundaries.',
    ...
    family({
      ...
      label: 'Paid Courses / Training Platforms',
      status: DEV_INTEL_SOURCE_FAMILY_STATUS.blocked,
      sourceIds: ['SRC-MYICRO-001'],
      feedsDev: false,
      blocker: 'Paid course/training platform extraction is blocked until Steve approves one exact source item and allowed content boundary.',
      ...
      nextAction: 'Approve one exact paid course/training platform lesson/source packet before course extraction. MyICOR is the current known example, not its own source-family tag.',
      notes: 'Source ID currently uses the historical repo spelling `SRC-MYICRO-001`; keep that as provenance, not taxonomy.',
    }),
    family({
  ...
  const checks = [
    ...
    { ok: rows.some(row => row.familyId === 'creator-source-stack' && row.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial && /newsletter/i.test(row.nextAction) && /Skool/i.test(row.nextAction)), check: 'creator source stack includes all creator surfaces', detail: 'creator-source-stack' },
    ...
    { ok: rows.some(row => row.familyId === 'skool-free-communities' && row.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial && /last 20 days/i.test(row.nextAction) && /paid\/private/i.test(row.nextAction)), check: 'free Skool communities are separate from paid Skool blockers', detail: 'skool-free-communities' },
    ...
    { ok: rows.some(row => row.familyId === 'paid-course-training-platforms' && row.status === DEV_INTEL_SOURCE_FAMILY_STATUS.blocked), check: 'paid course/training platforms stay blocked until source packet approval', detail: 'SRC-MYICRO-001 is one instance, not taxonomy' },
  ...
  const cases = [
    ...
    },
    {
      ...
      ok: byId.get('skool-paid-communities')?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.blocked &&
        byId.get('paid-course-training-platforms')?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.blocked,
    },
    {
    ...
    {
      name: 'source_god_mode_runtime_moved_key_free_sources_to_partial',
      ok: byId.get('creator-source-stack')?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial &&
        byId.get('creator-newsletters')?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial &&
        byId.get('skool-free-communities')?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial &&
        byId.get('github-public-repos')?.status === DEV_INTEL_SOURCE_FAMILY_STATUS.partial,
    },
    {
