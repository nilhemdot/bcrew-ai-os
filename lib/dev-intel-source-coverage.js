...
export function buildDevIntelSourceCoverageSnapshot({ generatedAt = new Date().toISOString() } = {}) {
  const rows = [
    ...
    family({
      ...
      feedsDev: false,
      feedPath: 'public repo/resource link -> repo:deep-review public README/docs/examples/license read -> implementation-pattern packet',
      ...
      nextCard: 'SOURCE-BROWSER-AGENTIC-RUNTIME-001',
      nextAction: 'Use `repo:deep-review` for public repo links before importing code or treating repo findings as build-ready.',
