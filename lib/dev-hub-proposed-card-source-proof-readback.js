...
function buildMorningBuildReview(summary = {}, rows = []) {
  const topRows = list(rows).slice(0, 3)
  const messageLines = [
    `${count(summary.proofItemCount)} proposed Dev build card(s) are ready for exact approval review.`,
    `${count(summary.sourceProofReadyCount)} have source proof ready; ${count(summary.candidateProofRowCount)} candidate proof row(s) and ${count(summary.sourceLineageRefCount)} source-lineage ref(s) are visible.`,
    'No cards were created and no builds were authorized.',
  ]
  return {
    status: count(summary.proofItemCount) > 0 && count(summary.missingProofCount) === 0
      ? 'ready_for_steve_review'
      : 'proof_missing',
    title: 'Morning build review',
    summaryText: messageLines.join(' '),
    approvalRequiredCount: count(summary.proofItemCount),
    sourceProofReadyCount: count(summary.sourceProofReadyCount),
    candidateProofRowCount: count(summary.candidateProofRowCount),
    sourceLineageRefCount: count(summary.sourceLineageRefCount),
    cardsCreatedByReadback: 0,
    buildAuthorizationsByReadback: 0,
    harlanDigestPreview: {
      status: 'preview_only',
      sendsMessageNow: false,
      messagePreview: messageLines.join('\n'),
    },
    topItems: topRows.map(row => ({
      proposedCardId: text(row.proposedCardId),
      title: text(row.title),
      portfolioScore: count(row.portfolioScore),
      candidateProofCount: count(row.candidateProofCount),
      sourceLineageCount: count(row.sourceLineageCount),
      approvalPhrase: text(row.approvalPhrase),
    })),
  }
}
...
export function buildDevHubProposedCardSourceProofReadback({
  ...
  const morningBuildReview = buildMorningBuildReview(summary, rows)
  const failures = []
  ...
  if (morningBuildReview.harlanDigestPreview?.sendsMessageNow !== false) failures.push('harlan_digest_preview_sent')
  if (morningBuildReview.cardsCreatedByReadback !== 0 || morningBuildReview.buildAuthorizationsByReadback !== 0) failures.push('morning_review_mutated_or_authorized')
...
export function validateDevHubProposedCardSourceProofReadback(snapshot = {}) {
  ...
  const morningBuildReview = snapshot.morningBuildReview || {}
  if (morningBuildReview.status !== 'ready_for_steve_review') failures.push('morning_build_review_not_ready')
  if (count(morningBuildReview.approvalRequiredCount) !== rows.length || count(morningBuildReview.sourceProofReadyCount) !== rows.length) failures.push('morning_build_review_count_mismatch')
  if (count(morningBuildReview.cardsCreatedByReadback) !== 0 || count(morningBuildReview.buildAuthorizationsByReadback) !== 0) failures.push('morning_review_mutated_or_authorized')
  if (morningBuildReview.harlanDigestPreview?.sendsMessageNow !== false || morningBuildReview.harlanDigestPreview?.status !== 'preview_only') failures.push('harlan_digest_preview_sent')
  if (!text(morningBuildReview.summaryText) || !list(morningBuildReview.topItems).length) failures.push('morning_build_review_context_missing')
