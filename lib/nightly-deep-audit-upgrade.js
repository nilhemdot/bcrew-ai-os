...
import { LLM_WORKLOADS, callLlm, planLlmRoute } from './llm-router.js'
export {
...
function buildSeniorReviewPrompt({ deterministicAudit = {}, reviewTargets = [], diff = {} } = {}) {
...
export function classifyDeepSeniorReviewRollup(review = {}) {
  if (review.executedThisRun === true && review.status === 'executed') {
    return {
      status: 'healthy',
      executed: true,
      detail: `deep senior review executed with ${review.findingCount || 0} finding(s)`,
    }
  }
  return {
    status: 'degraded',
...
export async function buildNightlyDeepAuditSeniorReview({
  ...
  const prompt = buildSeniorReviewPrompt({ deterministicAudit, reviewTargets, diff })
  ...
  try {
    ...
    const findings = buildSeniorReviewActionRoutes(Array.isArray(parsed.findings) ? parsed.findings : [])
    return {
      status: 'executed',
...
- Finding count: ${audit.llmReview?.findingCount || 0}
