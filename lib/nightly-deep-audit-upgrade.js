...
function compactFindingForNightlyJson(finding = {}) {
  return {
    id: finding.id || '',
    severity: finding.severity || '',
    title: finding.title || '',
    proposedCard: finding.proposedCard || finding.cardId || '',
    proposedOwner: finding.proposedOwner || finding.owner || '',
    refs: (finding.refs || []).slice(0, 3).map(ref => ({
      path: ref.path || '',
      line: ref.line || null,
      detail: ref.detail || '',
    })),
...
function compactReconciledFindingForNightlyJson(finding = {}) {
  return {
    ...compactFindingForNightlyJson(finding),
    reconciliationStatus: finding.reconciliationStatus || '',
    reconciliationReason: finding.reconciliationReason || '',
    route: finding.route ? {
      targetCardId: finding.route.targetCardId || '',
      targetCloseoutKey: finding.route.targetCloseoutKey || '',
      coveredByCardId: finding.route.coveredByCardId || '',
      coveredByCloseoutKey: finding.route.coveredByCloseoutKey || '',
    } : null,
    closeout: finding.closeout ? {
      key: finding.closeout.key || '',
      status: finding.closeout.status || '',
      acceptanceState: finding.closeout.acceptanceState || '',
    } : null,
  }
}

function countFindingsByProposedCard(findings = []) {
  return findings.reduce((counts, finding) => {
    const key = finding.proposedCard || finding.cardId || 'missing'
    counts[key] = (counts[key] || 0) + 1
    return counts
  }, {})
}

function compactDeterministicFindingReconciliationForNightlyJson(reconciliation = {}) {
  const activeFindings = reconciliation.activeFindings || []
  const reconciledFindings = reconciliation.reconciledFindings || []
  return {
    status: reconciliation.status || '',
    summary: reconciliation.summary || {},
    activeFindingIds: activeFindings.map(finding => finding.id).filter(Boolean),
    activeFindingCountsByProposedCard: countFindingsByProposedCard(activeFindings),
    reconciledFindings: reconciledFindings.map(compactReconciledFindingForNightlyJson),
  }
}

export function serializeNightlyDeepAuditUpgradeJson(audit = {}) {
  const activeFindings = audit.deterministicAudit?.findings || []
  const rawFindings = audit.rawDeterministicAudit?.findings || audit.deterministicAudit?.rawFindings || []
  const reconciliation = audit.deterministicFindingReconciliation || audit.deterministicAudit?.findingReconciliation || null
  return {
    ...
    findings: activeFindings.map(compactFindingForNightlyJson),
    rawFindingIds: rawFindings.map(finding => finding.id).filter(Boolean),
    rawFindingCountsByProposedCard: countFindingsByProposedCard(rawFindings),
    deterministicFindingReconciliation: reconciliation
      ? compactDeterministicFindingReconciliationForNightlyJson(reconciliation)
      : null,
