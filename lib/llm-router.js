...
export const DEFAULT_LLM_ROUTES = [
  ...
  },
  {
    ...
    priority: 10,
    provider: 'openclaw',
    model: process.env.LLM_OPENCLAW_EXTRACTION_MODEL || 'openai-codex/gpt-5.4',
    authPath: LLM_AUTH_PATHS.CHATGPT_SUBSCRIPTION_GATEWAY,
    credentialKey: 'openclaw-chatgpt-pro',
  ...
  },
  {
    ...
    priority: 10,
    provider: 'openclaw',
    model: process.env.LLM_OPENCLAW_SYNTHESIS_MODEL || 'openai-codex/gpt-5.4',
    authPath: LLM_AUTH_PATHS.CHATGPT_SUBSCRIPTION_GATEWAY,
    credentialKey: 'openclaw-chatgpt-pro',
...
async function callOpenClawModelRun({ model, prompt, timeoutMs = 240000 } = {}) {
...
async function callOpenAiResponsesApi({ model, messages, inputText, responseFormat, maxOutputTokens = 3000 } = {}) {
...
export async function callLlm({
  ...
  const call = await createLlmCall({
    ...
    metadata: {
      ...
      reason: dryRun
        ...
        : 'Router provider adapter selected by policy-aware route.',
    },
  }, 'llm-router')

  if (dryRun) {
    return { plan, call, outputText: '', model: plan.selectedRoute.model }
  }
  ...
  try {
    ...
    return { plan, call: finishedCall, outputText: result.outputText, raw: result.raw, model }
  } catch (error) {
    await finishLlmCall(call.callId, {
      status: 'failed',
