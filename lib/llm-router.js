...
const DEFAULT_GEMINI_VIDEO_MODEL = process.env.LLM_GEMINI_VIDEO_MODEL || 'gemini-2.5-flash'
...
export const DEFAULT_LLM_CREDENTIALS = [
  ...
  },
  {
    ...
    notes: OPENCLAW_EXTRACTION_ALLOWED || OPENCLAW_SYNTHESIS_ALLOWED
      ? 'OpenClaw is adapter-only. Gateway/subscription route allowed for bounded internal workloads after local operator opt-in and model probe, with non-OpenClaw fallback truth.'
      : 'OpenClaw is adapter-only. Gateway/subscription route is experimental until capability and policy probes pass; it must not become the Foundation architecture owner.',
  },
  {
  ...
  },
  {
    ...
    hubKey: 'foundation',
    workloadLane: 'vision',
    secretRef: 'GEMINI_API_KEY or GOOGLE_API_KEY',
    status: 'unknown',
    policyClassification: 'api_fallback',
...
export const DEFAULT_LLM_ROUTES = [
  ...
  },
  {
    ...
    metadata: buildOpenClawAdapterMetadata({
      ...
      fallbackRouteKey: 'foundation-extraction-openai-api',
      authPosture: OPENCLAW_EXTRACTION_ALLOWED ? 'available' : 'probe_required',
    }),
    notes: OPENCLAW_EXTRACTION_ALLOWED
      ? 'OpenClaw is adapter-only. Subscription route for bounded extraction after local operator opt-in and model probe; non-OpenClaw fallback truth remains required.'
      : 'OpenClaw is adapter-only. Candidate subscription route for high-volume extraction; do not schedule until probes classify it and never make it the Foundation architecture owner.',
  },
  {
  ...
  },
  {
    ...
    notes: OPENCLAW_SYNTHESIS_ALLOWED
      ? 'OpenClaw is adapter-only. Subscription route for bounded internal synthesis after local operator opt-in and model probe; non-OpenClaw fallback truth remains required.'
      : 'OpenClaw is adapter-only. Candidate subscription route for shared-comms synthesis; requires explicit synthesis opt-in before real workloads and must not own Foundation architecture.',
  },
  {
  ...
  },
  {
    ...
    notes: OPENCLAW_SYNTHESIS_ALLOWED
      ? 'OpenClaw is adapter-only. Subscription route for bounded internal deep-audit senior review after local operator opt-in and model probe; non-OpenClaw fallback truth remains required.'
      : 'OpenClaw is adapter-only. Candidate subscription route for deep-audit senior review; requires explicit synthesis/deep-audit opt-in before real workloads and must not own Foundation architecture.',
  },
  {
  ...
  },
  {
    ...
    priority: 90,
    provider: 'openai',
    model: process.env.LLM_DEEP_AUDIT_MODEL || process.env.LLM_SYNTHESIS_MODEL || DEFAULT_CODEX_DIRECT_MODEL,
    authPath: LLM_AUTH_PATHS.API_DIRECT,
    credentialKey: 'openai-api-default',
    fallbackRouteKey: null,
    status: 'probe_required',
