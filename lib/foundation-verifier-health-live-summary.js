...
import {
  BUILD_LANE_REPEATED_FAILURE_ACTION_GATE_CARD_ID,
  BUILD_LANE_REPEATED_FAILURE_ACTION_GATE_CLOSEOUT_KEY,
  BUILD_LANE_REPEATED_FAILURE_ACTION_GATE_SCRIPT_PATH,
  buildBuildLaneRepeatedFailureActionGateDogfoodProof,
} from './build-lane-repeated-failure-action-gate.js'
import {
  FOUNDATION_HEALTH_WATCH_TO_GREEN_APPROVAL_PATH,
  FOUNDATION_HEALTH_WATCH_TO_GREEN_CARD_ID,
  FOUNDATION_HEALTH_WATCH_TO_GREEN_CLOSEOUT_KEY,
...
} from './foundation-health-watch-to-green.js'
import {
  AUDIT_FINDING_TO_BACKLOG_ROUTER_APPROVAL_PATH,
  AUDIT_FINDING_TO_BACKLOG_ROUTER_CARD_ID,
  AUDIT_FINDING_TO_BACKLOG_ROUTER_CLOSEOUT_KEY,
...
} from './audit-finding-to-backlog-router.js'
...
export const VERIFIER_HEALTH_LIVE_SUMMARY_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-health-live-summary-split-closeout.md'
...
export async function evaluateFoundationVerifierHealthLiveSummary(input = {}) {
  ...
  const hardcodedKpiHealthYearPattern = new RegExp([
    ...
    kpiHealthDynamicYearContractCard &&
      ...
      (!kpiHealthDynamicYearContractClosed || (
        ...
        await repoFileExists('docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-16-kpi-health-dynamic-year-contract-closeout.md')
      )),
  ...
  const buildLaneRepeatedFailureActionGateCard = (foundationHub.backlogItems || []).find(item => item.id === BUILD_LANE_REPEATED_FAILURE_ACTION_GATE_CARD_ID) || null
  const foundationHealthWatchToGreenCard = (foundationHub.backlogItems || []).find(item => item.id === FOUNDATION_HEALTH_WATCH_TO_GREEN_CARD_ID) || null
  const auditFindingToBacklogRouterCard = (foundationHub.backlogItems || []).find(item => item.id === AUDIT_FINDING_TO_BACKLOG_ROUTER_CARD_ID) || null
  ...
  const buildLaneRepeatedFailureActionGateCloseout = foundationBuildCloseouts.find(closeout => closeout.key === BUILD_LANE_REPEATED_FAILURE_ACTION_GATE_CLOSEOUT_KEY) || null
  const foundationHealthWatchToGreenCloseout = foundationBuildCloseouts.find(closeout => closeout.key === FOUNDATION_HEALTH_WATCH_TO_GREEN_CLOSEOUT_KEY) || null
  const auditFindingToBacklogRouterCloseout = foundationBuildCloseouts.find(closeout => closeout.key === AUDIT_FINDING_TO_BACKLOG_ROUTER_CLOSEOUT_KEY) || null
  ...
  const buildLaneRepeatedFailureActionGateClosed = buildLaneRepeatedFailureActionGateCard?.lane === 'done'
  const foundationHealthWatchToGreenClosed = foundationHealthWatchToGreenCard?.lane === 'done'
  const auditFindingToBacklogRouterClosed = auditFindingToBacklogRouterCard?.lane === 'done'
  ...
  const buildLaneRepeatedFailureActionGateDogfood = buildBuildLaneRepeatedFailureActionGateDogfoodProof()
  const foundationHealthWatchToGreenDogfood = buildFoundationHealthWatchToGreenDogfoodProof()
  const auditFindingToBacklogRouterDogfood = buildAuditFindingToBacklogRouterDogfoodProof()
  ...
  const systemHealthNightlyRuntime = getFoundationJobRuntime(systemHealthNightlyJob || {}, {
    ...
    systemHealthNightlyAuditCard &&
      ...
      buildLaneRepeatedFailureActionGateDogfood.ok === true &&
      ...
      packageJson.scripts?.['process:build-lane-repeated-failure-action-gate-check'] === `node --env-file-if-exists=.env ${BUILD_LANE_REPEATED_FAILURE_ACTION_GATE_SCRIPT_PATH}` &&
      packageJson.scripts?.['process:foundation-health-watch-to-green-check'] === `node --env-file-if-exists=.env ${FOUNDATION_HEALTH_WATCH_TO_GREEN_SCRIPT_PATH}` &&
      packageJson.scripts?.['process:audit-finding-to-backlog-router-check'] === `node --env-file-if-exists=.env ${AUDIT_FINDING_TO_BACKLOG_ROUTER_SCRIPT_PATH}` &&
      ...
      foundationSystemHealthSource.includes('classifyFoundationSystemHealthFindings') &&
      foundationSystemHealthSource.includes('unclassifiedRiskCount') &&
      foundationRuntimeRenderersSource.includes('Build lane failures') &&
      buildLaneRepeatedFailureActionGateCard &&
      ['scoped', 'executing', 'done'].includes(buildLaneRepeatedFailureActionGateCard.lane) &&
      foundationHealthWatchToGreenCard &&
      ['scoped', 'executing', 'done'].includes(foundationHealthWatchToGreenCard.lane) &&
      auditFindingToBacklogRouterCard &&
      ['scoped', 'executing', 'done'].includes(auditFindingToBacklogRouterCard.lane) &&
      foundationHealthWatchToGreenDogfood.ok === true &&
      auditFindingToBacklogRouterDogfood.ok === true &&
      await repoFileExists(FOUNDATION_HEALTH_WATCH_TO_GREEN_PLAN_PATH) &&
      await repoFileExists(FOUNDATION_HEALTH_WATCH_TO_GREEN_APPROVAL_PATH) &&
      await repoFileExists(AUDIT_FINDING_TO_BACKLOG_ROUTER_PLAN_PATH) &&
      await repoFileExists(AUDIT_FINDING_TO_BACKLOG_ROUTER_APPROVAL_PATH) &&
      await repoFileExists('docs/_archive/handoffs/2026-05-30-doc-archive-move/system-health-2026-05-16.md') &&
      ...
      (!systemHealthNightlyAuditClosed || (
        ...
        await repoFileExists('docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-16-system-health-nightly-audit-closeout.md')
      )) &&
      (!scheduledJobStalenessDashboardClosed || (
        ...
        await repoFileExists('docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-16-scheduled-job-staleness-dashboard-closeout.md')
      )) &&
      ...
      (!buildLaneRepeatedFailureActionGateClosed || (
        String(buildLaneRepeatedFailureActionGateCard.statusNote || '').includes(BUILD_LANE_REPEATED_FAILURE_ACTION_GATE_CLOSEOUT_KEY) &&
        buildLaneRepeatedFailureActionGateCloseout?.operatorCloseout === true &&
        (buildLaneRepeatedFailureActionGateCloseout.backlogIds || []).includes(BUILD_LANE_REPEATED_FAILURE_ACTION_GATE_CARD_ID)
      )) &&
      (!foundationHealthWatchToGreenClosed || (
        String(foundationHealthWatchToGreenCard.statusNote || '').includes(FOUNDATION_HEALTH_WATCH_TO_GREEN_CLOSEOUT_KEY) &&
        foundationHealthWatchToGreenCloseout?.operatorCloseout === true &&
        (foundationHealthWatchToGreenCloseout.backlogIds || []).includes(FOUNDATION_HEALTH_WATCH_TO_GREEN_CARD_ID) &&
        await repoFileExists(FOUNDATION_HEALTH_WATCH_TO_GREEN_CLOSEOUT_PATH)
      )) &&
      (!auditFindingToBacklogRouterClosed || (
        String(auditFindingToBacklogRouterCard.statusNote || '').includes(AUDIT_FINDING_TO_BACKLOG_ROUTER_CLOSEOUT_KEY) &&
        auditFindingToBacklogRouterCloseout?.operatorCloseout === true &&
        (auditFindingToBacklogRouterCloseout.backlogIds || []).includes(AUDIT_FINDING_TO_BACKLOG_ROUTER_CARD_ID) &&
        await repoFileExists(AUDIT_FINDING_TO_BACKLOG_ROUTER_CLOSEOUT_PATH)
      )),
    'SYSTEM-HEALTH-NIGHTLY-AUDIT-001, SCHEDULED-JOB-STALENESS-DASHBOARD-001, BUILD-LANE-FAILURE-TELEMETRY-001, BUILD-LANE-REPEATED-FAILURE-ACTION-GATE-001, FOUNDATION-HEALTH-WATCH-TO-GREEN-001, and AUDIT-FINDING-TO-BACKLOG-ROUTER-001 surface and route hidden job/source/system/build-lane/audit staleness',
    systemHealthNightlyAuditCard && scheduledJobStalenessDashboardCard
      ? `lanes=${systemHealthNightlyAuditCard.lane}/${scheduledJobStalenessDashboardCard.lane}/${buildLaneFailureTelemetryCard?.lane || 'missing'}/${buildLaneRepeatedFailureActionGateCard?.lane || 'missing'}/${foundationHealthWatchToGreenCard?.lane || 'missing'}/${auditFindingToBacklogRouterCard?.lane || 'missing'} job=${systemHealthNightlyJob?.runtimeMode || 'missing'}/${systemHealthNightlyRuntime.scheduleStatus || 'missing'} dogfood=${systemHealthNightlyAuditDogfood.ok ? 'pass' : 'blocked'} telemetry=${buildLaneFailureTelemetryDogfood.ok ? 'pass' : 'blocked'} actionGate=${buildLaneRepeatedFailureActionGateDogfood.ok ? 'pass' : 'blocked'} auditRouter=${auditFindingToBacklogRouterDogfood.ok ? 'pass' : 'blocked'}`
      : 'missing system-health visibility cards',
  )

  const googleHealthResult = await runHealthScriptSafe('google:health')
  let googleServiceAccountHealthResult = { ok: false, output: '' }
  if (googleHealthResult.ok !== true) {
    try {
      googleServiceAccountHealthResult = {
        ok: true,
        output: await runHealthScriptWithArgs('google:health', ['--user=service-account']),
      }
    } catch (error) {
      googleServiceAccountHealthResult = {
        ok: false,
