...
export const RECENT_BUILDS_UI_BASELINE_PATH = 'docs/_archive/audits/2026-04-30-recent-builds-billion-dollar-ui-baseline.md'
export const RECENT_BUILDS_UI_MANUAL_REVIEW_PATH = 'docs/_archive/audits/2026-04-30-recent-builds-billion-dollar-ui-manual-review.md'
...
function findCloseout(builds = [], closeoutKey) {
  return builds.find(build => build.closeoutKey === closeoutKey || build.key === closeoutKey) || null
}
...
export async function buildRecentBuildsBillionDollarUiStatus({
  ...
  const gate003 = findCloseout(builds, 'gate-reliability-direct-verifier-deadlock-v1')
    || findCloseout(foundationBuildCloseouts, 'gate-reliability-direct-verifier-deadlock-v1')
  const phase1 = findCloseout(builds, 'phase-1-enforcement-v1')
    || foundationBuildCloseouts.find(closeout => closeout.key === 'phase-1-enforcement-v1')
    || null
  const thisCloseout = findCloseout(builds, RECENT_BUILDS_UI_CLOSEOUT_KEY)
    || findCloseout(foundationBuildCloseouts, RECENT_BUILDS_UI_CLOSEOUT_KEY)
