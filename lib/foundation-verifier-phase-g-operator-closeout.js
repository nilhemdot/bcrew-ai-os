...
function doneCardHasCloseout(card, closeoutKey, buildLogCloseout) {
  return card?.lane === 'done' &&
    (
      String(card?.statusNote || '').includes(closeoutKey) ||
      buildLogCloseout?.operatorCloseout === true
    )
}
...
export function evaluateFoundationVerifierPhaseGOperatorCloseout(input = {}) {
  ...
  const uiMenuLayoutPolishBuildLogExact = buildLogUiMenuLayoutPolishBuild?.backlogIds?.length === 1 &&
    ...
    checks,
    doneCardHasCloseout(uiMenuLayoutPolish, 'ui-menu-layout-polish-v1', buildLogUiMenuLayoutPolishBuild) &&
  ...
  const recentBuildsUiBuildLogExact = buildLogRecentBuildsUiBuild?.backlogIds?.length === 1 &&
    ...
    checks,
    doneCardHasCloseout(recentBuildsUi, 'recent-builds-billion-dollar-ui-v1', buildLogRecentBuildsUiBuild) &&
