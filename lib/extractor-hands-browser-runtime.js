...
function classifyUrlRisk(url = '') {
  ...
  if (/form|optin|subscribe|book|calendly/i.test(`${host} ${value}`)) return 'form_or_booking'
...
async function runLivePlaywrightHands(request = {}) {
  ...
  try {
    ...
    const matched = await page.$eval(policy.selector, element => ({
    ...
    await Promise.all([
      ...
      page.locator(policy.selector).first().click({ timeout: 15000 }),
    ])
