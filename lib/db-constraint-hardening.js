...
export const DB_CONSTRAINT_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-16-db-constraint-closeout.md'
...
function createFakeDecisionApplyClient() {
  const decisions = new Map([
    ['DEC-NEW', {
      id: 'DEC-NEW',
      status: 'proposed',
      source_ref: 'dogfood-source',
      decision_owner: 'Steve',
      confirmed_by: 'Steve',
      participant_names: ['Steve', 'Codex'],
      context_ref: 'dogfood-context',
      evidence_notes: 'Dogfood replacement decision.',
      provenance_type: 'direct',
      provenance_status: 'strong',
      source_ids: ['SRC-DOGFOOD-001'],
      route_refs: [],
      artifact_refs: ['artifact:dogfood'],
      meeting_ref: null,
      session_ref: 'session:dogfood',
      thread_ref: null,
      participant_roles: { decisionOwner: 'Steve', confirmedBy: 'Steve' },
      supersedes_ids: ['DEC-OLD', 'DEC-OLD', 'DEC-NEW', 'DEC-MISSING'],
    }],
  ...
  const client = {
    async query(sql, params = []) {
      ...
      if (/SELECT\s+\*\s+FROM\s+decisions/i.test(text) && /FOR\s+UPDATE/i.test(text)) {
