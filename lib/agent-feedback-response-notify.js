...
export async function buildAgentFeedbackResponseNotifyStatus({
  ...
  const closeout = (foundationBuildLog?.builds || foundationBuildLog?.closeouts || []).find(build =>
    build.closeoutKey === AGENT_FEEDBACK_RESPONSE_NOTIFY_CLOSEOUT_KEY || build.key === AGENT_FEEDBACK_RESPONSE_NOTIFY_CLOSEOUT_KEY
  ) || null
  ...
  addFinding(findings, verifierSource.includes(AGENT_FEEDBACK_RESPONSE_NOTIFY_CARD_ID), 'foundation verifier covers response notify card', AGENT_FEEDBACK_RESPONSE_NOTIFY_CARD_ID)
  ...
  ]), 'submit path calls response notification after DB save and ClickUp writeback attempt', '/api/agent-feedback/submit')
  ...
  addFinding(findings, !hasPrivateProofLeak(buildLogSource), 'build log source has no raw emails, token URLs, or feedback text for response notify closeout', 'metadata-only')
  addFinding(findings, notifyCard?.lane === 'done' && /agent-feedback-response-notify-v1/.test(notifyCard?.statusNote || ''), 'response notify card is done', notifyCard?.lane || 'missing')
  ...
  addFinding(findings, currentPlan.includes(AGENT_FEEDBACK_RESPONSE_NOTIFY_CARD_ID) && currentPlan.includes('Do not send Georgia onboarding survey'), 'current plan records response-notify boundary', 'current-plan')
  addFinding(findings, currentState.includes(AGENT_FEEDBACK_RESPONSE_NOTIFY_CARD_ID) && currentState.includes('Georgia live-send remains paused'), 'current state records response-notify boundary', 'current-state')
