...
function buildPermissionSummary(classifications) {
  return {
    ...
    crewbertCount: classifications.filter(item => item.category === 'crewbert' && item.allowed).length,
...
function buildPreflightFromPermissions({
  ...
  const crewbertPermissions = classifications.filter(classification => classification.category === 'crewbert' && classification.allowed)
  ...
  const missingCrewbert = crewbertPermissions.length === 0
