...
export const SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT = 'steve.zahnd@bensoncrew.ca'
export const SOURCE_SESSION_BROKER_MYICOR_GOOGLE_SSO_SOURCE = 'myicor-google-sso'
export const SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD = 'google_oauth_sign_in'
...
export function buildSourceSessionSecretRef({
  ...
  const credentialSource = resolveSourceSessionCredentialSource({ sourceFamily, source })
  if (!text(credentialSource) || !text(account)) return ''
  return buildKeychainSecretRef({ source: credentialSource, account })
}

export function resolveSourceSessionCredentialSource({
  sourceFamily = '',
  source = '',
} = {}) {
  const resolvedSource = text(source)
  if (sourceFamily === 'paid_course_training_platforms' && /myicor/i.test(resolvedSource)) {
    return SOURCE_SESSION_BROKER_MYICOR_GOOGLE_SSO_SOURCE
  }
...
export function buildSourceSessionBrokerContractSnapshot() {
  return {
    ...
    myicorSpecificDiscovery: {
      ...
      paidGoogleAccount: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
      googleSsoCredentialSource: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_SSO_SOURCE,
      ...
      wrongBranchRule: 'If the browser lands on Start Free, create profile, onboarding, new account, or signup, stop immediately, record wrong_signup_branch, clear/restart the isolated profile if needed, and return to the Log in with Google path.',
      mfaRule: 'If Google SSO asks for passkey, number matching, phone approval, authenticator, or another human verification, emit auth_needed through Harlan with the visible challenge and wait for DONE before reverify/resume.',
...
export function evaluateSourceSessionBrokerRequest(request = {}) {
  ...
  const signupSurfaceDetected = request.signupSurfaceDetected === true ||
    request.profileSetupDetected === true ||
    request.onboardingSurfaceDetected === true ||
    request.newAccountSurfaceDetected === true
  const authMethod = text(request.authMethod || request.accountAuthMethod)
  const isMyicorPaidCourse = sourceFamily === 'paid_course_training_platforms' && /myicor/i.test(source)
  ...
  const secretRef = buildSourceSessionSecretRef({ sourceFamily, source, account })
  ...
  if (isMyicorPaidCourse && (signupSurfaceDetected || requiresAccountCreation)) {
    return {
      ...base,
      ok: false,
      status: 'blocked',
      reason: 'myicor_wrong_signup_branch_existing_google_sso_required',
      authMethodRequired: SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD,
      expectedAccount: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
      nextAction: 'Stop this run. MyICOR paid access is an existing Google SSO account, not Start Free, signup, onboarding, or profile creation. Restart from Sign in / Log in with Google.',
    }
  }
  if (isMyicorPaidCourse && authMethod && authMethod !== SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD) {
    return {
      ...base,
      ok: false,
      status: 'blocked',
      reason: 'myicor_google_sso_required',
      authMethodRequired: SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD,
      expectedAccount: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
      nextAction: 'Use the existing Google SSO login route for MyICOR. Do not attempt a MyICOR password path unless Steve later confirms one exists.',
    }
  }
  ...
  if (isMyicorPaidCourse) {
    if (!loginRecipePresent) {
      return {
        ...base,
        ok: false,
        status: 'auth_needed',
        reason: 'myicor_google_sso_recipe_missing',
        authMethodRequired: SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD,
        expectedAccount: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
        authEvent: buildSourceSessionBrokerAuthNeededEvent({
          sourceSystem: source,
          accountLabel: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
          blocker: 'MyICOR requires a source-specific existing-account Google SSO login recipe.',
        }),
        nextAction: 'Add the MyICOR Google SSO login recipe: home/login page, choose Log in with Google, never Start Free/signup/profile creation, then reverify.',
      }
    }
    if (mfaChallenge) {
      return {
        ...
        ok: false,
        status: 'auth_needed',
        reason: 'myicor_google_sso_mfa_or_human_verification_required',
        authMethodRequired: SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD,
        expectedAccount: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
        authEvent: buildSourceSessionBrokerAuthNeededEvent({
          sourceSystem: source,
          accountLabel: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
          blocker: 'Google SSO MFA or human verification is required for MyICOR.',
        }),
        nextAction: 'Notify Steve through Harlan with the visible Google challenge, wait for DONE, silently reverify, then resume or fail closed.',
      }
    }
    if (!keychainPresent) {
      return {
        ...
        ok: false,
        status: 'auth_needed',
        reason: 'myicor_google_sso_session_or_keychain_missing',
        authMethodRequired: SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD,
        expectedAccount: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
        authEvent: buildSourceSessionBrokerAuthNeededEvent({
          sourceSystem: source,
          accountLabel: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
          blocker: 'Existing Google SSO session or approved Google credential path is missing for MyICOR.',
        }),
        nextAction: 'Authorize the existing Google SSO account in an isolated profile or approved connector flow. Do not create a new MyICOR profile.',
      }
    }
    return {
      ...base,
      ok: true,
      status: 'google_sso_login_recipe_ready',
      reason: 'myicor_existing_google_sso_path_available',
      authMethodRequired: SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD,
      expectedAccount: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
      nextAction: 'Attempt MyICOR through existing-account Google SSO inside the isolated profile; abort on signup/profile creation and emit auth_needed on MFA.',
    }
  }
  if (!keychainPresent) {
    return {
...
export function buildSourceSessionBrokerDogfoodProof() {
  ...
  const myicorWrongSignupBranch = evaluateSourceSessionBrokerRequest({
    sourceFamily: 'paid_course_training_platforms',
    source: 'myicor',
    account: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
    action: 'read_approved_course_area_after_packet',
  ...
  const myicorWrongAuthMethod = evaluateSourceSessionBrokerRequest({
    sourceFamily: 'paid_course_training_platforms',
    source: 'myicor',
    account: SOURCE_SESSION_BROKER_MYICOR_GOOGLE_ACCOUNT,
    action: 'read_approved_course_area_after_packet',
  ...
  const combined = JSON.stringify({
    ...
    mfaChallenge,
    myicorWrongSignupBranch,
    myicorWrongAuthMethod,
  ...
  const cases = [
    ...
    { name: 'myicor_uses_existing_google_sso_not_password_or_signup', ok: myicorLoginRecipe.status === 'google_sso_login_recipe_ready' && myicorLoginRecipe.authMethodRequired === SOURCE_SESSION_BROKER_MYICOR_AUTH_METHOD },
    { name: 'missing_google_sso_session_emits_auth_needed', ok: missingMyicorCredential.status === 'auth_needed' && missingMyicorCredential.reason === 'myicor_google_sso_session_or_keychain_missing' && missingMyicorCredential.authEvent?.eventType === 'auth_needed' },
    { name: 'myicor_google_sso_mfa_emits_auth_needed_instead_of_success', ok: mfaChallenge.status === 'auth_needed' && mfaChallenge.reason === 'myicor_google_sso_mfa_or_human_verification_required' },
    { name: 'myicor_signup_or_profile_creation_branch_fails_closed', ok: myicorWrongSignupBranch.status === 'blocked' && myicorWrongSignupBranch.reason === 'myicor_wrong_signup_branch_existing_google_sso_required' },
    { name: 'myicor_password_login_branch_fails_closed', ok: myicorWrongAuthMethod.status === 'blocked' && myicorWrongAuthMethod.reason === 'myicor_google_sso_required' },
