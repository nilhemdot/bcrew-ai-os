...
export const FOUNDATION_SPRINT_EXIT_CRITERIA = [
  ...
  'After this card ships, return to MEETING-VAULT-ACL-001 under the Sprint Ready rules.',
]
  ...
  return {
    sprint: {
      ...
      metadata: {
        ...
        currentStatus: foundationSprintCadenceDone ? 'returned_to_meeting_acl' : 'building_sprint_command_view',
        nextAction: foundationSprintCadenceDone
          ? 'Return to MEETING-VAULT-ACL-001 under the Sprint Ready rules. No Phase B mutation is approved.'
...
export function buildFoundationCurrentSprintStatus({
  ...
  const nextAction = nextItem
    ? (nextItem.stage === 'returned' ? nextItem.returnedReason : nextItem.backlogNextAction) ||
