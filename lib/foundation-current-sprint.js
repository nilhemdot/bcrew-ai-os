...
export const FOUNDATION_CURRENT_SPRINT_GOAL = 'Convert Foundation READY from process proof into behavior proof, then ship one owner-only Strategy operator loop.'
...
export const FOUNDATION_SPRINT_EXIT_CRITERIA = [
  ...
  'Plan Critic replacement is planned or built before the verifier behavior sweep can close.',
...
export function buildDefaultFoundationSprintSeed({ stage = 'building_now' } = {}) {
  const reconcileStage = stageKeys.has(stage) ? stage : 'building_now'
  const readyCheck = {
    existingCode: [
    ...
    exactGap: 'Foundation READY now passes, but audits found command-truth drift, behavior-proof gaps, and old-system capability gaps that must be reconciled before product work resumes.',
  ...
  return {
    sprint: {
      ...
      activeBlockerCardId: REBUILD_PLAN_RECONCILE_CARD_ID,
      metadata: {
        overlayOnly: true,
        noSecondBacklog: true,
        sprintCommandView: true,
        executiveSummary: 'Audit consensus says Foundation is real but drifting toward process proof over product behavior. This sprint locks one command truth, restores Plan Critic pressure, hardens security/verifier behavior proof, and then proves the spine through one owner-only Strategy meeting workflow.',
        currentStatus: 'reconciling_command_truth',
        nextAction: 'Finish REBUILD-PLAN-RECONCILE-001, then pull PLAN-CRITIC-REPLACEMENT-001 before closing the verifier behavior sweep.',
    ...
    items: [
      {
        ...
        planRef: 'docs/process/rebuild-plan-reconcile-001-plan.md',
      ...
      },
      {
        ...
        order: 2,
        stage: 'scoping',
        planRef: 'docs/process/plan-critic-replacement-001-plan.md',
      ...
      },
      {
        ...
        order: 3,
        stage: 'scoping',
        planRef: 'docs/process/security-behavior-proof-001-plan.md',
