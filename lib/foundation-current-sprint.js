...
import {
  ...
  DECISION_RESTRICTED_QUEUE_CARD_ID as DECISION_RESTRICTED_QUEUE_CARD_ID_VALUE,
...
import {
  ...
  FOUNDATION_UI_COMPLETE_CARD_ID as FOUNDATION_UI_COMPLETE_CARD_ID_VALUE,
...
export const CURRENT_SPRINT_DYNAMIC_TRUTH_CARD_ID = 'CURRENT-SPRINT-DYNAMIC-TRUTH-001'
export const CURRENT_SPRINT_DYNAMIC_TRUTH_CLOSEOUT_KEY = 'current-sprint-dynamic-truth-v1'
export const CURRENT_SPRINT_DYNAMIC_TRUTH_PLAN_PATH = 'docs/process/current-sprint-dynamic-truth-001-plan.md'
export const CURRENT_SPRINT_DYNAMIC_TRUTH_APPROVAL_PATH = 'docs/process/approvals/CURRENT-SPRINT-DYNAMIC-TRUTH-001.json'
export const CURRENT_SPRINT_DYNAMIC_TRUTH_SCRIPT_PATH = 'scripts/process-current-sprint-dynamic-truth-check.mjs'
export const SPRINT_STAGE_GATE_CARD_ID = 'SPRINT-STAGE-GATE-001'
export const SPRINT_STAGE_GATE_CLOSEOUT_KEY = 'sprint-stage-gate-v1'
export const SPRINT_STAGE_GATE_PLAN_PATH = 'docs/process/sprint-stage-gate-001-plan.md'
export const SPRINT_STAGE_GATE_APPROVAL_PATH = 'docs/process/approvals/SPRINT-STAGE-GATE-001.json'
export const SPRINT_STAGE_GATE_SCRIPT_PATH = 'scripts/process-sprint-stage-gate-check.mjs'
export const FOUNDATION_PLAN_RECONCILE_CARD_ID = 'FOUNDATION-PLAN-RECONCILE-001'
export const FOUNDATION_PLAN_RECONCILE_CLOSEOUT_KEY = 'foundation-plan-reconcile-control-plane-v1'
export const FOUNDATION_PLAN_RECONCILE_PLAN_PATH = 'docs/process/foundation-plan-reconcile-001-plan.md'
export const FOUNDATION_PLAN_RECONCILE_APPROVAL_PATH = 'docs/process/approvals/FOUNDATION-PLAN-RECONCILE-001.json'
export const FOUNDATION_PLAN_RECONCILE_SCRIPT_PATH = 'scripts/process-foundation-plan-reconcile-check.mjs'
...
// liveTruthPosture: historical_closeout_only - preserved only to verify the closed Source Once-Over sprint.
...
export const DECISION_RESTRICTED_QUEUE_CARD_ID = DECISION_RESTRICTED_QUEUE_CARD_ID_VALUE
export const FOUNDATION_UI_COMPLETE_CARD_ID = FOUNDATION_UI_COMPLETE_CARD_ID_VALUE
...
function normalizePlanCriticRun(run = {}) {
  const score = Number(run.score)
  const passThreshold = Number(run.passThreshold ?? run.pass_threshold ?? 9.8)
  return {
    cardId: normalizeText(run.cardId || run.card_id),
    status: normalizeText(run.status),
    score: Number.isFinite(score) ? score : 0,
    passThreshold: Number.isFinite(passThreshold) ? passThreshold : 9.8,
    runId: normalizeText(run.runId || run.run_id),
    createdAt: run.createdAt || run.created_at || null,
  }
}

function buildPlanCriticPassMap(planCriticRuns = []) {
  const map = new Map()
  for (const rawRun of Array.isArray(planCriticRuns) ? planCriticRuns : []) {
    const run = normalizePlanCriticRun(rawRun)
    if (!run.cardId) continue
    if (run.status === 'pass' && run.score >= run.passThreshold) {
      if (!map.has(run.cardId)) map.set(run.cardId, [])
      map.get(run.cardId).push(run)
    }
  }
  return map
}

function hasAfterActionRepair(item = {}) {
  const metadata = item.metadata && typeof item.metadata === 'object' ? item.metadata : {}
  return metadata.afterActionRepair === true || metadata.after_action_repair === true
}

export function validateFoundationSprintStageGate({
  sprint = null,
  items = [],
  backlogItems = [],
  closeouts = [],
  planCriticRuns = null,
  requirePlanCritic = Array.isArray(planCriticRuns),
} = {}) {
  const normalizedItems = (Array.isArray(items) ? items : []).map(normalizeFoundationSprintItem)
  const backlogMap = buildBacklogMap(backlogItems)
  const closeoutMap = buildCloseoutMap(closeouts)
  const planCriticPassMap = buildPlanCriticPassMap(planCriticRuns || [])
  const findings = []
  ...
  for (const item of normalizedItems) {
    ...
    const isReadyOrLater = ['sprint_ready', 'building_now', 'done_this_sprint'].includes(item.stage)
    if (isReadyOrLater && !readiness.ok) {
      findings.push(finding('stage_gate_existing_work_check_complete', `${item.cardId || 'missing'} cannot be ${getFoundationSprintStageLabel(item.stage)} with missing doctrine fields: ${readiness.missingFields.join(', ')}.`))
    }
    const planCriticRunsForCard = planCriticPassMap.get(item.cardId) || []
    const canUseAfterActionRepair = item.stage === 'done_this_sprint' && hasAfterActionRepair(item)
    if (requirePlanCritic && isReadyOrLater && !canUseAfterActionRepair && !planCriticRunsForCard.length) {
      findings.push(finding('stage_gate_plan_critic_pass_required', `${item.cardId || 'missing'} cannot be ${getFoundationSprintStageLabel(item.stage)} without a Plan Critic pass row.`))
    }
  ...
  const activeBlockerItem = normalizedItems.find(item => item.cardId === activeBlockerCardId) || null
  if (sprint?.status === 'active' && activeBlockerCardId && activeBlockerItem?.stage === 'done_this_sprint') {
    findings.push(finding('active_blocker_not_done_this_sprint', `${activeBlockerCardId} is active blocker but already Done This Sprint.`))
  }
...
export function buildDefaultFoundationSprintSeed({ stage = 'done_this_sprint' } = {}) {
  ...
  const cadenceReadyCheck = {
    ...
    existingDocs: [
      ...
      'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-10-foundation-sprint-capture.md',
      'docs/rebuild/current-plan.md',
      'docs/rebuild/current-state.md',
    ],
    existingScripts: [
...
export function buildFoundationSourceOnceOverSprintSeed({
  ...
  decisionRestrictedQueueStage = null,
  foundationUiCompleteStage = null,
} = {}) {
  ...
  const normalizedDecisionRestrictedQueueStage = stageKeys.has(decisionRestrictedQueueStage)
    ? decisionRestrictedQueueStage
    : perUserChangelogDone
      ? 'building_now'
      : 'sprint_ready'
  const decisionRestrictedQueueDone = normalizedDecisionRestrictedQueueStage === 'done_this_sprint'
  const normalizedFoundationUiCompleteStage = stageKeys.has(foundationUiCompleteStage)
    ? foundationUiCompleteStage
    : decisionRestrictedQueueDone
      ? 'building_now'
      : 'sprint_ready'
  const foundationUiCompleteDone = normalizedFoundationUiCompleteStage === 'done_this_sprint'
              ...
              ? brandStackDone
                ? tierBehavioralCompletionDone
                  ? verificationRunsDone
                    ? perUserChangelogDone
                      ? decisionRestrictedQueueDone
                        ? foundationUiCompleteDone
                          ? 'source_once_over_done_review_next'
                          : 'restricted_decisions_done_next_foundation_ui'
        ...
        nextAction: tierBehavioralCompletionDone
          ? verificationRunsDone
            ? perUserChangelogDone
              ? decisionRestrictedQueueDone
                ? foundationUiCompleteDone
                  ? 'Foundation Source Once-Over is done for v1. Run sprint review/rollover before pulling Reply/Watching Loop or product-layer rebuild work.'
                  : 'Pull FOUNDATION-UI-COMPLETE-001 next and make the Foundation depth state visible in one scan.'
    ...
    items: [
      ...
      },
      {
        ...
        metadata: {
          ...
          nextCardId: DECISION_RESTRICTED_QUEUE_CARD_ID,
      ...
      },
      {
        ...
        order: 9,
        stage: normalizedDecisionRestrictedQueueStage,
        planRef: DECISION_RESTRICTED_QUEUE_PLAN_PATH,
        ...
        proofCommands: [
          'npm run process:decision-restricted-queue-check',
          'npm run backlog:hygiene -- --json',
          'npm run foundation:verify',
        ],
      ...
      },
      {
        ...
        order: 10,
        stage: normalizedFoundationUiCompleteStage,
        planRef: FOUNDATION_UI_COMPLETE_PLAN_PATH,
        ...
        proofCommands: [
          'npm run process:foundation-ui-complete-check',
          'npm run backlog:hygiene -- --json',
          'npm run foundation:verify',
        ],
...
export function buildSyntheticFoundationCurrentSprintProof() {
  ...
  seed.sprint = {
    ...seed.sprint,
    activeBlockerCardId: VERIFY_GATE_TIERING_CARD_ID,
  }
  const backlogItems = [
    {
...
export function buildFoundationCurrentSprintStatus({
  ...
  const metadataExitCriteria = normalizeArray(sprintMetadata.exitCriteria)
  ...
  if (sprint && !metadataExitCriteria.length) {
    findings.push(finding('sprint_exit_criteria_required', 'Active sprint must provide exit criteria in live sprint metadata; hardcoded defaults are bootstrap-only.'))
  }

  const allSprintItemsDone = normalizedItems.length > 0 &&
    normalizedItems.every(item => item.stage === 'done_this_sprint')
  if (sprint && !allSprintItemsDone && !normalizeText(sprint.activeBlockerCardId || sprint.active_blocker_card_id)) {
  ...
  const stageGate = validateFoundationSprintStageGate({
    sprint,
  ...
  for (const stageFinding of stageGate.findings) {
    if (stageFinding.check.startsWith('stage_gate_') || stageFinding.check === 'active_blocker_not_done_this_sprint') {
      ...
      : nextItem?.stage === 'returned'
        ...
        : nextItem?.stage === 'sprint_ready'
          ...
          : nextItem?.stage === 'scoping'
            ? 'scoping'
            : 'complete'
  const exitCriteria = metadataExitCriteria.length
    ? metadataExitCriteria
    : sprint
      ? []
      : FOUNDATION_SPRINT_EXIT_CRITERIA
  const defaultNextAction = currentStatus === 'complete'
    ? 'Sprint review/rollover is next.'
    : 'No next card is resolved.'
  const nextAction = nextItem
    ...
    : currentStatus === 'complete'
      ? defaultNextAction
      : normalizeText(sprintMetadata.nextAction) || defaultNextAction
  ...
  const truthSource = {
    sprintRecord: sprint ? 'live-db' : 'missing',
    goal: sprint ? 'live-db' : 'missing',
    executiveSummary: normalizeText(sprintMetadata.executiveSummary) ? 'live-db-metadata' : 'derived-from-live-state',
    exitCriteria: metadataExitCriteria.length ? 'live-db-metadata' : sprint ? 'missing-live-db-metadata' : 'bootstrap-default',
    nextCard: nextItem ? 'live-stage-order' : 'none',
    currentBlocker: activeBlockerCardId ? 'live-sprint-record' : 'none',
    nextAction: nextItem
      ? 'live-sprint-item-or-backlog'
      : currentStatus === 'complete'
        ? 'derived-complete-review'
        : normalizeText(sprintMetadata.nextAction)
          ? 'live-db-metadata'
          : 'derived-empty',
  }
