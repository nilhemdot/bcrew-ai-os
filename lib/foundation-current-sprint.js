...
export const FOUNDATION_CURRENT_SPRINT_GOAL = 'Build the Current Sprint execution-control overlay so Foundation work cannot drift through chat-only sequencing.'
...
export function buildDefaultFoundationSprintSeed({ stage = 'building_now' } = {}) {
  const normalizedStage = stageKeys.has(stage) ? stage : 'building_now'
  const readyCheck = {
    existingCode: [
  ...
  return {
    sprint: {
      ...
      activeBlockerCardId: FOUNDATION_SPRINT_SYSTEM_CARD_ID,
      metadata: {
        overlayOnly: true,
    ...
    items: [
      ...
      },
      {
        ...
        order: 2,
        stage: 'returned',
        planRef: 'docs/process/meeting-vault-acl-001-plan.md',
        definitionOfDone: 'Meeting raw Drive ACL/vault safety closes only when Phase A proves every in-scope file already safe or separately approved Phase B mutates, rechecks, and records rollback proof.',
        proofCommands: [
        ...
        notNextBoundaries: FOUNDATION_SPRINT_NOT_NEXT_BOUNDARIES,
        existingWorkCheck: {},
        returnedReason: 'Phase B approval path paused; do not mutate Drive permissions.',
        metadata: {
          phaseBApproved: false,
          dryRunHash: 'bf950c74c80a1e0f5a2a8848fa2c39e6ecda8d89536770a7b2bf44110a88b8d5',
        },
      },
    ],
  }
