...
export function buildFoundationSourceOnceOverSprintSeed({ sourceMaturityStage = 'building_now' } = {}) {
  ...
  return {
    sprint: {
      ...
      activeBlockerCardId: sourceMaturityDone ? SOURCE_EXTRACTION_COVERAGE_CARD_ID : SOURCE_MATURITY_GRID_CARD_ID,
      metadata: {
        overlayOnly: true,
        noSecondBacklog: true,
        sprintCommandView: true,
        ...
        currentStatus: sourceMaturityDone ? 'source_maturity_grid_done_next_extraction_coverage' : 'source_maturity_grid_building_now',
        nextAction: sourceMaturityDone
    ...
    items: [
      ...
      },
      {
        ...
        order: 2,
        stage: sourceMaturityDone ? 'building_now' : 'sprint_ready',
        planRef: '',
      ...
      {
        cardId: 'VERIFICATION-RUNS-001',
        order: 7,
        stage: 'sprint_ready',
        planRef: '',
        definitionOfDone: 'Stale research/findings verification is scoped or implemented so stale items are refreshed, expired, promoted, killed, or deferred on a schedule.',
        proofCommands: [
          'npm run backlog:hygiene -- --json',
          'npm run foundation:verify',
        ],
      ...
      {
        cardId: 'DECISION-RESTRICTED-QUEUE-001',
        order: 9,
        stage: 'sprint_ready',
        planRef: '',
