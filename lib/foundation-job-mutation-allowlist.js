...
export const FOUNDATION_JOB_MUTATION_ALLOWLIST = Object.freeze({
  ...
  'verification-runs': blockScheduled('mutating', 'process:verification-runs-check closes backlog/current-sprint rows and must remain blocked until split into read-only proof plus explicit apply lane.'),
...
export function buildFoundationJobMutationAllowlistReport({ jobs = [] } = {}) {
  ...
  const ok = scheduledRows.length > 0 &&
    ...
    blockedRows.some(row => row.key === 'verification-runs')
