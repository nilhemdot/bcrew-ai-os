...
export function findGeminiApiCredential(env = process.env) {
  const candidates = [
    { envName: 'GEMINI_API_KEY', value: env.GEMINI_API_KEY },
    { envName: 'GOOGLE_API_KEY', value: env.GOOGLE_API_KEY },
  ]
  const selected = candidates.find(candidate => normalizeText(candidate.value))
  return {
    available: Boolean(selected),
    envName: selected?.envName || null,
    secretRef: selected?.envName || 'GEMINI_API_KEY or GOOGLE_API_KEY',
    accountLabel: selected ? `${selected.envName} Gemini API key` : 'GEMINI_API_KEY or GOOGLE_API_KEY',
...
export function getGeminiCredentialFingerprint(env = process.env) {
  ...
  const refs = ['GEMINI_API_KEY', 'GOOGLE_API_KEY'].map(envName => ({
    envName,
...
export function buildGeminiVideoCredential({
  ...
  return {
    ...
    accountLabel: selectedEnvName ? `${selectedEnvName} Gemini API key` : 'GEMINI_API_KEY or GOOGLE_API_KEY',
    hubKey: 'foundation',
    workloadLane: 'vision',
    secretRef: selectedEnvName || 'GEMINI_API_KEY or GOOGLE_API_KEY',
    status,
...
export async function collectGeminiVideoRouteStatus({
  ...
  if (!credential.available) {
    const error = new Error('Gemini API key is missing; set GEMINI_API_KEY or GOOGLE_API_KEY before live Gemini route probes.')
...
export function buildGeminiVideoRouteProbeInput(proof = {}) {
  ...
  return {
    ...
    capability: {
      ...
      accountLabel: status.accountLabel || proof.routeContract?.accountLabel || 'GEMINI_API_KEY or GOOGLE_API_KEY',
