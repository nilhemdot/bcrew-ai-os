...
export const VERIFIER_SURFACE_TRUST_ORCHESTRATION_SPLIT_HANDOFF_PATH = 'docs/handoffs/2026-05-17-verifier-surface-trust-orchestration-split-closeout.md'
...
export async function findMissingArtifactClaims(records, packageScripts, routeSources, options = {}) {
  ...
  for (const record of asArray(records)) {
    ...
    for (const filePath of extractClaimedFilesFromText(text)) {
      if (!await repoFileExists(repoRoot, filePath)) missing.push(`${record.label}: missing file ${filePath}`)
    }
