...
const DEFAULT_ARCHIVE_MOVE_MANIFEST_PATHS = [
...
async function loadArchiveMovePathMap(repoRoot, manifestPath = DEFAULT_ARCHIVE_MOVE_MANIFEST_PATHS) {
  const manifestPaths = Array.isArray(manifestPath) ? manifestPath : [manifestPath]
  const archiveMap = new Map()
  for (const currentManifestPath of manifestPaths.filter(Boolean)) {
    try {
      const manifestSource = await fs.readFile(path.resolve(repoRoot, currentManifestPath), 'utf8')
