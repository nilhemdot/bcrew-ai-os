...
function sumOperationalRows(rows = [], field) {
  return normalizeList(rows).reduce((sum, row) => sum + Number(row[field] || 0), 0)
}
...
export function buildSourceMaturityGridSnapshot({
  ...
  const rows = normalizeList(sources).map(contract => {
    ...
    const pendingRouteCount = operationalCount(routeMap, sourceId, ['pendingRoutes'])
    const approvedRouteCount = operationalCount(routeMap, sourceId, ['approvedRoutes'])
    const appliedRouteCount = operationalCount(routeMap, sourceId, ['appliedRoutes'])
    ...
    const stages = {
      ...
      routed: buildStage(
        ...
        routeCount > 0
          ? `${routeCount} action route signals: ${pendingRouteCount} pending review, ${approvedRouteCount} approved, ${appliedRouteCount} applied.`
    ...
    return {
      ...
      metrics: {
        ...
        pendingRouteSignals: pendingRouteCount,
        approvedRouteSignals: approvedRouteCount,
        appliedRouteSignals: appliedRouteCount,
      },
      atomFlow,
      stages,
    }
  ...
  const routeReview = {
    totalRoutes: sumOperationalRows(sourceMaturityOperational.routesBySource, 'totalRoutes'),
    activeRoutes: sumOperationalRows(sourceMaturityOperational.routesBySource, 'activeRoutes'),
    pendingRoutes: sumOperationalRows(sourceMaturityOperational.routesBySource, 'pendingRoutes'),
    approvedRoutes: sumOperationalRows(sourceMaturityOperational.routesBySource, 'approvedRoutes'),
    appliedRoutes: sumOperationalRows(sourceMaturityOperational.routesBySource, 'appliedRoutes'),
  }
  routeReview.waitingRoutes = routeReview.pendingRoutes + routeReview.approvedRoutes
...
export function buildSyntheticSourceMaturityGridProof() {
  ...
  const grid = buildSourceMaturityGridSnapshot({
    ...
    sourceMaturityOperational: {
      ...
      routesBySource: [{ sourceId: 'SRC-ONE-001', totalRoutes: 1, activeRoutes: 1, appliedRoutes: 1 }],
    },
  ...
  return {
    ok: grid.rows.length === 2 &&
      ...
      completeRow?.metrics?.appliedRouteSignals === 1 &&
      completeRow?.stages?.routed?.detail?.includes('pending review') &&
      grid.summary?.routeReview?.appliedRoutes === 1 &&
