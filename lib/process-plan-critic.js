...
export function buildSyntheticPlanCriticArchitecturalRulesProof() {
  const fileLineCounts = {
    ...
    'server.js': 7790,
    'lib/medium-foundation-module.js': 1801,
  }
...
`
  const verifierLargeFileNoSplitPlan = `${strongPlanBase}

Add 100 lines of new behavioral checks directly to scripts/foundation-verify.mjs. Keep the new logic in the root verifier because it already has the surrounding inputs.
`
  const verifierThinWrapperPlan = `${strongPlanBase}

Touch scripts/foundation-verify.mjs only as a thin wrapper/source aggregation line. Split plan: all new verifier behavior lives in lib/foundation-verifier-domain-proof.js and scripts/process-verifier-domain-proof-check.mjs; scripts/foundation-verify.mjs imports or passes the source only and receives no new responsibility.
...
Dogfood proof recreates the original failure mode with a synthetic bad verifier fixture and proves the focused module rejects it.
`
...
`
  const overBudgetWatchNoSplitPlan = `${strongPlanBase}

Add new behavior directly to lib/medium-foundation-module.js. It is below the old 5000-line emergency threshold, so keep the work in the existing module.
`
  const offScopeSideWorkNoCoordinationPlan = `${strongPlanBase}

Marketing Video Lab side build from a separate chat will add a FAL live route, update server route wiring, add package scripts, commit, and push. Keep moving fast without returning to the main session.
`
  const coordinatedSideWorkPlan = `${strongPlanBase}

Marketing Video Lab requested shared files for server.js and lib/security-access.js. Main-session approved coordination owns the route integration, requestedSharedFiles are declared, process:hub-work-check validates the handoff, and no hub chat commits or pushes.

Split plan: server.js stays a thin wrapper with no new responsibility; route behavior lives in a new module boundary. The route proof has a performance budget under 5 seconds and under 1 MB with curl --max-time and time_total/bytes output.
`
...
`
  ...
  const verifierLargeFileNoSplit = evaluatePlanCriticPlan({
    ...commonOptions,
    planText: verifierLargeFileNoSplitPlan,
    changedFiles: ['scripts/foundation-verify.mjs'],
  })
  const verifierThinWrapper = evaluatePlanCriticPlan({
    ...commonOptions,
    planText: verifierThinWrapperPlan,
    changedFiles: ['scripts/foundation-verify.mjs'],
  })
  ...
  const overBudgetWatchNoSplit = evaluatePlanCriticPlan({
    ...commonOptions,
    planText: overBudgetWatchNoSplitPlan,
    changedFiles: ['lib/medium-foundation-module.js'],
  })
  const offScopeSideWorkNoCoordination = evaluatePlanCriticPlan({
    ...commonOptions,
    planText: offScopeSideWorkNoCoordinationPlan,
    changedFiles: ['public/marketing.js', 'lib/marketing-video-live.js', 'server.js', 'package.json'],
  })
  const coordinatedSideWork = evaluatePlanCriticPlan({
    ...commonOptions,
    planText: coordinatedSideWorkPlan,
  ...
  return {
    ok: largeFileNoSplit.status === 'revise' &&
      verifierLargeFileNoSplit.status === 'revise' &&
      verifierThinWrapper.status === 'pass' &&
      ...
      overBudgetWatchNoSplit.status === 'revise' &&
      offScopeSideWorkNoCoordination.status === 'revise' &&
      coordinatedSideWork.status === 'pass' &&
      ...
      verifierLargeFileNoSplit.findings.some(finding => finding.key === ARCHITECTURAL_RULE_FINDING_KEYS.largeFileSplitPlan) &&
      !verifierThinWrapper.findings.some(finding => finding.key === ARCHITECTURAL_RULE_FINDING_KEYS.largeFileSplitPlan) &&
      ...
      overBudgetWatchNoSplit.findings.some(finding => finding.key === ARCHITECTURAL_RULE_FINDING_KEYS.fileSizeOverBudgetSplitPlan) &&
      offScopeSideWorkNoCoordination.findings.some(finding => finding.key === ARCHITECTURAL_RULE_FINDING_KEYS.processWipProtocol) &&
      !coordinatedSideWork.findings.some(finding => finding.key === ARCHITECTURAL_RULE_FINDING_KEYS.processWipProtocol),
    largeFileNoSplit,
    verifierLargeFileNoSplit,
    verifierThinWrapper,
    checkWriteNoApply,
    verifierLiveState,
    auditFixNoDogfood,
    noFocusedProof,
    hotRouteNoBudget,
    overBudgetWatchNoSplit,
    offScopeSideWorkNoCoordination,
    coordinatedSideWork,
    compliant,
  }
}
