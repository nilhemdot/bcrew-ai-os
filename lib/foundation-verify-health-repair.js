...
export const FOUNDATION_VERIFY_HEALTH_REPAIR_PROOF_PATH = 'docs/_archive/audits/2026-05-01-foundation-verify-health-repair-proof.md'
...
export async function buildFoundationVerifyHealthRepairStatus({
  ...
  const closeout = normalizeList(foundationBuildLog?.builds).find(build =>
    build.closeoutKey === FOUNDATION_VERIFY_HEALTH_REPAIR_CLOSEOUT_KEY ||
      build.key === FOUNDATION_VERIFY_HEALTH_REPAIR_CLOSEOUT_KEY
  ) || getFoundationBuildCloseouts().find(build =>
    build.closeoutKey === FOUNDATION_VERIFY_HEALTH_REPAIR_CLOSEOUT_KEY ||
      build.key === FOUNDATION_VERIFY_HEALTH_REPAIR_CLOSEOUT_KEY
  ) || null
  ...
  addFinding(findings, onboardingSystemSource.includes('Chris metadata-only source-state proof is current') ||
    onboardingSystemSource.includes('Georgia Huntley Day-30 and Chris Chopite Day-30 are Requested after the controlled production run'), 'onboarding checker uses current Chris/source-state wording', 'Chris source-state current after production send')
  ...
  addFinding(findings, verifierCoverageSource.includes(FOUNDATION_VERIFY_HEALTH_REPAIR_CARD_ID), 'foundation verifier covers health repair card', FOUNDATION_VERIFY_HEALTH_REPAIR_CARD_ID)
