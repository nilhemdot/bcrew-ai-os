...
export async function buildFoundationVerifyHealthRepairStatus({
  ...
  addFinding(findings, productionCard?.lane === 'scoped', 'production auto-send remains scoped', productionCard?.lane || 'missing')
  addFinding(findings, process.env.AGENT_FEEDBACK_AUTO_SEND_ENABLED !== 'true', 'production auto-send env flag remains disabled', process.env.AGENT_FEEDBACK_AUTO_SEND_ENABLED || 'unset')
  ...
  return {
    ...
    summary: {
      ...
      productionAutoSendEnabled: process.env.AGENT_FEEDBACK_AUTO_SEND_ENABLED === 'true',
