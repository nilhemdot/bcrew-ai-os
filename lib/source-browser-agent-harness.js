...
export function planSourceBrowserAgentRun({
  ...
  if (pageHealth.ok === false) {
    ...
    return {
      ...
      nextAction: 'Relaunch a clean isolated source-browser session or route to hosted browser fallback before extracting.',
    }
  }
...
export function buildSourceBrowserAgentDogfoodProof() {
  ...
  const cases = [
    ...
    },
    {
      ...
      ok: [publicPlan, repoPlan, newsletterPlan, skoolNeedsSession, skoolReady, myicorNoBoundary, myicorMfa, myicorWrongSignup, badBrowserState, dangerousAction]
