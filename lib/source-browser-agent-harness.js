...
import {
  ...
  LOCAL_VIRTUAL_BROWSER_HANDS_VERSION,
...
import {
  ...
  selectSourceBrowserBrainRoute,
} from './source-browser-brain-route-policy.js'
...
export const SOURCE_BROWSER_AGENT_SOURCE_ID = 'SRC-YOUTUBE-INTEL-001'
  ...
  if (challenge) {
    return {
      ...
      route: sourceSpecificSessionRequired
        ? 'source_specific_session_then_harlan_operator'
        : 'clean_isolated_retry_then_harlan_operator',
      ...
      nextAction: sourceSpecificSessionRequired
        ? 'Use Source Session Broker or the source-specific runner first; if the challenge remains, prepare Harlan/operator escalation and keep the source parked.'
        : 'Retry a clean isolated source-browser session; if the challenge remains, prepare Harlan/operator escalation and keep the source parked.',
      recoveryPolicy: {
        ...
        automaticSteps: sourceSpecificSessionRequired
          ? [
              ...
              'prepare Harlan/operator escalation if the challenge remains',
            ]
          : [
      ...
      allowedActions: [
        ...
        'prepare Harlan/operator escalation without sending',
...
export function buildSourceBrowserAgentHarnessSnapshot() {
  ...
  return {
    ...
    brainRoutePolicy: buildSourceBrowserBrainRoutePolicySnapshot(),
    runtimeAdapters: [
      {
        adapterId: 'source:local-browser-hands',
        version: LOCAL_VIRTUAL_BROWSER_HANDS_VERSION,
        cardId: LOCAL_VIRTUAL_BROWSER_HANDS_CARD_ID,
        runner: 'scripts/run-local-virtual-browser-hands.mjs',
        posture: 'local_isolated_playwright_hands_no_model_no_browserbase',
        allowedActions: ['read_public_page', 'bounded_safe_anchor_navigation', 'capture_page_health_artifacts'],
        forbiddenActions: ['normal_chrome_profile', 'browserbase_default', 'model_call', 'form_submit', 'download', 'purchase', 'post_comment_message', 'credential_mutation'],
        proofCommand: 'npm run process:local-virtual-browser-hands-runtime-check -- --json',
      },
    ],
    stateMachine: {
...
export function planSourceBrowserAgentRun({
  ...
  const brainRoute = selectSourceBrowserBrainRoute({
    ...normalizedPacket,
    sourceFamily,
    action,
    pageHealth,
  ...
  const newsletterNoSubmitRead = sourceFamily === 'creator_newsletters' &&
    !/signup|subscribe|submit|apply|join|account|login/i.test(action)
  const needsBroker = !newsletterNoSubmitRead && (
    tool.requiresSourceSession ||
...
export function buildSourceBrowserAgentCrawlItemInput(plan = {}, {
  ...
  return {
    ...
    metadata: {
      ...
      url,
      host: hostOf(url),
      creatorId: text(packet.creatorId || row.creatorId),
      creator: text(packet.creator || row.creator),
      sourceCreatorIds: uniqueTexts([packet.creatorId, row.creatorId, ...list(packet.sourceCreatorIds), ...list(row.sourceCreatorIds)]),
      sourceCreators: uniqueTexts([packet.creator, row.creator, ...list(packet.sourceCreators), ...list(row.sourceCreators)]),
      sourceVideoId: text(packet.sourceVideoId || row.sourceVideoId),
      sourceVideoIds: uniqueTexts([packet.sourceVideoId, row.sourceVideoId, ...list(packet.sourceVideoIds), ...list(row.sourceVideoIds)]),
      reportArtifactId: text(packet.reportArtifactId || row.reportArtifactId),
      reportArtifactIds: uniqueTexts([packet.reportArtifactId, row.reportArtifactId, ...list(packet.reportArtifactIds), ...list(row.reportArtifactIds)]),
      status: text(plan.status),
      terminalState,
      ...
      sourceBrowserAgentPlan: {
        ...
        sourceFamily,
        toolRoute: runner,
        brainRoute: plan.brainRoute ? {
          routeId: text(plan.brainRoute.routeId),
          selectedBrain: text(plan.brainRoute.selectedBrain),
          selectedRunner: text(plan.brainRoute.selectedRunner),
          status: text(plan.brainRoute.status),
          reason: text(plan.brainRoute.reason),
          browserbaseUsed: plan.brainRoute.browserbaseUsed === true,
          apiModelSpendAllowed: plan.brainRoute.apiModelSpendAllowed === true,
          sendsHarlanNow: plan.brainRoute.sendsHarlanNow === true,
          normalChromeProfileAllowed: plan.brainRoute.normalChromeProfileAllowed === true,
          version: text(plan.brainRoute.version || SOURCE_BROWSER_BRAIN_ROUTE_POLICY_VERSION),
        } : null,
        terminalState,
        status: text(plan.status),
        stopReason: text(plan.stopReason),
...
export function evaluateSourceBrowserAgentHarness(snapshot = buildSourceBrowserAgentHarnessSnapshot()) {
  ...
  for (const ref of [AGENT_CAPABILITY_REGISTRY_CARD_ID, AGENT_TEMPLATE_RUNTIME_CONTRACT_CARD_ID, SOURCE_SESSION_BROKER_CARD_ID, SOURCE_GOD_MODE_EXTRACTOR_RUNTIME_CARD_ID, LOCAL_VIRTUAL_BROWSER_HANDS_CARD_ID, SOURCE_BROWSER_BRAIN_ROUTE_POLICY_CARD_ID]) {
    if (!list(snapshot.agent?.protocolRefs).includes(ref)) findings.push({ check: 'protocol_ref_present', detail: ref })
  }
  if (snapshot.brainRoutePolicy?.version !== SOURCE_BROWSER_BRAIN_ROUTE_POLICY_VERSION) {
    findings.push({ check: 'brain_route_policy_present', detail: snapshot.brainRoutePolicy?.version || 'missing' })
  }
  const localHands = list(snapshot.runtimeAdapters).find(adapter => adapter.adapterId === 'source:local-browser-hands')
  if (!localHands) findings.push({ check: 'local_hands_adapter_present', detail: 'source:local-browser-hands' })
  if (localHands && !/no_model_no_browserbase/.test(localHands.posture || '')) {
    findings.push({ check: 'local_hands_no_model_no_browserbase', detail: localHands.posture || '' })
  }
