...
const closeoutRecords = [
  ...
  },
  {
    ...
    status: 'shipped',
    acceptanceState: 'Verified',
    whatChanged: 'Ran one approved Steve Zahnd Day-30 Agent Feedback live loop and recorded metadata-only proof.',
    whatItDoes: 'Sends the Steve-only request through Gmail, writes ClickUp Requested only after Gmail succeeds, saves a controlled response, writes ClickUp Completed/Score/Feedback, sends the internal response notification, proves reminders stop after completion, and proves duplicate resend protection before side effects.',
    whyItMatters: 'The workflow is now proven end to end on Steve before any production auto-send or Georgia-targeted request is allowed.',
    whereItLives: [
    ...
    proofCommands: [
      ...
      'npm run foundation:verify',
      'curl -s "http://localhost:3000/api/foundation/build-log?limit=5"',
      'npm run process:ship-check -- --card=AGENT-FEEDBACK-STEVE-FULL-LOOP-TEST-001 --planApprovalRef=docs/process/approvals/AGENT-FEEDBACK-STEVE-FULL-LOOP-TEST-001.json --closeoutKey=agent-feedback-steve-full-loop-test-v1',
      'npm run process:fanout-check -- --card=AGENT-FEEDBACK-STEVE-FULL-LOOP-TEST-001 --closeoutKey=agent-feedback-steve-full-loop-test-v1',
      'npm run process:foundation-ship -- --card=AGENT-FEEDBACK-STEVE-FULL-LOOP-TEST-001 --planApprovalRef=docs/process/approvals/AGENT-FEEDBACK-STEVE-FULL-LOOP-TEST-001.json --closeoutKey=agent-feedback-steve-full-loop-test-v1 --commitRef=HEAD',
    ],
    proofStatus: 'Focused proof checks Steve-only target selection, Company Email recipient source, BCC oversight with Steve dedupe, Gmail-before-Requested sequencing, DB response save, ClickUp Completed/Score/Feedback writeback, internal response notification, reminder stop after completion, duplicate resend prevention, and metadata-only tracked proof.',
    reviewNext: 'Stop for review. Next card is AGENT-FEEDBACK-PRODUCTION-AUTOSEND-ENABLE-001 only after Steve accepts the Steve full-loop proof.',
    knownLimits: [
      'No production auto-send was enabled in this build.',
