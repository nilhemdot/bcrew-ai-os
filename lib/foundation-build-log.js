import {
  ...
  getBuildCloseoutDataSourceRecords,
  validateBuildCloseoutDataSourceRecords,
...
export function getFoundationBuildCloseouts() {
  return getBuildCloseoutDataSourceRecords().map(record => ({
    ...record,
...
export function getFoundationBuildCloseoutValidation() {
  return validateBuildCloseoutDataSourceRecords(getFoundationBuildCloseouts())
}
...
function closeoutRecordAsPinnedBuildLogEntry(closeout) {
  const match = closeout.match || {}
  const shas = unique(match.shas || [])
  const shortShas = unique(match.shortShas || [])
  const subjects = unique(match.subjectIncludes || [])
  const sha = shas[0] || ''
  const shortSha = shortShas[0] || (sha ? sha.slice(0, 7) : '')
  const subject = subjects[0] || closeout.whatChanged || closeout.key || 'Historical closeout'
  const backlogIds = unique(closeout.backlogIds)
  const mentionedBacklogIds = unique(closeout.mentionedBacklogIds || [])
  return {
    sha,
    shortSha,
    committedAt: closeout.committedAt || closeout.closedAt || '',
    subject,
    files: normalizeList(closeout.whereItLives),
    fileGroups: ['Closeout registry'],
    fileCount: normalizeList(closeout.whereItLives).length,
    primaryFiles: normalizeList(closeout.whereItLives).slice(0, 8),
    ...
    relatedBacklog: backlogIds.map(id => ({ id, lane: 'done' })),
    mentionedBacklog: mentionedBacklogIds.map(id => ({ id })),
