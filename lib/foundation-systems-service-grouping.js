...
export const FOUNDATION_SYSTEMS_SERVICE_GROUPING_BASELINE_PATH = 'docs/audits/2026-04-30-foundation-systems-service-grouping-baseline.md'
export const FOUNDATION_SYSTEMS_SERVICE_GROUPING_MANUAL_REVIEW_PATH = 'docs/audits/2026-04-30-foundation-systems-service-grouping-manual-review.md'

export const FOUNDATION_SYSTEMS_EXISTING_SYSTEM_IDS = [
...
function validateSystemServiceAreas(systems) {
  ...
  const expectedIds = new Set(FOUNDATION_SYSTEMS_EXISTING_SYSTEM_IDS)
  const findings = []
  ...
  for (const expectedId of expectedIds) {
    if (!seenIds.has(expectedId)) findings.push({ systemId: expectedId, issue: 'missing-existing-system' })
  }
...
export async function buildFoundationSystemsServiceGroupingStatus({
  ...
  const foundationUiSource = await readOptionalText(repoRoot, 'public/foundation.js')
  const foundationStylesSource = await readOptionalText(repoRoot, 'public/styles.css')
  ...
  addFinding(findings, systems.length === FOUNDATION_SYSTEMS_EXISTING_SYSTEM_IDS.length, 'only existing grouped systems are classified', `${systems.length}/${FOUNDATION_SYSTEMS_EXISTING_SYSTEM_IDS.length}`)
  ...
  addFinding(findings, includesAll(foundationUiSource, [
  ...
  ]), 'systems UI renders service-area grouping and empty groups', 'public/foundation.js')
  ...
  if (foundationHub) {
    ...
    addFinding(findings, onboardingCard?.lane === 'scoped', 'Agent Onboarding Feedback system card remains scoped', onboardingCard?.lane || 'missing')
    addFinding(findings, sendCard?.lane === 'scoped', 'Agent feedback send card remains scoped', sendCard?.lane || 'missing')
    addFinding(findings, /No Gmail send/.test(cardText(groupingCard)), 'owner card records no Gmail send boundary', groupingCard?.id || 'missing')
  }

  const closeout = normalizeList(foundationBuildLog?.builds).find(build => build.closeoutKey === FOUNDATION_SYSTEMS_SERVICE_GROUPING_CLOSEOUT_KEY) || null
