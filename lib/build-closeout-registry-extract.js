...
export const BUILD_CLOSEOUT_REGISTRY_EXTRACT_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-27-hot-doc-cleanup/2026-05-18-build-closeout-registry-extract-closeout.md'
...
export const BUILD_CLOSEOUT_REGISTRY_EXTRACT_MODULE_PATHS = [
  ...
  'data/foundation-build-closeouts/source-newsletter-records.json',
  'data/foundation-build-closeouts/overnight-records.json',
...
export const BUILD_CLOSEOUT_REGISTRY_EXTRACT_PROOF_COMMANDS = [
  'node --check lib/build-closeout-registry-extract.js scripts/process-build-closeout-registry-extract-check.mjs scripts/foundation-verify.mjs lib/foundation-build-closeout-records.js lib/foundation-build-closeout-overnight-records.js lib/foundation-build-closeout-size-records.js lib/foundation-build-closeout-process-gate-records.js lib/foundation-build-closeout-process-gate-*-records.js lib/foundation-build-closeout-source-records.js lib/foundation-build-closeout-source-*-records.js',
...
export function buildBuildCloseoutRegistryExtractSnapshot({ repoRoot = process.cwd() } = {}) {
  ...
  const processGateRecordsSource = readRepoText(repoRoot, 'lib/foundation-build-closeout-process-gate-records.js')
  const missingProcessGateSpreads = [
  ...
  ].filter(exportName => !processGateRecordsSource.includes(`...${exportName}`))
  const sourceRecordsSource = readRepoText(repoRoot, 'lib/foundation-build-closeout-source-records.js')
  const missingSourceSpreads = [
  ...
  ].filter(exportName => !sourceRecordsSource.includes(`...${exportName}`))
  ...
  const ok = rootRows.every(row => row.lineCount < 3000) &&
    ...
    missingProcessGateSpreads.length === 0 &&
    missingSourceSpreads.length === 0 &&
  ...
  return {
    ...
    missingMovedKeys,
    missingRootSpreads,
    missingProcessGateSpreads,
    missingSourceSpreads,
