...
import {
  buildFreeResourceFileCandidates,
...
const AUTH_WALL_RE = /\b(log in to continue|sign in to continue|members only|private community|private group|you need to log in|join to view|request to join|verify you are human|access denied|checking your browser)\b/i
const BROWSER_CONTROL_BLOCKER_RE = /\b(about:blank|chrome:\/\/newtab|chrome-error:\/\/|restore pages|restore previous session|session restore|chrome didn't shut down correctly|aw, snap|this site can'?t be reached)\b/i
const BROWSER_CHALLENGE_RE = /\b(just a moment|checking your browser|checking browser|enable javascript and cookies|attention required|verify you are human)\b/i
const CAPTCHA_CHALLENGE_RE = /\b(?:complete|solve|pass)\s+(?:the\s+)?captcha\b|\bcaptcha\s+(?:required|challenge)\b/i
const CLOUDFLARE_CHALLENGE_RE = /\bcloudflare\b.{0,80}\b(?:checking|challenge|attention required|ray id|security)\b|\b(?:checking|challenge|attention required)\b.{0,80}\bcloudflare\b/i
const BLANK_BROWSER_URL_RE = /^(?:about:blank|chrome:\/\/newtab|chrome-error:\/\/)/i
...
const REPO_SOURCE_TYPE_RE = /github|gitlab|gist|repo|repos|repository|source[-_]?code|\bcode\b|docs/i
const REPO_GLOBAL_PATH_RE = /^\/(?:$|features(?:\/|$)|pricing(?:\/|$)|enterprise(?:\/|$)|customer(?:s|-stories)(?:\/|$)|topics(?:\/|$)|trending(?:\/|$)|explore(?:\/|$)|marketplace(?:\/|$)|sponsors(?:\/|$)|login(?:\/|$)|signin(?:\/|$)|signup(?:\/|$)|join(?:\/|$)|settings(?:\/|$)|notifications(?:\/|$)|pulls(?:\/|$)|issues(?:\/|$)|codespaces(?:\/|$)|copilot(?:\/|$)|security(?:\/|$)|readme(?:\/|$)|about(?:\/|$))/i
const REPO_LOCAL_SIGNAL_RE = /\b(readme|docs?|examples?|sample|samples|src|lib|packages?|claude\.md|agents\.md|skills?|recipes?|mcp|server|client|api|workflow|automation|memory|setup|install|usage|quickstart|architecture)\b/i
const REPO_RESERVED_OWNER_SEGMENTS = new Set([
  'features',
  'pricing',
  'enterprise',
  'customers',
...
function repoScopeForUrl(value = '', { sourceType = '' } = {}) {
  const parsed = parseUrl(value)
  if (!parsed) return null
  const host = parsed.hostname.replace(/^www\./, '').toLowerCase()
  const parts = parsed.pathname.split('/').map(text).filter(Boolean)
  const allowGenericPath = REPO_SOURCE_TYPE_RE.test(sourceType)
  if (host === 'github.com' && parts.length >= 2 && !REPO_RESERVED_OWNER_SEGMENTS.has(parts[0].toLowerCase())) {
    return { host, owner: parts[0].toLowerCase(), repo: parts[1].replace(/\.git$/i, '').toLowerCase(), kind: 'github_repo' }
  }
  if (host === 'gist.github.com' && parts.length >= 2) {
    return { host, owner: parts[0].toLowerCase(), repo: parts[1].toLowerCase(), kind: 'github_gist' }
  }
  if (host === 'gitlab.com' && parts.length >= 2 && !REPO_RESERVED_OWNER_SEGMENTS.has(parts[0].toLowerCase())) {
    return { host, owner: parts[0].toLowerCase(), repo: parts[1].replace(/\.git$/i, '').toLowerCase(), kind: 'gitlab_repo' }
  }
  if (allowGenericPath && parts.length >= 2 && !REPO_RESERVED_OWNER_SEGMENTS.has(parts[0].toLowerCase())) {
    return { host, owner: parts[0].toLowerCase(), repo: parts[1].replace(/\.git$/i, '').toLowerCase(), kind: 'generic_repo_path' }
  }
  return null
}

function sameRepoScope(a = '', b = '', { sourceType = '' } = {}) {
  const left = repoScopeForUrl(a, { sourceType })
  const right = repoScopeForUrl(b, { sourceType })
  return Boolean(left && right && left.host === right.host && left.owner === right.owner && left.repo === right.repo)
}

function isRepoSourceContext(sourceType = '', targetUrl = '') {
  return REPO_SOURCE_TYPE_RE.test(sourceType) || Boolean(repoScopeForUrl(targetUrl, { sourceType }))
}

function isRepoBrowserPageUrl(value = '') {
  const parsed = parseUrl(value)
  if (!parsed) return false
  const host = parsed.hostname.replace(/^www\./, '').toLowerCase()
  const pathname = parsed.pathname || ''
  if (host === 'github.com') return /\/(?:blob|tree)\//.test(pathname) || Boolean(repoScopeForUrl(value))
  if (host === 'gist.github.com') return Boolean(repoScopeForUrl(value))
  if (host === 'gitlab.com') return Boolean(repoScopeForUrl(value))
  return false
}

function isRepoGlobalChromeLink({ sourceUrl = '', targetUrl = '', sourceType = '' } = {}) {
  if (!isRepoSourceContext(sourceType, sourceUrl)) return false
  const parsed = parseUrl(targetUrl)
  if (!parsed) return false
  if (sameRepoScope(sourceUrl, targetUrl, { sourceType })) return false
  const host = parsed.hostname.replace(/^www\./, '').toLowerCase()
  return ['github.com', 'gist.github.com', 'gitlab.com'].includes(host) && REPO_GLOBAL_PATH_RE.test(parsed.pathname || '/')
}

function repoLinkPriority(link = {}) {
  const surface = `${link.url || ''} ${link.label || ''}`.toLowerCase()
  if (/readme|claude\.md|agents\.md/.test(surface)) return 0
  if (/docs?|architecture|quickstart|usage|setup|install/.test(surface)) return 1
  if (/examples?|samples?|recipes?|skills?/.test(surface)) return 2
  if (/src|lib|packages?|server|client|api|mcp|workflow|automation|memory/.test(surface)) return 3
  if (REPO_LOCAL_SIGNAL_RE.test(surface)) return 4
  return 8
}
...
export function buildSourceBrowserSessionPolicy({
  targetUrl = '',
  ...
  const host = hostOf(targetUrl) || slug(targetUrl || sourceType)
  const key = slug([sourceType, host, sourceAccount].filter(Boolean).join('-'))
  const persistent = profileMode === 'persistent_isolated'
  return {
    profileKey: key,
    profileMode: persistent ? 'persistent_isolated' : 'ephemeral_isolated',
    userDataDir: persistent && profileRoot ? path.resolve(profileRoot, key) : '',
    usesNormalChromeProfile: false,
    defaultChromeProfileForbidden: true,
    restorePreviousSessionDisabled: true,
    sourceOriginStateClearingAllowed: true,
    maxNavigationAttempts: 2,
    plainEnglish: persistent
      ? 'Use an isolated source browser profile for this source/account. Never attach to the normal Chrome profile.'
      : 'Use a fresh isolated browser context for this run. Never attach to the normal Chrome profile.',
  }
}

export function evaluateSourceBrowserPageHealth(snapshot = {}) {
  const url = text(snapshot.url)
  const title = text(snapshot.title)
  const body = text(snapshot.bodyTextPreview || snapshot.bodyText)
  const textChars = number(snapshot.textChars, body.length)
  const surface = `${url} ${title} ${body}`
  const findings = []
  if (!url || BLANK_BROWSER_URL_RE.test(url)) {
    findings.push({
      check: 'browser_page_not_blank',
      detail: url || 'missing_url',
      recovery: 'relaunch_clean_source_context_or_retry_exact_url',
    })
  }
  if (BROWSER_CONTROL_BLOCKER_RE.test(surface)) {
    findings.push({
      check: 'browser_control_surface_not_source_content',
      detail: compactWhitespace(surface).slice(0, 220),
      recovery: 'dismiss_restore_state_by_relaunching_clean_source_context',
    })
  }
  if (looksLikeBrowserChallengeText(surface)) {
    findings.push({
      check: 'browser_challenge_not_source_content',
      detail: compactWhitespace(surface).slice(0, 220),
      recovery: 'retry clean source-browser session, then route to hosted/browser-agent fallback or source-specific session if the challenge remains',
    })
  }
  if (!title && textChars === 0 && !body) {
    findings.push({
      check: 'browser_page_has_readable_source_content',
      detail: 'empty_title_and_body_after_navigation',
      recovery: 'retry_navigation_then_fail_closed_with_browser_state_artifact',
    })
  }
  return {
    ok: findings.length === 0,
    status: findings.length ? 'browser_state_blocked' : 'browser_state_healthy',
    findings,
  }
}
...
function formsOf(html = '') {
  ...
  const controls = [...source.matchAll(/<(input|button|textarea|select)\b([^>]*)>/gi)]
    .map(match => {
      ...
      return {
        tag,
        type: (attr(match[0], 'type') || '').toLowerCase(),
  ...
  const hasSubmit = controls.some(control =>
    control.type === 'submit' ||
    (control.tag === 'input' && /submit|image/.test(control.type)) ||
    (control.tag === 'button' && forms.length > 0 && (!control.type || control.type === 'submit'))
  )
  return {
...
function looksLikeAuthWallText(surface = '') {
  return AUTH_WALL_RE.test(surface) || CAPTCHA_CHALLENGE_RE.test(surface) || CLOUDFLARE_CHALLENGE_RE.test(surface)
}

function looksLikeBrowserChallengeText(surface = '') {
  return BROWSER_CHALLENGE_RE.test(surface) || CAPTCHA_CHALLENGE_RE.test(surface) || CLOUDFLARE_CHALLENGE_RE.test(surface)
}
...
function sourceFamilyForUrl(url = '', label = '', sourceType = '') {
  ...
  if (classifyFreeResourceFileLink({ url, label }).isFileResource) return 'download_or_file'
...
export function classifyGodModeSourceLink(link = {}, {
  ...
  const filePolicy = classifyFreeResourceFileLink({ url, label })
  if (filePolicy.isFileResource && !isRepoBrowserPageUrl(url)) {
    return {
      url,
      host,
      label,
      ...
      autoReadable: false,
      shouldFollow: false,
      blocker: 'File/download links need a file-reader policy before opening or capture.',
      nextAction: filePolicy.nextAction,
      filePolicy,
    }
  }
...
function detectPageBoundary(snapshot = {}) {
  ...
  const repoScope = repoScopeForUrl(snapshot.url)
  const repoChromeFormsOnly = Boolean(repoScope) &&
    forms.hasPasswordInput !== true &&
    RISKY_PATH_RE.test(snapshot.url || '') !== true
  const repoChromePurchaseOnly = Boolean(repoScope) &&
    RISKY_PATH_RE.test(snapshot.url || '') !== true &&
    !/checkout|payment|billing|purchase|cart|member-only|private/i.test(`${snapshot.url || ''} ${snapshot.title || ''}`)
  const blockers = []
  if (looksLikeAuthWallText(surface) || forms.hasPasswordInput) {
    blockers.push({
  ...
  if (PURCHASE_RE.test(surface) && !repoChromePurchaseOnly) {
    blockers.push({
  ...
  if (forms.externalActionRequired && !repoChromeFormsOnly) {
    blockers.push({
...
function extractSnapshotFromHtml({ url = '', html = '', screenshotPath = '', screenshotHash = '' } = {}) {
  ...
  snapshot.browserHealth = evaluateSourceBrowserPageHealth(snapshot)
...
function isRepoChromeBlocker(link = {}, sourceType = '') {
  if (!link?.blocker) return false
  return isRepoSourceContext(sourceType, link.url) && (
    isRepoGlobalChromeLink({ sourceUrl: link.url, targetUrl: link.url, sourceType }) ||
    REPO_GLOBAL_PATH_RE.test(pathOf(link.url) || '/')
  )
}
...
function scoreSourceValue(snapshots = [], linkDecisions = [], { sourceType = '', sopCompletion = null } = {}) {
  ...
  const blockerPenalty = Math.min(20, linkDecisions.filter(link => link.blocker && !isRepoChromeBlocker(link, sourceType)).length * 3)
...
function buildCreatorSourceStackUpdate({
  ...
  return {
    ...
    counts: {
      ...
      fileResourceCandidates: fileResourceCandidates.length,
...
function buildRuntimeCapabilities({ mode = '', snapshots = [], handsEvents = [], linkDecisions = [], sourceSop = {}, sideEffectState = {} } = {}) {
  const pageBlockers = snapshots.flatMap(snapshot => list(snapshot.pageBlockers))
  const blockedUnsafe = linkDecisions.some(link => link.blocker) || pageBlockers.length > 0
  const classifiedPageValue = snapshots.some(snapshot =>
    BUILD_VALUE_RE.test(`${snapshot.url || ''} ${snapshot.title || ''} ${snapshot.bodyTextPreview || ''}`)
  )
  const classifiedReadablePage = snapshots.some(snapshot => number(snapshot.textChars, 0) > 0 || text(snapshot.title))
  return {
    ...
    brain: linkDecisions.length > 0 || pageBlockers.length > 0 || classifiedPageValue || classifiedReadablePage ? 'working' : 'partial',
...
function evaluateReport(report = {}) {
  ...
  const pageBlockers = list(report.pages).flatMap(page => list(page.pageBlockers))
  const browserHealthFindings = list(report.pages).flatMap(page => list(page.browserHealth?.findings))
  const boundaryProofPresent = list(report.linkDecisions).some(link => link.blocker) ||
    pageBlockers.length > 0 ||
    browserHealthFindings.length > 0 ||
    list(report.blockers).length > 0
  ...
  for (const finding of browserHealthFindings) {
    findings.push({ check: 'browser_state_must_not_false_green', detail: `${finding.check}:${finding.detail || ''}` })
  }
  if (report.runtime?.requireBoundaryBlockerDogfood === true && !boundaryProofPresent) {
    findings.push({ check: 'boundary_blocker_dogfood_required', detail: 'proof should include blocked unsafe link/form/auth/download/paid path' })
  }
  return {
    ok: findings.length === 0,
...
function selectFollowCandidates(snapshot = {}, linkDecisions = [], visited = new Set(), maxDepth = 1, { sourceType = '', targetUrl = '' } = {}) {
  const base = linkDecisions
  ...
  if (isRepoSourceContext(sourceType, targetUrl || snapshot.url)) {
    const repoScoped = base
      .filter(link => sameRepoScope(targetUrl || snapshot.url, link.url, { sourceType }))
      .filter(link => !isRepoGlobalChromeLink({ sourceUrl: targetUrl || snapshot.url, targetUrl: link.url, sourceType }))
      .sort((left, right) =>
        repoLinkPriority(left) - repoLinkPriority(right) ||
        text(left.label || left.url).localeCompare(text(right.label || right.url))
      )
      .slice(0, 10)
    if (repoScoped.length) return repoScoped
  }
  return base.slice(0, 10)
}

function buildRepoRuntimeReview({ targetUrl = '', sourceType = '', snapshots = [], linkDecisions = [], handsEvents = [] } = {}) {
  const scope = repoScopeForUrl(targetUrl, { sourceType })
  if (!scope && !isRepoSourceContext(sourceType, targetUrl)) {
    return {
      status: 'not_repo_source',
      applies: false,
    }
  }
  const localRepoPages = snapshots.filter(snapshot => sameRepoScope(targetUrl, snapshot.url, { sourceType }))
  const globalChromePages = snapshots.filter(snapshot =>
    isRepoGlobalChromeLink({ sourceUrl: targetUrl, targetUrl: snapshot.url, sourceType })
  )
  const localRepoLinks = linkDecisions.filter(link => sameRepoScope(targetUrl, link.url, { sourceType }))
  const globalChromeLinks = linkDecisions.filter(link =>
    isRepoGlobalChromeLink({ sourceUrl: targetUrl, targetUrl: link.url, sourceType })
  )
  const hardBlockers = [
    ...linkDecisions.filter(link => link.blocker && !isRepoChromeBlocker(link, sourceType)),
    ...snapshots.flatMap(snapshot => list(snapshot.pageBlockers)),
  ]
  const signalSurface = localRepoPages
    .flatMap(snapshot => [snapshot.title, ...list(snapshot.headings).map(heading => heading.text)])
    .join(' ')
  const implementationSignalCount = (signalSurface.match(new RegExp(REPO_LOCAL_SIGNAL_RE.source, 'gi')) || []).length
  return {
    status: localRepoPages.length ? 'repo_readback_ready' : 'repo_readback_incomplete',
    applies: true,
    scope,
    localRepoPagesRead: localRepoPages.length,
    globalChromePagesOpened: globalChromePages.length,
    localRepoLinksFound: localRepoLinks.length,
    globalChromeLinksSkipped: globalChromeLinks.length,
    hardBlockerCount: hardBlockers.length,
    implementationSignalCount,
    handsRepoLocalFollowCount: handsEvents.filter(event => sameRepoScope(targetUrl, event.toUrl, { sourceType })).length,
    plainEnglish: 'Repo extraction stays inside the public repo path and skips GitHub/GitLab navigation chrome such as home, pricing, login, and marketing pages.',
    nextAction: 'Deep-review README/docs/examples/license/provenance next; do not clone, install, import, or download code from this runtime lane.',
  }
}
...
async function runSyntheticMode({
  ...
  while (queue.length && snapshots.length < maxPages) {
    ...
    const candidates = selectFollowCandidates(snapshot, decisions, visited, maxDepth, { sourceType, targetUrl })
...
async function clickAnchorByHref(page, targetUrl = '', timeoutMs = 15000) {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `click_failed_fallback_to_goto:${error instanceof Error ? error.message.slice(0, 180) : String(error).slice(0, 180)}`,
    }
  }
}

async function clearSourceOriginState(page, context, url = '') {
...
async function runLiveBrowserMode({
  ...
  artifactDir = '',
  profileMode = 'ephemeral_isolated',
  profileRoot = path.join(SOURCE_GOD_MODE_EXTRACTOR_RUNTIME_ROOT, 'profiles'),
} = {}) {
  ...
  try {
    while (queue.length && snapshots.length < maxPages) {
      ...
      if (!url || visited.has(url)) continue
      visited.add(url)
      const recoveryAttempt = number(item.recoveryAttempt, 0)
      ...
      try {
        ...
        const canRetry = recoveryAttempt + 1 < sessionPolicy.maxNavigationAttempts
        browserRecoveryEvents.push({
          eventId: `browser-recovery:${browserRecoveryEvents.length + 1}`,
          url,
          status: canRetry ? 'navigation_failed_retrying_clean_source_context' : 'navigation_failed_closed',
          attempt: recoveryAttempt + 1,
          reason: error instanceof Error ? error.message.slice(0, 240) : String(error).slice(0, 240),
          action: canRetry ? 'clear_source_origin_state_and_retry_exact_url' : 'fail_closed_with_navigation_error',
        })
        await clearSourceOriginState(page, context, url)
        if (canRetry) {
          visited.delete(url)
          queue.unshift({ ...item, recoveryAttempt: recoveryAttempt + 1, navigationMethod: 'browser_state_recovery_retry' })
          continue
        }
        linkDecisions.push({
          url,
          host: hostOf(url),
          label: item.linkText || '',
          sourceFamily: sourceFamilyForUrl(url, item.linkText || '', sourceType),
          decision: 'blocked_browser_navigation_failure',
          autoReadable: false,
          shouldFollow: false,
          blocker: 'Browser navigation failed after recovery attempts.',
          nextAction: 'Route to source-browser runtime repair or hosted browser fallback before claiming this source was extracted.',
        })
        continue
      }
      ...
      if (snapshot.browserHealth?.ok === false) {
        const canRetry = recoveryAttempt + 1 < sessionPolicy.maxNavigationAttempts
        browserRecoveryEvents.push({
          eventId: `browser-recovery:${browserRecoveryEvents.length + 1}`,
          url,
          status: canRetry ? 'browser_state_blocked_retrying_clean_source_context' : 'browser_state_blocked_closed',
          attempt: recoveryAttempt + 1,
          findings: snapshot.browserHealth.findings,
          action: canRetry ? 'clear_source_origin_state_and_retry_exact_url' : 'fail_closed_with_browser_state_artifact',
        })
        await clearSourceOriginState(page, context, url)
        if (canRetry) {
          visited.delete(url)
  ...
  return {
    ...
    sideEffectState: sideEffects({
      ...
      sourceOriginStateCleared: browserRecoveryEvents.some(event => /retrying|closed/.test(event.status || '')),
      browserStateRecoveryEvents: browserRecoveryEvents.length,
    }),
  }
}
...
export async function runSourceGodModeExtractor({
  ...
  const runtimeResult = mode === 'live_browser'
    ? await runLiveBrowserMode({
      ...
      artifactDir,
      profileMode,
      profileRoot: profileRoot || path.join(path.resolve(rootDir), 'profiles'),
    })
  ...
  const browserRecoveryEvents = runtimeResult.browserRecoveryEvents || []
  const sessionPolicy = runtimeResult.sessionPolicy || buildSourceBrowserSessionPolicy({ targetUrl, sourceType })
  const paidGateEvaluations = buildPaidGateEvaluations(linkDecisions)
  const fileResourceCandidates = buildFreeResourceFileCandidates(linkDecisions)
  ...
  const repoReview = buildRepoRuntimeReview({
    targetUrl,
    sourceType,
    snapshots,
    linkDecisions,
  ...
  const pages = snapshots.map(snapshot => ({
    ...
    browserHealth: snapshot.browserHealth || evaluateSourceBrowserPageHealth(snapshot),
    artifacts: {
  ...
  const report = {
    ...
    blockers: [
      ...snapshots.flatMap(snapshot => list(snapshot.pageBlockers)),
      ...snapshots.flatMap(snapshot => list(snapshot.browserHealth?.findings).map(finding => ({
        type: finding.check,
        reason: finding.detail,
        url: snapshot.url,
        nextAction: finding.recovery || 'retry clean source-browser session or route to hosted/browser-agent fallback',
      }))),
