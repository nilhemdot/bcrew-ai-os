...
import {
  ...
  summarizeConnectorCredentialsForMatrixRow,
...
export const CONNECTOR_MATRIX_ROWS = [
  ...
  },
  {
    ...
    blockedReason: 'Source contract exists as fail-closed prep; GA4 property/account authorization and extraction approval are still required.',
  },
  {
    key: 'gsc',
    ...
    blockedReason: 'Source contract exists as fail-closed prep; Search Console site-property authorization and extraction approval are still required.',
  },
  {
    key: 'gbp',
    ...
    blockedReason: 'Source contract exists as fail-closed prep; Google Business Profile account/location authorization and review-use approval are still required.',
  },
  {
    key: 'real',
...
export function buildSourceConnectorMatrixSnapshot({
  ...
  connectorCredentialRegistry = null,
} = {}) {
  const contractMap = byId(sources, 'sourceId')
  const connectorMap = byId(connectors, 'connectorId')
  const credentialRegistry = connectorCredentialRegistry || buildConnectorCredentialRegistrySnapshot({
  ...
  const rows = CONNECTOR_MATRIX_ROWS.map(definition => {
    ...
    const credential = summarizeConnectorCredentialsForMatrixRow(credentialRegistry, definition)
    const row = {
      ...definition,
      ...
      routeCount: routes,
      credentialRegistryKeys: credential.credentialRegistryKeys,
      hasCredential: credential.hasCredential,
      credentialStatus: credential.credentialStatus,
      credentialLastProbeStatus: credential.credentialLastProbeStatus,
      credentialBlockerReason: credential.credentialBlockerReason,
      sourceUnlockedByCredential: credential.sourceUnlockedByCredential,
    }
    ...
    row.flow = {
      ...
      has_credential: row.hasCredential,
  ...
  return {
    ...
    summary: {
      ...
      credentialCoveredCount: rows.filter(row => row.credentialRegistryKeys.length > 0).length,
      credentialAvailableCount: rows.filter(row => row.hasCredential).length,
    },
    connectorCredentialRegistry: {
      generatedAt: credentialRegistry.generatedAt,
