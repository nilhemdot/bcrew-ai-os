...
export const SOURCE_LIFECYCLE_COMPLETION_EXPECTED_SOURCE_COUNT = 35
...
const TARGET_RULES = {
  ...
  'SRC-MEETINGS-001': {
    ...
    scopeNote: 'Complete for readable/archive use; raw Drive ACL/vault proof remains on meeting blocker cards.',
  },
}
...
export function buildSourceLifecycleCompletionStatus({
  ...
  const missingRules = sourceContractIds.filter(sourceId => !ruleIds.has(sourceId))
  ...
  addFinding(findings, SOURCE_LIFECYCLE_COMPLETION_RULE_IDS.length === SOURCE_LIFECYCLE_COMPLETION_EXPECTED_SOURCE_COUNT, 'completion registry covers 35 sources', String(SOURCE_LIFECYCLE_COMPLETION_RULE_IDS.length))
  addFinding(findings, duplicateRules.length === 0, 'completion registry has unique source IDs', duplicateRules.join(', ') || 'none')
  addFinding(findings, sourceContracts.length === SOURCE_LIFECYCLE_COMPLETION_EXPECTED_SOURCE_COUNT, 'source-of-truth exposes exactly 35 source contracts', String(sourceContracts.length))
  addFinding(findings, missingRules.length === 0, 'every source contract has a terminal rule', missingRules.join(', ') || 'none')
  addFinding(findings, missingContracts.length === 0, 'every terminal rule has a source contract', missingContracts.join(', ') || 'none')
  addFinding(findings, targets.length === SOURCE_LIFECYCLE_REQUIRED_TARGET_COUNT, 'all 12 extraction targets are represented', String(targets.length))
  ...
  addFinding(findings, sourceContracts.length >= SOURCE_LIFECYCLE_MIN_SOURCE_CONTRACTS, 'minimum source lifecycle source count remains covered', String(sourceContracts.length))
  ...
  return {
    ...
    summary: {
      ...
      expectedSourceCount: SOURCE_LIFECYCLE_COMPLETION_EXPECTED_SOURCE_COUNT,
...
export async function buildSourceLifecycleCompletionCheck({
  ...
  addFinding(findings, planSource.includes('All 35 source contracts must be revalidated'), 'approved plan records all-source revalidation scope', SOURCE_LIFECYCLE_COMPLETION_PLAN_PATH)
