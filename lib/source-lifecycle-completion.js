...
export const SOURCE_LIFECYCLE_COMPLETION_BLOCKED_RULES = {
  ...
  'SRC-YOUTUBE-INTEL-001': {
    ...
    reason: 'Public no-auth YouTube metadata watch is proposal-only Build Intel, but YouTube is still not Strategy readiness supply and richer transcript/screenshot/comment/member/community/Gemini video extraction remains separately gated.',
    nextAction: 'Use the daily watch review pool for proposal-only source selection; route richer YouTube extraction/scout implementation through the explicit YouTube Build Intel cards.',
  },
  'SRC-LOOM-001': {
...
const TARGET_RULES = {
  ...
  'SRC-GCAL-001': {
    ...
    scopeNote: 'Complete for delegated read-side current reality: Calendar current-window event archive through calendar-current-day and calendar-sync-current. Calendar writes, invites, RSVP handling, event updates/deletes, credential mutation, broad calendar exposure, and meeting-note/transcript truth remain separate blocked boundaries.',
  },
  'SRC-GDRIVE-001': {
    ...
    scopeNote: 'Complete for delegated read-side current reality: Drive corpus inventory through drive-corpus-backfill, bounded Drive content extraction through drive-content-extract-backfill, and local Drive document/PDF/spreadsheet/text artifacts. Drive permission mutation, request-access sends, broad private Drive sweeps, credential/OAuth scope mutation, raw Drive exposure to broad team/agent surfaces, media/vision extraction, and treating the Strategy Folder as canonical strategy truth remain separate blocked boundaries.',
  },
  'SRC-VIDEO-001': {
...
function targetHasProof(target = {}) {
  ...
  const hasProof = targetProofCount(target) > 0 || targetItemCount(target) > 0
  if (target.status === 'blocked') return false
  if (target.runtimeMode === 'paused' && !['complete', 'completed', 'succeeded'].includes(target.status)) return false
...
function statusMatchesTerminal(contract = {}, rule = {}) {
  ...
  if (rule.requiredState === 'accepted_blocked') {
    return includesAny(status, ['gap', 'pending revalidation', 'scoped, not connected']) ||
      includesAny(validation, ['not signed off', 'proposal-only'])
  }
  return false
}
...
export function buildSourceLifecycleCompletionStatus({
  ...
  const closeoutLinksCard = Boolean(closeout?.backlogIds?.includes(SOURCE_LIFECYCLE_COMPLETION_CARD_ID))
  const liveBacklogHasCloseoutKey =
    normalizeText(readinessBlocker?.statusNote).includes(SOURCE_LIFECYCLE_COMPLETION_CLOSEOUT_KEY) ||
    (readinessBlocker?.lane === 'done' && closeoutLinksCard)
