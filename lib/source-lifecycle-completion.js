...
export function buildSourceLifecycleCompletionStatus({
  ...
  addFinding(findings, SOURCE_LIFECYCLE_COMPLETION_RULE_IDS.length === SOURCE_LIFECYCLE_COMPLETION_EXPECTED_SOURCE_COUNT, `completion registry covers ${SOURCE_LIFECYCLE_COMPLETION_EXPECTED_SOURCE_COUNT} sources`, String(SOURCE_LIFECYCLE_COMPLETION_RULE_IDS.length))
  addFinding(findings, duplicateRules.length === 0, 'completion registry has unique source IDs', duplicateRules.join(', ') || 'none')
  addFinding(findings, sourceContracts.length === SOURCE_LIFECYCLE_COMPLETION_EXPECTED_SOURCE_COUNT, `source-of-truth exposes exactly ${SOURCE_LIFECYCLE_COMPLETION_EXPECTED_SOURCE_COUNT} source contracts`, String(sourceContracts.length))
