...
export const CHANGE_LOG_COMPREHENSIVE_BASELINE_PATH = 'docs/_archive/audits/2026-04-30-change-log-comprehensive-baseline.md'
export const CHANGE_LOG_COMPREHENSIVE_MANUAL_REVIEW_PATH = 'docs/_archive/audits/2026-04-30-change-log-comprehensive-manual-review.md'
...
function buildEntryFromCloseout(closeout, typeKey = 'build_closeout') {
  const label = typeLabels.get(typeKey) || typeKey
  const sourceId = closeout?.key || typeKey
  return {
    id: `closeout:${sourceId}:${typeKey}`.replace(/\s+/g, '-'),
    occurredAt: null,
    title: normalizeText(closeout?.key || 'Build closeout'),
    summary: normalizeText(closeout?.whatChanged || closeout?.whatItDoes || 'Build closeout record.'),
    changeType: typeKey,
    changeTypeLabel: label,
    surface: closeout?.systemArea || classifySurface(typeKey, { systemArea: 'Foundation Recent Work' }, null),
    sourceKind: 'verified_closeout',
    sourceId,
    evidenceRefs: unique([
      closeout?.key ? `closeout:${closeout.key}` : '',
      normalizeList(closeout?.proofCommands)[0] ? `proof:${normalizeList(closeout?.proofCommands)[0]}` : '',
      ...normalizeList(closeout?.whereItLives).slice(0, 4).map(value => `where:${value}`),
    ]),
    ...
    files: normalizeList(closeout?.whereItLives).map(safeEvidencePath).slice(0, 12),
    confidence: 'verified',
    highlightReason: 'verified closeout registry',
...
export function buildFoundationChangeLog({
  ...
  if (!buildEntries.some(entry => entry.closeoutKey === CHANGE_LOG_COMPREHENSIVE_CLOSEOUT_KEY)) {
    const changeLogCloseout = (closeouts || []).find(closeout => closeout?.key === CHANGE_LOG_COMPREHENSIVE_CLOSEOUT_KEY)
    if (changeLogCloseout) buildEntries.push(buildEntryFromCloseout(changeLogCloseout))
  }
  const representedCloseoutKeys = new Set(buildEntries.map(entry => entry.closeoutKey).filter(Boolean))
  const supplementalVerifiedCloseouts = (closeouts || [])
    .filter(closeout =>
      closeout?.key &&
      !representedCloseoutKeys.has(closeout.key) &&
      /verified/i.test(closeout.acceptanceState || '')
    )
    .slice(0, CHANGE_LOG_VERIFIED_CLOSEOUT_MINIMUM)
  for (const closeout of supplementalVerifiedCloseouts) {
    buildEntries.push(buildEntryFromCloseout(closeout))
    representedCloseoutKeys.add(closeout.key)
  }
  ...
  allEntries
    .filter(entry => entry.sourceKind === 'verified_closeout' && /verified/i.test(entry.acceptanceState || ''))
    .slice(0, CHANGE_LOG_VERIFIED_CLOSEOUT_MINIMUM)
    .forEach(addSelected)
  allEntries
