...
function outOfScopeRoutesRemainOutsideModule(serverSource = '', moduleSource = '') {
  return includesAll(serverSource, OUT_OF_SCOPE_ROUTE_MARKERS) &&
...
export function buildAgentFeedbackRoutesSplitDogfoodProof({ serverSource = '', moduleSource = '', proofScriptSource = '' } = {}) {
  ...
  return {
    ...
    summary: 'Agent Feedback route split dogfood accepts healthy public route ownership and rejects missing module, old inline server ownership, missing registrar, moved admin dry-run routes, weak proof, and raw token logging proof.',
