...
export function registerAgentFeedbackRoutes(app, deps = {}) {
  ...
  async function sendAlreadySubmittedForExpiredTokenIfMatched(token, res) {
    try {
      const session = verifyAgentFeedbackToken(token, Date.now(), { allowExpired: true })
      ...
      if (!existingResponse && !existingMilestoneResponse) return false
      sendApiError(
        res,
        409,
  ...
  app.get('/api/agent-feedback/session', async (req, res) => {
    try {
      ...
      if (error instanceof Error && /expired/i.test(error.message) && await sendAlreadySubmittedForExpiredTokenIfMatched(req.query.token, res)) {
        return
      }
  ...
  app.post('/api/agent-feedback/submit', async (req, res) => {
    try {
      ...
      if (error instanceof Error && /expired/i.test(error.message) && await sendAlreadySubmittedForExpiredTokenIfMatched(req.body?.token, res)) {
        return
      }
      if (error instanceof Error && /already been used/i.test(error.message)) {
        sendApiError(
