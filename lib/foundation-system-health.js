...
const CLASSIFIED_FINDING_REPAIR_CARDS = {
  ...
  adminDealPolicy: 'ADMIN-DEAL-POLICY-SOURCE-CONTRACT-001',
  foundationVerifyHealthRepair: 'FOUNDATION-VERIFY-HEALTH-REPAIR-001',
  agentFeedbackAutoSend: 'AGENT-FEEDBACK-AUTO-SEND-001',
  agentFeedbackProductionAutoSend: 'AGENT-FEEDBACK-PRODUCTION-AUTOSEND-ENABLE-001',
  buildLaneActionGate: 'BUILD-LANE-REPEATED-FAILURE-ACTION-GATE-001',
}
...
function rawCoverageForFinding(finding = {}, rollupLevel = '') {
  const field = rollupLevel === 'risk' ? 'rawRiskCoverage' : 'rawWatchCoverage'
  const value = Math.floor(Number(finding[field] || 0))
...
function classifySystemHealthFinding(finding = {}) {
  ...
  if (id === 'scheduled_job_foundation-verify') {
    return buildFindingClassification({
      type: 'scheduled_verifier_repair_lane',
      owner: 'Foundation Builder',
      reason: 'The scheduled Foundation Verifier failed inside verifier/Agent Feedback proof, not inside the current Dev Hub readback card. It must stay red until the verifier failure is repaired or the latest scheduled run succeeds; do not rerun Agent Feedback send lanes from System Health.',
      threshold: 'Blocks green and normal sprint progression while the scheduled verifier latest run is failed. Manual foundation:verify success proves current code can pass, but it does not erase the scheduled-job failure row.',
      nextAction: 'Repair the Agent Feedback verifier/orchestration failure or rerun only the approved scheduled verifier path after the repair; do not send Agent Feedback email, mutate ClickUp, or change production send posture from this classifier.',
      repairCardId: CLASSIFIED_FINDING_REPAIR_CARDS.foundationVerifyHealthRepair,
      relatedRepairCardIds: [
        CLASSIFIED_FINDING_REPAIR_CARDS.agentFeedbackAutoSend,
        CLASSIFIED_FINDING_REPAIR_CARDS.agentFeedbackProductionAutoSend,
      ],
      blocksCurrentSprint: true,
    })
  }
  if (id === 'scheduled_job_admin-deal-backlog-review') {
    return buildFindingClassification({
      type: 'approval_bound_operational_write_lane',
      owner: 'Steve',
      reason: 'Admin deal backlog review writes governed Ops review/action/finding rows across Owners/FUB/ClickUp posture; a failed stale active-run row must stay parked until Steve approves the operational-write rerun or a safe read-only preflight narrows it.',
      threshold: 'Blocks sprint if rerun outside the Admin Deal policy/source contract, if required for the active card, or if it remains failed after an approved safe rerun.',
      nextAction: 'Park the failed operational-write row; use ADMIN-DEAL-POLICY-SOURCE-CONTRACT-001 as the source contract before any approved rerun.',
      repairCardId: CLASSIFIED_FINDING_REPAIR_CARDS.adminDealPolicy,
      blocksCurrentSprint: true,
    })
  }
  if (id === 'scheduled_job_conditional-deal-review-readonly') {
    return buildFindingClassification({
      type: 'approval_bound_business_source_read_lane',
      owner: 'Steve',
      reason: 'Conditional Deal Forecast Sync reads governed Owners/Google Workspace business-source data and currently fails at delegated Google auth. The row is a business-source repair signal, not a reason to rerun broad connector or sheet work without approval.',
      threshold: 'Blocks green while the job remains failed and unrerouted; blocks any Sales/Ops/business-source card that depends on conditional deal forecast freshness until an approved read-only preflight or credential repair succeeds.',
      nextAction: 'Park the failed read-only business-source row; use ADMIN-DEAL-POLICY-SOURCE-CONTRACT-001 and connector health proof before any approved rerun or credential repair.',
      repairCardId: CLASSIFIED_FINDING_REPAIR_CARDS.adminDealPolicy,
      relatedRepairCardIds: [
        CLASSIFIED_FINDING_REPAIR_CARDS.connectorMonitor,
      ],
      blocksCurrentSprint: true,
    })
  }
  if (id === 'runtime_jobs_failed') {
    return buildFindingClassification({
      ...
      reason: 'The remaining recent job failures are routed verifier, business-source, backfill, or operational-write rows; connector uptime has a later successful safe run and repeated build-lane reds are clear.',
      ...
      nextAction: 'Keep the failed-run ledger visible while FOUNDATION-VERIFY-HEALTH-REPAIR-001, EXTRACT-BACKFILL-001, and ADMIN-DEAL-POLICY-SOURCE-CONTRACT-001 own the governed repair proof.',
      repairCardId: CLASSIFIED_FINDING_REPAIR_CARDS.foundationVerifyHealthRepair,
      relatedRepairCardIds: [
        CLASSIFIED_FINDING_REPAIR_CARDS.foundationVerifyHealthRepair,
        CLASSIFIED_FINDING_REPAIR_CARDS.extractBackfill,
        CLASSIFIED_FINDING_REPAIR_CARDS.adminDealPolicy,
      ],
      blocksCurrentSprint: true,
    })
  }
  if (id.startsWith('build_lane_failure_')) {
    return buildFindingClassification({
      type: 'watch_threshold',
      owner: 'Foundation Process',
      reason: 'Build-lane failure telemetry is already evaluated by the repeated-failure action gate; yellow fingerprints stay visible as watch rows instead of becoming unowned System Health noise.',
      threshold: 'Blocks sprint when the action gate reports an unsatisfied red item or the fingerprint reaches the red threshold without a live repair route; yellow watch rows remain visible.',
      nextAction: finding.nextAction || 'Watch this fingerprint through BUILD-LANE-REPEATED-FAILURE-ACTION-GATE-001.',
      repairCardId: CLASSIFIED_FINDING_REPAIR_CARDS.buildLaneActionGate,
    })
  }
...
export function classifyFoundationSystemHealthFindings(findings = [], { rawRiskCount = 0, rawWatchCount = 0 } = {}) {
  ...
  const visibleRiskRawCoverage = visibleRiskFindings.reduce((sum, finding) => sum + rawCoverageForFinding(finding, 'risk'), 0)
  const visibleWatchRawCoverage = visibleWatchFindings.reduce((sum, finding) => sum + rawCoverageForFinding(finding, 'watch'), 0)
  const hiddenRiskCount = Math.max(0, effectiveRawRiskCount - visibleRiskRawCoverage)
  const hiddenWatchCount = Math.max(0, effectiveRawWatchCount - visibleWatchRawCoverage)
  const unclassifiedRiskCount = unclassifiedRiskFindings.reduce((sum, finding) => sum + rawCoverageForFinding(finding, 'risk'), 0) + hiddenRiskCount
  const unclassifiedWatchCount = unclassifiedWatchFindings.reduce((sum, finding) => sum + rawCoverageForFinding(finding, 'watch'), 0) + hiddenWatchCount
  return {
...
export function buildFoundationSystemHealthSnapshot({
  ...
  const coverageAnnotatedRawFindings = rawFindings.map(finding => {
    const id = String(finding.id || '')
    if (id === 'connector_down') {
      return { ...finding, rawRiskCoverage: Math.max(1, Number(connectors?.summary?.downCount || 0)) }
    }
    if (id === 'connector_degraded') {
      return { ...finding, rawWatchCoverage: Math.max(1, Number(connectors?.summary?.degradedCount || 0)) }
    }
    if (id === 'connector_blocked') {
      return { ...finding, rawWatchCoverage: Math.max(1, Number(connectors?.summary?.blockedCount || 0)) }
    }
    return finding
  })
  const classifiedFindings = classifyFoundationSystemHealthFindings(coverageAnnotatedRawFindings, {
    rawRiskCount,
    rawWatchCount,
  })
...
export function buildFoundationSystemHealthReportMarkdown(snapshot = {}) {
  ...
  return [
    ...
    '## Summary',
    '',
    `- Unclassified risk findings: ${summary.unclassifiedRiskCount || 0}`,
    `- Unclassified watch findings: ${summary.unclassifiedWatchCount || 0}`,
    `- Blocking classified risk/watch rows: ${summary.blockingClassifiedRiskCount || 0}/${summary.blockingClassifiedWatchCount || 0}`,
