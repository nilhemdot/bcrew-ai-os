...
function classifyScheduledJob(job = {}, now = new Date(), graceMinutes = SCHEDULED_JOB_STALENESS_GRACE_MINUTES) {
  ...
  if (activeRun) {
    return {
      ...base,
      status: activeRunStale ? 'risk' : 'watch',
      color: activeRunStale ? 'red' : 'yellow',
...
export function buildFoundationSystemHealthSnapshot({
  ...
  return {
    ...
    plainEnglish: status === 'risk'
      ? 'Foundation has unclassified red system-health findings. Do not treat every surface as green.'
      : status === 'watch'
        ? 'Foundation is usable but has unclassified yellow system-health findings to review.'
        : (rawRiskCount + rawWatchCount > 0)
            ? 'Foundation has no unclassified red or yellow system-health findings; classified approval-bound and threshold rows remain visible with owners and next actions.'
...
export function buildFoundationSystemHealthDogfoodProof() {
  ...
  const checks = [
    ...
    {
      ok: scheduledJobs.rows.some(row => row.key === 'fresh-running' && row.status === 'watch' && row.color === 'yellow'),
      check: 'fresh active scheduled job renders watch instead of red',
    },
    {
