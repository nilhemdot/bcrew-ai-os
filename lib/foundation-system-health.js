...
function classifySystemHealthFinding(finding = {}) {
  ...
  if (id === 'runtime_jobs_failed') {
    return buildFindingClassification({
      ...
      reason: 'The remaining recent job failures are approval-bound backfill or operational-write rows; connector uptime has a later successful safe run and repeated build-lane reds are clear.',
      ...
      nextAction: 'Keep the failed-run ledger visible while EXTRACT-BACKFILL-001 and ADMIN-DEAL-POLICY-SOURCE-CONTRACT-001 own the governed repair proof.',
      repairCardId: CLASSIFIED_FINDING_REPAIR_CARDS.extractBackfill,
      relatedRepairCardIds: [
