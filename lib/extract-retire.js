...
export function buildExtractRetireDecision({
  ...
  const retirementDisabled = runInput.retireEligible === false || runInput.disableRetirement === true
  const shouldRetire = !retirementDisabled && eligible && !alreadyTerminal && zeroWork && cleanZeroWorkRunStreak >= cleanRunThreshold
  const reason = currentDay
    ? 'current_day_targets_remain_scheduled'
    : retirementDisabled
      ? 'retirement_disabled_for_exact_external_id_run'
      : !eligible
