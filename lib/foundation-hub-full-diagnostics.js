...
function buildDiagnosticErrorSourceHealth({
  label,
  error,
  checkedAt = nowIso(),
} = {}) {
  const unavailable = buildUnavailableClickUpListSnapshot(CLICKUP_AGENT_ROSTER_LIST_ID, error, {
    listName: 'Agent Roster',
    checkedAt,
  })
  return {
    ...(unavailable.sourceHealth || {}),
    message: `${label || 'Agent Feedback diagnostic'} source-health read failed: ${unavailable.sourceHealth?.message || 'ClickUp source unavailable.'}`,
    failSoft: true,
  }
}

function normalizeDiagnosticReason(sourceHealth = null, fallback = 'runtime_diagnostic_timeout') {
  return sourceHealth?.reason || fallback
}

function buildTimeoutReport({ label, mode, sourceHealth, generatedAt = nowIso(), reason = 'runtime_diagnostic_timeout' } = {}) {
  const diagnosticReason = normalizeDiagnosticReason(sourceHealth, reason)
  const timedOut = diagnosticReason === 'runtime_diagnostic_timeout'
  return {
    mode,
    sourceId: AGENT_FEEDBACK_SEND_SOURCE_ID,
    listIdHash: '',
...
function buildAutoSendTimeoutFallback({
  deadlineMs,
  elapsedMs = deadlineMs,
  sourceHealth = null,
  reason = 'runtime_diagnostic_timeout',
} = {}) {
  const normalizedSourceHealth = sourceHealth || buildDiagnosticTimeoutSourceHealth({ label: 'Agent Feedback Auto-Send', deadlineMs })
  const diagnosticReason = normalizeDiagnosticReason(normalizedSourceHealth, reason)
  const timedOut = diagnosticReason === 'runtime_diagnostic_timeout'
  return {
    status: 'risk',
    ...
    summary: {
      ...
      failClosedReasons: [diagnosticReason],
      liveGuardDecision: diagnosticReason,
...
function buildProductionDryRunTimeoutFallback({
  deadlineMs,
  elapsedMs = deadlineMs,
  sourceHealth = null,
  reason = 'runtime_diagnostic_timeout',
} = {}) {
  const normalizedSourceHealth = sourceHealth || buildDiagnosticTimeoutSourceHealth({ label: 'Agent Feedback Production Dry-Run', deadlineMs })
  const diagnosticReason = normalizeDiagnosticReason(normalizedSourceHealth, reason)
  const timedOut = diagnosticReason === 'runtime_diagnostic_timeout'
  return {
...
function buildReminderTimeoutFallback({
  deadlineMs,
  elapsedMs = deadlineMs,
  sourceHealth = null,
  reason = 'runtime_diagnostic_timeout',
} = {}) {
  const normalizedSourceHealth = sourceHealth || buildDiagnosticTimeoutSourceHealth({ label: 'Agent Feedback Reminders', deadlineMs })
  const diagnosticReason = normalizeDiagnosticReason(normalizedSourceHealth, reason)
  const timedOut = diagnosticReason === 'runtime_diagnostic_timeout'
  return {
    status: 'risk',
  ...
  try {
    const runPromise = Promise.resolve()
      ...
      .catch(error => fallback({
        ...
        label,
        sourceHealth: buildDiagnosticErrorSourceHealth({ label, error }),
...
export async function buildSyntheticFoundationFullDiagnosticsDogfoodProof() {
  ...
  const clickUpRateLimitError = new Error('ClickUp GET /list/901113292355/field returned 429: {"err":"Rate limit reached","ECODE":"APP_002"}')
  const rateLimitBuilder = () => Promise.reject(clickUpRateLimitError)
  const startedAtMs = Date.now()
  ...
  const rateLimitDiagnostics = await buildFoundationHubAgentFeedbackDiagnostics({
    repoRoot: process.cwd(),
    foundationJobs: { jobs: [], latestRuns: [] },
    deadlineMs: 50,
    clickUpRequestTimeoutMs: 5,
    clickUpMaxPages: 1,
    builders: {
      autoSendReadiness: rateLimitBuilder,
      productionDryRun: rateLimitBuilder,
      reminderReadiness: rateLimitBuilder,
    },
  })
  const elapsedMs = Date.now() - startedAtMs
  const boundary = buildFoundationHubSourceOutageBoundary(diagnostics)
  const rateLimitBoundary = buildFoundationHubSourceOutageBoundary(rateLimitDiagnostics)
  return {
    ok: elapsedMs < 100 &&
      ...
      rateLimitDiagnostics.agentFeedbackAutoSend?.sourceHealth?.reason === 'external_api_error' &&
      rateLimitDiagnostics.agentFeedbackAutoSend?.summary?.sourceUnavailable === true &&
      rateLimitDiagnostics.agentFeedbackAutoSend?.report?.sideEffects?.gmailSent === false &&
      rateLimitDiagnostics.agentFeedbackAutoSend?.report?.sideEffects?.clickUpRequestedWritten === false &&
      rateLimitDiagnostics.agentFeedbackProductionAutoSendDryRun?.sourceHealth?.reason === 'external_api_error' &&
      rateLimitDiagnostics.agentFeedbackReminders?.sourceHealth?.reason === 'external_api_error' &&
      boundary.status === 'degraded' &&
      rateLimitBoundary.status === 'degraded' &&
    ...
    boundary,
    rateLimitBoundary,
    invariant: 'Slow or rate-limited optional ClickUp/Agent Feedback panels degrade as source health and do not hold the full diagnostics route open.',
  }
}
