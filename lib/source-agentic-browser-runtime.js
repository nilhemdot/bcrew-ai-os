...
export const SOURCE_AGENTIC_BROWSER_BROWSERBASE_ALLOWED = false
...
const STAGEHAND_MODEL_PROVIDER_RE = /^(openai|anthropic|google)\/[a-z0-9][a-z0-9._:-]*/i
...
export function evaluateSourceAgenticBrowserModelRoute(model = SOURCE_AGENTIC_BROWSER_DEFAULT_MODEL) {
  ...
  if (UNSUPPORTED_SUBSCRIPTION_MODEL_RE.test(normalized) || !STAGEHAND_MODEL_PROVIDER_RE.test(normalized)) {
    return {
      ...
      requiredFormat: 'openai/<model>, anthropic/<model>, or google/<model>',
    }
  }
  return {
    ok: true,
...
export function buildSourceAgenticBrowserCostPolicy({
  ...
  if (browserEnv === 'BROWSERBASE') {
    findings.push({
      check: 'browserbase_parked_outside_human_web_agent_v1',
      reason: 'Browserbase is parked outside Human Web Agent V1 and cannot run from env keys, CLI flags, or stale bakeoff approval.',
      nextAction: 'Use LOCAL, source:local-browser-hands, connectors, Source Session Broker, or Harlan/operator escalation.',
    })
  }
...
export function buildSourceAgenticBrowserCostGuardrailDogfood() {
  ...
  const cases = [
    ...
    {
      name: 'browserbase_blocks_even_with_env_request',
      ok: browserbaseNoAllow.ok === false &&
        browserbaseNoAllow.findings.some(finding => finding.check === 'browserbase_parked_outside_human_web_agent_v1'),
    ...
    {
      name: 'broad_browserbase_loop_blocks_at_parked_boundary',
      ok: tooBroadBrowserbase.ok === false &&
        tooBroadBrowserbase.findings.some(finding => finding.check === 'browserbase_parked_outside_human_web_agent_v1'),
      status: tooBroadBrowserbase.status,
    },
    {
      name: 'stale_tiny_browserbase_bakeoff_flag_still_blocks',
      ok: proofBrowserbase.ok === false &&
        proofBrowserbase.browserHoursMetered === false &&
        proofBrowserbase.findings.some(finding => finding.check === 'browserbase_parked_outside_human_web_agent_v1'),
...
export async function runSourceAgenticBrowserProbe({
  ...
  try {
    ...
    if (firstSnapshot.browserHealth?.ok === false) {
      ...
      agentResult = {
        ...
        recovery: 'retry clean source-browser session or route to Harlan/operator escalation before extraction',
      }
    } else {
      const observePrompt = [
  ...
  const report = {
    ...
    next: status === 'agentic_browser_blocked_browser_state'
      ? 'Retry with a clean isolated source session or Harlan/operator escalation before extracting; do not reason from a blank/restore browser page.'
      : status === 'agentic_browser_completed_with_blocker'
      ? 'If the blocker is auth/private/CAPTCHA, route to Source Session Broker or Harlan/operator escalation instead of manual browser babysitting.'
