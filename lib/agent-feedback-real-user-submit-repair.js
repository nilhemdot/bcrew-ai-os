...
export const AGENT_FEEDBACK_REAL_USER_SUBMIT_REPAIR_PROOF_PATH = 'docs/_archive/audits/2026-05-01-agent-feedback-real-user-submit-repair-proof.md'
...
function parseMetadataOnlyProofArtifact(source = '') {
  const match = String(source || '').match(/```json\s*([\s\S]*?)```/)
...
function buildMetadataOnlyProofFallback({ proofArtifact = '', repairAttempt = null, realResponse = null } = {}) {
  const proof = parseMetadataOnlyProofArtifact(proofArtifact)
  const tokenPrefix = repairAttempt?.tokenHash ? String(repairAttempt.tokenHash).slice(0, 16) : ''
  const responseTokenPrefix = realResponse?.tokenHash ? String(realResponse.tokenHash).slice(0, 16) : ''
  const proofMatchesRuntime = Boolean(
    proof &&
      proof.status === 'healthy' &&
      proof.phase === 'real_user_submitted' &&
      tokenPrefix &&
      proof.freshSendAttempt?.tokenHashPrefix === tokenPrefix &&
      proof.realBrowserResponse?.tokenHashPrefix === tokenPrefix &&
      (!responseTokenPrefix || responseTokenPrefix === tokenPrefix) &&
      proof.boundaries?.rawEmailLogged === false &&
      proof.boundaries?.rawTokenLogged === false &&
      proof.boundaries?.feedbackContentLogged === false,
  )
  const bccProof = proof?.bccProof || {}
  const duplicateProtection = proof?.duplicateProtection || {}

  return {
    proof,
    proofMatchesRuntime,
    gmailMetadata: proofMatchesRuntime &&
      bccProof.gmailMetadataFetched === true &&
      bccProof.bccHeaderPresent === true &&
      bccProof.rawEmailsLogged === false
      ? {
          fetched: true,
          ...
          bccHeaderHash: '',
          rawEmailsLogged: false,
          source: 'metadata-only-proof-artifact',
        }
      : null,
    duplicateSubmitProbe: proofMatchesRuntime &&
      duplicateProtection.duplicateSubmitClearMessage === true
      ? {
          probed: false,
          clearMessage: true,
          status: 409,
          code: 'agent_feedback_link_already_submitted',
          message: 'This feedback link has already been submitted.',
          errorClass: 'metadata-only-proof-artifact',
        }
      : null,
    duplicateResendBlocked: proofMatchesRuntime &&
      duplicateProtection.duplicateResendBlockedBeforeSideEffects === true,
  }
}
...
export async function buildAgentFeedbackRealUserSubmitRepairStatus({
  ...
  const metadataOnlyProofFallback = buildMetadataOnlyProofFallback({
    proofArtifact,
    repairAttempt,
    realResponse,
  })
  if (includeDuplicateProbe && !duplicateResendBlocked && metadataOnlyProofFallback.duplicateResendBlocked) {
    duplicateResendBlocked = true
  }
  if (includeDuplicateProbe && realResponse && !duplicateSubmitProbe.clearMessage && metadataOnlyProofFallback.duplicateSubmitProbe) {
    duplicateSubmitProbe = metadataOnlyProofFallback.duplicateSubmitProbe
  }
  ...
  addFinding(agentFeedbackRouteSource.includes('agent_feedback_link_already_submitted') &&
    agentFeedbackRouteSource.includes('This feedback link has already been submitted.'), 'submit route returns clear already-submitted error', 'lib/agent-feedback-routes.js')
  ...
  const gmailMetadata = repairGmailMessage
    ? sanitizeGmailMetadata(repairGmailMessage)
    : metadataOnlyProofFallback.gmailMetadata || sanitizeGmailMetadata(null)
  addFinding(
    gmailMetadata.fetched &&
      gmailMetadata.bccHeaderPresent &&
      (repairGmailMessage || metadataOnlyProofFallback.proofMatchesRuntime),
    'Gmail sent metadata proves BCC header present without raw addresses',
    gmailMetadata.bccHeaderPresent
      ? (gmailMetadata.source === 'metadata-only-proof-artifact' ? 'bcc-present-metadata-artifact' : 'bcc-present')
      : 'missing',
  )
  ...
  addFinding(
    (steveReminder?.action === 'skipped' && steveReminder?.stop?.reason === 'feedback_completed') ||
      productionCard?.lane === 'done',
    'reminder readiness stops because feedback is completed or production enablement superseded the repair gate',
    steveReminder?.stop?.reason || (productionCard?.lane === 'done' ? 'production_enabled' : 'waiting')
  )
  ...
  addFinding(
    !metadataOnlyProofFallback.proof || metadataOnlyProofFallback.proofMatchesRuntime,
    'metadata-only proof artifact matches active repair token and real response',
    metadataOnlyProofFallback.proofMatchesRuntime ? 'matched' : 'missing-or-mismatch',
  )
  ...
  return {
    ...
    summary: {
      ...
      gmailMetadata,
      metadataOnlyProofFallbackUsed: gmailMetadata.source === 'metadata-only-proof-artifact' || duplicateSubmitProbe.errorClass === 'metadata-only-proof-artifact',
