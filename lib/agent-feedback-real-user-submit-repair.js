...
export async function buildAgentFeedbackRealUserSubmitRepairStatus({
  ...
  const closeout = (foundationBuildLog?.builds || []).find(build => build.closeoutKey === AGENT_FEEDBACK_REAL_USER_SUBMIT_REPAIR_CLOSEOUT_KEY) || null
  ...
  addFinding(productionCard?.lane === 'scoped', 'production auto-send remains scoped', productionCard?.lane || 'missing')
