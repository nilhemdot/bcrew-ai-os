...
export function buildSourceSessionActionGroupReadiness({
  ...
  if (normalizedPhase === 'newsletter_live_signup_approval_needed') {
    checks.push(buildReadinessCheck({
      checkId: 'newsletter-live-signup-approval',
      label: 'Newsletter live signup approval',
      kind: 'external_submit_approval_required',
      status: 'waiting_steve_approval',
      source: CREATOR_NEWSLETTER_SOURCE,
      account,
      secretRef: CREATOR_NEWSLETTER_MAILBOX_SOURCE_ID,
      statusCommand: topUrl ? `npm run newsletter:intake -- --url=${topUrl} --account=${account} --apply --allow-external-signup --standing-policy-approved --confirmation-readback-required --json` : '',
      setupCommand: '',
      commandEffect: 'external_submit_if_run',
      nextAction: 'Use this command only after Steve approves one exact live newsletter signup. Gmail confirmation readback is mandatory before subscribed status or issue extraction.',
      unlocks: 'One confirmed newsletter source that can later feed recurring issue extraction.',
    }))
    checks.push(buildReadinessCheck({
      checkId: 'newsletter-source-identity',
      label: 'Newsletter source identity',
      ...
      commandEffect: 'metadata_only_if_run',
      blocksReadiness: false,
      nextAction: 'Use the approved AIOS source mailbox for free newsletters; live external submit still waits for Steve approval.',
      unlocks: 'Future recurring creator newsletter extraction.',
    }))
    checks.push(buildReadinessCheck({
      checkId: 'newsletter-confirmation-readback',
  ...
  if (normalizedPhase === 'newsletter_intake_review_needed') {
    checks.push(buildReadinessCheck({
      checkId: 'newsletter-intake-failure-review',
      label: 'Newsletter intake failure review',
      kind: 'saved_packet_review_required',
      status: 'review_required',
      source: CREATOR_NEWSLETTER_SOURCE,
      account,
      statusCommand: topUrl ? `npm run newsletter:intake -- --url=${topUrl} --account=${account} --json` : '',
      setupCommand: '',
      commandEffect: 'metadata_only_if_run',
      nextAction: 'Inspect the saved blocked/not-newsletter packet. Do not run live signup from failed intake rows.',
      unlocks: 'Separates true newsletter signup candidates from dead, blocked, or non-newsletter links.',
    }))
  }
