...
export function buildResearchCurationStatus({ backlogItems = [], foundationReviewSprint = null } = {}) {
  const researchCards = (backlogItems || []).filter(item => item.lane === 'research')
  const sprintSummary = foundationReviewSprint?.summary
  const fullSprintDispositions = foundationReviewSprint?.researchCuration?.allDispositions ||
    null
  const fullDispositionById = new Map((fullSprintDispositions || []).map(item => [item.cardId, item]))
  const taggedCards = researchCards.map(item => ({
    id: item.id,
    title: item.title,
    curationState: fullDispositionById.get(item.id)?.disposition || 'leave_parked',
    subTag: fullDispositionById.get(item.id)?.subTag || (/audit|review|research/i.test(`${item.title} ${item.summary || ''}`) ? 'research' : 'future-build'),
    plainEnglish: fullDispositionById.get(item.id)?.reason || 'Preserved in Research. This card is not auto-closed by Phase D.',
  }))
  ...
  return {
    ...
    summary: {
      ...
      foundation1100DispositionOnly: sprintSummary?.researchCardsDispositionOnly || 0,
    },
    cards: taggedCards,
    knownLimits: [
