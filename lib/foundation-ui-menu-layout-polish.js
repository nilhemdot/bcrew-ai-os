...
import { findFoundationBuildCloseoutEntry } from './foundation-build-log-source.js'
...
export const UI_MENU_LAYOUT_POLISH_BASELINE_PATH = 'docs/_archive/audits/2026-04-30-ui-menu-layout-polish-baseline.md'
export const UI_MENU_LAYOUT_POLISH_MANUAL_REVIEW_PATH = 'docs/_archive/audits/2026-04-30-ui-menu-layout-polish-manual-review.md'
...
const UI_MENU_PHASE_G_CLOSEOUT_BY_CARD_ID = {
  'CHANGE-LOG-COMPREHENSIVE-001': 'change-log-comprehensive-v1',
  'DAILY-EXEC-SUMMARY-001': 'daily-exec-summary-v1',
  'SOURCE-LIFECYCLE-EXPANSION-001': 'source-lifecycle-expansion-v1',
}

function closeoutKeyForPhaseGCard(cardId) {
  return UI_MENU_PHASE_G_CLOSEOUT_BY_CARD_ID[cardId] || ''
}
...
export async function buildUiMenuLayoutPolishStatus({
  ...
  addFinding(findings, foundationFrontendJs.includes('renderInventoryArchiveHistory'), 'archive/history renderer exists', 'renderInventoryArchiveHistory')
  addFinding(findings, foundationFrontendJs.includes("'inventory-archive-history'"), 'archive/history route is registered', 'inventory-archive-history')
  addFinding(findings, foundationFrontendJs.includes('splitInventoryDocs'), 'current/archive split helper exists in UI', 'splitInventoryDocs')
  ...
  for (const category of UI_MENU_CURRENT_DOC_CATEGORIES) {
    addFinding(findings, foundationFrontendJs.includes(category), `current-doc category listed: ${category}`, category)
  }
  for (const category of UI_MENU_ARCHIVE_HISTORY_CATEGORIES) {
    addFinding(findings, foundationFrontendJs.includes(category), `archive/history category listed: ${category}`, category)
  }
  for (const route of UI_MENU_LAYOUT_REQUIRED_ROUTES) {
    addFinding(findings, routeTextPresent(foundationHtml + foundationFrontendJs, route), `route exists: ${route}`, route)
  }
  ...
  const completedPhaseGCards = Array.isArray(phaseGReadiness.completedCards) ? phaseGReadiness.completedCards : []
  const sourceLifecycleCard = (foundationHub?.backlogItems || []).find(card => card.id === 'SOURCE-LIFECYCLE-EXPANSION-001')
  const sourceLifecycleBuild = findFoundationBuildCloseoutEntry(foundationBuildLog, 'source-lifecycle-expansion-v1', { backlogId: 'SOURCE-LIFECYCLE-EXPANSION-001' })
  const build = findFoundationBuildCloseoutEntry(foundationBuildLog, UI_MENU_LAYOUT_POLISH_CLOSEOUT_KEY, { backlogId: UI_MENU_LAYOUT_POLISH_CARD_ID })
  const phaseGTrack2Complete = !phaseGReadiness.nextPlanCard &&
    (
      completedPhaseGCards.includes('SOURCE-LIFECYCLE-EXPANSION-001') ||
      (
        sourceLifecycleCard?.lane === 'done' &&
        (
          /source-lifecycle-expansion-v1/.test(sourceLifecycleCard?.statusNote || '') ||
          Boolean(sourceLifecycleBuild)
        )
      )
    )
  if (foundationHub) {
    addFinding(
      findings,
      UI_MENU_LAYOUT_ALLOWED_NEXT_CARD_IDS.includes(phaseGReadiness.nextPlanCard) || phaseGTrack2Complete,
      'live current-truth next card advanced after closeout',
      phaseGReadiness.nextPlanCard || (phaseGTrack2Complete ? 'phase-g-track-2-complete' : 'missing'),
    )
    const uiCard = (foundationHub.backlogItems || []).find(card => card.id === UI_MENU_LAYOUT_POLISH_CARD_ID)
    addFinding(
      findings,
      uiCard?.lane === 'done' && (
        /ui-menu-layout-polish-v1/.test(uiCard?.statusNote || '') ||
        Boolean(build)
      ),
      'UI menu layout card is done with closeout key',
      `${uiCard?.lane || 'missing'} / ${uiCard?.statusNote || build?.closeoutKey || ''}`,
    )
    for (const cardId of UI_MENU_LAYOUT_FORBIDDEN_PHASE_G_CARD_IDS) {
      ...
      const separateCloseout = Boolean(findFoundationBuildCloseoutEntry(foundationBuildLog, closeoutKeyForPhaseGCard(cardId), { backlogId: cardId }))
      addFinding(
        findings,
        card?.lane !== 'done' || separateCloseout,
        `future Phase G card has separate closeout if done: ${cardId}`,
        `${card?.lane || 'missing'} / ${separateCloseout ? 'separate closeout' : 'no separate closeout'}`,
      )
    }
  }
  ...
  return {
    ...
    summary: {
      ...
      nextPlanCardStillAfterUiMenu: UI_MENU_LAYOUT_ALLOWED_NEXT_CARD_IDS.includes(phaseGReadiness.nextPlanCard) || phaseGTrack2Complete,
    },
    findings,
  }
}
