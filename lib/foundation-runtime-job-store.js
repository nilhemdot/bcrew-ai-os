...
function normalizeMaxRuntimeSecondsByJob(input = {}) {
  return Object.fromEntries(Object.entries(input || {})
    .map(([jobKey, seconds]) => [String(jobKey || '').trim(), Math.floor(Number(seconds) || 0)])
  ...
  return {
    ...
    async query(sql, params = []) {
      ...
      if (/UPDATE foundation_job_runs/.test(sql) && /status = 'failed'/.test(sql)) {
        const runtimeBudgets = params[2] ? JSON.parse(params[2] || '{}') : {}
        const maxRuntimeSeconds = Number(runtimeBudgets[rows.run.job_key] || 0)
        const thresholdSeconds = maxRuntimeSeconds > 0
          ? maxRuntimeSeconds + Number(params[3] || 0)
          : Number(params[0] || 30) * 60
        return {
          rows: [{
            ...rows.run,
            status: 'failed',
            error_message: 'Marked failed by stale active-run reaper.',
            metadata: {
              ...rows.run.metadata,
              staleThresholdSeconds: thresholdSeconds,
              staleRuntimeBudgeted: maxRuntimeSeconds > 0,
            },
          }],
        }
      }
...
export async function buildSyntheticFoundationRuntimeJobStoreBehaviorProof() {
  ...
  const budgetedReaped = await store.markStaleFoundationJobRuns({
    olderThanMinutes: 180,
    maxRuntimeSecondsByJob: { 'code-quality-nightly-audit': 60 },
    graceSeconds: 60,
  }, 'synthetic-proof')
  ...
  addCheck(
    checks,
    budgetedReaped[0]?.status === 'failed' &&
      budgetedReaped[0]?.metadata?.staleRuntimeBudgeted === true &&
      Number(budgetedReaped[0]?.metadata?.staleThresholdSeconds) === 120,
    'stale Foundation job reaper honors per-job runtime budget instead of flat global timeout',
    JSON.stringify(budgetedReaped[0]),
  )
...
async function markStaleFoundationJobRuns({
  olderThanMinutes = 180,
  maxRuntimeSecondsByJob = {},
  graceSeconds = 300,
} = {}, actor = 'system') {
  const normalizedMinutes = Math.max(30, Math.min(24 * 60, Number(olderThanMinutes) || 180))
  const normalizedGraceSeconds = Math.max(0, Math.min(60 * 60, Number(graceSeconds) || 300))
  const runtimeBudgets = normalizeMaxRuntimeSecondsByJob(maxRuntimeSecondsByJob)
  return withFoundationTransaction(async client => {
    const result = await client.query(
      `
        WITH runtime_budgets AS (
          SELECT key AS job_key,
                 GREATEST(1, value::integer) AS max_runtime_seconds
          FROM jsonb_each_text($3::jsonb)
        ),
        selected AS (
          SELECT runs.run_id,
                 COALESCE(runtime_budgets.max_runtime_seconds + $4::int, $1::int * 60) AS stale_threshold_seconds,
                 runtime_budgets.max_runtime_seconds IS NOT NULL AS stale_runtime_budgeted
          FROM foundation_job_runs runs
          LEFT JOIN runtime_budgets ON runtime_budgets.job_key = runs.job_key
          WHERE runs.status IN ('queued', 'running')
            AND COALESCE(runs.started_at, runs.created_at) < NOW() - (
              COALESCE(runtime_budgets.max_runtime_seconds + $4::int, $1::int * 60) * INTERVAL '1 second'
            )
        )
        UPDATE foundation_job_runs
        SET status = 'failed',
            ...
            metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object(
              ...
              'staleReapedAt', NOW(),
              'staleThresholdMinutes', CEIL(selected.stale_threshold_seconds / 60.0)::int,
              'staleThresholdSeconds', selected.stale_threshold_seconds,
              'staleRuntimeBudgeted', selected.stale_runtime_budgeted
            ),
            updated_at = NOW()
        FROM selected
        WHERE foundation_job_runs.run_id = selected.run_id
        RETURNING foundation_job_runs.run_id, job_key, title, job_type, status, command, requested_by,
                  started_at, finished_at, duration_ms, exit_code, signal,
                  output_tail, error_message, metadata, created_at, updated_at
      `,
      [normalizedMinutes, actor, JSON.stringify(runtimeBudgets), normalizedGraceSeconds]
    )
    ...
    for (const row of result.rows) {
      await insertChangeEvent(client, {
        ...
        metadata: {
          jobKey: row.job_key,
          staleThresholdMinutes: row.metadata?.staleThresholdMinutes || normalizedMinutes,
          staleThresholdSeconds: row.metadata?.staleThresholdSeconds || normalizedMinutes * 60,
          staleRuntimeBudgeted: row.metadata?.staleRuntimeBudgeted === true,
        },
      })
    }
