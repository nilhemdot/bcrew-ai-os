...
} from './foundation-identity-surface.js'
...
export const VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_CARD_ID = 'VERIFIER-SOURCE-TRUST-ORCHESTRATION-SPLIT-001'
export const VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_CLOSEOUT_KEY = 'verifier-source-trust-orchestration-split-v1'
export const VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_PLAN_PATH = 'docs/process/verifier-source-trust-orchestration-split-001-plan.md'
export const VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_APPROVAL_PATH = 'docs/process/approvals/VERIFIER-SOURCE-TRUST-ORCHESTRATION-SPLIT-001.json'
export const VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_SCRIPT_PATH = 'scripts/process-verifier-source-trust-orchestration-split-check.mjs'
export const VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-source-trust-orchestration-split-closeout.md'
export const VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_BEFORE_LINES = 6399
...
export function evaluateFoundationSourceTrustVerifier(input = {}) {
  ...
  const foundationIdentityValidation = validateFoundationIdentitySurface(input.systemInventory?.identity || {})
  addCheck(
    checks,
  ...
  addCheck(
    checks,
    foundationIdentityValidation.ok === true &&
      includesAll(foundationFrontendSource, [
        'renderFoundationIdentityPanel',
        'Workspace Identity',
        'metadata-only',
        'bcrew-foundation',
      ]),
    'System Inventory exposes metadata-only Foundation identity surface',
    foundationIdentityValidation.ok
      ? `identity=${input.systemInventory?.identity?.status || 'missing'}`
      : foundationIdentityValidation.checks.filter(check => !check.ok).map(check => check.check).join('; '),
  )
  ...
  addCheck(
    ...
    includesAll(foundationFrontendSource, [
      ...
      !foundationFrontendSource.includes('Phase Gates ↔ Live Backlog') &&
      !foundationFrontendSource.includes('Phase 1 · Truth Cleanup'),
    ...
    systemInventory: {
      ...
      identity: {
        ...
        runtimeCapabilities: {
          ...
          plugins: {
            total: REQUIRED_PLUGIN_NAMES.length,
            skillCount: 9,
...
export async function evaluateFoundationSourceTrustVerifierOrchestration(input = {}) {
  const {
  ...
  const phaseCApprovalFilesPresent = Array.isArray(input.phaseCApprovalFilesPresent)
    ? input.phaseCApprovalFilesPresent
    : await Promise.all(
      PHASE_C_VISIBILITY_CARD_IDS.map(cardId => repoFileExists(`docs/process/approvals/${cardId}.json`)),
    )
  const sourceTrustVerifier = evaluateFoundationSourceTrustVerifier({
    ...input,
    moduleSource: input.moduleSource || foundationSourceTrustVerifierSource,
    foundationVerifySource: input.foundationVerifySource || foundationVerifyRootSource,
    phaseCApprovalFilesPresent,
  })
  const checks = [...sourceTrustVerifier.checks]
  const sourceTrustDogfood = buildFoundationSourceTrustVerifierDogfoodProof()
  ...
  const verifierSourceTrustSplitModuleCard =
    item(VERIFIER_SOURCE_TRUST_SPLIT_MODULE_CARD_ID) ||
    activeSprintItem(VERIFIER_SOURCE_TRUST_SPLIT_MODULE_CARD_ID)
  const verifierSourceTrustSplitModuleCloseout =
    foundationBuildCloseouts.find(closeout => closeout.key === VERIFIER_SOURCE_TRUST_SPLIT_MODULE_CLOSEOUT_KEY) || null
  const verifierSourceTrustSplitModuleClosed = verifierSourceTrustSplitModuleCard?.lane === 'done'
  const sourceTrustOldInlinePatterns = [
    'const required' + 'WorkingConnectorIds =',
    'const expectedGroupedSystemIds =',
    'const kpiHealthBacklog =',
  ]
  const sourceTrustOrchestrationOldRootPatterns = [
  ...
  addCheck(
    checks,
    sourceTrustDogfood.ok === true &&
      sourceTrustDogfood.missingConnector.ok === false &&
      sourceTrustDogfood.staleKpi.ok === false &&
      sourceTrustDogfood.missingReferenceTrust.ok === false &&
      sourceTrustDogfood.stalePhaseCoverage.ok === false,
    'source-trust verifier dogfood rejects source-trust proof failures',
    sourceTrustDogfood.invariant,
  )
  ...
  addCheck(
    ...
    verifierSourceTrustSplitModuleCard &&
      ...
      sourceTrustDogfood.ok === true &&
      sourceTrustVerifier.summary.passed === sourceTrustVerifier.summary.total &&
      ...
      foundationVerifyRootSource.includes('evaluateFoundationSourceTrustVerifierOrchestration({') &&
      foundationVerifyRootSource.includes('sourceTrustOrchestrationVerifier.checks') &&
      sourceTrustOldInlinePatterns.every(pattern => !foundationVerifyRootSource.includes(pattern)) &&
    ...
    verifierSourceTrustSplitModuleCard
      ? `lane=${verifierSourceTrustSplitModuleCard.lane} dogfood=${sourceTrustDogfood.ok ? 'pass' : 'blocked'} sourceTrustChecks=${sourceTrustVerifier.summary.passed}/${sourceTrustVerifier.summary.total} lines=${VERIFIER_SOURCE_TRUST_SPLIT_MODULE_BEFORE_LINES}->${foundationVerifyLineCount}`
      : `missing ${VERIFIER_SOURCE_TRUST_SPLIT_MODULE_CARD_ID}`,
  )

  const verifierSourceTrustOrchestrationCard =
    item(VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_CARD_ID) ||
    activeSprintItem(VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_CARD_ID)
  const verifierSourceTrustOrchestrationCloseout =
    foundationBuildCloseouts.find(closeout => closeout.key === VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_CLOSEOUT_KEY) || null
  addCheck(
    checks,
    verifierSourceTrustOrchestrationCard &&
      ['executing', 'done'].includes(verifierSourceTrustOrchestrationCard.lane) &&
      String(verifierSourceTrustOrchestrationCard.statusNote || '').includes(VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_CLOSEOUT_KEY) &&
      verifierSourceTrustOrchestrationCloseout?.operatorCloseout === true &&
      (verifierSourceTrustOrchestrationCloseout.backlogIds || []).includes(VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_CARD_ID) &&
      sourceTrustDogfood.ok === true &&
      sourceTrustVerifier.summary.passed === sourceTrustVerifier.summary.total &&
      packageJson.scripts?.['process:verifier-source-trust-orchestration-split-check'] === `node --env-file-if-exists=.env ${VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_SCRIPT_PATH}` &&
      await repoFileExists(VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_PLAN_PATH) &&
      await repoFileExists(VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_APPROVAL_PATH) &&
      await repoFileExists(VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_HANDOFF_PATH) &&
      foundationSourceTrustVerifierSource.includes('evaluateFoundationSourceTrustVerifierOrchestration') &&
      foundationVerifyRootSource.includes('evaluateFoundationSourceTrustVerifierOrchestration({') &&
      foundationVerifyRootSource.includes('sourceTrustOrchestrationVerifier.checks') &&
      sourceTrustOrchestrationOldRootPatterns.every(pattern => !foundationVerifyRootSource.includes(pattern)) &&
      foundationVerifyLineCount < VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_BEFORE_LINES,
    'VERIFIER-SOURCE-TRUST-ORCHESTRATION-SPLIT-001 moves source-trust orchestration into the focused module',
    verifierSourceTrustOrchestrationCard
      ? `lane=${verifierSourceTrustOrchestrationCard.lane} dogfood=${sourceTrustDogfood.ok ? 'pass' : 'blocked'} sourceTrustChecks=${sourceTrustVerifier.summary.passed}/${sourceTrustVerifier.summary.total} lines=${VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_BEFORE_LINES}->${foundationVerifyLineCount}`
      : `missing ${VERIFIER_SOURCE_TRUST_ORCHESTRATION_SPLIT_CARD_ID}`,
  )

  return {
    checks,
    sourceTrustVerifier,
    dogfood: sourceTrustDogfood,
    phaseCApprovalFilesPresent,
  }
}
