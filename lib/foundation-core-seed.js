...
export function evaluateFoundationCoreSeedSplit({
  ...
  const seedConsumerSource = `${foundationDbSource}\n${schemaSeedSource}`
  ...
  addEvaluationCheck(
    checks,
    seedConsumerSource.includes('./foundation-core-seed.js') &&
      seedNames.every(name => seedConsumerSource.includes(name)),
    'schema/seed owner imports static seed arrays from the core seed module',
  ...
  addEvaluationCheck(
    checks,
    seedConsumerSource.includes('const includeBootstrapSeed = options?.includeBootstrapSeed === true') &&
      seedConsumerSource.includes('if (includeBootstrapSeed) {'),
