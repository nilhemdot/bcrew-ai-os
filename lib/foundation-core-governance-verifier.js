...
export const VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_CARD_ID = 'VERIFIER-CORE-GOVERNANCE-ORCHESTRATION-SPLIT-001'
export const VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_CLOSEOUT_KEY = 'verifier-core-governance-orchestration-split-v1'
export const VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_PLAN_PATH = 'docs/process/verifier-core-governance-orchestration-split-001-plan.md'
export const VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_APPROVAL_PATH = 'docs/process/approvals/VERIFIER-CORE-GOVERNANCE-ORCHESTRATION-SPLIT-001.json'
export const VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_SCRIPT_PATH = 'scripts/process-verifier-core-governance-orchestration-split-check.mjs'
export const VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_HANDOFF_PATH = 'docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-17-verifier-core-governance-orchestration-split-closeout.md'
export const VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_BEFORE_LINES = 6599
...
function hasReviewMarker(source = '') {
  return /Last reviewed:\s*20\d\d-\d\d-\d\d/.test(String(source || '')) && String(source || '').includes('Update Trigger')
}
...
export function evaluateFoundationCoreGovernanceVerifier(input = {}) {
  ...
  const foundationSharedCommsStoreSource = input.foundationSharedCommsStoreSource || ''
  ...
  const ownersGovernanceRoutesSource = input.ownersGovernanceRoutesSource || ''
  ...
  const sourceCrawlStoreOwnershipSource = input.sourceCrawlStoreOwnershipSource || ''
  ...
  addCheck(
    ...
    foundationHtmlSource.includes('<div class="found-nav-label">People and agents</div>') &&
      ...
      [usersDoc, steveDoc, agentModelDoc, harlanDoc, crewbertDoc, personalAgentOnboardingDoc].every(source => hasReviewMarker(source)) &&
  ...
  addCheck(
    ...
    sourceIdConstraintContractDogfood.ok === true &&
      sourceIdConstraintContractDogfood.valid?.auditedRelationCount === SOURCE_ID_CONSTRAINT_AUDIT_RELATIONS.length &&
      sourceIdConstraintContractDogfood.valid?.contractRelationCount === SOURCE_ID_CONSTRAINT_AUDIT_RELATIONS.length &&
  ...
  addCheck(
    checks,
      ...
      ].every(pattern => `${serverSource}\n${ownersGovernanceRoutesSource}`.includes(pattern)) &&
      [
...
function healthyInput(overrides = {}) {
  ...
  return {
    ...
    personalAgentOnboardingDoc: 'Last reviewed: 2026-05-18 Update Trigger AGENT-010 personal profile `ME.md` is only a working label daily nugget old BCrew-Buddy Harlan Pilot',
    ...
    foundationDbSource: 'assertBacklogDoneCloseout moving to done requires a closeout statusNote with build/change proof createBacklogItem updateBacklogItem CREATE TABLE IF NOT EXISTS source_crawl_target_runs',
    foundationSharedCommsStoreSource: "function ensureSharedCommunicationCandidateCanApply ['pending', 'approved'].includes(candidate.status) candidate.metadata.appliedTargetId FOR UPDATE",
    ...
    sourceIdConstraintContractDogfood: {
      ok: true,
      valid: {
        auditedRelationCount: SOURCE_ID_CONSTRAINT_AUDIT_RELATIONS.length,
        contractRelationCount: SOURCE_ID_CONSTRAINT_AUDIT_RELATIONS.length,
      },
      unsafeArrayFk: {
    ...
    llmRouterSource: '!dryRun && !plan.runnable No runnable LLM route available',
    sourceCrawlStoreOwnershipSource: 'AND lease_owner = $12 crawlRunId Source crawl target finish blocked `source_crawl_item:${targetKey}:${externalId}` entityId: item.itemKey',
    extractionTargetSource: 'runId: leasedTarget.crawlRunId',
    ...overrides,
  }
}
...
export function buildFoundationCoreGovernanceVerifierDogfoodProof() {
  ...
  const unsafeSharedCommsApply = evaluateFoundationCoreGovernanceVerifier(healthyInput({
    foundationSharedCommsStoreSource: 'function ensureSharedCommunicationCandidateCanApply',
  }))
  const runnableRouteBypass = evaluateFoundationCoreGovernanceVerifier(healthyInput({
    llmRouterSource: 'No runnable LLM route available',
  }))
  const unsafeSourceCrawlLedger = evaluateFoundationCoreGovernanceVerifier(healthyInput({
    sourceCrawlStoreOwnershipSource: 'crawlRunId Source crawl target finish blocked',
  }))
  ...
  return {
    ok: healthy.ok === true &&
      ...
      unsafeSharedCommsApply.ok === false &&
      runnableRouteBypass.ok === false &&
      unsafeSourceCrawlLedger.ok === false,
    ...
    rejectedCases: {
      ...
      unsafeSharedCommsApply: unsafeSharedCommsApply.ok === false,
      runnableRouteBypass: runnableRouteBypass.ok === false,
      unsafeSourceCrawlLedger: unsafeSourceCrawlLedger.ok === false,
    },
  }
}

export async function evaluateFoundationCoreGovernanceVerifierOrchestration(input = {}) {
  const {
  ...
  const checks = []
  const coreGovernanceVerifier = evaluateFoundationCoreGovernanceVerifier(input)
  checks.push(...coreGovernanceVerifier.checks)
  const coreGovernanceDogfood = buildFoundationCoreGovernanceVerifierDogfoodProof()
  ...
  const verifierCoreGovernanceSplitModuleCard =
    item(VERIFIER_CORE_GOVERNANCE_SPLIT_MODULE_CARD_ID) ||
    activeSprintItem(VERIFIER_CORE_GOVERNANCE_SPLIT_MODULE_CARD_ID)
  const verifierCoreGovernanceSplitModuleCloseout =
    foundationBuildCloseouts.find(closeout => closeout.key === VERIFIER_CORE_GOVERNANCE_SPLIT_MODULE_CLOSEOUT_KEY) || null
  ...
  const coreGovernanceOrchestrationOldRootPatterns = [
  ...
  addCheck(
    ...
    verifierCoreGovernanceSplitModuleCard &&
      ...
      (!verifierCoreGovernanceSplitModuleClosed || (
        ...
        await repoFileExists('docs/_archive/handoffs/2026-05-19-hot-doc-cleanup/2026-05-16-verifier-core-governance-split-module-closeout.md')
      )) &&
      coreGovernanceDogfood.ok === true &&
      ...
      foundationVerifyRootSource.includes('evaluateFoundationCoreGovernanceVerifierOrchestration({') &&
      foundationVerifyRootSource.includes('coreGovernanceOrchestrationVerifier.checks') &&
      coreGovernanceOldInlinePatterns.every(pattern => !pattern.test(foundationVerifyRootSource)) &&
      ...
      (activeFoundationSprint.sprint?.sprintId === VERIFIER_CORE_GOVERNANCE_SPLIT_MODULE_SPRINT_ID ||
        verifierCoreGovernanceSplitModuleClosed) &&
    ...
    verifierCoreGovernanceSplitModuleCard
      ? `lane=${verifierCoreGovernanceSplitModuleCard.lane} dogfood=${coreGovernanceDogfood.ok ? 'pass' : 'blocked'} coreChecks=${coreGovernanceVerifier.summary.passed}/${coreGovernanceVerifier.summary.total} lines=${VERIFIER_CORE_GOVERNANCE_SPLIT_MODULE_BEFORE_LINES}->${foundationVerifyLineCount}`
      : `missing ${VERIFIER_CORE_GOVERNANCE_SPLIT_MODULE_CARD_ID}`,
  )

  const verifierCoreGovernanceOrchestrationCard =
    item(VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_CARD_ID) ||
    activeSprintItem(VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_CARD_ID)
  const verifierCoreGovernanceOrchestrationCloseout =
    foundationBuildCloseouts.find(closeout => closeout.key === VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_CLOSEOUT_KEY) || null
  addCheck(
    checks,
    verifierCoreGovernanceOrchestrationCard &&
      ['executing', 'done'].includes(verifierCoreGovernanceOrchestrationCard.lane) &&
      String(verifierCoreGovernanceOrchestrationCard.statusNote || '').includes(VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_CLOSEOUT_KEY) &&
      verifierCoreGovernanceOrchestrationCloseout?.operatorCloseout === true &&
      (verifierCoreGovernanceOrchestrationCloseout.backlogIds || []).includes(VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_CARD_ID) &&
      coreGovernanceDogfood.ok === true &&
      coreGovernanceVerifier.summary.passed === coreGovernanceVerifier.summary.total &&
      packageJson.scripts?.['process:verifier-core-governance-orchestration-split-check'] === `node --env-file-if-exists=.env ${VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_SCRIPT_PATH}` &&
      await repoFileExists(VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_PLAN_PATH) &&
      await repoFileExists(VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_APPROVAL_PATH) &&
      await repoFileExists(VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_HANDOFF_PATH) &&
      foundationCoreGovernanceVerifierSource.includes('evaluateFoundationCoreGovernanceVerifierOrchestration') &&
      foundationVerifyRootSource.includes('evaluateFoundationCoreGovernanceVerifierOrchestration({') &&
      foundationVerifyRootSource.includes('coreGovernanceOrchestrationVerifier.checks') &&
      coreGovernanceOrchestrationOldRootPatterns.every(pattern => !foundationVerifyRootSource.includes(pattern)) &&
      foundationVerifyLineCount < VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_BEFORE_LINES,
    'VERIFIER-CORE-GOVERNANCE-ORCHESTRATION-SPLIT-001 moves core governance orchestration into the focused module',
    verifierCoreGovernanceOrchestrationCard
      ? `lane=${verifierCoreGovernanceOrchestrationCard.lane} dogfood=${coreGovernanceDogfood.ok ? 'pass' : 'blocked'} coreChecks=${coreGovernanceVerifier.summary.passed}/${coreGovernanceVerifier.summary.total} lines=${VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_BEFORE_LINES}->${foundationVerifyLineCount}`
      : `missing ${VERIFIER_CORE_GOVERNANCE_ORCHESTRATION_SPLIT_CARD_ID}`,
  )

  return {
    checks,
    coreGovernanceVerifier,
    dogfood: coreGovernanceDogfood,
  }
}
