...
export async function evaluateFoundationIntelligenceAuditVerifier(input = {}) {
  ...
  const nightlyDeepAuditJob = input.nightlyDeepAuditJob || getFoundationJobDefinitions().find(job => job.key === NIGHTLY_DEEP_AUDIT_JOB_KEY) || null
  ...
  addCheck(
    ...
    nightlyDeepAuditCard
      ? `lane=${nightlyDeepAuditCard.lane} findings=${nightlyDeepAudit.deterministicAudit?.summary?.findingCount || 0} targets=${nightlyDeepAudit.reviewTargets?.length || 0} job=${nightlyDeepAuditJob?.runtimeMode || 'missing'} scheduleDogfood=${nightlyAuditScheduleDogfood.ok === true}`
...
function syntheticBaseInput(overrides = {}) {
  ...
  return {
    ...
    foundationHub: { backlogItems, implementationIntelligence, buildIntelExtraction, gstackBuildIntel },
    backlogItems,
    ...
    codeQualityNightlySyntheticProof: { ok: true },
    nightlyDeepAudit,
    nightlyDeepAuditJob: { key: NIGHTLY_DEEP_AUDIT_JOB_KEY, runtimeMode: 'scheduled', mutationPosture: 'report_only' },
    nightlyDeepAuditDogfood: { ok: true },
    nightlyAuditScheduleDogfood: { ok: true },
    ...overrides,
  }
...
export async function buildFoundationIntelligenceAuditVerifierDogfoodProof() {
  ...
  const rejected = [implementationMutation, buildIntelPaidAuth, gstackImport, auditAutoFix, nightlyWrite, nightlyScheduleDogfoodMissing]
