...
import {
  buildBuildIntelSnapshotBaseline,
...
export async function evaluateFoundationIntelligenceAuditVerifier(input = {}) {
  ...
  addCheck(
    ...
    gstackBuildIntelCards.every(card => card?.lane === 'done' && String(card?.statusNote || '').includes(GSTACK_BUILD_INTEL_CLOSEOUT_KEY)) &&
      ...
      gstackBuildIntel.snapshotBaseline?.expectedSnapshotCommit === GSTACK_BUILD_INTEL_EXPECTED_COMMIT &&
      gstackBuildIntel.snapshotBaseline?.inspectedCommit === gstackBuildIntel.sourceCommit &&
      isBuildIntelSnapshotBaselineEvidence(gstackBuildIntel.snapshotBaseline) &&
      gstackBuildIntel.sourcePosture === 'inspected_snapshot' &&
  ...
  const codeQualityEndpointIds = Array.isArray(codeQualityNightlyAudit.endpointMetrics)
    ...
    : []
  const codeQualityReconciliationSummary = codeQualityNightlyAudit.findingReconciliation?.summary || {}
  const codeQualityRawFindingCount = Number(
    codeQualityNightlyAudit.summary?.rawFindingCount ??
    codeQualityReconciliationSummary.rawFindingCount ??
    codeQualityNightlyAudit.rawFindings?.length ??
    codeQualityNightlyAudit.summary?.findingCount ??
    0,
  )
  const codeQualityActiveFindingCount = Number(
    codeQualityReconciliationSummary.activeFindingCount ??
    codeQualityNightlyAudit.summary?.findingCount ??
    codeQualityNightlyAudit.findings?.length ??
    0,
  )
  const codeQualityReconciledClosedFindingCount = Number(
    codeQualityNightlyAudit.summary?.reconciledClosedFindingCount ??
    codeQualityReconciliationSummary.reconciledClosedFindingCount ??
    codeQualityNightlyAudit.findingReconciliation?.reconciledFindings?.length ??
    0,
  )
  addCheck(
    checks,
    codeQualityNightlyAuditCards.every(card => card?.lane === 'done' && String(card?.statusNote || '').includes(CODE_QUALITY_NIGHTLY_AUDIT_CLOSEOUT_KEY)) &&
      ...
      codeQualityRawFindingCount >= CODE_QUALITY_NIGHTLY_AUDIT_MIN_FINDING_COUNT &&
      codeQualityActiveFindingCount + codeQualityReconciledClosedFindingCount === codeQualityRawFindingCount &&
      codeQualityNightlyAudit.proposedCards?.length === Number(codeQualityNightlyAudit.summary?.proposedCardCount || 0) &&
    ...
    `cards=${codeQualityNightlyAuditCards.filter(card => card?.lane === 'done').length}/${CODE_QUALITY_NIGHTLY_AUDIT_CARD_IDS.length} active=${codeQualityActiveFindingCount} raw=${codeQualityRawFindingCount} reconciled=${codeQualityReconciledClosedFindingCount} proposed=${codeQualityNightlyAudit.proposedCards?.length || 0}`,
  )
  ...
  const nightlyDeepAuditHasBackendFoundationTarget = nightlyDeepAudit.reviewTargets?.some(target => [
    'lib/foundation-db.js',
    'lib/foundation-db-core.js',
  ...
  addCheck(
    ...
    nightlyDeepAuditCard?.lane === 'done' &&
      ...
      nightlyDeepAuditHasBackendFoundationTarget &&
...
function syntheticBaseInput(overrides = {}) {
  ...
  const gstackBuildIntel = {
    ...
    sourcePosture: 'inspected_snapshot',
    snapshotBaseline: buildBuildIntelSnapshotBaseline({
      sourceId: 'SRC-GITHUB-BUILD-INTEL-001',
      inspectedCommit: GSTACK_BUILD_INTEL_EXPECTED_COMMIT,
      expectedSnapshotCommit: GSTACK_BUILD_INTEL_EXPECTED_COMMIT,
      evidencePath: GSTACK_BUILD_INTEL_REPORT_PATH,
    }),
    patternScorecard: [
  ...
  const codeQualityNightlyAudit = {
    ...
    rawFindings: Array.from({ length: 12 }, (_, index) => ({ id: `raw-detector-signal-${index + 1}` })),
    findings: [],
    summary: {
      findingCount: 0,
      rawFindingCount: 12,
      reconciledClosedFindingCount: 12,
      proposedCardCount: 0,
    },
    findingReconciliation: {
      ...
      activeFindings: [],
      reconciledFindings: Array.from({ length: 12 }, (_, index) => ({ id: `raw-detector-signal-${index + 1}` })),
    },
    proposedCards: [],
  }
...
export async function buildFoundationIntelligenceAuditVerifierDogfoodProof() {
  ...
  const auditMissingClosedSignalReconciliation = await evaluateFoundationIntelligenceAuditVerifier(mutateBase(input => {
    input.codeQualityNightlyAudit = clone(input.codeQualityNightlyAudit)
    input.codeQualityNightlyAudit.summary.reconciledClosedFindingCount = 0
    input.codeQualityNightlyAudit.findingReconciliation.summary.reconciledClosedFindingCount = 0
    input.codeQualityNightlyAudit.findingReconciliation.reconciledFindings = []
  }))
  ...
  return {
    ...
    rejectedCases: {
      ...
      auditMissingClosedSignalReconciliation: auditMissingClosedSignalReconciliation.ok === false,
