...
function buildFastRecipientPlan(candidate, oversightReference = {}) {
  const dedupedRoles = roleSafeTargetLabel(candidate) === 'Georgia'
    ? ['Georgia']
    : []
...
function candidateMatchesAllowlist(candidate, allowlist = []) {
  return (Array.isArray(allowlist) ? allowlist : []).some(item => {
    ...
    const labelMatches = label && roleSafeTargetLabel(candidate).toLowerCase() === label
...
export function buildAgentFeedbackAutoSendGuardMatrix(candidate) {
  const testOnlyApproval = {
    ...
    allowlist: [{ targetName: 'Georgia', milestoneDay: 30 }],
...
export async function buildAgentFeedbackAutoSendStatus({
  ...
  addFinding(findings, currentPlan.includes('AGENT-FEEDBACK-AUTO-SEND-001` is done for readiness') && currentPlan.includes('test-only Georgia/day-30 live-send mode'), 'current plan records readiness ship and next decision', 'current-plan')
  addFinding(findings, currentState.includes('AGENT-FEEDBACK-AUTO-SEND-001` is done for readiness') && currentState.includes('test-only Georgia/day-30 live-send mode'), 'current state records readiness ship and next decision', 'current-state')
