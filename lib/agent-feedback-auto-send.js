...
export const AGENT_FEEDBACK_AUTO_SEND_READINESS_PROOF_PATH = 'docs/_archive/audits/2026-05-01-agent-feedback-auto-send-readiness.md'
...
export const AGENT_FEEDBACK_PRODUCTION_AUTOSEND_ENABLE_PROOF_PATH = 'docs/_archive/audits/2026-05-01-agent-feedback-production-autosend-enable-proof.md'
...
function summarizeAutoSendJobRuntime(foundationJobs = null) {
  ...
  return {
    ...
    lastRunHealthStatus: reconciliation.reconciled ? 'reconciled' : latestRun?.status || null,
...
export async function buildAgentFeedbackAutoSendStatus({
  ...
  const productionEnablementAccepted = status.summary?.productionAutoSendEnabled === true &&
    status.summary?.liveGuardDecision === 'live_send_allowed' &&
  ...
  const statusHasGovernedAction = ['would_send', 'sent', 'repair', 'skipped'].includes(status.summary?.georgiaDay30Action) ||
    productionEnablementAccepted
  const hubHasGovernedAction = ['would_send', 'sent', 'repair', 'skipped'].includes(hubStatus?.summary?.georgiaDay30Action) ||
    productionEnablementAccepted
  const opsHasGovernedAction = ['would_send', 'sent', 'repair', 'skipped'].includes(opsStatus?.summary?.georgiaDay30Action) ||
    productionEnablementAccepted
  ...
  addFinding(findings, includesAll(routeSource, ['buildAgentFeedbackAutoSendReadiness', 'agentFeedbackAutoSend']), 'Foundation and Ops APIs expose auto-send readiness status', 'server.js + lib/hub-read-routes.js')
  addFinding(findings, verifierCoverageSource.includes(AGENT_FEEDBACK_AUTO_SEND_CARD_ID), 'foundation verifier covers auto-send card', AGENT_FEEDBACK_AUTO_SEND_CARD_ID)
  ...
  addFinding(findings, statusHasGovernedAction, 'Georgia Day-30 has a governed production action or production enablement is current', status.summary?.georgiaDay30Action || 'production-enabled')
  ...
  addFinding(findings, status.summary?.bothKeysRequired === true || productionEnablementAccepted, 'both toggle and approved allowlist are required for test-only live send', productionEnablementAccepted ? 'production-all approval active; test-only target not applicable' : 'synthetic guard only, no send executed')
  ...
  addFinding(findings, hubHasGovernedAction, 'live Foundation Hub exposes Georgia production action or current production enablement', hubStatus?.summary?.georgiaDay30Action || 'production-enabled')
  addFinding(findings, opsHasGovernedAction, 'live Ops Hub exposes Georgia production action or current production enablement', opsStatus?.summary?.georgiaDay30Action || 'production-enabled')
