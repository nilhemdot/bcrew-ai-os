...
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_CARD_ID = 'SOURCE-MATURITY-FREEDOM-BHAG-ATOM-FLOW-REPAIR-001'
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_CLOSEOUT_KEY = 'source-maturity-freedom-bhag-atom-flow-repair-v1'
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_PLAN_PATH = 'docs/process/source-maturity-freedom-bhag-atom-flow-repair-001-plan.md'
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_APPROVAL_PATH = 'docs/process/approvals/SOURCE-MATURITY-FREEDOM-BHAG-ATOM-FLOW-REPAIR-001.json'
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_CLOSEOUT_PATH = 'docs/_archive/handoffs/2026-05-29-hot-doc-refresh/2026-05-18-source-maturity-freedom-bhag-atom-flow-repair-closeout.md'
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_SCRIPT_PATH = 'scripts/process-source-maturity-freedom-bhag-atom-flow-repair-check.mjs'
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_SPRINT_ID = 'source-maturity-freedom-bhag-atom-flow-repair-2026-05-18'
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID = 'SRC-FREEDOM-BHAG-001'
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_FACT_ID = 'fact:39533f268562a6b6c6533952'

export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES = [
...
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_PROOF_COMMANDS = [
...
export const SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_CHANGED_FILES = [
...
function targetFact(facts = []) {
  return list(facts).find(fact => (fact.factId || fact.fact_id) === SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_FACT_ID) || null
}

export function selectFreedomBhagAtomFlowRepairCandidate({
  beforeRow = {},
  sourceFacts = [],
  sourceId = SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
} = {}) {
  const failures = []
...
export function buildFreedomBhagAtomFromFact(candidate = {}, now = new Date().toISOString()) {
  const sourceId = candidate.sourceId || SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID
  ...
  const naturalKey = `source-maturity-freedom-bhag-atom-flow-repair:${sourceId}:${factId}`
  ...
  const atom = {
    ...
    evidenceExcerpt,
    derivedClaim: claim,
    topicRefs: ['source-maturity', 'freedom-bhag', 'community-goal', 'source-backed-goal-truth'],
    ...
    acceptedAt: now,
    acceptedBy: 'codex-source-maturity-freedom-bhag-atom-flow-repair',
    reviewNote: 'Promoted from an existing governed Freedom BHAG source fact only; no live extraction, provider call, Sheets read/write, or external write occurred.',
    tags: ['source-maturity', 'atom-flow-repair', 'freedom-bhag', 'no-live-extraction'],
    metadata: {
...
export function evaluateFreedomBhagAtomFlowRepair({
  beforeRow = {},
  afterRow = {},
  candidate = {},
  atomBuild = {},
  ...
  return {
    ...
    failures,
    summary: {
      sourceId: candidate.sourceId || beforeRow.sourceId || afterRow.sourceId || SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
...
export function buildSyntheticFreedomBhagAtomFlowRepairProof() {
  const fact = {
    factId: SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_FACT_ID,
    sourceId: SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID,
    ...
    sourceRef: 'docs/strategy/bhag-model.md',
    minTier: 1,
    sensitivity: 'neutral',
    status: 'active',
  }
  ...
  const candidate = selectFreedomBhagAtomFlowRepairCandidate({ beforeRow, sourceFacts: [fact] })
  const atomBuild = buildFreedomBhagAtomFromFact(candidate, '2026-05-18T05:50:00.000Z')
  const evaluation = evaluateFreedomBhagAtomFlowRepair({
    beforeRow,
    afterRow,
    candidate,
    atomBuild,
  ...
  const missingFact = selectFreedomBhagAtomFlowRepairCandidate({ beforeRow, sourceFacts: [] })
  const tierBlocked = selectFreedomBhagAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, minTier: 4 }] })
  const noEvidenceRef = selectFreedomBhagAtomFlowRepairCandidate({ beforeRow, sourceFacts: [{ ...fact, sourceRef: null, metadata: {} }] })
  return {
    ok: candidate.ok &&
      atomBuild.ok &&
      evaluation.ok &&
...
export function renderFreedomBhagAtomFlowRepairCloseout(snapshot = {}) {
  const summary = snapshot.summary || {}
  const lines = []
  lines.push('# Source Maturity Freedom BHAG Atom Flow Repair Closeout')
  lines.push('')
  lines.push(`Card: \`${SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_CARD_ID}\``)
  lines.push(`Closeout key: \`${SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_CLOSEOUT_KEY}\``)
  lines.push('')
  lines.push('## What Shipped')
  lines.push('')
  lines.push(`- Promoted one source-backed atom for \`${summary.sourceId || SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_SOURCE_ID}\` from an existing Freedom BHAG source fact.`)
  lines.push(`- Saved atom: \`${summary.atomId || 'pending atom'}\`.`)
  lines.push(`- Source fact evidence: \`${summary.sourceFactId || SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_TARGET_FACT_ID}\`.`)
  ...
  lines.push('')
  for (const boundary of SOURCE_MATURITY_FREEDOM_BHAG_ATOM_FLOW_REPAIR_NOT_NEXT_BOUNDARIES) lines.push(`- ${boundary}`)
  lines.push('')
  lines.push('## Next')
  lines.push('')
